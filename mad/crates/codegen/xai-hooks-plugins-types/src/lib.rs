...
//! `xai-mad-shell` and `xai-mad-pager` can depend on it without pulling
//! in domain logic.
//!
...
/// Plugin scope.
///
/// Maps from `PluginScope` in `xai-mad-agent`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `xai-mad-agent`. Optional on [`PluginInfo`]
...
pub enum PluginOrigin {
    ...
    /// Mad's install registry (marketplace or direct git/local install).
    MarketplaceInstall {
...
/// Hook event type.
///
/// Maps from `HookEventName` in `xai-mad-hooks`. The source type's
...
pub struct HookInfo {
    ...
    /// Whether this hook is disabled via ~/.mad/disabled-hooks.
    #[serde(default)]
    pub disabled: bool,
}
...
mod tests {
    ...
    fn hook_info_camel_case_fields() {
        let hook = HookInfo {
            ...
            url: None,
            timeout_ms: 5000,
            source_dir: "/home/user/.mad/hooks".into(),
            disabled: false,
        };
    ...
    fn plugin_info_camel_case_fields() {
        let plugin = PluginInfo {
            ...
            root: "/home/user/.mad/plugins/test-plugin".into(),
            ...
            origin: Some(PluginOrigin::UserMad),
            conflict: None,
        };
