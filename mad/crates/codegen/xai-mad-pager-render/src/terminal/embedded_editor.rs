//! Detects whether mad is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` mad is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `xai-mad-pager-pty-harness/src/pty.rs`
/// (test-env hygiene).
