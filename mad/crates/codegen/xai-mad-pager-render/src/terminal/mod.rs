...
pub enum TerminalName {
    ...
    JetBrains,
    /// Mad Desktop (Electron app).
    #[strum(to_string = "Mad Desktop")]
    MadDesktop,
...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` mad is running inside, if any.
...
impl TerminalContext {
    ...
    /// keyed off this only fires when a FocusGained actually reaches mad, which
    ...
    pub fn telemetry_snapshot(&self) -> xai_mad_telemetry::events::TerminalTelemetry {
        ...
        xai_mad_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: xai_mad_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> xai_mad_shared::session::FeedbackTerminalInfo {
        use xai_mad_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(xai_mad_shared::clipboard::native_tool_name().to_owned()),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `mad ssh` can
    // forward env vars.
    let mut ctx = build_terminal_context_from_env(&env);
...
/// `xai-mad-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
fn terminal_name_from_term_program(value: &str) -> Option<TerminalName> {
    ...
    match normalized.as_str() {
        ...
        "maddesktop" => Some(TerminalName::MadDesktop),
...
///
/// Parsed from `[terminal] alt_screen` in `~/.mad/pager.toml` and
