...
//! Dark themes (MadNight, TokyoNight) share `mad-night.tmTheme`;
//! MadDay uses `mad-day.tmTheme` with deepened colors for light backgrounds.
//!
...
//! `current_kind()` is a nominal `MadNight` (so leftover kind-keyed paths
...
pub use xai_mad_markdown::Syntect;
...
static SYNTECT_MADNIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_TOKYONIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_MADDAY: OnceLock<Syntect> = OnceLock::new();
...
/// reports a nominal `MadNight`, so this returns the night theme. Token
...
pub fn get_syntect() -> &'static Syntect {
    match crate::theme::Theme::current_kind() {
        ...
        | ThemeKind::Auto => SYNTECT_MADNIGHT
            .get_or_init(|| Syntect::new(include_bytes!("../assets/mad-night.tmTheme"))),
        ThemeKind::TokyoNight => SYNTECT_TOKYONIGHT
            .get_or_init(|| Syntect::new(include_bytes!("../assets/tokyo-night.tmTheme"))),
        ThemeKind::MadDay => SYNTECT_MADDAY
            .get_or_init(|| Syntect::new(include_bytes!("../assets/mad-day.tmTheme"))),
    }
}
