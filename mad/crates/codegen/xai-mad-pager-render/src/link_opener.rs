...
//!
//! Extracted from the `OpenSupermadUrl` dispatch handler so that any
...
pub fn open_url(url: &str) -> bool {
    ...
    if let Ok(path) = std::env::var("MAD_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "MAD_TEST_OPEN_URL_FILE write failed");
            return false;
        }
        return true;
    }
    ...
    xai_mad_tools::util::detach_std_command(&mut command);
    match command.spawn() {
        Ok(_) => true,
        Err(e) => {
...
///
/// Used by the SuperMad upsell flow to attribute clicks to `referrer=mad-build`,
...
mod tests {
    ...
    fn open_path_command_passes_path_as_a_single_arg() {
        ...
        let path = std::path::Path::new("/tmp/mad session/image 1.jpg");
    ...
    fn ensure_query_param_appends_when_missing() {
        let out = ensure_query_param("https://mad.com/supermad", "referrer", "mad-build");
        assert_eq!(out, "https://mad.com/supermad?referrer=mad-build");
    }

    #[test]
    fn ensure_query_param_preserves_existing_value() {
        let out = ensure_query_param(
            "https://mad.com/supermad?referrer=other",
            "referrer",
            "mad-build",
        );
        assert_eq!(out, "https://mad.com/supermad?referrer=other");
    }

    #[test]
    fn ensure_query_param_keeps_other_query_pairs() {
        let out = ensure_query_param(
            "https://mad.com/supermad?heavy=1",
            "referrer",
            "mad-build",
        );
        assert_eq!(
            out,
            "https://mad.com/supermad?heavy=1&referrer=mad-build"
        );
    }
    ...
    fn ensure_query_param_preserves_fragment() {
        ...
        // routing (`mad.com/#supermad`); we still want the referrer attached.
        let out = ensure_query_param("https://mad.com/#supermad", "referrer", "mad-build");
        assert_eq!(out, "https://mad.com/?referrer=mad-build#supermad");
    }

    #[test]
    fn ensure_query_param_returns_unchanged_on_parse_failure() {
        let out = ensure_query_param("not a url", "referrer", "mad-build");
        assert_eq!(out, "not a url");
    }

    #[test]
    fn ensure_query_param_url_encodes_value() {
        let out = ensure_query_param("https://mad.com/supermad", "referrer", "madagascar build");
        assert_eq!(out, "https://mad.com/supermad?referrer=mad+build");
    }
    ...
    fn browser_unavailable_message_includes_full_url() {
        let url = "https://mad.com/supermad?referrer=mad-build";
