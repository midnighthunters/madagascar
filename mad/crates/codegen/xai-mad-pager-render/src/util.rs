...
pub use xai_mad_config::mad_home;

/// Path to `$MAD_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    mad_home().join("pager.toml")
}

/// User-facing label for the user mad directory (``~/.mad`` or ``$MAD_HOME``).
///
/// Derived from resolved [`mad_home()`] vs `xai_mad_config::default_mad_home()`,
/// not from whether `MAD_HOME` is set in the environment.
pub fn display_mad_home_prefix() -> String {
    display_mad_home_prefix_for(&mad_home())
}

fn display_mad_home_prefix_for(home: &Path) -> String {
    if home == xai_mad_config::default_mad_home() {
        "~/.mad".to_string()
    } else {
        "$MAD_HOME".to_string()
    }
...
/// User-facing path under [`mad_home()`], e.g. ``~/.mad/config.toml``.
pub fn display_user_mad_path(relative: impl AsRef<Path>) -> String {
    display_user_mad_path_for(&mad_home(), relative)
}

fn display_user_mad_path_for(home: &Path, relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_mad_home_prefix_for(home);
...
/// Abbreviate an absolute path for display: prefer [`mad_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let mad = mad_home();
    if let Ok(rest) = path_buf.strip_prefix(&mad) {
        let prefix = display_mad_home_prefix();
...
/// True when `path` is under user [`mad_home()`] (not project `{cwd}/.mad`).
pub fn is_under_user_mad_home(path: &Path) -> bool {
    path.starts_with(mad_home())
}
...
mod tests {
    ...
    #[test]
    fn display_mad_home_prefix_default_install() {
        if std::env::var("MAD_HOME").is_ok() {
            return;
        }
        assert_eq!(display_mad_home_prefix(), "~/.mad");
    }

    #[test]
    fn display_user_mad_path_joins_relative() {
        let path = display_user_mad_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".mad") || path.contains("$MAD_HOME"));
    }

    #[test]
    fn display_user_mad_path_for_custom_home_uses_override_label() {
        let custom = std::env::temp_dir().join("mad-home-display-regression");
        assert_eq!(
            display_user_mad_path_for(&custom, "config.toml"),
            "$MAD_HOME/config.toml"
        );
        assert_eq!(
            display_user_mad_path_for(&custom, "sandbox.toml"),
            "$MAD_HOME/sandbox.toml"
        );
    }

    #[test]
    fn abbreviate_path_uses_home_when_under_default_mad() {
        if let Ok(home) = std::env::var("HOME") {
            if home.is_empty() {
                return;
            }
            let full = format!("{home}/.mad/memory/MEMORY.md");
