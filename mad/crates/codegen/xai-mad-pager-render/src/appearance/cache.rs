...
//!
//! Disk writes live in `xai_mad_shell::util::config::set_<field>()`.
...
use xai_mad_shared::ui_config::UiConfig;
...
/// Default ON when unset. The `MAD_PROMPT_SUGGESTIONS` env var overrides
...
/// `MAD_SCROLL_MODE` overrides `[ui].scroll_mode` (the `MAD_SCROLL_SPEED`
...
pub fn load_scroll_mode() -> ScrollMode {
    SCROLL_MODE_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("MAD_SCROLL_MODE")
                .ok()
...
/// `MAD_INVERT_SCROLL` (`1`/`true`/`0`/`false`) overrides `[ui].invert_scroll`.
pub fn load_invert_scroll() -> bool {
    INVERT_SCROLL_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("MAD_INVERT_SCROLL")
                .ok()
...
/// its own lines-per-tick. `MAD_SCROLL_LINES` overrides `[ui].scroll_lines`.
pub fn load_scroll_lines() -> Option<u8> {
    SCROLL_LINES_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("MAD_SCROLL_LINES")
                .ok()
...
fn load_bool_from_effective_config(key: &str, default: bool) -> bool {
    let root = match xai_mad_config::load_effective_config_disk_only() {
        Ok(r) => r,
