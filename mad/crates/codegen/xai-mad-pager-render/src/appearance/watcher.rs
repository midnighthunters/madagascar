...
//!
//! In dev mode, watches ~/.mad/pager.toml for changes and hot-reloads.
...
///
/// In dev mode: reads from ~/.mad/pager.toml, watches for changes.
...
impl ConfigWatcher {
    ...
    ///
    /// - In dev mode: reads/creates ~/.mad/pager.toml, watches for changes
    ...
    fn start_static() -> io::Result<Self> {
        let config = xai_mad_config::user_mad_home()
