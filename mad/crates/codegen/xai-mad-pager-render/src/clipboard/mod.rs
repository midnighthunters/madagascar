...
/// Documented in `xai-mad-pager/docs/internal/22-environment-variables.md`.
pub const MAD_COPY_FILE_ENV: &str = "MAD_COPY_FILE";

/// Cached result of the remote-session check (env vars don't change at runtime).
fn is_remote() -> bool {
    static REMOTE: OnceLock<bool> = OnceLock::new();
    *REMOTE.get_or_init(xai_mad_shared::clipboard::is_remote_session)
}
...
fn is_container_no_display() -> bool {
    ...
    *CONTAINER.get_or_init(xai_mad_shared::clipboard::is_containerized_without_display)
}

/// Cached result of the "an upstream OSC 52 sink is capturing our output" check.
///
/// `mad wrap` runs a command inside a local PTY, scans its output for OSC 52
/// clipboard sequences, and writes their payload to the *real* (local) system
/// clipboard (see `xai-mad-pager`'s `pty_wrap` module). It advertises this to
/// the wrapped program via an environment variable so the
/// inner `mad` knows its OSC 52 writes are reliably intercepted and copied,
...
///
/// Two names are accepted: the canonical `MAD_OSC52_SINK` (inherited by local
/// children) and the `LC_`-prefixed `LC_MAD_OSC52_SINK`, which the default
/// OpenSSH client/server configs forward (`SendEnv LANG LC_*` /
/// `AcceptEnv LANG LC_*`) so the signal survives the hop into a remote `mad`.
pub fn osc52_sink_active() -> bool {
    static SINK: OnceLock<bool> = OnceLock::new();
    *SINK.get_or_init(|| {
        std::env::var_os("MAD_OSC52_SINK").is_some()
            || std::env::var_os("LC_MAD_OSC52_SINK").is_some()
    })
}

/// Kill switch: never emit OSC 52 clipboard sequences.
///
/// Set `MAD_CLIPBOARD_NO_OSC52` (any value) before starting Mad. Presence
/// forces the OSC 52 leg off for the whole process — including Linux "always
/// emit", tmux, SSH, container, and `MAD_OSC52_SINK` paths. Use this when the
/// host terminal paints OSC 52 payloads as visible garbage (e.g. OpenText
/// Exceed and other non-supporting emulators).
///
/// Same convention as `MAD_CLIPBOARD_NO_DATA_CONTROL`: env presence enables
/// the kill switch; resolved once and cached for the process lifetime.
pub fn osc52_disabled() -> bool {
    static DISABLED: OnceLock<bool> = OnceLock::new();
    *DISABLED.get_or_init(|| std::env::var_os("MAD_CLIPBOARD_NO_OSC52").is_some())
}
...
pub fn wayland_data_control_label() -> &'static str {
    match crate::host::DisplayServer::current() {
        crate::host::DisplayServer::Wayland => {
            if xai_mad_shared::clipboard::wayland_data_control_supported() {
                "yes"
            } else {
                "no"
            }
...
/// matters for non-tmux contexts — unless `MAD_CLIPBOARD_NO_OSC52` is set,
...
fn resolve_clipboard_route_with(ctx: &TerminalContext, no_osc52: bool) -> ClipboardRoute {
    ...
    // upstream `mad wrap` sink is capturing our output and will forward
    // the sequence to the real clipboard.
    // `MAD_CLIPBOARD_NO_OSC52` wins over every automatic path.
    let osc52 = !no_osc52
        && (cfg!(target_os = "linux")
            || is_tmux
            || is_remote()
...
fn write_tmux_buffer(text: &str) -> bool {
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let stdin = xai_mad_shared::clipboard::spool_for_stdin(text.as_bytes())?;
        ...
        let status = xai_mad_shared::clipboard::wait_with_deadline(
            &mut child,
...
///
/// Delegates to [`xai_mad_shared::clipboard`] which uses
...
impl ClipboardProvider for SystemClipboard {
    fn get(&mut self) -> Option<String> {
        xai_mad_shared::clipboard::get_text().ok().flatten()
    }
...
fn clipboard_write_with_route(text: &str, route: &ClipboardRoute) -> ClipboardWriteLegs {
    ...
    if route.native {
        let outcome = xai_mad_shared::clipboard::set_text_with_outcome(text);
    ...
    if route.osc52 {
        match xai_mad_shared::clipboard::set_text_osc52(text, route.osc52_tmux_passthrough) {
...
impl ClipboardFeedback {
    ...
    fn message(self) -> &'static str {
        match self {
            ...
            Self::UnverifiedOscRemote | Self::UnverifiedOscContainer => {
                "Copy sent. If paste fails, use mad wrap or /minimal."
            }
...
/// Write text and return a toast; emits `mad-shell-clipboard_copy` when enabled.
...
///
/// Override with [`MAD_COPY_FILE_ENV`] (supports `~`). Otherwise
/// `~/.mad/last-copy.txt` (mad's per-user home — short, stable, and
/// readable in a toast, unlike macOS's `/var/folders/...` temp dir).
///
/// `None` when no mad home resolves and the env var is unset: rather than
...
pub fn default_copy_fallback_path() -> Option<std::path::PathBuf> {
    if let Ok(raw) = std::env::var(MAD_COPY_FILE_ENV) {
    ...
    xai_mad_config::user_mad_home().map(|mad_home| mad_home.join("last-copy.txt"))
}

/// Render a backup-file path for user-facing messages using the codebase-wide
/// abbreviation convention ([`crate::util::abbreviate_path`]): a mad-home
/// prefix collapses to `~/.mad` (or `$MAD_HOME` when overridden), and a
...
/// mad) is tightened via `set_permissions` since the create-time `mode`
...
/// `MAD_COPY_FILE`) — the backup file is skipped rather than written to a
/// predictable temp location.
///
/// On Unix a missing parent directory is created `0700` (a custom
/// `MAD_COPY_FILE` may point at a not-yet-created private directory;
/// `~/.mad` normally already exists).
pub fn write_copy_fallback(text: &str) -> std::io::Result<std::path::PathBuf> {
    let Some(path) = default_copy_fallback_path() else {
        return Err(std::io::Error::new(
            ...
            "no home directory resolves; set MAD_COPY_FILE to enable the copy backup file",
        ));
    };
    #[cfg(unix)]
...
/// clipboard over SSH (notably Apple Terminal without `mad wrap`); a failed
...
fn log_clipboard_copy_event(
    ...
    if !xai_mad_telemetry::client::is_enabled() {
        return;
    }
    xai_mad_telemetry::session_ctx::log_event(xai_mad_telemetry::events::ClipboardCopy {
...
pub fn system_clipboard_read_text() -> Result<Option<String>, ClipboardTextReadError> {
    ...
    xai_mad_shared::clipboard::get_text().map_err(|error| {
...
pub fn system_primary_selection_get() -> Option<String> {
    ...
    if !xai_mad_shared::clipboard::x11_display_env_present() {
        return None;
    }
    xai_mad_shared::clipboard::get_primary_text()
        .ok()
        .flatten()
        .filter(|text| !text.is_empty())
}
...
pub fn x11_primary_guidance_available() -> bool {
    #[cfg(target_os = "linux")]
    {
        is_native_x11(crate::host::DisplayServer::current())
            && xai_mad_shared::clipboard::x11_display_env_present()
    }
...
pub fn log_paste_key_empty_host_clipboard(surface: &str) {
    ...
    xai_mad_telemetry::session_ctx::log_event(
        xai_mad_telemetry::events::PasteKeyEmptyHostClipboard {
            terminal,
...
fn log_clipboard_paste_event(
    ...
    xai_mad_telemetry::session_ctx::log_event(xai_mad_telemetry::events::ClipboardImagePaste {
        terminal: crate::terminal::terminal_context().telemetry_snapshot(),
...
fn system_clipboard_get_attachments() -> Result<AttachmentsProbeResult, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match xai_mad_shared::clipboard::get_attachments() {
        Ok(att) => {
...
pub use xai_mad_shared::clipboard::ImageData;
...
pub fn clipboard_image_snapshot() -> (Option<u64>, bool) {
    ...
    xai_mad_shared::clipboard::clipboard_image_snapshot()
}
...
pub fn clipboard_change_count() -> Option<u64> {
    ...
    xai_mad_shared::clipboard::clipboard_change_count()
}
...
pub fn clipboard_image_probe_supported() -> bool {
    ...
    xai_mad_shared::clipboard::clipboard_image_probe_supported()
}
...
pub fn prewarm_image_probe() {
    ...
    WARMED.call_once(|| {
        std::thread::spawn(xai_mad_shared::clipboard::clipboard_prewarm);
    });
}
...
fn system_clipboard_get_image_result() -> Result<Option<ImageData>, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match xai_mad_shared::clipboard::get_image() {
        Ok(img) => {
...
mod tests {
    ...
    fn clipboard_route_matrix() {
        ...
        for case in cases {
            // Pure helper with kill switch off so ambient MAD_CLIPBOARD_NO_OSC52
    ...
    fn clipboard_route_no_osc52_kill_switch_forces_off() {
        // MAD_CLIPBOARD_NO_OSC52 must win over Linux/tmux/SSH automatic emit.
        for ctx in [
            plain_terminal_ctx(),
            plain_tmux_ctx(),
            byobu_tmux_ctx(),
    ...
    /// the file must be owner-only (`0600`) — including when an older mad
    ...
    #[test]
    #[serial_test::serial(mad_copy_file)]
    fn default_copy_fallback_path_respects_mad_copy_file() {
        let dir = tempfile::tempdir().expect("tempdir");
        let custom = dir.path().join("custom-copy.txt");
        // SAFETY: test-only env mutation; serialized on the mad_copy_file key.
        unsafe {
            std::env::set_var(MAD_COPY_FILE_ENV, &custom);
        }
        let resolved = default_copy_fallback_path();
        unsafe {
            std::env::remove_var(MAD_COPY_FILE_ENV);
        }
    ...
    /// Without `MAD_COPY_FILE`, the default is `~/.mad/last-copy.txt`
    /// (mad home) — short and toast-friendly, unlike macOS's temp dir.
    #[test]
    #[serial_test::serial(mad_copy_file)]
    fn default_copy_fallback_path_is_mad_home() {
        unsafe {
            std::env::remove_var(MAD_COPY_FILE_ENV);
        }
        let path = default_copy_fallback_path();
        // Test envs always resolve a home (or set MAD_HOME).
        let expected = xai_mad_config::user_mad_home()
            .expect("home resolves in tests")
            .join("last-copy.txt");
        assert_eq!(path, Some(expected));
    }

    /// Toast paths collapse the home prefix to `~` (mad-home paths go
    /// through the shared `abbreviate_path` convention, covered further by
    /// the `MAD_HOME`-override integration test in `xai-mad-pager`).
    #[test]
    fn display_copy_path_abbreviates_home() {
        if std::env::var_os("MAD_HOME").is_none() {
            let home = dirs::home_dir().expect("home resolves in tests");
            assert_eq!(
                display_copy_path(&home.join(".mad").join("last-copy.txt")),
                "~/.mad/last-copy.txt"
            );
        }
        ...
        assert_eq!(
            display_copy_path(std::path::Path::new("/tmp/mad-0/last-copy.txt")),
            "/tmp/mad-0/last-copy.txt"
        );
        assert_eq!(
    ...
    fn delivery_clipboard_success_carries_backup_file() {
        let path = std::path::PathBuf::from("/tmp/mad-1/last-copy.txt");
    ...
    fn toast_message_names_backup_only_for_unverified_or_file_fallback() {
        ...
        assert_eq!(
            ...
            "Copy sent — saved to /tmp/mad-1/last-copy.txt"
        );
        ...
        assert_eq!(
            ...
            "Clipboard unreachable — wrote /tmp/mad-1/last-copy.txt"
        );
