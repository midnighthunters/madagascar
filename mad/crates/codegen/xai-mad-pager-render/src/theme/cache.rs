...
//! lookup must be cheaper than re-loading from `~/.mad/config.toml`.
...
//! `xai_mad_shell::util::config::set_theme()` (and friends), invoked
...
static CURRENT: AtomicU8 = AtomicU8::new(ThemeKind::MadNight as u8);
...
/// back to `MadNight` if the byte is somehow out of range (which
...
fn theme_kind_from_u8(byte: u8) -> ThemeKind {
    match byte {
        x if x == ThemeKind::MadNight as u8 => ThemeKind::MadNight,
        x if x == ThemeKind::MadDay as u8 => ThemeKind::MadDay,
...
/// When `None`, `to_theme_kind()` defaults to `MadNight` / `MadDay`.
...
///
/// On the first call, reads from `~/.mad/config.toml` (via the shell's
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    xai_mad_markdown::set_color_level_cap(if locked {
        xai_mad_markdown::ColorLevel::Basic
    } else {
        xai_mad_markdown::ColorLevel::TrueColor
    });
    xai_mad_markdown::set_polarity_safe_syntax(locked);
}
...
///
/// Precedence:
/// 1. Environment variable (`MAD_THEME`)
...
/// and the user's dark/light theme mapping. Falls back to `MadNight`
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `xai_mad_shell::util::config::set_theme()` (and
...
fn load_from_disk() -> Option<ThemeKind> {
    let root = xai_mad_config::load_effective_config_disk_only().ok()?;
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = xai_mad_config::load_effective_config_disk_only() else {
...
pub fn reset_for_test() {
    ...
    CURRENT.store(ThemeKind::MadNight as u8, Ordering::Relaxed);
...
mod tests {
    ...
    fn terminal_native_lock_pins_kind_and_blocks_apply_kind() {
        with_test_env(|| {
            set(ThemeKind::MadDay);
            set_terminal_native_lock(true);
            assert!(terminal_native_locked());
            assert_eq!(current_kind(), ThemeKind::MadNight, "nominal kind");

            let applied = super::super::Theme::apply_kind(ThemeKind::MadDay);
            assert_eq!(applied, ThemeKind::MadNight, "apply_kind must no-op");
            assert_eq!(current_kind(), ThemeKind::MadNight);
    ...
    fn terminal_native_lock_serves_terminal_default_palette() {
        with_test_env(|| {
            ...
            assert_ne!(
                theme.text_primary,
                super::super::Theme::madday().text_primary,
                "must not serve the cached (MadDay) theme"
            );
        });
    }
    ...
    fn terminal_native_lock_enables_polarity_safe_syntax() {
        with_test_env(|| {
            assert!(!xai_mad_markdown::polarity_safe_syntax());
            set_terminal_native_lock(true);
            assert!(
                xai_mad_markdown::polarity_safe_syntax(),
    ...
    fn terminal_native_lock_caps_quantize_at_ansi16() {
        ...
        with_test_env(|| {
            ...
            for input in [
                Color::Rgb(0x26, 0x26, 0x26), // madday text_primary
    ...
    fn resolve_no_osc11_explicit_auto_and_default() {
        with_test_env(|| {
            assert_eq!(
                resolve_from_config(Some(ThemeKind::MadDay), false),
                ThemeKind::MadDay
            );
            assert!(!is_auto_mode());
            ...
            assert_eq!(resolve_from_config(None, false), ThemeKind::MadNight);
        });
    }
    ...
    #[test]
    fn resolve_auto_dark_system_returns_madnight() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Dark));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::MadNight);
        });
    }

    #[test]
    fn resolve_auto_light_system_returns_madday() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Light));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::MadDay);
        });
    }

    #[test]
    fn resolve_auto_detection_failure_returns_madnight() {
        with_test_env(|| {
    ...
    #[test]
    fn resolve_from_config_no_config_returns_madnight() {
        with_test_env(|| {
    ...
    fn resolve_from_config_explicit_theme_returns_it() {
        with_test_env(|| {
            let result = resolve_from_config(Some(ThemeKind::MadDay), true);
            assert_eq!(result, ThemeKind::MadDay);
            assert!(
                !is_auto_mode(),
    ...
    fn set_then_current_kind_round_trips() {
        with_test_env(|| {
            ...
            set(ThemeKind::MadDay);
            assert_eq!(current_kind(), ThemeKind::MadDay);
        });
    }
