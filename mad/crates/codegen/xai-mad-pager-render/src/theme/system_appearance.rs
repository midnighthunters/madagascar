...
/// in `config.toml`. When `None`, defaults to `MadNight` / `MadDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::MadDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::MadNight),
    }
}
