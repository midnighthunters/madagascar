...
//! The default theme is MadNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! MadNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::MadNight, ThemeKind::MadDay];
    ...
    /// when quantized to 256 or 16 colors. MadNight uses neutral grays
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "madnight" | "mad-night" | "dark" => Some(Self::MadNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "madday" | "mad-day" | "light" | "day" => Some(Self::MadDay),
...
/// `"madnight"` → `"Mad Night"`). Falls back to `value` verbatim.
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.mad/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::MadNight => Self::madnight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::MadDay => Self::madday(),
            ...
            // if reached, fall back to MadNight.
            ThemeKind::Auto => Self::madnight(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `MAD_FORCE_COLOR_LEVEL=basic`),
    ...
    ///    on madnight, erasing every state signal). We pin each
    ...
    /// theme (e.g., MadNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // MadNight bg field quantizes to Color::Black and these
            // become invisible.
...
pub fn apply_cursor_color() {
    ...
    xai_mad_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::MadNight.is_auto());
        assert!(!ThemeKind::MadDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::madday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::madnight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::madday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. MadNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::MadNight => Theme::madnight(),
                ThemeKind::MadDay => Theme::madday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_madnight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark MadNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::madnight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("madnight"),
            Some(ThemeKind::MadNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::MadNight));
        assert_eq!(ThemeKind::from_name("madday"), Some(ThemeKind::MadDay));
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::MadDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("madnight", ThemeKind::MadNight),
            ("mad-night", ThemeKind::MadNight),
            ("dark", ThemeKind::MadNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("madday", ThemeKind::MadDay),
            ("mad-day", ThemeKind::MadDay),
            ("light", ThemeKind::MadDay),
            ("day", ThemeKind::MadDay),
