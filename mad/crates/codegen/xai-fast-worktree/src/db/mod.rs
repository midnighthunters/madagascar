...
impl WorktreeDb {
    /// Open (or create) the DB at `mad_home/worktrees.db`.
    pub fn open(mad_home: &Path) -> Result<Self> {
        Self::open_at(&mad_home.join("worktrees.db"))
    }
    ...
    /// Open the default DB at `~/.mad/worktrees.db`.
    ///
    /// Discovers mad home via `$MAD_HOME`, falling back to the canonicalized
    /// `$HOME/.mad` (matching `xai_mad_config::mad_home`).
...
pub fn resolve_mad_home() -> Result<PathBuf> {
    if let Ok(v) = std::env::var("MAD_HOME") {
        return Ok(PathBuf::from(v));
    }
    let home = PathBuf::from(std::env::var("HOME").context("neither $MAD_HOME nor $HOME is set")?);
    // Canonicalize the home dir so worktree paths share the same physical .mad
    // tree as trust/hooks even when it is symlinked. The dunce canonicalization
    // must stay in sync with xai_mad_config::default_mad_home();
    // home resolution deliberately differs ($HOME here vs std::env::home_dir()).
    Ok(dunce::canonicalize(&home).unwrap_or(home).join(".mad"))
}

/// Serializes tests that mutate the process-global `MAD_HOME` env var so they
...
#[cfg(test)]
static MAD_HOME_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Test-only isolation for code that resolves the DB via `open_default()`.
///
/// Holds [`MAD_HOME_ENV_LOCK`] (serializing concurrent setters), points
/// `MAD_HOME` at a fresh private tmp dir, and restores the prior value on drop.
/// Use instead of hand-rolling the lock + restore guard + tmp dir per test.
///
/// `Drop` restores `MAD_HOME` before `_lock` releases, so the env is correct
/// before another waiting setter proceeds.
#[cfg(test)]
pub(crate) struct MadHomeFixture {
    _lock: std::sync::MutexGuard<'static, ()>,
    prev: Option<std::ffi::OsString>,
    /// The isolated mad home; pass to `WorktreeDb::open` to read the same DB
...
impl MadHomeFixture {
    pub(crate) fn new() -> Self {
        let lock = MAD_HOME_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let tmp = tempfile::TempDir::new().unwrap();
        let home = tmp.path().join("mad-home");
        std::fs::create_dir_all(&home).unwrap();
        // Warm up the DB (journal-mode conversion + schema) before exposing it
        // via MAD_HOME, sparing the test hot loop set_journal_mode's retry
        // sleeps. This open has exclusive access (nothing reaches the path
        // until MAD_HOME points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("MAD_HOME");
        unsafe { std::env::set_var("MAD_HOME", &home) };
        Self {
            _lock: lock,
            prev,
            home,
...
#[cfg(test)]
impl Drop for MadHomeFixture {
    fn drop(&mut self) {
        unsafe {
            match self.prev.take() {
                Some(p) => std::env::set_var("MAD_HOME", p),
                None => std::env::remove_var("MAD_HOME"),
            }
        }
    }
}
