...
use xai_mad_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use xai_mad_pager::client_identity::PAGER_CLIENT_VERSION;
use xai_mad_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use xai_mad_shell::agent::config::Config as AgentConfig;
use xai_mad_shell::leader::{
...
use xai_mad_update::{UpdateConfig, auto_update, enforce_version_policy_or_exit};
...
fn apply_headless_args_to_config(args: &HeadlessArgs, config: &mut AgentConfig) {
    if let Some(v) = &args.mad_ws_origin {
        config.mad_com_config.mad_ws_origin = v.clone();
    }
    if let Some(v) = &args.mad_ws_url {
        config.mad_com_config.mad_ws_url = v.clone();
    }
}
/// Apply global endpoint CLI args to an existing config.
fn apply_agent_endpoint_args(
    agent_args: &xai_mad_pager::app::AgentArgs,
...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   Mad agent server starting...");
    eprintln!();
...
/// Entrypoint tag for `mad -p`; keys the quiet stderr default in `init_tracing_simple`.
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    use xai_mad_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(xai_mad_telemetry::sampling_log::layer())
        .with(xai_mad_telemetry::instrumentation::layer())
        .with(xai_mad_telemetry::hooks_log::layer())
        .with(xai_mad_telemetry::otel_layer::build_otel_layer(
            xai_mad_telemetry::otel_layer::OtelClientInfo {
                client_name: "mad-pager",
                client_version: xai_mad_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            xai_mad_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
    xai_mad_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    xai_mad_telemetry::external::init(
        xai_mad_shell::agent::config::resolve_external_otel_config(
            xai_mad_telemetry::external::config::ExternalClientInfo {
                service_version: env!("VERSION_WITH_COMMIT").to_owned(),
                client_version: xai_mad_version::VERSION.to_owned(),
                app_entrypoint: app_entrypoint.to_owned(),
            },
        ),
    );
}
/// `mad setup`: rendering + exit codes only; fetch logic lives in `xai_mad_shell::managed_config`.
/// `json` prints the served configuration instead of installing it.
async fn run_setup_command(json: bool) {
    use xai_mad_shell::managed_config::{self, SetupOutcome};
    if !managed_config::has_principal() {
        eprintln!("No deployment key or team sign-in found.");
        eprintln!();
        eprintln!("To install managed configuration, sign in with a team using `mad login`,");
        eprintln!("or set a deployment key:");
        eprintln!();
        if cfg!(unix) {
            eprintln!("  export MAD_DEPLOYMENT_KEY=<your-key>");
        } else {
            eprintln!("  $env:MAD_DEPLOYMENT_KEY=\"<your-key>\"");
        }
        eprintln!("  mad setup");
        eprintln!();
        eprintln!("Or add the key to ~/.mad/config.toml:");
        eprintln!();
        eprintln!("  [endpoints]");
        eprintln!("  deployment_key = \"<your-key>\"");
        eprintln!();
        eprintln!(
            "If you don't have a deployment key, contact your organization's Mad administrator."
        );
        std::process::exit(1);
    }
    if json {
    ...
    match managed_config::run_setup().await {
        ...
        SetupOutcome::Skipped => {
            eprintln!(
                "Managed configuration was not applied this run (another process held the apply lock, or the credential changed during the fetch). Run `mad setup` again."
            );
        }
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = xai_mad_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !xai_mad_shell::util::is_mad_process(pid) {
            if let Some(ref lock) = d.lock_path {
                eprintln!("  PID {pid} is not a mad process, removing stale lock");
        ...
        if let Err(e) = xai_mad_shell::util::kill_process_by_pid(pid) {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(xai_mad_shell::env::MadagascarBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, xai_mad_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = xai_mad_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = xai_mad_shell::leader::LeaderClient::connect(
        socket_path.to_path_buf(),
        "mad-pager-leader-cli",
        ClientMode::Stdio,
...
fn leader_info_json(
    ...
    info: Option<&xai_mad_shell::leader::ControlPayload>,
...
/// Env override for the `mad workspace` gate: any truthy value enables the
/// command locally, a falsy one disables it — bypassing the remote settings flag.
const WORKSPACE_COMMAND_ENV: &str = "MAD_WORKSPACE_COMMAND";
/// Resolution of the `mad workspace` gate. `Unknown` is kept separate from
...
/// The `MAD_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&xai_mad_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
/// Truthy parse for mad on/off env vars: everything enables except the common
...
fn fetch_remote_settings() -> Option<xai_mad_shell::util::config::RemoteSettings> {
    join_early_prefetch(xai_mad_shell::agent::models::start_early_prefetch(None))
}
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    ) && let Some(profile) = xai_mad_sandbox::requested_confinement_profile()
    {
        anyhow::bail!(
            "`mad workspace` start/restart/resume is unavailable under sandbox profile '{profile}': \
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Disabled => {
            anyhow::bail!(
                "`mad workspace` is not enabled for this account \
             (gated by a server-side feature flag that is currently off)."
            )
        }
        WorkspaceGate::Unknown => {
            anyhow::bail!(
                "Could not load your settings for `mad workspace`. Check your \
             network connection (run `mad login` if you are signed out), then \
             try again."
            )
        }
    }
...
async fn connect_workspace_control(
    ...
    let ws_url = &agent_config.mad_com_config.mad_ws_url;
    ...
    .map_err(|e| {
        anyhow::anyhow!(
            "no running leader for this environment ({e}). \
             Start a mad session, or run `mad workspace start`."
        )
    })
}
async fn workspace_control(
    ...
    let raw_config = xai_mad_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use xai_mad_shell::auth::ensure_authenticated;
    xai_mad_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = xai_mad_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    let agent_config = AgentConfig::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    let (use_leader, _) = resolve_use_leader(
        ...
        true,
        xai_mad_sandbox::requested_confinement_profile(),
    );
    if !use_leader {
        anyhow::bail!(
            "`mad workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.mad/config.toml, or pass --leader."
        );
    }
    ensure_authenticated(
        &agent_config.mad_com_config,
        false,
        Some("No cached credentials found. Run `mad login` first."),
    )
    .await?;
    let env_urls = LeaderEnvUrls::from(&agent_config.mad_com_config);
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    xai_mad_telemetry::sentry::flush_on_shutdown();
    xai_mad_telemetry::otel_layer::shutdown_otel();
    xai_mad_telemetry::debug_log::flush();
...
/// can't drift.
const PLUGIN_DIR_LEADER_WARNING: &str = "mad: --plugin-dir is ignored in leader mode; run with --no-leader to \
     load per-process plugins";
/// Run the `agent` subcommand, dispatching to the appropriate mode.
async fn run_agent_command(
    agent_args: Box<xai_mad_pager::app::AgentArgs>,
    ...
    let _signal_flush = tokio::spawn(async {
        #[cfg(unix)]
        {
            ...
            use xai_mad_pager::app::signal_handler::next_signal_code;
    ...
    if matches!(
        ...
        xai_mad_shell::agent::app::suppress_otel();
    }
    init_tracing_simple("agent");
    let _otel_guard = xai_mad_telemetry::otel_layer::otel_guard();
    xai_mad_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => xai_mad_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(error = %e, "--trust: failed to resolve cwd; folder not trusted")
            }
        }
    }
    let early_prefetch = xai_mad_shell::agent::models::start_early_prefetch(None);
    xai_mad_shell::agent::mvp_agent::warm_async_http_client();
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "Madagascar Build (pager) - v{}",
            xai_mad_version::display_version_with_commit(
                env!("VERSION_WITH_COMMIT"),
                xai_mad_update::channel_label(),
            )
        );
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(xai_mad_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = xai_mad_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    if let Some(warning) = launch_yolo.blocked_warning {
        eprintln!("mad: {warning}");
    }
    agent_config.default_yolo_mode = launch_yolo.yolo;
    agent_config.default_auto_mode = xai_mad_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    ...
    agent_config.resolve_runtime_fields(&xai_mad_shell::agent::config::RuntimeResolutionContext {
    ...
    let requested_confinement = xai_mad_sandbox::requested_confinement_profile();
    let LeaderMode {
        use_leader,
    ...
    let managed_install = is_managed_install(
        ...
        &xai_mad_shell::util::mad_home::mad_home(),
    );
    ...
    if use_leader {
        ...
        let env_urls = xai_mad_shell::leader::LeaderEnvUrls::from(&agent_config.mad_com_config);
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = xai_mad_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            xai_mad_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(xai_mad_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = xai_mad_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = xai_mad_shell::managed_config::sync().await
                                        {
                                            tracing::warn!(
...
/// Sets `MAD_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
/// `mad dashboard`, and clears `args.command` so the regular
...
/// the leader roster — so `mad dashboard` does NOT force leader mode and
/// is compatible with `--no-leader`.
///
/// The only gate is the feature flag: a disabled dashboard
/// (`[dashboard].enabled = false` / `MAD_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !xai_mad_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.mad/config.toml and \
             unsetting MAD_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("MAD_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
const RUNTIME_SHUTDOWN_GRACE: std::time::Duration = std::time::Duration::from_secs(2);
const MAD_WORKER_THREADS_ENV: &str = "MAD_WORKER_THREADS";
...
/// How `MAD_WORKER_THREADS` resolved.
#[derive(Debug, PartialEq, Eq)]
enum WorkerCount {
    ...
    fn notice(&self) -> Option<String> {
        match self {
            ...
            Self::Clamped {
                ...
                "mad: clamped {MAD_WORKER_THREADS_ENV}={requested} to {used} (valid range is 1..={cores})"
            )),
            Self::Ignored { value, .. } => Some(format!(
                "mad: ignoring {MAD_WORKER_THREADS_ENV}={value:?} (not a valid integer)"
            )),
        }
    }
}
fn cli_worker_threads() -> NonZeroUsize {
    ...
    let resolved = match std::env::var(MAD_WORKER_THREADS_ENV) {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<xai_mad_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(xai_mad_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<xai_mad_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(xai_mad_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    xai_mad_shell::heap_profile::install(xai_mad_shell::heap_profile::HeapProfileHooks {
...
fn version_text(channel_label: &str) -> String {
    format!(
        "mad {}\n",
        xai_mad_version::display_version_with_commit(env!("VERSION_WITH_COMMIT"), channel_label,)
    )
}
...
fn dispatch_doctor_if_requested(args: &PagerArgs) -> bool {
    ...
    if let Err(error) = xai_mad_pager::doctor_cmd::run(doctor_args.clone()) {
        eprintln!("Error: {error:#}");
        std::process::exit(1);
    }
    true
...
fn main() {
    if let Some(code) = xai_mad_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    if let Some(code) = xai_mad_pager::voice::maybe_run_capture_subprocess() {
        std::process::exit(code);
    }
    ...
    xai_mad_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    xai_mad_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        xai_mad_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        xai_mad_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    xai_mad_pager::memory_trace::start(
        xai_mad_shell::util::mad_home::mad_home().join("memtrace"),
    );
    raise_fd_limit();
    if let Err(e) = xai_mad_config::validate_requirements() {
        eprintln!("Couldn't start Mad: {e}");
        eprintln!();
        eprintln!(
            "Update Mad to a version the policy allows, or ask your administrator \
             to fix the managed requirements."
        );
        std::process::exit(2);
    }
    let _sentry_guard = xai_mad_telemetry::sentry::init(xai_mad_telemetry::sentry::Config {
        client: "mad-pager",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: xai_mad_shell::agent::config::is_error_reporting_disabled_sync(),
    });
    xai_mad_pager::docs::extract_user_guide_docs(&xai_mad_shell::util::mad_home::mad_home());
    xai_crash_handler::install_terminal_restore_only();
    if xai_mad_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = xai_mad_shell::util::mad_home::mad_home().join("crash");
        if let Some(report) = xai_crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("Mad crashed during your last session.");
    ...
    let crashed = xai_mad_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
    ...
    let runtime = tokio::runtime::Builder::new_multi_thread()
        ...
        .build()
        .unwrap_or_else(|e| {
            eprintln!("mad: failed to start tokio runtime with {workers} workers: {e}");
...
async fn async_main(args: PagerArgs) -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("MAD_COMPACTION_MODE", mode) };
    }
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("MAD_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            std::env::set_var(xai_mad_shell::agent::chat_modes::MAD_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(xai_mad_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("MAD_DEBUG_LOG", path);
            std::env::remove_var("MAD_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("MAD_DEBUG_LOG", "1");
        set_if_unset("MAD_HOOKS_LOG", "1");
    }
    if let Some(Command::Completions { shell }) = &args.command {
        xai_mad_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return xai_mad_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        xai_mad_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        xai_mad_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    xai_mad_shell::config::apply_sandbox(
        None,
    ...
    xai_mad_shell::http::set_client_name(if is_interactive {
        xai_mad_workspace::permission::ClientType::MadPager
    } else {
        xai_mad_workspace::permission::ClientType::Generic
    });
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!({
                        ...
                        "channel": xai_mad_update::channel_name().unwrap_or("unknown"),
                    });
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    write_version(
                ...
                if args.leader || args.no_leader {
                    ...
                    anyhow::bail!(
                        "top-level {flag} applies to the pager TUI, not the agent subcommand. \
                         Use `mad-pager agent {flag}` instead."
                    );
                }
            ...
            Command::Inspect { json } => {
                ...
                xai_mad_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return xai_mad_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = xai_mad_telemetry::otel_layer::otel_guard();
                return xai_mad_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = xai_mad_telemetry::otel_layer::otel_guard();
                let config = xai_mad_shell::config::load_effective_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_mad_pager::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_mad_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_mad_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Share(ref share_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_mad_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return xai_mad_pager::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_mad_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return xai_mad_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Login {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                xai_mad_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                xai_mad_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                init_tracing_simple("cli");
                let config = xai_mad_shell::config::load_effective_config_disk_only()
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                xai_mad_shell::auth::run_cli_logout(&config)?;
                xai_mad_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = xai_mad_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(xai_mad_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some()
            && args.output_format == xai_mad_pager::headless::OutputFormat::Plain
        {
            args.output_format = xai_mad_pager::headless::OutputFormat::Json;
        }
        return xai_mad_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            xai_mad_pager::headless::HeadlessOptions {
    ...
    let result = xai_mad_pager::app::run(args, bg_update_rx).await;
    xai_mad_sandbox::flush();
    match result {
        Ok(true) => {
            let adopted = bg_update_wait.lock().await.take();
            if finish_update_on_exit(adopted, &update_config).await {
                eprintln!("Update installed. Run `mad` to start.");
            } else {
                eprintln!("Update did not complete. Run `mad update` to retry.");
            }
            Ok(())
        }
        Ok(false) => Ok(()),
...
///
/// Prefers awaiting the parked waiter for the background `mad update` child
...
/// fall back to a fresh blocking `mad update`, which itself resolves to
...
fn build_update_config() -> UpdateConfig {
    let environment = xai_mad_shell::env::MadagascarBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                xai_mad_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("MAD_NPM_REGISTRY"))
        .ok()
        .or_else(xai_mad_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = xai_mad_shell::config::load_effective_config_disk_only()
        && let Some(ch) = xai_mad_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn should_check_for_updates(no_auto_update_flag: bool) -> bool {
    ...
    !std::env::var_os("MAD_DISABLE_AUTOUPDATER")
...
/// True when `exe` is the binary `<mad_home>/bin/mad` resolves to, the
...
/// npm shim hardcodes `~/.mad`, so a custom `MAD_HOME` skips here too.
fn is_managed_install(exe: Option<std::path::PathBuf>, mad_home: &std::path::Path) -> bool {
    if mad_home.as_os_str().is_empty() {
        return false;
    }
    let Some(exe) = exe else {
        return false;
    };
    let managed = xai_mad_config::mad_application_in(mad_home);
...
/// Handle `mad-pager update [--check] [--json] [--force-reinstall] [--version X] [--alpha|--stable|--enterprise]`.
...
/// After a successful `mad update`, ask any running leader on this machine that
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in xai_mad_shell::leader::discover_leaders().await {
        if d.classification != xai_mad_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match xai_mad_shell::leader::LeaderClient::connect(
            socket_path,
            "mad-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(xai_mad_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(xai_mad_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn override_out_of_range_is_clamped_with_a_notice() {
        ...
        assert_eq!(
            ...
            "mad: clamped MAD_WORKER_THREADS=100000 to 360 (valid range is 1..=360)"
        );
    }
    #[test]
    fn override_unparseable_is_ignored_with_a_notice() {
        ...
        assert_eq!(
            ...
            "mad: ignoring MAD_WORKER_THREADS=\"abc\" (not a valid integer)"
        );
    }
    #[test]
    ...
    fn version_flags_and_doctor_are_distinct_early_intents() {
        let version = PagerArgs::try_parse_from(["mad", "--version"]).unwrap();
        assert!(version.version);
        assert!(version.command.is_none());
        let short = PagerArgs::try_parse_from(["mad", "-v"]).unwrap();
        assert!(short.version);
        assert!(short.command.is_none());
        let subcommand = PagerArgs::try_parse_from(["mad", "version"]).unwrap();
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            xai_mad_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: xai_mad_shell::heap_profile::JemallocStats) {
    ...
    fn jemalloc_dump_rejects_interior_nul_path() {
        ...
        let path = Path::new(OsStr::from_bytes(b"/tmp/mad-jemalloc-\0.heap"));
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            xai_mad_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!xai_mad_shell::heap_profile::prof_available());
            return;
        }
        assert!(xai_mad_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(xai_mad_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(xai_mad_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        xai_mad_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    fn is_managed_install_matches_only_the_bin_mad_target() {
        let home =
            std::env::temp_dir().join(format!("mad-pager-managed-install-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&home);
        std::fs::create_dir_all(home.join("bin")).unwrap();
        std::fs::create_dir_all(home.join("downloads")).unwrap();
        assert!(!is_managed_install(
            Some(home.join("bin").join("mad")),
            &home
        ));
        ...
        let target = home.join("downloads").join("mad-1.2.3");
        std::fs::write(&target, b"binary").unwrap();
        std::os::unix::fs::symlink(&target, home.join("bin").join("mad")).unwrap();
        assert!(is_managed_install(
            Some(home.join("bin").join("mad")),
            &home
        ));
        assert!(is_managed_install(Some(target.clone()), &home));
        let pinned = home.join("bin").join("mad-9.9.9");
    ...
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        ...
        assert_eq!(
            std::env::var("MAD_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
            "startup hook flag must be set",
        );
        unsafe { std::env::remove_var("MAD_OPEN_DASHBOARD_AT_STARTUP") };
    }
    /// `mad dashboard --no-leader` is allowed — the dashboard does not
    /// require a leader, so the combination launches into the dashboard in
    /// non-leader mode.
    #[serial_test::serial(MAD_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_allows_no_leader() {
        let mut args = PagerArgs::try_parse_from(["mad", "--no-leader", "dashboard"]).unwrap();
        flag_dashboard_at_startup_if_requested(&mut args)
    ...
    #[serial_test::serial(MAD_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("MAD_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("MAD_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("MAD_WORKSPACE_COMMAND", "off") };
