//! E2E path-deny and Mad hook write-deny (subprocess; arm64-tagged).
...
const MAD_HOME_ENV: &str = "SANDBOX_E2E_MAD_HOME";
...
fn apply_fixture_env(cmd: &mut Command, home: &Path, mad_home: &Path, workspace: &Path) {
    cmd.env(WORKSPACE_ENV, workspace.as_os_str())
        .env(HOME_ENV, home.as_os_str())
        .env(MAD_HOME_ENV, mad_home.as_os_str())
        .env("HOME", home.as_os_str())
        .env("MAD_HOME", mad_home.as_os_str());
}
...
fn run_scenario(
    ...
    apply_fixture_env(&mut cmd, home, mad_home, workspace);
    let output = cmd
...
fn skip_if_enforcement_unavailable() -> bool {
    ...
    let support = xai_mad_sandbox::SandboxManager::support_info();
...
fn profile_from_env() -> xai_mad_sandbox::ProfileName {
    xai_mad_sandbox::ProfileName::Custom(std::env::var(PROFILE_ENV).expect(PROFILE_ENV))
}
...
fn subprocess_entry() {
    ...
    // Isolate HOME/MAD_HOME before any config OnceLock init.
    let home = PathBuf::from(std::env::var(HOME_ENV).expect(HOME_ENV));
    let mad_home = PathBuf::from(std::env::var(MAD_HOME_ENV).expect(MAD_HOME_ENV));
    ...
    match scenario.as_str() {
        ...
        "hook_write_deny_marker_spoof" => subprocess_hook_write_deny_marker_spoof(&mad_home),
        other => {
...
fn subprocess_profile_and_bwrap_reexec(profile: &xai_mad_sandbox::ProfileName, workspace: &Path) {
    #[cfg(target_os = "linux")]
    {
        if !xai_mad_sandbox::is_inside_bwrap() {
            ...
            match xai_mad_sandbox::bwrap_reexec_for_profile(profile, workspace) {
                Some(mut cmd) => {
...
fn subprocess_block_deny(workspace: &Path) {
    ...
    let mut sandbox = xai_mad_sandbox::SandboxManager::new(profile, workspace);
...
fn subprocess_hook_write_deny_marker_spoof(_mad_home: &Path) {
    #[cfg(not(target_os = "linux"))]
    {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        unsafe {
            std::env::set_var("__MAD_INSIDE_BWRAP", "1");
        }
        match xai_mad_sandbox::verify_hook_write_deny_enforced() {
            Ok(()) => {
...
/// Workspace-profile Mad-owned hook write-deny probes (existing sources + first-run).
fn subprocess_hook_write_deny(workspace: &Path, first_run: bool) {
    let home = PathBuf::from(std::env::var(MAD_HOME_ENV).expect(MAD_HOME_ENV));

    let profile = xai_mad_sandbox::ProfileName::Workspace;
    subprocess_profile_and_bwrap_reexec(&profile, workspace);
    ...
    if first_run {
        ...
        eprintln!("OK: first-run Mad hook slots denied");
    } else {
    ...
    // Parent grants remain creatable: Mad runtime sibling, workspace, temp.
    assert_write_ok(
        "mad runtime sibling",
...
/// Create isolated HOME + MAD_HOME fixture dirs for a scenario.
fn fixture_homes(
    tag: &str,
) -> (
    PathBuf,
    ...
    let mad = unique_temp_dir(&format!("{tag}-mad"));
    let workspace = unique_temp_dir(&format!("{tag}-ws"));
    // Empty global sandbox config under fixture MAD_HOME so generic tests do
    // not inherit the developer/runner's ~/.mad/sandbox.toml.
    fs::write(mad.join("sandbox.toml"), "").expect("empty global sandbox.toml");
    (
        home.clone(),
        mad.clone(),
        workspace.clone(),
...
fn run_deny_case(
    ...
    let (home, mad, tmp, _ch, _cg, _cw) = fixture_homes(tag);
    ...
    let deny_list = deny_entries
        ...
        .collect::<Vec<_>>()
        .join(", ");
    fs::create_dir_all(tmp.join(".mad")).expect("mkdir .mad");
    fs::write(
        tmp.join(".mad").join("sandbox.toml"),
        format!("[profiles.{profile}]\nextends = \"workspace\"\ndeny = [{deny_list}]\n"),
    )
    .expect("write sandbox.toml");

    // Ensure Mad fixed slots exist so workspace-based custom profiles can
    // resolve hook write-deny without depending on the real user tree.
    fs::create_dir_all(mad.join("hooks")).expect("mkdir fixture hooks");
    fs::write(mad.join("hooks-paths"), b"").expect("write fixture hooks-paths");
    ...
    let (status, stderr) = run_scenario(&home, &mad, &tmp, profile, targets, controls, postlaunch);
    assert!(
        status.success(),
...
fn hardlinked_hooks_paths_refuses_startup() {
    ...
    let (home, mad, workspace, _ch, _cg, _cw) = fixture_homes("hook-hl");
    fs::create_dir_all(mad.join("hooks")).unwrap();
    let reg = mad.join("hooks-paths");
    let alias = mad.join("hooks-paths-alias");
    fs::write(&reg, b"").unwrap();
    fs::hard_link(&reg, &alias).unwrap();

    let (status, stderr) =
        run_hook_write_deny_scenario(&home, &mad, &workspace, "hook_write_deny");
    assert!(
        !status.success(),
...
/// Workspace profile: Mad-owned direct hook sources are write-denied but readable;
/// create / overwrite / unlink / rename / mkdir fail; absolute hooks-paths
/// targets are denied; parent rename is blocked; Mad/CWD/temp siblings stay writable.
#[test]
fn workspace_protects_direct_hook_sources() {
    ...
    let (home, mad, workspace, _ch, _cg, _cw) = fixture_homes("hook");

    fs::create_dir_all(mad.join("hooks")).expect("mkdir hooks");
    fs::write(mad.join("hooks").join("keep.json"), r#"{"keep-me":true}"#)
        .expect("write keep.json");
    let dynamic = mad.join("sessions").join("extra-hooks");
    ...
    for needle in [
        ...
        "OK: mad runtime sibling writable",
...
fn hardlinked_hooks_json_refuses_startup() {
    ...
    let (home, mad, workspace, _ch, _cg, _cw) = fixture_homes("hook-json-hl");
    fs::create_dir_all(mad.join("hooks")).unwrap();
    fs::write(mad.join("hooks-paths"), b"").unwrap();
    let active = mad.join("hooks").join("active.json");
    let alias = mad.join("hooks").join("active-alias.json");
...
fn symlinked_hooks_json_refuses_startup() {
    ...
    let (home, mad, workspace, _ch, _cg, _cw) = fixture_homes("hook-json-sym");
    fs::create_dir_all(mad.join("hooks")).unwrap();
    fs::write(mad.join("hooks-paths"), b"").unwrap();
    let real = mad.join("real-active.json");
    let active = mad.join("hooks").join("active.json");
...
/// First-run: missing fixed slots are created as real Mad state before apply,
...
fn workspace_protects_direct_hook_sources_first_run() {
    ...
    let (home, mad, workspace, _ch, _cg, _cw) = fixture_homes("hook-fr");
    // Intentionally leave hooks/ and hooks-paths absent (first-run ensure path).

    let (status, stderr) =
        run_hook_write_deny_scenario(&home, &mad, &workspace, "hook_write_deny_first_run");
    assert!(
        status.success(),
    ...
    for needle in [
        "OK: first-run Mad hook slots denied",
    ...
    // Post-exit host: Mad slots exist and are valid; no vendor artifacts.
    assert!(
        mad.join("hooks").is_dir(),
        "post-exit: hooks dir must exist as a real directory"
    );
    assert!(
        mad.join("hooks-paths").is_file(),
        "post-exit: hooks-paths must exist as a real file"
    );
    assert_eq!(
        fs::read(mad.join("hooks-paths")).expect("read hooks-paths"),
        b"",
...
fn hook_write_deny_refuses_marker_spoof() {
    ...
    #[cfg(target_os = "linux")]
    {
        let (home, mad, workspace, _ch, _cg, _cw) = fixture_homes("hook-spoof");
        fs::create_dir_all(mad.join("hooks")).unwrap();
        fs::write(mad.join("hooks").join("x.json"), b"{}").unwrap();
        fs::write(mad.join("hooks-paths"), b"").unwrap();
        let (status, stderr) =
            run_hook_write_deny_scenario(&home, &mad, &workspace, "hook_write_deny_marker_spoof");
        assert!(
            status.success(),
