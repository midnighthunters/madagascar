...
//! OS-level sandboxing for Madagascar Build via [nono](https://crates.io/crates/nono).
//!
...
//!
//! ```rust,no_run
//! use xai_mad_sandbox::{SandboxManager, ProfileName};
//! use std::path::Path;
//!
...
const BWRAP_ENV_VAR: &str = "__MAD_INSIDE_BWRAP";
...
impl SandboxManager {
    ...
    pub fn apply(&mut self, workspace: &Path) -> anyhow::Result<()> {
        ...
        if requires_hook_write_deny(&self.profile, workspace) {
            xai_mad_config::ensure_mad_hook_slots(paths::mad_home().as_path())
...
/// Zero-permission placeholder (file or dir) under `mad_home` used by bwrap bind-over.
///
/// The placeholder name is suffixed with the current PID so concurrent mad
...
fn bwrap_blocked_placeholder(name: &str, want_dir: bool) -> Option<PathBuf> {
    ...
    let path = paths::mad_home().join(format!("{name}.{}", std::process::id()));
...
mod tests {
    ...
    fn bwrap_hook_plan_binds_ancestors_then_leaves() {
        ...
        let sources = [xai_mad_config::GlobalHookSource {
            path: leaf.clone(),
            kind: xai_mad_config::GlobalHookSourceKind::ConfiguredSource,
        }];
    ...
    /// Create a temp workspace whose `.mad/sandbox.toml` contains `toml_body`.
    ...
    fn temp_workspace_with_sandbox_toml(tag: &str, toml_body: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let ws = std::env::temp_dir().join(format!("mad-{tag}-{}-{nanos}", std::process::id()));
        let mad = ws.join(".mad");
        std::fs::create_dir_all(&mad).unwrap();
        std::fs::write(mad.join("sandbox.toml"), toml_body).unwrap();
        ws
    }
    ...
    fn bwrap_reexec_uses_dir_placeholder_for_directories() {
        let _g = EnvGuard::remove(BWRAP_ENV_VAR);
        let dir = std::env::temp_dir().join(format!("mad-deny-dir-{}", std::process::id()));
        ...
        let args: Vec<String> = cmd
            ...
            .map(|a| a.to_string_lossy().to_string())
            .collect();
        let blocked_dir = paths::mad_home()
