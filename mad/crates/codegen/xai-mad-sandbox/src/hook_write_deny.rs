//! Mad-owned hook write-deny: plan, identity revalidation, and post-reexec checks.
//! Namespace lockdown is in [`crate::child_net`].

use std::path::{Path, PathBuf};

use xai_mad_config::{
    GlobalHookSource, ensure_mad_hook_slots, missing_configured_sources,
    resolve_global_hook_sources,
};

#[cfg(target_os = "linux")]
use xai_mad_config::unique_ancestors_rootward;
#[cfg(unix)]
use xai_mad_config::validated_hook_json_files_for_sources;
...
impl From<xai_mad_config::GlobalHookSourceError> for HookWriteDenyError {
    fn from(e: xai_mad_config::GlobalHookSourceError) -> Self {
...
fn reject_hardlinked_files(sources: &[GlobalHookSource]) -> Result<(), HookWriteDenyError> {
    use std::os::unix::fs::MetadataExt;
    use xai_mad_config::GlobalHookSourceKind;
    for s in sources {
...
pub fn resolve_hook_write_deny_snapshot() -> Result<Vec<GlobalHookSource>, HookWriteDenyError> {
    let mad = mad_home();
    let resolved =
        resolve_global_hook_sources(Some(mad.as_path()), /* reject_symlinks */ true)?;
...
pub fn prepare_hook_write_deny(
    ...
    let mad = mad_home();
    ensure_mad_hook_slots(mad.as_path())?;
...
fn capture_dir_json_snapshot(dir: &Path) -> Result<DirJsonSnapshot, HookWriteDenyError> {
    use xai_mad_config::{list_direct_hook_json_files, validate_direct_hook_json_file};
