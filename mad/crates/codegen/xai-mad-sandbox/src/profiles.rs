...
//! `off`. Custom profiles via `~/.mad/sandbox.toml` or `.mad/sandbox.toml`.
...
/// Load sandbox config from `~/.mad/sandbox.toml` and `.mad/sandbox.toml`.
///
...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Global config: ~/.mad/sandbox.toml
    let global_path = mad_home().join("sandbox.toml");
    if let Some(global) = load_config_file(&global_path) {
        config = global;
    }

    // Project config: <workspace>/.mad/sandbox.toml (additive only)
    let project_path = workspace.join(".mad").join("sandbox.toml");
...
pub fn sandbox_profile_conflicts(workspace: &Path) -> Vec<String> {
    let global = load_config_file(&mad_home().join("sandbox.toml")).unwrap_or_default();
    let project =
        load_config_file(&workspace.join(".mad").join("sandbox.toml")).unwrap_or_default();
...
impl ProfileName {
    ...
    pub(crate) fn capability_set_from_profile(
        ...
        // directories like ~/.mad/ that may not exist on first run.
        ...
        if !profile.write_deny.is_empty() {
            ...
            {
                let files =
                    xai_mad_config::validated_hook_json_files_for_sources(&profile.write_deny)
                ...
                let profile_config = config.profiles.get(name).ok_or_else(|| {
                    anyhow::anyhow!(
                        "Custom sandbox profile '{name}' not found. \
                         Define it in ~/.mad/sandbox.toml or .mad/sandbox.toml:\n\n\
                         [profiles.{name}]\n\
                         extends = \"workspace\"\n\
                         read_only = [\"/data\"]\n"
                    )
...
mod tests {
    ...
    /// Hosts with a retargetable `$MAD_HOME/hooks` symlink (fail-closed under
...
"#;
    ...
    fn enxio_device_file_is_skipped_but_directory_is_not() {
        ...
        let dir = std::env::temp_dir().join(format!("mad-sbx-test-{}", std::process::id()));
