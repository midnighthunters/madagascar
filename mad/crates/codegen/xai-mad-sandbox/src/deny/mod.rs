...
pub(crate) fn ancestors_within_writable_roots(
    ...
    let mut out = Vec::new();
    for anc in xai_mad_config::existing_ancestor_chain(path) {
...
pub(crate) fn apply_write_deny_paths_to_capability_set(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        tracing::info!(
            count = entries.len(),
            "Applied Seatbelt write-deny for Mad-owned direct hook sources"
        );
    }
    #[cfg(target_os = "linux")]
    {
...
mod tests {
    ...
    fn ancestors_pin_under_writable_root_not_home() {
        ...
        let mad = tmp.join("mad");
        let sessions = mad.join("sessions");
        let leaf = sessions.join("extra-hooks");
        std::fs::create_dir_all(&leaf).unwrap();
        let ws = tmp.join("ws");
        std::fs::create_dir_all(&ws).unwrap();

        let roots = [mad.clone(), ws.clone()];
        let pin = ancestors_within_writable_roots(&leaf, &roots);
        assert!(
            pin.iter().any(|p| p == &sessions),
            "must pin sessions under MAD_HOME: {pin:?}"
        );
        assert!(
            pin.iter().any(|p| p == &mad),
            "must pin MAD_HOME grant root: {pin:?}"
        );
        assert!(
