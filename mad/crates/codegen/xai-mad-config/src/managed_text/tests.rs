...
fn missing_empty_normal_no_final_newline_and_crlf_are_preserved() {
    ...
    let plan = ManagedConfig::plan(request(
        &missing,
        &[("terminal.ssh-wrap", "alias ssh='mad wrap ssh'")],
    ))
    .unwrap();
    assert_eq!(
        plan.updated_bytes(),
        expected("alias ssh='mad wrap ssh'", "\n").as_bytes()
    );
...
fn prose_and_exports_with_owned_words_and_chevrons_are_inert() {
    ...
    let content = [
        ...
        "# mad doctor docs say >>> run this later <<<",
