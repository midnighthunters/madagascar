//! Config file loading for Mad.
//!
//! Merge order (lowest → highest priority):
//! 1. `/etc/mad/managed_config.toml`
//! 2. `$MAD_HOME/managed_config.toml`
//! 3. `$MAD_HOME/config.toml`
//! 4. `$MAD_HOME/requirements.toml` (cloud cache; Ed25519-signed at rest once a
//!    key is embedded — see [`signed_policy`] — below the OS-protected layers)
//! 5. `/etc/mad/requirements.toml`
//! 6. macOS MDM managed preferences (`ai.x.mad`, admin-forced) — macOS only
//!
...
pub use global_hook_sources::{
    ...
    ensure_mad_hook_slots, existing_ancestor_chain, is_direct_hook_json_name,
...
pub use paths::{
    ...
    default_mad_home, encode_cwd_dirname, ensure_sessions_cwd_dir, mad_application,
    mad_application_in, mad_home, sessions_cwd_dir, system_config_dir, user_mad_home,
};
pub use validation::{
