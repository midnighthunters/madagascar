//! Filesystem locations for mad config files and binaries.

use std::path::PathBuf;
use std::sync::OnceLock;

static MAD_HOME: OnceLock<PathBuf> = OnceLock::new();
...
/// The default user mad directory (`~/.mad`, canonicalized) used when
/// `MAD_HOME` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`mad_home()`] is the default without duplicating the computation.
///
...
/// `~/.mad/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `xai_fast_worktree::db::resolve_mad_home` (deliberately standalone crate).
pub fn default_mad_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    dunce::canonicalize(&home).unwrap_or(home).join(".mad")
}

/// Per-user config directory: `$MAD_HOME` or `~/.mad`. Created if needed.
pub fn mad_home() -> PathBuf {
    MAD_HOME
        .get_or_init(|| {
            let mad_home = if let Ok(v) = std::env::var("MAD_HOME") {
                PathBuf::from(v)
            } else {
                default_mad_home()
            };
...
/// The user-global mad home, but only when one genuinely resolves: `Some` when
/// `$MAD_HOME` is set or a home directory is found, `None` otherwise. Unlike
/// [`mad_home()`], this never falls back to a cwd-relative `.mad`, so callers
/// that *scan* user-global mad resources (hooks, marketplace sources, ...) don't
/// mistake a project's `.mad` tree for the user-global one when no home resolves.
pub fn user_mad_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("MAD_HOME").is_some() || std::env::home_dir().is_some();
    resolvable.then(mad_home)
}

/// Canonical mad application path: `$MAD_HOME/bin/mad` (Unix) or `mad.exe` (Windows).
pub fn mad_application() -> PathBuf {
    mad_application_in(&mad_home())
}

/// [`mad_application`] under an explicit home instead of `$MAD_HOME`.
pub fn mad_application_in(home: &std::path::Path) -> PathBuf {
    let name = if cfg!(windows) { "mad.exe" } else { "mad" };
    home.join("bin").join(name)
}

/// System-wide config directory: `/etc/mad/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/mad"))
    } else {
        None
    }
}
...
/// `mad_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    mad_home().join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = mad_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_mad_home_has_no_verbatim_prefix() {
        ...
        let home = default_mad_home();
        assert!(!home.to_string_lossy().starts_with(r"\\?\"));
        assert!(home.ends_with(".mad"));
    }
