...
use crate::paths::{system_config_dir, user_mad_home};
...
/// mad config files; use [`load_toml_file`] directly for unrelated TOML.
...
pub fn load_from_disk() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_mad_home().as_deref(), USER_CONFIG_FILENAME)
}

/// User config filename (`$MAD_HOME/config.toml`), shared by the loaders here.
...
pub fn load_managed_config() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_mad_home().as_deref(), MANAGED_CONFIG_FILENAME)
}
...
/// `.mad/<filename>` (the cwd-fallback would silently promote an untrusted
/// project `.mad` to the user tier).
...
pub struct ManagedConfigLayer {
    ...
    /// `true` for the root-owned system layer (`/etc/mad`), derived from the
    /// load directory.
    pub is_system: bool,
}
...
pub fn managed_config_layers() -> Vec<ManagedConfigLayer> {
    managed_config_layers_at(system_config_dir().as_deref(), user_mad_home().as_deref())
}
...
/// A hook's origin (held by `xai_mad_hooks::HookSpec::layer`). Defined here, not
/// in `xai-mad-hooks`, since the dep direction is `xai-mad-hooks -> xai-mad-config`;
/// this crate sets the config tiers, `File`/`Plugin` are set downstream.
#[derive(Debug, Clone, Copy, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum HookProvenance {
    /// `/etc/mad/managed_config.toml`.
    SystemManaged,
    /// `$MAD_HOME/managed_config.toml` (server-synced).
    Managed,
...
pub fn hook_config_layers() -> Vec<HookConfigLayer> {
    hook_config_layers_at(system_config_dir().as_deref(), user_mad_home().as_deref())
}
...
impl ConfigLayers {
    ...
    /// wraps this with the `MAD_CAMPAIGNS_OVERRIDE` env layer.
...
/// `MAD_CAMPAIGNS=0` or `[features] campaigns = false` on pre-campaign base.
pub fn campaigns_application_disabled(base_effective: &toml::Value) -> bool {
    if crate::env_bool("MAD_CAMPAIGNS") == Some(false) {
        return true;
    }
    base_effective
        .get("features")
...
/// (`xai_mad_shell::util::config::load_effective_config`) when remote campaigns
/// or `MAD_CAMPAIGNS_OVERRIDE` must be honored. The name mirrors the
...
/// can't be parsed (broken `MAD_TEST_VERSION` in dev), silently strips
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match xai_mad_version::installed_semver() {
...
mod tests {
    ...
    fn load_user_config_layer_reads_file_when_home_present() {
        ...
        let dir = std::env::temp_dir().join(format!("mad-load-layer-{}", std::process::id()));
    ...
    fn parse_error_keeps_kind_but_not_snippet() {
        let dir = std::env::temp_dir().join(format!("mad-toml-leak-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    /// `MAD_CAMPAIGNS=0` disables campaign application regardless of config.
    /// `MAD_CAMPAIGNS` is process-global, so this test serializes itself with a
    ...
    fn kill_switch_env_var_disables() {
        ...
        let prior = std::env::var_os("MAD_CAMPAIGNS");
        let empty = toml::Value::Table(Default::default());

        // SAFETY: ENV_GUARD serializes this against itself; no other test in the
        // crate mutates or reads MAD_CAMPAIGNS concurrently.
        unsafe { std::env::set_var("MAD_CAMPAIGNS", "0") };
        assert!(campaigns_application_disabled(&empty));

        unsafe { std::env::remove_var("MAD_CAMPAIGNS") };
        assert!(!campaigns_application_disabled(&empty));

        match prior {
            Some(v) => unsafe { std::env::set_var("MAD_CAMPAIGNS", v) },
            None => unsafe { std::env::remove_var("MAD_CAMPAIGNS") },
        }
    }
}
