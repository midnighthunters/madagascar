//! Mad-owned direct global hook paths shared by shell discovery and sandbox
//! write-deny: `$MAD_HOME/hooks`, `hooks-paths`, and absolute registry targets.
...
pub enum GlobalHookSourceKind {
    /// `$MAD_HOME/hooks/` (discovered + protected).
    HookDirectory,
    /// `$MAD_HOME/hooks-paths` (protected; never loaded as hook JSON).
    RegistryFile,
...
pub enum GlobalHookSourceError {
    ...
    #[error("symlinked MAD_HOME is not allowed under sandbox write-deny: {path}")]
    SymlinkedMadHome { path: PathBuf },
    ...
    #[error("Mad hooks directory has wrong type (expected real directory): {path}")]
    InvalidHooksDir { path: PathBuf },
    #[error("Mad hooks-paths registry has wrong type (expected real file): {path}")]
    InvalidRegistryFile { path: PathBuf },
    #[error("cannot create Mad hooks directory {path}: {source}")]
    CreateHooksDir {
        path: PathBuf,
        #[source]
        source: io::Error,
    },
    #[error("cannot create Mad hooks-paths registry {path}: {source}")]
    CreateRegistryFile {
        path: PathBuf,
        #[source]
        source: io::Error,
...
/// Ensure real `$MAD_HOME/hooks` dir + `hooks-paths` file (create if missing).
/// Race-resistant create (`create_dir` / `create_new`+`O_NOFOLLOW`); never
/// truncates an existing registry; rejects symlinks/wrong types.
pub fn ensure_mad_hook_slots(mad_home: &Path) -> Result<(), GlobalHookSourceError> {
    if path_has_symlink_component(mad_home) {
        return Err(GlobalHookSourceError::SymlinkedMadHome {
            path: mad_home.to_path_buf(),
        });
    }

    match std::fs::create_dir(mad_home) {
        Ok(()) => {}
        Err(e) if e.kind() == io::ErrorKind::AlreadyExists => {}
        Err(e) if e.kind() == io::ErrorKind::NotFound => {
            std::fs::create_dir_all(mad_home).map_err(|source| {
                GlobalHookSourceError::CreateHooksDir {
                    path: mad_home.to_path_buf(),
                    source,
                }
    ...
    let mad_meta = std::fs::symlink_metadata(mad_home).map_err(|source| {
        GlobalHookSourceError::CreateHooksDir {
            path: mad_home.to_path_buf(),
            source,
        }
    })?;
    if mad_meta.file_type().is_symlink() || !mad_meta.file_type().is_dir() {
        return Err(GlobalHookSourceError::SymlinkedMadHome {
            path: mad_home.to_path_buf(),
        });
    }

    let hooks = mad_home.join("hooks");
    ...
    let registry = mad_home.join("hooks-paths");
...
/// Resolve Mad-owned direct global hook sources (`reject_symlinks` for sandbox).
pub fn resolve_global_hook_sources(
    ...
    if let Some(mad) = mad_home {
        if reject_symlinks && path_has_symlink_component(mad) {
            return Err(GlobalHookSourceError::SymlinkedMadHome {
                path: mad.to_path_buf(),
            });
        }

        let hooks = mad.join("hooks");
        let hooks_paths = mad.join("hooks-paths");
        if reject_symlinks {
