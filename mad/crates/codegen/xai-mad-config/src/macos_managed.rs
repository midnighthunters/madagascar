...
//! preference domain `ai.x.mad` (`requirements_toml_base64`). Only admin-*forced*
...
#[cfg(target_os = "macos")]
const MANAGED_PREFERENCES_DOMAIN: &str = "ai.x.mad";
#[cfg(target_os = "macos")]
...
pub const MDM_REQUIREMENTS_SOURCE: &str = "ai.x.mad:requirements_toml_base64";
...
fn read_forced_requirements() -> Option<String> {
    ...
    // per-user domain, which a local user can set (`defaults write ai.x.mad`)
...
mod tests {
    ...
    fn forced_payload_is_not_env_expanded() {
        ...
        let prior = std::env::var("MAD_MDM_NO_EXPAND_TEST").ok();
        unsafe { std::env::set_var("MAD_MDM_NO_EXPAND_TEST", "attacker") };
        let decoded = decode_managed_toml(&b64("base_url = \"${MAD_MDM_NO_EXPAND_TEST}/v1\"\n"));
        unsafe {
            match prior {
                Some(p) => std::env::set_var("MAD_MDM_NO_EXPAND_TEST", p),
                None => std::env::remove_var("MAD_MDM_NO_EXPAND_TEST"),
            }
        }
        assert_eq!(
            decoded
                ...
                .and_then(|v| v.as_str()),
            Some("${MAD_MDM_NO_EXPAND_TEST}/v1"),
