//! Parse marketplace sources from `~/.mad/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
...
/// `MAD_MARKETPLACE_REQUIRE_SHA=1`. Tighten-only: either source can enable,
...
pub fn env_require_sha() -> bool {
    xai_mad_config::env_bool("MAD_MARKETPLACE_REQUIRE_SHA").unwrap_or(false)
}
...
/// instead of `~/.mad`/`~/.claude`. Each root is checked for
...
mod tests {
    ...
    fn require_sha_policy_composition() {
        ...
        unsafe { std::env::remove_var("MAD_MARKETPLACE_REQUIRE_SHA") };
        assert!(!load_require_sha(&empty), "absent everywhere → off");
        assert!(load_require_sha(&enabled), "config alone can enable");

        unsafe { std::env::set_var("MAD_MARKETPLACE_REQUIRE_SHA", "1") };
        assert!(load_require_sha(&empty), "env alone can enable");

        unsafe { std::env::set_var("MAD_MARKETPLACE_REQUIRE_SHA", "0") };
        assert!(
