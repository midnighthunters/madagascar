...
//!
//! Catalog file lookup, in order: `.mad-plugin/marketplace.json` (preferred),
//! `.mad-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.mad-plugin/marketplace.json` (preferred xAI convention)
/// 2. `.mad-plugin/plugin.json`
...
mod tests {
    ...
    fn load_index_valid_mad_dir() {
        ...
        assert_eq!(result.unwrap().name, "mad");
    }

    #[test]
    fn load_index_mad_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".mad-plugin", "mad"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "mad");
    }
