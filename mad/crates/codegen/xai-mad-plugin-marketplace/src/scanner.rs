...
//!    the lookup order — `.mad-plugin/marketplace.json` is preferred), use it.
...
fn scan_single_plugin(plugin_dir: &Path, relative_path: &str) -> MarketplaceEntry {
    ...
    let manifest_result = xai_mad_agent::plugins::manifest::load_manifest(plugin_dir);
    let manifest = match &manifest_result {
        Ok(xai_mad_agent::plugins::manifest::ManifestLoadResult::Found(m)) => Some(m.as_ref()),
        _ => None,
    };
...
mod tests {
    ...
    fn url_sourced_entry_carries_keywords() {
        let dir = tempfile::tempdir().unwrap();
        let mad_dir = dir.path().join(".mad-plugin");
        std::fs::create_dir_all(&mad_dir).unwrap();
        std::fs::write(
            mad_dir.join("marketplace.json"),
            r#"{
                "name": "kw-marketplace",
                "plugins": [{
                    "name": "remote-plugin",
    ...
    #[test]
    fn mad_plugin_dir_index_drives_scan_end_to_end() {
        let dir = tempfile::tempdir().unwrap();
        make_plugin(dir.path(), "mad-plugin", "1.0.0");
        ...
        std::fs::write(
            mad_dir.join("marketplace.json"),
            r#"{
                "name": "mad-marketplace",
                "plugins": [{
                    "name": "mad-plugin",
                    "description": "From the .mad-plugin index",
                    "category": "design",
                    "source": { "type": "local", "path": "./plugins/mad-plugin" },
                    "tags": ["mad"]
                }]
            }"#,
        )
        ...
        let plugins = scan_marketplace(dir.path()).entries;
        assert_eq!(plugins.len(), 1);
        assert_eq!(plugins[0].name, "mad-plugin");
        assert_eq!(plugins[0].category.as_deref(), Some("design"));
        assert_eq!(plugins[0].tags, vec!["mad"]);
        assert!(plugins[0].keywords.is_empty());
    }
    ...
    fn write_mad_file(dir: &Path, file: &str, content: &str) {
        let mad_dir = dir.join(".mad-plugin");
        std::fs::create_dir_all(&mad_dir).unwrap();
        std::fs::write(mad_dir.join(file), content).unwrap();
    }
    ...
    fn url_entry_gets_components_when_sha_matches() {
        ...
        write_mad_file(dir.path(), "plugin-index.json", URL_CATALOG);
    ...
    fn url_entry_without_pinned_sha_gets_no_components() {
        let dir = tempfile::tempdir().unwrap();
        write_mad_file(dir.path(), "marketplace.json", &url_marketplace_index(""));
        write_mad_file(dir.path(), "plugin-index.json", URL_CATALOG);
    ...
    fn malformed_catalog_degrades_to_no_components() {
        ...
        write_mad_file(dir.path(), "plugin-index.json", "not json");
