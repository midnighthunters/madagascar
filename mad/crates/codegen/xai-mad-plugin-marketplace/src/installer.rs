...
use xai_mad_agent::plugins::manifest::{ManifestLoadResult, load_manifest, name_from_dirname};
...
mod tests {
    ...
    fn with_test_registry<T>(f: impl FnOnce(&mut InstallRegistry) -> T) -> T {
        ...
        // process-global `mad_home()` `OnceLock` (first-write-wins). A parallel
        // test in this binary can cache the real `~/.mad` before this runs,
    ...
    fn transactional_update_save_failure_rolls_back_install_and_disk_registry() {
        with_test_registry(|registry| {
            ...
            unsafe { std::env::set_var("XAI_MAD_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE", "1") };
            let result = update_from_marketplace_entry_transactional(
                marketplace.path(),
                &entry,
                provenance(marketplace.path(), "plugins/demo"),
            ...
            unsafe { std::env::remove_var("XAI_MAD_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE") };
