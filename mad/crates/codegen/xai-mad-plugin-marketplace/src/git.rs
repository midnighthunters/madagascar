...
//! Cache root: `~/.mad/marketplace-cache/<url-hash>/`
...
pub fn sync_source_cache_with_mode(
    ...
    let url = xai_mad_agent::plugins::git_install::validate_git_url(url)?;
    let branch = branch
        .map(xai_mad_agent::plugins::git_install::validate_git_ref)
        .transpose()?;
...
pub fn default_cache_root() -> PathBuf {
    xai_mad_config::mad_home().join("marketplace-cache")
}
...
mod tests {
    ...
    #[test]
    fn default_cache_root_under_mad() {
