...
//!
//! Credentials are stored in `$MAD_HOME/mcp_credentials.json`, keyed by a
...
///
/// Local helper (not shell-base): `xai-mad-mcp` sits below `config-types` in the
...
/// File name for the credential store inside `$MAD_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$MAD_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$MAD_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user mad home (set $MAD_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$MAD_HOME/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(xai_mad_config::user_mad_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$MAD_HOME/mcp_credentials.json`. Existing credential files must keep
    ...
    fn save_and_load_from_file() {
        let dir = std::env::temp_dir().join("mad-mcp-credentials-test");
