...
use xai_mad_telemetry::external::{self, ExternalOtelRemotePolicy, IdentityAttrs};
...
const SECRET_MODEL: &str = "mad-4-sk-LEAKmodel1234567890abcd";
...
fn external_stream_gates_on_end_to_end() {
    ...
    xai_mad_telemetry::log_event(xai_mad_telemetry::events::ToolCallCompleted {
    ...
    let harness = col::find_event(&collected, "mad_code.session_start")
        .expect("session_start must be present");
    assert_eq!(harness.scope_name, "ai.xai.mad_code");
    assert_eq!(
        harness
            .resource
            .get("service.name")
            .and_then(|v| v.as_str()),
        Some("mad-cli"),
        "service.name=mad-cli is a wire commitment"
    );
    assert_eq!(
        harness
            .resource
            .get("mad_code.schema.version")
            .and_then(|v| v.as_str()),
        Some("v1")
    );
    ...
    let prompt = col::find_event(&collected, "mad_code.user_prompt").expect("user_prompt present");
    ...
    let tool = col::find_event(&collected, "mad_code.tool_result").expect("tool_result present");
    assert_eq!(
    ...
    let tokens = col::find_metric(&collected, "mad_code.token.usage");
    ...
    let sessions = col::find_metric(&collected, "mad_code.session.count");
