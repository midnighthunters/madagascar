...
fn external_stream_end_to_end() {
    ...
    let mut cfg = xai_mad_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
            "MAD_EXTERNAL_OTEL" => Some("1".into()),
    ...
    cfg.client = xai_mad_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    xai_mad_telemetry::external::init(Some(cfg));
    assert!(xai_mad_telemetry::external::is_active());
    ...
    assert!(!xai_mad_telemetry::is_enabled());
    xai_mad_telemetry::log_event(xai_mad_telemetry::events::SessionNew {
        session_id: "sess-int-1".into(),
        client_identifier: None,
        client_version: None,
        is_git_repo: true,
        permission_mode: xai_mad_telemetry::enums::PermissionMode::Ask,
    });
    xai_mad_telemetry::log_event(xai_mad_telemetry::events::SessionHarness {
        session_id: "sess-int-1".into(),
    ...
    xai_mad_telemetry::log_event(xai_mad_telemetry::events::PromptSubmitted {
    ...
    xai_mad_telemetry::log_event(xai_mad_telemetry::events::ModelResponseReceived {
    ...
    assert!(
        resource_service_name
            .as_deref()
            .is_some_and(|s| s.contains("mad-cli")),
        "service.name=mad-cli is a wire commitment: {resource_service_name:?}"
    );
    for expected in [
        "mad_code.session_start",
        "mad_code.user_prompt",
        "mad_code.api_request",
    ] {
        assert!(
    ...
    assert_eq!(
        event_names
            .iter()
            .filter(|n| *n == "mad_code.session_start")
            .count(),
        1
    );
            ...
            for sm in &rm.scope_metrics {
                for metric in &sm.metrics {
                    ...
                    if let Some(Data::Sum(sum)) = &metric.data {
                        ...
                        if metric.name == "mad_code.session.count" {
    ...
    assert!(
        metric_names.iter().any(|n| n == "mad_code.session.count"),
        "missing session.count in {metric_names:?}"
    );
    assert!(metric_names.iter().any(|n| n == "mad_code.token.usage"));
    assert_eq!(
    ...
    xai_mad_telemetry::external::shutdown();
    assert!(
