...
//! file logger that writes to `~/.mad/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `xai_mad_hooks` or `xai_mad_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! MAD_HOOKS_LOG=1 mad              # enable, write to ~/.mad/logs/hooks.log
//! MAD_HOOKS_LOG=/tmp/h.log mad     # write to custom path
//! MAD_HOOKS_LOG=0 mad              # explicitly disable
//! tail -f ~/.mad/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "MAD_HOOKS_LOG";
...
///
/// Writes to `~/.mad/logs/hooks.log` (or custom path via `MAD_HOOKS_LOG`).
/// Filters to hooks (`xai_mad_hooks`) and plugins (`xai_mad_agent::plugins`) targets.
/// Set `MAD_HOOKS_LOG=0` to disable, `MAD_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "xai_mad_hooks=debug,xai_mad_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || mad_home().join("logs").join("hooks.log");
