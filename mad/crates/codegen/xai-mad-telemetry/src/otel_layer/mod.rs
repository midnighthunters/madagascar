...
//!
//! `xai-mad-pager` uses this module to set up OTLP
...
const ENV_OTEL_FILTER: &str = "MAD_OTEL_FILTER";
...
/// (`AuthManager`, `EndpointsConfig`, `MadComConfig`).
///
/// Built by the binaries (`xai-mad-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `xai_mad_shell::auth::credential_provider::build_otel_credential_provider`.
pub struct OtelLayerConfig {
    ...
    /// Value for the `X-XAI-Token-Auth` header (typically `"xai-mad-cli"`).
...
pub struct OtelClientInfo {
    /// Binary name (`mad-pager`) -> `client.name`.
...
pub struct OtelExporterConfig {
    /// Full OTLP traces endpoint URL (e.g. `https://cli-chat-proxy.mad.com/v1/traces`).
...
///
/// - `client_name`: binary name (e.g. `"mad-tui"`, `"mad-pager"`) -- stored as
///   `client.name` resource attribute for dashboards to distinguish client types.
/// - `client_version`: `CARGO_PKG_VERSION` -- sent in the `x-mad-client-version` header.
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let tracer = provider.tracer("mad-cli");
    ...
    let otel_filter = tracing_subscriber::filter::EnvFilter::try_new(&otel_filter)
        .unwrap_or_else(|e| {
            eprintln!(
                "[otel] Invalid MAD_OTEL_FILTER '{}': {}. Using default '{}'.",
...
fn build_export_headers(
    ...
    snapshot: &xai_mad_auth::CredentialSnapshot,
...
fn build_server_provider(client: OtelClientInfo, config: OtelLayerConfig) -> SdkTracerProvider {
    ...
    if config.exporter.enabled {
        ...
        static_headers.insert(
            "x-mad-client-version".to_string(),
...
mod tests {
    ...
    use std::sync::Mutex;
    use xai_mad_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
    ...
    fn make_exporter(
        ...
        RefreshableSpanExporter {
            ...
            token_header_value: Arc::from("xai-mad-cli"),
