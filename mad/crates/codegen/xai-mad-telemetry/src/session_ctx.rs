...
//!
//! Extracted from `xai-mad-shell::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `xai-mad-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `xai-mad-workspace` (remote sampler / workspace server).
    Workspace,
}
...
impl EmitterOrigin {
    ...
    pub fn event_prefix(self) -> &'static str {
        match self {
            EmitterOrigin::Shell => "mad-shell-",
            EmitterOrigin::Workspace => "mad-workspace-",
        }
    }
}
...
/// event-name prefix (`mad-shell-*` vs `mad-workspace-*`).
///
...
mod tests {
    ...
    fn event_prefix_is_stable_per_origin() {
        assert_eq!(EmitterOrigin::Shell.event_prefix(), "mad-shell-");
        assert_eq!(EmitterOrigin::Workspace.event_prefix(), "mad-workspace-");
    }

    /// The `Shell` reroute must reproduce the historical
    /// `format!("mad-shell-{suffix}")` event name byte-for-byte, since every
    ...
    fn shell_origin_event_name_matches_legacy_format() {
        ...
        let legacy = format!("mad-shell-{suffix}");
    ...
    fn workspace_origin_event_name_uses_workspace_prefix() {
        ...
        assert_eq!(name, "mad-workspace-turn");
    }
