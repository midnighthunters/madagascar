...
impl ExternalEventName {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SessionStart => "mad_code.session_start",
            Self::SessionEnd => "mad_code.session_end",
            Self::UserPrompt => "mad_code.user_prompt",
            Self::TurnCompleted => "mad_code.turn_completed",
            Self::ApiRequest => "mad_code.api_request",
            Self::ApiError => "mad_code.api_error",
            Self::ToolResult => "mad_code.tool_result",
            Self::ToolDecision => "mad_code.tool_decision",
            Self::McpServerConnection => "mad_code.mcp_server_connection",
            Self::PermissionModeChanged => "mad_code.permission_mode_changed",
            Self::SkillActivated => "mad_code.skill_activated",
            Self::PluginLoaded => "mad_code.plugin_loaded",
            Self::Compaction => "mad_code.compaction",
            Self::Subagent => "mad_code.subagent",
            Self::Auth => "mad_code.auth",
            Self::InternalError => "mad_code.internal_error",
            Self::ModelSwitched => "mad_code.model_switched",
            Self::ContextualTip => "mad_code.contextual_tip",
        }
    }
}
...
pub enum MetricIncrement {
    /// `mad_code.session.count`.
    SessionCount,
    /// `mad_code.token.usage` — `token_type` ∈ `input|output|reasoning|cache_read`.
    TokenUsage {
...
pub(crate) const METRIC_SESSION_COUNT: &str = "mad_code.session.count";
pub(crate) const METRIC_TOKEN_USAGE: &str = "mad_code.token.usage";
pub(crate) const METRIC_TURN_COUNT: &str = "mad_code.turn.count";
pub(crate) const METRIC_TOOL_DECISION: &str = "mad_code.tool.decision";
pub(crate) const METRIC_TOOL_USAGE: &str = "mad_code.tool.usage";
pub(crate) const METRIC_ERROR_COUNT: &str = "mad_code.error.count";
...
/// Built-in tool names (closed enum surface in `xai-mad-tools`) that pass
...
/// `SessionHarness` → `mad_code.session_start`. Emitted from a spawn outside
...
/// `SessionNew` → `mad_code.session.count` (metric only; the flagship
...
/// `SessionEnded` → `mad_code.session_end`.
...
/// `PromptSubmitted` → `mad_code.user_prompt`. Prompt text rides the
...
/// `TurnCompleted` → `mad_code.turn_completed` + `turn.count`
...
/// `ModelResponseReceived` → `mad_code.api_request` + `token.usage`.
...
/// `RateLimitHit` → `mad_code.api_error` (`error_category = rate_limit`).
...
/// `ApiError` → `mad_code.api_error`. Category/class enums only — no
...
/// `ToolCallCompleted` → `mad_code.tool_result` + `tool.usage`.
...
/// `PermissionDecisionPayload` → `mad_code.tool_decision` + `tool.decision`.
...
/// `McpServerConnected` → `mad_code.mcp_server_connection` (`status=connected`).
...
/// `McpServerFailed` → `mad_code.mcp_server_connection` (`status=failed`).
...
/// `PlanModeToggled` → `mad_code.permission_mode_changed`.
...
/// `ContextualTip` → `mad_code.contextual_tip`. Labels only (no user
...
/// `YoloToggled` → `mad_code.permission_mode_changed`.
...
/// `SkillDispatched` → `mad_code.skill_activated`. Skill names are
...
/// `PluginInstalled` → `mad_code.plugin_loaded`.
...
/// `PluginUsed` → `mad_code.plugin_loaded` (plugin name details-gated).
...
/// `CompactionCompleted` → `mad_code.compaction`.
...
/// `CompactionTriggered` → `mad_code.compaction` trigger attrs ride on the
/// completion event instead (one event per compaction); not mapped.
/// `SubagentLaunched` → `mad_code.subagent` (`phase=launched`).
...
/// `SubagentCompleted` → `mad_code.subagent` (`phase=completed`).
...
/// `InternalError` → `mad_code.internal_error`. Error class only — no
...
/// `ModelSwitched` → `mad_code.model_switched`.
