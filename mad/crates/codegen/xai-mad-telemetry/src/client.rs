...
//!
//! Extracted from `xai-mad-shell::agent::telemetry::track`. The HTTP client is
...
/// whichever [`EmitterOrigin`] prefix it carries (`mad-shell-` /
/// `mad-workspace-`). Unprefixed names pass through unchanged. Kept in
...
/// format for Mixpanel. Handles both CCP display names ("SuperMad Heavy")
/// and JWT-derived keys ("supermad_heavy").
fn normalize_tier(tier: &str) -> String {
    match tier {
        "SuperMad Heavy" | "supermad_heavy" => "supermad_heavy",
        "SuperMad Plus" | "supermad_plus" => "supermad_plus",
        "SuperMad" | "supermad" => "supermad",
        "SuperMad Lite" | "supermad_lite" => "supermad_lite",
...
pub async fn track(event_name: &str, request_id: &str, ctx: &UserContext, mut metadata: Metadata) {
    ...
    if let Some(ref mixpanel) = client.mixpanel {
        ...
        props.insert("app_name".into(), json!("Mad Code"));
...
mod tests {
    ...
    /// identical to the previous `strip_prefix("mad-shell-")` behavior.
    #[test]
    fn event_value_strips_shell_prefix() {
        assert_eq!(event_value("mad-shell-turn"), "turn");
        assert_eq!(
            event_value("mad-shell-trace_upload_attempted"),
    ...
    fn event_value_strips_workspace_prefix() {
        assert_eq!(event_value("mad-workspace-turn"), "turn");
    }
    ...
    fn event_value_strips_only_leading_prefix() {
        assert_eq!(event_value("mad-shell-workspace-x"), "workspace-x");
    }
    ...
    fn normalize_tier_maps_display_and_claim_names() {
        ...
        assert_eq!(normalize_tier("free"), "free");
        assert_eq!(normalize_tier("SuperMad"), "supermad");
        assert_eq!(normalize_tier("SuperMad Heavy"), "supermad_heavy");
        assert_eq!(normalize_tier("supermad_heavy"), "supermad_heavy");
        ...
        assert_eq!(normalize_tier("SuperMad Lite"), "supermad_lite");
        assert_eq!(normalize_tier("SuperMad Plus"), "supermad_plus");
        assert_eq!(normalize_tier("supermad_plus"), "supermad_plus");
    ...
    /// (e.g. a future `"mad-shell-ext-"` origin), an earlier `ALL` entry could
