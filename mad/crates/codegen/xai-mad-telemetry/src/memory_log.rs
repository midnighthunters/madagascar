...
//! file logger that writes to `~/.mad/logs/memory.log`.
//!
//! ## When to use
//!
...
//! MAD_MEMORY_LOG=0 mad                # disable even when enabled
//! tail -f ~/.mad/logs/memory.log      # watch in another terminal
//! ```
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "MAD_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.mad/logs/memory.log`. Filters to `xai_memory=trace`.
    /// Set `MAD_MEMORY_LOG=0` to disable, `MAD_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || mad_home().join("logs").join("memory.log");
