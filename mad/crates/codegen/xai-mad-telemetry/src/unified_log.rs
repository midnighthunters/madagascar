...
pub enum LogSource {
    ...
    #[serde(rename = "shell")]
    Shell,
    #[strum(serialize = "mad-pager")]
    #[serde(rename = "mad-pager")]
    MadPager,
    #[strum(serialize = "mad-desktop")]
    #[serde(rename = "mad-desktop")]
    MadDesktop,
}
...
fn log_path() -> PathBuf {
    mad_home().join(LOG_DIR).join(LOG_FILE)
}
...
/// healing a stale handle rather than re-resolving `$MAD_HOME` — which also
