//! Tracing layer for `target: "sampling_log"` → `~/.mad/logs/sampling.jsonl`.
//! Enable with `--log-sampling` or `MAD_LOG_SAMPLING=1`.
...
const ENV_VAR: &str = "MAD_LOG_SAMPLING";
    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync + 'static,
{
    ...
    let path = mad_home().join(crate::unified_log::LOG_DIR).join(LOG_FILE);
