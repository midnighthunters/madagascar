...
//!
//! These structs were extracted from `xai-mad-shell` so they can be
...
pub enum ContextualTipKind {
    ...
    WordSelect,
    /// SSH session without `mad wrap` → tip to wrap the ssh command locally.
    SshWrap,
}
...
// ---------------------------------------------------------------------------
// Pager events (called from xai-mad-pager via log_event)
// ---------------------------------------------------------------------------
...
/// first prompt when Madagascar Build is launched from a non-project directory).
...
#[derive(Serialize)]
pub struct SuperMadUpsellShown {
    pub source: SuperMadUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}

#[derive(Serialize)]
pub struct SuperMadUpsellClicked {
    pub source: SuperMadUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}
...
/// Modeled on [`SuperMadUpsell`]; lets the funnel attribute the click to the
...
pub struct ClipboardCopy {
    ...
    /// An explicit `mad wrap` OSC 52 sink was active.
    pub osc52_sink: bool,
...
/// is lifted — i.e. they subscribed (externally on mad.com) and came back.
/// This is the actual conversion signal for SuperMad Heavy subscriptions
/// attributed to Madagascar Build: the user saw the gate in Madagascar Build, went and
...
pub struct SubscriptionActivated {
    ...
    /// is strongly attributable to Madagascar Build's upsell surface.
...
/// `mad-shell-manual_auth` (the `manual_auth` binding gets the `mad-shell-`
...
telemetry_event!(SuperMadUpsellShown, "supermad_upsell_shown");
telemetry_event!(SuperMadUpsellClicked, "supermad_upsell_clicked");
