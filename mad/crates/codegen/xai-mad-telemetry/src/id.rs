...
//!
//! Extracted from `xai-mad-shell::agent::unique_identifier` so the
//! telemetry engine can stamp events without depending on shell internals.
//! `$MAD_HOME` is resolved through `xai-mad-config::mad_home`.
...
/// This function caches the result in `$MAD_HOME/agent_id` so subsequent calls
...
fn load_or_compute_agent_id() -> String {
    let cache_path = xai_mad_config::mad_home().join("agent_id");
...
/// Write `$MAD_HOME/agent_id` as owner-read/write only (Unix 0o600) — it is a
...
fn write_agent_id_cache(path: &std::path::Path, id: &str) -> std::io::Result<()> {
    ...
    xai_mad_config::fs_atomic::write_atomically(path, id, Some(0o600))
}
...
///
/// Enabled only when `MAD_TELEMETRY_SPECIAL_USER=1` (or `true`). There is no
/// hardcoded username allowlist.
pub fn is_special_user() -> bool {
    matches!(
        std::env::var("MAD_TELEMETRY_SPECIAL_USER").as_deref(),
