//! Integration tests for xai-mad-hooks.
//!
...
use xai_mad_hooks::discovery::load_hooks;
use xai_mad_hooks::dispatcher;
use xai_mad_hooks::event::*;
use xai_mad_hooks::result::HookDecision;
use xai_mad_hooks::runner::RunContext;
...
async fn non_blocking_dispatch() {
    ...
    assert!(matches!(
        &results[0],
        xai_mad_hooks::result::HookRunResult::Success { .. }
    ));
}
    ...
    let cases = vec![
        ...
        Case {
            ...
            payload: HookPayload::StopFailure {
                error: xai_mad_hooks::event::StopFailureKind::RateLimit,
...
/// runner-reserved keys (`MAD_HOOK_EVENT`, `MAD_HOOK_NAME`,
/// `MAD_SESSION_ID`, `MAD_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$MAD_HOOK_EVENT" > {f}; echo "NAME=$MAD_HOOK_NAME" >> {f}; echo "SESSION=$MAD_SESSION_ID" >> {f}; echo "ROOT=$MAD_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "MAD_HOOK_EVENT": "spoofed_event",
                                "MAD_HOOK_NAME": "spoofed_name",
                                "MAD_SESSION_ID": "spoofed_session",
                                "MAD_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "MAD_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed MAD_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "MAD_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed MAD_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "MAD_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed MAD_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
...
async fn http_hook_url_env_expansion_end_to_end() {
    ...
    use xai_mad_hooks::result::HookRunResult;
    let info = match run {
