...
use xai_mad_tools::util::ProcessGroup;
...
pub async fn run_command_hook(
    ...
    let debug_payloads = std::env::var("MAD_HOOK_DEBUG").is_ok_and(|v| v == "1");
    if debug_payloads {
        tracing::trace!(
            hook_name = %spec.name,
    ...
    let mut cmd = if is_shell_command {
        ...
        #[cfg(not(unix))]
        {
            let inv = xai_mad_config::shell::shell_command_argv(&command_str);
    ...
    // plugin) can spoof `MAD_HOOK_EVENT`, `MAD_HOOK_NAME`, `MAD_SESSION_ID`,
    // `MAD_WORKSPACE_ROOT`, or `CLAUDE_PROJECT_DIR`, which are the
    ...
    let mut child = match cmd
        ...
        .env("MAD_HOOK_EVENT", envelope.hook_event_name.to_string())
        .env("MAD_HOOK_NAME", &spec.name)
        .env("MAD_SESSION_ID", ctx.session_id)
        .env("MAD_WORKSPACE_ROOT", ctx.workspace_root)
        // Compatibility alias for external hooks that read this env name.
        // Same value as `MAD_WORKSPACE_ROOT`; native `.mad` hooks should use
        // `MAD_WORKSPACE_ROOT`.
...
///   hooks),
/// * the Mad process's own environment (which is inherited by the child),
...
mod tests {
    ...
    fn resolve_command_path_variants() {
        ...
        assert_eq!(
            ...
            Some(std::path::PathBuf::from(
                "/project/.mad/hooks/bin/check.sh"
            ))
        );
        assert_eq!(
    ...
    fn make_scoped_ctx(scope: xai_mad_tools::util::ProcessScope) -> RunContext<'static> {
        RunContext {
    ...
    fn find_unresolved_skips_resolvable_vars() {
        ...
        let v = find_unresolved_env_vars(
            "${MAD_HOOK_EVENT}/${CLAUDE_PROJECT_DIR}/${MAD_SESSION_ID}/${CLAUDE_PLUGIN_ROOT}/foo",
            &env,
        );
        assert!(
            v.is_empty(),
    ...
    async fn test_tilde_expansion_runs_via_shell() {
        let tmp = tempfile::tempdir().unwrap();
        // Create the script at <tmp>/.mad-test-hooks-gb856/tilde-test.sh
        let hook_dir = tmp.path().join(".mad-test-hooks-gb856");
        ...
        // Inject HOME via extra_env so `sh -c "~/.mad-test-hooks-gb856/..."`
        ...
        let spec = HookSpec {
            ...
            enabled: true,
            command: Some(std::path::PathBuf::from(
                "~/.mad-test-hooks-gb856/tilde-test.sh",
            )),
            command_raw: Some("~/.mad-test-hooks-gb856/tilde-test.sh".to_string()),
            url: None,
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::env::temp_dir(),
    ...
    async fn command_hook_session_close_reaps_whole_group() {
        ...
        let envelope = make_envelope();
        let scope = xai_mad_tools::util::ProcessScope::new();
