...
// (`~/.mad/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_mad_home>/trusted-hook-projects`), or `None` when no user mad home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(xai_mad_config::user_mad_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user mad home (set $MAD_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$MAD_HOME/disabled-hooks`, or `None` when no user mad
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(xai_mad_config::user_mad_home()?.join("disabled-hooks"))
}
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        let mad_dir = dir.join(".mad");
        std::fs::create_dir_all(&mad_dir).unwrap();
        mad_dir.join("trusted-hook-projects")
    }
