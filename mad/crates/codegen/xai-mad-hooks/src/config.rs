...
pub use xai_mad_config::HookProvenance;
...
pub fn parse_hooks_from_config_layers(
    layers: &[xai_mad_config::HookConfigLayer],
) -> (Vec<HookSpec>, Vec<HookError>) {
    let home = xai_mad_config::user_mad_home();
...
mod tests {
    ...
    fn config_layer(source_name: &str, toml_src: &str) -> xai_mad_config::HookConfigLayer {
        let value: toml::Value = toml::from_str(toml_src).unwrap();
        let hooks = value.get("hooks").cloned().unwrap();
        xai_mad_config::HookConfigLayer::new(
            xai_mad_config::HookProvenance::Managed,
            source_name,
            hooks,
        )
    }
    ...
    fn config_layers_additive_and_dedup_keeps_higher_authority() {
        ...
        use xai_mad_config::HookProvenance::{Managed, User};
    ...
    fn source_dir_from_file_path() {
        ...
        let (specs, _) = parse_hook_file(json, Path::new("/home/user/.mad/hooks/safety.json"));
        assert_eq!(specs[0].source_dir, PathBuf::from("/home/user/.mad/hooks"));
    }
    ...
    fn parse_hook_file_expands_env_var_in_command_from_process_env() {
        let key = "MAD_HOOKS_PARSE_TEST_CMD_PROC_ENV";
    ...
    fn parse_hook_file_expands_env_var_in_url_from_process_env() {
        let key = "MAD_HOOKS_PARSE_TEST_URL_PROC_ENV";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_command() {
        let key = "MAD_HOOKS_PARSE_TEST_NEVER_SET_AT_LOAD_TIME";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_url() {
        let key = "MAD_HOOKS_PARSE_TEST_URL_NEVER_SET_AT_LOAD_TIME";
        with_env_var(key, None, || {
            let json = format!(
                r#"{{
                    "hooks": {{
    ...
    fn parse_hook_file_matcher_is_not_env_expanded() {
        let key = "MAD_HOOKS_PARSE_TEST_MATCHER_VAR";
                        ...
                        "hooks": [
                            {
                                ...
                                "command": "echo hi",
                                "env": {
                                    "MAD_HOOK_EVENT": "spoofed",
                                    "MAD_HOOK_NAME": "spoofed",
                                    "MAD_SESSION_ID": "spoofed",
                                    "MAD_WORKSPACE_ROOT": "/etc",
