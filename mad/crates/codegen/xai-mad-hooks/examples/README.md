...
Sample hooks for Mad. Copy to `~/.mad/hooks/` to enable globally, or to `<project>/.mad/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.mad/hooks/bin
cp examples/hooks/safe-shell.json ~/.mad/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.mad/hooks/bin/
chmod +x ~/.mad/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.mad/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.mad/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.mad/hooks/bin/
chmod +x ~/.mad/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.mad/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.mad/hooks/bin
cp examples/hooks/session-log.json ~/.mad/hooks/
cp examples/hooks/bin/session-log.sh ~/.mad/hooks/bin/
chmod +x ~/.mad/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.mad/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.mad/hooks/bin
cp examples/hooks/tool-logger.json ~/.mad/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.mad/hooks/bin/
chmod +x ~/.mad/hooks/bin/tool-logger.sh
```
...
```sh
mkdir -p ~/.mad/hooks/bin
cp examples/hooks/stop-verify.json ~/.mad/hooks/
cp examples/hooks/bin/stop-verify.sh ~/.mad/hooks/bin/
chmod +x ~/.mad/hooks/bin/stop-verify.sh
```
...
- **Event names:** `SessionStart`, `PreToolUse`, `PostToolUse`, `Stop`, `SubagentStop`, `SessionEnd` (see the [user guide](../../xai-mad-pager/docs/user-guide/10-hooks.md) for the full set)
- **Matcher:** regex on tool name. Claude names like `Bash`, `Read`, `Edit` are auto-expanded to also match Mad names (`run_terminal_cmd`, `read_file`, `search_replace`)
...
Remove the JSON file from `~/.mad/hooks/`. The hook stops running on the next session.