...
pub struct ConversationRequest {
    ...
    pub x_mad_conv_id: Option<String>,
    pub x_mad_req_id: Option<String>,
    pub x_mad_session_id: Option<String>,
    pub x_mad_turn_idx: Option<String>,
    pub x_mad_agent_id: Option<String>,
    pub x_mad_deployment_id: Option<String>,
    pub x_mad_user_id: Option<String>,
    ...
    /// Sticky routing key for prompt-cache reuse; overrides `x_mad_conv_id` for routing.
...
pub struct ConversationResponse {
    ...
    /// only, opt-in via the `x-mad-doom-loop-check` header). Empty when the
...
// ---------------------------------------------------------------------------
//
// Part of the Mad Compaction unification. Lets the shared, transport-agnostic
// engine in `crates/common/xai-mad-compaction` operate over mad-build's
...
impl xai_mad_compaction::CompactionItem for ConversationItem {
    fn role(&self) -> xai_mad_compaction::CompactionRole {
        use xai_mad_compaction::CompactionRole;
        // mad-build has no distinct `Developer` role; everything maps onto
    ...
    fn is_compaction_summary(&self) -> bool {
        // mad-build has no structural marker that uniquely identifies a prior
        ...
        // (add a dedicated marker) before routing mad-build history through
        // the shared `history`/`inter` filter.
        false
    }

    fn attachment_refs(&self) -> Vec<xai_mad_compaction::CompactionFileRef> {
        // mad-build `UserItem`s carry only `Text`/`Image { url }` content
        // parts — there is no id+name attachment-ref concept like the chat harness's
        // `MadTurn`. The full-replace path does not read this; revisit if
        // image attachments need to survive into the `<mad_user_queries>`
        // preamble.
        Vec::new()
    }
}

impl xai_mad_compaction::CompactionItemFactory for ConversationItem {
...
/// ([stream/messages.rs:340](crates/codegen/xai-mad-sampler/src/stream/messages.rs))
...
///
/// This is `pub` so downstream crates (`xai-mad-shell`) can re-use it
...
impl From<ChatResponseMessage> for ConversationItem {
    fn from(msg: ChatResponseMessage) -> Self {
        ...
        // ([crates/codegen/xai-mad-sampler/src/stream/chat_completions.rs])
...
impl From<ConversationRequest> for ChatCompletionRequest {
    fn from(req: ConversationRequest) -> Self {
        ...
        ChatCompletionRequest {
            ...
            x_mad_conv_id: req.x_mad_conv_id,
            x_mad_req_id: req.x_mad_req_id,
            x_mad_session_id: req.x_mad_session_id,
            x_mad_turn_idx: req.x_mad_turn_idx,
            x_mad_agent_id: req.x_mad_agent_id,
            x_mad_deployment_id: req.x_mad_deployment_id,
            x_mad_user_id: req.x_mad_user_id,
            trace: None,
        }
    }
}
...
impl ConversationRequest {
    ...
    pub fn with_conv_id(mut self, conv_id: impl Into<String>) -> Self {
        self.x_mad_conv_id = Some(conv_id.into());
        self
    }

    /// Set request ID header
    pub fn with_req_id(mut self, req_id: impl Into<String>) -> Self {
        self.x_mad_req_id = Some(req_id.into());
        self
    }
...
pub fn build_messages_request(req: &ConversationRequest) -> crate::messages::MessagesRequest {
    ...
    // output without tools. The mad-shell agent does NOT use this path — a
...
/// ([crates/codegen/xai-mad-sampler/src/stream/messages.rs]) instead
...
mod compaction_item_bridge_tests {
    use super::*;
    use xai_mad_compaction::{CompactionItem, CompactionItemFactory, CompactionRole};
    ...
    fn metadata_accessors_are_conservative() {
        // mad-build has no structural compaction-summary marker, and no
...
mod tests {
    ...
    fn test_conversation_item_roundtrip() {
        ...
        let assistant = ConversationItem::assistant_with_model("Hi there!", "mad-3");
    ...
    fn test_conversation_request_to_chat_completion() {
        ...
        assert_eq!(chat_req.model, Some("mad-3".to_string()));
    ...
    fn test_conversation_request_to_responses_api() {
        ...
        assert_eq!(responses_req.model, Some("mad-3".to_string()));
    ...
    fn test_responses_api_response_to_conversation_item() {
        ...
        assert_eq!(a.model_id, Some("mad-3".to_string()));
        assert_eq!(
    ...
    fn test_assistant_with_content_and_tool_calls() {
        ...
        assert_eq!(chat_msg.tool_calls.len(), 1);
        assert_eq!(chat_msg.model_id, Some("mad-3".to_string()));
    }
    ...
    fn test_transform_cwd_transforms_tool_call_arguments() {
        ...
        let worktree = "/home/user/.mad/worktrees/project/ab-uuid-a";
    ...
    fn test_transform_cwd_worktree_to_root_syncback() {
        ...
        let worktree = "/home/user/.mad/worktrees/myproject/fork-a";
    ...
    fn test_transform_cwd_assistant_only_tool_calls_no_content() {
        ...
        let worktree = "/home/user/.mad/worktrees/proj/fork-a";
    ...
    fn test_conversation_request_builder() {
        ...
        assert_eq!(req.model, Some("mad-3".to_string()));
        assert_eq!(req.temperature, Some(0.5));
        assert_eq!(req.max_output_tokens, Some(1000));
        assert_eq!(req.x_mad_conv_id, Some("conv-123".to_string()));
        assert_eq!(req.x_mad_req_id, Some("req-456".to_string()));
    ...
    fn test_assistant_item_with_model_id() {
        ...
        assert_eq!(item.model_id, Some("mad-3".to_string()));
    }

    #[test]
    fn test_conversation_item_with_model_id() {
        let item = ConversationItem::assistant("Hello").with_model_id("mad-3");
        ...
        let user = ConversationItem::user("Hi").with_model_id("mad-3");
    ...
    fn responses_api_conversion_preserves_model_fingerprint() {
        ...
        assert_matches!(item, ConversationItem::Assistant(ref a) => {
            ...
            assert_eq!(a.model_id.as_deref(), Some("mad-4.5"));
    ...
    //      (earlier mad-build / chat-completions written as v1)
    ...
    #[test]
    fn upgrade_legacy_reasoning_singular_mad_build_shape() {
