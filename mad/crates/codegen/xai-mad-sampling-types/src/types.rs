...
// Re-export ToolDefinition and FunctionTool from xai-mad-tools.
...
pub use xai_mad_tools::types::definition::{FunctionTool, ToolDefinition};
...
mod tests {
    ...
    fn parse_reasoning_efforts_meta_absent_is_none() {
        ...
        assert!(
            parse_reasoning_efforts_meta(Some(json!({ "agentType": "mad" }).as_object().unwrap()))
                .is_none()
        );
    }
