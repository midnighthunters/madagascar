//! Minimal (scrollback-native) render mode — `mad --minimal`.
//!
...
//! # Wiring
//!
//! `xai-mad-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`xai_mad_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`xai-mad-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `xai-mad-pager`.
///
...
/// subsequent calls are ignored (see [`xai_mad_pager::minimal_hook`]).
pub fn install() {
    xai_mad_pager::minimal_hook::install(xai_mad_pager::minimal_hook::MinimalHooks { draw });
}
