...
/// Directory (under `<cwd>/.mad`) holding every session's checkpoint store.
...
pub(crate) struct CheckpointStore {
    /// Per-session store directory: `<cwd>/.mad/rewind-checkpoints/<session_id>`.
    dir: PathBuf,
