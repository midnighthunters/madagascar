...
use std::sync::Arc;
use xai_mad_tools::registry::types::{
...
pub(crate) fn resolve_session_toolset(
    ...
    notification_handle: Option<xai_mad_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<xai_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<xai_mad_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn xai_mad_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            xai_mad_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
/// enabled (`MAD_WORKSPACE_TOOL_STATE_ENABLED=true`; any other value keeps
/// legacy behavior).
pub fn tool_state_enabled() -> bool {
    std::env::var("MAD_WORKSPACE_TOOL_STATE_ENABLED").as_deref() == Ok("true")
}
...
/// `MAD_WORKSPACE_TOOL_STATE_ENABLED`. Aliased to the crate-wide
...
/// [`LocalTerminalBackend`]: xai_mad_tools::computer::local::LocalTerminalBackend
pub struct WorkspaceSessionContextFactory {
    auth: Option<xai_computer_hub_sdk::SharedAuthProvider>,
    api_base_url: Option<String>,
    /// Resolved `$MAD_WORKSPACE_HOME` when tool-state persistence is enabled;
...
impl WorkspaceSessionContextFactory {
    ...
    /// (`$MAD_WORKSPACE_HOME`). Callers should only invoke this when
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let fs = Arc::new(xai_mad_tools::computer::local::LocalFs)
            as Arc<dyn xai_mad_tools::computer::types::AsyncFileSystem>;
        let notification_handle = xai_mad_tools::notification::ToolNotificationHandle::noop();
    ...
    fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
        crate::config::SessionTerminalBackend::local(
            xai_mad_tools::computer::local::LocalTerminalBackend::new(),
        )
    }
...
fn build_proxy_headers(base_url: &str) -> indexmap::IndexMap<String, String> {
    ...
    let version = xai_mad_version::VERSION;
    headers.insert(
        "user-agent".to_string(),
        format!("xai-mad-workspace/{version}"),
    );
    headers.insert("x-mad-client-version".to_string(), version.to_string());
    headers.insert(
        "x-mad-client-identifier".to_string(),
        std::env::var("MAD_CLIENT_NAME").unwrap_or_else(|_| "mad-shell".to_string()),
    );
    if base_url.contains("cli-chat-proxy") || base_url.contains("chat-proxy") {
        headers.insert("X-XAI-Token-Auth".to_string(), "xai-mad-cli".to_string());
        headers.insert(
...
/// `MAD_DISABLE_WEB_FETCH=1` is set.
fn build_web_fetch_config() -> xai_mad_tools::implementations::mad_build::web_fetch::WebFetchConfig
{
    use xai_mad_tools::implementations::mad_build::web_fetch::{WebFetchConfig, WebFetchParams};
    if std::env::var("MAD_DISABLE_WEB_FETCH").is_ok_and(|v| v == "1" || v == "true") {
        return WebFetchConfig::Disabled;
    }
    let mut params = WebFetchParams::default();
    if let Ok(proxy) = std::env::var("MAD_WEB_FETCH_PROXY") {
        params.proxy_endpoint = Some(proxy);
    }
    if xai_mad_config::env_bool("MAD_WEB_FETCH_ALLOW_LOCAL") == Some(true) {
        params.allow_local = Some(true);
    }
    WebFetchConfig::Enabled { params }
}
fn default_web_search_model() -> String {
    std::env::var("MAD_WEB_SEARCH_MODEL").unwrap_or_else(|_| "mad-4.5".to_string())
}
    ...
    pub fn baseline_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("MadagascarBuild:search_replace", Some(ToolKind::Edit)),
                tc("MadagascarBuild:grep", Some(ToolKind::Search)),
...
mod tests {
    ...
    async fn resolve_session_toolset_mcp_merge_dedup_by_id_baseline_wins() {
        ...
        let mut mcp_dup = test_support::tc("MadagascarBuild:read_file", Some(ToolKind::Read));
    ...
    fn backfill_tool_kinds_fills_known_kindless_ids_only() {
        let kinds = HashMap::from([
            ("MadagascarBuild:search_replace".to_owned(), ToolKind::Edit),
            ("MadagascarBuild:read_file".to_owned(), ToolKind::Read),
        ]);
        let config = ToolServerConfig {
            tools: vec![
                test_support::tc("MadagascarBuild:search_replace", None),
                test_support::tc("adhoc.opaque", None),
                // Pre-set kinds must never be overwritten by the registry.
                test_support::tc("MadagascarBuild:read_file", Some(ToolKind::Search)),
            ],
        ...
        assert_eq!(kind_of("MadagascarBuild:search_replace"), Some(ToolKind::Edit));
        assert_eq!(
        ...
        assert_eq!(
            kind_of("MadagascarBuild:read_file"),
    ...
    async fn resolve_session_toolset_readonly_filters_kindless_pinned_tools() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("MadagascarBuild:read_file", None),
                test_support::tc("MadagascarBuild:grep", None),
                test_support::tc("MadagascarBuild:list_dir", None),
                test_support::tc("MadagascarBuild:search_replace", None),
                test_support::tc("MadagascarBuild:run_terminal_cmd", None),
            ],
            behavior_preset: None,
        };
        let (eff, ts, _backend) = resolve_session_toolset(
    ...
    async fn resolve_session_toolset_mcp_kind_none_dropped_under_readonly() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("MadagascarBuild:read_file", Some(ToolKind::Read)),
        ...
        assert!(
            kept_ids.contains(&"MadagascarBuild:read_file"),
    ...
    fn tool_state_enabled_only_true_enables() {
        ...
        let var = "MAD_WORKSPACE_TOOL_STATE_ENABLED";
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use xai_mad_tools::types::resources::{State, WebCitationCounter};
