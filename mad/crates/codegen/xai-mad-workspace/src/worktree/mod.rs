...
//!
//! Moved from `xai-mad-shell/src/session/worktree.rs` into the workspace
...
// Canonical in xai-mad-workspace-types; re-exported for existing paths.
pub use xai_mad_workspace_types::rpc::worktree::{
...
/// Resolve the mad home for worktree paths via the **same** resolver used for
/// `worktrees.db` (`xai_fast_worktree::resolve_mad_home`), so checkout dirs and
/// the metadata DB always live under the same `.mad` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `xai_mad_config::mad_home()`,
/// so worktree paths also agree with trust/hooks and other mad-home paths.
fn mad_home() -> std::path::PathBuf {
    xai_fast_worktree::resolve_mad_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
/// Returns `~/.mad/worktrees/<repo_slug>` for the given git root.
///
...
pub fn worktree_base_dir(git_root: &Path) -> std::path::PathBuf {
    ...
    mad_home().join("worktrees").join(slug)
}

/// Resolves the worktree base directory (`~/.mad/worktrees/<repo_name>`)
/// for a given source path, correctly handling mad-managed worktrees.
///
/// When `source_path` is already under `~/.mad/worktrees/<repo>/...`, the
...
/// original repo).
///
/// For paths outside the mad worktree directory, falls back to
...
pub fn worktree_base_dir_for_source(source_path: &Path) -> Result<std::path::PathBuf> {
    let worktrees_dir = mad_home().join("worktrees");
...
/// Walk up from `cwd` (staying within `~/.mad/worktrees/`) to its registered
/// worktree record.
///
...
/// The recorded source repo of the mad-managed worktree containing `cwd`, if any.
///
/// Thin wrapper over [`worktree_record_for_cwd`] that drops the DB handle;
/// returns `None` (without DB I/O) for paths outside `~/.mad/worktrees/`.
...
///
/// When the source path is already inside `~/.mad/worktrees/<repo>/`, the
...
///
/// ACP equivalent of `mad -w -r <session_id>` (optionally with `--ref`).
...
use xai_fast_worktree::{
    ...
    rebuild_worktree_db, resolve_mad_home, resolve_worktree_auto_gc_from_layers,
};
...
pub fn worktree_auto_gc_layer_from_settings(
    s: &xai_mad_config_types::WorktreeAutoGcSettings,
) -> WorktreeAutoGcLayer {
    use std::collections::BTreeMap;
    use xai_mad_config_types::WorktreeKindMaxAge;
...
/// Env + `$MAD_HOME/config.toml` only — **`remote=None` is intentional**.
///
...
/// processes; use `MAD_WORKTREE_AUTO_GC=0` / `MAD_WORKTREE_AUTO_GC_DRY_RUN=1`
...
fn resolve_worktree_auto_gc_local() -> xai_fast_worktree::ResolvedWorktreeAutoGc {
    use xai_mad_config_types::WorktreeAutoGcSettings;

    let local = if let Ok(home) = resolve_mad_home() {
...
/// Scan `~/.mad/worktrees/<repo_name>/` for subdirectories not tracked
...
mod tests {
    ...
    async fn snapshot_and_remove_captures_then_deletes() {
        ...
        let ref_name = "refs/mad/subagents/dispose-1";
    ...
    async fn snapshot_and_remove_then_rehydrate_round_trips() {
        ...
        let ref_name = "refs/mad/subagents/dispose-2";
    ...
    async fn snapshot_standalone_worktree_durable_after_removal_round_trips() {
        ...
        let ref_name = "refs/mad/subagents/standalone-1";
    ...
    /// Point `MAD_HOME` at an isolated tempdir (`resolve_mad_home` re-reads
    ...
    /// and restores `MAD_HOME` on drop (before releasing the lock), so the
    ...
    async fn cleanup_cancelled_worktree_removes_dir_and_deregisters() {
        ...
        // can't clobber this row (MadHomeFixture is not visible across crates).
