//! Hub [`AuthProvider`] from `~/.mad/auth.json` for the standalone
...
//!
//! The in-leader `mad workspace` exposure does NOT use this path — it sources
...
fn default_auth_path() -> anyhow::Result<PathBuf> {
    let mad = xai_mad_config::user_mad_home()
        .ok_or_else(|| anyhow::anyhow!("no user mad home (set $MAD_HOME or $HOME)"))?;
    Ok(mad.join("auth.json"))
}
...
fn read_auth_entry(path: &Path) -> anyhow::Result<(String, AuthEntry)> {
    if !path.exists() {
        anyhow::bail!(
            "No auth credentials found at {}. Run `mad login` first.",
            path.display()
        );
    }
    ...
    entries
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "no OIDC auth entry found in {}. Run `mad login` first.",
                path.display()
            )
        })
}
...
/// the default credential path (`~/.mad/auth.json`).
pub fn provider(
    hub_url: &Url,
