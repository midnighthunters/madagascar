...
impl WorkspaceIdentity {
    ...
    /// `MadAuth.team_id`, and for a Team principal `principal_id` *is* the
    /// team id.
...
pub struct WorkspaceEnvironment {
    ...
    /// Version of the `xai-mad-workspace` crate that produced the record.
    ...
    /// Sandbox profile name (`$MAD_SANDBOX_PROFILE`), e.g. `"devbox"`.
...
impl WorkspaceEnvironment {
    ...
    pub(crate) fn capture(
        ...
        Self::assemble(
            ...
            server_id,
            sandbox_id,
            std::env::var("MAD_SANDBOX_PROFILE").ok(),
