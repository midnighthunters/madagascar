...
use std::sync::Arc;
use xai_mad_tools::registry::types::{SessionContext, ToolRegistryBuilder, ToolServerConfig};
...
impl SessionTerminalBackend {
    ...
    pub fn local(backend: xai_mad_tools::computer::local::LocalTerminalBackend) -> Self {
    ...
    pub fn backend(&self) -> &Arc<dyn xai_mad_tools::computer::types::TerminalBackend> {
        &self.backend
    }
...
pub struct WorkspaceBindConfig {
    ...
    pub tools: Option<Vec<xai_mad_tools_api::ToolConfigEntry>>,
...
pub enum ResolvedToolset {
    ...
    InvalidToolConfig(xai_mad_tools::registry::proto_convert::ToolConfigEntryError),
}
    ...
    pub fn resolve(
        ...
        if let Some(cfg) = &self.tool_config {
            for (idx, tool) in cfg.tools.iter().enumerate() {
                if let Err(err) = xai_mad_tools_api::config_validation::validate_name_override(
                    idx,
                    &tool.id,
        ...
        if let Some(tools) = &self.tools {
            ...
            for (idx, entry) in tools.iter().enumerate() {
                ...
                match xai_mad_tools::registry::proto_convert::tool_config_from_entry(
                    idx,
                    entry.clone(),
                ) {
            ...
            if !unserved_tool_ids.is_empty() {
                tracing::warn!(
                    ...
                    running_version = xai_mad_version::VERSION,
...
mod bind_config_tests {
    ...
    fn presets_are_never_resolved() {
        for preset in ["explore", "mad-computer", "bogus"] {
    ...
    fn tools_entries_resolve_to_tool_server_config() {
        ...
        assert_eq!(grep.id, "MadagascarBuild:grep");
        assert_eq!(
            grep.params,
        ...
        assert_eq!(toolset.tools[1].id, "MadagascarBuild:read_file");
    }
    #[test]
...
pub struct WorkspaceConfig {
    ...
    /// Global hook sources (e.g. `~/.claude/settings.json`, `~/.mad/hooks/`).
    pub hook_global_sources: Vec<HookSourceConfig>,
    /// Project-scoped hook sources (e.g. `<project>/.mad/hooks/`).
    ...
    /// `<cwd>/.mad/lsp.json`: `false` drops them at load, `true` keeps them. The
...
pub struct WorkspaceServerMetadata {
    ...
    /// Logical sandbox-service session UUID, from the `MAD_SESSION_ID` env
...
impl WorkspaceServerMetadata {
    ...
    ///
    /// `env_session_id` is the raw `MAD_SESSION_ID`; empty is normalized to
...
/// `*.json` hook files. Maps 1:1 to [`xai_mad_hooks::discovery::HookSource`]
...
pub enum HookSourceConfig {
    ...
    /// A directory of `*.json` hook files (e.g. `~/.mad/hooks/`).
    Directory(PathBuf),
}
