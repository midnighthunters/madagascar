...
//!
//! Reads OIDC credentials from `~/.mad/auth.json`, connects to a
...
use url::Url;
use xai_mad_workspace::config::WorkspaceServerMetadata;
use xai_mad_workspace::daemonize;
use xai_mad_workspace::diag_server::{self, DiagHandle, ErrorClass};
use xai_mad_workspace::error::WorkspaceError;
use xai_mad_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
const SERVICE_NAME: &str = "prod_mad_workspace";
...
struct Args {
    ...
    #[arg(long)]
    capabilities: bool,
    #[arg(long, default_value = "wss://computer-hub.mad.com/v1/tools")]
    hub_url: String,
    #[arg(long)]
    ...
    /// `MAD_WORKSPACE_UPLOAD_QUEUE_ENABLED` env var to `false`) to fall back to
    /// the legacy inline path. Accepts `true`/`false`.
    #[arg(
        long,
        env = "MAD_WORKSPACE_UPLOAD_QUEUE_ENABLED",
    ...
    /// Trust project-scoped LSP servers from `<repo>/.mad/lsp.json`.
    /// Defaults off; sandbox opts in only after workspace trust is established.
    #[arg(
        long,
        env = "MAD_WORKSPACE_PROJECT_LSP_TRUSTED",
    ...
    /// Override with `MAD_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "MAD_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
...
/// `/usr/local/bin/xai-mad-preview-proxy` child (see `cli.rs` for the proxy's
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    tracing_subscriber::registry()
        ...
        .with(donating.clone())
        .init();
    let direct_otlp = match std::env::var("MAD_WORKSPACE_OTLP_ENDPOINT") {
    ...
    {
        use xai_mad_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("MAD_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(value = %val,
                        "Unrecognized MAD_SANDBOX_PROFILE, defaulting to workspace");
                    ProfileName::Workspace
                } else {
                    parsed
                }
            }
            None if xai_mad_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
            None => ProfileName::Workspace,
        };
        let profile_name = profile.to_string();
        if profile == ProfileName::Off {
            tracing::info!(profile = %profile_name, "Sandbox explicitly disabled via MAD_SANDBOX_PROFILE=off");
        } else {
            ...
            sandbox.install();
            let active = xai_mad_sandbox::is_active();
            ...
            tracing::info!(
                ...
                active,
                restrict_network_at_known_linux_launches = xai_mad_sandbox::should_restrict_child_network(),
                "{status_msg}"
            );
        }
    }
    let auth_provider = xai_mad_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(
        hub_url = %url,
        cwd = %cwd.display(),
    ...
    let session_id = std::env::var("MAD_SESSION_ID").ok();
    ...
    xai_mad_workspace::init_metrics();
    let ws_handle = match xai_mad_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = xai_mad_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            xai_mad_workspace::handle::DrainReason::Sigterm,
        )
        .await;
...
mod tests {
    ...
    fn project_lsp_trust_defaults_off_and_is_opt_in() {
        unsafe { std::env::remove_var("MAD_WORKSPACE_PROJECT_LSP_TRUSTED") };
        let args = Args::try_parse_from(["xai-workspace-server"]).unwrap();
    ...
    fn preview_flags_parse_and_lower_to_supervisor_config() {
        ...
        assert_eq!(
            ...
            Some("https://mad.com/preview-auth")
        );
