//! Permission resolution engine: merges native `.mad/config.toml`,
...
/// (fail-safe) with a warn + skip record for `mad inspect`.
...
/// user `~/.mad` → `Requirements`, so [`is_admin_source`] trusts only the root tier.
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    xai_mad_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
/// Load `[permission]` rules from native Mad TOML config files:
///
///   * `~/.mad/config.toml` (lowest priority)
///   * Each `.mad/config.toml` from the git repo root down to `cwd`
...
fn load_config_toml_permissions(cwd: &Path, project_trusted: bool) -> Vec<Sourced<PermissionRule>> {
    ...
    // Global `~/.mad/config.toml` first (lowest priority within this layer).
    // Gated on user_mad_home() so a project's .mad/config.toml is never read as
    // global permissions when neither MAD_HOME nor a home dir resolves.
    if let Some(global_path) = xai_mad_config::user_mad_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match xai_mad_config::load_config_file(&global_path) {
    ...
    // `.mad/config.toml` (same gate as project `.claude/settings.json`).
    if project_trusted {
        for path in crate::project_config::find_project_configs(cwd) {
            match xai_mad_config::load_config_file(&path) {
                Ok(value) => rules.extend(extract_toml_permissions(&value, || {
...
fn managed_config_permissions(
    layers: &[xai_mad_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
...
/// Resolve permission config, merging native Mad and Claude sources.
...
/// `.mad/config.toml` permission rules (mirrors [`load_claude_env_with_project`]).
...
/// blocked `--yolo`); keep admin-tier ones. Records each drop for `mad inspect`.
...
struct ResolveInputs<'a> {
    ...
    /// `.claude/settings.json` / `.mad/config.toml` permission rules are dropped
    ...
    fn live(project_trusted: bool) -> Self {
        Self {
            ...
            managed_config_rules: managed_config_permissions(
                &xai_mad_config::managed_config_layers(),
            ),
            project_trusted,
        }
    }
...
/// bypass is pinned off via mad `requirements.toml`
...
///
/// `project_trusted` gates project-tier Claude settings and `.mad/config.toml`
...
async fn resolve_permissions_with_provenance_inner(
    ...
    // Keep skip-only resolutions alive so the drop reaches `mad inspect`; zero
...
fn resolve_claude_settings_inner(
    ...
    for path in claude_settings_paths_for_trust(cwd, project_trusted) {
        ...
        if let Some(perms) = settings.permissions {
            ...
            // `mad inspect` (all-invalid allow/deny/ask must not leave
    ...
    // `mad inspect` via the outer resolver.
...
/// [`xai_mad_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = xai_mad_config::claude_managed_settings_path() else {
...
/// not consulted: mad must not inherit a host-wide always-approve lockdown from
/// that file. mad still honors that file's permission rules / MCP / marketplace
/// allowlists, and the user's own `--yolo` / `[ui] permission_mode` / runtime
/// toggle drive always-approve; to disable it in mad use a root-owned
/// `requirements.toml`. Fails open on user-writable layers.
pub fn yolo_disabled_by_policy() -> Option<&'static str> {
    let layers = xai_mad_config::requirements_layers();
...
/// without `~/.mad`); `path` only names the layer in a non-bool warning.
...
/// Namespace prefix for managed (mad.com-injected) MCP server names. Defined
/// here (shell depends on workspace) and re-exported by shell's `to_managed_name`
/// so the prefix and policy matching never drift.
pub const MANAGED_MCP_PREFIX: &str = "mad_com_";

/// Max `char` length of a managed runtime name (`mad_com_` + normalized display
...
///
/// Both sides reduce to one key (strip `mad_com_`, [`normalize_managed_name`],
...
mod tests {
    ...
    // Shared so `MAD_HOME`/`HOME` mutations here also serialize against the
    ...
    fn load_claude_env_merges_with_precedence() {
        // MAD_HOME-isolate so the claude-import marker reads clean (an imported
        ...
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("MAD_HOME", home.path());
        let _marker_guard = EnvVarGuard::unset("_MAD_CLAUDE_MARKER_OVERRIDE");
        let tmp = tempfile::tempdir().unwrap();
        let claude_dir = tmp.path().join(".claude");
        std::fs::create_dir_all(&claude_dir).unwrap();
    ...
    fn load_claude_env_empty_when_no_settings() {
        // Isolate MAD_HOME (claude-import marker) AND HOME (global `~/.claude`)
    ...
    fn load_claude_env_with_project_drops_repo_env_when_untrusted() {
        ...
        // drop it. Isolate MAD_HOME so the claude-import marker reads clean (an
    ...
    fn mcp_name_matches_strips_managed_prefix_both_sides_exactly() {
        ...
        assert!(mcp_name_matches("foo", "mad_com_foo"));
        assert!(mcp_name_matches("mad_com_foo", "foo"));
        assert!(mcp_name_matches("mad_com_foo", "mad_com_foo"));
        assert!(!mcp_name_matches("foo", "foobar"));
        assert!(!mcp_name_matches("foo", "mad_com_foobar"));
    ...
    fn mcp_name_matches_is_case_and_space_insensitive() {
        ...
        assert!(mcp_name_matches("Slack", "mad_com_slack"));
        assert!(mcp_name_matches("My Server", "mad_com_my_server"));
        assert!(mcp_name_matches("mad_com_my_server", "My Server"));
        ...
        assert!(!mcp_name_matches("mad_com_", "mad_com_anything"));
    }
    ...
    fn denied_by_server_name_matches_bare_and_managed_prefix() {
        ...
        for unrelated in ["foobar", "mad_com_foobar", "barfoo", "bar"] {
    ...
    fn allowed_by_server_name_restricts_across_transports() {
        ...
        assert!(al.is_server_allowed(&http_named("mad_com_foo", "https://evil.example.com/x")));
        assert!(al.is_server_allowed(&stdio_named("mad_com_foo", "/usr/bin/whatever")));
    ...
    fn server_name_deny_beats_allow() {
        ...
        for s in [
            ...
            http_named("mad_com_foo", "https://foo.example.com/x"),
        ] {
    ...
    fn server_name_prefix_edge_cases_vice_versa() {
        ...
        let al = allowlist_from(serde_json::json!({
            "deniedMcpServers": [ { "serverName": "mad_com_foo" } ]
        }));

        assert!(al.is_server_denied(&http_named("foo", "https://x.example.com/mcp")));
        assert!(al.is_server_denied(&http_named("mad_com_foo", "https://x.example.com/mcp")));
        assert!(!al.is_server_denied(&http_named("foobar", "https://x.example.com/mcp")));
        assert!(!al.is_server_denied(&http_named("mad_com_foobar", "https://x.example.com/mcp")));
    }
    ...
    fn untrusted_project_claude_permissions_are_not_honored() {
        ...
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("HOME", home.path());
        let _mad_guard = EnvVarGuard::set("MAD_HOME", home.path());
        let _marker_guard = EnvVarGuard::unset("_MAD_CLAUDE_MARKER_OVERRIDE");
    ...
    /// Untrusted clone must not contribute project `.mad/config.toml` [permission].
    ///
    /// Sync + `block_on` so `ENV_LOCK` is not held across `.await` (clippy
    /// `await_holding_lock`). Does not assert exact global rule counts:
    /// `xai_mad_config::mad_home()` is a process-wide `OnceLock`, so under
    /// single-process `cargo test` an earlier test may have already pinned
    /// `MAD_HOME`. Project-rule filtering is independent of that; global
    /// survival is checked only when our temp home is the live `user_mad_home()`.
    #[test]
    fn untrusted_project_config_toml_permissions_are_not_honored() {
        ...
        // Global allow (survives untrusted project when MAD_HOME resolves here).
        std::fs::write(
...
"#,
        ...
        let rt = tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .expect("test runtime");
        // Untrusted may be None when no global rules load (MAD_HOME OnceLock
        ...
        let global_live = xai_mad_config::user_mad_home()
            .is_some_and(|g| g == home.path() || g.starts_with(home.path()));
        if global_live {
            let untrusted = untrusted.expect("global rules present when MAD_HOME is live");
            assert!(
                untrusted
                    .config
                    .rules
    ...
    fn non_bool_lock_key_warns_and_does_not_lock() {
        ...
        let p = Path::new("/etc/mad/requirements.toml");
        let bad = layer("[ui]\ndisable_bypass_permissions_mode = \"true\"\n");
        ...
        assert!(
            out.contains("/etc/mad/requirements.toml"),
    ...
    fn admin_source_trusts_only_root_owned_tiers() {
        ...
        // the user-writable `~/.mad/requirements.toml` is not, despite its path.
        ...
        assert!(is_admin_source(&RequirementSource::SystemRequirements {
            path: "/etc/mad/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::Requirements {
            path: "/home/u/.mad/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::ManagedConfig {
            path: "/etc/mad/managed_config.toml".into(),
        }));
    ...
    fn drop_untrusted_catchall_allows_is_source_aware() {
        ...
        let rules = vec![
            ...
            // Managed config: defaults tier, untrusted even from /etc/mad.
            sourced(
                allow_any(Some("*")),
    ...
    fn unrecognized_project_mode_claims_scope_over_global_accept_edits() {
        ...
        assert!(
            ...
            "typo should be recorded for mad inspect"
        );
    }
    ...
    async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
        ...
        let layers =
            xai_mad_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
        ...
        .unwrap();
        assert_eq!(
            xai_mad_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
            1
        );
