...
impl ProtectedEditReason {
    ...
    pub fn description(self) -> Option<&'static str> {
        match self {
            ...
            Self::MadConfig => Some(
                "Note: This edit contains changes to Mad config, which can alter permissions, tools, and other behavior in later sessions.",
            ),
...
pub(crate) fn edit_target_protection(path: &Path) -> Option<ProtectedEditReason> {
    ...
    let lexical = xai_mad_paths::normalize_lexically(path);
...
fn protected_edit_reason(path: &Path) -> Option<ProtectedEditReason> {
    ...
    if protected_mad_hook_root(path, &string_components) {
    ...
    if string_components.ends_with(&[".mad", "config.toml"]) {
        return Some(ProtectedEditReason::MadConfig);
    }
...
fn path_is_under_user_mad_hook_root(path: &Path, mad_home: &Path) -> bool {
    path.starts_with(mad_home.join("hooks")) || path == mad_home.join("hooks-paths")
}

fn protected_mad_hook_root(path: &Path, components: &[&str]) -> bool {
    components.windows(2).any(|pair| pair == [".mad", "hooks"])
        || components.ends_with(&[".mad", "hooks-paths"])
        || xai_mad_config::user_mad_home().is_some_and(|mad_home| {
            let lexical_home = xai_mad_paths::normalize_lexically(&mad_home);
            path_is_under_user_mad_hook_root(path, &lexical_home)
                || resolve_following_symlinks(&lexical_home, 0).is_some_and(|resolved_home| {
                    path_is_under_user_mad_hook_root(path, &resolved_home)
                })
        })
}
...
mod tests {
    ...
    fn sensitive_edit_targets_and_lexical_aliases_prompt() {
        ...
        for path in [
            "/work/src/main.rs",
            "/work/project/.mad/config.toml/backup",
        ] {
            assert!(
    ...
    fn edit_target_protection_classifies_reasons() {
        let cases = [
            ...
            (
                "/home/user/.mad/config.toml",
                ProtectedEditReason::MadConfig,
            ),
            (
    ...
    fn sensitive_edit_targets_include_hook_roots() {
        for path in [
            "/home/user/.mad/hooks/evil.json",
            "/home/user/.mad/hooks/nested/deep.json",
            "/home/user/.mad/hooks-paths",
            "/home/user/.claude/settings.json",
            "/home/user/.claude/settings.local.json",
            "/home/user/.cursor/hooks.json",
            "/work/project/.mad/hooks/local.json",
            "/work/project/.mad/hooks-paths",
        ] {
            assert!(
                edit_target_protection(Path::new(path)).is_some(),
        ...
        for path in [
            "/home/user/.mad/hooks-disabled/note.json",
            "/home/user/.mad/hooks-evil/note.json",
    ...
    #[test]
    fn path_is_under_user_mad_hook_root_matches_relocated_home() {
        let home = Path::new("/custom/mad-home");
        for path in [
            "/custom/mad-home/hooks/x.json",
            "/custom/mad-home/hooks/nested/deep.json",
            "/custom/mad-home/hooks",
            "/custom/mad-home/hooks-paths",
        ] {
            assert!(
                path_is_under_user_mad_hook_root(Path::new(path), home),
                "must match under custom mad home: {path}"
            );
        }
        for path in [
            "/custom/mad-home/hooks-disabled/note.json",
            "/custom/mad-home/hooks-evil/note.json",
            "/custom/mad-home/config.toml",
            "/custom/other/hooks/x.json",
            "/custom/mad-home-extra/hooks/x.json",
        ] {
            assert!(
                !path_is_under_user_mad_hook_root(Path::new(path), home),
    ...
    fn sensitive_edit_targets_follow_symlinks() {
        ...
        .unwrap();
        let mad_hook = outside.path().join(".mad/hooks/evil.json");
        std::fs::create_dir_all(mad_hook.parent().unwrap()).unwrap();
        std::fs::write(&mad_hook, b"{}").unwrap();
        symlink(&mad_hook, ws.path().join("mad-hook-link")).unwrap();

        for path in [
            ws.path().join("file-link"),
            ws.path().join("hooks-link/new-hook"),
            ws.path().join("module-hooks-link/new-hook"),
            ws.path().join("mad-hook-link"),
        ] {
            assert!(
    ...
    fn resolved_root_alias_matches_physical_destination() {
        ...
        assert!(resolved_path_is_within_root(
            &resolved_root.join("mad-test"),
            Path::new("/etc")
        ));
        assert!(!resolved_path_is_within_root(
            Path::new("/tmp/mad-test"),
            Path::new("/etc")
        ));
    }
