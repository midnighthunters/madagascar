...
use xai_mad_tools::implementations::mad_build::web_fetch::{
    DomainMatcher, config::DEFAULT_ALLOWED_DOMAINS, domain::normalize_domain,
};
use xai_mad_tools::types::resources::resolve_model_path;
...
fn permission_mode_artifact_str(mode: xai_mad_telemetry::enums::PermissionMode) -> &'static str {
    use xai_mad_telemetry::enums::PermissionMode;
    match mode {
...
fn spawn_permission_manager_with_pin(
    ...
    let _task = tokio::task::spawn_local(async move {
        ...
        // persist it to ~/.mad/sessions/<cwd>/permission.toml. This caused
        // the allow to survive full restarts (new mad process, new agent
                ...
                PermissionCommand::Request {
                    ...
                    if matches!(&access, AccessKind::Bash(_))
                        && sandbox_may_auto_allow_bash(
                            bash_evaluation.as_ref(),
                            xai_mad_sandbox::should_auto_allow_bash(),
                        )
                        && !policy_forced_prompt
                        && !auto_forced_prompt
                    {
                    ...
                    let (decision, outcome_str, user_prompted) = match &access {
                        ...
                        _ => {
                            ...
                            let (decision, outcome_str) = match &prompt_outcome {
                                ...
                                PromptOutcome::AllowAlways => {
                                    // Fallback clients (Generic / MadWeb /
...
mod tests {
    ...
    /// always-approve option is only offered for `MadTUI | MadPager | Desktop`).
    ...
    async fn protected_edit_floor_covers_auto_config_allow_and_dont_ask() {
        ...
        local
            .run_until(async {
                for path in ["/etc/hosts", "/home/user/.mad/hooks/evil.json"] {
    ...
    async fn auto_classifier_block_denies_then_escalates_to_prompt() {
        ...
        local
            .run_until(async {
                ...
                let client = RecordingClient::default();
                let prompts = client.prompts.clone();
                // MadPager wires the always-approve option through to its YOLO
                // toggle; it is the option set the auto path prompts under.
                let (mgr, _e) =
                    manager_with_recording_client(&cwd, None, client, ClientType::MadPager);
                mgr.set_auto_mode(true);
                mgr.set_classifier(Some(LlmPermissionClassifier::with_fixed_model_text(
