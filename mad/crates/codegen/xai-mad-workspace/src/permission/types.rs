...
pub enum ClientType {
    ...
    #[default]
    #[serde(rename = "generic", alias = "mad-shell", alias = "mad_shell")]
    Generic,
    /// Mad TUI client - show fancy options with interactive bash term selection
    #[serde(rename = "mad-tui", alias = "mad_tui")]
    MadTUI,
    /// Mad Web client - identified by clientIdentifier "mad-web"
    #[serde(rename = "mad_web")]
    MadWeb,
    /// Named client (`"nebula"`) — uses the generic permission UI
    #[serde(rename = "nebula")]
    Nebula,
    /// IDE extension client (VS Code and similar) - identified by clientIdentifier "mad-code-extension"
    #[serde(rename = "extension")]
    Extension,
    /// Mad Pager client - TUI-like terminal pager with interactive permission UI.
    /// Treated identically to MadTUI for permission options (gets bash highlights +
    /// interactive selection). Reports as "pager" for telemetry attribution.
    ///
    /// Accepts both the hyphenated `"mad-pager"` (what the pager actually
    /// sends over the wire, matching `PAGER_CLIENT_TYPE`) and the underscored
    /// `"mad_pager"` form for symmetry with the rest of this enum.
    #[serde(rename = "mad-pager", alias = "mad_pager")]
    MadPager,
    /// Mad Desktop (Electron) client - identified by clientIdentifier "mad-desktop".
    /// Uses TUI-style bash permission options (primary command extraction + prefix matching)
    /// but without interactive `<`/`>` word selection.
    #[serde(rename = "mad_desktop")]
    Desktop,
}
impl ClientType {
    /// Product token for the `User-Agent` header (e.g. `mad-pager`).
    pub fn user_agent_label(&self) -> &'static str {
        match self {
            Self::Generic => "mad-shell",
            Self::MadTUI => "mad-tui",
            Self::MadWeb => "mad-web",
            Self::Nebula => "nebula",
            Self::Extension => "mad-code-extension",
            Self::MadPager => "mad-pager",
            Self::Desktop => "mad-desktop",
        }
    }
    /// Resolve from ACP `clientIdentifier` string (e.g. `"mad-web"`, `"mad-desktop"`).
    pub fn from_client_identifier(id: Option<&str>) -> Self {
        match id {
            Some("mad-web") => Self::MadWeb,
            Some("nebula") => Self::Nebula,
            Some("mad-code-extension") => Self::Extension,
            Some("mad-desktop") => Self::Desktop,
            Some("mad-pager") => Self::MadPager,
            _ => Self::Generic,
        }
    }
...
impl From<&xai_mad_tools::types::ToolInput> for AccessKind {
    fn from(input: &xai_mad_tools::types::ToolInput) -> Self {
...
pub enum RequirementSource {
    Unknown,
    /// User-writable `~/.mad/requirements.toml` — untrusted for keeping a
...
mod tests {
    ...
    fn hashline_edit_maps_to_edit_access() {
        use xai_mad_tools::implementations::mad_build_hashline::edit::types::HashlineEditInput;
        use xai_mad_tools::types::ToolInput;
    ...
    fn bash_maps_to_bash_access() {
        use xai_mad_tools::implementations::mad_build::bash::BashToolInput;
        use xai_mad_tools::types::ToolInput;
    ...
    fn use_tool_maps_to_mcp_tool_access() {
        use xai_mad_tools::implementations::use_tool::UseToolInput;
        use xai_mad_tools::types::ToolInput;
    ...
    fn monitor_maps_to_bash_access() {
        use xai_mad_tools::implementations::mad_build::monitor::types::MonitorInput;
        use xai_mad_tools::types::ToolInput;
    ...
    fn web_fetch_maps_to_web_fetch_access() {
        use xai_mad_tools::implementations::mad_build::web_fetch::WebFetchInput;
        use xai_mad_tools::types::ToolInput;
    ...
    fn web_search_maps_to_web_search_access() {
        use xai_mad_tools::implementations::mad_build::web_search::WebSearchInput;
        use xai_mad_tools::types::ToolInput;
    ...
    #[test]
    fn client_type_deserializes_mad_shell_as_generic() {
        assert_eq!(
            serde_json::from_value::<ClientType>("mad-shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
            serde_json::from_value::<ClientType>("mad_shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
