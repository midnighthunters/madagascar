...
//! [`crate::trust::TrustStore`] (`~/.mad/trusted_folders.toml`). The
//! consume/gating half (the `DECISIONS` cache, `resolve_and_record`,
//! `project_scope_allowed`, the loader filters) lives in `xai-mad-shell`.
//!
...
//! provisional and re-checked rather than cached — is a `xai-mad-shell`
...
use xai_mad_config_types::{BoolFlag, RemoteSettings};
...
pub fn decide_inputs_with_interactive(
    ...
    DecideInputs {
        ...
        // can't persist (Case 2: cwd IS $HOME, incl. the default `~/.mad`).
...
/// this binary — true on a local/dev build (no `MAD_VERSION` release stamp).
///
/// THE single security short-circuit: every explicit trust auto-grant site calls
/// this (greppable via `folder_trust_inert`). When true a self-built mad never
...
/// (`MAD_VERSION` unset at compile time) — i.e. a local/dev build.
///
/// Kept local (not in `xai-mad-version`) on purpose: adding a symbol to that
...
fn is_local_build() -> bool {
    // Runtime escape hatch: a pinned MAD_TEST_VERSION simulates a release build,
    // so tests/CI (which run unstamped, i.e. local-looking) can exercise the gate.
    if std::env::var(xai_mad_version::TEST_VERSION_ENV).is_ok() {
        return false;
    }
    option_env!("MAD_VERSION").is_none()
}

/// Resolve whether the folder-trust gate is enabled.
///
/// On a local/dev build (no `MAD_VERSION` release stamp) the feature is OFF
/// regardless of env/config/remote — a self-built mad auto-trusts (never
...
/// env `MAD_FOLDER_TRUST` > `[folder_trust] enabled` (user) > managed >
...
fn feature_enabled_for_build(remote: Option<&RemoteSettings>, is_local_build: bool) -> bool {
    ...
    // shipped, release-stamped binaries. Even an explicit MAD_FOLDER_TRUST/config
    // opt-in is ignored here so a self-built mad never prompts.
    if is_local_build {
        return false;
    }
    ...
    let user = xai_mad_config::load_from_disk().ok();
    let managed = xai_mad_config::load_managed_config().ok();
    BoolFlag::env("MAD_FOLDER_TRUST")
...
/// same `~/.mad/trusted_folders.toml`. Best-effort; a write failure is logged,
/// not fatal.
...
/// `[permission]` / `.claude` / `.mad/agents` / subdir-launch gaps. NOT itself
/// the trust gate.
pub fn repo_config_kinds(cwd: &Path) -> Vec<&'static str> {
    collect_repo_config_kinds(cwd, false)
}

/// Whether a project `.mad/config.toml` `[permission]` value would contribute
...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    let chain = xai_mad_agent::repo::RepoDirChain::resolve(cwd);
    ...
    // Project `.mad/config.toml` declaring repo-controlled code-exec or
    ...
    for path in crate::project_config::find_project_configs_in(&chain.dirs) {
        let Ok(root) = xai_mad_config::load_config_file(&path) else {
            continue;
        };
    ...
    // Project `.mad/lsp.json`.
    if cwd.join(".mad").join("lsp.json").is_file() {
        hit!("lsp");
    }
    ...
    // must be gated — else a hooks-only clone (e.g. `.mad/hooks/evil.json`) would
    ...
    if path_present_or_uncertain(&hook_root.join(".mad").join("hooks"))
    ...
    // that must be gated — else a plugin clone (e.g. `.mad/plugins/evil/`, even
    // one in a subdir launched via `cd sub && mad`) would resolve trusted and
    ...
    if !xai_mad_agent::plugins::project_plugin_dirs_in(&chain.dirs).is_empty() {
        hit!("plugins");
    }
    // Project AGENT dirs (`.mad/agents` / `.claude/agents`): a project agent
    ...
    if !xai_mad_agent::discovery::project_agent_dirs_in(&chain.dirs).is_empty() {
        hit!("agents");
    }
    // Presence matches exact-cwd discovery without parsing repository content.
    let mad = cwd.join(".mad");
    if directory_present_or_uncertain(&mad.join("roles")) {
        hit!("roles");
    }
    if directory_present_or_uncertain(&mad.join("personas")) {
        hit!("personas");
    }
    if directory_present_or_uncertain(&hook_root.join(".mad").join("workflows")) {
        hit!("workflows");
    }
...
pub fn prompt_for_trust(key: &Path) -> bool {
    ...
    let _ = writeln!(
        err,
        "This folder contains repo-local config (.mcp.json / .mad/lsp.json / hooks) \
...
mod tests {
    ...
    #[test]
    fn repo_configs_present_detects_mad_config_mcp_servers() {
        let tmp = repo_tmp();
        let mad = tmp.path().join(".mad");
        std::fs::create_dir_all(&mad).unwrap();
        std::fs::write(mad.join("config.toml"), "[mcp_servers.x]\ncommand=\"y\"\n").unwrap();
        assert!(repo_configs_present(tmp.path()));
    }

    #[test]
    fn repo_configs_present_detects_mad_lsp_json() {
        let tmp = repo_tmp();
        let mad = tmp.path().join(".mad");
        std::fs::create_dir_all(&mad).unwrap();
        std::fs::write(mad.join("lsp.json"), "{}").unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_agents() {
        // A `.mad/agents`-only clone must be gated: a project agent definition
        // can carry an inline `hooks:` block (code-exec) and can shadow a built-in
        // subagent by name.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".mad").join("agents")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_roles() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".mad").join("roles")).unwrap();
    ...
    fn repo_configs_present_detects_project_personas() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".mad").join("personas")).unwrap();
    ...
    fn project_subagent_marker_regular_file_is_absent() {
        ...
        let mad = tmp.path().join(".mad");
        std::fs::create_dir_all(&mad).unwrap();
        std::fs::write(mad.join("roles"), "not a directory").unwrap();
        assert!(!repo_configs_present(tmp.path()));
    }
    ...
    fn project_subagent_marker_at_repo_root_is_absent_from_subdir() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".mad/roles")).unwrap();
    ...
    fn project_subagent_marker_symlink_to_directory_is_present() {
        ...
        std::fs::create_dir_all(&mad).unwrap();
        std::os::unix::fs::symlink(&target, mad.join("roles")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn dangling_project_subagent_marker_is_absent() {
        ...
        let mad = tmp.path().join(".mad");
        std::fs::create_dir_all(&mad).unwrap();
        std::os::unix::fs::symlink("missing", mad.join("personas")).unwrap();
        assert!(!repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_workflows_from_subdir() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".mad").join("workflows")).unwrap();
        let subdir = tmp.path().join("crates").join("inner");
        std::fs::create_dir_all(&subdir).unwrap();
        assert!(repo_configs_present(&subdir));
    ...
    fn repo_configs_present_detects_project_hooks_file() {
        ...
        let mad = tmp.path().join(".mad");
        std::fs::create_dir_all(&mad).unwrap();
        std::fs::write(mad.join("hooks"), "{}").unwrap();
    ...
    fn repo_configs_present_detects_dangling_project_hooks_symlink() {
        ...
        let mad = tmp.path().join(".mad");
        std::fs::create_dir_all(&mad).unwrap();
        std::os::unix::fs::symlink("missing-hooks", mad.join("hooks")).unwrap();
    ...
    fn repo_configs_present_detects_project_plugins() {
        ...
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".mad").join("plugins").join("x")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_plugins_in_subdir() {
        ...
        std::fs::create_dir_all(subdir.join(".mad").join("plugins").join("evil")).unwrap();
        assert!(repo_configs_present(&subdir));
    }
    ...
    fn repo_configs_present_false_for_empty_mcp_servers_table() {
        ...
        let mad = tmp.path().join(".mad");
        std::fs::create_dir_all(&mad).unwrap();
        std::fs::write(mad.join("config.toml"), "[mcp_servers]\n").unwrap();
        assert!(!repo_configs_present(tmp.path()));
    }

    #[test]
    fn repo_configs_present_detects_mad_config_plugins_paths() {
    ...
    #[test]
    fn repo_configs_present_detects_mad_config_permission() {
    ...
    fn repo_config_kinds_matches_gate_and_reports_all_kinds() {
        ...
        // `.mad/agents` — even when launched from a SUBDIR (the cwd→git-root walk
        // that `first_only` shares). Guards against silent drift between the two.
        let tmp = repo_tmp();
        let mad = tmp.path().join(".mad");
        std::fs::create_dir_all(mad.join("agents")).unwrap();
        std::fs::write(mad.join("config.toml"), "[plugins]\npaths = [\"./x\"]\n").unwrap();
    ...
    // MAD_HOME-isolation idiom mirrored from this crate's `permission::claude_compat`
    // tests (the workspace crate has no `serial_test`/`xai-mad-test-support`
    ...
    fn simulate_release_build() -> EnvVarGuard {
        EnvVarGuard::set(xai_mad_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"))
    }
    ...
    fn local_build_ignores_remote_rollout() {
        ...
        let _home = EnvVarGuard::set("MAD_HOME", home.path());
        let _flag = EnvVarGuard::unset("MAD_FOLDER_TRUST");
    ...
    fn release_build_keeps_gate_when_enabled() {
        ...
        // config nor an ambient env flag can override it: empty MAD_HOME (no
        // config.toml/managed_config.toml) + MAD_FOLDER_TRUST unset. nextest's
        // process-per-test makes mad_home()'s OnceLock pick up the temp dir.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("MAD_HOME", home.path());
        let _flag = EnvVarGuard::unset("MAD_FOLDER_TRUST");
    ...
    fn local_build_ignores_explicit_env_optin() {
        ...
        // MAD_FOLDER_TRUST=1 does NOT enable the feature (so a self-built mad
        // never prompts). MAD_HOME isolated so on-disk config can't influence it.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("MAD_HOME", home.path());
        let _flag = EnvVarGuard::set("MAD_FOLDER_TRUST", Path::new("1"));
    ...
    fn release_build_defaults_on() {
        ...
        // the feature ON. Empty MAD_HOME (no config.toml/managed config) +
        // MAD_FOLDER_TRUST unset so only the default applies.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("MAD_HOME", home.path());
        let _flag = EnvVarGuard::unset("MAD_FOLDER_TRUST");
    ...
    fn is_local_build_honors_test_version_override() {
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        // A pinned MAD_TEST_VERSION simulates a release build => not a local build.
        {
            let _sim = EnvVarGuard::set(xai_mad_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"));
            assert!(!is_local_build());
        }
        // With it unset, an unstamped build (no MAD_VERSION) is a local build.
        // Guard to the unstamped case so a release-stamped test binary (CI release)
        // doesn't spuriously fail this arm.
        let _unset = EnvVarGuard::unset(xai_mad_version::TEST_VERSION_ENV);
        if option_env!("MAD_VERSION").is_none() {
            assert!(is_local_build());
        }
    }
    ...
    fn store_io_is_noop_on_local_build() {
        ...
        // (mirrors `is_local_build_honors_test_version_override`); MAD_HOME-isolated
        // and ENV_LOCK-serialized so toggling MAD_TEST_VERSION is race-safe.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("MAD_HOME", home.path());
        let _unset = EnvVarGuard::unset(xai_mad_version::TEST_VERSION_ENV);
    ...
    fn revoke_folder_trust_store_persists_untrust_for_trusted_folder() {
        ...
        // MAD_HOME-isolated so the seed/deny hit a temp store, not the real file.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("MAD_HOME", home.path());
        let _sim = simulate_release_build();
        let tmp = repo_tmp();
        let key = workspace_key(tmp.path());
    ...
    fn revoke_folder_trust_store_writes_no_deny_for_never_trusted_folder() {
        ...
        // MAD_HOME-isolated so the grant writes to a temp store.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("MAD_HOME", home.path());
        let _sim = simulate_release_build();
