...
use xai_mad_sampling_types::error::{try_parse_stream_error, user_facing_api_error_message};
...
pub use xai_mad_sampling_types::ApiBackend;

/// Process-level fallback for the `x-mad-client-identifier` header.
const DEFAULT_CLIENT_IDENTIFIER: &str = "mad-shell";

/// Product identifier baked into User-Agent strings.
const AGENT_PRODUCT: &str = "mad-shell";
const ANTHROPIC_DEFAULT_MAX_TOKENS: u32 = 128_000;

/// Per-request `x-mad-*` headers. Optional fields are skipped when empty/`None`.
struct MadRequestHeaders<'a> {
    conv_id: &'a str,
    req_id: &'a str,
    model_id: &'a str,
    session_id: &'a str,
...
impl MadRequestHeaders<'_> {
    fn apply(&self, builder: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
        let mut b = builder
            .header("x-mad-conv-id", self.conv_id)
            .header("x-mad-req-id", self.req_id)
            .header("x-mad-model-override", self.model_id)
            .header("x-mad-session-id", self.session_id)
            .header("x-mad-agent-id", self.agent_id);
        if let Some(idx) = self.turn_idx {
            b = b.header("x-mad-turn-idx", idx);
        }
        if let Some(id) = self.deployment_id.filter(|s| !s.is_empty()) {
            b = b.header("x-mad-deployment-id", id);
        }
        if let Some(id) = self.user_id.filter(|s| !s.is_empty()) {
            b = b.header("x-mad-user-id", id);
        }
        b
    }
}
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = xai_mad_sampling_types::reported_cost_ticks(
        value
...
fn extract_model_metadata(headers: &reqwest::header::HeaderMap) -> Option<ResponseModelMetadata> {
    let context_window = headers
        .get("x-mad-context-window")
        .and_then(|v| v.to_str().ok())
        .and_then(|s| s.parse::<u64>().ok());

    let max_completion_tokens = headers
        .get("x-mad-max-completion-tokens")
        .and_then(|v| v.to_str().ok())
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<xai_mad_sampling_types::DoomLoopRecoveryPolicy>,
}
...
fn agent_version() -> String {
    xai_mad_version::VERSION.to_string()
}
...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
        ...
        // Add x-mad-client-version header for version gating at the proxy.
        if let Some(client_version) = config.client_version.as_ref()
            && let Ok(header_value) = HeaderValue::from_str(client_version)
        {
            headers.insert(
                HeaderName::from_static("x-mad-client-version"),
                header_value,
            );
        }
        ...
        {
            headers.insert(
                HeaderName::from_static("x-mad-deployment-id"),
                header_value,
            );
        }
        ...
        {
            headers.insert(HeaderName::from_static("x-mad-user-id"), header_value);
        }

        {
            ...
            if let Ok(header_value) = HeaderValue::from_str(&client_id) {
                headers.insert(
                    HeaderName::from_static("x-mad-client-identifier"),
                    header_value,
                );
            }
        }
    ...
    pub async fn chat_completion(
        ...
        let x_mad_conv_id = &payload.x_mad_conv_id.clone().unwrap_or_default();
        let x_mad_req_id = &payload.x_mad_req_id.clone().unwrap_or_default();
        ...
        let mad_headers = MadRequestHeaders {
            conv_id: x_mad_conv_id,
            req_id: x_mad_req_id,
            model_id: &model_id,
            session_id: payload.x_mad_session_id.as_deref().unwrap_or_default(),
            turn_idx: payload.x_mad_turn_idx.as_deref(),
            agent_id: payload.x_mad_agent_id.as_deref().unwrap_or_default(),
            deployment_id: payload.x_mad_deployment_id.as_deref(),
            user_id: payload.x_mad_user_id.as_deref(),
        };
        let http_request = mad_headers
    ...
    pub async fn create_response(
        ...
        let x_mad_conv_id = request.x_mad_conv_id.as_deref().unwrap_or_default();
        let x_mad_req_id = request.x_mad_req_id.as_deref().unwrap_or_default();
        ...
        let mad_headers = MadRequestHeaders {
            ...
            req_id: x_mad_req_id,
            model_id: &model_id,
            session_id: request.x_mad_session_id.as_deref().unwrap_or_default(),
            turn_idx: request.x_mad_turn_idx.as_deref(),
            agent_id: request.x_mad_agent_id.as_deref().unwrap_or_default(),
            deployment_id: request.x_mad_deployment_id.as_deref(),
            user_id: request.x_mad_user_id.as_deref(),
        };
        ...
        xai_mad_sampling_types::patch_reasoning_text_types(&mut request_body);
        let http_request = mad_headers
    ...
    /// gate that adds the opt-in `x-mad-doom-loop-check` request header, so
    ...
    pub async fn create_response_stream(
        ...
        let mut http_request = mad_headers
            .apply(self.post(self.endpoint("responses")))
            .header(ACCEPT, HeaderValue::from_static("text/event-stream"));
    ...
    pub async fn conversation_stream_responses(
        ...
        let trace = request.trace.take();
        let x_mad_conv_id = request.x_mad_conv_id.clone();
        let x_mad_req_id = request.x_mad_req_id.clone();
        let x_mad_session_id = request.x_mad_session_id.clone();
        let x_mad_turn_idx = request.x_mad_turn_idx.clone();
        let x_mad_agent_id = request.x_mad_agent_id.clone();
        ...
        let extra_tools = xai_mad_sampling_types::extra_tool_entries(&request.hosted_tools);

        let responses_request: rs::CreateResponse = (&request).into();

        let mut wrapper = CreateResponseWrapper::new(responses_request);
        wrapper.x_mad_conv_id = x_mad_conv_id;
        wrapper.x_mad_req_id = x_mad_req_id;
        wrapper.x_mad_session_id = x_mad_session_id;
        wrapper.x_mad_turn_idx = x_mad_turn_idx;
        wrapper.x_mad_agent_id = x_mad_agent_id;
...
mod tests {
    use super::*;
    use indexmap::IndexMap;
    use xai_mad_sampling_types::types::ChatRequestMessage;
    ...
    fn streaming_chat_request_serializes_correctly() {
        let request = ChatCompletionRequest {
            ...
            x_mad_conv_id: None,
            x_mad_req_id: None,
            x_mad_session_id: None,
            x_mad_turn_idx: None,
            x_mad_agent_id: None,
            x_mad_deployment_id: None,
            x_mad_user_id: None,
            trace: None,
        };
    ...
    fn new_applies_extra_headers() {
        ...
        cfg.extra_headers
            .insert("x-XAI-token-auth".to_string(), "xai-mad-cli".to_string());
    ...
    fn user_agent_omits_origin_version_when_absent() {
        ...
        let ua = user_agent_string_for(&origin);
        // No slash between product and the mad-shell agent product.
        assert!(ua.starts_with("my-client mad-shell/"));
    }

    #[test]
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = xai_mad_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
        ...
        let make = |ticks: i64| {
            format!(
                ...
                "response": {{
                    ...
                    "model": "mad-build", "status": "completed", "output": [],
                    "usage": {{
                        "input_tokens": 10,
