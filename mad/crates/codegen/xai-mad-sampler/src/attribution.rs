...
//! it" buckets.
//!
//! `xai-mad-sampler` is intentionally decoupled from `xai-mad-shell`
...
/// tools live in `xai-mad-tools`
/// (`crates/codegen/xai-mad-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `xai_mad_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `xai_mad_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `xai-mad-shell/src/auth/manager.rs` to match, otherwise the
