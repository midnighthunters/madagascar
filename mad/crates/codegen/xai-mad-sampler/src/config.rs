...
//! `xai_mad_sampling_types::SamplingConfig` so that the sampler crate
...
/// composing chat-state's `xai_mad_sampling_types::SamplingConfig`
...
pub struct SamplerConfig {
    ...
    /// client itself sends the opt-in `x-mad-doom-loop-check` header on
