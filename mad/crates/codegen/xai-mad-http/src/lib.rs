...
//! `xai_mad_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `MAD_POOL_*` /
//! `MAD_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `MAD_SAMPLER_SHARED_CLIENT=0` falls back to
...
use xai_mad_workspace::permission::ClientType;
...
// `OriginClientInfo` is owned by `xai-mad-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `xai-mad-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `xai-mad-telemetry` consumes
// the same type via `xai_mad_sampler::OriginClientInfo`. The shell-specific
...
/// `MAD_CLIENT_VERSION` env vars. Returns `None` when
/// `MAD_CLIENT_NAME` is unset.
...
/// Process-level client identifier (`MAD_CLIENT_NAME` env var, default `"mad-shell"`).
...
/// (`mad -p`) run or an interactive session; feeds the `client_mode`
/// metric label.
pub const CLIENT_MODE_HEADER: &str = "x-mad-client-mode";

/// One-way latch: set to `"headless"` at startup by the non-TUI entry points
/// (`run_single_turn` for `mad -p`, `run_headless_inner` for
/// `mad agent [headless]`), `"interactive"` otherwise.
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn xai_mad_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(xai_mad_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
mod tests {
    ...
    fn session_user_agent_string_renders_expected_variants() {
        ...
        assert!(with_version.starts_with("mad-desktop/1.2.3 mad-shell/"));
        ...
        assert!(without_version.starts_with("mad-web mad-shell/"));
        assert!(!without_version.starts_with("mad-web/"));
    }
    ...
    fn user_agent_render_collapses_duplicate_origin_and_agent_identity() {
        ...
        assert_eq!(ua.render(), "mad-shell/0.1.171 (macos; aarch64)");
    }
}
