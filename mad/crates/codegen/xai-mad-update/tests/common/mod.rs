...
//! its own `OnceLock<MAD_HOME>`. The helpers below ensure the per-binary
//! initialization is identical: same env-var set, same isolation guarantees,
//! same reset between tests.
//!
//! Mirrors the MAD_HOME isolation pattern used in other integration tests.
//!
//! ## Usage
//!
//! ```ignore
...
//! #[serial_test::serial]
//! async fn my_test() {
//!     let _ = test_home();   // initializes MAD_HOME once per binary
...
/// Returns a process-wide test `MAD_HOME`, initialized exactly once per test
/// binary. Once initialized, `xai_mad_config::mad_home()` will resolve to
...
pub fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe {
            std::env::set_var("MAD_HOME", &path);
            std::env::remove_var("MAD_TEST_VERSION");
            std::env::remove_var("NPM_TOKEN");
            std::env::remove_var("MAD_INSTALLER");
            std::env::remove_var("MAD_MANAGED_BY_NPM");
            std::env::remove_var("MAD_MANAGED_BY_INTERNAL");
        }
        path
    })
}

/// Wipe state in `MAD_HOME` between tests so each test sees a clean home.
...
/// Override the version reported by `get_installed_mad_version()` for the
/// duration of the test (until [`reset_home`] or process exit).
pub fn set_test_version(v: &str) {
    // SAFETY: tests using this helper must be `#[serial]`.
    unsafe { std::env::set_var("MAD_TEST_VERSION", v) };
}
...
/// Minimal [`xai_mad_update::UpdateConfig`] for install tests.
pub fn make_update_config(channel: &str) -> xai_mad_update::UpdateConfig {
    xai_mad_update::UpdateConfig {
...
/// Backdate every file in `MAD_HOME/downloads` by ~2 hours.
///
