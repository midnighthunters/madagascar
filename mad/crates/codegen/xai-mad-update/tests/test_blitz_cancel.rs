...
//!
//! > `~/.mad/bin/mad` resolves to a binary that passes the smoke-test, OR it
...
use xai_mad_update::auto_update::install_internal_from_base;
...
/// (`mad` and `agent` — see `swap_managed_bin_links`). Returns the
...
/// What the active `mad` should resolve to after an install attempt.
...
/// to both `mad` and `agent` — `swap_managed_bin_links` moves them together.
...
/// `cargo nextest run -p xai-mad-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("MAD_BLITZ_ITERS")
        .ok()
