...
use xai_mad_shell::env::MadagascarBuildEnvironment;
use xai_mad_shell::util::mad_home::mad_home;

const TTL_SECONDS_BEFORE_AUTO_UPDATE: Duration = Duration::from_secs(60 * 30);
const NPM_PACKAGE: &str = "@xai-official/mad";
pub const GH_RELEASE_REPO: &str = "xai-org-shared/mad-build";
...
pub(crate) const CLI_BASE_URL_FALLBACK: &str =
    "https://storage.googleapis.com/mad-build-public-artifacts/cli";
...
///
/// Constructed once from `MadagascarBuildEnvironment` at startup and threaded through the
/// update call chain so that `auto_update` and `version` never need to know
/// about the `MadagascarBuildEnvironment` enum directly.
#[derive(Debug, Clone)]
pub struct UpdateConfig {
    /// Chat API proxy base URL (versioned `https://cli-chat-proxy.mad.com/v1` endpoint).
    pub proxy_base_url: String,
    /// Auth scope key for `~/.mad/auth.json`.
    pub auth_scope: String,
    /// Enterprise deployment key (MAD_DEPLOYMENT_KEY).
...
impl UpdateConfig {
    pub fn from_environment(env: &MadagascarBuildEnvironment) -> Self {
        Self {
            proxy_base_url: env.cli_chat_proxy_base_url(),
            auth_scope: xai_mad_shell::auth::MadComConfig::default().auth_scope(),
            deployment_key: None,
            alpha_test_key: None,
...
pub async fn write_version_cache(version: &str, stable_version: Option<&str>) {
    let version_path = mad_home().join("version.json");
    let now = time::OffsetDateTime::now_utc();
    let json = MadVersion::new(
        version.to_string(),
...
pub async fn is_version_cache_fresh() -> bool {
    ...
    if let Ok(version_str) = fs::read_to_string(&version_path).await
        && let Ok(version) = serde_json::from_str::<MadVersion>(&version_str)
...
pub use xai_mad_version::installed as get_installed_mad_version;

/// Version of the managed mad binary currently on disk, read from the
/// `~/.mad/bin/mad` symlink target (`../downloads/mad-<version>-<platform>`)
/// without exec'ing anything.
///
/// Concurrent updaters (TUI background download, leader hourly checker,
/// explicit `mad update`) decide staleness from this instead of their own
...
/// link whose target binary was deleted (e.g. manual `~/.mad/downloads`
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = xai_mad_shell::util::mad_home::mad_application();
        ...
        version_from_versioned_binary_name(target.file_name()?.to_str()?, "mad")
    }
    #[cfg(not(unix))]
    {
        None
...
///
/// Handles the internal layout (`mad-0.1.150-macos-aarch64`, including
/// pre-releases: `mad-0.1.150-alpha.1-linux-x86_64` → `0.1.150-alpha.1`)
/// and the npm layout without a platform suffix (`mad-0.1.150`,
/// `mad-0.1.150-alpha.1`): everything between the `{bin_prefix}-` prefix
/// and the first platform-OS component is the version, validated as semver
/// so unknown layouts (`mad-latest`, `mad-pager-*` when `bin_prefix` is
/// `mad`) return `None` instead of garbage.
///
...
/// Read the cached stable version from `~/.mad/version.json` (sync, for display).
///
...
pub fn cached_stable_version() -> Option<String> {
    ...
    let gv: MadVersion = serde_json::from_str(&content).ok()?;
    gv.stable_version
}
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(xai_mad_version::VERSION, &stable)
    })
}
...
/// `~/.mad/version.json` (written by the auto-updater):
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(xai_mad_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
...
mod tests {
    ...
    fn test_is_fresh_rejects_future_timestamp() {
        ...
        let v = MadVersion::new("0.1.200".to_string(), None, future);
        assert!(
    ...
    /// symlink-target file name (`mad-<version>-<platform>`).
    #[test]
    fn test_version_from_versioned_binary_name() {
        let cases: &[(&str, Option<&str>)] = &[
            ("mad-0.2.46-darwin-arm64", Some("0.2.46")),
            ("mad-0.1.220-linux-x86_64", Some("0.1.220")),
            ("mad-0.2.5-windows-x86_64.exe", Some("0.2.5")),
            ...
            ("mad-0.1.220-alpha.4-linux-x86_64", Some("0.1.220-alpha.4")),
            ("mad-0.1.220-alpha.4", Some("0.1.220-alpha.4")), // npm layout
            ("mad-pager-0.1.5-darwin-arm64", None),           // "pager" is not a version
            ("mad-garbage-darwin-arm64", None),               // unparseable version
            ("mad-0.2.46", Some("0.2.46")),                   // no platform suffix
            ("other-0.2.46-darwin-arm64", None),               // wrong prefix
            ("mad-latest", None),                             // symlink alias, not a version
            ("mad", None),                                    // bare name
            ("", None),
        ];
        for (name, expected) in cases {
            assert_eq!(
                version_from_versioned_binary_name(name, "mad").as_deref(),
                *expected,
        ...
        // prefix but not under "mad".
        assert_eq!(
            version_from_versioned_binary_name("mad-pager-0.1.5-darwin-arm64", "mad-pager")
                .as_deref(),
            Some("0.1.5")
        );
    }
    ...
    // ──────────────────────────────────────────────────────────────────────
    // MadVersion JSON shape — backward compatibility invariants
    // ──────────────────────────────────────────────────────────────────────
    ...
    fn test_version_json_backward_compat() {
        ...
        let v: MadVersion = serde_json::from_str(old).unwrap();
        assert_eq!(v.version, "0.1.180");
        assert!(v.stable_version.is_none());

        // New format with stable_version round-trips correctly.
        let now = time::OffsetDateTime::now_utc();
        let new = MadVersion::new("0.2.5".to_string(), Some("0.2.3".to_string()), now);
        let json = serde_json::to_string(&new).unwrap();
        let parsed: MadVersion = serde_json::from_str(&json).unwrap();
        ...
        assert!(serde_json::from_str::<MadVersion>(future).is_ok());

        // Missing required field (checked_at) is rejected.
        let missing = r#"{"version":"0.1.180"}"#;
        assert!(serde_json::from_str::<MadVersion>(missing).is_err());
    }
    ...
    fn test_is_fresh_ttl_boundaries() {
        let now = time::OffsetDateTime::now_utc();
        let v = MadVersion::new("0.1.200".to_string(), None, now);
    ...
    fn test_update_config_default_channel_is_stable() {
        use xai_mad_shell::env::MadagascarBuildEnvironment;
        let cfg = UpdateConfig::from_environment(&MadagascarBuildEnvironment::Production);
