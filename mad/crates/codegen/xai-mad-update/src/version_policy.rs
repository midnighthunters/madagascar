...
use crate::version::get_installed_mad_version;
use semver::Version;
use tracing::warn;
use xai_mad_shell::util::config::VersionPolicy;
...
pub(crate) enum VersionPolicyError {
    #[error(
        "Cannot install Mad {target}: the minimum allowed version is {minimum}. \
         Run `mad update` to install the latest allowed version."
    )]
