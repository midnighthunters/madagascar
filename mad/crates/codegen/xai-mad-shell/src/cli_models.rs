//! Data APIs for `mad models`. Clients own display.
...
/// Status for the `mad models` banner (display order ≠ sampling priority; see [`AuthStatus::resolve`]).
#[derive(Debug, PartialEq, Eq)]
pub enum AuthStatus {
    ApiKey,
    /// Auth host from `mad_ws_origin` (scheme stripped).
    LoggedIn(String),
...
impl AuthStatus {
    ...
    pub fn resolve(agent_config: &AgentConfig) -> Self {
        ...
        if agent_config.create_auth_manager().current().is_some() {
            let origin = &agent_config.mad_com_config.mad_ws_origin;
            let host = origin
                .strip_prefix("https://")
        ...
        if crate::agent::auth_method::should_advertise_xai_api_key(
            agent_config.mad_com_config.api_key_auth_disabled(),
            models.values(),
...
mod tests {
    ...
    ///
    /// Uses `MAD_AUTH_PATH` (not `MAD_HOME`) so a OnceLock-cached real home
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 7]) {
        ...
        let guards = [
            ...
            EnvGuard::unset("MAD_AUTH"),
            EnvGuard::set("MAD_AUTH_PATH", auth_path.to_str().unwrap()),
            EnvGuard::unset("MAD_DEPLOYMENT_KEY"),
            EnvGuard::unset("MAD_WS_ORIGIN"),
            EnvGuard::unset("MAD_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
    ...
    fn resolve_oauth_session() {
        ...
        let _auth = EnvGuard::set("MAD_AUTH", &json);

        assert_eq!(
            AuthStatus::resolve(&Config::default()),
            AuthStatus::LoggedIn("mad.com".to_owned())
        );
    }
