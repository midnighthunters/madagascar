...
use crate::auth::{AuthManager, MadAuth, MadComConfig, parse_output};
use crate::util::mad_home;
...
/// session.
fn is_cached_credential_compatible(auth: &MadAuth, mad_com_config: &MadComConfig) -> bool {
    let expected_issuer = mad_com_config
        .oidc
        .as_ref()
        .map(|c| c.issuer.as_str())
        .or_else(|| mad_com_config.oauth2.as_ref().map(|c| c.issuer.as_str()));
    ...
    if let Some(policy) = crate::auth::oidc::login_principal_policy(mad_com_config) {
...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("MAD_LOGIN_DEVICE_FLOW")
...
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `MAD_LOGIN_DEVICE_FLOW` env >
/// `[auth] login_device_flow` config > `mad_build_login_device_flow` remote feature flag > loopback.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("MAD_LOGIN_DEVICE_FLOW");
...
async fn run_external_auth_provider(
    ...
    let principal_policy =
        crate::auth::oidc::login_principal_policy(auth_manager.mad_com_config());
...
async fn run_auth_flow_inner(
    ...
    tracing::info!(
        has_oidc = mad_com_config.oidc.is_some(),
        has_oauth2 = mad_com_config.oauth2.is_some(),
        has_external_auth = mad_com_config.auth_provider_command.is_some(),
        reauth,
    ...
    if !force_interactive && let Some(auth) = auth_manager.current() {
        if is_cached_credential_compatible(&auth, mad_com_config) {
    ...
    if !force_interactive && !reauth && auth_manager.is_expired() {
        ...
        if disk_auth.as_ref().is_some_and(|d| {
            !crate::auth::is_expired(d) && is_cached_credential_compatible(d, mad_com_config)
        }) {
            xai_mad_telemetry::unified_log::info(
    ...
    if let Some(ref cmd) = mad_com_config.auth_provider_command {
    ...
    // `mad login --devbox` uses run_devbox_login and is not gated here.
    if !mad_com_config.blocks_automatic_oidc()
    ...
    // --device-auth / MAD_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
    if crate::auth::oidc::is_configured(mad_com_config) {
        return crate::auth::oidc::run_login_flow(mad_com_config, auth_manager, channels).await;
    }

    if let Some(ref oauth2_cfg) = mad_com_config.oauth2 {
    ...
    anyhow::bail!(
        "No OAuth2 configuration available. Run `mad login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
pub async fn try_ensure_fresh_auth(mad_com_config: &MadComConfig) -> Option<MadAuth> {
    try_ensure_fresh_auth_with(&build_startup_auth_manager(mad_com_config)).await
}

/// Builds and configures the startup `AuthManager`; the policy helpers below
/// take it injected so tests can substitute their own.
fn build_startup_auth_manager(mad_com_config: &MadComConfig) -> Arc<AuthManager> {
    let auth_manager = Arc::new(AuthManager::new(
        &mad_home::mad_home(),
        mad_com_config.clone(),
    ));
    // auth()'s OIDC/external refresh needs the refresher configured first.
    auth_manager.configure_refresher(mad_com_config.auth_provider_command.clone(), None);
    auth_manager
}

/// Policy: cached-valid creds, else silent refresh (no interactive login).
async fn try_ensure_fresh_auth_with(auth_manager: &Arc<AuthManager>) -> Option<MadAuth> {
    match auth_manager.auth().await {
        Ok(auth) => Some(auth),
        Err(e) => {
...
pub(crate) async fn try_noninteractive_auth_no_mint(
    ...
    try_noninteractive_auth_no_mint_with(&build_startup_auth_manager(mad_com_config)).await
}

/// Policy behind [`try_noninteractive_auth_no_mint`], with the `AuthManager`
/// injected for tests.
async fn try_noninteractive_auth_no_mint_with(auth_manager: &Arc<AuthManager>) -> Option<MadAuth> {
...
fn expired_refreshable_session(auth_manager: &AuthManager) -> Option<MadAuth> {
    auth_manager
        .current_or_expired()
...
pub(crate) async fn mint_session_noninteractive(
    ...
    let mad_com_config = auth_manager.mad_com_config();
    ...
    if let Some(cmd) = mad_com_config.auth_provider_command.as_deref() {
...
async fn persist_or_use_minted(auth_manager: &AuthManager, new_auth: MadAuth) -> MadAuth {
...
fn report_signed_in(auth: &MadAuth) {
    eprint!("\r\x1b[K");
    match auth.email {
...
pub async fn ensure_authenticated_with_override(
    ...
    let mad_home = mad_home::mad_home();
    let auth_manager = Arc::new(AuthManager::new(&mad_home, mad_com_config.clone()));
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    message_prefix: Option<&str>,
) -> anyhow::Result<Option<MadAuth>> {
    if has_noninteractive_auth {
        Ok(try_ensure_fresh_auth(mad_com_config).await)
    } else {
        ensure_authenticated(mad_com_config, false, message_prefix)
            .await
            .map(Some)
    }
}

/// Unified `mad login` handler for CLI entry points (tui, pager).
///
/// Precedence: `--oauth` forces loopback, `--device-auth` forces device,
/// otherwise `MAD_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub async fn run_cli_login(
    ...
    // supports the device flow. Without this guard, `mad login` on an
    ...
    let authenticated = if devbox {
    ...
    } else if cli_should_use_device(&config.mad_com_config, login_override).await {
        if config.mad_com_config.oauth2.is_none() {
        ...
        let mad_home = mad_home::mad_home();
        let auth_manager = Arc::new(AuthManager::new(&mad_home, config.mad_com_config.clone()));
        ...
        if device_auth && crate::auth::oidc::is_configured(&config.mad_com_config) {
            eprintln!(
    ...
    // config was actually applied; `mad setup` reports the no-config case.
...
pub struct LogoutResult {
    ...
    /// `true` if `XAI_API_KEY` / `MAD_CODE_XAI_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        xai_mad_telemetry::external::set_identity(
            xai_mad_telemetry::external::IdentityAttrs::default(),
        );
        xai_mad_telemetry::external::flush();
...
/// `mad logout` CLI handler. Calls [`perform_logout`] and formats
/// the result to stderr.
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let mad_home = mad_home::mad_home();
    let auth_manager = AuthManager::new(&mad_home, config.mad_com_config.clone());
...
mod tests {
    ...
    /// Run `f` with `MAD_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("MAD_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("MAD_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("MAD_LOGIN_DEVICE_FLOW"),
        };
        f()
    }

    // A mad.com first-party (x.ai-issuer) OIDC session — `is_xai_auth()` true.
    fn oidc_session(key: &str, refresh: Option<&str>) -> MadAuth {
        MadAuth {
            key: key.into(),
    ...
    fn expired_refreshable_session_gate() {
        ...
        let mgr = AuthManager::new(dir.path(), MadComConfig::default());
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only mad_home: reading a missing auth.json succeeds (empty), but
        // writing fails — exercising the save-failure path.
        let dir = tempfile::tempdir().unwrap();
        std::fs::set_permissions(dir.path(), std::fs::Permissions::from_mode(0o500)).unwrap();
        let mgr = Arc::new(AuthManager::new(dir.path(), MadComConfig::default()));
    ...
    async fn external_reauth_without_prev_auth_enriches_inline() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), MadComConfig::default())
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("MAD_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("MAD_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `mad login` must use loopback, not device —
    ...
    fn oidc_auth(issuer: &str) -> MadAuth {
        MadAuth {
    ...
    fn pinned_cfg(team: &str) -> MadComConfig {
        MadComConfig {
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        let mut cfg = MadComConfig::default();
        cfg.oauth2.as_mut().unwrap().issuer = "http://127.0.0.1:1".into();

        let writer = Arc::new(
            AuthManager::new(dir.path(), cfg.clone()).with_proxy_base_url("http://127.0.0.1:1"),
        );
        let expired_no_rt = MadAuth {
        ...
        // MAD_LOGIN_DEVICE_FLOW / the real config file (the CLI override
    ...
    fn extract_url_from_external_provider_stderr() {
        ...
        assert_eq!(
            extract(
                "Visit the following link to sign into Mad: https://auth.example.com/login?code=abc"
            ),
    ...
    /// CLI `mad login` passes `on_stderr=None`; stderr must be inherited so
