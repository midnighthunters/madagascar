...
pub(crate) trait AuthSnapshot: Send + Sync {
    ...
    fn current(&self) -> Option<MadAuth>;
    /// Read the expired in-memory bearer (for its `refresh_token`).
    fn expired_auth(&self) -> Option<MadAuth>;
    ...
    fn read_disk_auth(&self) -> Option<MadAuth>;
...
pub(crate) trait ExternalCommandRunner: Send + Sync {
    ...
    async fn run_external_command(&self, command: &str) -> Option<MadAuth>;
}
...
impl RefreshOutcome {
    ...
    pub(crate) fn success(auth: MadAuth) -> Self {
    ...
    /// flow). Any refresher holding the [`MadAuth`] it sent must use
    ...
    ///
    /// Prefer this wherever the attempted [`MadAuth`] is in hand: it captures
...
mod tests {
    use super::*;
    use crate::auth::{AuthMode, MadAuth, MadComConfig};
