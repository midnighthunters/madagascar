...
/// Parse stdout into a session-credential `MadAuth`.
pub(crate) fn parse_output(output: &std::process::Output) -> anyhow::Result<MadAuth> {
...
pub(crate) async fn run_external_refresh(command: &str) -> Option<MadAuth> {
...
pub(crate) async fn refresh_with_command(command: &str, prev_auth: &MadAuth) -> Option<MadAuth> {
...
mod tests {
    ...
    #[tokio::test]
    async fn sets_mad_auth_expired_env_on_refresh() {
        let auth = run_external_refresh("echo $MAD_AUTH_EXPIRED")
            .await
            .unwrap();
