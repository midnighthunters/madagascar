...
use crate::auth::config::MadComConfig;
use crate::auth::error::AuthError;
use crate::auth::token_type::TokenType;
use xai_mad_telemetry::events::ManualAuthSurface;

#[cfg(test)]
use super::model::UserInfo;
use super::model::{
    AuthMode, MadAuth, early_invalidation, is_expired, is_expired_with_buffer, lookup_auth,
    token_suffix,
};
...
pub struct AuthManager {
    ...
    inner: Arc<RwLock<Option<MadAuth>>>,
    path: PathBuf,
    scope: String,
...
impl AuthManager {
    pub fn new(mad_home: &Path, mad_com_config: MadComConfig) -> Self {
        let scope = mad_com_config.auth_scope();
        let proxy_base_url =
        ...
        xai_mad_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "mad_home": mad_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "MAD_HOME": std::env::var("MAD_HOME").unwrap_or_else(|_| "(unset)".into()),
                "MAD_AUTH_PATH": std::env::var("MAD_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "MAD_AUTH": std::env::var("MAD_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // MAD_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("MAD_AUTH") {
            if let Ok(auth) = serde_json::from_str::<MadAuth>(&inline_json) {
                return Self::assemble(
                    Some(auth),
                    mad_home.join("auth.json"),
                    scope,
                    mad_com_config,
                    proxy_base_url,
                    None,
            ...
            tracing::warn!("MAD_AUTH set but failed to parse as JSON, falling back to file");
        }

        // MAD_AUTH_PATH: custom file path (overrides default $MAD_HOME/auth.json).
        let path = std::env::var("MAD_AUTH_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| mad_home.join("auth.json"));
    ...
    /// else allowed.
    pub(crate) fn cached_token_policy_error(&self, auth: &MadAuth) -> Option<AuthError> {
        ...
        let policy = crate::auth::oidc::login_principal_policy(&self.mad_com_config)?;
    ...
    fn vet_cached(&self, auth: MadAuth) -> Option<MadAuth> {
    ...
    pub(crate) fn current(&self) -> Option<MadAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    #[inline]
    pub(crate) fn with_inner_write<R>(&self, f: impl FnOnce(&mut Option<MadAuth>) -> R) -> R {
    ...
    #[inline]
    pub(crate) fn with_inner_read<R>(&self, f: impl FnOnce(Option<&MadAuth>) -> R) -> R {
    ...
    pub(crate) fn current_or_expired(&self) -> Option<MadAuth> {
    ...
    pub(crate) fn current_wire_valid(&self) -> Option<MadAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    pub(crate) fn expired_auth(&self) -> Option<MadAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    fn is_token_expired(&self, auth: &MadAuth) -> bool {
    ...
    fn is_token_hard_expired(&self, auth: &MadAuth) -> bool {
        self.token_expired_with_buffer(auth, Duration::zero())
    }

    fn token_expired_with_buffer(&self, auth: &MadAuth, buffer: Duration) -> bool {
        if auth.expires_at.is_some() {
            return is_expired_with_buffer(auth, buffer);
        }
        if auth.auth_mode == AuthMode::External
            && let Some(ttl) = self.mad_com_config.auth_token_ttl
        {
    ...
    ///
    /// Returns the input `MadAuth` BEFORE enrichment lands; callers
    /// needing the post-enrichment view re-read `current()`.
    pub(crate) async fn update(self: &Arc<Self>, auth: MadAuth) -> std::io::Result<MadAuth> {
        ...
        match &write_result {
            Ok(()) => xai_mad_telemetry::unified_log::info(
            ...
            Err(e) => xai_mad_telemetry::unified_log::error(
    ...
    fn spawn_user_info_enrichment(self: &Arc<Self>, auth: MadAuth) {
        enrichment::spawn(Arc::clone(self), auth);
    }

    /// Blocking `/user` enrichment for login flows that exit before the background task lands.
    pub(crate) async fn enrich_auth_inline(&self, auth: &mut MadAuth) {
        enrichment::enrich_inline(self, auth).await;
    }

    pub(crate) fn mad_com_config(&self) -> &MadComConfig {
        &self.mad_com_config
    }
    ...
    pub(crate) async fn run_external_refresh_command(&self, command: &str) -> Option<MadAuth> {
    ...
    pub(crate) fn hot_swap(&self, new_auth: MadAuth) {
    ...
    fn try_adopt_disk_token(&self, reason: RefreshReason, msg: &str) -> Option<MadAuth> {
    ...
    fn inner_auth_or_external_default(&self) -> MadAuth {
        self.inner.read().clone().unwrap_or_else(|| MadAuth {
    ...
    #[cfg(test)]
    fn persist_and_swap(&self, auth: MadAuth) -> Option<MadAuth> {
    ...
    pub(crate) fn read_disk_auth(&self) -> Option<MadAuth> {
    ...
    fn read_disk_auth_silent(&self) -> Option<MadAuth> {
    ...
    pub(crate) fn read_disk_auth_with_state(&self) -> (Option<MadAuth>, DiskAuthState) {
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in MAD_AUTH mode).
            DiskAuthState::Ok => {
                xai_mad_telemetry::unified_log::info(
    ...
    pub async fn auth(self: &Arc<Self>) -> Result<MadAuth, AuthError> {
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<MadAuth, AuthError> {
        // Snapshot inner ONCE for dispatch atomicity (closes a TOCTOU
        // where a concurrent `clear()` raced `token_type()` + `inner.read()`).
        let snapshot: Option<MadAuth> = self.with_inner_read(|inner| inner.cloned());
        ...
        if let Some(err) = self.permanent_failure() {
            ...
            if !self.mad_com_config.blocks_automatic_oidc()
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // assumption that a sibling process (`mad login` from
        ...
        // preferred_method=api_key forbids automatic OIDC mint.
        if result.is_err()
            && !self.mad_com_config.blocks_automatic_oidc()
            && self.is_devbox_environment()
            && let Ok(auth) = self.try_devbox_recovery().await
        {
            return Ok(auth);
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<MadAuth, AuthError> {
        if self.mad_com_config.blocks_automatic_oidc() {
            tracing::debug!(
        ...
        xai_mad_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    pub(crate) fn is_different_token(&self, candidate: &MadAuth) -> bool {
        let current_key = self.inner.read().as_ref().map(|a| a.key.clone());
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<xai_mad_telemetry::events::ManualAuth> {
...
pub(crate) fn compute_proactive_sleep(this: &AuthManager) -> StdDuration {
    ...
    match this.inner.read().as_ref().and_then(|a| a.expires_at) {
        ...
        // `[mad_com] auth_token_ttl` to drive a real schedule.
...
impl xai_mad_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
fn prefers_static_api_key(am: &AuthManager) -> bool {
    matches!(
        am.mad_com_config.preferred_method,
...
fn resolve_static_api_key(am: &AuthManager) -> Option<String> {
    if am.mad_com_config.api_key_auth_disabled() {
        return None;
    }
    if matches!(
        am.mad_com_config.preferred_method,
...
) -> xai_mad_tools::types::SharedApiKeyProvider {
