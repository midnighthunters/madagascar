...
use crate::auth::model::{MadAuth, UserInfo, lookup_auth};
...
pub(super) fn spawn(manager: Arc<AuthManager>, auth: MadAuth) {
...
async fn fetch_user_info(manager: &AuthManager, key: &str, log_label: &str) -> Option<UserInfo> {
    ...
    let token_header = &manager.mad_com_config.token_header;
...
pub(super) async fn enrich_inline(manager: &AuthManager, auth: &mut MadAuth) {
...
async fn run_user_info_enrichment(manager: &AuthManager, auth: MadAuth) {
    let started = std::time::Instant::now();
...
pub(super) fn apply_user_info_enrichment(disk: &mut MadAuth, user_info: UserInfo) {
