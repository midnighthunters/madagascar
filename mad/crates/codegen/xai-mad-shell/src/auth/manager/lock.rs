...
mod tests {
    ...
    // `MAD_TEST_LOCK_HOLDER` env var on an `#[ignore]`d entry-point test —
    // no external `python3` dependency.

    /// Line printed to stdout once the subprocess holds the flock.
    #[cfg(unix)]
    const LOCK_HOLDER_READY: &str = "__MAD_LOCK_HOLDER_READY__";

    /// Subprocess entry point for the cross-process lock tests. Only does
    /// anything when re-executed with `MAD_TEST_LOCK_HOLDER` set; a normal
    ...
    fn subprocess_lock_holder() {
        let Ok(spec) = std::env::var("MAD_TEST_LOCK_HOLDER") else {
