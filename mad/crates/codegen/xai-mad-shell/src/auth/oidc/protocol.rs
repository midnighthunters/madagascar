...
use super::super::config::{ForceLoginTeam, MadComConfig, OAuth2ProviderConfig, OidcAuthConfig};
use super::super::{AuthMode, MadAuth};
use base64::Engine;
...
pub fn is_configured(config: &MadComConfig) -> bool {
    config.oidc.is_some()
}
...
pub(crate) fn login_principal_policy(cfg: &MadComConfig) -> Option<ForceLoginTeam> {
...
mod tests {
    ...
    fn authorize_url_includes_team_principal_params() {
        let config = OidcAuthConfig {
            ...
            client_id: TEST_CLIENT_ID.into(),
            scopes: vec!["offline_access".into(), "mad-cli:access".into()],
            audience: None,
        };
        ...
        assert!(url.contains("referrer=mad-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
            "expected exactly one referrer param, got: {url}"
    ...
    fn authorize_url_uses_oauth2_referrer_override_once() {
        ...
        let oauth2 = OAuth2ProviderConfig {
            ...
            principal_type: None,
            principal_id: None,
            referrer: Some("mad-desktop".into()),
        };
        let discovery = Discovery {
            authorization_endpoint: "https://auth.x.ai/authorize".into(),
            token_endpoint: "https://auth.x.ai/token".into(),
        ...
        assert!(url.contains("referrer=mad-desktop"));
        assert!(!url.contains("referrer=mad-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
    ...
    fn peek_access_token_principal_matrix() {
        ...
        let team_jwt = make_jwt(serde_json::json!({
            ...
            "exp": 9999999999u64,
            "iat": 1000000000u64,
            "scope": "offline_access mad-cli:access api:access",
            "principal_type": "Team",
