...
//!
//! 1. [`xai_mad_telemetry::unified_log::warn`] for the local
//!    `~/.mad/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!
//! [`xai_mad_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`xai_mad_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`xai_mad_sampler::SamplerConfig::attribution_callback`] at every
...
use xai_mad_sampler::{Auth401AttributionCallback, SamplingConsumer};
use xai_mad_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`xai_mad_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `xai_mad_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn xai_mad_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `xai_mad_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// in `xai-mad-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `xai-mad-shell` go through this helper -- the
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.mad/logs/unified.jsonl) + scrubbed
...
/// and the mint/expiry fields from the resulting `MadAuth`.
///
...
fn compute_attribution_payload(
    ...
    // Single read-lock acquisition: pull the live `MadAuth` (or
    ...
    // `mad_com_config.auth_token_ttl` when `expires_at` is `None`
...
mod tests {
    ...
    use crate::auth::{AuthManager, MadAuth, MadComConfig};

    use super::*;

    /// Test helper: build a fresh `AuthManager` rooted at a tempdir so
    /// nothing from a developer's actual `~/.mad/auth.json` leaks in.
    ...
    /// `ShellAttribution` implements `xai_mad_tools::Auth401AttributionCallback`
