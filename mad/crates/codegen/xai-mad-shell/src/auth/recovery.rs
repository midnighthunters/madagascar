...
use crate::auth::model::MadAuth;
use crate::auth::token_type::TokenType;
use xai_mad_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};
...
impl RejectedAuth {
    pub(crate) fn capture(auth: Option<&MadAuth>) -> Self {
        Self {
...
impl ManualAuthTracker {
    ...
    pub(crate) fn record(
        ...
        xai_mad_telemetry::session_ctx::log_event(event);
    }
...
impl UnauthorizedRecovery {
    ...
    pub async fn next(&mut self) -> Result<MadAuth, AuthError> {
    ...
    async fn resolve_next(&mut self) -> Result<MadAuth, AuthError> {
    ...
    async fn next_step_loop(&mut self) -> Result<MadAuth, AuthError> {
        loop {
            match self.step {
                ...
                RecoveryStep::DevboxRecovery => {
                    ...
                    if !self.auth_manager.mad_com_config().blocks_automatic_oidc()
    ...
    async fn try_reload_from_disk(&self) -> Option<MadAuth> {
        let _lock = self
            .auth_manager
        ...
        let Some(disk_auth) = self.auth_manager.read_disk_auth() else {
            ...
            xai_mad_telemetry::unified_log::debug("auth recovery: no disk entry", None, None);
            return None;
        };
    ...
    fn fresh_mint_guard(&self) -> Option<MadAuth> {
    ...
    async fn try_refresh_from_authority(&self) -> Result<MadAuth, AuthError> {
    ...
    fn is_different_token(&self, candidate: &MadAuth) -> bool {
...
mod tests {
    ...
    use crate::auth::model::{AuthMode, MadAuth};
    ...
    fn rejected_cred() -> Option<MadAuth> {
        Some(MadAuth {
    ...
    fn mgr() -> (tempfile::TempDir, Arc<AuthManager>) {
        ...
        let m = Arc::new(AuthManager::new(dir.path(), MadComConfig::default()));
        (dir, m)
    }
    ...
    async fn recover_with_ok_refresher(m: &Arc<AuthManager>) -> (Result<MadAuth, AuthError>, u32) {
        let calls = Arc::new(AtomicU32::new(0));
        m.set_refresher(Arc::new(OkRefresher {
            calls: calls.clone(),
        }));
    ...
    async fn reload_from_disk_picks_up_different_token() {
        ...
        let scope = m.mad_com_config().auth_scope();
        let fresh = MadAuth {
