...
/// grandchildren -- and the `MAD_AUTH_PROVIDER_*` credentials in their env --
...
async fn run_capped(
    ...
    // the direct child.
    let mut group = xai_mad_tools::util::ProcessGroup::new()
...
async fn mint_provider_token(
    ...
    if let Some(prev) = previous {
        cmd.env("MAD_AUTH_PROVIDER_ACCESS_TOKEN", &prev.token);
        if let Some(refresh) = &prev.refresh_token {
            cmd.env("MAD_AUTH_PROVIDER_REFRESH_TOKEN", refresh);
        }
        if let Some(expires_at) = prev.expires_at {
            cmd.env("MAD_AUTH_PROVIDER_EXPIRES_AT", expires_at.to_rfc3339());
        }
    }
