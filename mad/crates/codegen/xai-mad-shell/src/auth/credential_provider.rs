...
use crate::util::mad_auth_credentials::MadAuthCredentials;
...
fn api_key_id_for(auth: Option<&crate::auth::MadAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| xai_mad_telemetry::config::deployment_id_from_key(&a.key))
}
...
pub struct ShellAuthCredentialProvider {
    ...
    static_credentials: MadAuthCredentials,
}
impl ShellAuthCredentialProvider {
    pub(crate) fn new(
        ...
        let mut static_credentials = MadAuthCredentials::new(None);
...
pub(crate) fn embedding_session_credentials(
...
) -> xai_mad_memory::EndpointScopedCredentials {
    ...
    xai_mad_memory::EndpointScopedCredentials::for_endpoint(
        embed_base_url,
...
///
/// - `"mad-shell"`   — classic Madagascar CLI / TUI (xai-mad-shell)
/// - `"mad-pager"`   — new Mad Pager / TUI (xai-mad-pager)
/// - `"mad-desktop"` — Mad Desktop app
/// - `"mad-extension"` — VS Code / browser extension
///
/// The factory forwards both:
/// - `x-mad-client-version`   (e.g. "0.1.210-alpha.5 (279ffacddb)")
...
pub fn build_storage_client_for_proxy(
    ...
    if let Some(am) = auth_manager {
        ...
        .with_client_identity(xai_mad_version::VERSION, client_identifier)
        .with_client_mode(crate::http::process_client_mode())
        .with_attribution(bridge)
    } else {
        let mut creds = MadAuthCredentials::new(user_token);
...
impl HttpAuth for OtelAuthCredentialProvider {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        ...
        let mut creds = MadAuthCredentials::new(None);
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        xai_mad_telemetry::external::set_identity(
            xai_mad_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `xai-mad-pager` and `xai-mad-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> xai_mad_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let mad_com_config = crate::auth::MadComConfig::default();
    let exporter = xai_mad_telemetry::otel_layer::OtelExporterConfig {
    ...
    let token_header_value = mad_com_config.token_header.clone();
    let mad_home = crate::util::mad_home::mad_home();
    let bootstrap = Arc::new(AuthManager::new(&mad_home, mad_com_config));
    let provider = Arc::new(OtelAuthCredentialProvider::new(bootstrap));
    let _ = OTEL_PROVIDER.set(provider.clone());
    xai_mad_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    use crate::auth::MadComConfig;
    use crate::auth::manager::AuthManager;
    use chrono::{Duration as ChronoDuration, Utc};
    use std::sync::Mutex;
    use xai_mad_auth::AuthCredentialProvider;
    /// Serializes tests that pin `MAD_AUTH_EARLY_INVALIDATION_SECS`, since
    /// env vars are process-global and parallel tests would race.
    static EARLY_INVALIDATION_LOCK: Mutex<()> = Mutex::new(());
    /// RAII guard: pins `MAD_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("MAD_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("MAD_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("MAD_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("MAD_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    fn make_auth(key: &str, expires_in: ChronoDuration) -> MadAuth {
        MadAuth {
            key: key.to_string(),
    ...
    fn make_manager(dir: &tempfile::TempDir, initial: Option<MadAuth>) -> Arc<AuthManager> {
    ...
    async fn refresh_after_unauthorized_drives_recovery_state_machine() {
        ...
        let mgr = Arc::new(AuthManager::new(
            dir.path(),
            crate::auth::MadComConfig::default(),
        ));
        ...
        impl crate::auth::refresh::TokenRefresher for OkRefresher {
            async fn refresh(
                ...
                crate::auth::refresh::RefreshOutcome::Success(Box::new(MadAuth {
                    key: "fresh".into(),
                    auth_mode: crate::auth::AuthMode::Oidc,
    ...
    fn embedding_session_credentials_scopes_to_first_party() {
        ...
        let api_key_provider: xai_mad_tools::types::SharedApiKeyProvider =
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use xai_mad_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
    ...
    /// user token, matching the precedence in `MadAuthCredentials::apply`.
