...
/// auth.json scope key for plain API key auth (desktop login, `mad login --api-key`).
...
/// server enrichment opts in. Single source for `MadAuth`, `AuthMeta`, and
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from mad-desktop login or `mad login --api-key`)
    ApiKey,
}
...
pub struct MadAuth {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub has_mad_code_access: Option<bool>,
...
impl std::fmt::Debug for MadAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("MadAuth")
...
impl MadAuth {
    ...
    /// `true` when this auth can access mad.com managed MCP connectors.
    ...
    pub(crate) fn carry_user_profile_from(&mut self, prev: &MadAuth) {
...
#[cfg(test)]
impl MadAuth {
    /// Returns a `MadAuth` with sensible defaults for tests. Override fields
    /// with struct update syntax:
    /// ```ignore
    /// MadAuth { key: "my-key".into(), ..MadAuth::test_default() }
    /// ```
...
pub(crate) type AuthStore = BTreeMap<String, MadAuth>;
...
///
/// Legacy `WebLogin` tokens (from the pre-OIDC `mad login --legacy`
...
pub fn lookup_auth(map: &AuthStore, scope: &str) -> Option<MadAuth> {
...
/// Early-invalidation buffer. Override with `MAD_AUTH_EARLY_INVALIDATION_SECS`
/// for testing (e.g. `=5` to shrink the buffer to 5 seconds).
pub(super) fn early_invalidation() -> Duration {
    std::env::var("MAD_AUTH_EARLY_INVALIDATION_SECS")
        .ok()
...
pub(crate) fn is_expired(auth: &MadAuth) -> bool {
...
pub(crate) fn is_expired_with_buffer(auth: &MadAuth, buffer: Duration) -> bool {
...
mod tests {
    ...
    fn make_auth(mode: AuthMode) -> MadAuth {
        MadAuth {
            key: "k".into(),
            auth_mode: mode,
            create_time: Utc::now(),
    ...
    fn is_xai_auth_matrix() {
        ...
        let with_issuer = |mode: AuthMode, issuer: Option<&str>| MadAuth {
    ...
    fn user_info_subscription_tier_present() {
        let json = r#"{
            "userId": "u1",
            "subscriptionTier": "SuperMadPro"
        }"#;
        let info: UserInfo = serde_json::from_str(json).unwrap();
        assert_eq!(info.subscription_tier.as_deref(), Some("SuperMadPro"));
    }
