...
/// Resolved absolute path to bash. On Unix uses the `xai_mad_config` shell
/// resolution cascade (`$MAD_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        xai_mad_config::shell::unix_shell_path(xai_mad_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `xai-mad-tools`.
pub use xai_mad_tools::util::pager_env;
...
/// xai-mad-pager which renders its own UI and doesn't need raw ANSI codes).
///
