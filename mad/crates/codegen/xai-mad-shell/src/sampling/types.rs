// Re-export all types from the standalone xai-mad-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use xai_mad_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `xai-mad-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `xai-mad-sampler` crate can reference them without a circular
// dep on `xai-mad-shell`.

// Tests for the types now live in xai-mad-sampling-types crate.

use xai_mad_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `xai-mad-sampling-types` or
/// `xai-mad-tools`) so neither of those crates needs to depend on
