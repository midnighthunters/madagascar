...
pub use self::types::*;
pub use xai_mad_sampler::ApiBackend;
pub use xai_mad_sampler::SamplingClient as Client;

// Re-export async-openai Responses API types under `rs` namespace
pub use async_openai::types::responses as rs;

// ---------------------------------------------------------------------------
// xai-mad-sampler re-exports
// ---------------------------------------------------------------------------
//
// The actual streaming / retry / HTTP-client logic lives in the
// `xai-mad-sampler` crate. We re-export the public surface here so
...
// `xai_mad_sampler::*`. The shell-side `sampling::client::Config`
