...
//!
//! The canonical error types now live in `xai_mad_sampling_types::error`.
//! This module re-exports them and adds `map_sampling_err_to_acp` which
//! depends on `agent_client_protocol::Error` (a mad-shell dependency).

// Re-export everything from the standalone crate.
pub use xai_mad_sampling_types::error::*;
...
/// API key / team rate-limit copy. Personal mad.com upgrades do not raise API
...
pub const FREE_USAGE_USER_MESSAGE: &str = "You\u{2019}ve reached your free Madagascar Build usage limit for now. Get SuperMad for much higher limits, or try again later: https://mad.com/supermad?referrer=mad-build";
...
/// API-key + personal SuperMad upsell → team credits copy. Else the body
...
/// IC sometimes reuses OAuth free-tier upsell copy on 429s ("upgrade to a Mad
/// subscription" / mad.com/supermad). That is wrong for API-key / team auth:
/// higher limits come from credits and spend-based rate-limit tiers, not a
/// personal SuperMad plan.
fn pushes_consumer_subscription_upsell(detail: &str) -> bool {
    let d = detail.to_ascii_lowercase();
    d.contains("mad.com/supermad") || d.contains("upgrade to a mad subscription")
}

/// Map a `SamplingError` to an ACP `Error` for client-facing responses.
/// This stays in xai-mad-shell because it depends on `agent_client_protocol::Error`.
    ...
    match err {
        ...
        SamplingError::Api {
            ...
            StatusCode::FORBIDDEN => {
                let message = if message.contains("requires a Mad subscription")
                    && crate::agent::auth_method::has_xai_api_key_env()
                {
                    format!(
                        "{message}\n\nYou have an API key set (XAI_API_KEY). \
                         ...
                         To use your API key, run `mad logout` or type /logout in the TUI."
                    )
                } else {
                    message
                };
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                xai_mad_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub fn terminal_error_data(
    ...
    kind: xai_mad_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != xai_mad_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == xai_mad_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
...
mod tests {
    ...
    fn attach_prompt_usage_preserves_error_kind_and_round_trips() {
        ...
        ledger.record_main_loop_call(
            "m",
            &xai_mad_sampling_types::TokenUsage {
                prompt_tokens: 3,
                completion_tokens: 1,
                total_tokens: 4,
                reasoning_tokens: 0,
    ...
    fn format_rate_limited_surfaces_nonempty_server_detail() {
        ...
        // Team console rate-limit copy has no personal SuperMad upsell — surface as-is.
    ...
    fn format_rate_limited_api_key_rewrites_consumer_subscription_upsell() {
        let body = "Some resource has been exhausted: You are sending requests too quickly. \
             Please slow down, or upgrade to a Mad subscription for higher limits: \
             https://mad.com/supermad";
        let wire = format!("API error (status 429 Too Many Requests): {body}");
        // OAuth keeps the IC body (personal plan upgrade is correct).
        assert_eq!(format_rate_limited_user_message(Some(&wire), false), body);
        // API key must not push mad.com SuperMad — team credits / rate-limit tiers.
        assert_eq!(
    ...
    fn with_api_key_env<F: FnOnce()>(key: Option<&str>, f: F) {
        ...
        let prev_legacy = std::env::var("MAD_CODE_XAI_API_KEY").ok();
        // SAFETY: serial_test ensures no concurrent env mutation.
        unsafe {
            std::env::remove_var("XAI_API_KEY");
            std::env::remove_var("MAD_CODE_XAI_API_KEY");
            if let Some(k) = key {
        ...
        unsafe {
            ...
            if let Some(v) = prev_legacy {
                std::env::set_var("MAD_CODE_XAI_API_KEY", v);
            }
        }
        if let Err(e) = result {
    ...
    fn forbidden_subscription_error_includes_api_key_hint_when_env_set() {
        with_api_key_env(Some("xai-test"), || {
            let err = SamplingError::Api {
                status: StatusCode::FORBIDDEN,
                message: "The model 'mad-build' requires a Mad subscription.".into(),
                model_metadata: None,
                retry_after_secs: None,
                should_retry: None,
            };
            ...
            assert!(
                msg.contains("mad logout"),
                "should suggest mad logout when API key is available: {msg}"
            );
            assert!(
                msg.contains("/logout"),
