...
pub enum ClientMode {
    /// Headless mode (mad agent, mad agent headless) - uses websocket relay.
    /// Leader connects to websocket relay once and forwards messages.
    Headless,
    /// Stdio mode (mad agent stdio, mad -p) - uses local IPC.
...
pub struct LeaderCapabilities {
    ...
    /// (driven by `mad update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `mad update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `mad update` just installed; the leader uses
...
mod tests {
    ...
    fn workspace_status_payload_roundtrip() {
        let payload = ControlPayload::WorkspaceStatus {
            ...
            uptime_ms: 4200,
            active_tool_calls: 2,
            sessions: vec!["mad-a".into(), "mad-b".into()],
            pid: 4242,
        };
