//! Leader-follower IPC architecture for mad-shell.
//!
...
//! │  └────────────────────────┬────────────────────────────────┘│
//! └───────────────────────────┼──────────────────────────────────┘
//!                             │ IPC (Unix socket at ~/.mad/leader.sock)
//!         ┌───────────────────┼───────────────────┐
//!         ▼                   ▼                   ▼
//! ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
...
//!
//! ```ignore
//! use xai_mad_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
//! // Connect to existing leader or spawn a new one
//! let caps = ClientCapabilities {
//!     yolo_mode: true,
//!     default_model: Some("mad-3-fast".to_string()),
//! };
...
mod transport;
use crate::env::MadagascarBuildEnvironment;
...
const CLIENT_LEADER_VERSION: &str = xai_mad_version::VERSION;
...
/// How long the SAME live mad flock-holder may stay unconnectable before
...
/// Maximum reconnection attempts for bounded mode (headless/`mad -p`).
...
pub struct LeaderEnvUrls {
    pub mad_ws_url: String,
    pub mad_ws_origin: String,
}
...
pub struct LeaderDescriptor {
    ...
    pub environment: Option<MadagascarBuildEnvironment>,
...
pub enum LeaderTarget {
    Environment(MadagascarBuildEnvironment),
    WsUrl(String),
    Pid(u32),
}
fn known_environment_for_ws_url(ws_url: &str) -> Option<MadagascarBuildEnvironment> {
    let environments: &[MadagascarBuildEnvironment] = &[MadagascarBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
fn known_environment_for_suffix(ws_url_suffix: &str) -> Option<MadagascarBuildEnvironment> {
    let environments: &[MadagascarBuildEnvironment] = &[MadagascarBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
pub async fn discover_leaders() -> Vec<LeaderDescriptor> {
    discover_leaders_in(&crate::util::mad_home::mad_home()).await
}
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!({
                            ...
                            "reason": reason,
                            "killer_ver": xai_mad_version::VERSION,
                        })),
                    );
                }
                Err(e) => {
...
impl From<&crate::auth::MadComConfig> for LeaderEnvUrls {
    fn from(c: &crate::auth::MadComConfig) -> Self {
        Self {
            mad_ws_url: c.mad_ws_url.clone(),
            mad_ws_origin: c.mad_ws_origin.clone(),
        }
    }
}
...
pub enum ReconnectPolicy {
    ...
    /// Suitable for headless/`mad -p` where hanging forever is unacceptable.
...
///     "mad-tui", ClientMode::Stdio, env_urls, caps, status_tx,
/// );
///
...
enum ZombieAction {
    ...
    Clear,
    /// A live mad holder is still unconnectable; timer (re)armed, keep waiting.
    Wait,
...
/// The live *mad* PID that ACTUALLY holds the flock on the lock file, if any.
/// `None` for a dead / non-mad PID, OR when the file PID can't be confirmed to be
...
/// stricter (name-matching) mad check since this drives the auto-kill path.
///
...
fn live_mad_lock_holder(lock: &LeaderLock) -> Option<u32> {
    let file_pid = lock.read_pid()?;
    let pid = evictable_holder(file_pid, confirmed_flock_holder(lock.lock_path()))?;
    (crate::util::is_process_alive(pid) && crate::util::is_mad_process_strict(pid)).then_some(pid)
}
...
///
/// The `env_urls.mad_ws_url` determines which leader instance to connect to.
/// Different WS URLs get different leader processes (via hashed socket paths).
///
/// # Arguments
///
/// * `client_type` - Identifier for the client type (e.g., "mad-tui", "vscode")
...
pub async fn connect_or_spawn(
    ...
    if let Some(profile) = xai_mad_sandbox::requested_confinement_profile() {
        return Err(ConnectionError::SandboxConfinement(profile));
    }
    let start = std::time::Instant::now();
    let mut lock = LeaderLock::new(&env_urls.mad_ws_url);
    ...
    loop {
        ...
        match wait_for_socket_connectable(&sock_path, client_type, mode, capabilities.clone()).await
        {
            ...
            Err(e) if is_connect_level_failure(&e) => {
                let holder = live_mad_lock_holder(&lock);
...
///
/// For a **managed install** — the running binary lives under `mad_home`
/// (e.g. `~/.mad/...`) — prefer the managed `~/.mad/bin/mad` symlink. After an
/// auto-update or `mad update` atomically swaps that symlink, `current_exe()`
...
/// `xai_mad_update::auto_update::resolve_restart_exe`.
///
/// For a **dev / out-of-tree binary** (`cargo run`, integration tests, installs
/// not under `mad_home`), keep `current_exe()` so the spawned leader matches the
/// calling binary.
///
/// Falls back to `~/.mad/bin/mad` only when `current_exe()` is unavailable.
fn resolve_exe_for_spawn() -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_with_home(&crate::util::mad_home::mad_home())
}
fn resolve_binary_with_home(mad_home: &Path) -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_impl(mad_home, std::env::current_exe().ok())
}
/// Binary file name for the managed mad install (`mad` / `mad.exe`).
fn managed_mad_bin_name() -> &'static str {
    if cfg!(windows) { "mad.exe" } else { "mad" }
}
...
fn resolve_binary_impl(
    ...
    let managed_bin = mad_home.join("bin").join(managed_mad_bin_name());
    if let Some(ref exe) = current_exe
        && path_is_under(exe, mad_home)
        && managed_bin.exists()
    {
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    cmd.arg("--mad-ws-url").arg(&env_urls.mad_ws_url);
    cmd.arg("--mad-ws-origin").arg(&env_urls.mad_ws_origin);
    ...
    let log_path = crate::util::mad_home::mad_home().join("leader.log");
    ...
    let leader_log = std::env::var("MAD_LEADER_LOG")
        .or_else(|_| std::env::var("RUST_LOG"))
        .unwrap_or_else(|_| "xai_mad_shell=info,xai_acp_lib=warn,xai_mad_mcp=warn".into());
...
mod tests {
    ...
    /// No live mad holder → `Clear`, and any pending timer is reset.
    #[test]
...
";
    ...
    /// `live_mad_lock_holder` returns `None` for a missing or dead PID, so the
    /// zombie net never times/kills a recycled or unrelated PID.
    #[test]
    fn live_mad_lock_holder_none_for_missing_or_dead_pid() {
        let temp = TempDir::new().unwrap();
        let lock = LeaderLock::from_paths(
            temp.path().join("leader.lock"),
            temp.path().join("leader.sock"),
        );
        assert_eq!(live_mad_lock_holder(&lock), None);
    ...
    async fn reconnect_against_hung_leader_exhausts_attempts_without_respawn() {
        ...
        let env_urls = LeaderEnvUrls {
            mad_ws_url: "wss://test.invalid".into(),
            mad_ws_origin: "https://test.invalid".into(),
        };
    ...
    fn resolve_binary_prefers_current_exe() {
        ...
        std::fs::write(bin_dir.join("mad"), "fake-binary").unwrap();
    ...
    fn resolve_binary_prefers_current_exe_over_symlink() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let target_v2 = bin_dir.join("mad-v2");
        std::fs::write(&target_v2, "new-binary").unwrap();
        std::os::unix::fs::symlink(&target_v2, bin_dir.join("mad")).unwrap();
        let result = resolve_binary_with_home(temp.path()).unwrap();
        let current = std::env::current_exe().unwrap();
        assert_eq!(result, current);
    }
    ...
    fn resolve_binary_prefers_managed_symlink_for_managed_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let new_target = bin_dir.join("mad-v2");
        std::fs::write(&new_target, "new-binary").unwrap();
        let managed = bin_dir.join("mad");
        std::os::unix::fs::symlink(&new_target, &managed).unwrap();
        let stale_target = bin_dir.join("mad-v1");
    ...
    fn resolve_binary_prefers_current_exe_for_out_of_tree_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        std::fs::write(bin_dir.join(managed_mad_bin_name()), "managed").unwrap();
    ...
    fn resolve_binary_falls_back_to_managed_when_no_current_exe() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let managed = bin_dir.join(managed_mad_bin_name());
