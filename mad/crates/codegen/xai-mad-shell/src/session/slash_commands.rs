...
use xai_mad_tools::implementations::skills::skill::format_skill_name;
...
///   matching today's Madagascar Build flow that calls our dedicated `skill` tool.
...
pub(super) async fn build_skill_information_for_refs(
    ...
    use xai_mad_tools::implementations::skills::skill::{
...
/// `xai-mad-tools` so it stays identical to the pager's `LoopCommand` and the
...
mod tests {
    ...
    fn listing(name: &str) -> crate::session::workflow::registry::WorkflowListing {
        crate::session::workflow::registry::WorkflowListing {
            ...
            when_to_use: None,
            source: "project",
            path: Some(format!(".mad/workflows/{name}.rhai")),
        }
    }
