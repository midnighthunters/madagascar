...
//! `crate::session::prompt_queue::*` and cross-crate `xai_mad_shell::session::prompt_queue::*`
...
mod tests {
    ...
    fn queue_entry_wire_round_trips_last_editor() {
        let entry = QueueEntryWire {
            ...
            version: 3,
            owner: Some("mad-tui".to_string()),
            last_editor: Some("mad-vscode".to_string()),
            kind: "prompt".to_string(),
            text: "hello".to_string(),
            position: 0,
            combined_texts: None,
        ...
        assert_eq!(json["lastEditor"], "mad-vscode");
