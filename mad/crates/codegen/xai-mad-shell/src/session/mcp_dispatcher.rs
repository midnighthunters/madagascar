...
//!
//! Receives [`xai_mad_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`xai_mad_mcp::liveness`]),
//! - the [`xai_mad_mcp::servers::MadClientHandler`] (server-pushed
...
pub struct McpServerStatusPayload {
    ...
    /// MCP server name (`mad_com_linear`, `github`, ...).
    pub name: String,
    /// `managed` (sourced from cli-chat-proxy / `mad_com_` prefix)
    /// or `local` (user `.mad/config.toml`).
...
pub enum McpServerStatus {
    /// Client is in [`xai_mad_mcp::servers::ClientStateKind::Ready`]
...
/// [`xai_mad_mcp::servers::McpClientEventKind`]). Used as the
...
pub fn build_payload(
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        // NeedsAuth ("visit mad.com"), not a generic Unavailable, so a
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && xai_mad_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
mod tests {
    ...
    fn managed_handshake_non_auth_stays_unavailable() {
        for reason in ["403 Forbidden", "cli-chat-proxy returned 502"] {
            ...
            let ev = McpClientEvent::HandshakeFailed {
                server: "mad_com_slack".to_string(),
    ...
    /// Contract: managed server names (starting with `mad_com_`)
    /// are classified as `Managed`; everything else as `Local`.
    #[test]
    fn classify_source_uses_managed_prefix() {
        assert_eq!(classify_source("mad_com_linear"), McpServerSource::Managed);
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        ...
        use xai_mad_mcp::servers::{McpClient, McpState};
    ...
    fn recoverable_http_servers_excludes_managed_stdio_and_disabled() {
        let configs = vec![
            ...
            http_cfg("mad_com_slack"), // managed
