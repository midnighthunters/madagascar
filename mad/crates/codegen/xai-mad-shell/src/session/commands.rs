...
pub enum SessionCommand {
    ...
    RestorePlanApproval,
    GetToolOverrides {
        respond_to: oneshot::Sender<Option<xai_mad_sampling_types::ToolOverrides>>,
    },
    ...
    SetToolOverrides {
        overrides: xai_mad_sampling_types::ToolOverrides,
    },
    Prompt {
        prompt_id: String,
        ...
        tool_overrides_update: Option<xai_mad_sampling_types::ToolOverridesUpdate>,
    ...
    SetSessionModel {
        sampling_config: xai_mad_sampler::SamplerConfig,
        use_concise: bool,
    ...
    /// rather than the agent-level default (e.g. `mad-4.5`).
    ///
    /// Keeps the existing base_url, api_key, and other config — only changes
    /// the `model` field sent in the `x-mad-model-override` header and merges
    ...
    ReloadPlugins {
        registry: Option<std::sync::Arc<xai_mad_agent::plugins::PluginRegistry>>,
    },
    /// Re-discover the session's own project hooks (`.mad/hooks`,
    ...
    SnapshotToolDefinitions {
        respond_to: oneshot::Sender<Vec<xai_mad_sampling_types::ToolSpec>>,
    },
    ...
    KillBackgroundTask {
        task_id: String,
        respond_to: oneshot::Sender<Result<xai_mad_tools::types::KillOutcome, String>>,
    },
    DeleteScheduledTask {
        task_id: String,
    ...
    ListTasks {
        respond_to: oneshot::Sender<Option<Vec<xai_mad_tools::types::TaskSnapshot>>>,
    },
    ...
    PluginsList {
        respond_to:
            oneshot::Sender<Option<std::sync::Arc<xai_mad_agent::plugins::PluginRegistry>>>,
    },
    ...
    /// tool-free model call (default `mad-build-0.1` when available via
    ...
    /// `mad-build` model, low temperature, and capped output tokens.
    RewriteMemoryNote {
        raw_text: String,
    ...
    TakeHarnessTraceTurns {
        respond_to:
            oneshot::Sender<Vec<Vec<xai_mad_sampling_types::conversation::ConversationItem>>>,
    },
