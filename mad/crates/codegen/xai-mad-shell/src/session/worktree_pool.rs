...
//!   under `~/.mad/worktree_pool/<instance_id>/` with a `.pid` liveness
...
/// `~/.mad/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.mad/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.mad/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.mad/worktree_pool/`, reads each `.pid` file, and checks
...
/// The base pool directory under `~/.mad/`.
...
/// `CleanAll` mode. Placed under `~/.mad/worktree_pool/<instance_id>/<uuid>/`.
