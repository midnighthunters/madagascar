use xai_mad_tools::computer::local::{LocalTerminalBackend, MockFs};
...
async fn tool_bridge_routes_writes_through_injected_fs() {
    ...
    let config = ToolServerConfig {
        tools: vec![
            ToolConfig {
                id: "MadagascarBuild:read_file".into(),
                params: None,
                name_override: None,
    ...
    let ctx = SessionContext {
        ...
        fs,
        cwd: cwd.clone(),
        session_folder: std::env::temp_dir().join("mad-test-fs"),
        ...
        skills: vec![],
        state_path: std::env::temp_dir().join("mad-test-fs/tool_state.json"),
        memory_backend: None,
