        ...
        .run_until(async {
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::RecapRequest(artifact) = msg {
                    ...
                    assert!(
                        artifact.x_mad_req_id.starts_with("xai-recap-"),
                        "req id: {}",
                        artifact.x_mad_req_id
                    );
...
/// shape, so it also covers the laxer mad ChatCompletions/Responses shapes.)
#[test]
fn over_budget_recap_serializes_to_well_formed_messages_request() {
    use crate::session::helpers::session_recap;
    use xai_mad_sampling_types::messages::{ContentBlock, MessageContent, MessageRole};
    use xai_mad_sampling_types::{ConversationRequest, ToolCall, rs};
    ...
    // mad backend => strip_reasoning=false; the over-budget branch strips anyway.
    let items = session_recap::budget_recap_items(conv, "system-reminder", false, 8_000);
    let req = ConversationRequest::from_items(items);
    let msg = xai_mad_sampling_types::build_messages_request(&req);
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            cfg.api_backend = xai_mad_sampling_types::ApiBackend::Responses;
            ...
            let conv_id = recap_req
                .header("x-mad-conv-id")
                .expect("recap must send x-mad-conv-id");
            assert!(
...
async fn recap_request_sends_hosted_tools_under_backend_search() {
    use xai_mad_sampling_types::HostedTool;
    use xai_mad_test_support::MockInferenceServer;
        ...
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            // Backend-search fixture: agent carries hosted tools and both gates are on.
            {
                ...
                *agent_slot = xai_mad_agent::Agent::new(
...
async fn recap_hosted_tools_reflect_the_active_per_turn_override() {
    use xai_mad_sampling_types::{HostedTool, SearchDateBound, ToolOverrides, XSearchOptions};
    use xai_mad_test_support::MockInferenceServer;
