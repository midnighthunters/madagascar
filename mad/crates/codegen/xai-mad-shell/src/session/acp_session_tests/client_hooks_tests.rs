...
fn install_client_hook(
    actor: &SessionActor,
    event: xai_mad_hooks::event::HookEventName,
    callback_ids: &[&str],
) {
...
async fn client_hooks_fire_without_file_registry() {
    ...
    local
        .run_until(async {
            ...
            install_client_hook(
                &actor,
                xai_mad_hooks::event::HookEventName::Stop,
                &["cb_0"],
            );
            ...
            actor.fire_hook(
                xai_mad_hooks::event::HookEventName::Stop,
                None,
                xai_mad_hooks::event::HookPayload::Stop {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
                xai_mad_tools::registry::types::ToolConfig::for_tool::<
                    xai_mad_tools::implementations::use_tool::UseTool,
                >(),
            ])
            .await;
            ...
            client_hooks.insert(
                ...
                vec![crate::extensions::hooks::ClientHookGroup {
                    matcher: Some(
                        xai_mad_hooks::matcher::HookMatcher::new("linear__save_issue").unwrap(),
                    ),
...
async fn post_tool_use_and_failure_never_double_fire() {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_mad_build_agent_with_todo().await;
...
fn file_registry_with_stop_spec(
    event: xai_mad_hooks::event::HookEventName,
    script: &str,
) -> xai_mad_hooks::discovery::HookRegistry {
    let (mut registry, _) = xai_mad_hooks::discovery::load_hooks(None, None);
    registry.append_specs(vec![xai_mad_hooks::config::HookSpec {
...
async fn subagent_session_gates_on_subagent_stop() {
    ...
    local
        .run_until(async {
            ...
            *actor.hook_registry.borrow_mut() =
                Some(std::sync::Arc::new(file_registry_with_stop_spec(
                    xai_mad_hooks::event::HookEventName::SubagentStop,
...
/// `"subagent_stop"` on the wire, matching `MAD_HOOK_EVENT`.
#[tokio::test(flavor = "current_thread")]
async fn alias_envelope_serializes_canonical_event_name() {
    ...
    local
        .run_until(async {
            ...
            let envelope = actor.make_hook_envelope(
                xai_mad_hooks::event::HookEventName::SubagentEnd,
                None,
                xai_mad_hooks::event::HookPayload::SubagentStop {
                    phase: xai_mad_hooks::event::SubagentStopPhase::Observe,
