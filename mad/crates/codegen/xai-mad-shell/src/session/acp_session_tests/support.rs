...
#[cfg(test)]
pub(crate) async fn test_agent_default() -> xai_mad_agent::Agent {
    test_agent_with_tools(vec![]).await
}
#[cfg(test)]
pub(crate) async fn test_agent_backend_search(
    hosted_tools: Vec<xai_mad_sampling_types::HostedTool>,
) -> xai_mad_agent::Agent {
    let base = test_agent_default().await;
    xai_mad_agent::Agent::new(
        base.definition().clone(),
        xai_mad_agent::PromptContext::default(),
        String::new(),
        base.tool_bridge().clone(),
        xai_mad_agent::ReminderPolicy::default(),
        xai_mad_agent::CompactionPolicy::default(),
        hosted_tools,
        true,
    )
}
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> xai_mad_agent::Agent {
    use xai_mad_tools::implementations::mad_build::update_goal::UpdateGoalTool;
    use xai_mad_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![ToolConfig::for_tool::<UpdateGoalTool>()]).await
}
/// Mad-build agent with the real `TodoWriteTool` (id `todo_write`, kind
/// `Plan`) registered, so `tool_for_kind(ToolKind::Plan)` resolves through the
/// live toolset instead of the literal fallback.
#[cfg(test)]
pub(crate) async fn test_mad_build_agent_with_todo() -> xai_mad_agent::Agent {
    use xai_mad_tools::implementations::mad_build::todo::TodoWriteTool;
    use xai_mad_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> xai_mad_agent::Agent {
    use xai_mad_tools::implementations::mad_build::enter_plan_mode::EnterPlanModeTool;
    use xai_mad_tools::implementations::mad_build::exit_plan_mode::ExitPlanModeTool;
    use xai_mad_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<xai_mad_tools::registry::types::ToolConfig>,
) -> xai_mad_agent::Agent {
    test_agent_from_config(
        xai_mad_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        xai_mad_agent::AgentDefinition::default_mad_build(),
        std::sync::Arc::new(xai_mad_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
pub(crate) async fn test_agent_with_user_message_template(
    template: xai_mad_agent::prompt::user_message::UserMessageTemplate,
) -> xai_mad_agent::Agent {
    let mut definition = xai_mad_agent::AgentDefinition::default_mad_build();
...
async fn test_agent_from_config(
    ...
    definition: xai_mad_agent::AgentDefinition,
    backend: std::sync::Arc<dyn xai_mad_tools::computer::types::TerminalBackend>,
) -> xai_mad_agent::Agent {
    use xai_mad_tools::computer::local::LocalFs;
    use xai_mad_tools::computer::types::AsyncFileSystem;
    ...
    let ctx = SessionContext {
        ...
        session_folder: std::env::temp_dir().join("mad-test"),
...
pub(crate) async fn create_test_actor_ex(
    ...
    let cwd = xai_mad_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
    let fs = Arc::new(xai_mad_workspace::file_system::MockFs::new(
        cwd.to_path_buf(),
    ));
    ...
    tool_context.task_completion_reservations =
        Some(xai_mad_tools::reminders::task_completion::TaskCompletionReservations::default());
    tool_context.task_wake_suppressed =
        Some(xai_mad_tools::reminders::task_completion::TaskWakeSuppressed::default());
    ...
    let actor = SessionActor {
        ...
        permissions: xai_mad_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
