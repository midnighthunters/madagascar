...
fn drop_cli_catchall_allows(
    rules: Vec<xai_mad_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<xai_mad_workspace::permission::types::PermissionRule>,
    Vec<xai_mad_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if xai_mad_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
mod runtime_containment_tests {
    ...
    const CHILD_ENV: &str = "XAI_MAD_SHELL_RUNTIME_CONTAINMENT_CHILD";
...
mod cli_catchall_drop_tests {
    ...
    use xai_mad_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    use xai_mad_workspace::permission::rules::parse_permission_rule;
    use xai_mad_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
...
pub(crate) async fn spawn_session_actor(
    ...
    attribution_callback: Option<xai_mad_sampler::SharedAttributionCallback>,
    ...
    web_search_sampling_config: Option<xai_mad_sampler::SamplerConfig>,
    ...
    app_builder_deployer_config: xai_mad_tools::implementations::mad_build::deploy_app::AppBuilderDeployerConfig,
    ...
    prompt_audience: xai_mad_agent::prompt::context::PromptAudience,
    ...
    plugin_registry: Option<std::sync::Arc<xai_mad_agent::plugins::PluginRegistry>>,
    plugin_registry_handle: Option<xai_mad_agent::plugins::SharedPluginRegistryHandle>,
    models_manager: crate::agent::models::ModelsManager,
    inherited_permission_handle: Option<xai_mad_workspace::permission::PermissionHandle>,
    api_key_provider: Option<xai_mad_tools::types::SharedApiKeyProvider>,
    image_description_model: String,
    hook_registry_override: Option<std::sync::Arc<xai_mad_hooks::discovery::HookRegistry>>,
    workspace_ops: xai_mad_workspace::WorkspaceOps,
    cli_permission_rules: Vec<xai_mad_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    parent_terminal_backend: Option<
        std::sync::Arc<dyn xai_mad_tools::computer::types::TerminalBackend>,
    >,
    parent_scheduler_handle: Option<
        xai_mad_tools::implementations::mad_build::scheduler::types::SchedulerHandle,
    >,
    ...
    xai_mad_agent::AgentBuildError,
> {
    if max_turns == Some(0) {
        return Err(xai_mad_agent::AgentBuildError::InvalidConfig(
        ...
        inherited_permission_handle
    {
        ...
        let mut permission_config =
            xai_mad_workspace::permission::resolution::resolve_permission_config_with_fallback(
                tool_context.cwd.as_path(),
                project_trusted,
            )
            .await;
        let yolo_pin = xai_mad_workspace::permission::resolution::yolo_disabled_by_policy();
            ...
            && !dropped_catchalls.is_empty()
        {
            ...
            if startup_hints.non_interactive {
                eprintln!("mad: --allow catch-all ignored: {reason}");
            }
        }
            ...
            match &mut permission_config {
                ...
                None => {
                    permission_config = Some(
                        xai_mad_workspace::permission::types::PermissionConfig::new(
        ...
        let deny_read_globs = permission_config
            .as_ref()
            .map(xai_mad_workspace::permission::resolution::deny_read_globs_from_config)
            .unwrap_or_default();
        let hub_permission = if xai_mad_workspace::permission::hitl_permission_live_enabled() {
            ...
            let transport = server
                .and_then(|server| {
                    xai_mad_workspace::permission::ToolServerPermissionTransport::from_session_id(
                        server,
                ...
                .map(|t| {
                    std::sync::Arc::new(t)
                        as std::sync::Arc<
                            dyn xai_mad_workspace::permission::PermissionHookTransport,
                        >
                });
        ...
        let (permissions, permission_events_rx) =
            xai_mad_workspace::permission::spawn_permission_manager_with_hub(
    ...
    let web_search_config = if disable_web_search {
        xai_mad_tools::implementations::WebSearchConfig::Disabled
    } else if let Some(cfg) = web_search_sampling_config {
        if let Some(api_key) = cfg.api_key {
            xai_mad_tools::implementations::WebSearchConfig::Enabled {
                api_key,
    ...
    let context_window_override = std::env::var("MAD_DEBUG_CONTEXT_WINDOW")
        .ok()
    ...
    if let Some(cw) = context_window_override {
        tracing::warn!(
            ...
            "MAD_DEBUG_CONTEXT_WINDOW override active"
        );
    }
    let chat_state_sampling_config = xai_mad_sampling_types::SamplingConfig {
    ...
    let task_completion_reservations =
        xai_mad_tools::reminders::task_completion::TaskCompletionReservations::default();
    let task_wake_suppressed =
        xai_mad_tools::reminders::task_completion::TaskWakeSuppressed::default();
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let index_root_for_session =
        xai_mad_workspace::session::git::find_git_root_from_path(tool_context.cwd.as_path())
    ...
    let resolve_search_shadows = || {
        ...
        xai_mad_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    let resolve_policy = || crate::util::config::resolve_shell_env_policy(effective_cfg.as_ref());
    let terminal_backend: std::sync::Arc<dyn xai_mad_tools::computer::types::TerminalBackend> =
        match terminal_backend_kind {
            ...
            TerminalBackendKind::AcpClient => {
                std::sync::Arc::new(crate::terminal::AcpTerminalAdapter::new(
                    ...
                    as std::sync::Arc<dyn xai_mad_tools::computer::types::TerminalBackend>
            }
    ...
    let fs_backend: std::sync::Arc<dyn xai_mad_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(xai_mad_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(xai_mad_tools::computer::local::LocalFs)
        };
    ...
    let compaction_policy = xai_mad_agent::CompactionPolicy {
    ...
    let (user_question_tx, user_question_rx) = tokio::sync::mpsc::unbounded_channel::<
        xai_mad_tools::implementations::mad_build::ask_user_question::types::UserQuestionRequest,
    >();
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn xai_mad_tools::types::memory_backend::MemoryBackend>,
    > = if let Some(ref storage) = memory_storage_for_session {
        if let Err(e) = storage.ensure_initialized() {
            tracing::warn!(
                target: xai_mad_telemetry::memory_log::TARGET,
                error = %e,
        ...
        let backend: std::sync::Arc<dyn xai_mad_tools::types::memory_backend::MemoryBackend> =
        ...
        xai_mad_telemetry::session_ctx::log_event(
            xai_mad_telemetry::memory_telemetry::MemorySessionInit {
                session_id: session_info.id.to_string(),
                memory_enabled: true,
                watcher_config_enabled: watcher_config.enabled,
                watcher_started,
    ...
    let managed_gateway_tool_client = auth_manager.as_ref().map(|am| {
        xai_mad_tools::types::resources::ManagedGatewayToolClient(Arc::new(
    ...
    agent
        ...
        .await;
    let harness_metrics = if telemetry_enabled || xai_mad_telemetry::external::is_active() {
        ...
        Some(super::telemetry::SessionHarnessMetrics {
            ...
            permission_mode: if session_yolo_mode {
                xai_mad_telemetry::enums::PermissionMode::AlwaysApprove
            } else if session_auto_mode
                && crate::util::config::auto_permission_mode_enabled_from_disk()
            {
                xai_mad_telemetry::enums::PermissionMode::Auto
            } else {
                xai_mad_telemetry::enums::PermissionMode::Ask
            },
    ...
    let resolved_task_output =
        xai_mad_tools::reminders::task_completion::resolve_task_output_tool_name(
            agent.tool_bridge(),
        )
        .await;
    let resolved_read =
        xai_mad_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge())
            .await;
    ...
    tool_context.task_output_tool_name = resolved_task_output.unwrap_or_else(|| {
        xai_mad_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL.to_string()
    });
    let scheduler_handle_for_handle = {
        let toolset = agent.tool_bridge().toolset();
        let res = toolset.resources.lock().await;
        res.get::<xai_mad_tools::implementations::mad_build::scheduler::types::SchedulerHandle>()
            .cloned()
    };
    ...
    let feedback_client_type = match client_type {
        ClientType::MadTUI => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Tui,
        ClientType::MadWeb => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Web,
        ...
        ClientType::MadPager => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Tui,
    };
    ...
    let resolved_workspace_root = xai_mad_workspace::session::git::find_git_root_from_path(
    ...
    let sampler_retry_policy = xai_mad_sampler::RetryPolicy {
    ...
    let (sampler_event_tx, sampler_event_rx) =
        tokio::sync::mpsc::unbounded_channel::<xai_mad_sampler::SamplingEvent>();
    let sampler_handle = xai_mad_sampler::SamplerActor::spawn(
    ...
    let mut hook_discovery_errors: Vec<xai_mad_hooks::error::HookError> = Vec::new();
    let built_hook_registry: Option<Arc<xai_mad_hooks::discovery::HookRegistry>> =
        if let Some(override_reg) = hook_registry_override {
            ...
            let git_root = xai_mad_workspace::session::git::find_git_root_from_path(cwd_path).ok();
    ...
    let (goal_update_tx, goal_update_rx) = tokio::sync::mpsc::unbounded_channel::<
        xai_mad_tools::implementations::mad_build::update_goal::UpdateGoalEnvelope,
    >();
    ...
    let (workflow_launch_tx, mut workflow_launch_rx) = tokio::sync::mpsc::unbounded_channel::<
        xai_mad_tools::implementations::mad_build::workflow::WorkflowLaunchEnvelope,
    >();
    {
        ...
        tokio::spawn(async move {
            ...
            use xai_mad_tools::implementations::mad_build::workflow::WorkflowLaunchAck;
                ...
                if !background_workflows_enabled {
                    let _ = ack.send(WorkflowLaunchAck::Rejected {
                        ...
                        detail: "Background workflows are disabled for this session \
                                 ([workflows] enabled = false / MAD_WORKFLOWS=0 / remote flag)."
                            .into(),
                    });
                    continue;
                }
    ...
    let resolved_tool_overrides: std::sync::Arc<
        arc_swap::ArcSwapOption<xai_mad_sampling_types::ToolOverrides>,
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        max_turns,
        max_retries: xai_mad_sampler::resolve_max_retries(max_retries),
        ...
        vcs_kind: {
            ...
            match xai_mad_workspace::session::git::discover_git_root(root) {
                xai_mad_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                    xai_mad_workspace::session::git::detect_vcs_kind(&git_root)
                }
                _ => xai_mad_workspace::session::git::VcsKind::None,
            }
        },
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(xai_mad_tools::types::tool_index::ToolIndex(
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(xai_mad_tools::types::resources::PlanFilePath(plan_path))
            .await;
    }
    ...
    session
        ...
        .tool_bridge()
        .update_resource(
            xai_mad_tools::implementations::mad_build::workflow::WorkflowLaunchHandle(
    ...
    if !background_workflows_enabled {
        session
            ...
            .tool_bridge()
            .update_resource(
                xai_mad_tools::implementations::mad_build::update_goal::GoalUpdateHandle(
    ...
    if let Some(storage) = session.memory.storage() {
        ...
        tokio::task::spawn_local(async move {
            ...
            ) && let Ok(files) = storage.list_memory_files()
            {
                ...
                xai_mad_telemetry::session_ctx::log_event(
                    xai_mad_telemetry::memory_telemetry::MemoryReindex {
    ...
    }
    {
        ...
        use xai_mad_tools::implementations::mad_build::ask_user_question::{
        ...
        tokio::task::spawn_local(async move {
            while let Some(mut request) = user_question_rx.recv().await {
                use xai_mad_tools::implementations::mad_build::ask_user_question::AskUserQuestionMode;
    ...
    let telemetry_ctx = xai_mad_telemetry::session_ctx::TelemetryCtx::new(
    ...
    if let Some(metrics) = harness_metrics {
        ...
        tokio::spawn(async move {
            ...
            xai_mad_telemetry::session_ctx::log_event_dual(telemetry_enabled, ev);
        });
    }
    tokio::task::spawn_local(async move {
        xai_mad_telemetry::session_ctx::with_session_ctx(
            telemetry_ctx,
            run_session(
                session,
                cmd_rx,
...
> {
    let (init_tx, init_rx) = tokio::sync::oneshot::channel::<
        Result<SessionInitResult, xai_mad_agent::AgentBuildError>,
    >();
        ...
        .spawn(move || {
            ...
            let rt = match build_session_runtime() {
                ...
                Err(e) => {
                    ...
                    let _ = init_tx.send(Err(xai_mad_agent::AgentBuildError::RuntimeBuild(e)));
                    return;
                }
            };
    ...
    conversation.iter().any(|item| {
        ...
        let contains = |needle: &str| {
            u.content.iter().any(|part| {
                matches!(
                    part,
                    xai_mad_sampling_types::conversation::ContentPart::Text { text }
