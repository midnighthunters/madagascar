...
fn partition_rules_by_scope(
    files: Vec<xai_mad_agent::prompt::agents_md::AgentConfigFile>,
    mad_home: &std::path::Path,
    vendor_homes: &[(std::path::PathBuf, bool)],
    workspace_root: Option<&std::path::Path>,
) -> (
    Vec<xai_mad_agent::prompt::user_message::RuleEntry>,
    Vec<xai_mad_agent::prompt::user_message::RuleEntry>,
) {
    ...
    for file in files {
        ...
        let entry = xai_mad_agent::prompt::user_message::RuleEntry::from(file);
        if is_user_rule {
            user.push(entry);
        } else {
...
mod partition_rules_by_scope_tests {
    ...
    use std::path::Path;
    use xai_mad_agent::prompt::agents_md::AgentConfigFile;
    ...
    fn paths(entries: &[xai_mad_agent::prompt::user_message::RuleEntry]) -> Vec<&str> {
        entries.iter().map(|entry| entry.content.as_str()).collect()
    }
    #[test]
    fn partitions_custom_mad_and_vendor_home_rules_as_user_scope() {
        let files = vec![
    ...
    #[test]
    fn mad_home_nested_in_workspace_keeps_direct_surfaces_user_scoped() {
        let files = vec![
            file("/repo/config/AGENTS.md"),
            file("/repo/config/rules/global.md"),
            file("/repo/config/.mad/rules/project.md"),
    ...
    #[test]
    fn nested_mad_home_workspace_files_stay_workspace_scoped() {
        let files = vec![
            file("/custom/mad/rules/global.md"),
            file("/custom/mad/worktrees/repo/.cursor/rules/project.md"),
            file("/custom/mad/worktrees/repo/src/AGENTS.md"),
        ];
        let (workspace, user) = partition_rules_by_scope(
            files,
            Path::new("/custom/mad"),
            &[],
            Some(Path::new("/custom/mad/worktrees/repo")),
        );
        assert_eq!(paths(&user), vec!["/custom/mad/rules/global.md"]);
        assert_eq!(
            paths(&workspace),
            vec![
...
/// end for mad ordering), so a failed offload never
...
impl SessionActor {
                ...
                !matches!(
                    ...
                    ConversationItem::User(u)
                        if u.synthetic_reason
                            == Some(xai_mad_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    ...
    async fn build_templated_user_message(
        ...
        use xai_mad_agent::prompt::agents_md::read_agents_config_with_paths;
        use xai_mad_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use xai_mad_workspace::file_system::{git_status_short, jj_status};
        use xai_mad_workspace::session::git::VcsKind;
    ...
    ) -> Vec<xai_mad_agent::prompt::user_message::McpServerEntry> {
        use xai_mad_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let client = xai_mad_sampler::SamplingClient::new(sampler_config).map_err(|e| {
