...
impl SessionActor {
    pub(super) async fn handle_set_session_model(
        ...
        self.chat_state_handle
            .update_sampling_config(xai_mad_sampling_types::SamplingConfig {
                ...
                if let ConversationItem::System(sys) = item {
                    if use_concise {
                        sys.content = std::sync::Arc::<str>::from(
                            xai_mad_agent::prompt::template::COMPACT_SYSTEM_PROMPT,
                        );
                    } else {
                        sys.content =
    ...
    ///
    /// Builds a fresh [`xai_mad_agent::Agent`] from the cached
    /// [`crate::session::agent_rebuild::AgentRebuildSpec`] + the supplied
    /// [`xai_mad_agent::AgentDefinition`], replaces `self.agent`,
