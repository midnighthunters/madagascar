...
/// `SamplingError::is_auth_error` in xai-mad-sampling-types: 403 is
...
pub(super) async fn call_with_auth_retry<F, Fut>(
    ...
    tool_name: &str,
    mut call: F,
) -> Result<xai_mad_tools::types::output::ToolRunResult, xai_tool_runtime::ToolError>
where
    F: FnMut() -> Fut,
    Fut: std::future::Future<
            Output = Result<
                xai_mad_tools::types::output::ToolRunResult,
...
impl SessionActor {
    ...
    fn resolve_hosted(
        ...
        Vec<xai_mad_sampling_types::HostedTool>,
        xai_mad_sampling_types::ToolOverrides,
    ) {
        let mut tools = self.agent.borrow().hosted_tools().to_vec();
        let applied = xai_mad_sampling_types::apply_tool_overrides(
            &mut tools,
    ...
    pub(crate) fn effective_hosted_tools(&self) -> Vec<xai_mad_sampling_types::HostedTool> {
        self.resolve_hosted().0
    }
    pub(crate) fn hosted_tools_for_turn(&self) -> Vec<xai_mad_sampling_types::HostedTool> {
    ...
    /// inherits it.
    pub(crate) fn set_tool_overrides(&self, overrides: xai_mad_sampling_types::ToolOverrides) {
    ...
    pub(crate) fn apply_tool_overrides_update(
        &self,
        update: Option<xai_mad_sampling_types::ToolOverridesUpdate>,
    ) {
    ...
    pub(super) async fn reconstruct_full_config(&self) -> SamplingConfig {
        ...
        impl xai_mad_sampler::HeaderInjector for TraceContextInjector {
        ...
        impl xai_mad_sampler::BearerResolver for AuthManagerBearerResolver {
        ...
        let cfg = self
            ...
            .get_sampling_config()
            .await
            .unwrap_or_else(|| xai_mad_sampling_types::SamplingConfig {
                base_url: String::new(),
                model: String::new(),
        ...
        SamplingConfig {
            ...
            bearer_resolver: if use_bearer_resolver {
                self.auth_manager
                    .as_ref()
                    .map(|am| -> xai_mad_sampler::SharedBearerResolver {
    ...
    pub(crate) async fn wire_permission_auto_llm_classifier(self: &Arc<Self>) {
        ...
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<(
            Vec<xai_mad_workspace::permission::ClassifierMessage>,
            tokio::sync::oneshot::Sender<
                Result<String, xai_mad_workspace::permission::ClassifierFailure>,
            >,
        )>();
            ...
            while let Some((messages, respond_to)) = rx.recv().await {
                let result = async {
                    let (sampling_client, model) = match &aux_classifier_sampler {
                        ...
                        None => {
                            let client = session
                                ...
                                .await
                                .map_err(|e| xai_mad_workspace::permission::ClassifierFailure::TransportError(
                                    e.to_string(),
                                ))?;
                            let model = session
                                .chat_state_handle
                    ...
                    let items = messages
                        .into_iter()
                        .map(|m| match m.role {
                            xai_mad_workspace::permission::ClassifierMessageRole::System => {
                                ConversationItem::system(m.text)
                            }
                            xai_mad_workspace::permission::ClassifierMessageRole::User => {
                    ...
                    let request = ConversationRequest {
                        ...
                        json_schema: Some(
                            xai_mad_workspace::permission::classifier_output_json_schema(),
                        ),
                        ...
                        x_mad_session_id: Some(session_id),
                    ...
                    let response = tokio::time::timeout(classify_timeout, fut)
                        .await
                        .map_err(|_| {
                            xai_mad_workspace::permission::ClassifierFailure::Timeout
                        })?
                        .map_err(|e| xai_mad_workspace::permission::ClassifierFailure::TransportError(
                            e.to_string(),
                        ))?;
        ...
        let clf =
            xai_mad_workspace::permission::LlmPermissionClassifier::with_channel(tx, prompt_type);
        debug_assert!(
            clf.has_side_query(),
    ...
    pub(super) async fn resolve_aux_sampler_config(
        &self,
        slug: &str,
    ) -> Option<xai_mad_sampler::SamplerConfig> {
        let creds = self.chat_state_handle.get_credentials().await;
        ...
        let disable_api_key_auth = self
            .auth_manager
            .as_ref()
            .map(|am| am.mad_com_config().api_key_auth_disabled())
            .unwrap_or(false);
    ...
    async fn resolve_auto_classifier_sampler(
        &self,
        slug: &str,
    ) -> Option<(xai_mad_sampler::SamplingClient, String)> {
        ...
        let client = xai_mad_sampler::SamplingClient::new(cfg)
            .map_err(|e| {
    ...
    pub(super) async fn prepare_chat_completion(
        &self,
        force_http1: bool,
    ) -> Result<xai_mad_sampler::SamplingClient, acp::Error> {
        self.refresh_token_if_expired().await;
        let mut full_config = self.reconstruct_full_config().await;
        full_config.force_http1 = force_http1;
        let sampling_client =
            xai_mad_sampler::SamplingClient::new(full_config).map_err(|e| self.to_acp_error(e))?;
        Ok(sampling_client)
    }
    ...
    /// `xai-mad-sampler` instead of constructing a new
    /// `OaiCompatClient`.
    ///
    ...
    pub(crate) async fn handle_sampling_failure(
        self: &Arc<Self>,
        error: xai_mad_sampler::SamplingErrorInfo,
    ) -> Result<SamplerFailureRecovery, acp::Error> {
        use xai_mad_sampler::SamplingErrorKind;
        if self.tool_context.task_output_token_budget.is_some() {
        ...
        let client_version = xai_mad_version::VERSION;
        if auth_mode == crate::auth::AuthMode::WebLogin {
            let msg = format!(
                "{detailed_message}\n\n\
                 ...
                 To fix: run `mad logout` then `mad login` to re-authenticate with OAuth2.\n\n\
        ...
        let error_type = if xai_mad_sampling_types::is_context_length_error(&error.message) {
            "context_length"
        } else {
            error.kind.as_str()
        };
    ...
    pub(crate) async fn run_turn_via_sampler(
        ...
        let request_id = xai_mad_sampler::RequestId::random();
            ...
            .await
        {
            ...
            Err(rich_err) => {
                self.turn_stream_drained.lock().take();
                let info = xai_mad_sampler::SamplingErrorInfo::from(&rich_err);
...
fn resolve_configured_cutoff(
    seed: Option<xai_mad_sampling_types::ToolOverrides>,
    base: Option<&xai_mad_sampling_types::ToolOverrides>,
) -> xai_mad_sampling_types::ToolOverrides {
    use xai_mad_sampling_types::{ToolOverrides, WebSearchOptions, XSearchOptions};
    let ToolOverrides {
        x_search: seed_x,
        web_search: seed_w,
...
mod configured_cutoff_tests {
    ...
    fn inherited_cutoff_agrees_with_the_wire_echo() {
        use xai_mad_sampling_types::{HostedTool, apply_tool_overrides};
