...
impl SessionActor {
    ...
    pub(super) async fn handle_side_question(&self, question: &str) -> Result<String, String> {
        ...
        let request = ConversationRequest {
            ...
            temperature: None,
            x_mad_conv_id: Some(btw_session_id.clone()),
            x_mad_req_id: Some(format!("xai-btw-{}", uuid::Uuid::new_v4())),
            x_mad_session_id: Some(parent_session_id.clone()),
    ...
    pub(super) async fn handle_recap(&self, auto: bool) {
        ...
        let x_mad_conv_id = format!("recap-{}", uuid::Uuid::new_v4());
        let x_mad_req_id = format!("xai-recap-{}", uuid::Uuid::new_v4());
        ...
        let request = ConversationRequest {
            ...
            model: Some(model.clone()),
            temperature: None,
            x_mad_conv_id: Some(x_mad_conv_id.clone()),
            x_mad_req_id: Some(x_mad_req_id.clone()),
            x_mad_session_id: Some(self.session_info.id.to_string()),
            x_mad_agent_id: Some(xai_mad_telemetry::id::agent_id()),
        ...
        // Applies to manual `/recap` too: spinner-less clients (e.g. Mad
    ...
    fn persist_recap_request_artifact(
        ...
        let artifact = RecapRequestFile {
            ...
            x_mad_req_id: x_mad_req_id.to_owned(),
            x_mad_conv_id: x_mad_conv_id.to_owned(),
            strip_reasoning,
    ...
    /// (`MAD_PROMPT_SUGGESTIONS_MODEL`) > `[models] prompt_suggestion`
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// [`prompt_suggest::DEFAULT_SUGGEST_MODEL`] (`mad-build-0.1`). Every
    ...
    /// `mad-build-0.1` for OAuth users) the request is **skipped
    ...
    /// a reasoning model's response, and some models (e.g. `mad-build`)
    ...
    pub(super) async fn handle_suggest_prompt(
        ...
        let request = ConversationRequest {
            ...
            model: Some(model),
            temperature: None,
            x_mad_conv_id: Some(format!("promptsuggest-{}", uuid::Uuid::new_v4())),
            x_mad_req_id: Some(format!("xai-promptsuggest-{}", uuid::Uuid::new_v4())),
            x_mad_session_id: Some(self.session_info.id.to_string()),
            x_mad_agent_id: Some(xai_mad_telemetry::id::agent_id()),
            ..Default::default()
        };
        ...
        tracing::debug!(
            raw_preview = %xai_mad_tools::util::truncate_str(raw.trim(), 60),
