...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        &self,
        bridge: &xai_mad_tools::bridge::ToolBridge,
    ) -> Result<(), String> {
        ...
        bridge
            .register_mcp_tools(
                ...
                xai_mad_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                xai_mad_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) fn emit_memory_session_summary(
        ...
        xai_mad_telemetry::session_ctx::log_event(
            xai_mad_telemetry::memory_telemetry::MemorySessionSummary {
    ...
    async fn run_dream_model_call(&self, user_message: &str) -> Result<String, acp::Error> {
        ...
        let request = ConversationRequest {
            ...
            model: Some(model),
            x_mad_conv_id: Some(format!("dream-{}", uuid::Uuid::new_v4())),
            x_mad_req_id: Some(format!("xai-dream-{}", uuid::Uuid::new_v4())),
    ...
    pub(super) async fn run_memory_flush(
        ...
        tracing::info!(target: xai_mad_telemetry::memory_log::TARGET, "MEMORY_FLUSH: starting");
        ...
        let result = async {
            ...
            xai_mad_telemetry::session_ctx::log_event(
                xai_mad_telemetry::memory_telemetry::MemoryFlushStart {
                    session_id: self.session_info.id.to_string(),
            ...
            let request = ConversationRequest {
                items,
                model: Some(model),
                x_mad_conv_id: Some(format!("flush-{}", uuid::Uuid::new_v4())),
                x_mad_req_id: Some(format!("xai-flush-{}", uuid::Uuid::new_v4())),
                x_mad_session_id: Some(session_id.clone()),
                x_mad_agent_id: Some(xai_mad_telemetry::id::agent_id()),
                ..Default::default()
            };
        ...
        tracing::info!(target: xai_mad_telemetry::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
        ...
        xai_mad_telemetry::session_ctx::log_event(
            xai_mad_telemetry::memory_telemetry::MemoryFlushComplete {
                session_id: self.session_info.id.to_string(),
                trigger: trigger.to_owned(),
        ...
        let flush_trigger = match trigger {
            "slash_command" => xai_mad_telemetry::events::MemoryFlushTrigger::SlashCommand,
            "interval" => xai_mad_telemetry::events::MemoryFlushTrigger::Interval,
            "pre_compaction" => xai_mad_telemetry::events::MemoryFlushTrigger::PreCompaction,
            _ => xai_mad_telemetry::events::MemoryFlushTrigger::UserRequested,
        };
        xai_mad_telemetry::session_ctx::log_event(xai_mad_telemetry::events::MemoryFlushed {
    ...
    /// LLM call using the `mad-build` model.
    ///
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    xai_mad_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
                xai_mad_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Responses => {
                let (raw, meta, doom_loop) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = xai_mad_sampler::stream_responses(
                    raw,
                    meta,
                    request_id,
                    idle_timeout,
            ...
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = xai_mad_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                xai_mad_sampler::collect_response(events).await
            }
        };
