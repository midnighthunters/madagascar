...
pub(super) async fn run_session(
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<xai_mad_mcp::servers::McpClientEvent>();
        {
        ...
        tokio::select! {
                ...
                _ = &mut idle_flush_sleep, if session.idle_flush_timeout.is_some()
                    ...
                    if current_len > last_len {
                        tracing::info!(target: xai_mad_telemetry::memory_log::TARGET,
                        ...
                        tracing::debug!(target: xai_mad_telemetry::memory_log::TARGET,
                ...
                maybe_cmd = cmd_rx.recv() => {
                    let Some(cmd) = maybe_cmd else {
                        ...
                        let envelope = session.fire_hook(
                            xai_mad_hooks::event::HookEventName::SessionEnd,
                            None,
                            xai_mad_hooks::event::HookPayload::SessionEnd {
                        ...
                        if let Some(registry) = session.hook_registry.borrow().clone() {
                            ...
                            let results = xai_mad_hooks::dispatcher::dispatch_non_blocking(
                                &registry,
                                xai_mad_hooks::event::HookEventName::SessionEnd,
                                &envelope,
                                &ctx,
                        ...
                        }
                        {
                            ...
                            if let Some(signals) = session.signals_handle().snapshot().await {
                                xai_mad_telemetry::session_ctx::log_event(
                                    xai_mad_telemetry::events::SessionEnded {
                    ...
                    match cmd {
                        ...
                        SessionCommand::OverrideModelName { model_name, extra_headers, context_window } => {
                            ...
                            if let Some(mut cfg) = session.chat_state_handle.get_sampling_config().await {
                                ...
                                // the agent-level default (e.g. "mad-4.5").
                        ...
                        SessionCommand::InjectNotification { prompt_id, prompt_blocks, priority, source } => {
                            ...
                            if is_turn_active && priority == NotificationPriority::Next {
                                ...
                                if let Some(buffer) = &session.tool_context.monitor_event_buffer {
                                    ...
                                    buffer.push_capped(
                                        xai_mad_tools::implementations::mad_build::monitor::types::MonitorEventNotification {
                        ...
                        SessionCommand::RefreshSkillBaseline => {
                            ...
                            tokio::task::spawn_local(async move {
                                ...
                                let new_skills = xai_mad_agent::prompt::skills::list_skills_with_plugins(
                                    Some(&cwd),
                                    &skills_config,
                                    pr.as_deref(),
                        ...
                        SessionCommand::UpdateMcpServers { mcp_servers, respond_to } => {
                            ...
                            // also fires for `<cwd>/.mad/config.toml` edits,
                            ...
                            {
                                let _ = tx.send(
                                    xai_mad_mcp::servers::McpClientEvent::ConfigDiff {
                        ...
                        SessionCommand::DispatchSessionStartHook { source } => {
                            let envelope = session.fire_hook(
                                xai_mad_hooks::event::HookEventName::SessionStart,
                                None,
                                xai_mad_hooks::event::HookPayload::SessionStart {
                                    source,
                                    model_id: None,
                                    agent_type: None,
                                },
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[xai_mad_sampling_types::ConversationItem],
    turn_number: usize,
