...
//! [`xai_mad_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`xai_mad_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`xai_mad_agent::Agent`] owns an [`xai_mad_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`xai_mad_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use xai_mad_agent::config::AgentDefinition;
use xai_mad_agent::error::AgentBuildError;
use xai_mad_agent::prompt::context::PromptAudience;
use xai_mad_agent::prompt::skills::SkillsConfig;
use xai_mad_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
use xai_mad_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use xai_mad_tools::implementations::mad_build::ask_user_question::types::UserQuestionRequest;
...
use xai_mad_tools::implementations::mad_build::task::types::{SubagentEvent, TaskModelValidator};
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<xai_mad_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client:
        Option<xai_mad_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        if let Some(event_tx) = subagent_event_tx.clone() {
            use xai_mad_tools::implementations::mad_build::task::backend::{
        ...
        agent
            .tool_bridge()
            .update_resource(xai_mad_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(xai_mad_tools::types::resources::SchedulerBackgroundLoops(
                *scheduler_background_loops,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(xai_mad_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        ...
        {
            use xai_mad_tools::implementations::mad_build::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            xai_mad_tools::computer::local::LocalTerminalBackend::new_local(
                xai_mad_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
        ...
        subagent_depth: 0,
        subagents_max_depth: xai_mad_tools::implementations::mad_build::task::MAX_SUBAGENT_DEPTH,
        ...
        system_prompt_label: xai_mad_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(xai_mad_tools::types::tool::ToolKind::Task)
            .expect("MadagascarBuild Task tool should be present");
        toolset
            .tool_definitions()
            .into_iter()
            .find(|definition| definition.function.name == task_name)
            .and_then(|definition| definition.function.description)
            .expect("MadagascarBuild Task description should be present")
    }
    ...
    async fn rebuild_projects_fresh_public_model_keys_into_task_description() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let first = spec
                    .build_agent(AgentDefinition::default_mad_build())
                    .await
