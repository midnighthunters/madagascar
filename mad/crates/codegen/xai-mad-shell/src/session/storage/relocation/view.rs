...
impl RelocationView {
    pub(crate) fn load(mad_home: &Path) -> Result<Self> {
        Self::load_for_sessions_root(&mad_home.join("sessions"))
    }
    pub(crate) fn journal_ids(mad_home: &Path) -> Result<Vec<String>> {
        Ok(load_journals(mad_home)?.into_keys().collect())
    }
    pub(crate) fn load_for_sessions_root(sessions_root: &Path) -> Result<Self> {
        let mad_home = sessions_root
            .parent()
            .ok_or_else(|| RelocationError::Inconsistent("sessions root has no parent".into()))?;
        let journals = load_journals(mad_home)?;
        let (all_candidates, persisted_candidates) = load_candidates(sessions_root)?;
        Ok(Self {
            mad_home: mad_home.into(),
    ...
    fn select(
        ...
        let selected = if let Some(relocation) = self.journals.get(session_id) {
            let expected =
                journal::session_dir_at(&self.mad_home, authoritative_cwd(relocation), session_id);
            let path = self
                .all_candidates
                .get(session_id)
            ...
            RelocationStorage::new(self.mad_home.clone())
...
fn load_journals(mad_home: &Path) -> Result<HashMap<String, RelocationJournal>> {
    let dir = journal::relocation_dir(mad_home);
    ...
    for entry in entries {
        ...
        journals.insert(session_id.to_owned(), journal::read(mad_home, session_id)?);
    }
    Ok(journals)
}
