...
impl RelocationStorage {
    pub(crate) fn new(mad_home: PathBuf) -> Self {
        Self {
            mad_home,
            #[cfg(test)]
            fault: None,
    ...
    #[cfg(test)]
    fn with_fault(mad_home: PathBuf, fault: TestFault) -> Self {
        Self {
            mad_home,
            fault: Some(fault),
        }
    ...
    pub(crate) fn sync_journal_namespace(&self) -> Result<()> {
        ...
        journal::sync_namespace(&self.mad_home)
    }
    ...
    pub(crate) fn stage_and_publish(
        ...
        journal.validate(&self.mad_home)?;
        let source =
            journal::session_dir_at(&self.mad_home, &journal.source_cwd, &journal.session_id);
    ...
    pub(crate) fn recover_all(&self) -> Result<()> {
        for session_id in RelocationView::journal_ids(&self.mad_home)? {
    ...
    pub(crate) fn finalize_terminal(
        ...
        proof.journal.validate(&self.mad_home)?;
        ...
        let path = journal::journal_path(&self.mad_home, &lease.session_id);
    ...
    fn finish_rollback(&self, journal: &mut RelocationJournal) -> Result<TerminalRelocation> {
        ...
        let result = (|| {
            ...
            self.remove_transaction_directory(&self.staging_dir(journal))?;
            let target =
                journal::session_dir_at(&self.mad_home, &journal.target_cwd, &journal.session_id);
            if target.exists() {
                self.validate_target(journal)?;
            }
    ...
    fn validate_request(&self, lease: &RelocationLease, request: &RelocationRequest) -> Result<()> {
        ...
        if journal::session_dir_at(&self.mad_home, &request.source_cwd, &request.session_id)
            == journal::session_dir_at(&self.mad_home, &request.target_cwd, &request.session_id)
        {
            return Err(RelocationError::Inconsistent(
    ...
    pub(super) fn validate_authoritative_dir(
        ...
        if path
            != journal::session_dir_at(&self.mad_home, expected_cwd, &journal.session_id).as_path()
        {
            return Err(RelocationError::Inconsistent(
    ...
    fn target_parent(&self, cwd: &str) -> PathBuf {
        self.mad_home
            .join("sessions")
            .join(xai_mad_config::encode_cwd_dirname(cwd))
    }

    fn ensure_target_parent(&self, cwd: &str) -> Result<PathBuf> {
        let sessions = self.mad_home.join("sessions");
        fs::create_dir_durable(&sessions)?;
        let encoded = xai_mad_config::encode_cwd_dirname(cwd);
    ...
    fn staging_dir(&self, journal: &RelocationJournal) -> PathBuf {
        self.mad_home
            .join("sessions")
            .join(xai_mad_config::encode_cwd_dirname(&journal.target_cwd))
