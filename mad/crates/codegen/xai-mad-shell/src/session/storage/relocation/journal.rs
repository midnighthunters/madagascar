...
impl RelocationJournal {
    ...
    pub(super) fn validate(&self, mad_home: &Path) -> Result<()> {
        ...
        if session_dir_at(mad_home, &self.source_cwd, &self.session_id)
            == session_dir_at(mad_home, &self.target_cwd, &self.session_id)
        {
            return Err(RelocationError::Inconsistent(
...
pub(super) fn acquire(mad_home: &Path, session_id: &str) -> Result<RelocationLease> {
    validate_component("session id", session_id)?;
    let dir = relocation_dir(mad_home);
...
pub(super) fn read(mad_home: &Path, session_id: &str) -> Result<RelocationJournal> {
    validate_component("session id", session_id)?;
    let path = journal_path(mad_home, session_id);
...
pub(super) fn write(
    ...
    let path = journal_path(mad_home, &journal.session_id);
...
pub(super) fn relocation_dir(mad_home: &Path) -> PathBuf {
    mad_home.join(RELOCATION_DIR)
}

pub(super) fn journal_path(mad_home: &Path, session_id: &str) -> PathBuf {
    relocation_dir(mad_home).join(format!("{session_id}.json"))
}

pub(super) fn session_dir_at(mad_home: &Path, cwd: &str, session_id: &str) -> PathBuf {
    mad_home
        .join("sessions")
...
pub(super) fn sync_namespace(mad_home: &Path) -> Result<()> {
    let dir = relocation_dir(mad_home);
