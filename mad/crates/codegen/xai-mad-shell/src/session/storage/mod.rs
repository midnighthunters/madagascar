...
//   - production, current mad home:   `load_updates_for_replay`
//   - production, streaming (bounded): `stream_replay_updates_at`
//   - tests, explicit mad home:       `load_updates_for_replay_at` (typed reference)
...
/// mad home. Typed, materialize-all replay reader: collects every update into
...
pub fn load_updates_for_replay_at(
    ...
    let Some(updates_path) = resolve_replay_updates_path(session_id, mad_home)? else {
        return Ok(None);
    };
    Ok(Some(collect_replay_updates(&updates_path)?))
}
...
/// Resolve `updates.jsonl` for `session_id` under `mad_home`, or `None` when
...
/// Invoke `f` once per replay-ready ACP update for a session under `mad_home`,
...
mod tests {
    ...
    fn stream_replay_updates_at_missing_session_is_empty() {
        ...
        std::fs::create_dir_all(mad_home.path().join("sessions")).unwrap();

        let mut count = 0usize;
        let emission =
            stream_replay_updates_at("does-not-exist", mad_home.path(), |_| count += 1).unwrap();
    ...
    fn stream_replay_updates_at_surfaces_read_errors() {
        let mad_home = tempfile::tempdir().unwrap();
        let session_dir = mad_home.path().join("sessions").join("cwd").join("sess");
        std::fs::create_dir_all(&session_dir).unwrap();
        std::fs::write(session_dir.join(SUMMARY_FILE), "{}").unwrap();
        std::fs::create_dir(session_dir.join(UPDATES_FILE)).unwrap();

        let result = stream_replay_updates_at("sess", mad_home.path(), |_| {});
        assert!(
            result.is_err(),
