...
pub(crate) fn bounded_goal_transcript(items: &[ConversationItem]) -> String {
    ...
    for item in items.iter().rev() {
        ...
        let capped = xai_mad_tools::util::truncate_str(trimmed, ITEM_MAX_BYTES);
...
pub(crate) fn build_goal_evaluator_request(
    ...
    ConversationRequest {
        ...
        x_mad_conv_id: Some(session_id.to_owned()),
        x_mad_req_id: Some(format!("xai-goal-eval-{}", uuid::Uuid::new_v4())),
        x_mad_session_id: Some(session_id.to_owned()),
