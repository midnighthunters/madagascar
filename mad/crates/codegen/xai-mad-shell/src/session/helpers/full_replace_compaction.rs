//! mad-build's L5 wiring onto the shared full-replace engine
//! (`xai_mad_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](xai_mad_compaction::sample_full_replace_summary);
//! this module adapts mad-build's transport and telemetry to its two seams:
//!
...
//! [`FullReplaceError`](xai_mad_compaction::FullReplaceError).
...
use xai_mad_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use xai_mad_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
...
/// [`CompactionSampler`] for mad-build's full-replace pass.
///
...
/// else the structured mad-build prompt), so the shared `CompactionPrompt`
/// the engine passes is ignored — the engine builds the mad-build prompt,
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // message (compat short vs structured mad-build), ignoring the shared
...
/// Map mad-build's [`CompactFailure`] onto the shared engine's
...
/// [`FullReplaceObserver`] that reproduces mad-build's per-attempt telemetry:
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    xai_mad_telemetry::session_ctx::log_event(CompactionRetryDegraded {
