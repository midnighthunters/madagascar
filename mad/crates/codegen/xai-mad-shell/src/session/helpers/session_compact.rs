...
///
/// All other agents (mad-build, etc.) continue to use the detailed
...
pub(crate) use xai_mad_sampling_types::is_context_length_error;
...
pub(crate) async fn generate_session_compact(
    ...
    let output = match sampling_config.api_backend {
        ApiBackend::ChatCompletions => {
            ...
            message.x_mad_conv_id = Some(sid.clone());
            message.x_mad_req_id = Some(format!("xai-compact-{}", uuid::Uuid::new_v4()));
            message.x_mad_session_id = Some(sid);
            message.x_mad_agent_id = Some(xai_mad_telemetry::id::agent_id());
            tracing::info!(
                    ...
                    Ok(chunk) => {
                        if let Some(choice) = chunk.choices.first() {
                            ...
                            if let Some(fr) = choice.finish_reason {
                                let sr = xai_mad_sampling_types::StopReason::from(fr);
                                truncated =
                                    matches!(sr, xai_mad_sampling_types::StopReason::Length);
        ...
        ApiBackend::Responses => {
            let request = ConversationRequest {
                ...
                x_mad_conv_id: Some(session_id.to_string()),
                x_mad_req_id: Some(format!("xai-compact-{}", uuid::Uuid::new_v4())),
                x_mad_session_id: Some(session_id.to_string()),
            ...
            loop {
                ...
                match chunk_result {
                    Ok(event) => {
                        if !matches!(
                            &event,
                            xai_mad_sampling_types::messages::MessageStreamEvent::Ping
                        ) {
                            last_progress_at = std::time::Instant::now();
                        }
                        match event {
                            xai_mad_sampling_types::messages::MessageStreamEvent::ContentBlockDelta {
                                delta: xai_mad_sampling_types::messages::StreamDelta::TextDelta {
                                    text,
                                },
                                ..
                            } => {
                            ...
                            xai_mad_sampling_types::messages::MessageStreamEvent::MessageDelta {
                                delta,
                                ..
                            } => {
                                if let Some(sr) = delta.stop_reason {
                                    truncated = matches!(
                                    sr,
                                    xai_mad_sampling_types::messages::StopReason::MaxTokens
                                        | xai_mad_sampling_types::messages::StopReason::ModelContextWindowExceeded
                                );
                                    stop_reason = Some(
                                        match sr {
                                            xai_mad_sampling_types::messages::StopReason::EndTurn => {
                                                "end_turn".to_string()
                                            }
                                            xai_mad_sampling_types::messages::StopReason::MaxTokens => {
                                                "max_tokens".to_string()
                                            }
                                            xai_mad_sampling_types::messages::StopReason::ToolUse => {
                                                "tool_use".to_string()
                                            }
                                            xai_mad_sampling_types::messages::StopReason::StopSequence => {
                                                "stop_sequence".to_string()
                                            }
                                            xai_mad_sampling_types::messages::StopReason::Refusal => {
                                                "refusal".to_string()
                                            }
                                            xai_mad_sampling_types::messages::StopReason::PauseTurn => {
                                                "pause_turn".to_string()
                                            }
                                            xai_mad_sampling_types::messages::StopReason::ModelContextWindowExceeded => {
                                                "model_context_window_exceeded".to_string()
                                            }
                                            xai_mad_sampling_types::messages::StopReason::Unknown(
                                                s,
                                            ) => s,
                                        },
                                    );
...
mod compacted_history_shape_tests {
    ...
    /// Regression guard: mad-build must DROP the working
    /// tail post-compaction. A prior change routed mad-build to keep `recent_messages`,
    /// which survive only as `Tool call omitted...` stubs (dead tokens). Mirrors
    /// `summary_before_recent_compaction_with_no_user_query_yields_three_messages` for mad-build
    /// (`summary_before_recent = false`).
    #[tokio::test]
    async fn mad_build_compaction_drops_working_tail_regression_206460() {
        let conversation = vec![
            ConversationItem::system("You are a helpful assistant."),
            ConversationItem::user(
                "<user_info>\nOS: macos\n</user_info>\n\n<user_query>\nread auth.rs\n</user_query>",
        ...
        assert!(
            ...
            "mad-build must drop recent_messages post-compaction",
        );
        let compacted = build_compacted_history(
            "You are a helpful assistant.",
            "<user_info>\nOS: macos\n</user_info>",
        ...
        assert!(
            !compacted
                ...
                .any(|i| matches!(i, ConversationItem::ToolResult(_))
                    || i.text_content() == "Tool call omitted..."),
            "no tail (ToolResult or stub) may leak into the mad-build compacted history",
        );
    }
