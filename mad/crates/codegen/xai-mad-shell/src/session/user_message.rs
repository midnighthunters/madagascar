    ...
    match result {
        Ok(Ok(status)) => {
            ...
            let status = if vcs_kind.is_jj() {
                ...
                xai_mad_agent::prompt::user_message::normalize_git_status(&status)
            };
...
mod tests {
    use super::*;
    use xai_mad_workspace::file_system::FsError;
    ...
    async fn construct_user_message_returns_without_git_status_on_bad_dir() {
        let dir = std::path::Path::new("/tmp/nonexistent-mad-test-dir");
