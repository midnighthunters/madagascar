...
pub(crate) fn new_submission(
    ...
    s.shell_version = Some(xai_mad_version::VERSION.to_string());
    s
}
...
pub(crate) async fn submit_feedback_workflow(
    ...
    if let Some(user_meta) = crate::agent::mvp_agent::parse_json_object_env("MAD_USER_METADATA") {
    ...
    if telemetry_enabled {
        ...
        xai_mad_telemetry::session_ctx::log_event(xai_mad_telemetry::events::UserFeedback {
...
pub struct FeedbackManagerConfig {
    ...
    /// ratings). Gated by `MAD_FEEDBACK_ENABLED`.
    pub feedback_enabled: bool,
    /// Whether session analytics (signal sync, turn deltas) are enabled.
    /// Gated by `MAD_TELEMETRY_ENABLED`. These are analytics data that
...
mod author_identity_tests {
    ...
    fn text_submission() -> FeedbackSubmission {
        ...
        s.model_id = Some("mad-4".to_string());
        s
    }
    ...
    async fn env_var_identity_reaches_the_wire_end_to_end() {
        let _email =
            xai_mad_test_support::env::EnvGuard::set("MAD_TEST_WORK_EMAIL", "ada@corp.example");
        let _name =
            xai_mad_test_support::env::EnvGuard::set("MAD_TEST_WORK_NAME", "Ada Lovelace");

        // The loader expands `$VAR` at load, exactly as a trusted config tier ships it.
        let mut value = toml::from_str::<toml::Value>(
            r#"
[feedback.user]
name = ["$MAD_TEST_WORK_NAME"]
email = ["$MAD_TEST_WORK_EMAIL"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(persisted.model_id.as_deref(), Some("mad-4"));
    }

    /// `MAD_USER_METADATA` is merged into the submission and travels with it:
    ...
    async fn workflow_merges_user_metadata_into_submission() {
        let _guard = xai_mad_test_support::env::EnvGuard::set(
            "MAD_USER_METADATA",
