...
pub struct ContextInfo {
    ...
    /// always matches the actual trigger (e.g. 65 for mad-build in remote settings).
...
pub struct SessionInfoData {
    /// Agent definition name for this session (e.g. `mad-build`).
...
pub fn is_coding_model_slug(model: &str) -> bool {
    matches!(model, "mad-build" | "mad-4.5")
}
...
mod tests {
    ...
    fn should_show_model_fingerprint_truth_table() {
        ...
        assert!(should_show_model_fingerprint(false, "mad-build"));
        assert!(should_show_model_fingerprint(false, "mad-4.5"));
    ...
    fn desktop_client_type_deserializes_and_round_trips() {
        ...
        let submission = input.to_submission(Some("mad-3".into()), None, None, Some(5));
        assert_eq!(
