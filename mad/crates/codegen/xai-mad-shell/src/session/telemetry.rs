use xai_mad_telemetry::events::SessionHarness;
use xai_mad_workspace::permission::Decision;
...
pub(crate) fn skill_source_label(skill_path: &str, cwd: &str) -> &'static str {
    ...
    let user = crate::util::mad_home::mad_home()
        .parent()
...
pub(crate) fn format_hook_name(spec: &xai_mad_hooks::config::HookSpec) -> String {
...
fn format_hook_source(spec: &xai_mad_hooks::config::HookSpec) -> &'static str {
    use xai_mad_hooks::config::HookOrigin as O;
    match xai_mad_hooks::config::hook_origin(spec) {
...
impl HookRegInfo {
    pub(crate) fn from_spec(spec: &xai_mad_hooks::config::HookSpec) -> Self {
        Self {
...
pub(crate) struct SessionHarnessMetrics {
    ...
    pub permission_mode: xai_mad_telemetry::enums::PermissionMode,
    ...
    pub compat: xai_mad_tools::types::compat::CompatConfig,
...
impl SessionHarnessMetrics {
    pub async fn into_event(self, hooks: Vec<HookRegInfo>) -> SessionHarness {
        ...
        let agents_md_dir_names = xai_mad_agent::prompt::agents_md::read_agents_config_with_paths(
            &self.cwd,
            self.compat,
        )
        .await
        ...
        SessionHarness {
            ...
            is_git_repo: xai_mad_telemetry::context::collect_git_context(&self.cwd).is_git_repo,
