...
//!       [--api-key <key> | $XAI_API_KEY | <mad-home>/auth.json] \
//!       [--min-confidence 0.7] \
...
use clap::Parser;
use xai_mad_shell::trace_classifier::{RunArgs, run, validate_min_confidence};
...
struct Cli {
    ...
    /// has access to.
    #[arg(long, default_value = "mad-4.5")]
    model: String,
    ...
    /// `$XAI_API_KEY`, then `<mad-home>/auth.json` (`xai::api_key`
    ...
    /// shell uses (`$MAD_HOME` or `~/.mad`). Exposed primarily for
    /// tests / sandboxed invocations.
    #[arg(long)]
    mad_home: Option<PathBuf>,
}
...
mod tests {
    ...
    #[test]
    fn cli_mad_home_override_parses() {
        ...
        .expect("parse");
        assert_eq!(cli.mad_home, Some(PathBuf::from("/tmp/scratch-mad")));
    }
    ...
    fn cli_defaults_match_documented_values() {
        ...
        assert_eq!(by_id("model"), vec!["mad-4.5"]);
