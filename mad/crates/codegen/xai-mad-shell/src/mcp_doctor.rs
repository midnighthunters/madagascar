//! `mad mcp doctor` -- runtime health check for MCP servers.
...
fn discover_servers(cwd: &Path) -> (Vec<ConfigSourceStatus>, Vec<DiscoveredServer>) {
    ...
    let user_config = mad_home.join("config.toml");
    if user_config.is_file() {
        sources.push(ConfigSourceStatus {
            path: "~/.mad/config.toml".to_string(),
...
// ── Managed (mad.com) server discovery ─────────────────────────

const MANAGED_SOURCE_LABEL: &str = "mad.com";
...
/// Discover managed `mad_com_*` servers if the user has xAI auth on disk.
async fn try_discover_managed_servers() -> (ConfigSourceStatus, Vec<DiscoveredServer>) {
    let mad_home = xai_mad_tools::util::mad_home::mad_home();
    let mad_com_config = MadComConfig::default();
    let auth_manager = Arc::new(crate::auth::AuthManager::new(&mad_home, mad_com_config));
    ...
    let token = match auth_manager.get_valid_token().await {
        Ok(key) => key,
        Err(_) => return managed_skipped("auth expired — run `mad login`"),
    };
...
async fn check_tools_list(service: &mcp_servers::McpService) -> Check {
    use xai_mad_mcp::rmcp::model::PaginatedRequestParams;
    match service
...
pub async fn run_doctor(cwd: &Path, name_filter: Option<&str>) -> DoctorReport {
    ...
    // Folder-trust gate: `mad mcp doctor` actually STARTS each server
    ...
    //
    // `remote = None` is intentional: standalone `mad mcp doctor` has no loaded
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `mad mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `mad mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );
