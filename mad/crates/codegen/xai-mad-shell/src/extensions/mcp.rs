...
/// `xai_mad_mcp::wire`), these methods are private to the agent↔client channel and
...
pub async fn build_mcp_status(
    ...
    tool_bridge: &Arc<xai_mad_tools::bridge::ToolBridge>,
...
//
// Implements the `McpResourceProvider` trait from xai-mad-tools so that
// `ListMcpResources` / `FetchMcpResource` tools can access MCP
// servers without depending on `xai-mad-mcp` directly.
...
impl xai_mad_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<xai_mad_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(xai_mad_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<xai_mad_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(xai_mad_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(xai_mad_tools::types::resources::McpResourceContent::Text(
                    text,
                )),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                ...
                description: None,
                mime_type,
                content: Some(xai_mad_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
...
mod tests {
    ...
    fn test_mcp_list_response_serialization() {
            ...
            servers: vec![
                McpServerEntry {
                    ...
                    config: McpServerConfig::Http {
                        ...
                        scope_name: Some("Madagascar CLI".to_string()),
                    },
                    source_label: None,
                    setup: None,
                    setup_values: None,
        ...
        assert_eq!(json["servers"][0]["scopeName"], "Madagascar CLI");
    ...
    fn disabled_managed_http_rows_keep_non_gateway_placeholder_config() {
        let entry = disabled_server_placeholder_entry("mad_com_slack");
    ...
    fn disabled_placeholders_match_handle_list_catalog_relationships() {
        ...
        let mut servers = build_mcp_catalog_with_gateway_tools(&[], &[], None, &Default::default());
        append_disabled_like_handle_list(&mut servers, &["mad_com_slack"], false);
        assert_eq!(servers.len(), 1);
        assert_eq!(servers[0].name, "mad_com_slack");
