...
) -> Result<crate::auth::MadAuth, acp::Error> {
    super::auth_gate::require_xai_auth(
        auth_manager,
        "Authentication required to share session",
        "Share session is disabled. Run `mad login` to authenticate.",
    )
}
...
mod tests {
    ...
    #[test]
    fn share_rejects_non_xai_auth_with_actionable_mad_login_message() {
        let dir = tempdir().expect("tempdir");
        let mgr = Arc::new(crate::auth::AuthManager::new(
            dir.path(),
            MadComConfig::default(),
