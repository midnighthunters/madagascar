...
/// detailed migration guidance (`mad logout` / `mad login`) in the
...
pub struct CompactionSegmentFile {
    pub items: Vec<xai_mad_sampling_types::ConversationItem>,
...
pub struct CompactionRequestFile {
    ...
    /// Note: this is a per-artifact ID, not the model API's `x_mad_req_id`
    ...
    /// or `"detailed"` (10-section structured prompt for mad-build and similar agents).
...
pub struct RecapRequestFile {
    ...
    /// API's `x_mad_req_id` (also recorded below for proxy correlation).
    pub request_id: String,
    ...
    pub x_mad_req_id: String,
    /// Sampling conversation id (`recap-{uuid}`).
    pub x_mad_conv_id: String,
...
mod tests {
    ...
    fn recap_request_file_roundtrips() {
        let artifact = RecapRequestFile {
            ...
            x_mad_req_id: "xai-recap-abc".into(),
            x_mad_conv_id: "recap-abc".into(),
        ...
        assert_eq!(parsed.x_mad_req_id, "xai-recap-abc");
        assert_eq!(
    ...
    fn memory_flush_completed_with_path_roundtrips() {
        let update = SessionUpdate::MemoryFlushCompleted {
            result: "written".into(),
            path: Some("/home/user/.mad/memory/ws/sessions/log.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_dream_completed_roundtrips() {
        let update = SessionUpdate::MemoryDreamCompleted {
            ...
            path: Some("/home/user/.mad/memory/ws/MEMORY.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_session_saved_roundtrips() {
        let update = SessionUpdate::MemorySessionSaved {
            path: "/home/user/.mad/memory/ws/sessions/2026-01-15-fix-auth-abc12345.md".into(),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_files_variant_roundtrips_through_json() {
        let update = SessionUpdate::MemoryFiles {
            files: vec![
                MemoryFileInfo {
                    path: "/home/user/.mad/memory/MEMORY.md".into(),
                    source: "global".into(),
                    size_bytes: 1024,
                    modified_epoch_secs: Some(1_700_000_000),
                },
                MemoryFileInfo {
                    path: "/project/.mad/memory/MEMORY.md".into(),
    ...
    fn make_goal_updated_full() -> SessionUpdate {
        SessionUpdate::GoalUpdated {
            ...
            live_tokens_by_model: vec![("mad-4".into(), 6_000), ("mad-3".into(), 4_000)],
    ...
    fn goal_updated_serializes_snake_case_tag() {
        ...
        assert_eq!(json["live_tokens_by_model"][0][0], "mad-4");
    ...
    fn model_changed_envelope_carries_session_id_at_top_level() {
        ...
        assert_eq!(json["update"]["model_id"], "mad-4");
    }
