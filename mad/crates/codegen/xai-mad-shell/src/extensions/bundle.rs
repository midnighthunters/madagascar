...
mod tests {
    ...
    fn test_auth() -> crate::auth::MadAuth {
        crate::auth::MadAuth {
            key: "token".to_string(),
    ...
    fn status_only_reports_bundled_cache_not_higher_priority_sources() {
        ...
        std::fs::create_dir_all(project_root.join(".mad/personas")).unwrap();
        std::fs::create_dir_all(project_root.join(".mad/roles")).unwrap();
        std::fs::write(
            project_root.join(".mad/personas/researcher.toml"),
            "instructions = \"project persona\"\n",
        )
        .unwrap();
        std::fs::write(
            project_root.join(".mad/roles/reviewer.toml"),
