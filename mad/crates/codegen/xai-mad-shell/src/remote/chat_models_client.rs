//! mad.com chat-product model catalog (`POST /rest/modes`) — the models
//! mad-web's chat picker shows, distinct from the CLI `/v1/models` build
...
const MAD_WEB_URL: &str = "https://mad.com";
...
impl ChatModelsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("MAD_MODES_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("MAD_CONVERSATIONS_BASE_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .or_else(|| {
                std::env::var("MAD_CODE_WEB_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .unwrap_or_else(|| MAD_WEB_URL.to_string());
        Self {
    ...
    /// Gated only on a valid mad.com bearer — deliberately NOT `is_xai_auth()`
    ...
    pub async fn list_modes(&self, locale: &str) -> Result<ListModesResponse, ChatModelsError> {
        ...
        let mut builder = self
            ...
            .header(
                "X-XAI-Token-Auth",
                self.auth.mad_com_config().token_header.clone(),
            )
