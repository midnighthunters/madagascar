...
use crate::auth::{MadAuth, MadComConfig};
...
const MAD_CODE_BACKEND_URL: &str = "https://code.mad.com";
const DEFAULT_TIMEOUT: Duration = Duration::from_secs(30);
const MAD_CODE_WEB_URL: &str = "https://mad.com";
/// Build a share URL from a permission ID
pub fn share_url(permission_id: &str) -> String {
    let web_url =
        std::env::var("MAD_CODE_WEB_URL").unwrap_or_else(|_| MAD_CODE_WEB_URL.to_string());
...
fn add_cli_chat_proxy_headers_blocking(
    ...
    let mut builder = builder
        ...
        .header("X-XAI-Token-Auth", MadComConfig::default().token_header)
        .header("x-userid", &auth.user_id)
        .header("x-mad-client-version", xai_mad_version::VERSION);
...
async fn add_bundle_fetch_headers(
    ...
    let mut credentials = crate::util::mad_auth_credentials::MadAuthCredentials::new(
...
async fn fetch_bundle_inner(
    ...
    let client: reqwest_middleware::ClientWithMiddleware = if let Some(am) = auth_manager {
        let provider: std::sync::Arc<dyn xai_mad_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
...
impl BackendClient {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            base_url: std::env::var("MAD_CODE_BACKEND_URL")
                .unwrap_or_else(|_| MAD_CODE_BACKEND_URL.to_string()),
            auth_manager: None,
        }
    }
    ...
    /// instead of requiring the caller to pass `&MadAuth`.
    pub fn with_auth_manager(mut self, manager: std::sync::Arc<crate::auth::AuthManager>) -> Self {
        let credentials: std::sync::Arc<dyn xai_mad_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
                crate::auth::credential_provider::ShellAuthCredentialProvider::new(
                    manager.clone(),
                    None,
    ...
    async fn resolve_auth(&self) -> Result<MadAuth, BackendError> {
        let manager = self
            .auth_manager
            .as_ref()
    ...
    /// Must include X-XAI-Token-Auth so nginx auth subrequest routes to authenticate_xai_mad_cli_token.
    /// See: crates/codegen/xai-mad-shell/src/agent/app.rs:run_headless
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        headers.insert(
            "X-XAI-Token-Auth",
            required(&MadComConfig::default().token_header, "X-XAI-Token-Auth")?,
        );
        ...
        if let Ok(v) = HeaderValue::from_str(&crate::http::process_client_identifier()) {
            headers.insert("x-mad-client-identifier", v);
        }
        headers.insert(
            crate::http::CLIENT_MODE_HEADER,
        ...
        headers.insert(
            "x-mad-client-version",
            HeaderValue::from_static(xai_mad_version::VERSION),
        );
        Ok(headers)
    }
...
/// Fetch `mad_build_login_device_flow` from cli-chat-proxy `GET /v1/login-config`.
///
/// Unauthenticated (pre-login); `x-mad-agent-id` is the per-install bucketing key.
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(xai_mad_telemetry::id::agent_id)
        .await
        .ok()?;
    ...
    let response = client
        ...
        .header("x-mad-agent-id", agent_id)
        .header("x-mad-client-version", xai_mad_version::VERSION)
        .header(
            "x-mad-client-identifier",
            crate::http::process_client_identifier(),
...
pub(crate) fn fetch_models_blocking(
    ...
    match source.auth {
        ...
        EndpointAuth::Session => {
            ...
            request = request
                .header("Authorization", format!("Bearer {}", &auth.key))
                .header("X-XAI-Token-Auth", "xai-mad-cli")
                .header("x-userid", &auth.user_id)
                .header("x-mad-client-version", xai_mad_version::VERSION)
                .header(
                    crate::http::CLIENT_MODE_HEADER,
...
pub fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| xai_mad_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(xai_mad_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
fn parse_compaction_at_tokens(
    v: &serde_json::Value,
) -> Option<xai_mad_sampling_types::CompactionAtTokens> {
...
fn parse_compactions_remaining(
    v: &serde_json::Value,
) -> Option<xai_mad_sampling_types::CompactionsRemaining> {
...
mod tests {
                ...
                get(
                    |State(state): State<LoginConfigServerState>, headers: HeaderMap| async move {
                        state.seen.lock().unwrap().push(LoginConfigHeaders {
                            ...
                            agent_id: header_str(&headers, "x-mad-agent-id"),
                            client_identifier: header_str(&headers, "x-mad-client-identifier"),
                            client_version: header_str(&headers, "x-mad-client-version"),
                        });
    ...
    async fn fetch_login_device_flow_sends_only_unauthenticated_headers() {
        ...
        assert!(
            ...
            "must send x-mad-agent-id (the bucketing key)"
        );
        assert!(
            h.client_identifier.is_some(),
            "must send x-mad-client-identifier"
        );
        assert!(
            h.client_version.is_some(),
            "must send x-mad-client-version"
        );
    ...
    fn test_auth_manager() -> Arc<crate::auth::AuthManager> {
        ...
        let mgr = crate::auth::AuthManager::new(dir.path(), crate::auth::MadComConfig::default());
    ...
    async fn fetch_subagent_bundle_success() {
        ...
        assert_eq!(headers.token_auth.as_deref(), Some("xai-mad-cli"));
    ...
    fn parse_openai_format_uses_id_field() {
        ...
        assert_eq!(result.model, "mad-3");
        assert_eq!(result.base_url, "https://api.x.ai/v1");
        assert_eq!(result.name.as_deref(), Some("mad-3"));
    }
    #[test]
    ...
    fn inference_url_defaults_to_proxy() {
        let ep = endpoints("https://proxy.mad.com/v1", None, None);
        assert_eq!(ep.resolve_inference_base_url(), "https://proxy.mad.com/v1");
    }
    #[test]
    fn inference_url_uses_models_base_url() {
        let ep = endpoints(
            "https://proxy.mad.com/v1",
    ...
    fn models_fetch_endpoint_matches_auth_mode() {
        ...
        assert_eq!(session.url, "https://cli-chat-proxy.mad.com/v1/models");
        ...
        assert_eq!(deployment.url, "https://cli-chat-proxy.mad.com/v1/models");
    ...
    /// REGRESSION: `mad setup` must send the deployment key to
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::set_var("MAD_DEPLOYMENT_KEY", "xai-token-ENTERPRISE") };
        ...
        assert_eq!(url, "https://cli-chat-proxy.mad.com/v1/deployment/config");
        assert!(
        ...
        unsafe { std::env::remove_var("MAD_DEPLOYMENT_KEY") };
    }
    #[derive(Clone)]
