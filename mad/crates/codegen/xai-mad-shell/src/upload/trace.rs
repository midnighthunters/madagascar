...
use xai_mad_workspace::permission::PermissionEvent;
...
fn record_upload_failure(ctx: &PromptTraceContext, f: UploadFailure<'_>) {
    ...
    if level == UploadFailureLogLevel::Warn {
        xai_mad_telemetry::unified_log::warn(&msg, sid, log_ctx);
    } else {
        xai_mad_telemetry::unified_log::error(&msg, sid, log_ctx);
    }
}
...
pub(crate) fn local_sandbox_telemetry() -> Option<LocalSandboxTelemetry> {
    let profile = xai_mad_sandbox::configured_profile_name()?;
    Some(LocalSandboxTelemetry {
        profile: profile.to_owned(),
        applied: xai_mad_sandbox::is_active(),
    })
}
...
pub(crate) async fn upload_plugin_state(
    ctx: &PromptTraceContext,
    registry: Option<&xai_mad_agent::plugins::PluginRegistry>,
) {
...
pub(crate) async fn upload_unified_log(ctx: &PromptTraceContext, wait: UploadWait) {
    ...
    let log_bytes = match tokio::task::spawn_blocking(move || {
        xai_mad_telemetry::unified_log::snapshot_session_log(&session_id)
    })
    .await
    {
        Ok(Some(bytes)) => bytes,
    ...
    .await;
    let full_log_bytes =
        tokio::task::spawn_blocking(xai_mad_telemetry::unified_log::snapshot_log).await;
    let user_id = ctx
        .auth_manager
        .current_or_expired()
        .map(|a| a.user_id)
...
pub(crate) fn build_chat_history_session_state(
    messages: &[xai_mad_sampling_types::conversation::ConversationItem],
...
impl TraceExportSource for DynamicResolver {
    ...
    fn proxy_credentials(&self) -> Option<Arc<dyn xai_mad_auth::AuthCredentialProvider>> {
...
pub(crate) fn spawn_startup_spill_reconcile(
    ...
    tokio::spawn(async move {
        match queue {
            Some(queue) => {
                let report =
                    xai_mad_workspace::recovery::run_startup_recovery(&mad_home, &queue).await;
            ...
            None => {
                let purged = tokio::task::spawn_blocking(move || {
                    xai_mad_workspace::recovery::purge_spilled_items(&mad_home)
                })
                .await;
                match purged {
                    Ok(removed) => {
...
pub(crate) fn spawn_purge_stale_upload_scratch() {
    {
        ...
        let dir = crate::util::mad_home::mad_home()
            .join("upload_queue")
            .join("scratch");
...
pub(crate) fn spawn_upload_queue(
    ...
    let queue = UploadQueue::spawn(mad_home, resolver, UploadRetryPolicy::default());
...
mod tests {
    ...
    fn dynamic_resolver_refreshes_proxy_token() {
        ...
        let mad_com_config = MadComConfig::default();
        let scope = mad_com_config.auth_scope();
        let initial_auth = MadAuth {
    ...
    async fn resolve_async_falls_back_when_no_refresher() {
        ...
        let auth_json = serde_json::to_string_pretty(&store).unwrap();
        std::fs::write(dir.path().join("auth.json"), &auth_json).unwrap();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(dir.path(), mad_com_config));
        let resolver = DynamicResolver {
            auth_manager,
            base_config: TraceExportConfig {
                bucket_url: None,
    ...
    async fn resolve_async_drives_refresh_chain_when_token_expired() {
        ...
        impl crate::auth::refresh::TokenRefresher for FreshRefresher {
            async fn refresh(
                ...
                crate::auth::refresh::RefreshOutcome::Success(Box::new(crate::auth::MadAuth {
    ...
    async fn dynamic_resolver_auth_recovery_wakes_on_already_rotated_token() {
        ...
        auth_manager.hot_swap(crate::auth::MadAuth {
