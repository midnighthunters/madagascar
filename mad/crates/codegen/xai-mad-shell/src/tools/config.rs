...
use xai_mad_sampler::SamplerConfig;
use xai_mad_tools::implementations::mad_build;
use xai_mad_tools::registry::types::ToolConfig;

/// Production mad-build foreground command-timeout ceiling (seconds). The
...
impl BashToolConfig {
    ...
    pub fn to_bash_params_json(
        ...
        // production mad-build opts up to 10h by sending it explicitly
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `MAD_WEB_FETCH_PROXY` env > remote settings > None.
    ...
    /// Resolution: TOML > `MAD_WEB_FETCH_ALLOW_LOCAL` env > false.
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
    ...
    ) -> xai_mad_tools::implementations::mad_build::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("MAD_WEB_FETCH_PROXY"))
        ...
        let allow_local = self
            .allow_local
            .or_else(|| xai_mad_config::env_bool("MAD_WEB_FETCH_ALLOW_LOCAL"));
...
/// from `xai_mad_tools::registry::types::ToolsetConfig` which holds
...
///
/// Selects between the standard `MadagascarBuild` toolset (`read_file`,
/// `search_replace`, `grep`) and the anchor-based `MadagascarBuildHashline`
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "MadagascarBuildHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "MadagascarBuildHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"MadagascarBuild:read_file"));
        assert!(ids.contains(&"MadagascarBuild:search_replace"));
        assert!(ids.contains(&"MadagascarBuild:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"MadagascarBuildHashline:hashline_read"));
        assert!(ids.contains(&"MadagascarBuildHashline:hashline_edit"));
        assert!(ids.contains(&"MadagascarBuildHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            xai_mad_agent::config::AgentDefinition::plan(),
            xai_mad_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "MadagascarBuild:search_replace"),
            ...
            assert!(
                ids.contains(&"MadagascarBuildHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"MadagascarBuild:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"MadagascarBuildHashline:hashline_edit"),
    ...
    fn max_timeout_secs_defaults_to_production_10h() {
        ...
        assert_eq!(
            ...
            "production mad-build must set the 10h foreground ceiling"
        );
    }
