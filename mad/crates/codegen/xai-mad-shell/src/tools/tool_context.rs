...
//! xai-mad-tools. This struct is NOT used for tool execution — it's session infrastructure.
//!
...
use xai_mad_workspace::file_system::{AsyncFileSystem, AsyncFsWrapper};
use xai_mad_workspace::session::file_state::FileStateHandle;
...
pub(crate) fn subagent_foreground_wait(
    state: Arc<BlockingWaitState>,
) -> xai_mad_tools::implementations::mad_build::task::types::SubagentForegroundWait {
    xai_mad_tools::implementations::mad_build::task::types::SubagentForegroundWait::new(
...
pub struct ToolContext {
    ...
    pub lsp: Option<Arc<dyn xai_mad_tools::implementations::lsp::LspBackend>>,
    ...
    pub monitor_event_buffer:
        Option<xai_mad_tools::implementations::mad_build::monitor::types::MonitorEventBuffer>,
    ...
    pub task_wake_suppressed:
        Option<xai_mad_tools::reminders::task_completion::TaskWakeSuppressed>,
...
impl ToolContext {
    ...
    pub fn new(
        ...
        let session_env = xai_mad_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
