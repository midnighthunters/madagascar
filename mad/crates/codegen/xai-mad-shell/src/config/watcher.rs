...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.mad/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** mad process
    /// sharing the same `~/.mad` (the writer may also be this process;
...
/// Watches `~/.mad/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.mad/config.toml`, `.mcp.json`,
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.mad/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let mad_home_buf = mad_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(mad_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(mad_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(mad_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(mad_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %mad_home.display(),
                    error = %e,
                    "failed to watch mad home directory"
                )
            })
            .ok()?;
        ...
        // includes `<cwd>/.mad/config.toml` so the loop above
        // watches `<cwd>/.mad/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.mad/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.mad/config.toml` trigger a
    ...
    /// `<cwd>/.mad/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.mad/` does not yet
/// exist at session-open time, the `.mad/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.mad/` followed by a
/// write to `<cwd>/.mad/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let mad_dir = cwd.join(".mad");
    if let Err(e) = debouncer
        .watcher()
        .watch(&mad_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .mad directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let mad_dir = cwd.join(".mad");
    if let Err(e) = debouncer.watcher().unwatch(&mad_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .mad directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.mad/` hasn't
...
fn discovery_change_for_path(path: &Path) -> Option<DiscoveryChange> {
    if path.file_name().is_some_and(|name| name == ".mad") {
...
/// recursively: `mad_home` (`~/.mad`, or `$MAD_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.mad` alone has `worktrees/`,
...
fn is_global_config_dir(dir: &Path, mad_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, mad_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".mad", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, mad_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(mad_home) {
        return true;
    }
...
impl ProjectDiscoveryWatcher {
    pub fn start(cwd: &Path) -> Option<(Self, mpsc::UnboundedReceiver<DiscoveryChange>)> {
        ...
        let project_mad = project_root.join(".mad");
        let workflows = project_mad.join("workflows");
        let (tx, rx) = mpsc::unbounded_channel();
        let project_mad_for_events = project_mad.clone();
        let mut debouncer =
            new_filtered_debouncer(SKILLS_DEBOUNCE, move |res: DebounceEventResult| {
                ...
                for event in events
                    .iter()
                    .filter(|event| event.path.starts_with(&project_mad_for_events))
                {
        ...
        let initial = if project_mad.is_dir() {
            project_mad.clone()
        } else {
            project_root.clone()
        };
        ...
        let refresh_dirs = vec![
            (project_mad, RecursiveMode::NonRecursive),
            (
                project_root.join(".mad").join("skills"),
                RecursiveMode::Recursive,
            ),
            (
                project_root.join(".mad").join("commands"),
...
impl SkillsFileWatcher {
    ///
    /// Uses [`collect_skill_config_dirs`](xai_mad_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let dirs_to_watch = xai_mad_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &mad_home,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &mad_home) {
        ...
        let mut refresh_dirs = vec![(mad_home.join("workflows"), RecursiveMode::NonRecursive)];
        if let Some(cwd) = cwd {
            let project_root = crate::session::workflow::registry::project_root(cwd);
            let project_mad = project_root.join(".mad");
            let parent_watch = if project_mad.is_dir() {
                project_mad.clone()
            } else {
                project_root
            };
            ...
            refresh_dirs.push((project_mad.clone(), RecursiveMode::NonRecursive));
            refresh_dirs.push((project_mad.join("workflows"), RecursiveMode::NonRecursive));
        }
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only mad_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_mad_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let mad_home = home.join(".mad");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &mad_home, Some(home));

        // mad_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&mad_home));
        ...
        assert!(!g(&home.join("repo").join(".mad")));
    }

    /// Regression for the ~/.mad inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.mad/worktrees/`) must not
    ...
    fn workflow_change_classifies_missing_directory_creation() {
        let mad = Path::new("/tmp/project/.mad");
        assert_eq!(
            discovery_change_for_path(mad),
            Some(DiscoveryChange::Skills),
            "first .mad creation must take the broader skills reload path"
        );
        assert_eq!(
            discovery_change_for_path(&mad.join("workflows")),
            Some(DiscoveryChange::Workflows)
        );
        assert_eq!(
            discovery_change_for_path(&mad.join("workflows/review.rhai")),
            Some(DiscoveryChange::Workflows)
        );
        assert_eq!(
            discovery_change_for_path(&mad.join("skills/review/SKILL.md")),
            Some(DiscoveryChange::Skills)
        );
    }
    ...
    /// A write to `<mad_home>/models_cache.json` must surface as
    /// `ConfigChangeEvent::ModelsCacheChanged` so a long-running leader can
    /// hot-load a catalog fetched by another mad process.
    #[test]
    #[cfg_attr(
        target_os = "macos",
        ignore = "flaky on macOS: FSEvents does not reliably deliver events in test harness"
    ...
    /// A write to `<cwd>/.mad/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        let mad_home = TempDir::new().unwrap();
        let cwd = TempDir::new().unwrap();
        let project_mad = cwd.path().join(".mad");
        fs::create_dir_all(&project_mad).unwrap();
        // Seed the file before the watcher starts so we observe the
        // modification rather than the creation event.
        fs::write(project_mad.join("config.toml"), "").unwrap();
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.mad/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.mad/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_mad = new_cwd.path().join(".mad");
        fs::create_dir_all(&project_mad).unwrap();
        fs::write(project_mad.join("config.toml"), "").unwrap();
