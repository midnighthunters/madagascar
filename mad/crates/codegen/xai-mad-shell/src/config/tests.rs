...
fn expands_env_vars_in_toml_strings() {
    with_env_var(
        "MAD_TEST_CONFIG_EXPAND",
        "expanded",
        || {
            let toml_str = r#"
[mcp_servers.test]
command = "$MAD_TEST_CONFIG_EXPAND/bin/server"
args = ["--path", "${MAD_TEST_CONFIG_EXPAND}/data"]
"#;
...
fn leaves_missing_env_vars_unchanged() {
    let toml_str = r#"
[mcp_servers.test]
command = "$MAD_TEST_CONFIG_MISSING/bin/server"
"#;
    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
    expand_env_vars_in_toml(&mut value);
    let toml::Value::Table(table) = value else {
    ...
    let command = test.get("command").and_then(|v| v.as_str()).unwrap();
    assert_eq!(command, "$MAD_TEST_CONFIG_MISSING/bin/server");
}
#[test]
...
/// Mutex to serialize tests that touch the MAD_MEMORY env var.
...
/// Run `f` with MAD_MEMORY explicitly unset.
fn without_mad_memory<T>(f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("MAD_MEMORY", None, f)
}
/// Run `f` with MAD_MEMORY set to a specific value.
fn with_mad_memory<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("MAD_MEMORY", Some(value), f)
}
#[test]
...
fn memory_config_env_var_zero_does_not_enable() {
    with_mad_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(!mem.enabled, "MAD_MEMORY=0 should not enable memory");
        },
    );
}
#[test]
fn memory_config_env_var_false_does_not_enable() {
    with_mad_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(!mem.enabled, "MAD_MEMORY=false should not enable memory");
        },
    );
}
#[test]
...
fn memory_config_env_zero_force_disables_toml_enabled() {
    with_mad_memory(
        ...
        || {
            ...
            assert!(
                !mem.enabled,
                "MAD_MEMORY=0 should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_env_false_force_disables_toml_enabled() {
    with_mad_memory(
        ...
        || {
            ...
            assert!(
                !mem.enabled,
                "MAD_MEMORY=false should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_cli_flag_overrides_env_disable() {
    with_mad_memory(
        ...
        || {
            ...
            assert!(
                mem.enabled,
                "CLI --experimental-memory should override MAD_MEMORY=0"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_all() {
    with_mad_memory(
        ...
        || {
            ...
            assert!(
                !mem.enabled,
                "--no-memory should override --experimental-memory, MAD_MEMORY=1, and TOML enabled=true"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_env_enable() {
    with_mad_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, true, &config, None);
            assert!(!mem.enabled, "--no-memory should override MAD_MEMORY=1");
        },
    );
}
#[test]
...
"#;
        ...
        assert_eq!(mem.flush.flush_model.as_deref(), Some("mad-4"));
        ...
        || {
            with_env_var(
                ...
                || {
                    let toml_str = r#"
[mcp_servers.test]
command = "$MAD_TEST_USER $MAD_TEST_ROOT"
"#;
                    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
                    expand_env_vars_in_toml(&mut value);
                    let toml::Value::Table(table) = value else {
...
/// Mutex to serialize tests that touch the MAD_SUBAGENTS env var.
static SUBAGENTS_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
/// Run `f` with MAD_SUBAGENTS explicitly unset.
fn without_mad_subagents<T>(f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("MAD_SUBAGENTS", None, f)
}
/// Run `f` with MAD_SUBAGENTS set to a specific value.
fn with_mad_subagents<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("MAD_SUBAGENTS", Some(value), f)
}
#[test]
fn subagents_config_default_enabled() {
    without_mad_subagents(|| {
        let config = toml::Value::Table(toml::map::Map::new());
    ...
    with_mad_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str("[subagents]\nenabled = true")
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config);
            assert!(!sa.enabled, "MAD_SUBAGENTS=0 should override config file");
        },
    );
}
#[test]
...
fn subagents_config_env_var_disables_default() {
    with_mad_subagents(
        ...
        || {
            ...
            assert!(
                !sa.enabled,
                "MAD_SUBAGENTS=0 should override the enabled default"
            );
        },
    );
}
...
fn subagents_config_cli_flag_overrides_env_var() {
    with_mad_subagents(
        ...
        || {
            ...
            assert!(
                sa.enabled,
                "--subagents CLI flag should override MAD_SUBAGENTS=0"
            );
        },
    );
}
...
fn subagents_config_models_parsed() {
    without_mad_subagents(|| {
        ...
        assert_eq!(sa.models.get("explore").unwrap(), "mad-3-fast");
        assert_eq!(sa.models.get("plan").unwrap(), "mad-4.5");
    });
}
#[test]
    ...
    with_mad_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str(
                ...
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config);
            assert!(sa.enabled, "MAD_SUBAGENTS=1 should enable");
            assert_eq!(sa.models.get("explore").unwrap(), "mad-3-fast");
        },
    );
}
...
fn with_managed_mcp_env<T>(
    ...
    with_env_var_opt(
        "MAD_MANAGED_MCPS_ENABLED",
        managed_mcps,
        || with_env_var_opt("MAD_MANAGED_MCP_GATEWAY_TOOLS_ENABLED", gateway_tools, f),
    )
}
#[test]
#[serial_test::serial]
...
fn with_model_overrides_env_full<T>(
    ...
    let _guard = LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt(
        "MAD_WEB_SEARCH_MODEL",
        ws,
        || with_env_var_opt(
            "MAD_SESSION_SUMMARY_MODEL",
            ss,
            || with_env_var_opt(
                "MAD_IMAGE_DESCRIPTION_MODEL",
                id,
                || with_env_var_opt("MAD_PROMPT_SUGGESTIONS_MODEL", ps, f),
            ),
        ),
    )
}
...
#[test]
fn model_overrides_default_image_description_is_mad_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
#[test]
fn model_overrides_default_session_summary_is_mad_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
fn with_tools_env<T>(
    ...
    with_env_var_opt(
        "MAD_RESPECT_GITIGNORE",
        respect_gitignore,
        || with_env_var_opt("MAD_DISABLE_ZDR_INCOMPATIBLE_TOOLS", disable_zdr, f),
    )
}
fn without_mad_respect_gitignore<T>(f: impl FnOnce() -> T) -> T {
    with_tools_env(None, None, f)
}
fn with_mad_respect_gitignore<T>(value: &str, f: impl FnOnce() -> T) -> T {
    with_tools_env(Some(value), None, f)
}
#[test]
fn tools_config_default_disabled() {
    without_mad_respect_gitignore(|| {
        let config = toml::Value::Table(toml::map::Map::new());
    ...
    with_mad_respect_gitignore(
        ...
        || {
            ...
            assert!(
                ...
                "MAD_RESPECT_GITIGNORE=false should override config file"
            );
        },
    );
}
...
fn roles_parse_from_toml() {
    let toml_str = r#"
            ...
            prompt_file = ".mad/prompts/impl.md"
        "#;
    ...
    assert_eq!(researcher.model.as_deref(), Some("mad-3"));
...
fn discover_roles_loads_from_directory() {
    ...
    let roles_dir = tmp.path().join(".mad").join("roles");
...
fn personas_parse_from_toml() {
    let toml_str = r#"
            ...
            instructions_file = ".mad/personas/concise.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
...
fn discover_personas_loads_from_directory() {
    let tmp = tempfile::TempDir::new().unwrap();
    let dir = tmp.path().join(".mad").join("personas");
...
fn bundled_personas_and_roles_have_lowest_priority_in_resolve_order() {
    ...
    let bundled = home.join(".mad").join("bundled");
    std::fs::create_dir_all(workspace.join(".mad").join("roles")).unwrap();
    std::fs::create_dir_all(workspace.join(".mad").join("personas")).unwrap();
    std::fs::create_dir_all(home.join(".mad").join("roles")).unwrap();
    std::fs::create_dir_all(home.join(".mad").join("personas")).unwrap();
    ...
    std::fs::write(
        ...
        .unwrap();
    std::fs::write(
            home.join(".mad/roles/reviewer.toml"),
            r#"description = "User reviewer""#,
        )
        .unwrap();
    std::fs::write(
            home.join(".mad/personas/reviewer.toml"),
            r#"instructions = "User persona""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".mad/roles/reviewer.toml"),
            r#"description = "Project reviewer""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".mad/personas/reviewer.toml"),
    ...
    std::fs::remove_file(workspace.join(".mad/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(workspace.join(".mad/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
    ...
    std::fs::remove_file(home.join(".mad/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(home.join(".mad/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
...
fn render_io_summary_shows_bundled_for_bundled_personas() {
    let persona = SubagentPersona {
        ...
        source_path: Some("/tmp/home/.mad/bundled/personas/reviewer.toml".to_string()),
        ..Default::default()
    };
...
/// a repo `.mad/config.toml` structurally cannot supply one.)
#[test]
...
cli_chat_proxy_base_url = "https://cli-chat-proxy.mad.com/v1"
...
"#,
    ...
    assert_eq!(
            ...
            "https://cli-chat-proxy.mad.com/v1/deployment/config"
        );
    assert!(
            !cfg.endpoints
...
/// RCE guard: a project `.mad/config.toml` must never source
...
#[test]
fn validate_hooks_path_rejects_outside_mad_home() {
    let result = validate_hooks_path("/tmp/evil-hooks");
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
            msg.contains("must be under ~/.mad/"),
            "should mention ~/.mad/ restriction, got: {msg}"
        );
}
#[test]
fn validate_hooks_path_rejects_traversal_attack() {
    let mad_home = crate::util::mad_home::mad_home();
    let traversal = format!("{}/../evil", mad_home.display());
...
#[test]
fn validate_hooks_path_accepts_mad_hooks_subdir() {
    let mad_home = crate::util::mad_home::mad_home();
    let valid_path = mad_home.join("hooks").join("my-hooks");
    let _ = std::fs::create_dir_all(&valid_path);
    let result = validate_hooks_path(valid_path.to_str().unwrap());
    assert!(result.is_ok(), "path under ~/.mad/ should be accepted");
}
#[test]
fn managed_settings_disables_features_and_requirements_overrides() {
    use xai_mad_workspace::permission::resolution::ManagedSettingsFeatures;
    let mut cfg = crate::agent::config::Config::default();
...
/// disableBypassPermissionsMode (-> features.disable_yolo) must NOT clamp the user's own mad yolo.
#[test]
...
fn simulate_release_build() -> xai_mad_test_support::EnvGuard {
    xai_mad_test_support::EnvGuard::set(xai_mad_version::TEST_VERSION_ENV, "0.0.0-sim")
}
#[test]
...
fn base_resolver_without_project_cwd_keeps_project_files_out() {
    let tmp = tempfile::tempdir().unwrap();
    write_subagent_definitions(&tmp.path().join(".mad"), &[("project", "Project")]);
    let base = SubagentsConfig::resolve_base_with_sources(
        false,
...
#[test]
fn explicit_mad_root_is_the_only_user_source() {
    let tmp = tempfile::tempdir().unwrap();
    let ambient = tmp.path().join("ambient-home/.mad");
    let configured = tmp.path().join("configured-mad-home");
...
/// verdicts untouched). MAD_HOME-isolated + `#[serial]` for folder-trust
/// store hygiene (empty store ⇒ deterministic untrusted;
/// `EnvGuard` restores MAD_HOME even on panic). No user-global
/// `$MAD_HOME/config.toml` is seeded: `mad_home()` is `OnceLock`-cached,
...
/// in `xai-mad-agent`. An ABSOLUTE plugin path is used so the merged
...
/// MAD_HOME-isolated + `#[serial]` (`EnvGuard` restores it even on panic).
#[test]
#[serial_test::serial]
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use xai_mad_agent::plugins::{TrustStore, discover_plugins};
    use xai_mad_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("MAD_HOME", home.path());
    let _flag = EnvGuard::unset("MAD_FOLDER_TRUST");
...
/// arm (store-only reconcile) could never lift. MAD_HOME-isolated (empty
/// store); MAD_FOLDER_TRUST unset so the kill-switch is the only signal.
#[test]
#[serial_test::serial]
fn kill_switched_cold_cwd_stays_allowed_through_plugins_config_read() {
    ...
    let mad = repo.path().join(".mad");
    std::fs::create_dir_all(&mad).unwrap();
    std::fs::write(mad.join("config.toml"), "[plugins]\npaths = [\"./proj-plugin\"]\n")
        .unwrap();
    let cwd = repo.path();
    let remote = crate::util::config::RemoteSettings {
...
/// Writeback requires mad.com auth: remote may advertise it, but a non-xai
