...
use crate::auth::{MadAuth, read_auth_json};
...
pub enum ConfigUpdate {
    ...
    ///
    /// 1. The global `[mcp_servers]` table in `~/.mad/config.toml`
    ///    changed.
    ...
    ///
    /// Project-scoped changes (`<cwd>/.mad/config.toml`,
    ...
    /// (`<cwd>/.mad/config.toml`, `<cwd>/.mcp.json`, or
    ...
    /// mad process sharing the home dir). The agent should consult the cache
    /// thrash unrelated sessions.
    ProjectMcpServersChanged {
        /// The project root whose `.mad/`, `.mcp.json`, or
    ...
    ModelsChanged,
    /// `~/.mad/models_cache.json` was rewritten on disk (possibly by another
...
impl ConfigReloader {
    ...
    pub async fn run(
        ...
        loop {
            ...
            if has_auth {
                ...
                match result {
                    Ok(Err(e)) => {
                        ...
                        let path = self.mad_home.join("auth.json");
            ...
            // same tick (e.g. `~/.mad/config.toml` AND
    ...
    pub(crate) fn reload_auth(&mut self) -> anyhow::Result<()> {
        let auth_path = self.mad_home.join("auth.json");
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // config (`~/.mad/config.toml`) via toml::Value. Project-
        // scoped changes (`<cwd>/.mad/config.toml`,
...
/// |----------------------------|-------------------------|-------------------|
/// | `ProjectConfigChanged`     | `<cwd>/.mad/config.toml` | `<cwd>`           |
...
/// for `.mad/config.toml`, `find_mcp_json_files` for `.mcp.json`) so
...
/// (`~/.mad/config.toml`, `~/.claude.json`, `~/.cursor/mcp.json`)
...
/// `ConfigUpdate::Skills`) and `mad inspect` (via the `crate::config`
...
pub(crate) fn parse_skills_config(
    config: &toml::Value,
) -> xai_mad_agent::prompt::skills::SkillsConfig {
    config
        .get("skills")
...
fn parse_compat_config(config: &toml::Value) -> xai_mad_tools::types::compat::CompatConfigToml {
    config
        .get("compat")
        .and_then(|v| v.clone().try_into().ok())
        .unwrap_or_default()
...
mod tests {
    ...
    async fn reloader_detects_new_auth_key() {
        ...
        assert!(
            matches!(update, ConfigUpdate::Auth(a) if a.key == "new-key"), // a is Box<MadAuth>, Deref coercion
    ...
    /// The hash must reflect ancestor `.mad/config.toml` and `.mcp.json`
    ...
    fn hash_project_mcp_config_covers_ancestors() {
        ...
        std::fs::write(tmp.path().join(".mad").join("config.toml"), "x = 1").unwrap();
        let h3 = hash_project_mcp_config(&child).expect("readable");
        assert_ne!(
            h2, h3,
            "ancestor .mad/config.toml create must change the hash"
        );
    }
    ...
    fn parse_skills_config_with_paths() {
        let config: toml::Value = toml::from_str(
            r#"
[skills]
paths = ["/home/user/.mad/skills"]
ignore = ["/tmp"]
"#,
        )
        .unwrap();
        let skills = parse_skills_config(&config);
        assert_eq!(skills.paths, vec!["/home/user/.mad/skills".to_string()]);
...
"#,
        ...
        assert!(yolo);
        assert_eq!(fork.as_deref(), Some("mad-4.5"));
    }
...
"#,
    ...
    /// `ProjectConfigChanged` (`<cwd>/.mad/config.toml`) and
    ...
    fn collect_project_cwds_excludes_home_claude_json() {
        let batch = vec![
            ...
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/x/.mad/config.toml"),
            },
        ];
    ...
    fn collect_project_cwds_dedupes_and_extracts() {
        let batch = vec![
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/a/.mad/config.toml"),
            },
            ConfigChangeEvent::McpConfigChanged {
                path: PathBuf::from("/repo/a/.mcp.json"),
            },
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/b/.mad/config.toml"),
            },
        ];
        let cwds = collect_project_cwds(&batch);
        assert_eq!(
