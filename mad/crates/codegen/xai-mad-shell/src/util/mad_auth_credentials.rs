...
use std::sync::Arc;
/// Credentials for authenticating with mad backend services.
///
...
/// User token (xAI users) sends `Bearer` + `X-XAI-Token-Auth: xai-mad-cli`.
/// Deployment key takes precedence when both are present.
#[derive(Clone)]
pub struct MadAuthCredentials {
...
impl std::fmt::Debug for MadAuthCredentials {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("MadAuthCredentials")
            .field(
                "user_token",
...
impl MadAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your MAD_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `mad login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
    ...
    pub fn resolve(&self) -> MadAuthCredentials {
    ...
    /// the bearer.
    pub async fn resolve_async(&self) -> MadAuthCredentials {
...
impl xai_mad_auth::HttpAuth for MadAuthCredentials {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        MadAuthCredentials::apply(self, builder, base_url)
    }
}
#[cfg(test)]
mod tests {
    ...
    fn resolve_returns_token_when_not_expired() {
        ...
        let creds = MadAuthCredentials::new(None).with_auth_manager(mgr);
    ...
    fn resolve_returns_token_during_early_invalidation_window() {
        ...
        let creds = MadAuthCredentials::new(None).with_auth_manager(mgr.clone());
    ...
    fn resolve_returns_static_token_when_no_auth_manager() {
        let creds = MadAuthCredentials::new(Some("static-token".into()));
