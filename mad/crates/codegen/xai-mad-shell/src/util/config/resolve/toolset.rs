...
/// [`xai_mad_config::env_bool`] parser (`MAD_TOOLS_FIND_BFS` /
/// `MAD_TOOLS_GREP_UGREP`, plus the `MAD_FIND_BFS` / `MAD_GREP_UGREP` aliases).
///
/// Pass the **merged** requirements ([`crate::config::load_merged_requirements`])
/// so an org policy in any requirements layer — not only
/// `~/.mad/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`,
/// which the caller bakes into a
/// [`xai_mad_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub fn resolve_search_tools_enabled(
    ...
    let disable = xai_mad_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = xai_mad_config::env_bool(primary).or_else(|| xai_mad_config::env_bool(alias));
    ...
    (
        resolve("MAD_TOOLS_FIND_BFS", "MAD_FIND_BFS", "find_bfs"),
        resolve("MAD_TOOLS_GREP_UGREP", "MAD_GREP_UGREP", "grep_ugrep"),
    )
}
...
) -> Option<xai_mad_tools::util::ShellEnvironmentPolicy> {
    let value = effective_cfg?.get("shell_environment_policy")?.clone();
    match value.try_into::<xai_mad_tools::util::ShellEnvironmentPolicy>() {
...
const ENV_LOGIN_SHELL_CAPTURE: &str = "MAD_LOGIN_ENV";
...
const ENV_SCHEDULER_BACKGROUND_LOOPS: &str = "MAD_SCHEDULER_BACKGROUND_LOOPS";
...
///
/// Precedence: requirements > env (`MAD_SCHEDULER_BACKGROUND_LOOPS`) > user
...
/// the shared [`xai_mad_config::env_bool`] via `BoolFlag`). The secs env var
/// lives in the tools crate (`RESPONSE_TIMEOUT_ENV`), parsed once there.
const ENV_ASK_USER_QUESTION_TIMEOUT_ENABLED: &str = "MAD_ASK_USER_QUESTION_TIMEOUT_ENABLED";
...
///
/// Precedence: requirements > env (`MAD_ASK_USER_QUESTION_TIMEOUT_ENABLED`)
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            xai_mad_tools::implementations::mad_build::ask_user_question::RESPONSE_TIMEOUT
                .as_secs(),
        )
}
...
///
/// Precedence: requirements > env (`MAD_ASK_USER_QUESTION_TIMEOUT_SECS`,
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        xai_mad_tools::implementations::mad_build::ask_user_question::response_timeout_env_secs(),
...
) -> xai_mad_tools::implementations::mad_build::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
mod ask_user_question_timeout_tests {
    ...
    use xai_mad_tools::implementations::mad_build::ask_user_question::RESPONSE_TIMEOUT_ENV;
    ...
    fn timeout_secs_tier_precedence() {
        let d = xai_mad_tools::implementations::mad_build::ask_user_question::RESPONSE_TIMEOUT
            .as_secs();
...
mod tests {
    ...
    // Assumes MAD_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
...
mod shell_env_policy_tests {
    use super::*;
    use xai_mad_tools::util::{EnvironmentVariablePattern, ShellEnvironmentPolicyInherit};
