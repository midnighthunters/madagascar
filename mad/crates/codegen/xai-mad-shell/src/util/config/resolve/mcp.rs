...
/// | env          | `MAD_MCP_LIVENESS_WATCHERS` (handled by `BoolFlag::env`)       |
/// | config       | `[features] mcp_liveness_watchers` in `~/.mad/config.toml`     |
...
/// | cli          | (none — no CLI flag)                                            |
/// | env          | `MAD_MCP_AUTO_RESTART` (handled by `BoolFlag::env`)            |
/// | config       | `[features] mcp_auto_restart` in `~/.mad/config.toml`          |
...
/// | cli          | (none — no CLI flag)                                            |
/// | env          | `MAD_MCP_PUSH_SERVER_STATUS` (handled by `BoolFlag::env`)      |
/// | config       | `[features] mcp_push_server_status` in `~/.mad/config.toml`    |
...
/// | cli          | (none — no CLI flag)                                                |
/// | env          | `MAD_MCP_RECURSIVE_CONFIG_WATCH` (handled by `BoolFlag::env`)      |
/// | config       | `[features] mcp_recursive_config_watch` in `~/.mad/config.toml`    |
...
/// Kept in sync with `xai_mad_mcp::servers`'s standalone fallback.
...
/// Env override for the MCP startup timeout, in seconds (mad-native).
const ENV_MCP_STARTUP_TIMEOUT_SECS: &str = "MAD_MCP_STARTUP_TIMEOUT_SECS";
...
/// `MAD_MCP_STARTUP_TIMEOUT_SECS` secs) > effective `config.toml [mcp]` >
...
/// `MAD_MCP_STARTUP_TIMEOUT_SECS` (secs). Unparseable/zero values are ignored.
...
pub const DEFAULT_MAX_MCP_OUTPUT_BYTES: usize = xai_mad_tools::MCP_MAX_OUTPUT_BYTES;
...
pub fn cache_remote_max_mcp_output_bytes(remote: Option<u64>) {
    xai_mad_tools::set_mcp_max_output_bytes(resolve_max_mcp_output_bytes(remote));
}
...
///   2. env `MAD_MAX_MCP_OUTPUT_BYTES` / `MAX_MCP_OUTPUT_BYTES`
///      (Mad-native wins when both set)
...
pub fn resolve_max_mcp_output_bytes(remote: Option<u64>) -> usize {
    ...
    resolve_max_mcp_output_bytes_precedence(
        requirements,
        xai_mad_tools::mcp_max_output_bytes_from_env(),
...
/// `.mad/config.toml` chain (`cwd` → git root), deepest file wins.
///
...
fn project_max_mcp_output_bytes(cwd: &std::path::Path) -> Option<usize> {
    ...
    for config_path in crate::config::find_project_configs(cwd) {
        if let Ok(toml_val) = xai_mad_config::load_config_file(&config_path)
...
pub fn resolve_max_mcp_output_bytes_for_cwd(cwd: &std::path::Path) -> Option<usize> {
    let requirements = crate::config::load_merged_requirements()
        .as_ref()
        .and_then(max_mcp_output_bytes_from_toml);
    if requirements.is_some() || xai_mad_tools::mcp_max_output_bytes_from_env().is_some() {
        return None;
    }
...
mod max_mcp_output_bytes_tests {
    ...
    fn project_chain_deepest_file_wins() {
        ...
        std::fs::create_dir_all(sub.join(".mad")).unwrap();
        std::fs::create_dir_all(root.join(".mad")).unwrap();
        std::fs::write(
            root.join(".mad/config.toml"),
        ...
        std::fs::write(sub.join(".mad/config.toml"), "[ui]\nvim_mode = true\n").unwrap();
        assert_eq!(super::project_max_mcp_output_bytes(&sub), Some(30_000));

        // No .mad files with the key anywhere → None.
        std::fs::remove_file(root.join(".mad/config.toml")).unwrap();
