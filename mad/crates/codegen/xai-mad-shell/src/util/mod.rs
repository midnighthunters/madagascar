pub mod config;
pub mod mad_auth_credentials;
pub mod hooks;
pub mod limits;
pub(crate) mod subprocess;
pub(crate) mod user_identity;

// The foundation utilities live in `xai-mad-shell-base` (upstream of this
// crate so they build in parallel). Re-exported at the original paths so
// existing `crate::util::…` / `xai_mad_shell::util::…` users compile
// unchanged.
pub use xai_mad_shell_base::util::*;
...
pub(crate) fn is_user_instruction_path(
    ...
    let mad_rules = mad_home.join("rules");
    let is_exact_home_surface = parent
        .is_some_and(|parent| parent == mad_home || parent == mad_rules)
...
mod is_user_instruction_path_tests {
    ...
    #[test]
    fn mad_home_named_file_nested_in_workspace_is_user_scoped() {
    ...
    #[test]
    fn workspace_descendants_under_mad_home_stay_project_scoped() {
        assert!(!is_user_instruction_path(
            Path::new("/custom/mad/worktrees/repo/src/AGENTS.md"),
            Path::new("/custom/mad"),
            &[],
