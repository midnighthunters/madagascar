//! Built-in files extracted to `~/.mad/` on startup.

const BUILTIN_FILES: &[(&str, &str)] = &[("README.md", include_str!("../README.md"))];

/// Extract built-in metadata files to `~/.mad/` on startup.
///
/// User skills under `~/.mad/skills/` are never managed here. Platform skills
/// are delivered separately through the bundled skill cache.
pub fn extract_builtin_files(mad_home: &std::path::Path) {
    let version = xai_mad_version::VERSION;
    let marker = mad_home.join(".metadata_version");
    ...
    for stale in &["CHANGELOG.json", "CHANGELOG.md"] {
        let _ = std::fs::remove_file(mad_home.join(stale));
    }

    for &(filename, content) in BUILTIN_FILES {
        if let Err(e) = std::fs::write(mad_home.join(filename), content) {
...
mod tests {
    ...
    fn same_version_does_not_restore_missing_or_delete_legacy_skills() {
        ...
        std::fs::write(home.join(".metadata_version"), xai_mad_version::VERSION).unwrap();
