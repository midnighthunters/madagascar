...
pub const LEGACY_XAI_API_KEY_ENV_VAR: &str = "MAD_CODE_XAI_API_KEY";

/// Read the API key from the environment.
///
/// Checks `XAI_API_KEY` first, then falls back to the legacy
/// `MAD_CODE_XAI_API_KEY` for backward compatibility.
...
/// Returns `true` if either `XAI_API_KEY` or `MAD_CODE_XAI_API_KEY` is set.
...
/// `MAD_DISABLE_API_KEY_AUTH`) is the admin kill switch: when true the
...
pub struct AuthMethodsBuildInputs<'a> {
    ...
    /// Optional display label for the login method (`mad.com` or `oidc`).
    pub login_label: Option<&'a str>,
    /// True if `mad_com_config.auth_provider_command` is configured (sets
    /// `meta.external_provider = true` on the `mad.com` method).
...
fn push_interactive_login(
    ...
    if has_enterprise_oidc {
        ...
        // the same `cfg.mad_com_config.oidc` Option, so the inconsistent
        // `(true, None)` combination is a programmer error -- panic loudly
        // (matches the original `cfg.mad_com_config.oidc.as_ref().unwrap()`
        ...
        methods.push(mad_com_auth_method(login_label, has_auth_provider_command));
    }
}
...
/// `true` for session-based ACP methods (cached_token, mad.com, oidc).
...
pub const AUTH_ERROR_SESSION_EXPIRED: &str =
    "Session expired. Run `mad login` to re-authenticate.";

pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `mad login`, set XAI_API_KEY, or add api_key to ~/.mad/config.toml.";
...
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `mad login` to authenticate.";
...
pub fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.mad/auth.json".to_string())),
    )
}

pub const MAD_COM_METHOD_ID: &str = "mad.com";

/// xAI OAuth2/OIDC auth. Method id `"mad.com"` kept for ACP wire-compat.
pub fn mad_com_auth_method(
    label: Option<&str>,
    has_auth_provider_command: bool,
) -> acp::AuthMethod {
    let name = label.unwrap_or("Mad");
    ...
    acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new(acp::AuthMethodId::new(MAD_COM_METHOD_ID), name.to_string())
...
mod tests {
    ...
    /// No advertiseable API-key credentials → interactive `mad.com`.
    #[test]
    fn after_cached_token_unavailable_falls_to_mad_com_without_api_key() {
        assert_eq!(
    ...
    /// `mad.com` — `auth_methods.first()` does NOT need interactive login,
    ...
    /// Brand-new user (no API key, no cached token): only `mad.com` is
    ...
    #[test]
    fn fresh_user_only_advertises_mad_com_and_requires_login() {
        let built = build_auth_methods(default_inputs());

        assert_eq!(first_kind(&built.methods), Some(AuthMethodKind::MadCom));
        assert!(built.default_auth_method_id.is_none());
        assert_eq!(built.methods.len(), 1);
    }

    /// Enterprise OIDC replaces `mad.com` (mutually exclusive). xai.api_key,
    /// when present, still leads.
    #[test]
    fn enterprise_oidc_replaces_mad_com_but_xai_api_key_still_first() {
        let inputs = AuthMethodsBuildInputs {
            has_external_api_key: true,
            has_cached_token: false,
        ...
        assert!(
            !built
                .methods
                .iter()
                .any(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::MadCom),
            "mad.com and oidc are mutually exclusive",
        );
    }

    /// `has_auth_provider_command` is plumbed through to the `mad.com` method
    ...
    fn auth_provider_command_sets_external_provider_meta() {
        ...
        let mad = built
            .methods
            .iter()
            .find(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::MadCom)
            .expect("mad.com must be advertised");
        assert_eq!(mad.name(), "Acme Corp");
        let meta = mad.meta().expect("meta should be set");
        assert_eq!(
    ...
    /// `~/.mad/config.toml` skeleton from the bug report, walks it through
    ...
    fn enterprise_byok_config_does_not_require_login() {
        ...
        // pager's `startup_auth_metadata()` returns `needs_login = false`.
        {
            ...
            let built = build_auth_methods(AuthMethodsBuildInputs {
                ...
                // mad.com login (no enterprise OIDC).
                has_cached_token: false,
                ..default_inputs()
            });
            assert_eq!(
    ...
    /// Legacy `MAD_CODE_XAI_API_KEY` env var is accepted as a fallback
    ...
    /// When both `XAI_API_KEY` and `MAD_CODE_XAI_API_KEY` are set,
    ...
    // -- mad login --legacy regression coverage ------------------------
    //
    // `mad login --legacy` produces a MadAuth with `auth_mode: WebLogin`,
    // `oidc_issuer: None`, and no `expires_at` (30-day hardcoded TTL).
    // When this token is present via the `MAD_AUTH` env var (or via legacy
    ...
    // to-end. A regression in MAD_AUTH JSON parsing or in auth method
    // ordering would send legacy-token users to the login screen.

    /// END-TO-END REGRESSION TEST: a legacy auth token (WebLogin, no
    /// expires_at) present in the `MAD_AUTH` env var, with no other auth
    ...
    #[test]
    #[serial]
    fn mad_login_legacy_token_does_not_require_login() {
        use crate::auth::{AuthManager, AuthMode, MadAuth, MadComConfig};

        // Ensure clean slate for "no other auth available".
        let _g1 = EnvGuard::unset("MAD_AUTH_PATH");
        let _g2 = EnvGuard::unset(XAI_API_KEY_ENV_VAR);

        // Construct a legacy-style token exactly as `mad login --legacy`
        // produces: WebLogin mode, no OIDC fields, no refresh_token, no
        // expires_at (is_expired falls back to 30-day age check).
        let legacy_token = MadAuth {
        ...
        // Provide it via MAD_AUTH env var (highest priority code path in
        ...
        let _g = EnvGuard::set("MAD_AUTH", &legacy_json);
        ...
        assert!(
            current.is_some(),
            "legacy token in MAD_AUTH env MUST be loaded directly -- if this fails, \
    ...
    fn no_legacy_token_means_no_cached_token_advertised() {
        ...
        let _g1 = EnvGuard::unset("MAD_AUTH");
        let _g2 = EnvGuard::unset("MAD_AUTH_PATH");
        ...
        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::MadCom),
            "no cached token AND no api key: pager must show login (mad.com first)",
        );
    }
    ...
    fn pin_oidc_with_session_hides_api_key() {
        ...
        assert_eq!(
            method_ids(&built),
            vec![CACHED_TOKEN_AUTH_METHOD_ID, MAD_COM_METHOD_ID]
        );
    ...
    fn pin_oidc_without_session_is_interactive_only() {
        ...
        assert_eq!(method_ids(&built), vec![MAD_COM_METHOD_ID]);
        assert!(built.default_auth_method_id.is_none());
    }
}
