...
pub fn start_early_prefetch_with_auth(auth: Option<MadAuth>) -> Option<EarlyPrefetchHandle> {
...
pub fn start_early_prefetch(mad_com_config: Option<MadComConfig>) -> Option<EarlyPrefetchHandle> {
    start_early_prefetch_impl(mad_com_config, true)
}
...
pub fn start_early_prefetch_settings_only(
    mad_com_config: Option<MadComConfig>,
) -> Option<EarlyPrefetchHandle> {
    start_early_prefetch_impl(mad_com_config, false)
}
...
fn start_early_prefetch_impl(
    ...
    let auth = AuthManager::new(&mad_home, mad_com_config.unwrap_or_default()).current();
