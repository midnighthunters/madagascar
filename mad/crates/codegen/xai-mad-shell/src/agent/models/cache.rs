...
pub(crate) struct ModelsCache {
    ...
    pub(crate) mad_version: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl ModelsCacheManager {
    pub(crate) fn new() -> Self {
        Self {
            path: crate::util::mad_home::mad_home().join(MODELS_CACHE_FILE),
            ttl: CACHE_TTL,
        }
    }
    ...
    pub(crate) fn load_fresh(
        ...
        if cache.mad_version.as_deref() != Some(xai_mad_version::VERSION) {
    ...
    /// Per-writer temp path: `~/.mad` is shared across concurrent CLI
