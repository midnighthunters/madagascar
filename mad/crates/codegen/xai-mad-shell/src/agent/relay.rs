...
//! connection to the mad.com relay server with automatic reconnection.
...
/// Config for the mad.com WebSocket relay. Fields are private so the only
...
impl RelayConfig {
    /// Session gate: builds only for a mad.com first-party session
    ...
    pub(crate) fn for_session(
        ...
        Some(Self {
            ws_url: ctx.mad_ws_url.clone(),
            ws_origin: ctx.mad_ws_origin.clone(),
...
///
/// The relay maintains a persistent WebSocket connection to mad.com with
...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!("Authentication required. Run `mad login` to re-authenticate.");
        cancel.cancel();
        return false;
    };
    ...
    loop {
        ...
        match connect_to_relay(&config, proxy_url.as_deref(), &cancel).await {
            Ok(ws) => {
                ...
                tprintln!("Disconnected from Mad WebSocket server");
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        ...
        axum::http::header::HeaderValue::from_static(xai_mad_version::VERSION),
    );
    req.headers_mut().insert(
...
mod tests {
    ...
    /// Helper to create a test MadAuth with the given key.
    fn test_auth(key: &str) -> MadAuth {
        MadAuth {
            key: key.to_string(),
    ...
    fn for_session_builds_only_for_xai_issuer() {
        ...
        let builds = |a: &MadAuth| RelayConfig::for_session(a, &cfg, None, None).is_some();
        let xai = MadAuth {
    ...
    /// Helper: write a MadAuth to disk under the given scope.
    fn write_test_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &MadAuth) {
