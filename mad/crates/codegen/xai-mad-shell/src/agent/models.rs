...
use xai_mad_sampling_types::{ReasoningEffort, ReasoningEffortOption};
...
impl Default for ModelsManager {
    fn default() -> Self {
        ...
        let auth_manager = Arc::new(AuthManager::new(&mad_home, MadComConfig::default()));
        Self::new(
            None,
            IndexMap::new(),
...
impl ModelsManager {
    ...
    /// Hot-reload the catalog from `~/.mad/models_cache.json` after an external write (config-watcher detected).
    ...
    async fn bounded_startup_auth(auth_manager: &Arc<AuthManager>) -> Option<MadAuth> {
    ...
    /// without a live IdP.
    async fn bounded_auth_refresh<F>(fut: F) -> Option<MadAuth>
    where
        F: std::future::Future<Output = Option<MadAuth>>,
    {
