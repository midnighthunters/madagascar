//! mad.com chat-product model catalog: caches `/rest/modes` and maps modes to
...
/// ~54 min, matching mad-web's refetch cadence.
...
pub const MAD_CHAT_MODE_ENV: &str = "MAD_CHAT_MODE";
...
impl ChatModesManager {
    ...
    /// The active mad.com identity, or `None` when unauthenticated. Modes are
...
/// Maps mad.com modes → `SessionModelState`: keeps only `available` modes,
...
mod tests {
    ...
    fn name_falls_back_to_id_when_title_blank() {
        ...
        let state = modes_to_model_state(&resp);
        assert_eq!(state.available_models[0].name, "mad-4.5");
    }
}
