...
) -> Option<xai_mad_sampling_types::ToolOverrides> {
...
fn insert_applied_tool_overrides(
    ...
    echo: Option<&xai_mad_sampling_types::ToolOverrides>,
) {
...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        xai_mad_telemetry::unified_log::info("agent initialized", None, None);
        ...
        {
            let root = crate::util::mad_home::mad_home();
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    xai_mad_workspace::permission::cleanup_stale_permission_state(
        ...
        xai_mad_workspace::trust::migrate_legacy_hook_trust();
        ...
        if !self.tier_allowed.get() && let Some(auth) = self.auth_manager.current() {
            self.enforce_mad_code_access(&auth).await;
        }
        ...
        if client_type == ClientType::Generic {
            match client_identifier.as_deref() {
                Some("mad-web") => client_type = ClientType::MadWeb,
                Some("nebula") => client_type = ClientType::Nebula,
                Some("mad-code-extension") => client_type = ClientType::Extension,
                Some("mad-desktop") => client_type = ClientType::Desktop,
                _ => {}
            }
        }
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        let (
            ...
            let cfg = self.cfg.borrow();
            let issuer = cfg.mad_com_config.oidc.as_ref().map(|o| o.issuer.clone());
            (
                cfg.mad_com_config.auth_provider_label.clone(),
                cfg.mad_com_config.auth_provider_command.is_some(),
                cfg.mad_com_config.oidc.is_some(),
                issuer,
            )
        };
        if has_enterprise_oidc {
            ...
            tracing::info!(
                label = ?login_label,
                has_auth_provider,
                "auth: advertising mad.com auth method",
            );
        }
        let preferred_method = self.cfg.borrow().mad_com_config.preferred_method;
        ...
        xai_mad_telemetry::unified_log::info(
            ...
            Some(
                serde_json::json!({
                "mad_home": crate::util::mad_home::mad_home().display().to_string(),
        ...
        Ok(
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    let metadata = parse_json_object_env("MAD_AGENT_METADATA");
                    serde_json::json!({
                    "madShell": true,
    ...
    async fn authenticate(
        ...
        if let Some(preferred) = self.cfg.borrow().mad_com_config.preferred_method {
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::XAI_API_KEY_METHOD_ID => {
                ...
                log_event(xai_mad_telemetry::events::Login {
            ...
            auth_method::CACHED_TOKEN_AUTH_METHOD_ID => {
                ...
                let pin_blocks_oidc_mint = matches!(
                    self.cfg.borrow().mad_com_config.preferred_method,
            ...
            auth_method::MAD_COM_METHOD_ID | auth_method::OIDC_METHOD_ID => {
                let mad_ctx = self.auth_manager.mad_com_config();
                let auth_meta = AuthRequestMeta::from_json(arguments.meta.as_ref());
                tracing::info!(
                ...
                let use_oidc = self.cfg.borrow().resolve_mad_oauth(cli_oauth);
                    ...
                    })?;
                {
                    ...
                    tracing::debug!("auth: mad.com/oidc handler set api_key (SessionToken)");
                    xai_mad_telemetry::unified_log::debug(
                        "auth: mad.com/oidc handler set api_key (SessionToken)",
                        None,
                        None,
                    );
                }
    ...
    async fn new_session(
        ...
        xai_mad_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::SessionStarted {
        ...
        if product_analytics || xai_mad_telemetry::external::is_active() {
            ...
            tokio::spawn(async move {
                let git = xai_mad_telemetry::context::collect_git_context(&cwd_str);
                let ev = xai_mad_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                xai_mad_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) = match xai_mad_workspace::session::git::discover_git_root(
            cwd.as_path(),
        ) {
    ...
    async fn load_session(
            ...
            Some(s) => {
                match s.state {
                    crate::session::plan_mode::PlanModeState::Inactive => {
                        xai_mad_telemetry::events::PlanModeState::Inactive
                    }
                    crate::session::plan_mode::PlanModeState::Pending => {
                        xai_mad_telemetry::events::PlanModeState::Pending
                    }
                    crate::session::plan_mode::PlanModeState::Active
                    | crate::session::plan_mode::PlanModeState::ExitPending => {
                        xai_mad_telemetry::events::PlanModeState::Active
                    }
                }
            }
            None => xai_mad_telemetry::events::PlanModeState::Inactive,
        };
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            xai_mad_workspace::session::git::warn_registry_disabled_restore(
                session_id.0.as_ref(),
            );
        }
        let restore_checkout_allowed = xai_mad_workspace::session::git::restore_code_checkout_allowed(
            cwd.as_path(),
        ...
        {
            tracing::warn!(
                target: xai_mad_workspace::session::git::RESTORE_CODE_LOG,
                session_id = %session_id.0,
                supplied_cwd = %cwd.as_str(),
                persisted_cwd = %summary.info.cwd,
                target_sha = %target_sha,
                "restore_code: skipping session HEAD checkout — supplied cwd is neither a mad worktree nor the session's persisted cwd (refusing to detach the source repo)"
            );
            xai_mad_telemetry::unified_log::warn(
        ...
        if restore_code_requested && restore_checkout_allowed
            && let Some(ref target_sha) = summary.head_commit
        {
            use xai_mad_workspace::session::git::RestoreKind;
            let outcome = xai_mad_workspace::session::git::checkout_session_commit(
                    cwd.as_path(),
                    target_sha,
                    true,
                    session_id.0.as_ref(),
        ...
        let preloaded_envrc = xai_mad_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
        ...
        let is_mad_build = persisted_model.0.starts_with("mad-build");
        let same_family_fallback = if is_mad_build {
            available.keys().find(|id| id.0.starts_with("mad-build")).cloned()
        } else {
            available.keys().find(|id| !id.0.starts_with("mad-build")).cloned()
        };
        ...
        if summary.head_commit.is_some() && let Some(ref cwd) = session_cwd
            && summary
                ...
                .as_deref()
                .is_none_or(|root| {
                    xai_mad_workspace::session::git::find_git_root_from_path(
                ...
                })
        {
            ...
            let current_head = xai_mad_workspace::session::git::git_cli(
                    cwd_path,
                    &["rev-parse", "HEAD"],
                )
                .await
                .ok();
            if let Some(divergence) = xai_mad_workspace::session::git::detect_head_divergence(
        ...
        if self.product_analytics_enabled() {
            log_event(xai_mad_telemetry::events::SessionLoad {
                session_id: session_id.0.to_string(),
    ...
    async fn prompt(
            ...
            .and_then(|m| m.get("toolOverrides"))
        {
            None => None,
            Some(value) => {
                match xai_mad_sampling_types::ToolOverridesUpdate::parse(value) {
            ...
            Ok(turn_ok) => {
                ...
                if let Some(ctx) = trace_context {
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = xai_mad_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = xai_mad_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "x.ai/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                xai_mad_telemetry::session_ctx::log_event(xai_mad_telemetry::events::NonGitDecisionEvent {
                    decision: params.decision,
                    session_id: params.session_id,
                    client_version: params.client_version,
                });
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_followup" {
            ...
            >(args.params.get()) {
                ...
                xai_mad_telemetry::session_ctx::log_event(xai_mad_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| xai_mad_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_apply" {
            ...
            >(args.params.get()) {
                ...
                xai_mad_telemetry::session_ctx::log_event(xai_mad_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_discard" {
            ...
            >(args.params.get()) {
                ...
                xai_mad_telemetry::session_ctx::log_event(xai_mad_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| xai_mad_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == xai_mad_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                xai_mad_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            xai_mad_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
