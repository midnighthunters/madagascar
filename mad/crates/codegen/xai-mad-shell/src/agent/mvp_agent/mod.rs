...
use xai_mad_telemetry::id::{agent_id, agent_instance_id};
use xai_mad_telemetry::session_ctx::log_event;
use xai_mad_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use xai_mad_workspace::permission::{ClientType, PermissionEvent};
...
use xai_mad_workspace::session::git::GitDiscoveryResult;
...
pub(crate) fn resolve_subscription_tier_for_telemetry(
    ...
    auth: Option<&crate::auth::MadAuth>,
) -> Option<String> {
...
/// `/user` already reports a newly qualifying tier (e.g. `SuperMadPro`). In
...
pub(crate) fn jwt_claim_matches_user_subscription_tier(
    ...
    match user_subscription_tier {
        "MadPro" => jwt_claim == "supermad",
        "XBasic" => jwt_claim == "x_basic",
        "XPremium" => jwt_claim == "x_premium",
        "XPremiumPlus" => jwt_claim == "x_premium_plus",
        "SuperMadPro" => jwt_claim == "supermad_heavy",
        "SuperMadLite" => jwt_claim == "supermad_lite",
        "SuperMadPlus" => jwt_claim == "supermad_plus",
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a xai_mad_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[xai_mad_announcements::RemoteAnnouncement]>,
    last_emitted: &[xai_mad_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<xai_mad_announcements::RemoteAnnouncement>> {
    let current = xai_mad_announcements::filter_expired_at(
...
/// Override with `MAD_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("MAD_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
pub struct MvpAgent {
    ...
    /// mad.com chat-product catalog (`/rest/modes`) for chat sessions; distinct
    ...
    /// list. Set by `enforce_mad_code_access`; defaults to `true` (API-key and
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.mad/`).
    ///
    ...
    /// Local session-registry override: `MAD_SESSION_REGISTRY` env, else
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        xai_mad_tools::implementations::mad_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: xai_mad_tools::implementations::mad_build::monitor::types::MonitorEventBuffer,
    ...
    pub(crate) plugin_registry_handle: xai_mad_agent::plugins::SharedPluginRegistryHandle,
    ...
    /// cwd→git root plus user/marketplace dirs and stalled mad-desktop's first
    ...
    workspace_ops: RefCell<Option<xai_mad_workspace::WorkspaceOps>>,
    ...
    last_emitted_announcements: RefCell<Vec<xai_mad_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &xai_mad_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<xai_mad_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !xai_mad_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = xai_mad_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
/// unchanged — compatible stock switches (e.g. `mad-build` →
/// `mad-build-plan`) intentionally preserve the session's original ACP
/// `agentProfile`.
...
/// default wire format and toolset, so switching e.g. `mad-build` →
/// `mad-build-plan` doesn't require rebuilding the harness and would
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use xai_mad_agent::config::is_strict_harness_agent_type;
    match (
...
///
/// Headers injected:
///  - `x-mad-client-version` -- required by the proxy's version-gate check.
...
fn inject_proxy_headers(
    ...
    headers
        .entry("x-mad-client-version".to_string())
        .or_insert_with(|| {
            client_version
                .map(String::from)
    ...
    headers
        .entry("x-mad-client-identifier".to_string())
...
impl MvpAgent {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = xai_mad_tools::types::TaskSnapshot {
    ...
    pub(super) async fn enforce_mad_code_access(&self, auth: &crate::auth::MadAuth) {
        if !auth.is_xai_auth() {
        ...
        *self.allow_access_resolved_for.borrow_mut() = Some(auth.user_id.clone());
        if !allow {
            tracing::info!(
                "auth: user blocked by allow_access (remote settings mad_build_access_gate)"
            );
    ...
    /// cached_token, mad.com/oidc); the cheap pre-checks below absorb repeats
    ...
    ///    against `~/.mad/bundled/` and the manifest.
