...
fn auth_with_mode(mode: crate::auth::AuthMode, key: &str) -> crate::auth::MadAuth {
    crate::auth::MadAuth {
        key: key.into(),
        auth_mode: mode,
...
fn jwt_claim_matches_user_subscription_tier_known_pairs() {
    let cases = [
        ...
        ("supermad_heavy", "SuperMadPro"),
        ("9", "EnterpriseMystery"),
        ("supermad_lite", "SuperMadLite"),
        ("supermad_plus", "SuperMadPlus"),
    ];
...
fn jwt_claim_matches_user_subscription_tier_rejects_stale_and_unknown() {
    ...
    assert!(!jwt_claim_matches_user_subscription_tier("free", "MadPro"));
...
fn harness_pair(id: &str) -> Vec<xai_mad_sampling_types::conversation::ConversationItem> {
    use xai_mad_sampling_types::ToolCall;
    use xai_mad_sampling_types::conversation::ConversationItem;
    vec![
...
#[test]
#[serial_test::serial]
fn resolve_agent_definition_defaults_to_mad_build() {
    let prev = std::env::var("MAD_AGENT").ok();
    unsafe {
        std::env::remove_var("MAD_AGENT");
    }
    ...
    if let Some(v) = prev {
        unsafe { std::env::set_var("MAD_AGENT", v) }
    }
}
/// When model_agent_type = Some("codex"), the codex agent is selected even
/// though the default chain would return mad-build.
#[test]
#[serial_test::serial]
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = xai_mad_agent::AgentDefinition::from_json(&serde_json::json!(
...
/// `mad-build-plan`, models in the catalog that still declare
/// `agent_type = "mad-build"` explicitly must NOT preempt an ACP
/// profile. Any value in the `mad-build*` family is the stock harness
/// with no strict requirement.
#[test]
#[serial_test::serial]
fn resolve_agent_definition_acp_profile_wins_for_explicit_mad_build_family() {
    let prev = std::env::var("MAD_AGENT").ok();
    unsafe {
        std::env::remove_var("MAD_AGENT");
    }
    ...
    .expect("agent definition must parse");
    for family_variant in ["mad-build", "mad-build-plan", "mad-build-concise"] {
        let def = MvpAgent::resolve_agent_definition(
            tmp.path(),
            None,
            &config::AgentSelectionConfig::default(),
        ...
        assert_eq!(
            def.name, "custom-devbox-profile",
            "ACP profile must win for mad-build family variant `{family_variant}`"
        );
    }
    if let Some(v) = prev {
        unsafe { std::env::set_var("MAD_AGENT", v) }
...
fn inherited_harness_template_skips_nonstrict_model() {
    use xai_mad_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let agents_dir = tmp.path().join(".mad").join("agents");
    ...
    assert_eq!(
        def.model,
        xai_mad_agent::config::ModelOverride::Override("test-model-123".to_string()),
        "agent profile model override must be preserved through resolution"
    );
    match prev {
        Some(v) => unsafe { std::env::set_var("MAD_AGENT", v) },
        None => unsafe { std::env::remove_var("MAD_AGENT") },
    }
}
#[test]
...
/// flow: a zero-turn switch from `mad-build` (a client profile name) to
/// `mad-build-plan` (the default model agent_type) must be
...
fn harnesses_are_compatible_for_stock_family_pairs() {
    assert!(harnesses_are_compatible("mad-build", "mad-build-plan"));
    assert!(harnesses_are_compatible("mad-build-plan", "mad-build"));
    assert!(harnesses_are_compatible("mad-build", "mad-build"));
...
fn harnesses_are_compatible_rejects_strict_mismatches() {
    ...
    assert!(!harnesses_are_compatible("mad-build-plan", "codex"));
}
#[test]
fn explicit_agent_type_wins_over_session_default() {
    assert_eq!(
        resolve_required_agent_type(Some("cursor"), "mad-build-plan"),
        "cursor"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_mad_build_plan() {
    assert_eq!(
        resolve_required_agent_type(None, "mad-build-plan"),
        "mad-build-plan"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_mad_build() {
    assert_eq!(
        resolve_required_agent_type(None, "mad-build"),
        "mad-build"
    );
}
#[test]
fn null_agent_type_returns_to_session_default_after_cursor_switch() {
    let session_default = "mad-build-plan";
    let required_after_null = resolve_required_agent_type(None, session_default);
    assert_eq!(required_after_null, "mad-build-plan");
...
fn agent_name_unchanged_without_harness_rebuild() {
    let unchanged = agent_name_after_model_switch(false, "mad-build-plan", "remote-sidebar");
    assert_eq!(
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use xai_mad_tools::computer::local::{LocalFs, LocalTerminalBackend};
    use xai_mad_tools::notification::ToolNotificationHandle;
    use xai_mad_tools::registry::types::SessionContext;
    let tmp = tempfile::tempdir().unwrap();
    ...
    let builder = xai_mad_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: xai_mad_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: xai_mad_tools::implementations::mad_build::image_gen::ImageGenConfig::default(),
        video_gen_config: xai_mad_tools::implementations::mad_build::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: xai_mad_tools::implementations::mad_build::deploy_app::AppBuilderDeployerConfig::default(),
        api_key_provider: None,
        auth_provider: None,
        attribution_callback: None,
        system_reminder_tag: xai_mad_tools::reminders::DEFAULT_REMINDER_TAG,
    };
    let toolset = builder
...
fn make_test_handle(
    ...
    crate::session::SessionHandle {
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            xai_mad_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(xai_mad_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
        ...
        permission_handle: xai_mad_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        agent_name: "mad-build".to_string(),
        managed_mcp_proxy_base_url: String::new(),
        session_default_agent_profile: None,
        allowed_subagent_types: None,
        hook_registry: None,
        workspace_ops: xai_mad_workspace::WorkspaceOps::for_test(),
...
async fn lookup_session_model_returns_per_session_model() {
    ...
    let sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (sid_a.clone(), make_test_handle("mad-3-fast", false, None)),
...
async fn lookup_session_model_fallback_no_session() {
    let default_model = acp::ModelId::new("mad-3");
...
async fn set_session_model_does_not_cross_contaminate() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (sid_a.clone(), make_test_handle("mad-3", false, None)),
        (sid_b.clone(), make_test_handle("mad-3", false, None)),
    ]
    .into();
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use xai_mad_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn session_config_options_resolves_routing_slug_to_catalog_model() {
    use crate::agent::config::{EndpointsConfig, ModelEntry};
    use xai_mad_sampling_types::ReasoningEffort;
    let agent = build_minimal_agent_for_tests();
...
async fn yolo_toggle_scoped_by_client_identifier() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("mad-3", false, Some("mad-tui")),
        ),
        (
            sid_vscode.clone(),
            make_test_handle("mad-3", false, Some("mad-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("mad-tui"), true);
...
async fn yolo_toggle_can_disable_session_started_with_yolo_enabled() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("mad-3", true, Some("mad-tui")),
        ),
        (
            sid_other.clone(),
            make_test_handle("mad-3", true, Some("mad-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("mad-tui"), false);
...
async fn test_per_session_code_nav_isolation() {
    let web_handle = {
        let mut h = make_test_handle("model", false, Some("mad-web"));
        h.code_nav_enabled = true;
        h
    };
    let tui_handle = {
        let mut h = make_test_handle("model", false, Some("mad-tui"));
    ...
    let check = |handle: &crate::session::SessionHandle| {
        ...
        if !matches!(ct, ClientType::MadWeb) {
...
async fn ext_method_routes_auth_cleared_and_refreshes_resident_sessions() {
    ...
    local
        .run_until(async {
            let agent = build_agent_with_auth(crate::auth::MadAuth {
                key: "eligible".into(),
                auth_mode: crate::auth::AuthMode::WebLogin,
                ..crate::auth::MadAuth::test_default()
            });
            use acp::Agent as _;
...
fn build_minimal_agent_for_tests() -> MvpAgent {
    ...
    use crate::auth::{AuthManager, MadComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(temp_dir.path(), MadComConfig::default()));
...
fn build_agent_with_auth(auth: crate::auth::MadAuth) -> MvpAgent {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, MadComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
    let auth_manager =
...
async fn ensure_plugin_registry_lazily_populates_snapshot() {
    ...
    let mad_home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("MAD_HOME", mad_home.path());
    ...
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(auth_home.path(), MadComConfig::default()));
    let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
    let gateway = GatewaySender::new(tx);
...
async fn resident_activity_reports_needs_input_when_pending() {
    ...
    let handle = make_test_handle("mad-3", false, None);
...
async fn push_roster_activity_delta_broadcasts_overridden_activity() {
    ...
    agent
        .sessions
        .borrow_mut()
        .insert(sid.clone(), make_test_handle("mad-3", false, None));
...
async fn test_web_session_with_capability_is_eligible() {
    ...
    let mut handle = make_test_handle("model", false, Some("mad-web"));
...
async fn test_tui_session_is_rejected() {
    ...
    let mut handle = make_test_handle("model", false, Some("mad-tui"));
    handle.code_nav_enabled = true;
    let sessions = [(sid.clone(), handle)].into();
    assert_eq!(
...
async fn test_leader_mode_two_sessions_stay_isolated() {
    ...
    let mut web_handle = make_test_handle("model", false, Some("mad-web"));
    web_handle.code_nav_enabled = true;
    let mut tui_handle = make_test_handle("model", false, Some("mad-tui"));
...
async fn test_unknown_session_id_returns_session_required() {
    ...
    let mut known_handle = make_test_handle("model", false, Some("mad-web"));
...
mod eligibility_gates {
    ...
    fn check_gates(
        ...
        if !matches!(client_type, ClientType::MadWeb) {
            return Err(CodeNavEligibility::ClientNotWeb);
        }
        if !code_nav_enabled {
            return Err(CodeNavEligibility::CapabilityNotAdvertised);
    ...
    fn tui_client_rejected() {
        assert_eq!(
            check_gates(ClientType::MadTUI, true, true),
            Err(CodeNavEligibility::ClientNotWeb)
        );
    }
    #[test]
    fn web_client_no_capability_rejected() {
        assert_eq!(
            check_gates(ClientType::MadWeb, false, true),
            Err(CodeNavEligibility::CapabilityNotAdvertised)
        );
    }
    #[test]
...
async fn auth_type_session_based_no_current_returns_session_token() {
    for method_id in [
        ...
        crate::agent::auth_method::MAD_COM_METHOD_ID,
...
async fn auth_type_session_based_with_current_returns_session_token() {
    use crate::auth::MadAuth;
    let agent = build_minimal_agent_for_tests();
    agent.set_auth_method(acp::AuthMethodId::new(
        crate::agent::auth_method::OIDC_METHOD_ID,
    ));
    agent.auth_manager.hot_swap(MadAuth::test_default());
...
/// Minimal agent whose `mad_com_config` engages the api-key kill switch
...
fn build_agent_with_api_key_auth_disabled() -> MvpAgent {
    ...
    let gateway = GatewaySender::new(tx);
    let mut cfg = AgentConfig::default();
    cfg.mad_com_config.disable_api_key_auth = Some(true);
    MvpAgent::new(gateway, &cfg, auth_manager, None).expect("valid test config")
}
...
async fn cached_token_fallthrough_prefers_api_key_for_deployment_key() {
    ...
    use xai_mad_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("MAD_DISABLE_API_KEY_AUTH");
...
/// switch keeps the fallthrough on interactive `mad.com` (api-key auth is
/// neither advertised nor an eligible fallthrough).
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_respects_kill_switch() {
    use crate::agent::auth_method::{MAD_COM_METHOD_ID, XAI_API_KEY_ENV_VAR};
    use xai_mad_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("MAD_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(XAI_API_KEY_ENV_VAR, "test-deployment-key");
    ...
    assert_eq!(
        agent
            ...
            .as_ref()
            .map(|id| id.0.as_ref()),
        Some(MAD_COM_METHOD_ID),
        "disable_api_key_auth must keep the cached_token fallthrough on \
         interactive mad.com so XAI_API_KEY can't bypass forced IdP login",
    );
}
/// No advertiseable credentials at all (no env key, no kill switch): the user
/// genuinely needs to log in, so the fallthrough is interactive `mad.com`.
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_falls_to_mad_com_without_credentials() {
    use crate::agent::auth_method::{
        MAD_COM_METHOD_ID, LEGACY_XAI_API_KEY_ENV_VAR, XAI_API_KEY_ENV_VAR,
    };
    use xai_mad_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("MAD_DISABLE_API_KEY_AUTH");
    ...
    assert_eq!(
        agent
            ...
            .map(|id| id.0.as_ref()),
        Some(MAD_COM_METHOD_ID),
        "no API-key creds and no kill switch -> interactive mad.com login",
    );
}
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use xai_mad_tools::implementations::mad_build::video_gen::{
...
async fn prepare_video_gen_config_respects_feature_flag() {
    use xai_mad_tools::implementations::mad_build::video_gen::VideoGenConfig;
    let agent = build_minimal_agent_for_tests();
    agent.sampling_config.borrow_mut().api_key = Some("test-key".to_string());
    assert!(matches!(
        agent.prepare_video_gen_config(),
...
async fn prepare_image_gen_config_fails_open_without_auth() {
    use xai_mad_tools::implementations::mad_build::image_gen::ImageGenConfig;
    let agent = build_minimal_agent_for_tests();
    agent.sampling_config.borrow_mut().api_key = Some("test-key".to_string());
...
/// Build traffic via the `x-mad-client-identifier` header. If this header is
...
async fn data_collection_enabled_for_normal_user() {
    let agent = build_agent_with_auth(crate::auth::MadAuth::test_default());
    assert!(
...
fn chat_initial_model_matrix() {
    let cases: &[(&str, bool, Option<&str>, Option<&str>)] = &[
        ("chat_with_model", true, Some("mad-4.5"), Some("mad-4.5")),
        ("chat_without_model", true, None, None),
        ("build_with_model", false, Some("mad-4.5"), None),
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = xai_mad_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = xai_mad_workspace::WorkspaceOps::for_test();
    let toolset =
        std::sync::Arc::new(xai_mad_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<xai_mad_workspace::permission::PermissionEvent>();
    agent
        .retained_resources
        .borrow_mut()
        .entry(sid.clone())
...
fn ext_method_rewind_uses_local_dispatch_without_bridge() {
    use acp::Agent as _;
    let _env = crate::env::EnvVarGuard::remove(crate::env::MAD_DISABLE_CUSTOM_BRIDGE_ENV);
...
/// `test_cancel_ends_in_flight_turn_and_frees_slot` (mad-agent-sdk).
#[test]
...
fn make_live_session_handle(
    ...
    let mut handle = make_test_handle("test-model", false, Some("mad-tui"));
...
async fn storage_mode_self_corrects_to_writeback_when_settings_arrive() {
    let _env = crate::env::EnvVarGuard::remove("MAD_STORAGE_MODE");
    let auth = crate::auth::MadAuth {
...
fn post_auth_settings_not_coalesced_by_in_flight_reapply() {
    run_local_for_bridge_test(|| async {
        ...
        agent.spawn_settings_reapply();
        assert!(agent.settings_reapply_in_flight.get());
        agent.spawn_post_auth_settings(crate::auth::MadAuth::test_default());
        assert_eq!(
...
impl Drop for RestoreOtelGate {
    fn drop(&mut self) {
        xai_mad_telemetry::external::mark_external_otel_settings_resolved();
    }
}
...
async fn access_gate_does_not_leak_verdict_across_identities() {
    ...
    use crate::auth::{MadAuth, XAI_OAUTH2_ISSUER};
    let auth_a = MadAuth {
        oidc_issuer: Some(XAI_OAUTH2_ISSUER.to_string()),
        user_id: "user-a".into(),
        ..MadAuth::test_default()
    };
    ...
    *agent.allow_access_resolved_for.borrow_mut() = Some("user-a".to_string());
    ...
    agent.enforce_mad_code_access(&auth_b).await;
    assert!(
...
async fn post_auth_settings_xai_upgrades_writeback_emits_and_opens_gate() {
    ...
    let _storage_env = crate::env::EnvVarGuard::remove("MAD_STORAGE_MODE");
    let server = xai_mad_test_support::MockInferenceServer::start()
        .await
        .unwrap();
    ...
    xai_mad_telemetry::external::suppress_external_otel_until_settings();
    assert!(!xai_mad_telemetry::external::is_settings_gate_open());
    ...
    assert!(
        xai_mad_telemetry::external::is_settings_gate_open(),
...
async fn post_auth_settings_non_xai_keeps_local_but_still_emits() {
    use crate::agent::config::AgentMode;
    use crate::auth::{AuthMode, MadAuth};
    let _restore = RestoreOtelGate;
    let server = xai_mad_test_support::MockInferenceServer::start()
        .await
        .unwrap();
...
async fn settings_self_heal_refetches_after_token_rotation() {
    ...
    use crate::auth::{MadAuth, XAI_OAUTH2_ISSUER};
    let _restore = RestoreOtelGate;
    let server = xai_mad_test_support::MockInferenceServer::start_with_required_auth(
        vec![xai_mad_test_support::MockModelEntry::new("mad-build")],
        "rotated-key",
    )
    .await
    .unwrap();
    ...
    impl TokenRefresher for RotatingRefresher {
        async fn refresh(&self, _r: crate::auth::manager::RefreshReason) -> RefreshOutcome {
            RefreshOutcome::Success(Box::new(MadAuth {
...
fn write_project_subagent_definitions(cwd: &std::path::Path) {
    let roles = cwd.join(".mad/roles");
    let personas = cwd.join(".mad/personas");
    ...
    run_local_for_bridge_test(|| async {
        ...
        agent.sessions.borrow_mut().insert(sid.clone(), handle);
        {
            ...
            cfg.subagent_roles.insert(
                "refreshed".into(),
                xai_mad_subagent_resolution::config::SubagentRole {
...
/// End-to-end gate wiring: project `.mad/roles` / `personas` alone must drive
...
fn project_roles_personas_gated_via_resolve_and_record_chain() {
    ...
    let _env = EnvGuard::set("MAD_HOME", home.path());
    let _sim = EnvGuard::set(xai_mad_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("MAD_FOLDER_TRUST");
    let repo = tempfile::tempdir().unwrap();
    git2::Repository::init(repo.path()).unwrap();
    write_project_subagent_definitions(repo.path());
    run_local_for_bridge_test(|| async {
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use xai_mad_test_support::EnvGuard;
    use xai_mad_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("MAD_HOME", home.path());
    let _sim = EnvGuard::set(xai_mad_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("MAD_FOLDER_TRUST");
...
fn ann(id: &str) -> xai_mad_announcements::RemoteAnnouncement {
    xai_mad_announcements::RemoteAnnouncement {
...
/// fields as needed).
fn settings_with(
    announcements: Option<Vec<xai_mad_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = xai_mad_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = xai_mad_announcements::RemoteAnnouncement {
