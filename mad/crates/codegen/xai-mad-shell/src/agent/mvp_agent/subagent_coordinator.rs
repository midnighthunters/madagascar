...
//! The shared coordinator actor lives in `xai-mad-tools`; this module plugs
...
impl xai_mad_tools::implementations::mad_build::task::coordinator::ChildRunner
    for ShellChildRunner
{
    ...
    type RunFuture = xai_mad_tools::implementations::mad_build::task::coordinator::LocalBoxFuture<
        xai_mad_tools::implementations::mad_build::task::coordinator::ChildRunOutput<
            Self::CompletionData,
        >,
    >;
    type ValidateFuture =
        xai_mad_tools::implementations::mad_build::task::coordinator::LocalBoxFuture<
            xai_mad_tools::implementations::mad_build::task::types::SubagentValidateTypeOutcome,
        >;
    type DescribeFuture =
        xai_mad_tools::implementations::mad_build::task::coordinator::LocalBoxFuture<
            xai_mad_tools::implementations::mad_build::task::types::SubagentDescribeOutcome,
        >;
    fn run(
        &self,
        run: xai_mad_tools::implementations::mad_build::task::coordinator::ChildRunRequest<
            Self::Control,
        >,
    ) -> Self::RunFuture {
        let agent_ref = self.agent_ref.clone();
        Box::pin(async move {
            let xai_mad_tools::implementations::mad_build::task::coordinator::ChildRunRequest {
                request,
                cancellation,
                reporter,
            } = run;
            ...
            let Some(mut ctx) = this.try_build_subagent_spawn_context(&parent_sid) else {
                ...
                return xai_mad_tools::implementations::mad_build::task::coordinator::ChildRunOutput {
                    result: xai_mad_tools::implementations::mad_build::task::types::SubagentResult {
                        success: false,
                        error: Some(
        ...
        Box::pin(async move {
            ...
            match this.try_build_subagent_spawn_context(&parent_session_id) {
                ...
                None => {
                    ...
                    xai_mad_tools::implementations::mad_build::task::types::SubagentDescribeOutcome::Unavailable
                }
            }
        })
    }
    fn on_completed(
        &self,
        completion: xai_mad_tools::implementations::mad_build::task::coordinator::ChildCompletion<
            Self::CompletionData,
        >,
    ) {
...
impl MvpAgent {
    ...
    /// [`SubagentCoordinator`](xai_mad_tools::implementations::mad_build::task::coordinator::SubagentCoordinator)
    ...
    pub(super) fn start_subagent_coordinator(&self) {
        ...
        let config =
            xai_mad_tools::implementations::mad_build::task::coordinator::CoordinatorConfig {
                foreground_budget:
                    xai_mad_tools::implementations::mad_build::task::backend::env_duration_or(
                        "MAD_SUBAGENT_AWAIT_BUDGET_MS",
        ...
        tokio::task::spawn_local(
            xai_mad_tools::implementations::mad_build::task::coordinator::SubagentCoordinator::new(
                    rx,
                    runner,
                    config,
                )
                ...
                ps.map(|h| h.tool_context.fs.inner().clone())
                    .unwrap_or_else(|| {
                        let cwd = ps
                            ...
                            .unwrap_or_default();
                        std::sync::Arc::new(xai_mad_workspace::file_system::LocalFs::new(cwd))
                    }),
        ...
        Some(crate::agent::subagent::SubagentSpawnContext {
            ...
            task_output_tool_name: {
                ...
                sessions
                    ...
                    .unwrap_or_else(|| {
                        xai_mad_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL
                            .to_string()
                    })
            },
