...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    use xai_mad_workspace::permission::types::{
    ...
    local
        .run_until(async {
            ...
            let cwd = xai_mad_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                xai_mad_workspace::permission::spawn_permission_manager(
                    sid.clone(),
                    gateway,
                    cwd,
                    xai_mad_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                xai_mad_workspace::permission::AccessKind::Read(Some(".env".into())),
                xai_mad_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        xai_mad_workspace::permission::Decision::PolicyDeny(_)
                    ),
...
async fn subagent_spawn_context_inherits_parent_configured_cutoff() {
    ...
    let cutoff = xai_mad_sampling_types::ToolOverrides {
        x_search: Some(xai_mad_sampling_types::XSearchOptions {
            date_bound: Some(
                xai_mad_sampling_types::SearchDateBound::new(None, Some("2020-01-01".to_string()))
                    .unwrap(),
            ),
        }),
        web_search: None,
