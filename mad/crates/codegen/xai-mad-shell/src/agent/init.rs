...
fn ensure_remote_settings_side_effects(cfg: &mut AgentConfig, sync_managed: bool) {
    ...
    if cfg.remote_settings.is_none() {
        let handle = if sync_managed {
            crate::agent::models::start_early_prefetch(Some(cfg.mad_com_config.clone()))
        } else {
            crate::agent::models::start_early_prefetch_settings_only(Some(
                cfg.mad_com_config.clone(),
            ))
        };
...
fn resolve_config(cfg: &AgentConfig, auth_manager: &AuthManager) -> AgentConfig {
    ...
    // env var > remote settings > Local. Skip remote settings for Generic (mad -p, subagents).
    ...
    // A CLI/env-set Writeback still requires mad.com auth.
    if cfg.storage_mode == StorageMode::Writeback && !has_xai_auth {
        tracing::info!("Writeback is disabled: requires auth with mad.com");
...
fn init_process(cfg: &AgentConfig, auth_manager: &AuthManager) {
    ...
    INIT.call_once(|| {
        ...
        crate::builtin::extract_builtin_files(&mad_home);

        crate::extensions::marketplace::purge_default_skills_installs(&mad_home);
        ...
        if cfg.resolve_official_marketplace_auto_register().value {
            crate::extensions::marketplace::ensure_official_marketplace_source(&mad_home);
        }
...
pub fn update_telemetry_config(config: &AgentConfig, auth_manager: &AuthManager) {
    let mad_auth = auth_manager.current().filter(|a| a.is_xai_auth());
    let user_id = mad_auth.as_ref().map(|a| a.user_id.clone());
    let team_id = mad_auth.as_ref().and_then(|a| a.team_id.clone());
