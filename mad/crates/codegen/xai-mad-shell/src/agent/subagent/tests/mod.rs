...
use xai_mad_subagent_resolution::resolve_effective_overrides;
...
async fn subagent_inherits_session_cli_overrides() {
    use xai_mad_agent::config::{AgentDefinition, PermissionMode};
...
fn subagent_bypass_permission_mode_gated_by_policy_pin() {
    use xai_mad_agent::config::PermissionMode;
    const PIN: &str = xai_mad_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    assert_eq!(
...
fn resume_worktree_action_covers_three_outcomes() {
    ...
    assert_eq!(
            resume_worktree_action(true, Some("refs/mad/subagents/x")),
            ResumeWorktreeAction::Rehydrate
        );
    assert_eq!(
            resume_worktree_action(false, Some("refs/mad/subagents/x")),
            ResumeWorktreeAction::Rehydrate
        );
    assert_eq!(
...
fn subagent_inherits_parent_lsp_via_context() {
    let parent: std::sync::Arc<dyn xai_mad_tools::implementations::lsp::LspBackend> = Arc::new(
        DummyLspDispatch,
    );
    let mut ctx = ctx_with_toggle(HashMap::new());
...
fn inject_subagent_completed_prompt_sends_prompt_and_marks_delivered() {
    ...
    let reservations = xai_mad_tools::reminders::task_completion::TaskCompletionReservations::default();
...
fn explicit_override_takes_precedence_over_role() {
    ...
    let role = xai_mad_subagent_resolution::config::SubagentRole {
...
fn persona_resolved_from_config() {
    ...
    personas
        .insert(
            ...
            xai_mad_subagent_resolution::config::SubagentPersona {
...
fn model_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_model = xai_mad_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
...
fn reasoning_effort_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_re = xai_mad_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
    let r = resolve_effective_overrides(
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use xai_mad_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
            ConversationItem::system("parent system"),
...
async fn bootstrap_fork_live_parent_chat_state_is_forked_with_marker() {
    ...
    let req = bootstrap_test_request(true);
    let mut ctx = ctx_with_toggle(HashMap::new());
    let chat = spawn_test_parent_chat_state("mad-4.5");
...
fn summarize_tool_config_uses_name_override_and_strips_namespace() {
    ...
    let mut read = ToolConfig::from_id("MadagascarBuild:read_file");
...
/// Regression: on the DEFAULT mad-build host —
...
fn subagent_keeps_default_flavor_when_parent_model_is_non_strict() {
    ...
    ctx.parent_model_agent_type = Some(
        BuiltinAgentName::MadagascarBuildPlan.as_ref().to_string(),
    );
...
fn subagent_auth_type_rule() {
    ...
    let byok = byok_model_entry("mad-byok");
    let plain = test_model_entry("mad-plain");
    assert_eq!(
...
fn fresh_tool_model_accepts_visible_key_and_internal_id() {
    ...
    models.insert("mad-3".to_string(), test_model_entry("mad-3-2025-02-15"));
    assert!(
...
fn fresh_tool_model_rejects_unknown_and_nonavailable_entries() {
    ...
    for requested in [
        ...
        assert!(!error.contains("mad models"));
    }
    assert!(
            super::handle_request::task_model_override_error(
                Some("oauth-only"),
...
fn test_sampling_config(model_slug: &str) -> xai_mad_sampling_types::SamplingConfig {
