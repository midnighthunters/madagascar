...
use xai_mad_tools::implementations::mad_build::task::backend::ChannelBackend;
#[test]
fn normalize_forked_context_strips_project_layout() {
    ...
    let (conv, _) = xai_mad_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn normalize_forked_context_consecutive_users() {
    ...
    let (conv, prefix_len) = xai_mad_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn end_to_end_normalized_conversation_shape() {
    ...
    let (mut conv, prefix_len) = xai_mad_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
    assert_eq!(prefix_len, 2);
...
fn last_user_message_is_task_after_normalization() {
    ...
    let (mut conv, _) = xai_mad_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
        ...
        .find_map(|item| {
            if let ConversationItem::User(u) = item {
                let text: String = u
                    ...
                    .iter()
                    .filter_map(|p| match p {
                        xai_mad_sampling_types::conversation::ContentPart::Text {
                            text,
                        } => Some(text.as_ref()),
                        _ => None,
                    })
            ...
            if let ConversationItem::User(u) = i {
                u.content
                    ...
                    .any(|p| {
                        matches!(
                    p,
                    xai_mad_sampling_types::conversation::ContentPart::Text { text } if text.contains("<background_context>")
                )
                    })
            } else {
                false
...
fn snapshot_ref_field_in_meta_roundtrips() {
    let meta = SubagentMeta {
        ...
        resumed_from: None,
        child_cwd: None,
        worktree_path: Some("/tmp/mad-wt/sa-snap".into()),
        snapshot_ref: Some("refs/mad/subagent-snapshots/sa-snap".into()),
        effective_model_id: None,
    };
    let json = serde_json::to_string(&meta).unwrap();
    assert!(json.contains("snapshot_ref"));
    assert!(json.contains("refs/mad/subagent-snapshots/sa-snap"));
    let parsed: SubagentMeta = serde_json::from_str(&json).unwrap();
    assert_eq!(
            parsed.snapshot_ref.as_deref(),
            Some("refs/mad/subagent-snapshots/sa-snap")
        );
}
#[test]
...
fn snapshot_test_meta(id: &str) -> SubagentMeta {
    SubagentMeta {
        ...
        resumed_from: None,
        child_cwd: None,
        worktree_path: Some("/tmp/mad-wt/subagent-x".into()),
        snapshot_ref: None,
        effective_model_id: None,
    }
}
...
fn snapshot_ref_write_promotes_nonterminal_status_to_terminal() {
    ...
    assert!(update_subagent_meta_snapshot_ref(
            dir.path(),
            "refs/mad/subagents/x",
            "completed"
        ));
    let data = std::fs::read_to_string(dir.path().join("meta.json")).unwrap();
    let reread: SubagentMeta = serde_json::from_str(&data).unwrap();
    assert_eq!(
            Some("refs/mad/subagents/x"),
            reread.snapshot_ref.as_deref()
        );
...
async fn completion_snapshot_sequence_persists_ref_then_removes_worktree() {
    ...
    let ref_name = "refs/mad/subagents/glue-1";
...
fn subagent_session_metadata_roundtrip() {
    ...
    assert_eq!(session_meta.model_id.as_deref(), Some("mad-4.5"));
...
fn upload_lifecycle_spawn_then_completion_preserves_fields() {
    ...
    assert_eq!(spawn_gcs.model_id.as_deref(), Some("mad-4.5"));
    ...
    assert_eq!(completion_gcs.model_id.as_deref(), Some("mad-4.5"));
...
fn resume_source_worktree_reuse() {
    let source_with_worktree = ResumeSourceData {
        ...
        worktree_path: Some(
            PathBuf::from("/home/user/.mad/worktrees/myrepo/subagent-sub-wt"),
        ),
        snapshot_ref: None,
        subagent_type: "general-purpose".into(),
        persona: None,
    ...
    assert_eq!(
            worktree.as_deref(),
            Some(Path::new(
                "/home/user/.mad/worktrees/myrepo/subagent-sub-wt",
            )),
...
fn resume_inherited_cwd_requires_existing_non_worktree_dir() {
    ...
    let missing = ResumeSourceData {
        child_cwd: "/no/such/dir/mad-missing".into(),
        ..present.clone()
    };
...
fn select_override_cwd_resume_never_falls_through_to_request_cwd() {
    let source = ResumeSourceData {
        ...
        worktree_path: Some(
            PathBuf::from("/home/user/.mad/worktrees/repo/subagent-sub-wt"),
        ),
        snapshot_ref: None,
        subagent_type: "general-purpose".into(),
        persona: None,
...
fn token_estimation_accounts_for_images() {
    use xai_mad_sampling_types::conversation::{ContentPart, ConversationItem, UserItem};
...
fn durable_fallback_roundtrips_child_cwd_and_worktree() {
    ...
    let meta = SubagentMeta {
        ...
        worktree_path: Some("/tmp/mad-wt/sa-dur".into()),
        snapshot_ref: None,
        effective_model_id: Some("mad-3".into()),
    };
    ...
    assert_eq!(loaded.worktree_path.as_deref(), Some("/tmp/mad-wt/sa-dur"));
...
fn resume_identity_does_not_gate_on_model() {
    ...
    assert!(
            xai_mad_subagent_resolution::validate_resume_identity(
                "general-purpose",
                None,
                &source,
            )
...
async fn read_parent_sampling_config_keeps_auto_catalog_id_with_routing_slug() {
    ...
    models.insert("auto".to_string(), test_model_entry("mad-4.5"));
    let ctx = ctx_with_parent_chat_state("auto", "mad-4.5", "composer-2-fast", models);
    let (config, model_id) = read_parent_sampling_config(&ctx).await;
    assert_eq!(config.model, "mad-4.5");
    assert_eq!(model_id.0.as_ref(), "auto");
}
#[tokio::test]
async fn read_parent_sampling_config_keeps_auto_when_catalog_has_slug_key_only() {
    let mut models = indexmap::IndexMap::new();
    models.insert("mad-4.5".to_string(), test_model_entry("mad-4.5"));
    let ctx = ctx_with_parent_chat_state("auto", "mad-4.5", "auto", models);
    let (config, model_id) = read_parent_sampling_config(&ctx).await;
    assert_eq!(config.model, "mad-4.5");
    assert_eq!(model_id.0.as_ref(), "auto");
}
...
async fn read_parent_sampling_config_resolves_backend_search_from_catalog() {
    let mut entry = test_model_entry("mad-4.5");
    entry.info.supports_backend_search = true;
    let mut models = indexmap::IndexMap::new();
    models.insert("auto".to_string(), entry);
    let mut ctx = ctx_with_parent_chat_state("auto", "mad-4.5", "auto", models);
...
async fn runtime_override_wins_over_subagents_models_pin_in_precedence_path() {
    use xai_mad_agent::config::ModelOverride;
    let build_ctx = || {
        let mut models = indexmap::IndexMap::new();
...
async fn resolve_subagent_inherits_parent_model_without_pins() {
    use xai_mad_agent::config::ModelOverride;
    for parent_model in ["mad-4.5", "composer-2-fast", "my-custom-byok-model"] {
        let mut ctx = ctx_with_toggle(HashMap::new());
...
/// (`mad-4.5`) and a custom parent (`composer-2-fast`)
...
async fn resolve_subagent_config_override_pin_applies_for_any_parent() {
    use xai_mad_agent::config::ModelOverride;
    for parent_model in ["mad-4.5", "composer-2-fast"] {
        let mut ctx = ctx_with_toggle(HashMap::new());
        ctx.sampling_config.model = parent_model.to_string();
        ctx.model_id = acp::ModelId::new(parent_model);
        ctx.available_models
...
async fn resolve_subagent_agent_definition_pin_applies_for_light_parent() {
    use xai_mad_agent::config::ModelOverride;
    let mut ctx = ctx_with_toggle(HashMap::new());
    ctx.sampling_config.model = "mad-4.5".to_string();
    ctx.model_id = acp::ModelId::new("mad-4.5");
    ctx.available_models
        .insert("pinned-model".to_string(), test_model_entry("pinned-model"));
...
async fn resolve_subagent_config_override_unknown_model_falls_through_to_inherit() {
    ...
    let (config, model_id) = resolve_subagent_sampling_config(
        ...
        .await;
    assert_eq!(config.model, "mad-4.5");
    assert_eq!(model_id.0.as_ref(), "mad-4.5");
}
...
async fn subagent_override_provider_model_spawns_cache_only_credentials() {
    ...
    ctx.model_id = acp::ModelId::new("mad-4.5");
    ctx.available_models = models;
    ctx.auth = Some(crate::auth::MadAuth {
...
fn non_cursor_persona_injected_as_system_reminder() {
    use xai_mad_sampling_types::conversation::{ConversationItem, SyntheticReason};
...
fn filter_inheritance_all_passes_everything_through() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_mad_agent::config::McpInheritance::All,
    );
...
fn filter_inheritance_none_returns_none() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_mad_agent::config::McpInheritance::None,
    );
...
fn filter_inheritance_named_selects_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_mad_agent::config::McpInheritance::Named(
...
fn filter_inheritance_except_excludes_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_mad_agent::config::McpInheritance::Except(
...
fn filter_inheritance_named_empty_list_gives_empty_pool() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_mad_agent::config::McpInheritance::Named(vec![]),
    );
...
fn filter_inheritance_except_empty_list_keeps_all() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_mad_agent::config::McpInheritance::Except(vec![]),
    );
    let result = result.expect("Except([]) should return Some");
...
fn filter_inheritance_except_nonexistent_servers_ignored() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_mad_agent::config::McpInheritance::Except(vec!["nonexistent".into()]),
    );
    let result = result.expect("Except should return Some");
    assert_eq!(pool_names(&result), vec!["github", "linear"]);
}
...
fn filter_inheritance_named_all_nonexistent_gives_empty() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_mad_agent::config::McpInheritance::Named(vec!["foo".into(), "bar".into()]),
    );
    let result = result.expect("Named should return Some");
    assert_eq!(result.server_names().count(), 0);
}
...
fn resolve_inherited_pool_named_filters() {
    ...
    let result = super::resolve_inherited_mcp_pool(
            Some(pool),
            &xai_mad_agent::config::McpInheritance::Named(vec!["atlassian".into()]),
        )
...
fn make_test_skill(
    ...
    xai_mad_tools::implementations::skills::types::SkillInfo {
        ...
        scope: xai_mad_tools::implementations::skills::types::SkillScope::Local,
        enabled: true,
        user_invocable: true,
