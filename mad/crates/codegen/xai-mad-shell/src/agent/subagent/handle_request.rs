...
use xai_mad_tools::implementations::{mad_build, opencode};
...
/// `xai-mad-tools` coordinator actor and reached only through `reporter`.
#[tracing::instrument(
...
pub(crate) async fn run_shell_child(
    ...
    let mut effective_runtime = xai_mad_subagent_resolution::resolve_runtime_config(
        &request.subagent_type,
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = xai_mad_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    effective_runtime.capability_mode = xai_mad_subagent_resolution::intersect_capability_modes(
    ...
    xai_mad_subagent_resolution::apply_child_tool_policy(
        &mut definition,
        effective_runtime.capability_mode,
    ...
    {
        let reminder = xai_mad_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let child_cwd_abs = xai_mad_paths::AbsPathBuf::new(child_cwd).unwrap_or_else(|_| {
        xai_mad_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let attribution_callback: Option<xai_mad_sampler::SharedAttributionCallback> =
    ...
    let yolo_policy_block = xai_mad_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
        let memory_tools: Vec<xai_mad_tools::registry::types::ToolConfig> = vec![
            ...
            && let Ok(content) = std::fs::read_to_string(&memory_md)
        {
            ...
            let truncated: String = content
                ...
                .join("\n");
            let truncated =
                xai_mad_tools::util::truncate::truncate_str(&truncated, MAX_BYTES).to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == xai_mad_hooks::event::HookEventName::Stop {
                            s.event = xai_mad_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if !agent_owned_mcp_servers_allowed(is_plugin_agent) {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    xai_mad_agent::config::McpServerRef::Named(name) => {
                    ...
                    xai_mad_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            xai_mad_agent::prompt::skills::list_skills_with_plugins(
                Some(&parent_cwd_str),
    ...
    xai_mad_telemetry::session_ctx::log_event(xai_mad_telemetry::events::SubagentLaunched {
    ...
    let spawn_result = session::spawn_session_on_thread(
        ...
        xai_mad_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
        ...
        std::sync::Arc::new(parking_lot::Mutex::new(
            xai_mad_workspace::file_system::CodebaseIndexManager::new(),
        )),
        false,
        subagent_fs_watch,
        None,
        ...
        if inherit_skills {
            ...
            xai_mad_agent::prompt::skills::SkillsConfig::default()
        },
        if inherit_skills {
        ...
        ctx.yolo_mode
            || matches!(
                ...
                xai_mad_agent::config::PermissionMode::BypassPermissions
            ),
        false,
        None,
        ...
        Vec::new(),
        xai_mad_agent::prompt::context::PromptAudience::Subagent,
    ...
    let outcome = if result.success {
        xai_mad_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        xai_mad_telemetry::events::Outcome::Cancelled
    } else {
        xai_mad_telemetry::events::Outcome::Error
    };
    xai_mad_telemetry::session_ctx::log_event(xai_mad_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        outcome,
    ...
    if let Some(ref wt_path) = worktree_path {
        if snapshot_dispose_enabled {
            let ref_name = format!("refs/mad/subagents/{}", request.id);
            let source_repo = resolve_subagent_source_repo(&ctx);
