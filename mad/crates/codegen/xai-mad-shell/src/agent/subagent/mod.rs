...
//! `xai-mad-tools` coordinator actor. This module keeps shell-specific
...
use xai_mad_agent::config::{McpInheritance, ModelOverride, PermissionMode};
use xai_mad_sampling_types::conversation::ConversationItem;
use xai_mad_subagent_resolution::ResumeSourceData;
use xai_mad_tools::implementations::mad_build::monitor::types::MonitorEventBuffer;
use xai_mad_tools::implementations::mad_build::task::coordinator::{
    ChildCompletion, ChildControl, ChildReporter, ChildRunOutput, LocalBoxFuture, StartedChild,
    SubagentProgress,
};
use xai_mad_tools::implementations::mad_build::task::types::*;
use xai_mad_tools::types::tool::ToolKind;
use xai_mad_workspace::file_system::AsyncFileSystem;
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub lsp: Option<std::sync::Arc<dyn xai_mad_tools::implementations::lsp::LspBackend>>,
    ...
    /// (not an intermediate parent's) because xai-mad-tools task/coordinator.rs
    ...
    pub sampling_config: xai_mad_sampler::SamplerConfig,
    ...
    pub auth: Option<crate::auth::MadAuth>,
    pub parent_cwd: PathBuf,
    pub parent_session_id: String,
    /// The parent's cutoff at spawn, applied to the child's first turn. `None` if unset.
    pub inherited_tool_overrides: Option<xai_mad_sampling_types::ToolOverrides>,
    pub yolo_mode: bool,
    ...
    pub parent_terminal_backend: Option<Arc<dyn xai_mad_tools::computer::types::TerminalBackend>>,
    ...
    pub parent_notification_handle:
        Option<xai_mad_tools::notification::types::ToolNotificationHandle>,
    ...
    pub web_search_sampling_config: Option<xai_mad_sampler::SamplerConfig>,
    /// Resolved config for web fetch.
    pub web_fetch_config: xai_mad_tools::implementations::mad_build::web_fetch::WebFetchConfig,
    /// Image generation config (parent-inherited).
    pub image_gen_config: xai_mad_tools::implementations::mad_build::image_gen::ImageGenConfig,
    /// Resolved config for video generation.
    pub video_gen_config: xai_mad_tools::implementations::mad_build::video_gen::VideoGenConfig,
    ...
    pub plugin_registry: Option<std::sync::Arc<xai_mad_agent::plugins::PluginRegistry>>,
    ...
    pub file_tool_overrides: Option<Vec<xai_mad_tools::registry::types::ToolConfig>>,
    ...
    pub hook_registry: Option<std::sync::Arc<xai_mad_hooks::discovery::HookRegistry>>,
    pub permission_handle: Option<xai_mad_workspace::permission::PermissionHandle>,
    pub worktree_type: crate::util::config::WorktreeType,
    pub api_key_provider: Option<xai_mad_tools::types::SharedApiKeyProvider>,
    pub image_description_model: String,
    /// Dual-mode workspace operations handle.
    pub workspace_ops: xai_mad_workspace::WorkspaceOps,
    ...
    pub attribution_callback: Option<xai_mad_sampler::SharedAttributionCallback>,
    /// Parent session's agent name (e.g. "mad-build").
    ...
    pub parent_tool_definitions: Option<Vec<xai_mad_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<xai_mad_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: xai_mad_agent::prompt::skills::SkillsConfig,
    ...
    pub parent_compat: xai_mad_tools::types::compat::CompatConfig,
    /// Shared completion reservations held by auto-wake prompts.
    pub task_completion_reservations:
        Option<xai_mad_tools::reminders::task_completion::TaskCompletionReservations>,
...
impl SubagentSpawnContext {
    ...
    fn apply_session_cli_overrides(&self, def: &mut xai_mad_agent::config::AgentDefinition) {
    ...
    pub fn resolve_compaction_verbatim_input(&self) -> bool {
        crate::agent::config::BoolFlag::env("MAD_COMPACTION_VERBATIM_INPUT")
            .config(
                self.agent_config
                    .as_ref()
    ...
    pub fn resolve_subagent_worktree_snapshot_enabled(&self) -> bool {
        crate::agent::config::BoolFlag::env("MAD_SUBAGENT_WORKTREE_SNAPSHOT")
            .config(
                self.agent_config
                    .as_ref()
...
async fn resolve_subagent_sampling_config(
    agent_name: &str,
    agent_model: &xai_mad_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (xai_mad_sampler::SamplerConfig, acp::ModelId) {
...
async fn resolve_effective_model_config(
    ...
    subagent_type: &str,
    definition_model: &xai_mad_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (xai_mad_sampler::SamplerConfig, acp::ModelId) {
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &xai_mad_sampler::SamplerConfig,
    resolved_id: &acp::ModelId,
    parent: &xai_mad_sampler::SamplerConfig,
) {
    ...
    if let Some(ref chat_state) = ctx.parent_chat_state {
        if let Some(cfg) = chat_state.get_sampling_config().await {
            ...
            let auth_scheme = crate::agent::config::try_resolve_model_credentials(&cfg.model, None)
                .map(|r| r.auth_scheme)
                .unwrap_or_default();
            let inherited = xai_mad_sampler::SamplerConfig {
...
fn resolve_model_override_to_config(
    model_id: &str,
    ctx: &SubagentSpawnContext,
) -> Option<(xai_mad_sampler::SamplerConfig, acp::ModelId)> {
...
pub(crate) fn resume_inherited_prefix_len(
    conversation: &[xai_mad_sampling_types::conversation::ConversationItem],
) -> usize {
    conversation
        .iter()
...
struct InitialContext {
    ...
    conversation: Vec<xai_mad_sampling_types::conversation::ConversationItem>,
...
fn forked_initial_context(
    mut items: Vec<xai_mad_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        xai_mad_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[xai_mad_sampling_types::conversation::ConversationItem],
) -> bool {
    matches!(
        items.last(),
...
fn verbatim_or_normalize_fork(
    items: Vec<xai_mad_sampling_types::conversation::ConversationItem>,
    ...
    let (conversation, prefix_len) =
        xai_mad_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
...
fn resolve_inherited_mcp_pool(
    ...
    inheritance: &xai_mad_agent::config::McpInheritance,
...
fn resolve_agent_definition(
    subagent_type: &str,
    ctx: &SubagentSpawnContext,
) -> Option<xai_mad_agent::config::AgentDefinition> {
    let cli_agents = ctx
        .agent_config
        .as_ref()
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    let resolution_context = xai_mad_subagent_resolution::DefinitionResolutionContext {
        cwd: &ctx.parent_cwd,
    ...
    let mut def = xai_mad_subagent_resolution::discover_agent_definition(
        subagent_type,
        &resolution_context,
    )?;
...
fn available_agent_names(ctx: &SubagentSpawnContext) -> Vec<String> {
    let cli_agents = ctx
        ...
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    xai_mad_subagent_resolution::available_agent_names(
        &xai_mad_subagent_resolution::DefinitionResolutionContext {
            cwd: &ctx.parent_cwd,
            plugins: ctx.plugin_registry.as_deref(),
            cli_agents,
            toggles: &ctx.subagent_toggle,
...
pub(crate) struct SubagentValidationContext {
    pub parent_cwd: PathBuf,
    pub plugin_registry: Option<Arc<xai_mad_agent::plugins::PluginRegistry>>,
...
pub(crate) fn validate_subagent_type(
    ...
    let context = xai_mad_subagent_resolution::DefinitionValidationContext {
        cwd: &ctx.parent_cwd,
        plugins: ctx.plugin_registry.as_deref(),
    ...
    match xai_mad_subagent_resolution::validate_agent_name(subagent_type, &context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(xai_mad_subagent_resolution::ResolutionError::Unknown { available, .. }) => {
            SubagentValidateTypeOutcome::Unknown { available }
        }
        Err(xai_mad_subagent_resolution::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        Err(xai_mad_subagent_resolution::ResolutionError::NotAllowed { allowed, .. }) => {
            SubagentValidateTypeOutcome::NotAllowed { allowed }
        }
        Err(
            xai_mad_subagent_resolution::ResolutionError::PersonaResolution(_)
            | xai_mad_subagent_resolution::ResolutionError::ResumeValidation(_),
...
fn gate_subagent_type(
    ...
    match xai_mad_subagent_resolution::gate_agent_definition(subagent_type, &resolution_context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(xai_mad_subagent_resolution::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        ...
        Err(
            xai_mad_subagent_resolution::ResolutionError::Unknown { .. }
            | xai_mad_subagent_resolution::ResolutionError::PersonaResolution(_)
            | xai_mad_subagent_resolution::ResolutionError::ResumeValidation(_),
        ) => SubagentValidateTypeOutcome::ValidationUnavailable,
    }
...
pub(crate) fn subagent_harness_flavor_is_representable(agent_type: &str) -> bool {
    xai_mad_subagent_resolution::subagent_harness_flavor_is_representable(agent_type)
}
/// Apply the harness-dependent toolset/prompt re-selection to a resolved
/// agent definition.
///
/// The harness flavor (alternate vs mad-build) normally follows the PARENT
/// agent: `MadagascarBuildOrchestrator` parents give children
...
/// (so a mad-build session can run an alternate-harness verifier and vice-versa);
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut xai_mad_agent::config::AgentDefinition,
) {
    let resolution_context = xai_mad_subagent_resolution::HarnessToolsetContext {
    ...
    xai_mad_subagent_resolution::apply_harness_toolset(
        subagent_type,
        &resolution_context,
        definition,
    );
...
fn summarize_tool_config(
    config: &xai_mad_tools::registry::types::ToolServerConfig,
) -> SubagentTypeSummary {
...
fn resolve_subagent_permission_mode(
    requested: xai_mad_agent::config::PermissionMode,
    is_plugin: bool,
    policy_block: Option<&'static str>,
) -> xai_mad_agent::config::PermissionMode {
    if is_plugin {
...
fn resolve_subagent_source_repo(ctx: &SubagentSpawnContext) -> std::path::PathBuf {
    ...
    xai_mad_workspace::session::git::find_main_repo_root_from_path(&source_cwd)
...
fn inject_subagent_completed_prompt(
    ...
    task_completion_reservations: &Option<
        xai_mad_tools::reminders::task_completion::TaskCompletionReservations,
    >,
    parent_cmd_tx: Option<&mpsc::UnboundedSender<SessionCommand>>,
    ...
    let summary =
        xai_mad_tools::implementations::mad_build::task::completion_summary(request, result);
    let message = xai_mad_tools::reminders::task_completion::format_subagent_completion(
        &summary,
        Some(task_output_tool_name),
    );
    let wrapped = xai_mad_tools::reminders::wrap_reminder(&message);
...
pub(crate) async fn reconcile_orphaned_subagents_with_backend(
    ...
    backend: &xai_mad_tools::implementations::mad_build::task::backend::ChannelBackend,
    session_dir: &Path,
    parent_session_id: &str,
    gateway: &GatewaySender,
    parent_cmd_tx: Option<&mpsc::UnboundedSender<SessionCommand>>,
