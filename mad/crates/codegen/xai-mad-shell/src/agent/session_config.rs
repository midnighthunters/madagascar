...
mod tests {
    ...
    fn options_have_one_selected_model_and_a_mode_per_effort() {
        let models = [
            model("mad-build", "Madagascar Build"),
            model("mad-4.5", "Mad 4.5"),
        ];
        let current = acp::ModelId::from("mad-build");
        ...
        assert_eq!(selected_models[0].id, "mad-build");
    ...
    fn none_effort_is_not_a_user_selectable_mode() {
        ...
        let models = [model("mad-build", "Madagascar Build")];
        let current = acp::ModelId::from("mad-build");
        let opts = build_session_config_options(
            &models,
            &current,
    ...
    fn model_label_falls_back_to_id_when_name_empty() {
        let models = [model("mad-build", "")];
        let current = acp::ModelId::from("mad-build");
        let opts = build_session_config_options(&models, &current, &[], None);
        assert_eq!(opts[0].label, "mad-build");
    }

    #[test]
    fn session_config_option_serializes_camel_case() {
        let opt = SessionConfigOption {
            id: "mad-build".to_string(),
            category: "model".to_string(),
            label: "Madagascar Build".to_string(),
            description: None,
            selected: true,
        };
        let v = serde_json::to_value(&opt).expect("serialize");
        assert_eq!(v["id"], "mad-build");
        assert_eq!(v["category"], "model");
        assert_eq!(v["label"], "Madagascar Build");
        assert_eq!(v["selected"], true);
        assert!(v.get("description").is_none());
    }

    #[test]
    fn mad_session_detail_serializes_camel_case() {
        let detail = MadSessionDetail::build(
            "sess-1".to_string(),
        ...
        assert_eq!(v["currentModelId"], "mad-build");
