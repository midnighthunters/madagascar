...
use crate::auth::{AuthManager, AuthMode, MadAuth, MadComConfig, run_auth_flow};
...
/// or live subagent). The second signal covers relay-driven (mad.com
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = xai_mad_agent::prompt::workspace_user::optional_workspace_user_dir();
...
pub async fn run_stdio_agent(
    ...
    // whoever spawned it (mad-desktop, IDE clients, the agent SDKs, a parent
    ...
    xai_mad_telemetry::unified_log::set_version(xai_mad_version::VERSION);
    ...
    // Log the client that launched us (set by mad-desktop when spawning `mad agent stdio`).
    // This appears early in unified.jsonl and is extremely useful for auth diagnostics.
    if let Ok(version) = std::env::var("MAD_CLIENT_VERSION") {
    ...
    let result = local_set
        .run_until(async move {
            ...
            // `mad agent stdio` is a local/interactive entrypoint (spawned by
            // mad-desktop), so it needs the gate like the leader and pager paths;
            ...
            apply_otel_config(&auth_manager, &agent_config.mad_com_config);
...
async fn run_headless_inner(
    ...
    register_fs_watch_runtime();
    xai_mad_telemetry::unified_log::set_version(xai_mad_version::VERSION);
    // `mad agent [headless]` serves non-TUI automation; stamp proxy requests
    // as headless. IDE-facing `mad agent stdio` stays interactive.
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a mad.com session. \
        Run `mad login` to sign in, or use `mad agent stdio` for API-key access.";
    ...
    let ctx = &agent_config.mad_com_config;
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        match auth_manager.current() {
            ...
            None if auth_manager.is_expired() => {
                anyhow::bail!("Session expired. Please run 'mad login' to re-authenticate.")
            }
            None => anyhow::bail!("No cached credentials found. Run `mad login`."),
        }
    } else if reauthenticate {
        let auth_manager = Arc::new(AuthManager::new(&mad_home::mad_home(), ctx.clone()));
        run_auth_flow(
            &auth_manager,
            ctx,
            true,
    ...
    // Capture the madagascar build URL for the first-connection callback
    let mad_code_url = format!("{}/build", ctx.mad_ws_origin);
    ...
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow && !no_browser {
            ...
            eprintln!();
            eprintln!(
                "Open Madagascar Build: {} (press Enter to open in browser)",
                mad_code_url
            );
            eprintln!();
            let url_for_open = mad_code_url.clone();
...
fn should_seed_shared_session(existing: Option<&MadAuth>, session: &MadAuth) -> bool {
    match existing {
        None => true,
        Some(existing) => {
...
/// Start the leader's mad.com relay connection according to the start policy,
/// parking the [`RelayHandle`](crate::agent::relay::RelayHandle) in `slot`
/// once the connection task is running.
///
/// * `relay_on_demand == false` (default — explicit `mad agent leader`
...
///   duplication of mirroring every agent message to mad.com.
///
...
/// Everything needed to arm the leader's mad.com relay *after* startup.
///
...
struct DeferredRelayArm {
    ...
    slot: Rc<std::cell::RefCell<Option<crate::agent::relay::RelayHandle>>>,
    mad_com_config: crate::auth::MadComConfig,
...
impl DeferredRelayArm {
    ...
    /// [`spawn_leader_relay`]).
    fn arm_if_eligible(self, session: &MadAuth, auth_manager: &Arc<AuthManager>) -> Option<Self> {
        ...
        info!("Relay-eligible auth token appeared after startup — arming mad.com relay");
        spawn_leader_relay(
            self.slot,
            relay_config,
...
pub fn apply_otel_config(auth_manager: &AuthManager, mad_com_config: &MadComConfig) {
    suppress_otel();
    ...
    if crate::agent::otel_gate::should_open_at_startup(crate::agent::otel_gate::StartupGate {
        ...
        session_pending: crate::agent::otel_gate::is_session_pending(has_session, mad_com_config),
...
/// When a mad.com session is present, the leader connects to the websocket relay
...
/// * `relay_on_demand` - If true, defer the mad.com relay WebSocket until the
...
pub async fn run_leader(
    ...
    let control_state = LeaderServerControlState::new(LeaderServerMetadata {
        ...
        leader_binary_version: xai_mad_version::VERSION.to_string(),
    })
    ...
    let auth: Option<MadAuth> = crate::auth::try_noninteractive_auth_no_mint(ctx).await;

    // ── Phase 6b: Legacy devbox auth migration ─────────────────────────────
    let auth: Option<MadAuth> = migrate_devbox_auth_if_legacy(auth, &agent_config).await;

    // A session-less leader that can still mint one (auth provider / devbox) will
    // acquire a mad.com session post-readiness whose fleet policy governs
    ...
    let has_session = auth.is_some()
        || agent_config
            ...
            .is_some();
    let session_pending =
        crate::agent::otel_gate::is_session_pending(has_session, &agent_config.mad_com_config);
    if crate::agent::otel_gate::should_open_at_startup(crate::agent::otel_gate::StartupGate {
        has_session,
        has_api_key_env: crate::agent::auth_method::has_xai_api_key_env(),
        session_pending,
    ...
    local_set
        .run_until(async move {
            ...
            // Bridge websocket messages to agent (from mad.com relay)
            ...
            // Start (or arm) the mad.com relay. Eager by default — a bare
            // `mad agent leader` (devbox / systemd) has no local IPC clients
            ...
            if let Some(relay_config) = relay_config {
                ...
                info!(
                    "Relay not started: no mad.com session token \
                ...
                deferred_relay_arm = Some(DeferredRelayArm {
                    ...
                    mad_com_config: agent_config.mad_com_config.clone(),
            ...
            let auth_scope = agent_config.mad_com_config.auth_scope();
            // Gated on user_mad_home() so a cwd-relative .mad/auth.json is never
            // read as the user auth store when no home resolves.
            let initial_auth_key_hash = xai_mad_config::user_mad_home()
                ...
                while let Some(update) = config_update_rx.recv().await {
                    match update {
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            // External write to ~/.mad/models_cache.json
                            // (another mad process fetched a fresher /v1/models
...
mod tests {
    ...
    fn oidc_session(key: &str, create_time: chrono::DateTime<chrono::Utc>) -> MadAuth {
        MadAuth {
            key: key.into(),
    ...
    fn test_relay_config(addr: std::net::SocketAddr) -> crate::agent::relay::RelayConfig {
        ...
        let cfg = crate::auth::MadComConfig {
            mad_ws_url: format!("ws://{addr}"),
            mad_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
    ...
    fn embedded_otel_gate_keeps_a_session_user_fail_closed() {
        ...
        use xai_mad_telemetry::external::{
        ...
        let session = MadAuth {
            ...
            // or a leaked `MAD_AUTH_EARLY_INVALIDATION_SECS` buffer; the gate reads
        ...
        let with_session = {
            ...
            let am = Arc::new(AuthManager::new(dir.path(), MadComConfig::default()));
            am.hot_swap(session);
            am
        };
    ...
    /// `mad agent leader` (devbox/systemd — no local IPC clients,
    /// `relay_on_demand == false`) must connect the mad.com relay eagerly.
        ...
        local
            .run_until(async {
                ...
                spawn_leader_relay(
                    slot.clone(),
                    config,
                    false, // eager: explicit `mad agent leader` invocation
                    demand_rx,
                    ws_to_agent_tx,
                    agent_to_ws_tx.clone(),
                    cancel.clone(),
    ...
    async fn deferred_arm_connects_relay_when_auth_appears() {
        ...
        let mad_com_config = crate::auth::MadComConfig {
            mad_ws_url: format!("ws://{addr}"),
            mad_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
        let tmp = tempfile::tempdir().unwrap();
        let auth_manager = Arc::new(AuthManager::new(tmp.path(), mad_com_config.clone()));
        ...
        local
            .run_until(async {
                ...
                let ineligible = MadAuth::test_default();
                let arm = arm
