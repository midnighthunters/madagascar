...
use crate::auth::{AuthManager, MadComConfig, OidcAuthConfig};
...
use std::sync::Arc;
use xai_mad_agent::prompt::skills::SkillsConfig;
use xai_mad_sampler::{AuthScheme, SamplerConfig};
use xai_mad_sampling_types::{
...
use xai_mad_tools::types::compat::{
...
pub const DEFAULT_AGENT_TYPE: &str = "mad-build-plan";
...
pub const CLI_CHAT_PROXY_BASE_URL_DEFAULT: &str = "https://cli-chat-proxy.mad.com/v1";
...
pub const ASSET_SERVER_URL_DEFAULT: &str = "https://assets.mad.com";
...
pub struct EndpointsConfig {
    ...
    /// Env: `MAD_MODELS_BASE_URL`. Enables custom endpoint mode.
    /// List URL defaults to `{models_base_url}/models`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub models_base_url: Option<String>,
    /// Env: `MAD_MODELS_LIST_URL`. Overrides the default `{base}/models` list URL.
    #[serde(alias = "models_endpoint", skip_serializing_if = "Option::is_none")]
    pub models_list_url: Option<String>,
    /// Env: `MAD_FEEDBACK_BASE_URL`. Where feedback submissions go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub feedback_base_url: Option<String>,
    /// Env: `MAD_TRACE_UPLOAD_URL`. Where trace uploads go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_url: Option<String>,
    /// Env: `MAD_TRACE_UPLOAD_BUCKET`. Direct bucket (`gs://` or `s3://`), bypasses proxy.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_bucket: Option<String>,
    /// Env: `MAD_TRACE_UPLOAD_REGION`. AWS region (S3 only).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_region: Option<String>,
    /// Env: `MAD_TRACE_UPLOAD_CREDENTIALS_FILE`. Path to GCS SA key or AWS credentials file.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Env: `MAD_TRACE_UPLOAD_ENDPOINT_URL`. Custom S3-compatible endpoint.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_endpoint_url: Option<String>,
    /// Env: `MAD_DEPLOYMENT_KEY`. Management API key for enterprise deployments.
    /// Sent on telemetry and service requests for deployment-level attribution.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub deployment_key: Option<String>,
    /// Env: `MAD_MANAGED_CONFIG_URL`. Override the managed config endpoint.
    ...
    /// favor of `MAD_INTERNAL_OTLP_TRACES_ENDPOINT`, and ignored by the internal
    /// pipeline when `MAD_EXTERNAL_OTEL` is set (the standard `OTEL_*` vars then
    ...
    /// Env: `MAD_INTERNAL_OTLP_TRACES_ENDPOINT`. Full INTERNAL traces endpoint,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub mad_internal_otlp_traces_endpoint: Option<String>,
    /// Env: `MAD_INTERNAL_OTLP_HEADERS`. `k=v,k2=v2` extra headers for the
    /// internal export (debug). Wins over the legacy `OTEL_EXPORTER_OTLP_HEADERS`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub mad_internal_otlp_headers: Option<String>,
    /// External-OTEL master switch, captured at construction via
    /// [`external_otel_master_switch_resolved`] — the same layered resolution
    /// (requirement pin > `MAD_EXTERNAL_OTEL` env > `[telemetry].otel_enabled`
...
pub(crate) fn default_asset_server_url() -> String {
    std::env::var("MAD_ASSET_SERVER_URL").unwrap_or_else(|_| ASSET_SERVER_URL_DEFAULT.to_owned())
}
...
/// format, shared with `MAD_INTERNAL_OTLP_HEADERS`): split on `,`,
...
impl EndpointsConfig {
    ...
    /// Managed deployment-config URL (`mad setup`): explicit `managed_config_url`,
    ...
    /// 1. `mad_internal_otlp_traces_endpoint` (verbatim)
    ...
    pub fn resolve_otlp_traces_endpoint(&self) -> String {
        if let Some(full) = blank_as_unset(&self.mad_internal_otlp_traces_endpoint) {
            ...
            && let Some(legacy) = self.legacy_internal_otlp_traces_endpoint()
        {
            tracing::warn!(
                "Repointing the internal trace pipeline via OTEL_EXPORTER_OTLP_ENDPOINT / \
                 ...
                 MAD_INTERNAL_OTLP_TRACES_ENDPOINT instead — the standard OTEL_* vars will \
    ...
    /// Extra headers for the INTERNAL export: `mad_internal_otlp_headers`
    ...
    pub fn resolve_otlp_headers(&self) -> Vec<(String, String)> {
        if let Some(headers) = blank_as_unset(&self.mad_internal_otlp_headers) {
    ...
    /// `mad_internal_otlp_*` override shadowed that half.
    ///
    ...
    pub fn internal_otlp_consumed_standard_vars(&self) -> bool {
        ...
        let endpoint_consumed = blank_as_unset(&self.mad_internal_otlp_traces_endpoint).is_none()
            && self.legacy_internal_otlp_traces_endpoint().is_some();
        let headers_consumed = blank_as_unset(&self.mad_internal_otlp_headers).is_none()
    ...
    /// still honored by the internal pipeline even with `MAD_EXTERNAL_OTEL`
...
impl Default for EndpointsConfig {
    fn default() -> Self {
        Self {
            cli_chat_proxy_base_url: std::env::var("MAD_CLI_CHAT_PROXY_BASE_URL").ok(),
            xai_api_base_url: std::env::var("MAD_XAI_API_BASE_URL")
                .unwrap_or_else(|_| XAI_API_BASE_URL_DEFAULT.to_owned()),
            alpha_test_key: None,
            models_base_url: env_string("MAD_MODELS_BASE_URL"),
            models_list_url: env_string("MAD_MODELS_LIST_URL"),
            feedback_base_url: env_string("MAD_FEEDBACK_BASE_URL"),
            trace_upload_url: env_string("MAD_TRACE_UPLOAD_URL"),
            trace_upload_bucket: env_string("MAD_TRACE_UPLOAD_BUCKET"),
            trace_upload_region: env_string("MAD_TRACE_UPLOAD_REGION"),
            trace_upload_credentials_file: env_string("MAD_TRACE_UPLOAD_CREDENTIALS_FILE"),
            trace_upload_credentials: None,
            trace_upload_endpoint_url: env_string("MAD_TRACE_UPLOAD_ENDPOINT_URL"),
            deployment_key: env_string("MAD_DEPLOYMENT_KEY"),
            managed_config_url: env_string("MAD_MANAGED_CONFIG_URL"),
            ...
            mad_internal_otlp_traces_endpoint: env_string("MAD_INTERNAL_OTLP_TRACES_ENDPOINT"),
            mad_internal_otlp_headers: env_string("MAD_INTERNAL_OTLP_HEADERS"),
...
pub use xai_mad_config_types::{BoolFlag, ConfigSource, LazinessDetectorPerModelConfig, Resolved};
...
/// environment so it can't inherit the keys Mad uses for its own first-party
/// requests. Keep in sync with every first-party credential env read across the
/// crate: `auth::manager` (`MAD_AUTH`/`MAD_AUTH_PATH`), `auth_method`
...
pub(crate) const FIRST_PARTY_CREDENTIAL_ENV_VARS: &[&str] = &[
    ...
    "MAD_DEPLOYMENT_KEY",
    "MAD_EXTRA_AUTH_KEY",
    "MAD_TRACE_UPLOAD_CREDENTIALS_FILE",
    "OTEL_EXPORTER_OTLP_HEADERS",
    "MAD_INTERNAL_OTLP_HEADERS",
];
...
pub use xai_mad_config::env_bool;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(xai_mad_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: xai_mad_tools::types::compat::CompatCell,
...
pub(crate) use xai_mad_telemetry::config::env_telemetry_mode;
pub use xai_mad_telemetry::config::{TelemetryConfig, TelemetryMode};
...
impl PluginsConfig {
    ...
    /// Native `.mad/config.toml` entries already present take precedence:
    ...
    pub fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                xai_mad_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub fn to_discovery_config(&self) -> xai_mad_agent::plugins::discovery::DiscoveryConfig {
        xai_mad_agent::plugins::discovery::DiscoveryConfig {
...
pub struct CliConfig {
    ...
    /// Env `MAD_MINIMUM_VERSION`. See [`crate::util::config::VersionPolicy`] for
    /// the version-policy knobs. (Unrelated to
    /// `version_overrides[].maximum_version`, which gates config patches.)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub minimum_version: Option<String>,
    /// Env `MAD_MAXIMUM_VERSION`. See [`crate::util::config::VersionPolicy`].
    #[serde(skip_serializing_if = "Option::is_none")]
    pub maximum_version: Option<String>,
    /// Env `MAD_REQUIRED_MINIMUM_VERSION`. See [`crate::util::config::VersionPolicy`].
    #[serde(skip_serializing_if = "Option::is_none")]
    pub required_minimum_version: Option<String>,
    /// Env `MAD_REQUIRED_MAXIMUM_VERSION`. See [`crate::util::config::VersionPolicy`].
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct ModelsConfig {
    ...
    /// Unset = remote pin, then the client hint / built-in `mad-build-0.1`
...
/// (`mad workspace` / leader `with_default_hub_url`). Does **not** enable
...
pub struct HubConfig {
    ...
    /// `mad workspace start` when the CLI does not pass `--hub-url`.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl SandboxSettingsConfig {
    ...
    pub fn resolve_profile(
        ...
        resolve_string_flag(cli_arg, "MAD_SANDBOX", self.profile.as_deref(), None)
    ...
    pub fn resolve_auto_allow_bash(&self, requirement: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("MAD_SANDBOX_AUTO_ALLOW_BASH")
...
impl SuggestionsConfig {
    ...
    pub fn resolve_ai_enabled(
        ...
        BoolFlag::env("MAD_SUGGESTIONS_AI")
    ...
    pub fn resolve_ai_model(&self) -> String {
        resolve_string_flag(
            None,
            "MAD_SUGGESTIONS_AI_MODEL",
            self.ai_model.as_deref(),
            None,
        )
        .map(|r| r.value)
        .unwrap_or_else(|| "mad-build".to_owned())
    }
}
...
///
/// These supplement the built-in scan locations (`.mad/skills/`,
/// `.agents/skills/`, `~/.mad/skills/`). They're written by `/import-claude`
...
pub struct Config {
    ...
    pub mad_com_config: MadComConfig,
    ...
    /// Typed as `MadComConfig` (same schema) so sub-field typos are caught.
    #[serde(default, skip_serializing)]
    pub auth: Option<MadComConfig>,
    /// `[desktop]` section — owned by mad-desktop (Electron app), opaque to the CLI agent.
    #[serde(default, skip_serializing)]
    pub desktop: Option<toml::Value>,
    /// Top-level `announcements` array — consumed by `resolve_announcements`.
    #[serde(default, skip_serializing)]
    pub announcements: Vec<xai_mad_announcements::RemoteAnnouncement>,
    ...
    /// `[marketplace]` — also read by `xai_mad_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    /// Defaults to reading from MAD_STORAGE_MODE env var.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<xai_mad_agent::config::AgentDefinition>,
    #[serde(skip)]
    pub cli_agent_overrides: CliAgentOverrides,
    /// Whether subagent (task tool) support is enabled. Enabled by default;
    /// disabled only via `MAD_SUBAGENTS=0` or `[subagents] enabled = false`.
    /// Not remotely gated.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, xai_mad_subagent_resolution::config::SubagentRole>,
    /// Trust-independent personas from inline, user, and bundled sources.
    #[serde(skip)]
    pub subagent_personas:
        std::collections::HashMap<String, xai_mad_subagent_resolution::config::SubagentPersona>,
    ...
    pub zdr_video_output_s3:
        Option<xai_mad_tools::implementations::mad_build::video_gen::ZdrVideoOutputS3Config>,
    ...
    /// Image describe model (`mad-build` default via `ModelOverrideConfig::resolve`).
    #[serde(skip)]
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<xai_mad_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<xai_mad_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub fn apply_to_definition(&self, def: &mut xai_mad_agent::config::AgentDefinition) {
    ...
    pub fn apply_to_subagent_definition(&self, def: &mut xai_mad_agent::config::AgentDefinition) {
...
pub use xai_mad_agent::config::AgentDefinition;
pub use xai_mad_agent::config::Effort;
pub use xai_mad_agent::config::PermissionMode;
pub use xai_mad_shared::ui_config::{ContextualHints, UiConfig};
...
/// ```toml
/// [agent]
/// # Use a named agent (looked up via discovery: .mad/agents/, ~/.mad/agents/, built-ins)
...
/// 4. `MAD_AGENT` env var
/// 5. Default `mad-build` agent
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct AgentSelectionConfig {
    /// Name of a built-in or discovered agent definition.
    /// Looked up via `xai_mad_agent::discovery::by_name_in_cwd()`.
    /// Examples: "mad-build", "browser-use", or a custom agent name.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    /// Path to an agent definition file (.md with YAML frontmatter).
    /// When set, the agent is loaded from this file.
    /// Supports environment variable expansion (e.g., `$HOME/.mad/agents/my-agent.md`).
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            mad_com_config: MadComConfig::default(),
            ...
            client_version: Some(xai_mad_version::VERSION.to_string()),
...
/// (surfaced by `mad inspect`) and is skipped, so it fails closed for the
...
impl Config {
    ...
    pub fn create_auth_manager(&self) -> AuthManager {
        AuthManager::new(
            &crate::util::mad_home::mad_home(),
            self.mad_com_config.clone(),
        )
    ...
    pub fn new_from_toml_cfg(raw_config: &toml::Value) -> Result<Self, String> {
        ...
        if config.mad_com_config.oidc.is_none() {
            config.mad_com_config.oidc = OidcAuthConfig::from_env();
        }
        if config.mad_com_config.oidc.is_none() && config.mad_com_config.oauth2.is_none() {
            config.mad_com_config.oauth2 = crate::auth::OAuth2ProviderConfig::from_env();
        }
    ...
    pub fn resolve_runtime_fields(&mut self, ctx: &RuntimeResolutionContext<'_>) {
        ...
        self.auto_wake_enabled = BoolFlag::env("MAD_AUTO_WAKE")
    ...
    /// If the TOML contains `[auth]`, copy its contents under `[mad_com_config]`.
    /// `[mad_com_config]` takes precedence if both are present (explicit wins).
    ///
    /// This lets customers write the shorter `[auth.oidc]` instead of `[mad_com_config.oidc]`.
    fn expand_auth_alias(raw_config: &toml::Value) -> toml::Value {
        ...
        {
            if let Some(gcc) = table.get_mut("mad_com_config") {
                ...
                table.insert("mad_com_config".to_owned(), auth);
            }
        }
        config
    }
    fn apply_env_overrides(&mut self) {
        ...
        if let Some(mode) = env_telemetry_mode("MAD_TELEMETRY_ENABLED") {
    ...
    /// config.toml key, or `MAD_TWO_PASS_COMPACTION` env.
    ...
    pub(crate) fn resolve_trace_upload(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("MAD_TELEMETRY_TRACE_UPLOAD")
    ...
    pub(crate) fn trace_upload_decision_debug(&self) -> serde_json::Value {
        ...
        serde_json::json!({
            ...
            "in_env_trace_upload": std::env::var("MAD_TELEMETRY_TRACE_UPLOAD").ok(),
            "in_env_telemetry_enabled": std::env::var("MAD_TELEMETRY_ENABLED").ok(),
    ...
    pub(crate) fn resolve_feedback(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("MAD_FEEDBACK_ENABLED")
    ...
    pub(crate) fn resolve_two_pass_compaction(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("MAD_TWO_PASS_COMPACTION")
            .config(self.features.two_pass_compaction)
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    /// Server-side doom-loop check policy (the `x-mad-doom-loop-check`
    ...
    /// `MAD_DOOM_LOOP_RECOVERY` > TOML `enabled` > remote `enabled` >
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<xai_mad_sampling_types::DoomLoopRecoveryPolicy> {
        use xai_mad_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
            .and_then(|s| s.doom_loop_recovery.as_ref());
        let enabled = BoolFlag::env("MAD_DOOM_LOOP_RECOVERY")
    ...
    /// Precedence: env `MAD_OFFICIAL_MARKETPLACE_AUTO_REGISTER` > remote settings >
    ...
    pub(crate) fn resolve_official_marketplace_auto_register(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("MAD_OFFICIAL_MARKETPLACE_AUTO_REGISTER")
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    ...
    pub(crate) fn resolve_ask_user_question(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("MAD_ASK_USER_QUESTION")
    ...
    ///
    /// Precedence: requirements > `MAD_VOICE_MODE` > config/managed
    ...
    pub(crate) fn resolve_image_gen(&self) -> Resolved<bool> {
        use xai_mad_tools::implementations::mad_build::IMAGE_GEN_TOOL_NAME;
    ...
    pub(crate) fn resolve_image_edit(&self) -> Resolved<bool> {
        use xai_mad_tools::implementations::mad_build::IMAGE_EDIT_TOOL_NAME;
        ...
        BoolFlag::env("MAD_IMAGE_EDIT").default(true).resolve()
    }
    ...
    pub(crate) fn resolve_video_gen(&self) -> Resolved<bool> {
        use xai_mad_tools::implementations::mad_build::{
    ...
    pub(crate) fn resolve_goal(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("MAD_GOAL")
            .config(self.goal.enabled)
            .feature_flag(ff)
            .default(true)
            .resolve()
    }
    /// Background workflows (`workflow` tool, `.mad/workflows/*.rhai`,
    ...
    pub(crate) fn resolve_goal_classifier_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("MAD_GOAL_CLASSIFIER")
    ...
    pub(crate) fn resolve_goal_use_current_model_only(&self) -> Resolved<bool> {
        BoolFlag::env("MAD_GOAL_USE_CURRENT_MODEL_ONLY")
    ...
    pub(crate) fn resolve_backend_tools(&self) -> Resolved<bool> {
        BoolFlag::env("MAD_BACKEND_SEARCH")
            .config(self.features.backend_tools)
            .default(true)
            .resolve()
    }
    /// Resolve the mode (env `MAD_COMPACTION_MODE` > config > remote settings >
    ...
    pub(crate) fn resolve_compaction_mode(&self) -> xai_chat_state::CompactionMode {
        resolve_compaction_mode_from(
            env_string("MAD_COMPACTION_MODE").as_deref(),
    ...
    /// Resolve verbatim-input flag: env `MAD_COMPACTION_VERBATIM_INPUT` > config > remote settings > default `true`.
    pub(crate) fn resolve_compaction_verbatim_input(&self) -> bool {
        BoolFlag::env("MAD_COMPACTION_VERBATIM_INPUT")
    ...
    /// Precedence: env `MAD_COMPACTION_DETAIL`, then config
    ...
    fn resolve_compaction_detail(&self) -> xai_chat_state::CompactionDetail {
        resolve_compaction_detail_from(
            env_string("MAD_COMPACTION_DETAIL").as_deref(),
    ...
    pub fn resolve_cancel_rewind(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("MAD_CANCEL_REWIND")
            .config(self.features.cancel_rewind)
            .feature_flag(ff)
            .default(true)
            .resolve()
    }
    /// Resolve whether to use mad's default OAuth2 (xAI auth.x.ai).
    ///
    ...
    ///
    /// Priority: `--oauth` > MAD_OAUTH_ENABLED env > default (true = OAuth).
    pub fn resolve_mad_oauth(&self, cli_oidc: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("MAD_OAUTH_ENABLED")
            .cli(cli_oidc)
            .default(true)
            .resolve()
    }
    ...
    /// narrow non-recursive watches for `<cwd>/` and `<cwd>/.mad/`.
    ///
    /// Thin delegate to the canonical
...
///
/// `requirement > cli > env (MAD_MCP_LIVENESS_WATCHERS) > config >
...
pub fn resolve_mcp_liveness_watchers(
    ...
    BoolFlag::env("MAD_MCP_LIVENESS_WATCHERS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// `BoolFlag` precedence:
///
/// `requirement > cli > env (MAD_MCP_AUTO_RESTART) > config >
/// managed > feature_flag > default (true)`.
///
...
/// precedence is single-sourced.
///
/// Recovery is on by default; opt out via `MAD_MCP_AUTO_RESTART=false`,
...
pub fn resolve_mcp_auto_restart(
    ...
    BoolFlag::env("MAD_MCP_AUTO_RESTART")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
///
/// `requirement > cli > env (MAD_MCP_PUSH_SERVER_STATUS) > config >
/// managed > feature_flag > default (true)`.
///
...
pub fn resolve_mcp_push_server_status(
    ...
    BoolFlag::env("MAD_MCP_PUSH_SERVER_STATUS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// [`resolve_mcp_liveness_watchers`]:
///
/// `requirement > cli > env (MAD_MCP_RECURSIVE_CONFIG_WATCH) >
...
/// operators flip this flag (e.g. via `MAD_MCP_RECURSIVE_CONFIG_
...
pub fn resolve_mcp_recursive_config_watch(
    ...
    BoolFlag::env("MAD_MCP_RECURSIVE_CONFIG_WATCH")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
impl SyncBoolFlag {
    ...
    /// Either-direction env resolver (typically `MAD_*`). Returns
...
pub fn is_telemetry_disabled_sync() -> bool {
    !SyncBoolFlag::new(telemetry_enabled_from_toml)
        ...
        .enable_env(mad_telemetry_env_enabled)
        .resolve()
}
...
pub fn is_error_reporting_disabled_sync() -> bool {
    !SyncBoolFlag::new(error_reporting_enabled_from_toml)
        ...
        .enable_env(|| env_bool("MAD_ERROR_REPORTING"))
...
/// `MAD_TELEMETRY_ENABLED` resolved through `TelemetryMode::parse` so the
/// extended string forms (e.g. `"session_metrics"`) are accepted.
fn mad_telemetry_env_enabled() -> Option<bool> {
    env_telemetry_mode("MAD_TELEMETRY_ENABLED").map(|m| !m.is_disabled())
}
/// Load `~/.mad/requirements.toml` standalone so the admin pin can beat
/// env vars. The merged config layer can't express that — last-merge-wins
/// loses provenance.
pub(crate) fn read_requirements_toml() -> Option<toml::Value> {
    let path = crate::util::mad_home::mad_home().join("requirements.toml");
    let content = std::fs::read_to_string(&path).ok()?;
    toml::from_str(&content).ok()
}
/// Resolve the external-OTEL master switch exactly the way the external
/// stream's activation does: **requirement pin > `MAD_EXTERNAL_OTEL` env >
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        xai_mad_config::load_merged_requirements().as_ref(),
        env_bool("MAD_EXTERNAL_OTEL"),
...
/// by `mad setup`) sit under the env vars, requirements pins are applied on
...
pub fn resolve_external_otel_config(
    client: xai_mad_telemetry::external::config::ExternalClientInfo,
) -> Option<xai_mad_telemetry::external::ExternalOtelConfig> {
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<xai_mad_telemetry::external::ExternalOtelFileConfig> = effective_config
        .and_then(|cfg| cfg.get("telemetry"))
        .map(|t| xai_mad_telemetry::external::ExternalOtelFileConfig {
    ...
    let getenv_pinned = |name: &str| -> Option<String> {
        let pin = match name {
            xai_mad_telemetry::external::config::ENV_MASTER_SWITCH => req_enabled,
    ...
    let mut resolved = xai_mad_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub fn apply_external_otel_remote_policy(settings: Option<&crate::util::config::RemoteSettings>) {
    ...
    let policy = xai_mad_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        xai_mad_telemetry::external::apply_remote_policy(policy);
    }
}
...
pub fn apply_remote_settings_side_effects(settings: Option<&crate::util::config::RemoteSettings>) {
    if let Some(s) = settings {
        ...
        xai_mad_config::signed_policy::apply_remote_managed_config_signature_verification(
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = xai_mad_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    xai_mad_workspace::permission::resolution::json_env_flag(json.get("env"), key)
}
...
pub struct ModelEntryConfig {
    ...
    /// Can also be set via the `MAD_MAX_RETRIES` environment variable.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_retries: Option<u32>,
...
pub struct ModelInfo {
    ...
    /// The routing slug sent in API requests.
    pub model: String,
    /// The base URL of the model (session endpoint). e.g. "https://cli-chat-proxy.mad.com/v1"
    pub base_url: String,
    ...
    /// Always has a value; defaults to `"mad-build-plan"` when the server
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<xai_mad_workspace::permission::ClassifierPromptType>,
...
pub struct Features {
    ...
    /// Show a blocking warning when Mad starts outside a Git repository.
    /// Default: false. Used as the local fallback when the `non_git_warning` remote settings
    /// flag in `mad_build_settings` is absent. When the remote flag is present it takes
    ...
    /// (`image_gen_model_override`) / env / default (`mad-imagine-image-quality`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// env (`MAD_COMPACTION_MODE`). Parsed via `CompactionMode::parse`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compaction_mode: Option<String>,
    /// `none` | `minimal` | `balanced` | `verbose` (default). `None` = defer to
    /// env (`MAD_COMPACTION_DETAIL`). The `segments` verbatim detail level.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// `MAD_MCP_AUTO_RESTART` to opt out).
    ///
...
pub fn enforce_disable_api_key_auth(
        ...
        && crate::util::is_xai_api_url(&creds.base_url)
    {
        ...
        xai_mad_telemetry::unified_log::debug(
...
pub use xai_mad_telemetry::config::deployment_id_from_key;
...
pub fn try_resolve_model_credentials(
    ...
    enforce_disable_api_key_auth(
        &mut credentials,
        cfg.mad_com_config.api_key_auth_disabled(),
        session_key,
    );
    Some(credentials)
}
...
/// BYOK / non-proxy routes for internal slugs like `mad-build`).
...
pub fn inject_url_derived_headers(
    ...
    if crate::util::is_cli_chat_proxy_url(base_url) {
        headers
            ...
            .or_insert_with(|| "xai-mad-cli".to_string());
        headers
...
mod tests {
    ...
    use xai_mad_test_support::EnvGuard;
    #[test]
    fn main_cli_tools_override_preserves_profile_injection_policy() {
        ...
        let mut cases = vec![(AgentDefinition::default_mad_build(), true)];
    ...
    fn auto_mode_config_parses_from_toml_and_json_equivalently() {
        use xai_mad_workspace::permission::ClassifierPromptType;
        let toml_src = r#"
enabled = true
prompt_type = "no_user_tool_prefix"
classifier_model = "mad-4.5"
...
"#;
        ...
        for cfg in [&from_toml, &from_json] {
            ...
            assert_eq!(cfg.classifier_model.as_deref(), Some("mad-4.5"));
    ...
    fn session_resolver_is_not_stamped_onto_third_party_samplers() {
        ...
        impl xai_mad_sampler::BearerResolver for SessionResolver {
    ...
    fn shell_environment_policy_known_keys_track_the_policy_struct() {
        let xai_mad_tools::util::ShellEnvironmentPolicy {
            inherit: _,
            ignore_default_excludes: _,
            exclude: _,
            set: _,
            include_only: _,
        } = xai_mad_tools::util::ShellEnvironmentPolicy::default();
    ...
    fn parses_model_api_key() {
        ...
        assert_eq!(model.info.model, "mad-4.5");
    ...
    async fn set_env_key_shadows_warm_provider_at_resolve_time() {
        use xai_mad_test_support::EnvGuard;
        let var = "MAD_TEST_ENVKEY_SHADOW";
    ...
    fn env_keys_resolve_first_set_wins() {
        let keys = EnvKeys::new(["MAD_TEST_ENV_KEY_PRIMARY", "MAD_TEST_ENV_KEY_FALLBACK"]);
        assert_eq!(keys.resolve_value_with(|_| None), None, "none set");
        assert_eq!(
            keys.resolve_value_with(
                |n| (n == "MAD_TEST_ENV_KEY_FALLBACK").then(|| "from-fallback".into())
            ),
            Some("from-fallback".into())
        );
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "MAD_TEST_ENV_KEY_PRIMARY" => Some("from-primary".into()),
                "MAD_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
        ...
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "MAD_TEST_ENV_KEY_PRIMARY" => Some(String::new()),
                "MAD_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
            Some("from-fallback".into())
    ...
    fn env_keys_resolve_skips_whitespace_only_value() {
        let keys = EnvKeys::new(["MAD_TEST_WS_PRIMARY", "MAD_TEST_WS_FALLBACK"]);
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "MAD_TEST_WS_PRIMARY" => Some("   ".into()),
                "MAD_TEST_WS_FALLBACK" => Some("real".into()),
                _ => None,
            }),
            Some("real".into())
        );
        assert_eq!(
            EnvKeys::single("MAD_TEST_WS_ONLY").resolve_value_with(|_| Some("   ".into())),
            None
        );
        assert_eq!(
            EnvKeys::single("MAD_TEST_WS_PAD").resolve_value_with(|_| Some("  tok  ".into())),
            Some("  tok  ".into())
        );
    }
    #[test]
    ...
    fn first_own_credential_empty_api_key_falls_through_to_env_key() {
        use xai_mad_test_support::EnvGuard;
        let var = "MAD_TEST_FIRST_OWN_CRED_ENV";
        let _guard = EnvGuard::set(var, "env-token");
    ...
    fn resolve_credentials_multi_env_key_uses_lc_alias() {
        use xai_chat_state::AuthType;
        let primary = "MAD_TEST_MULTI_ENV_PRIMARY";
        let alias = "MAD_TEST_MULTI_ENV_LC_ALIAS";
        unsafe {
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_session() {
        use xai_chat_state::AuthType;
        use xai_mad_test_support::EnvGuard;
        let primary = "MAD_TEST_EMPTY_ENV_PRIMARY";
        let alias = "MAD_TEST_EMPTY_ENV_LC_ALIAS";
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_global_key() {
        ...
        let primary = "MAD_TEST_EMPTY_ENV_GLOBAL_PRIMARY";
        let alias = "MAD_TEST_EMPTY_ENV_GLOBAL_ALIAS";
        let _primary = EnvGuard::set(primary, "");
        let _alias = EnvGuard::set(alias, "");
    ...
    fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
        ...
        assert_eq!(config.api_backend, ApiBackend::Messages);
        let client = xai_mad_sampler::SamplingClient::new(config).expect("client should build");
    ...
    fn user_override_adds_api_key_to_default_model() {
        ...
        assert_eq!(
            model.info.base_url, "https://cli-chat-proxy.mad.com/v1",
    ...
    fn user_override_parses_compaction_at_tokens_from_toml() {
        use xai_mad_sampling_types::CompactionAtTokens;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn user_override_parses_compactions_remaining_from_toml() {
        use xai_mad_sampling_types::CompactionsRemaining;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    /// xai-mad-shell/src/agent/models.rs:857 for any BYOK Claude config.
    #[test]
    ...
    #[test]
    fn model_agent_type_defaults_to_mad_build() {
        let raw_config: toml::Value = toml::from_str(
            r#"
            [model.my-model]
            model = "my-model"
    ...
    fn invalid_glob_is_rejected_by_validation() {
        ...
        assert!(ModelGlobSet::compile(Some(&vec!["mad[".to_string()])).is_err());
        let raw: toml::Value = toml::from_str(
            r#"
            [models]
            allowed_models = ["mad["]
    ...
    #[test]
    fn auth_alias_maps_to_mad_com_config() {
        let raw: toml::Value = toml::from_str(
            r#"
            [auth.oidc]
        ...
        let oidc = cfg.mad_com_config.oidc.expect("oidc should be set");
        assert_eq!(oidc.issuer, "https://example.okta.com");
        assert_eq!(oidc.client_id, "test-id");
    }
    #[test]
    fn mad_com_config_still_works() {
        let raw: toml::Value = toml::from_str(
            r#"
            [mad_com_config.oidc]
            issuer = "https://example.okta.com"
    ...
    fn disable_api_key_auth_parses_from_auth_alias() {
        ...
        assert_eq!(absent.mad_com_config.disable_api_key_auth, None);
        let raw: toml::Value = toml::from_str(
            r#"
            [auth]
        ...
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        assert_eq!(cfg.mad_com_config.disable_api_key_auth, Some(true));
    }
    ...
    fn force_login_team_uuid_parses_string_and_array() {
        ...
        let absent = Config::new_from_toml_cfg(&toml::from_str("").unwrap()).unwrap();
        assert_eq!(absent.mad_com_config.force_login_team_uuid, None);
        let raw: toml::Value = toml::from_str(
            r#"
            [auth]
        ...
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        assert_eq!(
            cfg.mad_com_config.force_login_team_uuid,
    ...
    fn force_login_team_uuid_implies_api_key_auth_disabled() {
        use crate::auth::{ForceLoginTeam, MadComConfig};
        let base = MadComConfig {
            disable_api_key_auth: None,
            force_login_team_uuid: None,
            ..MadComConfig::default()
        };
    ...
    fn config_models_default_is_not_overwritten_by_default_models_json() {
        ...
        let resolved = resolve_string_flag(
            None,
            "MAD_DEFAULT_MODEL_TEST_NONEXISTENT",
            config_default,
    ...
    fn config_models_default_custom_model_is_in_resolved_model_list() {
        ...
        assert!(
            models.contains_key("acme-mad"),
            "user-defined model must be in the resolved model list"
        );
        let model = models.get("acme-mad").unwrap();
        assert_eq!(model.info.model, "mad-4.5");
        assert_eq!(model.info.base_url, "https://inference.example.com/v1");
    }
    #[test]
    fn e2e_default_model_with_session_routes_to_proxy() {
        ...
        assert_eq!(
            sampling.base_url, "https://cli-chat-proxy.mad.com/v1",
    ...
    fn e2e_user_config_overrides_prefetched_model() {
        ...
        prefetched.insert(
            dm.to_string(),
            test_model_entry(dm, "https://cli-chat-proxy.mad.com/v1", None, None, None),
        );
        let (_, models) = resolve_models_from_toml(
            &format!(
                r#"
    ...
    fn e2e_duplicate_model_field_both_entries_survive() {
        ...
        let user = models.get("acme-mad").unwrap();
        ...
        assert_eq!(sampling.base_url, "https://cli-chat-proxy.mad.com/v1",);
    }
    #[test]
    ...
    fn e2e_acp_model_info_no_dedup_on_model_field() {
        let mut models = IndexMap::new();
        models.insert(
            "default-mad".to_string(),
            test_model_entry(
                crate::models::default_model(),
                "https://cli-chat-proxy.mad.com/v1",
                None,
                None,
                Some("https://api.x.ai/v1"),
            ),
        ...
        assert!(
            acp_models.contains_key(&acp::ModelId::new("default-mad")),
            "default entry should be addressable by map key"
        );
        assert!(
            acp_models.contains_key(&acp::ModelId::new("acme-mad")),
    ...
    fn unset_endpoint_env_vars() {
        for k in [
            "MAD_CLI_CHAT_PROXY_BASE_URL",
            "MAD_XAI_API_BASE_URL",
            "MAD_FEEDBACK_BASE_URL",
            "MAD_TRACE_UPLOAD_URL",
            "MAD_MANAGED_CONFIG_URL",
            "MAD_MODELS_BASE_URL",
            "MAD_MODELS_LIST_URL",
            "OTEL_EXPORTER_OTLP_ENDPOINT",
            "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT",
            "OTEL_EXPORTER_OTLP_HEADERS",
            "MAD_INTERNAL_OTLP_TRACES_ENDPOINT",
            "MAD_INTERNAL_OTLP_HEADERS",
            "MAD_EXTERNAL_OTEL",
        ] {
    ...
    fn resolve_feedback_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("MAD_FEEDBACK_ENABLED") };
        unsafe { std::env::remove_var("MAD_TELEMETRY_ENABLED") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("MAD_SESSION_RECAP") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_env_off_overrides_default() {
        unsafe { std::env::set_var("MAD_SESSION_RECAP", "0") };
        let cfg = Config::default();
        let r = cfg.resolve_session_recap();
        assert!(!r.value);
    ...
    fn resolve_two_pass_compaction_precedence() {
        unsafe { std::env::remove_var("MAD_TWO_PASS_COMPACTION") };
        ...
        assert!(r.value);
        assert_eq!(r.source, ConfigSource::Config);
        unsafe { std::env::set_var("MAD_TWO_PASS_COMPACTION", "0") };
        let r = config_over_remote.resolve_two_pass_compaction();
    ...
    fn resolve_doom_loop_recovery_precedence() {
        ...
        unsafe { std::env::remove_var("MAD_DOOM_LOOP_RECOVERY") };
        let default_cfg = Config::default();
        let p = default_cfg
        ...
        unsafe { std::env::set_var("MAD_DOOM_LOOP_RECOVERY", "0") };
        assert!(
    ...
    fn resolve_feedback_env_overrides_all() {
        unsafe { std::env::set_var("MAD_FEEDBACK_ENABLED", "true") };
        let mut cfg = Config::default();
    ...
    fn resolve_trace_upload_disabled_when_telemetry_off_despite_remote_flag() {
        unsafe { std::env::remove_var("MAD_TELEMETRY_ENABLED") };
        unsafe { std::env::remove_var("MAD_TELEMETRY_TRACE_UPLOAD") };
        let mut cfg = Config::default();
    ...
    fn resolve_goal_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("MAD_GOAL") };
        let cfg = Config::default();
    ...
    fn resolve_goal_env_overrides_config_without_remote_kill_switch() {
        unsafe { std::env::set_var("MAD_GOAL", "1") };
        let mut cfg = Config::default();
    ...
    fn background_workflows_default_on_without_affecting_goal() {
        unsafe { std::env::remove_var("MAD_WORKFLOWS") };
        let cfg = Config::default();
    ...
    fn resolve_workflows_remote_false_kills_local_opt_in() {
        unsafe { std::env::set_var("MAD_WORKFLOWS", "1") };
        let mut cfg = Config::default();
    ...
    fn resolve_workflows_env_wins() {
        unsafe { std::env::set_var("MAD_WORKFLOWS", "0") };
        let cfg = Config::default();
        let r = cfg.resolve_workflows();
        assert_eq!(r.source, ConfigSource::Env);
        assert!(
    ...
    fn resolve_ask_user_question_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("MAD_ASK_USER_QUESTION") };
        let cfg = Config::default();
    ...
    fn resolve_ask_user_question_env_overrides_remote_settings() {
        unsafe { std::env::set_var("MAD_ASK_USER_QUESTION", "1") };
        let cfg = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
    ...
    fn resolve_image_gen_model_override_remote_settings_or_config() {
        unsafe { std::env::remove_var("MAD_IMAGE_GEN_MODEL_OVERRIDE") };
        ...
        assert_eq!(
            with(None, Some("mad-imagine-image")).resolve_image_gen_model_override(),
            Some("mad-imagine-image".to_owned())
        );
        assert_eq!(
            with(Some("mad-imagine-image-pro"), Some("mad-imagine-image"))
                .resolve_image_gen_model_override(),
            Some("mad-imagine-image-pro".to_owned())
        );
    }
    #[test]
    #[serial]
    fn resolve_image_edit_model_override_remote_settings_or_config() {
        unsafe { std::env::remove_var("MAD_IMAGE_EDIT_MODEL_OVERRIDE") };
        let with = |config: Option<&str>, gb: Option<&str>| Config {
            features: Features {
        ...
        assert_eq!(
            with(None, Some("mad-imagine-image")).resolve_image_edit_model_override(),
            Some("mad-imagine-image".to_owned())
        );
        assert_eq!(
            with(Some("mad-imagine-image-pro"), Some("mad-imagine-image"))
        ...
        let gen_only = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
                image_gen_model_override: Some("mad-imagine-image".to_owned()),
                ..Default::default()
            }),
            ..Default::default()
        };
    ...
    fn imagine_tools_disabled_gates_image_edit() {
        unsafe { std::env::remove_var("MAD_IMAGE_EDIT") };
        ...
        unsafe { std::env::set_var("MAD_IMAGE_EDIT", "1") };
    ...
    fn resolve_image_gen_gates() {
        unsafe { std::env::remove_var("MAD_IMAGE_GEN") };
        ...
        unsafe { std::env::set_var("MAD_IMAGE_GEN", "1") };
        let denied = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
    ...
    fn resolve_video_gen_gates() {
        unsafe { std::env::remove_var("MAD_VIDEO_GEN") };
    ...
    fn clear_goal_envs() {
        unsafe {
            std::env::remove_var("MAD_GOAL");
            std::env::remove_var("MAD_GOAL_CLASSIFIER");
            std::env::remove_var("MAD_GOAL_PLANNER");
            std::env::remove_var("MAD_GOAL_SUMMARY");
            std::env::remove_var("MAD_GOAL_VERIFIER_N");
            std::env::remove_var("MAD_GOAL_CLASSIFIER_MAX");
            std::env::remove_var("MAD_GOAL_STRATEGIST_EVERY");
            std::env::remove_var("MAD_GOAL_REVERIFY_AFTER");
        }
    }
    ...
    fn resolve_goal_classifier_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("MAD_GOAL_CLASSIFIER", "0") };
        let r = cfg_with_goal_and_remote(true, remote_classifier(true))
            .resolve_goal_classifier_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("MAD_GOAL_CLASSIFIER", "1") };
    ...
    fn resolve_goal_summary_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("MAD_GOAL_SUMMARY", "0") };
        let r =
...
"#,
    ...
    const GOAL_USE_CURRENT_ENV: &str = "MAD_GOAL_USE_CURRENT_MODEL_ONLY";
...
[goal]
enabled = true
planner_model = { model = "mad-build", agent_type = "mad-build-plan" }

[goal.strategist_model]
model = "mad-composer-2.5-fast"
agent_type = "cursor"

[[goal.skeptic_models]]
model = "mad-build"
agent_type = "mad-build-plan"
...
"#;
        ...
        let cfg = Config::new_from_toml_cfg(&raw).unwrap();
        assert_eq!(cfg.goal.planner_model.as_ref().unwrap().model, "mad-build");
        assert_eq!(
            cfg.goal.strategist_model.as_ref().unwrap().agent_type,
            "cursor"
        );
        assert_eq!(cfg.goal.skeptic_models.len(), 2);
        assert_eq!(cfg.goal.skeptic_models[0].model, "mad-build");
        assert_eq!(
...
planner_model = { agent_type = "mad-build-plan" }
"#;
        let raw: toml::Value = toml::from_str(toml_str).unwrap();
        let cfg = Config::new_from_toml_cfg(&raw)
...
"#;
        ...
        assert_eq!(cfg.goal.skeptic_models.len(), 2);
        assert_eq!(cfg.goal.skeptic_models[0].model, "mad-build");
        assert_eq!(cfg.goal.skeptic_models[1].model, "mad-composer-2.5-fast");
    }
...
classifier_max_runs = 6
planner_model = { model = "mad-build", agent_type = "mad-build-plan" }
strategist_model = { model = "mad-composer-2.5-fast", agent_type = "cursor" }
...
"#,
        ...
        let mad_build = crate::util::config::GoalRoleModel {
            model: "mad-build".into(),
            agent_type: "mad-build-plan".into(),
        };
        let composer = crate::util::config::GoalRoleModel {
            model: "mad-composer-2.5-fast".into(),
        ...
        assert_eq!(
            planner.value,
            GoalRoleModelChoice::Explicit(mad_build.clone())
        );
        ...
        assert_eq!(
            ...
            vec![
                GoalRoleModelChoice::Explicit(mad_build),
    ...
    fn internal_otlp_test_config() -> EndpointsConfig {
        EndpointsConfig {
            ...
            mad_internal_otlp_traces_endpoint: None,
            mad_internal_otlp_headers: None,
            external_otel_master_switch: false,
            ..Default::default()
        }
    }
    /// `mad_internal_otlp_traces_endpoint` wins over the legacy `OTEL_*`
    /// fields regardless of the master switch.
    #[test]
    fn internal_otlp_endpoint_mad_internal_wins_regardless_of_switch() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                mad_internal_otlp_traces_endpoint: Some(
            ...
            assert_eq!(
                ...
                "switch={switch}: MAD_INTERNAL_OTLP_TRACES_ENDPOINT must win verbatim (trailing / trimmed)"
            );
        }
    }
    ...
    fn internal_otlp_consumed_standard_vars_cases() {
        ...
        for case in cases {
            let cfg = EndpointsConfig {
                ...
                mad_internal_otlp_traces_endpoint: case
                    .internal_ep
                    .then(|| "https://internal.example/traces".to_string()),
                mad_internal_otlp_headers: case.internal_headers.then(|| "ik=iv".to_string()),
                ..internal_otlp_test_config()
            };
            assert_eq!(
    ...
    /// Headers precedence: `mad_internal_otlp_headers` wins; legacy
    /// `otel_exporter_otlp_headers` only when the master switch is unset.
    #[test]
    fn internal_otlp_headers_precedence() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                mad_internal_otlp_headers: Some("x-debug=1".to_string()),
    ...
    fn ext_client() -> xai_mad_telemetry::external::config::ExternalClientInfo {
        xai_mad_telemetry::external::config::ExternalClientInfo::default()
    }
    #[test]
    ...
    fn external_otel_requirements_pin_wins_over_env() {
        ...
        assert!(
            resolve_external_otel_config_with(
                None,
                Some(&req),
                ext_env(&[("MAD_EXTERNAL_OTEL", "1"), ("OTEL_LOGS_EXPORTER", "otlp"),]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    /// (managed config / requirements — no `MAD_EXTERNAL_OTEL` env var) must
    ...
    fn clear_runtime_env_vars() {
        unsafe {
            std::env::remove_var("MAD_SUBAGENTS");
            std::env::remove_var("MAD_RESPECT_GITIGNORE");
            std::env::remove_var("MAD_WEB_SEARCH_MODEL");
            std::env::remove_var("MAD_SESSION_SUMMARY_MODEL");
            std::env::remove_var("MAD_CURSOR_SKILLS_ENABLED");
            std::env::remove_var("MAD_CURSOR_RULES_ENABLED");
            std::env::remove_var("MAD_CURSOR_AGENTS_ENABLED");
            std::env::remove_var("MAD_CLAUDE_SKILLS_ENABLED");
            std::env::remove_var("MAD_CLAUDE_RULES_ENABLED");
            std::env::remove_var("MAD_CLAUDE_AGENTS_ENABLED");
        }
    }
    fn clear_managed_mcp_env_vars() {
        unsafe {
            std::env::remove_var("MAD_MANAGED_MCPS_ENABLED");
            std::env::remove_var("MAD_MANAGED_MCP_GATEWAY_TOOLS_ENABLED");
        }
    }
...
"#,
    ...
    fn resolve_raw_compat_sessions_load_failure_allows_env_override() {
        let _env = isolate_compat_env();
        let _codex = EnvGuard::set("MAD_CODEX_SESSIONS_ENABLED", "true");
        let resolved = resolve_compat_sessions_from_raw(Err(()), None);
        assert!(!resolved.cursor.sessions);
        assert!(!resolved.claude.sessions);
        assert!(resolved.codex.sessions);
    ...
    fn resolve_compat_env_sessions_disable_independently() {
        ...
        for (vendor, env_var) in [
            (CompatVendor::Cursor, "MAD_CURSOR_SESSIONS_ENABLED"),
            (CompatVendor::Claude, "MAD_CLAUDE_SESSIONS_ENABLED"),
            (CompatVendor::Codex, "MAD_CODEX_SESSIONS_ENABLED"),
        ] {
    ...
    fn resolve_compat_precedence_and_reserved_codex_hook() {
        ...
        let _session = EnvGuard::set("MAD_CURSOR_SESSIONS_ENABLED", "true");
        let _hook = EnvGuard::set("MAD_CODEX_HOOKS_ENABLED", "true");
        let resolved = resolve_compat_config(&config, Some(&remote));
        assert!(resolved.cursor.sessions);
        assert!(resolved.codex.hooks);
    }
    ...
    fn resolve_runtime_fields_compat_asymmetric_sources() {
        let _env = isolate_compat_env();
        let _cursor = EnvGuard::set("MAD_CURSOR_SESSIONS_ENABLED", "false");
        let raw: toml::Value =
    ...
    fn resolve_runtime_fields_gitignore_from_env() {
        clear_runtime_env_vars();
        unsafe { std::env::set_var("MAD_RESPECT_GITIGNORE", "0") };
        let raw = empty_config();
        let mut cfg = Config::new_from_toml_cfg(&raw).unwrap();
        cfg.resolve_runtime_fields(&RuntimeResolutionContext {
            raw_config: &raw,
...
"#,
    ...
    fn is_telemetry_explicitly_disabled_sync_env_signals() {
        unsafe { std::env::set_var("MAD_TELEMETRY_ENABLED", "0") };
        unsafe { std::env::remove_var("DISABLE_TELEMETRY") };
        assert!(is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::set_var("MAD_TELEMETRY_ENABLED", "1") };
        assert!(!is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::remove_var("MAD_TELEMETRY_ENABLED") };
...
"#,
        ...
        xai_mad_config::apply_version_overrides(&mut value, &v).unwrap();
        let cfg = Config::new_from_toml_cfg(&value).unwrap();
        assert_eq!(cfg.models.default.as_deref(), Some("mad-4.5"));
    }
    /// Reproduce the enterprise managed config bug: [model.mad-build] sets
    /// context_window=500k for model="mad-4.5", but
    /// [models].default="mad-4.5" resolves to the bare
    /// prefetched entry (256k) because Layer 3 only overrides key
    /// "mad-build", not key "mad-4.5".
    ///
    ...
    fn slug_propagation_enterprise_managed_config_key_mismatch() {
        ...
        prefetched.insert("mad-4.5".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let by_key = resolved
            .get("mad-build")
            .expect("mad-build key must exist");
        assert_eq!(by_key.info.context_window.get(), 500_000);
        assert_eq!(by_key.info.model, "mad-4.5");
        let by_latest = resolved.get("mad-4.5").expect("mad-4.5 key must exist");
        assert_eq!(
            by_latest.info.context_window.get(),
            500_000,
            "BUG: prefetched 'mad-4.5' should inherit 500k from \
             sibling 'mad-build' (same model slug), not stay at {default_cw}"
        );
    }
    ...
    fn slug_propagation_inherits_api_backend_but_not_agent_type() {
        ...
        let mut prefetched = IndexMap::new();
        let mut entry =
            test_model_entry("mad-4.5", "https://test.example.com/v1", None, None, None);
        entry.info.context_window = NonZeroU64::new(default_cw).unwrap();
        entry.info.agent_type = default_agent_type();
        entry.info.api_backend = ApiBackend::default();
        prefetched.insert("mad-4.5".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let latest = resolved.get("mad-4.5").unwrap();
        assert_eq!(
    ...
    fn resolve_model_list_config_reasoning_efforts_beats_remote() {
        ...
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw_config).expect("config should parse");
        let mut entry = prefetch_model_entry("mad-x", 200_000, ApiBackend::default());
        entry.info.reasoning_efforts = vec![ReasoningEffortOption {
            id: "high".to_string(),
            value: ReasoningEffort::High,
            label: "High".to_string(),
        ...
        let mut prefetched = IndexMap::new();
        prefetched.insert("mad-x".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
    ...
    fn mcp_liveness_watchers_default_is_true() {
        unsafe { std::env::remove_var("MAD_MCP_LIVENESS_WATCHERS") };
    ...
    fn mcp_liveness_watchers_requirement_wins_over_everything() {
        unsafe { std::env::set_var("MAD_MCP_LIVENESS_WATCHERS", "true") };
    ...
    fn mcp_liveness_watchers_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("MAD_MCP_LIVENESS_WATCHERS", "false") };
    ...
    fn mcp_auto_restart_default_is_true() {
        unsafe { std::env::remove_var("MAD_MCP_AUTO_RESTART") };
    ...
    fn mcp_auto_restart_requirement_wins_over_everything() {
        unsafe { std::env::set_var("MAD_MCP_AUTO_RESTART", "false") };
    ...
    fn mcp_auto_restart_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("MAD_MCP_AUTO_RESTART", "true") };
    ...
    fn mcp_push_server_status_default_is_true() {
        unsafe { std::env::remove_var("MAD_MCP_PUSH_SERVER_STATUS") };
    ...
    fn mcp_push_server_status_requirement_wins_over_everything() {
        unsafe { std::env::set_var("MAD_MCP_PUSH_SERVER_STATUS", "true") };
    ...
    fn mcp_push_server_status_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("MAD_MCP_PUSH_SERVER_STATUS", "false") };
    ...
    fn mcp_recursive_config_watch_default_is_true() {
        unsafe { std::env::remove_var("MAD_MCP_RECURSIVE_CONFIG_WATCH") };
    ...
    fn mcp_recursive_config_watch_requirement_wins_over_everything() {
        unsafe { std::env::set_var("MAD_MCP_RECURSIVE_CONFIG_WATCH", "true") };
    ...
    fn mcp_recursive_config_watch_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("MAD_MCP_RECURSIVE_CONFIG_WATCH", "false") };
    ...
    fn remote_settings_disarm_managed_config_signatures() {
        ...
        assert!(xai_mad_config::signed_policy::verification_active());
    ...
    fn remote_settings_disarm_requires_prod_proxy_when_keys_embedded() {
        ...
        unsafe {
            std::env::remove_var("MAD_CLI_CHAT_PROXY_BASE_URL");
        }
        apply_remote_settings_side_effects(Some(&settings));
        assert!(
            !xai_mad_config::signed_policy::verification_active(),
