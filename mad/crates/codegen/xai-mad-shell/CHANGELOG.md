...
- **Mad** no longer crashes on startup when the host machine has no free threads.
...
- **MCP servers** can now be enabled or disabled directly from the CLI with `mad mcp enable <name>` and `mad mcp disable <name>`.
- **Full plan markdown** can now be copied to the clipboard with `y` during plan approval or preview.
- **Added support for the new SuperMad Plus subscription tier** in authentication and feature gating.
...
- **Invalid MCP server entries** in config.toml no longer prevent Mad from starting; problems are shown in `mad inspect`.
...
- **Fixed credential loss** across multiple mad processes sharing the same auth file.
...
- **New /tutorial slash command** opens an opt-in nine-topic onboarding tour of Mad.
...
- **`mad doctor fix`** can now repair common tmux clipboard and passthrough problems.
- **Per-provider auth helpers** now work on Windows and can run from a configurable working directory.
- **/resume** now shows only native Mad sessions by default and shows a hint when external sessions are hidden.
- **`mad --resume`** can now resume a session by its title as well as by ID.
...
- **Web search** now defaults to mad-4.5.
...
- You can now run `mad doctor fix` commands directly from inside the TUI instead of only from the CLI.
...
- **npm package** now installs the native binary under `$MAD_HOME/bin` (honoring the same override as the Rust CLI).
...
- Mad now automatically stops a turn that keeps repeating the exact same tool call many times in a row.
...
- When the auto-permission classifier times out or fails, Mad now shows a normal permission prompt instead of silently denying.
...
- **mad doctor fix ssh-wrap** can set up `mad wrap ssh` automatically for Bash, zsh, and fish.
...
- **Added MAD_CLIPBOARD_NO_OSC52** env var to stop clipboard sequences from appearing as garbage in unsupported terminals.
...
- **/btw** now works inside `mad --minimal`, showing answers in the live area and committing them to scrollback on Esc.
...
- **Default model** is now Mad 4.5 with high/medium/low reasoning effort and improved compaction settings.
...
- **Global rules** from ~/.mad/rules and compatible vendor homes are now discovered correctly.
...
- **SSH sessions** now show a one-time tip recommending `mad wrap ssh <host>` for clipboard and terminal restore.
...
- **mad** and the agent binary now stay in sync even when no update is installed.
- **Copying** a multiline queued prompt now copies the complete text instead of a collapsed summary.
- **mad wrap** now restores the terminal after SSH disconnects or other abrupt child exits.
- **Voice speech-to-text** now works with per-model API keys in config.toml without requiring `mad login`.
...
- **mad login** now requests Mad Projects scopes so workspace listing works after consent.
...
- **`mad plugin install`** now accepts a marketplace's registered name as a qualifier.
...
- **Fixed background task tracking** when using mad -p --no-wait-for-background so tasks are properly reaped on exit.
...
- **mad inspect** now shows effective compatibility settings for Cursor, Claude, and Codex sessions.
...
- **Resume hint** after quitting minimal mode now prints the correct mad --minimal --resume command.
...
- **Mad no longer crashes** when printing resume hints after the terminal pane has closed.
...
- **mad setup --json** prints your team's managed configuration without installing it.
...
- **How-to Guides** modal now shows a tip linking to Ask Mad above the footer shortcuts.
...
- **`mad worktree ls`** now works as a short alias for `mad worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.mad/config.toml`.
...
- **Resumed mad.com chats** now use the conversation's last model instead of the gateway default.
...
- **File links and media** now work for worktree sessions under ~/.mad/worktrees/.
...
- **Terminal command cards** on mad.com now correctly settle after foreground bash tasks.
...
- **mad inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain mad launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive mad agent stdio without silent parse drops on session/* calls.
...
- **mad --minimal** now supports native drag-select on classic Windows conhost terminals.
...
- The "Managed by mad.com" link in the Extensions modal is now clickable and underlined.
...
- **mad --minimal** now aligns the prompt, status bar, and messages flush-left with the welcome card.
...
- **mad update** on Windows no longer fails when a previous .old executable is still running.
...
- **mad models** banner now correctly reports per-model API keys and deployment keys.
...
- You can now add a local directory as a plugin marketplace source with `mad plugin marketplace add`.
...
- **Plan files** now default to .mad/plan.md to match Mad conventions.
...
- **--minimal** flag now shows in `mad --help`.
...
- **Tool result previews** no longer paint opaque panels in `mad --minimal`.
- **mad wrap** now correctly handles quoted strings and shell aliases.
...
- **Mad.com chat sessions** now integrate fully with the gateway bridge for model catalog and resume.
...
- **Chat sessions show the mad.com model catalog** in the picker.
...
- **`mad update`** downloads have a longer timeout.
...
- **Pasting images** from the local clipboard now works when running commands through `mad wrap`.
...
- **mad agent stdio** now checks for updates in the background like other modes.
...
- **mad wrap** now shows copy success over SSH and suggests native drag-select when paste fails.
...
- **Fixed `mad agent stdio` hangs** on Windows when used with persistent clients such as VS Code.
...
- **Added `mad wrap`** to run any command with local clipboard support.
...
- **Fixed `mad agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or mad-desktop.
...
- You can now pass --json-schema to mad -p and receive a validated JSON object instead of free text.
...
- **Agent-run terminal commands** now set `MAD_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **mad -w --ref <branch>** now creates worktrees based on the specified ref instead of HEAD.
...
- **--disable-web-search** now honored in mad -p and mad agent; auxiliary model routing respects catalog overrides.
...
- The legacy `agent` command is now kept in sync with `mad` after running `mad update`.
- Headless (`mad -p`) runs now wait for background tasks and subagents to finish before exiting.
...
- **`mad plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **mad sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **mad quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, mad sessions list) are now sorted by the same activity time shown in the UI.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, MAD_HUNK_TRACKER, or config.
...
- **Mad** can now export usage metrics and events to your own OpenTelemetry collector when enabled.
...
- **`mad dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`mad mcp add`** now accepts positional arguments (e.g. `mad mcp add filesystem -- npx ...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`mad update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.mad.
...
- **MCP servers menu** and slash commands now work when starting mad outside a project directory.
...
- **mad --resume** now correctly finds the real session instead of failing on empty image-only folders.
...
- **Fixed `mad --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `mad update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`mad update`** now restarts any older running leader so all clients get the new binary.
...
- **`mad login`** now defaults to device code flow, which works reliably in SSH, WSL, VPN, and browser-restricted environments.
...
- **API-key users** can now run `mad agent --leader` without forced interactive login or timeouts.
...
- **image_gen** can now be enabled via the harness model using [features] in config.toml or the MAD_IMAGE_GEN_HARNESS env var.
...
- **mad inspect** now shows the resolved on/off state and source for every Claude/Cursor compatibility toggle.
- **Cursor MCP servers and hooks** are now discovered and can be disabled independently via MAD_CURSOR_MCPS_ENABLED / MAD_CURSOR_HOOKS_ENABLED.
...
- **Cursor skills and rules** are now discovered alongside Mad and Claude directories.
...
- **Background tasks** started inside subagents now continue running after the subagent session ends.
- **Image description** now reliably uses the mad-build model instead of falling back to the active session model.
...
- **image_gen** now uses the higher-quality mad-imagine-image-quality model.
