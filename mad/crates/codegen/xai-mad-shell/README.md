...
- **Using Mad**
  ...
  - [SSH Passthrough](#ssh-passthrough-mad-ssh) — Apple Terminal clipboard support
- **Configuration**
...
- **Reference**
  - [Introspection (`mad inspect`)](#introspection)
...
- [Building with Mad](#building-with-mad) — headless API, ACP SDK integration
...
On first launch, Mad opens your browser to authenticate with mad.com:

```bash
mad
```

Credentials are stored in `~/.mad/auth.json` and persist across sessions. Tokens expire after 7 days; Mad will prompt you to re-authenticate when needed.
...
export MAD_OIDC_ISSUER="https://acme.okta.com"
export MAD_OIDC_CLIENT_ID="0oa1b2c3d4e5f6g7h8i9"
```

Customers typically also override the API endpoint to point at their own proxy:
```bash
export MAD_CLI_CHAT_PROXY_BASE_URL="https://mad-proxy.acme.com/v1"
```

**3. Run `mad`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.mad/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
Mad is provider-agnostic — it doesn't know or care how your binary authenticates. It just runs the command, reads a token from stdout, and stores it. Your binary is a black box that handles the entire auth flow.

#### How It Works

```
┌──────────────┐     sh -c     ┌────────────────────────┐
│     Mad     │──────────────▶│  your auth binary      │
│              │               │                        │
...
1. Mad runs your command via `sh -c "<command>"`
...
4. **stdout** → captured by Mad and saved to `~/.mad/auth.json` as the access token
5. exit 0 → success; exit non-zero → Mad falls through to interactive login
...
|--------|---------------|-------------|
| **stdout** | The token — nothing else | Mad (parsed and stored in `auth.json`) |
| **stderr** | Login URLs, status messages, errors, progress | The user (displayed in their terminal) |

**Do not print anything to stdout except the token.** No progress messages, no debug output, no "Login successful!" text. Mad reads stdout verbatim and tries to parse it as a token. Any extra text will break parsing.
...
Use JSON if your tokens expire and you want Mad to automatically re-run the binary before expiry. The `expires_in` field (seconds until expiry) tells Mad when to proactively refresh. Without it, Mad assumes tokens last 30 days.
...
# Print ONLY the token to stdout (Mad captures this)
...
```bash
# Or via environment variables
export MAD_AUTH_PROVIDER_COMMAND="/usr/local/bin/my-auth-provider"
export MAD_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export MAD_AUTH_TOKEN_TTL=3600               # optional
```

If your binary outputs a bare token string (not JSON with `expires_in`), set `auth_token_ttl` to the token's expected lifetime in seconds. Without it, Mad cannot detect expiry proactively and will only refresh after a 401.
...
> **Windows:** write the path as a TOML *literal* string (single quotes) so backslashes survive: `auth_provider_command = 'C:\corp\mad-auth.exe'`. Inside a double-quoted TOML string `\t`, `\n`, `\r`, `\b` and `\f` are escape sequences, so `"C:\temp\auth.exe"` parses into a path containing a tab character and the provider fails to start — after which Mad falls back to browser login as if the setting were ignored.

When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with mad.com". In headless mode (`mad -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
RESP=$(curl -s -X POST https://auth.acme.com/device/code -d "client_id=mad-cli")
...
When Mad needs to refresh an expired token, it re-runs your binary with `MAD_AUTH_EXPIRED=1` set in the environment. Your binary can use this to take a faster silent-refresh path:

```bash
#!/bin/sh
if [ "$MAD_AUTH_EXPIRED" = "1" ]; then
...
`MAD_AUTH_EXPIRED` is optional — if your binary ignores it, Mad still works. It just runs the same flow for both login and refresh.

### Automatic Credential Refresh

Mad supports automatic credential refresh for external auth providers and OIDC. When Mad detects that your token is expired (either locally based on `expires_in`, or when the server returns a 401), it automatically re-runs your `auth_provider_command` to obtain new credentials before retrying the request.

This is transparent — you don't need to do anything. Mad handles it in the background during your session.

**When does refresh happen?**

- **Before expiry:** If your binary returned `expires_in` in its JSON output, or you set `auth_token_ttl` in config, Mad re-runs the binary ~5 minutes before the token expires, so you never see an auth error.
- **On auth error:** If the server rejects a request with 401/403 (e.g. token was revoked or expired), Mad re-runs the binary and retries the request once.
- **OIDC:** If you're using OIDC and have a `refresh_token`, Mad silently refreshes via your IdP without re-opening the browser.

**Tuning the refresh buffer:**

```bash
# Mad refreshes tokens 5 minutes before expiry by default.
# Set to 0 to only refresh on 401. Set higher for very short-lived tokens.
export MAD_AUTH_EARLY_INVALIDATION_SECS=300
```

**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `mad login` before starting — Mad runs your binary automatically on first launch. You _can_ run `mad login` to explicitly hydrate `auth.json` ahead of time if you prefer.
- If both OIDC and `auth_provider_command` are configured: at **login** time, Mad tries OIDC silent refresh first (if a `refresh_token` exists), then the external binary, then browser-based login. During a **session**, whichever method is configured is used exclusively — if `auth_provider_command` is set it handles all mid-session refreshes; otherwise OIDC silent refresh is used.
...
```bash
mad --debug-file /tmp/mad-auth.log -p "hello"
tail -f /tmp/mad-auth.log
```
...
`auth_provider_command` above replaces Mad's *session* auth: it mints the token sent to xAI's backend. If you instead want xAI models on normal xAI login while **other models** route through a gateway (LiteLLM, corporate proxy) whose bearer tokens rotate, use a named auth provider — the rotating-token analogue of a per-model `api_key`/`env_key`.
...
- `MAD_AUTH_EXPIRED=1` is set whenever Mad re-mints over a token still cached in memory, whether from near-expiry rotation or a rejection. The first mint on a cold cache runs without it.

**Token lifecycle:**

- Tokens are cached in memory per provider and shared by every model referencing the provider; nothing is written to disk. The command is a credential helper: it owns durable storage and OAuth2 refresh (keychain, its own dotdir, etc.), exactly like `gcloud auth print-access-token` or a git credential helper. On an in-session re-mint the last credential is handed back via `MAD_AUTH_PROVIDER_ACCESS_TOKEN` (and, when present, `MAD_AUTH_PROVIDER_REFRESH_TOKEN` / `MAD_AUTH_PROVIDER_EXPIRES_AT`), so a refresh-grant command can refresh instead of re-authenticating. The command must be non-interactive and fast; do any interactive login out of band, and Mad re-runs the command on restart to re-mint.
- Mad runs the command before a chat turn when the token is missing or within about a minute of expiring, and once more after the server rejects a token. A token rejected within 30 seconds of being fetched is not refetched again, so a broken helper surfaces one clear error instead of looping.
...
**Security:** provider commands execute code, so they are honored only from trusted config layers (`~/.mad/config.toml`, managed config, requirements). A project's `.mad/config.toml` can never define one. Whatever layer sets a model's `base_url` decides where that model's minted token is sent, and `base_url` (unlike the provider table) is not stripped from remote or campaign patches, the same as for a static `env_key`. Keep provider tables and the model `base_url` in layers you trust. The command inherits Mad's environment (so it sees `PATH`, `HOME`, and any other secrets there), but Mad's own first-party credentials (`XAI_API_KEY`, `MAD_DEPLOYMENT_KEY`, and related keys) are removed so a BYOK helper never receives them; write helpers that read only what they need, and prefer the `MAD_AUTH_PROVIDER_*` handback for the prior credential.

### Using auth.json for API Access

If you've authenticated with `mad login`, you can use the stored credentials to call the CLI chat proxy directly via curl. The proxy requires specific headers that mirror what the mad CLI sends internally:

```bash
curl -s -N -X POST "https://cli-chat-proxy.mad.com/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $(jq -r '."https://accounts.x.ai/sign-in".key' ~/.mad/auth.json)" \
  -H "X-XAI-Token-Auth: xai-mad-cli" \
  -H "x-mad-model-override: mad-build" \
  -d '{
    "model": "mad-build",
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.mad/auth.json` (set by `mad login`)                                                                                                                              |
| `X-XAI-Token-Auth: xai-mad-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-mad-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `mad-build` which is on the default route, but always safe to include. |
...
| --------------------- | -------------- | ------------ |
| `mad-build`    | ✅ Supported   | ✅ Supported |

> **Note:** `auth.json` tokens expire after 7 days. Run `mad login` to refresh.
...
mad --cwd ~/projects/my-app

# Start with an initial task
mad --prompt "Review this codebase and suggest improvements"

# Add project-specific rules
mad --rules "Always use TypeScript. Prefer functional components."

# Auto-approve mode for trusted tasks
mad --always-approve --prompt "Format all files"
```
...
Run Mad non-interactively from the command line. Use headless mode when you need to:
...
| `-m, --model <MODEL>`   | Model to use (e.g., `mad-build`)               |
...
mad -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
mad -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
mad -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
mad -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
mad -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
mad -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
mad -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
mad -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
mad -p "Build the project" --allow "Bash"

# Combine: allow fetching docs sites, deny other URLs
mad --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
mad -p "What does this project do?"

# Use a specific model
mad -p "Optimize this function" -m mad-build

# Get JSON output for parsing
mad -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
mad -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
mad -p "Remember: the secret number is 42" --output-format json
mad -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
mad -p "Continue where we left off" -c

# Run in a different directory
mad -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
mad -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
mad -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
mad -p "Generate a README" > README.md

# Parse JSON output with jq
mad -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
mad -p "Review changes for bugs and security issues." \
  --output-format json --always-approve | jq -r '.text' > review.md

# Pipeline: chain with other tools
git diff --staged | mad -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  mad -p "Migrate $file from CommonJS to ES modules." --always-approve
done

# Pre-commit hook
mad -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
Run Mad as an ACP (Agent Client Protocol) agent for integration with IDEs, editors, and custom tooling.
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `mad-build`)                           |
...
```bash
mad agent headless --mad-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`mad ssh`)

Use `mad ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.
...
# With SSH flags
mad ssh -t user@host
mad ssh -L 8080:localhost:8080 user@host

# With remote command
mad ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `mad ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.
...
Mad can be used as an OpenAI-compatible chat completion backend. Choose between two integration modes:
...
Use headless mode for simple integrations. Spawns `mad -p` and parses JSON output.
...
class MadChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["mad", "-p", prompt, "-m", model, "--cwd", self.cwd,
                "--output-format", "streaming-json" if stream else "json", "--always-approve"]

    async def create(self, messages, model="mad-build", stream=False):
...
class MadChat {
  ...
  async *streamResponse(prompt: string, model: string) {
    const proc = execa("mad", this.buildArgs(prompt, model, true));
...
# Usage
async def main():
    client = await MadACPChat(cwd=".").init()
...
class MadACPChat {
  ...
  async init() {
    this.proc = spawn("mad", ["agent", "stdio"]);
...
// Usage
const client = await new MadACPChat(".").init();
...
Mad implements the [Agent Client Protocol (ACP)](https://agentclientprotocol.com), a standard for AI agent communication.
...
Mad reads configuration from `~/.mad/config.toml`. If the file doesn't exist, Mad uses sensible defaults. You only need to specify values you want to override.
...
[models]
default = "mad-4.5"                   # model used for new sessions
web_search = "mad-4.5"                # model used by the web_search tool
...
telemetry = false                      # anonymous usage telemetry (env: MAD_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: MAD_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: MAD_RESPECT_GITIGNORE)
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: MAD_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: MAD_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: MAD_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: MAD_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: MAD_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `MAD_TELEMETRY_BUILD_EVENTS_URL`, `MAD_TELEMETRY_BUILD_EVENTS_API_KEY`, and `MAD_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.

### LSP Servers

Mad can connect to Language Server Protocol (LSP) servers configured in JSON files. LSP integration gives Mad language-aware code intelligence while it works in your repository.

LSP support is used in two ways:

- **Passive diagnostics** — after edits, Mad can surface language-server diagnostics such as errors and warnings.
- **The `lsp` tool** — Mad can actively query the language server for `goToDefinition`, `findReferences`, `hover`, `goToImplementation`, `documentSymbol`, and `workspaceSymbol`.

Reference: [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)

#### Config locations

Mad looks for server definitions in:

- project config: `<repo>/.mad/lsp.json`
- user config: `~/.mad/lsp.json`
...
- LSP tools are enabled (`MAD_LSP_TOOLS=1` or `[features] lsp_tools = true`)
...
If LSP tools are enabled but no usable server config is found, Mad emits a non-fatal warning in logs and continues without the `lsp` tool. If config exists but every server fails to start, the tool may still be present and will fail on first use with a startup error.
...
Mad does not bundle language server binaries. You must install the server yourself and make sure the configured `command` is runnable on your machine.
...
- Passive diagnostics do **not** require `MAD_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
[model.company-mad]
model = "mad-build"
base_url = "https://mad-proxy.acme.com/"
name = "Madagascar Build Latest (Proxy)"
...
With this config, `mad` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.

---

## AGENTS.md

Add project-specific instructions by creating an agent rules file (e.g., `AGENTS.md`). Mad reads these files and appends their contents to the system prompt.

Mad scans for agent rules in this order:
...
Within each directory, Mad checks for these filenames:
...
Skills are reusable prompt packages that extend Mad with specialized workflows, domain knowledge, and tool integrations. Use them to encode repeatable procedures that would otherwise require re-explaining each session.

### Skill Locations

Mad discovers skills from these directories (in priority order):

| Location                    | Scope | Priority |
| --------------------------- | ----- | -------- |
| `./.mad/skills/`           | Local | Highest  |
| `<repo_root>/.mad/skills/` | Repo  | Medium   |
| `~/.mad/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.mad/skills/`) are outside the repo and never filtered.
...
| `description` | What the skill does and when to use it—this is how Mad decides to invoke it |
...
Users can reference skills as `/skill-name` (e.g., `/commit`). When you see this pattern, Mad invokes the corresponding skill.

> **Tip:** The `description` field is critical — it determines when Mad automatically invokes the skill. Be specific about trigger phrases and use cases.
...
Mad discovers agent definitions from `.mad/agents/` (project), `~/.mad/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `MAD_AGENT` env var
4. Default `mad-build` agent
...
```bash
mad --agent-profile ./my-agent.md
# or
export MAD_AGENT="my-custom-agent"
```
...
```bash
export MAD_SUBAGENTS=0              # Environment variable
```
...
[subagents.models]
explore = "mad-build"              # route explore to a lighter model
```
...
model = "mad-build"
prompt_file = ".mad/prompts/researcher.md"

[subagents.personas.concise]
instructions = "Be extremely concise. No filler words."
# instructions_file = ".mad/personas/concise.md"  # or load from file
```

Both are also discovered from `.mad/roles/*.toml` and `.mad/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).

---

## Plugins

Plugins extend Mad with additional tools, skills, and MCP servers from external packages.
...
Mad discovers hooks from `.mad/hooks/` in the project directory. Manage them with:
...
The `context_window` parameter is used to calculate when auto-compact should trigger. If not specified, Mad falls back to built-in defaults for known models.
...
[model.mad-4.20-0309-reasoning]
temperature = 0.5
api_key = "sk-custom"
```

**How it works:** When you override a built-in model, Mad starts with the default configuration (including the correct `base_url` from your `[endpoints]` setting), then applies only the fields you specify. Unspecified fields inherit from the default.
...
**Web search model:** Set `[models] web_search`, `MAD_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
> `mad models` output). You also need a `[model.*]` entry so Mad knows
...
>
> [model.my-custom-model]              # 2. tell Mad how to reach it
...
Point Mad at a custom OpenAI-compatible `/v1/models` endpoint instead of the default cli-chat-proxy. Useful when models are served behind a corporate gateway or self-hosted inference stack.

**Environment variables:**

| Variable | Required | Description |
|----------|----------|-------------|
| `MAD_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `MAD_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |

**Setup:**

```bash
export MAD_MODELS_BASE_URL="https://api.acme.com/v1"
export XAI_API_KEY="xai-..."
mad
```

Mad fetches the model list from `{MAD_MODELS_BASE_URL}/models` on startup and sends inference requests to `MAD_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `MAD_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.mad/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, Mad uses API key auth (`Authorization: Bearer`) instead of session auth. `mad login` is not required — only the API key.

---

## MCP Servers

Extend Mad's capabilities with [Model Context Protocol](https://modelcontextprotocol.io) servers.

### Configuration

MCP servers are configured in `~/.mad/config.toml`:
...
MCP servers can also be configured per-project in `.mad/config.toml`. Mad walks from the current directory up to the git repo root, loading `.mad/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.mad/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.mad/config.toml` | This repository   | ↑        |
| `<cwd>/.mad/config.toml`       | Current directory | Highest  |
...
**Example:** commit a `.mad/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.mad/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.mad/config.toml`. Other config sections (models, shortcuts, etc.) are only read from `~/.mad/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `MAD_MEMORY=1` / `[memory] enabled = true` in config).

Cross-session memory lets Mad remember facts, decisions, code patterns, and debugging workflows across separate sessions in the same project.

### How it works

Memory is stored as Markdown files under `~/.mad/memory/`:
- **Global** (`~/.mad/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.mad/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.mad/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries
...
At the end of each session, Mad saves a **structured metadata summary** to the daily session log:
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.mad/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
Mad searches memory automatically on the first turn of each session and after compaction. The first-turn injection can be disabled or given its own score threshold under `[memory.initial_injection]`. You can also invoke `memory_search` and `memory_get` directly via the model prompt:
...
Key options under `[memory]` in `~/.mad/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.mad/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, Mad emits the `mad-shell-memory_injection`
...
Mad can restrict what the agent process and its spawned commands can access on
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.mad/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.mad/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.mad/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.mad/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.mad/sandbox.toml` (global) or `.mad/sandbox.toml` (per-project):
...
The sandbox is applied to the **entire mad process** at startup using kernel
...
- **Platform support**: Sandbox enforcement uses Landlock on Linux (kernel ≥ 5.13)
  ...
  kernel, missing entitlements), Mad logs a warning and continues without
  enforcement.
...
Sandbox events (profile applied, violations) are logged to `~/.mad/sandbox-events.jsonl`
...
Use `mad inspect` to see everything Mad discovers in the current directory:

```bash
mad inspect          # human-readable output
mad inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.mad/skills/`, `~/.mad/skills/`, plugins, and config paths
...
Mad automatically discovers configuration from Claude Code directories alongside native `.mad/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How Mad uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.mad/skills/`) |
...
Claude Code plugins can provide skills (`skills/`), commands (`commands/`), agents (`agents/`), hooks (`hooks/hooks.json`), MCP servers (`.mcp.json`), and LSP servers (`.lsp.json`). All component types are discovered and used by Mad at runtime.

---

## Built-in Tools

Mad includes these tools by default:
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `MAD_WEB_FETCH=1`. 
...
Mad automatically persists conversations to disk. This works across all modes: TUI, headless, and agent stdio.

### Storage Layout

Sessions are stored under `~/.mad/sessions/`, organized by URL-encoded working directory:

```
~/.mad/sessions/<encoded-cwd>/<session-id>/
...
mad -p "Remember: X=42" -s my-session
mad -p "What is X?" -s my-session

# Resume existing session (errors if not found)
mad -p "Continue" -r my-session

# Continue most recent session in current directory
mad -p "What were we doing?" -c
```

Session ID is returned in JSON output:

```bash
mad -p "Hello" --output-format json | jq -r '.sessionId'
```
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.mad/config.toml` | Configuration file                                  |
| `~/.mad/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.mad/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.mad/memory/`     | Cross-session memory files and index                |
| `~/.mad/skills/`     | User-scoped skill definitions                       |
| `~/.mad/plugins/`    | User-scoped plugins                                 |
| `~/.mad/agents/`     | User-scoped agent definitions                       |
| `.mad/config.toml`   | Project-scoped config (MCP servers)                 |
| `.mad/skills/`       | Project-scoped skill definitions                    |
| `.mad/plugins/`      | Project-scoped plugins                              |
| `.mad/agents/`       | Project-scoped agent definitions                    |
| `.mad/hooks/`        | Project-scoped hooks                                |
| `.mad/lsp.json`      | LSP server configuration                            |
...
| `MAD_CLI_CHAT_PROXY_BASE_URL`  | Override the cli-chat-proxy URL (default: `https://cli-chat-proxy.mad.com/v1`)                          |
| `MAD_MODELS_BASE_URL`          | Custom base URL for inference. Model list auto-fetched from `{base_url}/models` (see [Custom Models Endpoint](#custom-models-endpoint)) |
| `MAD_MODELS_LIST_URL`          | Override the model list URL if it differs from `{MAD_MODELS_BASE_URL}/models`                                              |
| `MAD_AUTH_PROVIDER_COMMAND`     | External auth binary (alternative to config file). See [External Auth Provider](#external-auth-provider) |
| `MAD_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `MAD_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `MAD_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `MAD_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `MAD_HOME`                     | Override config directory (default: `~/.mad`)                                                           |
| `MAD_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `MAD_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `MAD_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `MAD_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `MAD_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `MAD_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `MAD_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `MAD_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `MAD_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `MAD_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.mad/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `MAD_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `mad` commands and flags.
...
mad completions bash > ~/.local/share/bash-completion/completions/mad
```

Reload your shell or run `source ~/.bashrc`.

Alternative (Mad-managed location):

```bash
mkdir -p ~/.mad/completions/bash
mad completions bash > ~/.mad/completions/bash/mad.bash
```

Add to `~/.bashrc`:

```bash
[[ -r "$HOME/.mad/completions/bash/mad.bash" ]] && source "$HOME/.mad/completions/bash/mad.bash"
```
...
mad completions zsh > ~/.zsh/completions/_mad
```
...
```bash
mkdir -p ~/.mad/completions/zsh
mad completions zsh > ~/.mad/completions/zsh/_mad
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.mad/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `mad` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `mad --debug` or `MAD_LOG_FILE` instead:

```bash
# Per-session debug log (~/.mad/debug/<sessionId>.txt)
mad --debug

# Log to a custom path
MAD_LOG_FILE=/tmp/mad-debug.log mad

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.mad/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `MAD_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
MAD_LOG_FILE=/tmp/mad-debug.log RUST_LOG="info,xai_mad_shell::auth=debug" mad
```
...
cat ~/.mad/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.mad/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.mad/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
