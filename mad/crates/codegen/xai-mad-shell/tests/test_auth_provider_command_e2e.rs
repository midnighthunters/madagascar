...
//! a throwaway `MAD_HOME`, no network, and a provider command that needs no
...
use chrono::Utc;
use xai_mad_shell::auth::{AuthMode, MadAuth, MadComConfig, try_ensure_fresh_auth};

const SEED_TOKEN: &str = "stale-token-that-must-be-replaced";

/// Point the process at a throwaway mad home. `mad_home()` memoizes into a
...
fn seed_expired_credential(home: &Path, scope: &str) {
    ...
    let store: BTreeMap<String, MadAuth> = [(scope.to_owned(), expired)].into_iter().collect();
    std::fs::write(
        home.join("auth.json"),
...
async fn auth_provider_command_mints_the_session_credential() {
    ...
    use_temp_mad_home(home.path());

    // `echo <token>` is valid in both `sh -c` and `cmd /C`, so this phase needs
    // no external binary and runs identically on every platform.
    let token = mint_with_provider(home.path(), "echo mad-ext-token").await;
    assert_eq!(token, "mad-ext-token");
