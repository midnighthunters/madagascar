...
//! `mad -p` exit.
//!
//! `#[ignore]`d by default — needs the mad binary (`MAD_BINARY` or a local
//! debug build):
//! ```bash
//! cargo test -p xai-mad-shell --test test_session_end_hook_e2e -- --ignored
//! ```
//!
//! CI coverage of the same machinery without a built binary lives in
//! `xai_mad_shell::agent::activity` tests (the flush quiesce loop and its
//! grace expiry) and `xai_mad_pager::acp::spawn` tests (the worker join:
...
async fn run_with_session_end_hook() -> (HeadlessResult, MockInferenceServer, tempfile::TempDir) {
    ...
    let hooks_dir = sandbox.mad_home().join("hooks");
