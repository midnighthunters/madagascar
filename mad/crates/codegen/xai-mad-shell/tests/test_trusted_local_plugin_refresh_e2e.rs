...
//! 4. Smoke-validate session JSON under `$MAD_HOME/sessions/` after exit.
//!
//! Requires a built `mad` binary (`MAD_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p xai-mad-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p xai-mad-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
use xai_mad_agent::plugins::SharedPluginRegistryHandle;
use xai_mad_agent::plugins::discovery::DiscoveryConfig;
use xai_mad_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _mad_guard = EnvVarGuard::set("MAD_HOME", &mad_home);
    ...
    let mut registry = InstallRegistry::empty(mad_home.join("installed-plugins"));
    ...
    // mad_home() caches the first MAD_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        xai_mad_agent::plugins::PluginScope::CliOverride
    );
...
#[tokio::test]
#[ignore = "requires pre-built mad binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/MAD_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via mad_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _mad_guard = EnvVarGuard::set("MAD_HOME", &mad_home);
    ...
    // Smoke: session storage under MAD_HOME/sessions has JSON artifacts.
    let sessions_root = mad_home.join("sessions");
    assert!(
