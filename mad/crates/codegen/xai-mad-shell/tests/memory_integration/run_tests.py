...
Launches mad agent stdio in isolated environments ($HOME override)
...
Usage:
    ...
    python3 crates/codegen/xai-mad-shell/tests/memory_integration/run_tests.py

    # Run only fast (no-model) tests:
    python3 crates/codegen/xai-mad-shell/tests/memory_integration/run_tests.py --fast

    # Run a single test by name:
    python3 crates/codegen/xai-mad-shell/tests/memory_integration/run_tests.py test_fts_search_quality

    # Skip build (use pre-built binary):
    MAD_BINARY=/path/to/xai-mad-pager python3 run_tests.py --fast
"""
...
class AcpClient:
    """Talks ACP (JSON-RPC over NDJSON) to a mad agent stdio subprocess."""
...
class IsolatedEnv:
    """Creates a fully isolated mad environment with custom $HOME.

    The agent subprocess gets a fake $HOME with:
      ~/.mad/auth.json   (copied from real home)
      ~/.mad/memory/     (pre-populated by tests)
      ~/.mad/logs/       (memory.log appears here)

    And a workspace directory used as cwd when spawning the agent.
    """

    def __init__(self, workspace_name="test-project"):
        self.root = tempfile.mkdtemp(prefix="mad-memtest-")
        ...
        # Create .mad dirs in fake home
        self.mad_home = os.path.join(self.fake_home, ".mad")
        self.memory_dir = os.path.join(self.mad_home, "memory")
        self.logs_dir = os.path.join(self.mad_home, "logs")
        os.makedirs(self.memory_dir)
        os.makedirs(self.logs_dir)

        # Copy auth from real home
        real_auth = os.path.expanduser("~/.mad/auth.json")
        if os.path.isfile(real_auth):
            shutil.copy2(real_auth, os.path.join(self.mad_home, "auth.json"))

    def write_config(self, toml_str):
        """Write global ~/.mad/config.toml (where the agent reads config)."""
        with open(os.path.join(self.mad_home, "config.toml"), "w") as f:
            f.write(toml_str)
    ...
    def spawn_agent(self, extra_env=None):
        binary = os.environ.get("MAD_BINARY", "mad")
        if not shutil.which(binary) and not os.path.isfile(binary):
            print(f"{R}mad binary not found: {binary}{N}")
            sys.exit(1)
        env = os.environ.copy()
...
def _default_config() -> str:
    embed_model = os.environ.get("MAD_MEMORY_EMBEDDING_MODEL", "").strip()
...
""")
        ...
        client = env.spawn_agent(extra_env={"MAD_MEMORY": "0"})
        init_session(client)
        time.sleep(2)
...
def test_memory_enabled_env_override():
    """MAD_MEMORY=1 env var enables memory even without config."""
    section("13. MAD_MEMORY Env Var Override")
    env = IsolatedEnv()
    try:
        # Config enables memory
        env.write_config(INDEXING_ONLY_CONFIG)
        env.write_global_memory("# Env test content\n* Key value here\n")
        client = env.spawn_agent(extra_env={"MAD_MEMORY": "1"})
        init_session(client)
        time.sleep(3)

        db = env.get_db_path()
        if db:
            ok("index created with MAD_MEMORY=1")
        else:
            fail("no index despite MAD_MEMORY=1 and config enabled")
...
def test_multiple_workspace_isolation():
    ...
    try:
        ...
        # Config is already in global ~/.mad/config.toml from env1.write_config

        # Start agent in workspace beta (reuse env1's fake home)
        binary = os.environ.get("MAD_BINARY", "mad")
        env_vars = os.environ.copy()
        env_vars["HOME"] = env1.fake_home
        env_vars["MAD_MEMORY"] = "1"
        env_vars["RUST_LOG"] = "warn"
        env_vars["MAD_NO_LEADER"] = "1"
...
def build_binary():
    """Build xai-mad-pager with --features dev --release. Returns binary path."""
    repo = find_repo_root()
    if not repo:
        print(f"{R}Could not find repo root (no Cargo.toml + crates/ above cwd){N}")
        sys.exit(1)

    binary = os.path.join(repo, "target", "release", "xai-mad-pager")

    # Find rg for MAD_SHELL_BUNDLE_RG_PATH
    rg = shutil.which("rg")
    if not rg:
    ...
    env = os.environ.copy()
    env["MAD_SHELL_BUNDLE_RG_PATH"] = rg

    print(f"{B}Building xai-mad-pager (release + dev)...{N}")
    result = subprocess.run(
        ["cargo", "build", "-p", "xai-mad-pager", "--features", "dev", "--release"],
        cwd=repo,
        env=env,
    )
...
def main():
    ...
    if "MAD_BINARY" in os.environ:
        binary = os.environ["MAD_BINARY"]
        print(f"Binary: {binary} (from $MAD_BINARY, skipping build)")
    else:
        binary = build_binary()
        os.environ["MAD_BINARY"] = binary
