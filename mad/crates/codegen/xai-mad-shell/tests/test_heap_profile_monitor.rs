...
use xai_mad_shell::auth::{AuthManager, AuthMode, MadAuth, MadComConfig};
use xai_mad_shell::heap_profile::{
...
use xai_mad_test_support::{EnvGuard, MockInferenceServer};
use xai_mad_version::TEST_VERSION_ENV;
...
fn seed_auth_json(home: &Path, token: &str) {
    let scope = MadComConfig::default().auth_scope();
    let auth = MadAuth {
        key: token.to_owned(),
...
fn assert_storage_auth(uploads: &[xai_mad_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
...
impl Harness {
    async fn start(remote: RemoteSettings) -> Self {
        ...
        let auth = Arc::new(AuthManager::new(home.path(), MadComConfig::default()));
