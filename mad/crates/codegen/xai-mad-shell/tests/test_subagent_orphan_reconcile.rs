...
//!
//! This test spawns a real `mad agent stdio` process, seeds an orphaned
...
//! ```bash
//! cargo test -p xai-mad-shell --test test_subagent_orphan_reconcile -- --ignored
//! ```
...
async fn resume_reconciles_orphaned_running_subagent() {
    with_local_set(|| async {
        ...
        // leaves behind.
        let mad_home = shared_sandbox.mad_home().to_path_buf();
        let session_dir = locate_session_dir(&mad_home, session_id.0.as_ref());
