...
//!
//! Runs the built mad binary against the mock inference server with a
//! caller-owned `$MAD_HOME`, then inspects `~/.mad/debug/`:
...
//!   `~/.mad/debug/<sessionId>.txt` with first-party content, and does NOT enable
//!   sampling/instrumentation. Regression for the master switch having bundled
//!   `MAD_LOG_SAMPLING`/`MAD_INSTRUMENTATION`, whose global `TargetFilterLayer`
...
//! - a live `agent` session (explicit `MAD_DEBUG_LOG=1`) writes a per-session
//!   `~/.mad/debug/<sessionId>.txt` with real first-party content + `latest.txt`.
//! - `--debug-file <path>` writes one explicit file and bypasses per-session
//!   routing entirely (no `~/.mad/debug/` files).
//! - `MAD_LOG_FILE=<path>` writes that explicit file (back-compat single file).
//!
...
//! ```bash
//! cargo test -p xai-mad-shell --test test_debug_logging -- --ignored
//! ```
...
/// The per-session firehose directory under a pinned `$MAD_HOME`.
fn debug_dir(home: &Path) -> PathBuf {
    home.join(".mad").join("debug")
}

/// List firehose `*.txt` files under `~/.mad/debug` (excluding the `latest.txt`
...
/// Build a headless `mad -p` command with a pinned `$MAD_HOME` so the firehose
/// lands under `<home>/.mad/debug`. Firehose env knobs are cleared so the test
...
fn debug_cmd(
    ...
    sandbox
        ...
        .set_env("MAD_HOME", home.join(".mad"));
...
/// assert it carries first-party (`xai_mad`) content. Panics with the captured
/// stderr tail if it never fills. Shared by the live-agent tests.
async fn read_session_firehose_when_ready(path: &Path, client: &MadStdioClient) -> String {
...
/// headless `mad -p` client is near-silent, so its lazily-opened firehose may
...
async fn debug_flag_enables_firehose_without_crashing() {
    ...
    assert_headless_success(&result, "mad --debug headless", Some(&server));
...
async fn no_debug_flag_writes_no_debug_dir() {
    ...
    assert_headless_success(&result, "mad headless (no --debug)", Some(&server));
    assert!(
...
/// A live `agent` session writes `~/.mad/debug/<sessionId>.txt` with real
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        sandbox.set_env("MAD_DEBUG_LOG", "1");
        let mad_home = sandbox.mad_home().to_path_buf();

        let client =
            MadStdioClient::spawn_with_sandbox(&server, workdir.workspace(), sandbox).await;
        ...
        let session_file = mad_home.join("debug").join(format!("{sid}.txt"));
        ...
        // file, so `tail -f ~/.mad/debug/latest.txt` follows the live session.
        #[cfg(unix)]
        {
            let link = mad_home.join("debug").join("latest.txt");
...
/// The `--debug` FLAG (not `MAD_DEBUG_LOG` directly) drives the firehose end to
/// end through the master switch. Regression: the master switch used to also set
/// `MAD_LOG_SAMPLING`/`MAD_INSTRUMENTATION`, whose `TargetFilterLayer` globally
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // Drive `mad --debug agent stdio`: the master switch (which runs before
        // the agent dispatch) must be what enables the firehose — NOT a direct
        // MAD_DEBUG_LOG env. The sandbox baseline excludes inherited firehose
        ...
        // clears MAD_LOG_SAMPLING (hermetic), so the sampling layer stays off and
        // `~/.mad/logs/sampling.jsonl` is never written — the `--debug`
        ...
        // MAD_INSTRUMENTATION=disabled, so that assertion would be vacuous.
        let sampling = mad_home.join("logs").join("sampling.jsonl");
...
/// `--debug-file <path>` writes one explicit file and bypasses per-session
/// routing entirely (no `~/.mad/debug/` files created).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn debug_file_flag_writes_single_file_and_bypasses_routing() {
    ...
    assert_headless_success(&result, "mad --debug-file", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        explicit.exists(),
...
/// `MAD_LOG_FILE=<path>` (no `--debug`) writes that exact file (back-compat).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn mad_log_file_explicit_path_is_written() {
    let server = MockInferenceServer::start()
        .await
        .expect("start mock server");
    let workdir = git_workdir();
    ...
    sandbox.set_env("MAD_LOG_FILE", &custom);
    let result = run_headless_in_sandbox(cmd, sandbox).await;

    assert_headless_success(&result, "mad MAD_LOG_FILE=path", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        custom.exists(),
        "explicit MAD_LOG_FILE path not written: {custom:?}\nstderr tail:\n{}",
        stderr_tail(&result.stderr, 800)
    );
    // Single-file mode bypasses per-session routing.
    assert!(
        firehose_txt_files(home.path()).is_empty(),
        "MAD_LOG_FILE must bypass per-session routing, found: {:?}",
        firehose_txt_files(home.path())
    );
}
