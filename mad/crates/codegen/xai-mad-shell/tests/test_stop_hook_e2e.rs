...
//! default since they need the mad binary (`MAD_BINARY` or a local debug build):
//! ```bash
//! cargo test -p xai-mad-shell --test test_stop_hook_e2e -- --ignored
//! ```
...
async fn stop_block_loop_ends_at_continuation_cap() {
    ...
    assert_eq!(
        run.invocations(),
        xai_mad_shell::session::MAX_STOP_HOOK_CONTINUATIONS_PER_TURN,
