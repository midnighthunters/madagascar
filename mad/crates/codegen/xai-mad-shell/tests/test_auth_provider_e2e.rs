...
//!
//! Runs the built mad binary headless against the mock inference server with
...
//! the release artifact via `MAD_BINARY`; run locally (auto-builds the pager):
//! ```bash
//! cargo test -p xai-mad-shell --test test_auth_provider_e2e -- --ignored
//! ```
//!
...
async fn provider_backed_model_sends_minted_token_on_the_wire() {
    ...
    sandbox.remove_env("MAD_LEADER_SOCKET");

    let mad_home = sandbox.mad_home().to_path_buf();
    std::fs::create_dir_all(&mad_home).expect("create .mad home");

    let counter = mad_home.join("mint-count");
    let helper = mad_home.join("mock-auth.sh");
    std::fs::write(
        &helper,
        format!(
...
async fn provider_with_args_and_json_output_sends_minted_token() {
    ...
    let seen_args = mad_home.join("seen-args");
    let helper = mad_home.join("mock-auth-json.sh");
    std::fs::write(
        &helper,
        format!(
            "#!/bin/sh\nprintf '%s' \"$*\" > {}\n\
