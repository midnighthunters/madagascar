...
//!
//! ```bash
//! LEADER_SOAK_SECS=1200 cargo test -p xai-mad-shell --features test-support \
...
async fn leader_soak_churning_clients_no_leaks_no_zombies() {
    ...
    let sock_path = mad_home.path().join("leader-soak.sock");
    ...
    local
        .run_until(async {
            ...
            xai_mad_shell::leader::in_process::spawn_agent(acp_rx, response_tx.clone());
