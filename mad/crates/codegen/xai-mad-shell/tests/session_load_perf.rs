...
//! [`synth`](xai_mad_shell::session::testkit::synth) generator (redundant
//! `available_commands_update` + big rewind snapshots; size knobs via
//! `MAD_PERF_*`), or a real session dir via `MAD_PERF_SESSION_SRC=<session-dir>`.
//!
//! Run (needs the `test-support` feature; on by default under Bazel):
//!   cargo test -p xai-mad-shell --features test-support --test session_load_perf -- --nocapture
//!   cargo test -p xai-mad-shell --features test-support --test session_load_perf full_session_load_e2e -- --ignored --nocapture
...
use xai_mad_shell::session::info::Info;
use xai_mad_shell::session::storage::{
    JsonlStorageAdapter, StorageAdapter, load_updates_for_replay_at,
};
use xai_mad_shell::session::testkit::e2e::load_session_via_agent;
use xai_mad_shell::session::testkit::synth::{self, SessionSpec};

// ───────────────────────── session spec ─────────────────────────

/// Perf-tool defaults over the shared [`SessionSpec`], tuned to the pathological
/// real session; scale/override via `MAD_PERF_*` (e.g. `MAD_PERF_TURNS`,
/// `MAD_PERF_SCALE`), or point `MAD_PERF_SESSION_SRC` at a real session dir.
...
/// `MAD_PERF_SESSION_SRC` set, copy a real session over a registered stub
...
async fn prepare_session(root: &Path, cwd: &Path, spec: &SessionSpec) -> (Info, PathBuf) {
    let Ok(src) = std::env::var("MAD_PERF_SESSION_SRC") else {
...
fn generate_or_restore_rewind(path: &Path, spec: &SessionSpec) {
    if std::env::var("MAD_PERF_SESSION_SRC").is_ok() {
        return;
    }
...
/// `--ignored` (optionally `MAD_PERF_SESSION_SRC=...`) to get the numbers.
...
async fn full_session_load_e2e() {
    ...
    let cwd = TempDir::new().unwrap();
    let spec = perf_spec();
    let instr_log = mad_home.path().join("instr.jsonl");

    // SAFETY: single-threaded current-thread runtime; set before any agent code
    // reads these process-globals (mad_home()/instrumentation mode are OnceLock).
    unsafe {
        std::env::set_var("MAD_HOME", mad_home.path());
        std::env::set_var("MAD_INSTRUMENTATION", "log");
        std::env::set_var("MAD_INSTRUMENTATION_LOG", &instr_log);
        std::env::set_var("MAD_CLI_CHAT_PROXY_BASE_URL", server.url());
        std::env::set_var("MAD_XAI_API_BASE_URL", server.url());
        std::env::set_var("XAI_API_KEY", "test-key-for-ci");
        std::env::set_var("MAD_TELEMETRY_ENABLED", "false");
        std::env::set_var("MAD_FEEDBACK_ENABLED", "false");
        std::env::set_var("MAD_TRACE_UPLOAD", "false");
    }
    ...
    let _ = tracing_subscriber::registry()
        .with(xai_mad_shell::instrumentation::layer::<Registry>())
        .try_init();

    let (info, dir) = prepare_session(mad_home.path(), cwd.path(), &spec).await;
    let updates_path = dir.join("updates.jsonl");
    let rewind_path = dir.join("rewind_points.jsonl");
    eprintln!(
    ...
    local
        .run_until(async move {
            ...
            let _ = xai_mad_shell::instrumentation::finalize();
