...
/// Shared `MAD_HOME` for the entire test binary. The `OnceLock` in
/// `xai-mad-config` only allows one value per process, so all tests share
...
async fn mcp_tool_grant_persists_and_short_circuits_next_request() {
    run_actor_test(ClientType::MadPager, |handle, gw, cwd| async move {
...
async fn fallback_client_plain_allow_always_persists_mcp_tool() {
    // Regression: Generic / MadWeb / Extension clients
...
async fn dont_ask_policy_denies_without_prompting() {
    use xai_mad_workspace::permission::types::{PermissionConfig, PromptPolicy};
