...
//! Run locally:
//! ```bash
//! cargo test -p xai-mad-shell --test test_settings_refresh
//! ```
...
async fn test_fetch_settings_blocking_round_trip() {
    with_local_set(|| async {
        ...
        let auth = xai_mad_shell::auth::MadAuth {
            key: "test-key".into(),
            ..Default::default()
        };
        let result = tokio::task::spawn_blocking({
            ...
            let auth = auth.clone();
            move || xai_mad_shell::remote::fetch_settings_blocking(&url, &auth, None).into_option()
        })
        .await
        .unwrap();
        assert!(
...
async fn test_doom_loop_recovery_settings_round_trip() {
    use xai_mad_shell::util::config::DoomLoopRecoverySettings;
