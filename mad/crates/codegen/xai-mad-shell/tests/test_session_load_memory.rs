...
//!
//! Run:
//!   cargo test -p xai-mad-shell --features dhat-heap,test-support --test test_session_load_memory \
...
use xai_mad_shell::session::storage::{JsonlStorageAdapter, StorageAdapter, prepare_replay_lines};
use xai_mad_shell::session::testkit::synth::{self, SessionSpec};
use xai_mad_test_support::env::env_parse;
...
mod rss {
    ...
    use xai_mad_test_support::resources::ResourceSnapshot;
    ...
    async fn count_replayed_notifications(session_id: acp::SessionId, cwd: PathBuf) -> u64 {
        ...
        local
            .run_until(async move {
                ...
                let loaded = xai_mad_shell::session::testkit::e2e::load_session_via_agent(
    ...
    async fn session_load_e2e_peak_rss() {
        ...
        let (info, dir) = synth::prepare_session(mad_home.path(), cwd.path(), &opts).await;
