...
//! Backend environment presets for the Madagascar CLI crate family: endpoint URL
//! defaults, environment selection, and env-var test support.
//!
//! Public builds expose production endpoints. Values resolve as a `MAD_*`
//! env-var override when set, else the compiled production default.
/// The endpoint set for one backend environment.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct MadagascarBuildEndpoints {
...
const PRODUCTION_ENDPOINTS: MadagascarBuildEndpoints = MadagascarBuildEndpoints {
    cli_chat_proxy_base_url: "https://cli-chat-proxy.mad.com/v1",
    asset_server_url: "https://assets.mad.com",
    relay_ws_url: "wss://code.mad.com/ws/code-agent",
    gateway_ws_url: "wss://mad.com/ws/gw/",
    ws_origin: "https://mad.com",
};
...
pub enum MadagascarBuildEnvironment {
    #[default]
    Production,
}
impl MadagascarBuildEnvironment {
    pub fn from_flags(_dev: bool, _staging: bool) -> Self {
        MadagascarBuildEnvironment::Production
    }
    /// Indicator string for display; `None` for Production.
    pub fn indicator(&self) -> Option<&'static str> {
        match self {
            MadagascarBuildEnvironment::Production => None,
        }
    }
    pub fn is_production(&self) -> bool {
        matches!(self, MadagascarBuildEnvironment::Production)
    }
    fn env_prefix(&self) -> &'static str {
        match self {
            MadagascarBuildEnvironment::Production => "MAD_PRODUCTION",
        }
    }
    /// Compiled endpoint set for this environment (production by default).
    pub fn endpoints(&self) -> MadagascarBuildEndpoints {
        match self {
            MadagascarBuildEnvironment::Production => PRODUCTION_ENDPOINTS,
        }
    }
    ...
    /// The relay WebSocket URL (Web Frontend at `mad.com/code` driving a
    ...
    /// `MAD_GATEWAY_URL` opt-in takes precedence.
    pub fn gateway_ws_url(&self) -> String {
        self.resolve("_GATEWAY_WS_URL", self.endpoints().gateway_ws_url)
    }
}
impl std::fmt::Display for MadagascarBuildEnvironment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            MadagascarBuildEnvironment::Production => write!(f, "production"),
        }
    }
}
...
mod tests {
    ...
    fn test_env_prefix() {
        assert_eq!(
            MadagascarBuildEnvironment::Production.env_prefix(),
            "MAD_PRODUCTION"
        );
    }
    #[test]
    fn env_var_guard_set_value_updates_then_restores_on_drop() {
        const KEY: &str = "XAI_MAD_ENV_VAR_GUARD_SET_VALUE_PROBE";
    ...
    /// loop mistakenly connecting to `wss://mad.com/ws/gw/`).
    #[test]
    fn relay_and_gateway_urls_are_distinct() {
        assert_ne!(
            MadagascarBuildEnvironment::Production.relay_ws_url(),
            MadagascarBuildEnvironment::Production.gateway_ws_url(),
        );
    }
    #[test]
    fn test_from_flags() {
        assert_eq!(
            MadagascarBuildEnvironment::from_flags(false, false),
            MadagascarBuildEnvironment::Production
        );
    }
}
