...
//!
//! Extracted from `xai-mad-shell/src/agent/subagent/` `resolve_effective_overrides()`.

use std::collections::HashMap;
use std::path::Path;

use serde::de::DeserializeOwned;
use xai_mad_tools::implementations::mad_build::task::types::SubagentRuntimeOverrides;
...
mod tests {
    use super::*;
    use xai_mad_tools::implementations::mad_build::task::types::ModelOverrideProvenance;
    ...
    fn explicit_model_overrides_role() {
        let overrides = make_overrides(Some("mad-light"), None, None, None, None);
        let role = SubagentRole {
            model: Some("mad-3".into()),
            ..Default::default()
        };
        let result =
            resolve_effective_overrides(&overrides, Some(&role), &empty_personas(), None, None);
        assert_eq!(result.model.as_deref(), Some("mad-light"));
    }
    ...
    fn role_model_used_when_no_explicit() {
        ...
        let result =
            resolve_effective_overrides(&overrides, Some(&role), &empty_personas(), None, None);
        assert_eq!(result.model.as_deref(), Some("mad-3"));
    }
    ...
    fn persona_not_found_error_is_non_fatal() {
        ...
        let overrides = make_overrides(Some("mad-3"), Some("missing"), None, None, None);
        let role = SubagentRole {
            model: Some("mad-light".into()),
            default_isolation: Some("worktree".into()),
            ..Default::default()
        };
        let result =
    ...
    fn persona_file_error_returns_early_with_defaults() {
        ...
        let overrides = make_overrides(Some("mad-3"), Some("broken"), None, None, None);
        let dir = tempfile::tempdir().unwrap();
        let mut personas = HashMap::new();
        personas.insert(
            "broken".to_string(),
