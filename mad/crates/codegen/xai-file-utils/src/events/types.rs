...
pub enum Event {
    ...
    /// `TODO_GATE_*` discriminator constant in `xai-mad-shell::session::events`.
    TodoGateFired {
        fires: u32,
        pending: usize,
        in_progress: usize,
    ...
    /// constants in `xai-mad-shell::session::events`.
    ...
    /// in `xai-mad-shell::session::events`.
    ...
    /// discriminator constants in `xai-mad-shell::session::events`.
    ...
    /// `xai-mad-shell::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `xai-mad-shell::session::goal_stop_detector::PATTERN_LABELS`;
...
/// Telemetry mirror of `xai-mad-shell`'s `GoalClassifierVerdict`. Two
/// crates due to the orphan rule; the conversion lives in
/// `xai-mad-shell/src/session/events.rs`.
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "snake_case")]
...
/// Telemetry mirror of `xai-mad-shell`'s `GoalPauseReason`. The two types
