...
const EXCLUDED_DIR_NAMES: &[&str] = &[
    ".mad", ".cache", ".daemon", ".config", ".npm", ".cargo", ".rustup", ".vscode", ".gemini",
    ".hermes", ".claude",
];
...
mod tests {
    ...
    mod config_and_cache {
        ...
        #[test]
        fn mad_dirs_are_unsafe() {
            if let Some(home) = dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".mad")));
                assert!(!is_project_dir(&home.join(".mad/bin")));
            }
        }

        #[test]
        fn mad_prefixed_dirs_are_unsafe() {
            if let Some(home) = dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".mad-proxy-work")));
            }
        }
