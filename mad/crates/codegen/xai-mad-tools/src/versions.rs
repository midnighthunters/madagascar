...
pub struct ToolVersionEntry {
    /// Fully-qualified tool ID (e.g. `"MadagascarBuild:run_terminal_cmd"`).
...
pub struct PresetEntry {
    ...
    /// Keys are fully-qualified tool IDs (e.g. `"MadagascarBuild:run_terminal_cmd"`).
...
/// between namespaces (e.g. `MadagascarBuild:run_terminal_cmd` vs.
...
pub const TOOL_VERSION_REGISTRY: &[ToolVersionEntry] = &[
    ToolVersionEntry {
        fq_tool_id: "MadagascarBuild:run_terminal_cmd",
        versions: &[V_CURRENT, V_LEGACY_BASH],
    },
    ToolVersionEntry {
        fq_tool_id: "MadagascarBuild:read_file",
        versions: &[V_CURRENT, V_LEGACY_READ_FILE],
    },
    ToolVersionEntry {
        fq_tool_id: "MadagascarBuild:search_replace",
        versions: &[V_CURRENT, V_LEGACY_SEARCH_REPLACE],
    },
    ToolVersionEntry {
        fq_tool_id: "MadagascarBuild:list_dir",
        versions: &[V_CURRENT, V_LEGACY_LIST_DIR],
    },
    ToolVersionEntry {
        fq_tool_id: "MadagascarBuild:grep",
        versions: &[V_CURRENT], // Managed but no legacy implementation.
    },
    ToolVersionEntry {
        fq_tool_id: "MadagascarBuild:kill_task",
        versions: &[V_CURRENT, V_LEGACY_KILL_TASK],
    },
    ToolVersionEntry {
        fq_tool_id: "MadagascarBuild:get_task_output",
...
pub const PRESETS: &[PresetEntry] = &[
    ...
    PresetEntry {
        ...
        lifecycle: BehaviorLifecycle::Active,
        tool_defaults: &[
            ("MadagascarBuild:run_terminal_cmd", "legacy-0.4.10"),
            ("MadagascarBuild:read_file", "legacy-0.4.10"),
            ("MadagascarBuild:search_replace", "legacy-0.4.10"),
            ("MadagascarBuild:get_task_output", "legacy-0.4.10"),
            ("MadagascarBuild:kill_task", "legacy-0.4.10"),
            ("MadagascarBuild:list_dir", "legacy-0.4.10"),
        ],
    },
    ...
    PresetEntry {
        ...
        tool_defaults: &[
            ("MadagascarBuild:run_terminal_cmd", "current"),
            ("MadagascarBuild:read_file", "current"),
            ("MadagascarBuild:search_replace", "current"),
            ("MadagascarBuild:list_dir", "current"),
            ("MadagascarBuild:grep", "current"),
            ("MadagascarBuild:kill_task", "current"),
            ("MadagascarBuild:get_task_output", "current"),
        ],
    },
];
...
mod tests {
    ...
    fn current_preset_resolves_to_current() {
        let v = resolve_version("current", "MadagascarBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_resolves_ported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "MadagascarBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_falls_back_to_current_for_unported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "MadagascarBuild:grep", None).unwrap();
        assert_eq!(v, Some("current".to_string()));
    }
    ...
    fn unmanaged_tool_returns_none() {
        let v = resolve_version("current", "MadagascarBuild:web_search", None).unwrap();
        assert_eq!(v, None);
    }
    ...
    fn override_on_unmanaged_tool_errors() {
        let err =
            resolve_version("current", "MadagascarBuild:web_search", Some("legacy-0.4.10")).unwrap_err();
        assert!(err.contains("unmanaged tool"));
    }

    #[test]
    fn unknown_preset_errors() {
        let err = resolve_version("nonexistent", "MadagascarBuild:run_terminal_cmd", None).unwrap_err();
    ...
    fn unknown_override_version_errors() {
        let err =
            resolve_version("current", "MadagascarBuild:run_terminal_cmd", Some("v999")).unwrap_err();
        assert!(
    ...
    fn concise_namespace_not_managed() {
        // MadagascarBuildConcise tools should NOT be version-managed.
        assert!(!is_version_managed("MadagascarBuildConcise:run_terminal_cmd"));
        let v = resolve_version("current", "MadagascarBuildConcise:run_terminal_cmd", None).unwrap();
        assert_eq!(v, None);
    }

    #[test]
    fn is_version_managed_matches_catalog() {
        assert!(is_version_managed("MadagascarBuild:run_terminal_cmd"));
        assert!(is_version_managed("MadagascarBuild:read_file"));
        assert!(is_version_managed("MadagascarBuild:search_replace"));
        assert!(is_version_managed("MadagascarBuild:list_dir"));
        assert!(is_version_managed("MadagascarBuild:grep"));
        assert!(is_version_managed("MadagascarBuild:kill_task"));
        // Not managed:
        assert!(!is_version_managed("MadagascarBuild:todo_write"));
    }
    ...
    fn active_version_produces_no_warning() {
        let res =
            resolve_version_with_warnings("current", "MadagascarBuild:run_terminal_cmd", None).unwrap();
        assert!(
    ...
    fn active_bundle_produces_no_bundle_level_warning() {
        let res =
            resolve_version_with_warnings("legacy-0.4.10", "MadagascarBuild:run_terminal_cmd", None)
                .unwrap();
        assert!(
    ...
    fn deprecated_version_produces_tool_warning() {
        ...
        let (version, warnings) =
            validate_and_resolve("MadagascarBuild:run_terminal_cmd", "current").unwrap();
        ...
        let err = validate_and_resolve("MadagascarBuild:grep", "legacy-0.4.10").unwrap_err();
    ...
    fn resolve_with_warnings_unmanaged_returns_none_no_warnings() {
        let res = resolve_version_with_warnings("current", "MadagascarBuild:web_search", None).unwrap();
    ...
    fn resolve_with_warnings_unsupported_version_errors_with_list() {
        let err = resolve_version_with_warnings("current", "MadagascarBuild:grep", Some("legacy-0.4.10"))
            .unwrap_err();
        assert!(
            err.contains("is not supported for tool"),
        ...
        assert!(
            err.contains("MadagascarBuild:grep"),
