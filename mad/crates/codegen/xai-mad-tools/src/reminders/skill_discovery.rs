...
//! after every tool call to check for SKILL.md files in `.mad/skills/`,
...
/// Directories that contain skill definitions (`.mad/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".mad", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.mad/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.mad/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.mad/) are outside the git root so the walk breaks early.
