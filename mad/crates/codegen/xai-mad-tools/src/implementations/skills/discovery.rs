...
///
/// `skills` is the standard layout (`.mad/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into Madagascar Build sessions.
...
/// name (e.g. `~/.mad/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into Madagascar Build.
...
/// `cwd` (exclusive). At each directory, checks for `.mad/skills/`,
...
/// visits known config roots (`.mad`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.mad` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".mad", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.mad is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.mad/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.mad/ → kept (user content).
        let mad_shell = tmp.path().join(".mad").join("skills").join("shell");
        std::fs::create_dir_all(&mad_shell).unwrap();
        std::fs::write(
            mad_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (mad_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.mad/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .mad skill in an intermediate dir.
        ...
        .unwrap();
        let mad_skill = sub.join(".mad").join("skills").join("mad-dyn");
        std::fs::create_dir_all(&mad_skill).unwrap();
        std::fs::write(mad_skill.join("SKILL.md"), "---\nname: mad-dyn\n---\n").unwrap();
        ...
        assert!(
            names_on.contains(&"mad-dyn"),
            "mad-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only mad-dyn discovered.
        ...
        assert!(
            names_off.contains(&"mad-dyn"),
            "mad-dyn missing: {names_off:?}"
        );
        assert!(
