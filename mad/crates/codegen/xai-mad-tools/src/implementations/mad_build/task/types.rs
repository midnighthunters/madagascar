...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"mad-build-plan"`) whose `AgentDefinition` decides the child's harness
...
/// defense-in-depth check in `xai-mad-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "MadagascarBuild:run_terminal_cmd" | "MadagascarBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
register_resource!("mad_build", "SubagentEventSender", SubagentEventSender);
...
register_resource!("mad_build", "SubagentDepthCounter", SubagentDepthCounter);

/// Host-injected max nesting depth; absent → [`super::MAX_SUBAGENT_DEPTH`].
#[derive(Debug, Clone, Copy)]
pub struct MaxSubagentDepth(pub u32);

register_resource!("mad_build", "MaxSubagentDepth", MaxSubagentDepth);
...
register_resource!("mad_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("mad_build", "SessionIdResource", SessionIdResource);
...
///
/// Set by xai-mad-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by xai-mad-shell at turn start.
...
register_resource!("mad_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("MadagascarBuild:run_terminal_cmd", ToolKind::Execute),
                tc("MadagascarBuild:read_file", ToolKind::Read),
                tc("MadagascarBuild:list_dir", ToolKind::List),
                tc("MadagascarBuild:grep", ToolKind::Search),
                tc("MadagascarBuild:kill_task", ToolKind::KillTaskAction),
                tc("MadagascarBuild:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_only_filter_keeps_background_task_tools_when_task_tool_remains() {
        let mut config = ToolServerConfig {
            tools: vec![
                ...
                tc("MadagascarBuild:kill_task", ToolKind::KillTaskAction),
                tc("MadagascarBuild:get_task_output", ToolKind::BackgroundTaskAction),
                tc("MadagascarBuild:task", ToolKind::Task),
            ],
            behavior_preset: None,
        };
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("MadagascarBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
