...
mod versions;
use crate::types::schema::MadIntegerSchema;
...
crate::register_resource!("mad_build", "ReadFile", ReadFileParams);
...
/// an empty window (harness-aligned; not a Mad-line clamp).
...
";
    ...
    async fn skill_file_ignores_offset_and_limit() {
        let tmp = TempDir::new().unwrap();
        let skill_dir = tmp.path().join(".mad/skills/commit");
        ...
        let resources = test_resources(tmp.path());
        let input = ReadFileInput {
            path: ".mad/skills/commit/SKILL.md".to_string(),
            offset: Some(3),
            limit: Some(1),
            pages: None,
            format: None,
    ...
    async fn md_in_skills_dir_ignores_model_offset_and_limit() {
        let tmp = TempDir::new().unwrap();
        let skill_dir = tmp.path().join(".mad/skills/my-skill");
        std::fs::create_dir_all(&skill_dir).unwrap();
        let content = (1..=1200)
            .map(|n| format!("line{n}"))
            .collect::<Vec<_>>()
        ...
        let resources = test_resources(tmp.path());
        let input = ReadFileInput {
            path: ".mad/skills/my-skill/reference.md".to_string(),
            offset: Some(3),
            limit: Some(1),
            pages: None,
            format: None,
    ...
    /// phantom field only (start past any `split_inclusive` line), so Mad
