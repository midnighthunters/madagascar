...
//!
//! - **`xai-mad-tools`** — tool blocks on a oneshot, formats the result.
//! - **`xai-mad-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`xai-mad-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `xai-mad-tools`.
...
register_resource!("mad_build", "UserQuestionSender", UserQuestionSender);
