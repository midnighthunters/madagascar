...
/// params are injected, `MAD_ASK_USER_QUESTION_TIMEOUT_SECS` (positive
/// integer seconds) still overrides this default directly —
/// e.g. `MAD_ASK_USER_QUESTION_TIMEOUT_SECS=8` for tests / TUI repro.
...
pub const RESPONSE_TIMEOUT_ENV: &str = "MAD_ASK_USER_QUESTION_TIMEOUT_SECS";
...
crate::register_resource!("mad_build", "AskUserQuestion", AskUserQuestionParams);
...
pub struct QuestionOption {
    ...
    /// Opaque id; hidden from the model. Mad callers leave it `None`.
...
/// session-owned coordinator (in xai-mad-shell), which performs an ACP
