...
const XAI_IMAGINE_MODEL: &str = "mad-imagine-image-quality";
// Some Imagine models (e.g. `mad-imagine-image`, selectable via `model_override`)
...
pub use xai_mad_tools_api::slash_commands::{
...
/// SuperMad upsell modal instead; this covers the natural-language path.
pub(crate) const TIER_RESTRICTED_UPSELL: &str = "Image generation is a SuperMad feature and isn't available on the free or X Basic tier. Let the user know they can unlock image and video generation by upgrading to SuperMad: https://mad.com/supermad?referrer=mad-build. Do not retry this tool.";
...
pub struct ImageGenClient {
    ...
    /// HTTP call and return the SuperMad upsell prose instead. See
...
impl ImageGenClient {
    ...
    /// with the SuperMad upsell instead of issuing a doomed request.
...
pub enum ImageGenConfig {
    ...
    Enabled {
        ...
        /// SuperMad upsell prose instead of a doomed request. Set by the
...
pub const SESSION_ID_HEADER: &str = "x-mad-session-id";
...
mod tests {
    ...
    fn per_tool_gates_are_independent() {
        let cfg = ImageGenConfig::Enabled {
            ...
            image_edit_enabled: true,
            model_override: Some("mad-imagine-image".into()),
        ...
        assert_eq!(cfg.model_override(), Some("mad-imagine-image"));
    ...
    fn client_selects_model_from_override() {
        ...
        assert_eq!(
            ImageGenClient::new(&mk(Some("mad-imagine-image")), None)
                .unwrap()
                .model,
            "mad-imagine-image"
        );
    ...
    fn client_selects_edit_model_from_override() {
        ...
        let client = ImageGenClient::new(&mk(Some("mad-imagine-image-v2")), None).unwrap();
        assert_eq!(client.edit_model(), "mad-imagine-image-v2");
    ...
    async fn tier_restricted_short_circuits_with_upsell() {
        // A free / X Basic user's image_gen call returns the SuperMad upsell
        ...
        match result {
            ToolOutput::Text(t) => {
                assert!(t.text.contains("SuperMad"), "got: {}", t.text);
                assert!(t.text.contains("supermad?referrer=mad-build"));
            }
