...
//! `LspTool` (Tool trait impl) under the `MadagascarBuild` namespace.
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    xai_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.mad/lsp.json or <cwd>/.mad/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
