//! LSP server configuration from `.mad/lsp.json`.
...
/// This is the canonical merge function — both session startup and `mad inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::mad_home::mad_home().join("lsp.json");
    let project_path = cwd.join(".mad").join("lsp.json");
...
/// Load LSP server configs from `~/.mad/lsp.json` and `<cwd>/.mad/lsp.json`.
