...
use crate::implementations::mad_build::todo::{TodoItem, TodoState};
...
/// protocol boundary in `xai-mad-shell`.
#[derive(Debug, Clone, Serialize, Deserialize, schemars::JsonSchema)]
...
pub struct BashOutput {
    ...
    /// Set by the mad_build `run_terminal_cmd` implementation when the
    /// command was detected as a bare `echo "<msg>"` (or close variant:
    /// echo -n, echo -e, simple printf for literal output, etc.).
    ///
    /// Used for:
    /// - Telemetry / statistics on this pattern for the mad_build backend.
...
pub struct WebFetchContent {
    ...
    pub bytes: usize,
    /// Internal path to the complete converted body when MadagascarBuild persisted overflow.
    #[serde(skip)]
    #[schemars(skip)]
...
pub enum ToolOutput {
    ...
    Monitor(crate::implementations::mad_build::monitor::types::MonitorOutput),
    SchedulerCreate(crate::implementations::mad_build::scheduler::create::SchedulerCreateOutput),
    SchedulerDelete(crate::implementations::mad_build::scheduler::delete::SchedulerDeleteOutput),
    SchedulerList(crate::implementations::mad_build::scheduler::list::SchedulerListOutput),
    UpdateGoal(crate::implementations::mad_build::update_goal::UpdateGoalOutput),
    Workflow(crate::implementations::mad_build::workflow::WorkflowToolOutput),
...
mod tests {
    use super::*;
    use crate::implementations::mad_build::todo::{TodoPriority, TodoStatus};
    use serde_json::json;
    ...
    fn subagent_completed_prompt_format_with_worktree() {
        let output = ToolOutput::SubagentCompleted(SubagentCompletedOutput {
            ...
            turns: 1,
            duration_ms: 2000,
            worktree_path: Some("/tmp/mad-worktree/wt-agent".into()),
            persona: None,
        ...
        let rendered = output.to_prompt_format();
        assert!(
            rendered.contains("<worktree_path>/tmp/mad-worktree/wt-agent</worktree_path>"),
