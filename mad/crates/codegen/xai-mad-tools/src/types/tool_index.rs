...
//!
//! `ToolSearchIndex` is defined in `xai-mad-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `xai-mad-shell`
...
/// in `xai-mad-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `xai-mad-shell` after MCP initialization.
#[derive(Clone)]
