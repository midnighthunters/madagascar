...
//!
//! `MemoryBackend` is defined in `xai-mad-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `xai-mad-tools`.
/// Mirrors `xai_mad_shell::session::memory_log::TARGET`.
