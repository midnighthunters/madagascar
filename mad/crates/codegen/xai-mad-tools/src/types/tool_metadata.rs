//! `ToolMetadata` — mad-tools-specific metadata for tools.
//!
...
/// Mad-tools-specific metadata trait.
///
...
pub trait ToolMetadata: Send + Sync {
    ...
    /// Namespace grouping (MadagascarBuild, Cursor, OpenCode, ...).
    /// Used to build the fully-qualified tool ID at registration time
    /// (e.g., `"MadagascarBuild:grep"`).
