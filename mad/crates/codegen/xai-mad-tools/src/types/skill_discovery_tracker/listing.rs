...
impl<'a> SkillEntry<'a> {
    ...
    // ── Budgeted XML rendering (madagascar build harness) ─────────────
...
///
/// `"/path/.mad/skills/my-skill/SKILL.md"` -> `"/path/.mad/skills/"`
...
/// Used by the budgeted (madagascar build) XML rendering path.
...
pub enum XmlRenderMode {
    ...
    Verbatim,
    /// Mad build harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
        ...
        skills
            ...
            .filter(|s| {
                s.enabled
                    ...
                    // Compat mode renders all skills verbatim; only the mad
...
mod tests {
    ...
    fn verbatim_bypasses_is_listable_filter() {
        // mad-build drops a description-less skill; compat mode renders all.
    ...
    // ── budgeted mode: madagascar build harness (budgeted XML) ───────────
