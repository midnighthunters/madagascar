...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.mad/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.mad/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.mad/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.mad/ or ~/.claude/.
    User { path: PathBuf },
