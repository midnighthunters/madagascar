//! Canonical external-settings tool name ↔ Mad tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`xai-mad-hooks`) needs the
//! Mad tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`xai-mad-agent`) needs the [`ToolKind`] a `tools:`
...
//!
//! The `mad` names are test-checked against the live registry.

use super::tool::ToolKind;
use ToolKind::*;

/// One Claude tool's correspondence to Mad, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// Mad [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// Mad tool names this Claude tool maps to (empty when there is no direct
    /// Mad tool — the entry then only contributes a `kind`).
    mad: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, mad: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        mad,
...
const fn match_only(claude: &'static str, mad: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        mad,
...
const CLAUDE_TOOLS: &[ClaudeTool] = &[
    ...
    k("Skill",           Read,                 &["skill"]),                           // matcher: opencode's `skill` tool; allowlist Read (mad-build reads SKILL.md)
...
/// The Mad [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The Mad tool names a Claude matcher term fires on.
pub fn mad_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.mad)
...
/// The Claude names that map to `mad_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(mad_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.mad.contains(&mad_name))
        .map(|t| t.claude)
}

/// Every distinct Mad name the table references, for the `xai-mad-agent` drift-check
/// test that asserts each is a real client tool name.
pub fn mad_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.mad.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a Mad name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.mad.is_empty(),
                "dead row: {}",
                t.claude
            );
        }
