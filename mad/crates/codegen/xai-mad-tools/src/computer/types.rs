...
/// (`xai-mad-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (xai-mad-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in xai-mad-tools, spawns processes)
/// - `AcpTerminalBackend` (in xai-mad-shell, calls ACP protocol)
#[async_trait::async_trait]
