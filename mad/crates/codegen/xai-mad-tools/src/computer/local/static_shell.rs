...
pub use xai_mad_config::shell::UnixShellKind;

const INIT_MARKER: &str = "__MAD_STATIC_SHELL_MARKER__";
...
fn shell_binary(shell: UnixShellKind) -> &'static str {
    xai_mad_config::shell::unix_shell_path(shell)
}
...
impl StaticShellSnapshot {
    ...
    pub async fn init(cwd: &Path) -> Self {
        let shell = xai_mad_config::shell::detect_unix_shell_kind();
...
mod tests {
    ...
    async fn replays_aliases_and_functions() {
        ...
        let output = run_static(
            "alias mad_alias_probe='echo ALIAS_OK'\nmad_fn_probe() { echo FN_OK; }\n",
            "mad_alias_probe && mad_fn_probe",
        )
        .await;
