...
//!
//! ```
//! use xai_mad_mermaid::{default_engine, render_checked, RenderLimits, RenderParams};
//!
...
pub enum MermaidTheme {
    /// Light surfaces with dark text (e.g. `MadDay`).
    #[default]
    Light,
    /// Dark surfaces with light text (e.g. `MadNight`, `TokyoNight`).
    Dark,
}
