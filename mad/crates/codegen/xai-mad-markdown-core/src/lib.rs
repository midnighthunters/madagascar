//! Headless markdown analysis sharing Madagascar Build's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `xai-mad-markdown` so analysis matches what Mad
//! Build actually renders 1:1.
//!
//! After parsing, Mad applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// The exact `pulldown-cmark` option set Madagascar Build uses to render markdown.
///
...
/// Offset event stream from Mad's parser, with single-tilde strikethrough demoted.
///
...
/// Parse `text` with Madagascar Build's options; count elements and flag structural issues.
