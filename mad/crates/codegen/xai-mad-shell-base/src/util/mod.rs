...
pub mod tips;
pub mod uname;
pub use xai_mad_shared::clipboard;
pub use xai_mad_shared::stderr::{stderr_lock, with_locked_stderr};
...
/// toggles via `MAD_CLI_CHAT_PROXY_BASE_URL` (or similar) pointing at an
...
/// True if `pid` is a mad process; pairs with [`kill_process_by_pid`] to avoid killing a recycled PID.
/// Best-effort on macOS/BSD (liveness-only via `kill -0`), exact on Linux (/proc cmdline) and Windows (image path).
pub fn is_mad_process(pid: u32) -> bool {
    #[cfg(target_os = "linux")]
    {
        let cmdline_path = format!("/proc/{pid}/cmdline");
        match std::fs::read(&cmdline_path) {
            Ok(data) => String::from_utf8_lossy(&data).contains("mad"),
            Err(_) => false,
        }
    }
    #[cfg(windows)]
...
/// Stricter [`is_mad_process`] for the auto-kill zombie path: on macOS/BSD it
...
/// exactly, so this delegates there. Use the permissive [`is_mad_process`] for
/// operator-driven `mad leaders kill`.
pub fn is_mad_process_strict(pid: u32) -> bool {
    #[cfg(all(not(target_os = "linux"), not(windows)))]
    {
        ...
        match cmd.output() {
            Ok(out) if out.status.success() => {
                ...
                comm.lines()
                    ...
                    .is_some_and(|name| name.to_ascii_lowercase().contains("mad"))
            }
            _ => false,
        }
    }
...
mod tests {
    ...
    fn test_is_cli_chat_proxy_url_accepts_proxy_subpath() {
        assert!(is_cli_chat_proxy_url(
            "https://cli-chat-proxy.mad.com/v1/chat/completions"
        ));
    }
    #[test]
    ...
    fn test_is_cli_chat_proxy_url_rejects_spoofed_hostname() {
        assert!(!is_cli_chat_proxy_url(
            "https://cli-chat-proxy.mad.com.evil.example/v1"
        ));
    }
    #[test]
    fn test_is_cli_chat_proxy_url_rejects_v11_prefix_confusion() {
        assert!(!is_cli_chat_proxy_url(
            "https://cli-chat-proxy.mad.com/v11/chat/completions"
        ));
    }
    #[test]
    ...
    #[test]
    fn is_mad_process_self_true_impossible_pid_false() {
        assert!(is_mad_process(std::process::id()));
        assert!(!is_mad_process(u32::MAX));
    }
    #[test]
    fn is_mad_process_strict_self_true_impossible_pid_false() {
        assert!(is_mad_process_strict(std::process::id()));
        assert!(!is_mad_process_strict(u32::MAX));
    }
    #[cfg(unix)]
    #[test]
