//! Memory-system configuration value types, extracted from xai-mad-shell
...
//! `xai-mad-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.mad/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.mad/memory/` are scanned:
