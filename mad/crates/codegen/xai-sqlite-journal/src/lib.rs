...
//! do not provide. When `$HOME` (and thus `~/.mad`) is NFS-mounted on
...
impl JournalMode {
    ...
    /// yet), so callers must create it first. `MAD_SQLITE_JOURNAL_MODE`
    /// (`wal`|`truncate`) overrides detection as a field kill-switch.
    pub fn for_db_path(db_path: &Path) -> Self {
        let env = std::env::var("MAD_SQLITE_JOURNAL_MODE").ok();
        match mode_from_env(env.as_deref()) {
            EnvOverride::Mode(mode) => {
                ...
                tracing::info!(
                    ...
                    mode = mode.as_str(),
                    source = "env",
                    "sqlite journal mode forced by MAD_SQLITE_JOURNAL_MODE"
                );
                return mode;
            }
            EnvOverride::Invalid => {
                ...
                tracing::warn!(
                    ...
                    "invalid MAD_SQLITE_JOURNAL_MODE (accepted: wal, truncate); using detection"
                );
            }
            EnvOverride::Unset => {}
        }
...
/// Parse result of the `MAD_SQLITE_JOURNAL_MODE` kill-switch.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum EnvOverride {
    Unset,
...
/// `MAD_SQLITE_JOURNAL_MODE` override parsing (pure for testability).
...
mod tests {
    ...
    fn windows_unc_classifies() {
        assert!(is_windows_unc(r"\\server\share\mad"));
        assert!(is_windows_unc(r"\\?\UNC\server\share\mad"));
        assert!(is_windows_unc(r"\\?\unc\server\share"));
        assert!(!is_windows_unc(r"\\?\C:\Users\x"));
        assert!(!is_windows_unc(r"\\.\pipe\mad"));
        assert!(!is_windows_unc(r"C:\Users\x"));
