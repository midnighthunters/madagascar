//! Hermetic filesystem and child-environment owner for mad integration tests.
...
/// mad network kill switches, and explicit overrides.
...
impl TestSandbox {
    ...
    /// Explicit mad state root.
    pub fn mad_home(&self) -> &Path {
        &self.mad_home
    }
    ...
    pub fn diagnostic_summary(&self) -> String {
        let mut summary = format!(
            "root={} home={} mad_home={} workspace={} temp={}",
            self.root().display(),
            self.home.display(),
            self.mad_home.display(),
...
impl TestSandboxBuilder {
    /// Wire mad API, models, feedback, trace, conversation, and web traffic to
    ...
    pub fn build(self) -> TestSandbox {
        ...
        for path in [&home, &mad_home, &workspace, &temp] {
        ...
        let mut env = baseline_env(&home, &mad_home, &temp, &parent_cwd);
...
impl TestSandbox {
    fn init_git_workspace(&self) {
        ...
        run_git(self, &["config", "user.name", "Mad Test"]);
...
fn baseline_env(
    ...
    baseline_env_from_parent(home, mad_home, temp, parent_cwd, &parent_env)
}
...
fn baseline_env_from_parent(
    ...
    for (key, value) in [
        ("MAD_TELEMETRY_ENABLED", "false"),
        ("MAD_TELEMETRY_TRACE_UPLOAD", "false"),
        ("MAD_FEEDBACK_ENABLED", "false"),
        ("MAD_TRACE_UPLOAD", "false"),
        ("MAD_INSTRUMENTATION", "disabled"),
        ("OTEL_SDK_DISABLED", "true"),
        ("DISABLE_TELEMETRY", "1"),
        ("DISABLE_FEEDBACK_COMMAND", "1"),
        ("MAD_DISABLE_AUTOUPDATER", "1"),
        ("MAD_PROMPT_SUGGESTIONS", "false"),
    ...
    env.insert(
        ...
        mad_home.join("gitconfig").into_os_string(),
    );
    env
}
...
fn diagnostic_value_is_sensitive(key: &OsStr) -> bool {
    ...
    is_secret_key(&key)
        ...
        || matches!(
            ...
            "HOME" | "USERPROFILE" | "MAD_HOME" | "TMPDIR" | "TMP" | "TEMP" | "GIT_CONFIG_GLOBAL"
        )
}
...
mod tests {
    ...
    fn owns_distinct_isolated_paths() {
        ...
        assert_eq!(sandbox.mad_home(), sandbox.home().join(".mad"));
    }
    ...
    fn git_command_uses_sandbox_state_without_process_global_mutation() {
        ...
        let sandbox = TestSandbox {
            ...
            mad_home: root.path().join("home/.mad"),
    ...
    fn baseline_is_hermetic_and_network_quiet() {
        ...
        assert_eq!(
            env_value(&sandbox, "MAD_HOME"),
            Some(sandbox.mad_home().into())
        );
        assert_eq!(
        ...
        assert_eq!(
            env_value(&sandbox, "MAD_DISABLE_AUTOUPDATER").as_deref(),
            Some(OsStr::new("1"))
        );
        assert_eq!(
            env_value(&sandbox, "MAD_TELEMETRY_TRACE_UPLOAD").as_deref(),
        ...
        assert_eq!(env_value(&sandbox, "MAD_LEADER_SOCKET"), None);
        assert_eq!(env_value(&sandbox, "MAD_DISABLE_WEB_FETCH"), None);
        assert_eq!(env_value(&sandbox, "MAD_WEB_FETCH"), None);
    }
    ...
    fn command_application_clears_ambient_env_and_command_override_wins() {
        ...
        cmd.env("AMBIENT_SECRET", "must-disappear")
            .env("MAD_PROMPT_SUGGESTIONS", "ambient");
        sandbox.apply_to_std_command(&mut cmd);
        cmd.env("MAD_PROMPT_SUGGESTIONS", "command");
        ...
        assert_eq!(
            env.get(OsStr::new("MAD_PROMPT_SUGGESTIONS"))
                .map(OsString::as_os_str),
    ...
    fn explicit_overrides_win_and_can_remove_baseline_entries() {
        ...
        sandbox
            ...
            .set_env("MAD_PROMPT_SUGGESTIONS", "true")
            .set_env("NO_PROXY", "override.invalid")
            .remove_env("MAD_DISABLE_AUTOUPDATER");
        assert_eq!(
            env_value(&sandbox, "TERM_PROGRAM").as_deref(),
            Some(OsStr::new("vscode"))
        );
        assert_eq!(
            env_value(&sandbox, "MAD_PROMPT_SUGGESTIONS").as_deref(),
            Some(OsStr::new("true"))
        );
        assert_eq!(
            env_value(&sandbox, "NO_PROXY").as_deref(),
        ...
        assert_eq!(env_value(&sandbox, "MAD_DISABLE_AUTOUPDATER"), None);
    }
    ...
    fn diagnostics_fail_closed_for_credential_keys() {
        ...
        for (key, value) in [
            ...
            ("MAD_DEPLOYMENT_KEY", "deployment-key-do-not-print"),
            ("MAD_EXTRA_AUTH_KEY", "alpha-test-key-do-not-print"),
