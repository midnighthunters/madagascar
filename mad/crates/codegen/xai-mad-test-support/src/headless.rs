//! Headless mode (`mad -p`) test runner.
//!
//! Runs the mad binary as a subprocess with the mock server, captures output.
...
/// Timeout for one headless mad invocation: 60 seconds, multiplied by
/// [`crate::scaled`]'s `MAD_TEST_TIMEOUT_SCALE`.
...
/// Run `mad` with the given args against the mock server, bounded by the
...
mod tests {
    ...
    async fn custom_headless_env_is_explicit_and_wins_after_sandbox_baseline() {
        ...
        cmd.args([
            "-c",
            "printf '%s|%s|%s|%s' \"${AMBIENT_ONLY-unset}\" \"$MAD_PROMPT_SUGGESTIONS\" \"$FEATURE_TEST_VAR\" \"$HOME\"",
        ])
        .env("AMBIENT_ONLY", "discarded")
        .env("MAD_PROMPT_SUGGESTIONS", "command-level-discarded");

        let result = run_headless_in_sandbox_borrowed_with_env(
            cmd,
            &sandbox,
            &[
                ("MAD_PROMPT_SUGGESTIONS", "explicit-override"),
