...
fn local_mad_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("xai-mad-pager{}", std::env::consts::EXE_SUFFIX))
}

fn ensure_local_mad_binary(binary: &Path) {
    if binary.exists() {
        return;
    }
    ...
    let output = cmd
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build xai-mad-pager: {e}"));

    assert!(
        output.status.success(),
        "failed to build xai-mad-pager for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
    ...
    assert!(
        binary.exists(),
        "xai-mad-pager build completed but binary missing at {}",
        binary.display()
    );
}

/// Resolve mad binary: `MAD_BINARY` env (CI) or a locally built `xai-mad-pager` binary.
pub fn mad_binary() -> PathBuf {
    if let Ok(path) = std::env::var("MAD_BINARY") {
        let p = PathBuf::from(path);
        assert!(p.exists(), "MAD_BINARY does not exist: {}", p.display());
        // Bazel's MAD_BINARY is runfiles-relative; the harness spawns the child
        // with a different cwd, so absolutize against the (runfiles-root) cwd now.
        return std::path::absolute(&p).unwrap_or(p);
    }

    if let Ok(path) = std::env::var("CARGO_BIN_EXE_xai-mad-pager") {
        let p = PathBuf::from(path);
        if p.exists() {
            return p;
        }
    ...
    let binary = local_mad_binary_path();
    ensure_local_mad_binary(&binary);
    binary
}
