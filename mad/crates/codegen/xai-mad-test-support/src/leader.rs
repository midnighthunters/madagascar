//! Leader-mode (`mad agent --leader stdio`) test harness.
//!
...
/// (version-skew) test. Falls back to [`mad_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "MAD_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a
/// two-binary test. Falls back to [`mad_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "MAD_BINARY_CLIENT";
...
/// A `mad agent --leader stdio` client subprocess speaking ACP over pipes.
...
impl LeaderFixture {
    ...
    pub async fn start(
        ...
        Self::start_with_binary(&mad_binary(), server, cwd, sandbox).await
    }
    ...
    async fn start_with_binary_base_url_timeout(
        ...
        let socket = sandbox.mad_home().join("leader.sock");
        let lock = sandbox.mad_home().join("leader.lock");
        ...
        cmd.envs(xai_tty_utils::pager_env())
            .env("MAD_CLI_CHAT_PROXY_BASE_URL", base_url)
            .env("MAD_XAI_API_BASE_URL", base_url)
            .env("MAD_MODELS_BASE_URL", base_url)
            .env("MAD_FEEDBACK_BASE_URL", base_url)
            .env("MAD_TRACE_UPLOAD_URL", base_url)
            .env("XAI_API_KEY", "test-key-for-ci")
            .env("MAD_LEADER_SOCKET", &socket)
            .env("RUST_LOG", "xai_mad_shell=debug");
        let log_path = sandbox.mad_home().join("leader.log");
        ...
        let pid = child.id();
        let tree = match TestProcessTree::try_attach(pid, "persistent mad test leader") {
            Ok(tree) => tree,
            Err(error) => {
                let _ = child.kill();
    ...
    async fn spawn_with_binary_and_socket(
        ...
        let mut process = TestProcess::spawn(
            ...
            TestProcessConfig::new()
                ...
                .env("MAD_TRACE_UPLOAD_URL", base_url)
                .env("XAI_API_KEY", "test-key-for-ci")
                .env("MAD_LEADER_SOCKET", leader_socket)
                .env("RUST_LOG", "xai_mad_shell=debug"),
        )
        .map_err(|error| {
            io::Error::new(
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".mad").join("leader.lock")
}
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".mad").join("leader.log")).unwrap_or_default()
}
