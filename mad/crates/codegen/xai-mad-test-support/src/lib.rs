...
//! Shared test utilities for mad-build crates: mock inference server, SSE
...
//! - [`MadStdioClient`] — ACP client that drives `mad agent stdio` as a subprocess
//! - [`RawStdioClient`] — raw-wire ACP driver for bytes the typed client can't
//!   produce (Foundation `\/` methods, string UUID ids)
//! - [`leader::LeaderStdioClient`] — ACP client that drives `mad agent --leader stdio` (unix)
...
//! - [`run_headless`] — Run `mad -p` against the mock server and capture output
//! - [`git_workdir`] — Create a git-initialized [`TestSandbox`]
//! - [`mad_binary`] — Resolve the mad binary path (MAD_BINARY env or cargo_bin)
...
/// Multiply a harness timeout by `MAD_TEST_TIMEOUT_SCALE` (positive integer,
/// default 1). CI lanes on shared runner pools raise it so pool load slows
/// tests instead of failing them (see the Madagascar Build merge CI workflow).
pub fn scaled(base: std::time::Duration) -> std::time::Duration {
    let scale = std::env::var("MAD_TEST_TIMEOUT_SCALE")
        .ok()
...
#[cfg(unix)]
pub mod uds_proxy;
pub use acp_client::{MadStdioClient, RawStdioClient};
pub use counting_server::spawn_counting_server;
pub use env::{EnvGuard, git_workdir, mad_binary};
pub use headless::{
