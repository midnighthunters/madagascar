...
/// emitter in `xai-mad-shell` and the upsert/detection here — a drift would
...
pub enum AuthType {
    /// From AuthManager (mad login, OIDC, external binary). Refreshable.
    #[default]
    SessionToken,
...
/// `xai_mad_sampling_types::SamplingConfig` (which is secret-free).
