...
pub fn estimate_tool_definition_tokens(td: &xai_mad_sampling_types::ToolDefinition) -> u64 {
...
pub fn estimate_tool_definitions_tokens(tds: &[xai_mad_sampling_types::ToolDefinition]) -> u64 {
...
pub fn estimate_item_tokens(item: &ConversationItem) -> u64 {
    ...
    match item {
        ...
        ConversationItem::Reasoning(r) => {
            ...
            let text_bytes = xai_mad_sampling_types::reasoning_item_text(r).len();
...
/// mad-build's [`ItemTokenCounter`](xai_mad_compaction::ItemTokenCounter)
/// for the shared compaction engine: the bytes/4 estimate mad-build already
...
/// mad-build estimates instead, reusing [`estimate_item_tokens`] so the
/// per-variant arithmetic (images, reasoning blobs, tool-call args) stays in
/// one place.
pub struct EstimatedItemTokenCounter;

impl xai_mad_compaction::ItemTokenCounter<ConversationItem> for EstimatedItemTokenCounter {
...
mod tests {
    ...
    fn estimated_item_token_counter_matches_estimate_item_tokens() {
        use xai_mad_compaction::ItemTokenCounter;
    ...
    fn estimate_tool_definition_tokens_counts_name_desc_params() {
        ...
        let td = xai_mad_sampling_types::ToolDefinition::function(
            "search",
            Some("find a file"),
            serde_json::json!({}),
        );
    ...
    fn estimate_tool_definitions_tokens_sums_across_slice() {
        let a = xai_mad_sampling_types::ToolDefinition::function(
            "a",
            None::<&str>,
            serde_json::json!({}),
        );
        let b = xai_mad_sampling_types::ToolDefinition::function(
            "b",
            None::<&str>,
            serde_json::json!({}),
        );
