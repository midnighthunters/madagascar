...
//!
//! These tests exercise the real `xai-mad-pager` binary through a PTY using
...
/// enterprise deploy report: with `MAD_GOAL=1`, `/goal` must
...
/// explicitly off (`MAD_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `MAD_FOLDER_TRUST=1` and a git repo that
...
/// also pins `MAD_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
