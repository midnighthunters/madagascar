...
//!
//! Uses `xai-mad-pager-pty-harness` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.mad/auth.json` from `MAD_AUTH_JSON` (path) or the
//! developer's `~/.mad/auth.json` so the pager skips device-login when
...
//!
//! Run with:
//! `cargo test -p xai-mad-pager --test pty_auto_mode -- --ignored --nocapture`

use std::path::PathBuf;
use std::time::Duration;

use xai_mad_pager_pty_harness::{PtyHarness, pager_binary};
use xai_mad_test_support::TestSandbox;
...
/// Prefer explicit path, else the user's real `~/.mad/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("MAD_AUTH_JSON") {
...
fn prepare_sandbox(sandbox: &mut TestSandbox, gate_on: bool) -> Vec<(String, String)> {
    ...
    let mad = sandbox.mad_home();
    let _ = std::fs::create_dir_all(mad);
    if let Some(src) = auth_json_source() {
        let dest = mad.join("auth.json");
        ...
        eprintln!("pty_auto_mode: no ~/.mad/auth.json — may hit device login");
    }
    ...
    // of the developer's shell. `MAD_AUTO_PERMISSION_MODE` is the highest gate
    // layer below requirements; "1"/"0" parse to on/off (xai_mad_config::
...
fn require_auth() -> bool {
    std::env::var("MAD_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness =
        PtyHarness::new_in_sandbox(&binary, ROWS, COLS, &[], &sandbox, &env_refs, None)
            .expect("spawn pager in PTY (xai-mad-pager-pty-harness)");
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "MAD_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );
