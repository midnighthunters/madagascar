...
/// backend, often with the mad.com proxy blocked. A TCP listener that accepts
...
async fn embedded_mode_boots_without_hanging_on_blocked_backend() {
    ...
    let mad_home = home.path().join(".mad");
    std::fs::create_dir_all(&mad_home).unwrap();
    let env = [
        ("HOME", home.path().to_str().unwrap()),
        ("MAD_HOME", mad_home.to_str().unwrap()),
        ("XAI_API_KEY", "test-key-for-ci"),
        ("MAD_CLI_CHAT_PROXY_BASE_URL", base.as_str()),
        ("MAD_XAI_API_BASE_URL", base.as_str()),
