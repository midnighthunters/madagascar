...
pub(crate) use xai_mad_pager_pty_harness::{
...
pub(crate) const CONTEXTUAL_HINTS_ENV: &[(&str, &str)] = &[("MAD_CONTEXTUAL_HINTS", "1")];
...
// `xai_mad_pager_pty_harness::flows` (re-exported above).
...
/// would never fetch `/v1/settings`. Spawns WITHOUT `MAD_ANNOUNCEMENTS_OVERRIDE`
...
pub(crate) fn spawn_polling_session_with_env(
    ...
    overrides.push(EnvOp::set("MAD_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS", "1"));
...
/// Explicit overrides for a folder-trust run. A self-built mad auto-trusts,
/// so `MAD_TEST_VERSION` simulates a release; the gate is pinned both ways.
pub(crate) fn trust_env(feature_on: bool) -> [(&'static str, &'static str); 2] {
    [
        ("MAD_TEST_VERSION", "0.0.0-sim"),
        ("MAD_FOLDER_TRUST", if feature_on { "1" } else { "0" }),
    ]
}
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".mad")
        .join(xai_mad_workspace::trust::TRUST_FILE_NAME);
    let store = xai_mad_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&xai_mad_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `xai_mad_pager_pty_harness::flows`
// (re-exported above).
...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    let mad_home = content.home().join(".mad");
    std::fs::create_dir_all(&mad_home).expect("create fake MAD_HOME");
    let config = format!(
        "[mcp_servers.{MCP_TEST_SERVER}]\ncommand = \"{command}\"\nargs = []\nstartup_timeout_sec = 2\n"
    );
    std::fs::write(mad_home.join("config.toml"), config).expect("write config.toml");
}
...
// unit tests in xai-mad-hooks::discovery::tests. The PTY E2E test requires
...
/// Seed `~/.mad/config.toml` with a `[ui]` section body (e.g.
/// `"vim_mode = true"`). Same `{MAD_HOME|HOME}/.mad/config.toml` location
...
pub(crate) fn seed_ui_config(content: &ContentController, ui_body: &str) {
    let mad_home = content.home().join(".mad");
    std::fs::create_dir_all(&mad_home).expect("create .mad");
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let mad_home = content.home().join(".mad");
    std::fs::create_dir_all(&mad_home).expect("create .mad");
    // Minimal opt-in only — matches load_config's `{MAD_HOME|HOME}/.mad/config.toml`.
...
pub(crate) fn spawn_mouse_toggle_pager(content: &ContentController) -> PtyHarness {
    ...
    PtyHarness::spawn_with_content_env_ops(
        ...
        content,
        &[],
        &[EnvOp::set("MAD_MOUSE_REPORTING_TOGGLE", "true")],
    )
    .expect("spawn pager")
}
...
pub(crate) const ESC_DOUBLE_PRESS_ENV: &str = "MAD_ESC_DOUBLE_PRESS_MS";
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    let binary = pager_binary().expect("resolve pager binary");
    let value = xai_mad_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string();
    PtyHarness::spawn_with_content_env_ops(
        &binary,
        DEFAULT_ROWS,
        DEFAULT_COLS,
...
/// Locate `<mad_home>/sessions/<encoded cwd>/<session id>/`, where the shell
...
pub(crate) fn session_dir(content: &ContentController, harness: &mut PtyHarness) -> PathBuf {
    let sessions = content.home().join(".mad").join("sessions");
    for _ in 0..100 {
...
/// `mad wrap` run budget. Same contention math as the requirements-version
...
/// Run `mad wrap <wrap_args...>` to completion inside a PTY with an isolated
/// `MAD_HOME`, returning the exit code (`None` only while still running at
...
pub(crate) fn run_wrap_driving(
    ...
    let code = match wait_for_exit_status(&mut harness, WRAP_TIMEOUT) {
        ...
        Ok(PtyExitPoll::Running) => {
            harness.quit().expect("kill mad wrap after timeout");
            None
        }
        Ok(PtyExitPoll::PendingStatus) => {
            panic!("mad wrap exited but portable status remained unavailable for {WRAP_TIMEOUT:?}")
        }
        Err(error) => panic!("poll mad wrap exit: {error:#}"),
    };
...
/// `mad wrap` hands to the user's shell without depending on any real
...
/// Dump an asciinema cast of `harness` into `$MAD_PTY_CAST_DIR/<file_name>`
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("MAD_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
...
pub(crate) use xai_mad_pager_pty_harness::host_clipboard::{
...
pub(crate) use xai_mad_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
