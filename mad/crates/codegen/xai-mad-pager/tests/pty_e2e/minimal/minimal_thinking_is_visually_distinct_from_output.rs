...
async fn run_reasoning_turn(collapse_thinking: bool) -> Turn {
    ...
    std::fs::create_dir_all(content.home().join(".mad")).expect("mk .mad");
    std::fs::write(
        content.home().join(".mad/config.toml"),
        "[ui]\nshow_thinking_blocks = true\n",
    )
    .expect("write config");
    if collapse_thinking {
        let mad_home = content.sandbox().mad_home().to_path_buf();
        std::fs::create_dir_all(&mad_home).expect("mk mad home");
        std::fs::write(
