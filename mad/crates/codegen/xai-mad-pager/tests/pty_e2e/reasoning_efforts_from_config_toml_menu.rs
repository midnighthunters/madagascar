...
async fn reasoning_efforts_from_config_toml_menu() {
    let content = ContentController::start_with_models(vec![
        MockModel::new("mad-4.5").with_supports_reasoning_effort(true),
    ])
    .await
    .expect("start content");
    content.set_response(format!("{MOCK_RESPONSE_SENTINEL} turn."));

    // Seed `~/.mad/config.toml` with a per-model reasoning-effort menu.
    let mad_home = content.home().join(".mad");
    std::fs::create_dir_all(&mad_home).expect("create .mad");
    // Quote the dotted model id: bare `[model.mad-4.5]` is TOML key-path syntax (model.mad-4.5), not the id "mad-4.5".
    std::fs::write(
