...
//!
//! The stack is `tmux -> nvim :terminal -> mad`: nvim/tmux repaint mad's
//! pane out-of-band (no mad PTY resize), and mad's diff renderer only
...
//! into the virtual terminal, bypassing mad) and then check whether mad
//! heals it. `NVIM` is set in mad's env so it sees the embedded-editor
//! context the fix keys off (mirroring a real nvim `:terminal`).
//!
//! The fix: mad forces a full clear+repaint on `FocusGained` in
//! editor/multiplexer contexts, so the injected row is gone after refocus.
//! The final assertion guards that heal.

use super::common::*;

/// Unique sentinel that mad would never render on its own.
...
async fn out_of_band_stale_row_heals_on_focus_gained() {
    ...
    let overrides: Vec<(String, String)> =
        vec![("NVIM".into(), "/tmp/mad-pty-harness-fake-nvim.sock".into())];
    ...
    // virtual screen at col 1 — the static left margin mad's diff renderer
    ...
    // mad must not self-heal out-of-band content via ordinary diff redraws:
    // it only rewrites cells whose own model changed, so this row is stranded.
    h.update(Duration::from_millis(300));
    assert!(
        h.contains_text(STALE_MARKER),
        "stale row should survive a normal redraw (mad's diff renderer doesn't own it)\nscreen:\n{}",
        h.screen_contents()
    );

    // A FocusGained (CSI I) forces a full clear+repaint that re-asserts mad's
