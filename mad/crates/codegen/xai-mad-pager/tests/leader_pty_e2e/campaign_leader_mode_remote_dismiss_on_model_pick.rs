...
#[ignore = "PTY e2e; run with cargo test -p xai-mad-pager --test leader_pty_e2e -- --ignored --test-threads=1"]
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // socket under the shared MAD_HOME so every spawn elects/attaches to the
    ...
    let socket = mad_home.join("leader-e2e.sock");
    ...
    let state_path = mad_home.join("campaigns_state.json");
    ...
    let config = std::fs::read_to_string(mad_home.join("config.toml")).expect("read config.toml");
    assert!(
