...
Mad asks before doing anything risky — and can plan before it codes.

## Permissions

When Mad wants to run a risky command or edit a file, it pauses and asks:
...
For bigger or more ambiguous tasks, use **plan mode**: Mad explores the
...
`s` requests changes — Mad iterates until you're happy, then implements.
...
background — Mad keeps working and you're notified when it finishes
