...
Control what Mad can access and do: permission modes, allow/ask/deny rules, hooks, and the optional OS-level sandbox.

- **Modes** set how often Mad asks for approval (always-approve, auto, ask, and related).
...
When Mad edits a file, runs a command, or calls an external tool, it may pause for approval. Permission modes control how often that happens.
...
```bash
mad -p "Run the tests" --always-approve
mad agent --always-approve stdio
mad agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```
...
```bash
mad --always-approve -p "Run the test suite"
mad --permission-mode auto
mad agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```
...
```bash
mad -p "Deploy the service" --always-approve --deny 'Bash(rm -rf *)'
```
...
Organizations can prevent always-approve from being enabled via CLI, TUI, or `/always-approve`. Set this in `requirements.toml` (user-level under `~/.mad/`, or system-wide under `/etc/mad/` for enforcement users cannot remove):
...
Mad can still load Claude-style permission **rules** from managed settings; always-approve is locked with `requirements.toml` as shown above.
...
Mad reads permission rules from three compatible sources. Rules from all sources are merged into one set; a rule's effect depends on its action (`deny` > `ask` > `allow`), not on which file it came from.
...
|-------|------|-----------------------|
| Global (all projects) | `~/.mad/config.toml` | No |
| Project (committed) | `<project>/.mad/config.toml` | Yes (commit it) |
| Project (personal) | `<project>/.claude/settings.local.json` | No (gitignore it) |
| Interactive grants | Stored internally by Mad, per project | No |

Notes on scoping:

- Mad discovers a `.mad/config.toml` at every directory level from the repository root down to your working directory, so a subdirectory can add rules on top of the repo root's.
...
- Mad has no native `config.local.toml`. For personal, uncommitted rules in a project, use `.claude/settings.local.json`; Mad reads it directly (see [Claude Code Compatibility](#3-claude-code-compatibility-claudesettingsjson)).
...
To stop prompts for a specific command in one project, add a narrow allow rule to that project's `.mad/config.toml` (or `.claude/settings.json`):
...
### 2. Native Configuration (`~/.mad/config.toml` and `.mad/config.toml`)
...
Rules from the global `~/.mad/config.toml` and every project `.mad/config.toml` (from the repo root down to your working directory) are merged into one rule set, alongside any `.claude/settings.json` rules.

Managed configuration deployed by your organization also contributes `[permission]` rules: the system `/etc/mad/managed_config.toml`, and a user-level copy that Mad maintains automatically at `~/.mad/managed_config.toml`. Managed rules merge like rules from any other source, with two properties specific to managed `allow` rules: your own `deny` and `ask` rules win over a managed `allow` (severity ordering), and a catch-all managed `allow` is ignored when always-approve is locked off. For rules that users cannot edit away, use the root-owned system `/etc/mad/requirements.toml`.
...
Mad reads `~/.claude/settings.json` and `~/.claude/settings.local.json`, plus the project-level `<project>/.claude/settings.json` and `settings.local.json` (walking up to the repo root). The native `.mad` source for permission rules is `config.toml`, described in the section above.
...
Supported `defaultMode` values include `default`, `auto`, `acceptEdits`, `bypassPermissions`, `dontAsk`, and `plan`. Mad reads `defaultMode` from its canonical location under `permissions`; a top-level `defaultMode` is also accepted when the nested key is absent.
...
**Chained commands.** Mad parses each command like a shell and splits it on `&&`, `||`, `;`, `|`, and newlines. The rule actions treat segments differently:
...
`MCPTool(...)` patterns match the full Mad tool name in `server__tool` form, with glob support: `MCPTool(linear__*)` matches every tool from the `linear` server. Mad tool names carry no `mcp__` prefix, so a rule written as `mcp__server__tool` never matches an MCP call; write `MCPTool(server__tool)` instead.
...
Interactive grants are stored in Mad's own state directory under your home directory, scoped to the directory you launched Mad from. A grant made in one project never applies in another, grants are not written into the repository, and they are not meant to be hand-edited.

Interactive grants are personal, per-machine state. For an allowlist you can review in code review and share with teammates, use declarative rules in the project's `.mad/config.toml` instead.
...
```bash
chmod +x ~/.mad/hooks/git-gh-only.sh
```
...
```bash
mad -p "Implement the feature using only git and GitHub CLI" \
  --allow 'Read' \
  --allow 'Grep' \
  --allow 'Bash(git *)' \
  --allow 'Bash(gh *)'
...
3. **Review project configuration from unfamiliar sources.** Project permission rules in `.mad/config.toml` and `.claude/settings.json`, including `allow` rules, apply without a separate trust prompt. Review them, and any project hooks, before working in an unfamiliar checkout (see the security notes in [10-hooks.md](10-hooks.md)).
