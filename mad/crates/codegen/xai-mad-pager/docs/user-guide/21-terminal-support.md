...
Madagascar Build runs as a full-screen TUI. It relies on terminal support for color,
...
Run `/doctor` in Mad to check the current session and see available fixes. If
Mad cannot start, run `mad doctor` in your shell. Use `mad doctor --json`
...
`mad doctor --json` reports the same color capability when piped. Microphone
...
`/doctor fix tmux-clipboard` or `mad doctor fix dcs-passthrough --yes`.
...
to guess if that directory is unavailable or unsafe. Mad preserves the file's
...
Mad deliberately does **not** run `tmux source-file` or change the live tmux
...
Mad detects these terminal emulators from environment variables:
...
- Inside tmux, variables that identify the outer terminal may not reach Mad.
...
Mad writes through up to three routes, shown in `/doctor` under **Clipboard**:

- **native** — the local operating-system clipboard.
- **tmux** — the tmux paste buffer when Mad runs inside tmux.
...
terminal focused. Older compositors may require Mad to remain focused until
the copy message appears. Mad shows a startup warning when this applies; run
`/doctor` for the detected status and steps.

`MAD_CLIPBOARD_NO_DATA_CONTROL=1` is an advanced fallback that disables the
data-control route. Copies then use command-line clipboard tools.

#### OSC 52 kill switch

Mad emits OSC 52 on Linux and across tmux, SSH, or displayless containers when
that route is enabled. A terminal that does not implement OSC 52 may display the
encoded payload as text. Set `MAD_CLIPBOARD_NO_OSC52=1` before starting Mad to
...
A remote Mad process normally cannot read the local terminal's selection. Use
...
When Mad cannot identify the outer terminal over SSH, it predicts that OSC 52
...
clipboard. Each copy is still saved to a backup file (`~/.mad/last-copy.txt` by
default; override with `MAD_COPY_FILE`); the toast names that path when delivery
...
through `mad wrap`, for example `mad wrap ssh user@host`. The same command can
wrap container and pod shells. It also restores terminal modes after a dropped
connection.

When an SSH session is not using `mad wrap`, Mad shows the one-time tip
...
`[ui.contextual_hints]` in `$MAD_HOME/config.toml`. This setting does not hide
the Doctor recommendation.

For repeated SSH use, Doctor offers `mad doctor fix ssh-wrap`. It also shows
...
> **Warning**: `mad wrap` is experimental and may not work in every setup.
...
Zellij and tmux control mode can limit the alternate screen. Mad normally uses
inline mode in those environments. Run `/doctor` to see the detected condition.
You can configure `[terminal] alt_screen` in `~/.mad/pager.toml`, or run
`mad --no-alt-screen` to confirm inline mode works.

### Zellij keybindings interfere with Mad

Zellij can intercept Ctrl/Alt keys before they reach Mad. On Zellij 0.41 or
...
mode, if `Ctrl+G` still does not reach Mad, open the command palette and select
...
WezTerm ships with the Kitty keyboard protocol disabled. Run `/doctor` in Mad.
...
keys. Mad therefore does not negotiate the protocol there, and Shift+Enter can
...
If Mad stops receiving mouse input, re-enable mouse reporting in the terminal:
...
After about 10 seconds without a transcript, Mad stops capture and shows
...
to the terminal hosting Mad. Open **System Settings → Privacy & Security →
...
Run `mad doctor`, or run `/doctor` while voice mode is on. The **Voice** section
shows the microphone Mad would use. If no input device is available, Doctor
...
problem, set `MAD_VOICE_CAPTURE=inprocess` to use the in-process fallback for
comparison.
