...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.mad/agents/` or `~/.mad/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.mad/personas/`; applied during subagent resolution |
...
| **Examples** | `mad-build`, `explore`, `plan` | `researcher`, `concise` |
...
Madagascar Build discovers file-based personas from these locations, in priority order:

- `.mad/personas/*.toml` (project)
- `~/.mad/personas/*.toml` (user)
...
> **Note:** Madagascar Build applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, Madagascar Build resolves the effective model and reasoning effort in this order, highest priority first:
...
Madagascar Build manages worktrees through the `x.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "mad-build"               # route explore to a specific model
```
...
# instructions_file = ".mad/personas/concise.md"  # or load from a file
```

Madagascar Build also discovers roles from `.mad/roles/*.toml` and personas from `.mad/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

Madagascar Build shows running and finished work in side panes on the agent screen:
...
- `Subagent running: "do the thing" (Implementer · mad-3) — Thinking`
