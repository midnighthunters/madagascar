...
Madagascar Build is a terminal-based AI coding assistant from SpaceXAI. It runs as a TUI (Terminal User Interface) that understands your codebase, executes shell commands, edits files, searches the web, and manages tasks.
...
```powershell
$env:MAD_VERSION="0.1.42"; irm https://x.ai/cli/install.ps1 | iex
```

The PowerShell installer automatically adds `%USERPROFILE%\.mad\bin` to your User PATH. Alternatively, install via [Git for Windows](https://gitforwindows.org/) (Git Bash) or MSYS2 using the bash script above. WSL users get the Linux binary automatically.
...
Start Mad by running:

```bash
mad
```

On first launch, Mad opens your browser to authenticate with mad.com. After you sign in, Mad stores your credentials in `~/.mad/auth.json`, where they persist across sessions. Mad refreshes your credentials automatically and prompts you to sign in again when they can no longer be renewed.
...
Once authenticated, Mad presents a full-screen TUI with two main areas:

- **Scrollback** -- the conversation history showing your prompts, Mad's responses, tool calls, file edits, and more.
- **Prompt** -- the input area at the bottom where you type messages.

Type a message and press `Enter` to send it. Mad reads files, runs commands, and edits code as needed. Each tool run streams into the scrollback in real time.
...
By default, Mad asks for permission before executing shell commands or editing files. You can approve individually or toggle always-approve mode:

- Press `Ctrl+O` to toggle always-approve mode
- Use the `--yolo` flag at launch: `mad --yolo`
...
Every conversation is a **session**. Sessions are automatically saved to `~/.mad/sessions/` and can be resumed later. Each session tracks the full conversation history, tool calls, file edits, and task state.

- Start a new session: `Ctrl+N` or `/new`
- Resume a previous session: `/resume` in the TUI, or `--resume <ID>` from the CLI
- Continue the most recent session: `mad -c`
...
- **Agent messages** -- Mad's responses with full markdown rendering and syntax highlighting
- **Thinking blocks** -- Mad's reasoning process (collapsible)
...
mad --resume <session-id>

# Continue the most recent session
mad -c

# Experimental scrollback-native render mode. Sticky: plain `mad` reopens in
...
Run Mad non-interactively for scripting, CI/CD, and automation:
...
```bash
mad -p "Review changes for bugs" --output-format json --yolo | jq -r '.text'
```
...
Add per-project instructions by creating an `AGENTS.md` file in your repository. Mad reads these files and injects their contents as a project-instructions message at the start of the conversation:

```
~/.mad/AGENTS.md           # Global rules (apply to all projects)
<repo-root>/AGENTS.md       # Repository-level rules
<cwd>/AGENTS.md             # Directory-level rules (highest priority)
```

Deeper files take precedence. Mad also reads `CLAUDE.md` files for compatibility.
