...
MCP (Model Context Protocol) servers extend Mad with external tool integrations. They let Mad interact with any service that implements the MCP standard.

---

## What Are MCP Servers?

An MCP server is a process that exposes tools to Mad over a standardized protocol. When you configure an MCP server, its tools become available to the model alongside Mad's built-in tools. The model can discover and call these tools during a session.
...
MCP servers are configured in `~/.mad/config.toml` under `[mcp_servers.<name>]` sections.
...
Mad spawns a local process and communicates over stdin/stdout:
...
> `MAD_MCP_STARTUP_TIMEOUT_SECS` (seconds). A per-server `startup_timeout_sec`
...
>
> - env `MAD_MAX_MCP_OUTPUT_BYTES` or `MAX_MCP_OUTPUT_BYTES` (bytes; Mad-native
>   wins if both set; Claude-style name, but we bound by **bytes** not tokens)
> - `config.toml` — user-level (`~/.mad/config.toml`) **or repo-level**
>   (`.mad/config.toml` anywhere on the cwd → git-root chain; the deepest
...
> ```
>
> Precedence: requirements.toml > env > repo `.mad/config.toml` >
...
mad mcp list
mad mcp list --json          # Machine-readable output

# Add a stdio server. Everything after -- is the server command, so flags
# like -y reach the server instead of being parsed by mad.
mad mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /path/to/dir

# Add a stdio server with environment variables (-e is repeatable)
mad mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

# Add a remote HTTP server
mad mcp add --transport http sentry https://mcp.sentry.dev/mcp

# Add a remote server with an authentication header (--header is repeatable)
mad mcp add --transport http api https://mcp.example.com/mcp --header "Authorization: Bearer YOUR_TOKEN"

# Add a remote SSE server
mad mcp add --transport sse linear https://mcp.linear.app/sse

# Remove a server
mad mcp remove github

# Enable or disable a local/TOML (or compat-sourced) server
mad mcp enable github
mad mcp disable github

# Diagnose a server's configuration and connectivity
mad mcp doctor               # Check every configured server
mad mcp doctor github        # Check one server
mad mcp doctor --json        # Machine-readable output
```

The transport defaults to `stdio`; pass `--transport http` or `--transport sse` for remote servers.

By default `mad mcp add` writes to `~/.mad/config.toml` (`--scope user`). Use `--scope project` to write to `.mad/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `mad mcp list` shows servers from both scopes, marking project-scoped ones with `(project)` and disabled ones with `(disabled)`.

`mad mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope — pass `--scope` to say which one to remove.

`mad mcp enable` / `disable` persist the personal on/off state to user `~/.mad/config.toml` (`disabled_mcp_servers`, and `[mcp_servers.<name>].enabled` when that entry exists). Scope:

- **Known names:** user/project Mad TOML, names already on the disabled list, compat sources (`.mcp.json`, Claude, Cursor), **plugin** MCP servers (same discovery as doctor/`/mcps`), and legacy managed `mad_com_*` (no local entry required).
...
MCP servers can be configured per-project by placing a `.mad/config.toml` in your repository:
...
When a server exposes a native HTTP/SSE endpoint, prefer the `url` form over wrapping it in a stdio proxy such as `npx mcp-remote <url>`. Mad handles HTTP/SSE and OAuth directly, so the native form avoids an extra subprocess per session. It also registers Mad's own OAuth client with the provider.

Mad walks from the current directory up to the git repo root, loading `.mad/config.toml` at each level:

| Location | Scope | Priority |
|----------|-------|----------|
| `~/.mad/config.toml` | All projects | Lowest |
| `<repo-root>/.mad/config.toml` | This repository | Medium |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Mad reads most other config sections only from `~/.mad/config.toml`.
...
You can enable or disable MCP servers without restarting Mad (TUI `/mcps` or CLI — see [CLI Management](#cli-management)).
...
Mad loads MCP server configurations from multiple sources for compatibility:

| Source | Format | Location | Configurable |
|--------|--------|----------|-------------|
| `config.toml` | Native Mad config | `~/.mad/config.toml`, `.mad/config.toml` | Always on |
...
The Claude and Cursor MCP sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] mcps = false` in `~/.mad/config.toml` or the corresponding environment variable (`MAD_CURSOR_MCPS_ENABLED`, `MAD_CLAUDE_MCPS_ENABLED`). See [Configuration](05-configuration.md#harness-compatibility) for details. Use `mad inspect` to see which MCP servers were loaded and their vendor origin (`[cursor]`, `[claude]`).

---

## MCP OAuth

For MCP servers that require OAuth authentication, Mad handles the credential flow automatically. When an MCP server requests OAuth credentials, Mad opens a browser-based authorization flow and stores the resulting tokens for future use.
...
To avoid putting secrets in the config file, reference an environment variable with `${VAR}` (or `${VAR:-default}`). Mad expands string fields in `[mcp_servers.*]` — `url`, `command`, `args`, and the values in `env` and `headers` — at load time:
...
1. The parent session actually connected the server (see Extensions / `mad inspect`)
...
For stdio servers, Mad captures the process's standard error to `~/.mad/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.mad/logs/mcp/filesystem.stderr.log
```

### Viewing Server Status

Use `mad inspect` to see all loaded MCP servers and their sources:

```bash
mad inspect          # Human-readable
mad inspect --json   # Machine-readable
```

### Debug Logging

```bash
RUST_LOG=debug MAD_LOG_FILE=/tmp/mad.log mad
tail -f /tmp/mad.log
```
