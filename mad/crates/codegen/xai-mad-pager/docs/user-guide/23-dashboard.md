...
- **`mad dashboard`** — launch the TUI into the dashboard.
- **`/dashboard`** (aliases **`/agents-dashboard`**, **`/sessions`**) — open
  from inside a session.
- **`Ctrl+\`** — same view as the slash command.

Hidden in minimal mode. Set `MAD_AGENT_DASHBOARD=0` or
...
```
 Madagascar Build · Dashboard — 4 agents · 2 awaiting
...
State icons match other session lists in Madagascar Build:
...
Per-user preferences under `[dashboard]` in `~/.mad/config.toml`:
