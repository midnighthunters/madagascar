...
Agent mode runs Mad as a long-lived server that clients talk to over [ACP](https://agentclientprotocol.com) (JSON-RPC). Use it from IDEs, SDKs, eval harnesses, and custom apps. For a one-shot prompt that prints and exits, use `mad -p` instead ([headless mode](14-headless-mode.md)).
...
The [Agent Client Protocol (ACP)](https://agentclientprotocol.com) defines how clients talk to coding agents over JSON-RPC. With Mad it covers:
...
```bash
mad agent --always-approve --model mad-build stdio
mad agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```

| Flag | Description |
| ---- | ----------- |
| `-m, --model <MODEL>` | Model ID (for example `mad-build`). |
...
Clients connect over WebSocket and authenticate with the secret token. If you omit `--secret`, the agent prints a generated token at startup, or set `MAD_AGENT_SECRET`. The process keeps state across client reconnects. Permissions match other entry points; see [Permissions and safety](22-permissions-and-safety.md).
...
```bash
mad agent --always-approve headless --mad-ws-url wss://your-relay.example.com/ws
```
...
Beyond the base ACP protocol, Mad defines extension methods under the `x.ai/` prefix for SpaceXAI-specific functionality. These cover:
...
class MadACPChat {
  ...
  async init() {
    this.proc = spawn("mad", ["agent", "--always-approve", "stdio"]);
