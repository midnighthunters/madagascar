...
Hooks let you run a script or send an HTTP request at key moments in a Mad session. Use them to automate tasks, enforce safety checks, log activity, send notifications, and integrate your own tools.

---

## What Are Hooks?

A hook is a shell command or HTTP endpoint that Mad calls when a specific lifecycle event occurs. Hooks can:
...
2. Create a hook file, e.g. `~/.mad/hooks/session-start.json`:
   ...
   ```json
   {
     "hooks": {
       "SessionStart": [
         {
           "hooks": [
             { "type": "command", "command": "echo 'Mad session started in '$(pwd)" }
           ]
         }
       ]
     }
...
3. Start (or restart) a Mad session. The hook runs automatically on `SessionStart`.
...
|-------|------|----------|-------|
| Global | `~/.mad/hooks/*.json` | Always | Personal hooks |
...
| Project | `<project>/.mad/hooks/*.json` | Requires trust | Per-repo automation |
...
| Config | `~/.mad/config.toml` | Always | Your hooks alongside the rest of your config |
| Config | `managed_config.toml` (`$MAD_HOME` and `/etc/mad`) | Always | Organization-distributed hooks (server-synced and on-device) |
...
Config-file hooks live in the same TOML your organization already controls; see [Hooks in Config Files](#hooks-in-config-files) for the format. The compatible vendor hook sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] hooks = false` in `~/.mad/config.toml` or the corresponding environment variable. See [Configuration](05-configuration.md#harness-compatibility) for details.

**Trusting a project**: The first time you open a project with hooks, you must trust it before its project hooks will run -- until then they are silently skipped. Grant trust by running `/hooks-trust` (or launching with `--trust`); the decision is recorded in the unified folder-trust store (`~/.mad/trusted_folders.toml`), the same gate that governs repo-local MCP/LSP servers. Global hooks in `~/.mad/hooks/` are always trusted and need no entry. This prevents untrusted repos from running arbitrary code.

Because hooks are unified under folder-trust, a `--trust` / `/hooks-trust` grant trusts the whole folder for **MCP, LSP, and hooks** together, and cascades to subdirectories. Conversely, disabling folder-trust (`MAD_FOLDER_TRUST=0` or `[folder_trust] enabled = false`) ungates project hooks along with MCP/LSP.
...
Mad accepts Cursor's camelCase hook event names, so `~/.cursor/hooks.json` loads unchanged:
...
- **Event name** (top-level key): any event listed in [Hook Events](#hook-events). Mad skips unrecognized event names so a shared Claude or Cursor settings file still loads.
...
In a `matcher`, Mad maps Claude-style tool names to its own so hooks migrated from Claude fire correctly. Common aliases include:
...
Hooks can also live directly in your Mad config, so a team can distribute them with the rest of their configuration instead of shipping separate JSON files. The same `hooks` object is read from three TOML files:

| File | Tier | Who sets it |
|------|------|-------------|
| `~/.mad/config.toml` | User | You |
| `managed_config.toml` (`$MAD_HOME`, `/etc/mad`) | Managed / system | Your organization |
...
- **camelCase input**: mad's stdin envelope uses camelCase keys throughout where Claude uses snake_case. A script reading `.stop_hook_active`, `.hook_event_name`, or `.background_tasks[].agent_type` must switch to `.stopHookActive`, `.hookEventName`, and `.backgroundTasks[].agentType` (the event value is `"stop"`). Hooks registered through the mad-agent-sdk convert both the top-level keys and the `backgroundTasks`/`sessionCrons` entry keys to snake_case, so the wire's `.backgroundTasks[].agentType` reads as `.background_tasks[].agent_type` in the SDK.
...
- **StopFailure classes**: the emitted set is Claude Code's vocabulary — `rate_limit`, `authentication_failed`, `invalid_request`, `server_error`, `max_output_tokens`, `unknown`. mad emits a subset: capacity errors (503/529) fold into `rate_limit` as in Claude, and `billing_error` is never emitted (no signal), so a `billing_error` matcher will not fire.
- **permission_mode values**: mad emits `default`, `auto`, `plan`, or `bypassPermissions`. Claude's `acceptEdits`/`dontAsk` have no mad equivalent (mad's `auto` is the nearest), so a check like `permission_mode === "acceptEdits"` never matches.
...
- **`/goal`**: mad's goal loop is a separate feature that runs before the stop gate; it is not a prompt-type Stop hook.
...
Mad sets several environment variables on every hook process. These are useful when writing context-aware or plugin-aware hook scripts.
...
|-----------------------|-------------|
| `MAD_HOOK_EVENT`     | The name of the event that triggered the hook (e.g. `pre_tool_use`, `session_start`, `post_tool_use`, `session_end`, `stop`, `notification`). |
| `MAD_HOOK_NAME`      | The configured name of this specific hook (includes the plugin prefix for plugin-provided hooks). |
| `MAD_SESSION_ID`     | The unique identifier of the current Mad session. |
| `MAD_WORKSPACE_ROOT` | Absolute path to the root of the current workspace. |
| `CLAUDE_PROJECT_DIR`  | Absolute path to the workspace root. A Claude Code-compatible alias for `MAD_WORKSPACE_ROOT`, set for every hook. |
...
When a hook originates from a plugin, Mad additionally injects the following variables:

| Variable             | Description |
|----------------------|-------------|
| `MAD_PLUGIN_ROOT`   | Absolute path to the plugin's installed directory. |
| `MAD_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory (for storing plugin state, caches, etc.). |

These values are provided by the plugin system. For the four plugin-related keys (`MAD_PLUGIN_ROOT`, `MAD_PLUGIN_DATA`, and their Claude aliases), the plugin adapter ensures the official plugin values always win over any user-declared values in the hook's `env` map.
...
```json
{ "type": "http", "url": "https://hooks.example.com/mad-event", "timeout": 15 }
```
...
Press `r` in the Hooks tab to reload all hooks from disk. Mad re-reads every hook source, so this picks up changes you made to hook files during the session.
...
- Global hooks (`~/.mad/hooks/`) run with your user permissions -- treat them like shell scripts.
...
5. **Version control project hooks** -- commit `.mad/hooks/` (but never secrets).
...
- **See errors?** Capture logs by launching with `RUST_LOG=debug MAD_LOG_FILE=/tmp/mad.log mad`, then check `/tmp/mad.log`.
