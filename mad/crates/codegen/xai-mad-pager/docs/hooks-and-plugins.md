...
Madagascar Build supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.mad/hooks/`
- **Project hooks** — from `.mad/hooks/` in your repo
...
Plugin hooks automatically receive `MAD_PLUGIN_ROOT` and `MAD_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.mad/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `mad plugin marketplace add <source>`)
...
Sources land in `~/.mad/config.toml`:
...
Or in `~/.mad/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../xai-mad-hooks/examples/README.md) — ready-to-use sample hooks
