...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("MAD_EVENT", event_str)
        .env("MAD_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("MAD_SESSION_ID", sid);
    }
...
mod tests {
    ...
    fn sets_environment_variables() {
        ...
        let command = format!(
            "printf 'MAD_EVENT=%s\\nMAD_MESSAGE=%s\\nMAD_SESSION_ID=%s\\n' \
             \"$MAD_EVENT\" \"$MAD_MESSAGE\" \"$MAD_SESSION_ID\" > {}",
            out.display()
        );
        ...
        assert!(
            content.contains("MAD_EVENT=Turn complete"),
            "missing MAD_EVENT: {content}"
        );
        assert!(
            content.contains("MAD_MESSAGE=hello world"),
            "missing MAD_MESSAGE: {content}"
        );
        assert!(
            content.contains("MAD_SESSION_ID=sess-42"),
            "missing MAD_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("MAD_SESSION_ID"),
            "MAD_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("MAD_EVENT=Turn complete"));
        assert!(content.contains("MAD_MESSAGE=test body payload"));
        assert!(content.contains("MAD_SESSION_ID=test-session-123"));
    }
}
