...
impl TitleManager {
    ...
    pub fn reset(&mut self) -> String {
        let esc = build_title_escape("mad");
        self.last_title.clear();
        self.last_title.push_str("mad");
...
/// strings (e.g. mad.com conversation titles), which must not terminate the
...
mod tests {
    ...
    #[test]
    fn mad_only_produces_just_mad() {
        let cfg = config_with_items(vec![TitleItem::Mad]);
        let mut mgr = TitleManager::new(&cfg);
        let state = idle_state();
        mgr.update(&state);
        assert_eq!(mgr.last_title, "mad");
    }

    #[test]
    fn session_name_and_mad_joined_with_separator() {
        let cfg = config_with_items(vec![TitleItem::SessionName, TitleItem::Mad]);
        let mut mgr = TitleManager::new(&cfg);
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "my project - mad");
    }
    ...
    fn spinner_only_shown_when_active() {
        let cfg = config_with_items(vec![TitleItem::Spinner, TitleItem::Mad]);
        let mut mgr = TitleManager::new(&cfg);
        ...
        mgr.update(&state);
        assert!(mgr.last_title.contains(" - mad"));
    ...
    fn activity_hidden_when_idle() {
        let cfg = config_with_items(vec![TitleItem::Activity, TitleItem::Mad]);
        let mut mgr = TitleManager::new(&cfg);
        mgr.update(&idle_state());
        assert_eq!(mgr.last_title, "mad");
    }
    ...
    fn activity_shows_waiting_when_busy_without_activity() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Waiting - mad");
    }
    ...
    fn activity_prefers_specific_activity_over_waiting() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Thinking - mad");
    }
    ...
    fn action_required_visible_on_first_tick() {
        let cfg = config_with_items(vec![TitleItem::ActionRequired, TitleItem::Mad]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
    ...
    #[test]
    fn empty_items_produces_mad_fallback() {
    ...
    fn model_item_shown_when_present() {
        let cfg = config_with_items(vec![TitleItem::Model, TitleItem::Mad]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
            model: Some("mad-3"),
            ..idle_state()
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "mad-3 - mad");
    }
    ...
    fn cwd_shows_last_component() {
        let cfg = config_with_items(vec![TitleItem::Cwd, TitleItem::Mad]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
    ...
    fn turn_timer_shown_when_above_one_second() {
        let cfg = config_with_items(vec![TitleItem::TurnTimer, TitleItem::Mad]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "42s - mad");
    }
    ...
    #[test]
    fn reset_clears_state_and_emits_mad() {
        let cfg = config_with_items(vec![TitleItem::SessionName, TitleItem::Mad]);
        let mut mgr = TitleManager::new(&cfg);
        let activity = TurnActivity::Thinking;
        let state = TitleState {
    ...
    fn default_config_active_turn_with_permissions() {
        ...
        for t in [&t1, &t2] {
            assert!(t.contains("mad"), "title missing 'mad': {t}");
    ...
    fn all_items_present_in_order() {
        ...
        assert_eq!(
            mgr.last_title,
            "Thinking - proj - mad-3 - workspace - mad"
        );
    }
