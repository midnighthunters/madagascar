//! `mad mcp` — manage MCP server configurations from the command line.
...
use xai_mad_shell::util::config::{McpServerConfig, McpServerTransportConfig};

use crate::util::display_user_mad_path;

const ADD_AFTER_HELP: &str = "\
Examples:
  # Add a stdio server (everything after -- is the server command)
  mad mcp add xcode -- xcrun mcpbridge
  ...
  # Add to the project config (./.mad/config.toml) instead of ~/.mad/config.toml
  mad mcp add --scope project github -- npx -y @modelcontextprotocol/server-github";
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum McpScope {
    /// `~/.mad/config.toml`, available in all your projects
    User,
    /// `./.mad/config.toml`, shared with everyone working in this directory
    Project,
}
...
pub struct AddArgs {
    ...
    /// flags such as `-y` are passed to the server instead of mad.
    #[arg(value_name = "ARGS")]
    args: Vec<String>,
    ...
    /// Config to write to: user (~/.mad/config.toml) or project (./.mad/config.toml)
...
fn run_list(json: bool) -> Result<()> {
    ...
    let servers = xai_mad_shell::util::config::load_mcp_server_configs_with_project(&cwd);
    let disabled = xai_mad_shell::util::config::disabled_mcp_server_names(&cwd);
    ...
    if json {
        ...
        println!("No MCP servers configured. Run `mad mcp add --help` to get started.");
    } else {
...
async fn run_add(args: AddArgs) -> Result<()> {
    ...
    xai_mad_shell::util::config::save_mcp_server_config_at(&path, name, &config).await?;
    ...
    match transport {
        McpTransport::Stdio => {
            let Some(command) = source else {
                bail!(
                    "A command is required for stdio servers. Usage: mad mcp add <name> -- <command> [args...]"
                );
            };
        ...
        McpTransport::Http | McpTransport::Sse => {
            ...
            let Some(url) = source else {
                bail!(
                    "A URL is required for {label} servers. Usage: mad mcp add --transport {label} <name> <url>"
                );
            };
...
fn scope_target(scope: McpScope) -> PathBuf {
    match scope {
        McpScope::User => xai_mad_shell::util::config::user_config_path(),
        McpScope::Project => {
            xai_mad_shell::util::config::project_config_path(&current_dir_or_exit())
        }
    }
}
...
fn scope_display(scope: McpScope, path: &Path) -> String {
    match scope {
        McpScope::User => display_user_mad_path("config.toml"),
...
fn select_remove_site(
    ...
    use xai_mad_shell::util::config::user_config_path;
    ...
    project_site
        ...
        .or_else(|| {
            user_defined.then(|| {
                (
                    McpScope::User,
                    xai_mad_shell::util::config::user_config_path(),
                )
            })
        })
}

/// TOML / disabled list / compat JSON / legacy `mad_com_*` (not gateway).
fn mcp_server_is_known(name: &str, cwd: &Path) -> bool {
    if name.starts_with("mad_com_") {
        return true;
    }
    xai_mad_shell::util::config::cli_known_mcp_server_names(cwd).contains(name)
}

fn available_mcp_server_names(cwd: &Path) -> Vec<String> {
    let mut names: Vec<String> = xai_mad_shell::util::config::cli_known_mcp_server_names(cwd)
        .into_iter()
        .collect();
    names.sort();
    names
...
async fn run_set_enabled(name: &str, enabled: bool) -> Result<()> {
    ...
    if !mcp_server_is_known(name, &cwd) {
        ...
        if !available.is_empty() {
            ...
            eprintln!("No MCP servers configured. Run `mad mcp add --help` to get started.");
        }
        std::process::exit(1);
    }

    let was_disabled = xai_mad_shell::util::config::disabled_mcp_server_names(&cwd).contains(name);

    let modified =
        xai_mad_shell::util::config::save_mcp_server_enabled_in(name, enabled, &cwd).await?;

    let now_disabled = xai_mad_shell::util::config::disabled_mcp_server_names(&cwd).contains(name);
    ...
    let user_config = xai_mad_shell::util::config::user_config_path();
    for path in &modified {
        if path == &user_config {
            println!("File modified: {}", display_user_mad_path("config.toml"));
        } else {
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let find_project_site = || {
        xai_mad_shell::config::find_project_configs(&cwd)
            .into_iter()
            .rev()
    ...
    let (scope, path) = match select_remove_site(user_defined, find_project_site(), requested_scope)
    {
        ...
        Err(RemoveError::Ambiguous { project_path }) => {
            ...
            eprintln!("  user: {}", display_user_mad_path("config.toml"));
            eprintln!("  project: {}", project_path.display());
            eprintln!("Specify which one to remove, e.g.: mad mcp remove {name} --scope project");
            std::process::exit(1);
        }
    };
    ...
    // ancestor .mad/config.toml, where it still resolves for sessions.
...
async fn run_doctor(json: bool, name: Option<String>) -> Result<()> {
    let cwd = current_dir_or_exit();
    let report = xai_mad_shell::mcp_doctor::run_doctor(&cwd, name.as_deref()).await;
    ...
    if json {
        ...
        xai_mad_shell::mcp_doctor::print_report(&report);
    }
...
mod tests {
    ...
    fn add_hyphen_flag_without_double_dash_is_rejected() {
        let err = PagerArgs::try_parse_from(["mad", "mcp", "add", "fs", "npx", "-y"])
    ...
    fn add_default_transport_warns_on_url_looking_command() {
        let add = parse_add(&["mad", "mcp", "add", "api", "https://mcp.example.com/mcp"]);
        ...
        let add = parse_add(&["mad", "mcp", "add", "local", "localhost:3000"]);
    ...
    fn add_scope_project_parses_and_invalid_scope_is_rejected() {
        let add = parse_add(&[
            "mad", "mcp", "add", "-s", "project", "fs", "--", "npx", "pkg",
        ]);
        assert_eq!(add.scope, McpScope::Project);

        let err = PagerArgs::try_parse_from([
            "mad", "mcp", "add", "-s", "local", "fs", "--", "npx", "pkg",
        ])
        .expect_err("local is not a mad scope");
    ...
    fn add_legacy_multi_value_env_is_rejected() {
        ...
        let add = parse_add(&[
            "mad", "mcp", "add", "pg", "--env", "A=1", "B=2", "--", "npx", "-y", "server",
        ]);
    ...
    fn add_validates_name_env_and_headers() {
        let add = parse_add(&["mad", "mcp", "add", "fs", "-e", "NOT_A_PAIR", "--", "npx"]);
        ...
        let add = parse_add(&["mad", "mcp", "add", "bad name!", "--", "npx"]);
    ...
    fn add_rejects_mismatched_options_per_transport() {
        let add = parse_add(&["mad", "mcp", "add", "fs", "-H", "X: y", "--", "npx"]);
    ...
    fn add_requires_a_source_for_each_transport() {
        let add = parse_add(&["mad", "mcp", "add", "fs"]);
        ...
        let add = parse_add(&["mad", "mcp", "add", "--transport", "http", "api"]);
    ...
    fn remove_accepts_optional_scope() {
        let args = PagerArgs::try_parse_from(["mad", "mcp", "remove", "fs", "-s", "project"])
    ...
    fn enable_and_disable_parse_name() {
        let args = PagerArgs::try_parse_from(["mad", "mcp", "enable", "user-grafana"])
        ...
        let args = PagerArgs::try_parse_from(["mad", "mcp", "disable", "mad_com_slack"])
            .expect("disable should parse");
        match args.command {
            Some(Command::Mcp(McpArgs {
                command: McpCommand::Disable { name },
            })) => assert_eq!(name, "mad_com_slack"),
    ...
    fn enable_disable_require_name() {
        let err = PagerArgs::try_parse_from(["mad", "mcp", "enable"])
            .expect_err("enable without name must fail");
        assert_eq!(err.kind(), clap::error::ErrorKind::MissingRequiredArgument);

        let err = PagerArgs::try_parse_from(["mad", "mcp", "disable"])
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        let user = xai_mad_shell::util::config::user_config_path();
        let project = PathBuf::from("/repo/.mad/config.toml");
