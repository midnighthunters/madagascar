...
//! render mode.
//!
//! `xai-mad-pager` cannot depend on `xai-mad-pager-minimal`: that crate reads
...
//!
//! The composition-root binary (`xai-mad-pager-bin`) wires it up once at
//! startup by calling `xai_mad_pager_minimal::install()`. When nothing has
