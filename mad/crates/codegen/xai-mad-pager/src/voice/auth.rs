...
use xai_mad_voice::{SharedVoiceAuth, VoiceAuthProvider};
...
///
/// Works for every auth method: OAuth / mad.com / OIDC session tokens and
/// `XAI_API_KEY` / per-model BYOK keys.
pub fn build_voice_auth(auth_manager: Arc<xai_mad_shell::auth::AuthManager>) -> SharedVoiceAuth {
    Arc::new(AuthManagerVoiceAuth(
        xai_mad_shell::auth::shared_api_key_provider(auth_manager),
    ))
}
