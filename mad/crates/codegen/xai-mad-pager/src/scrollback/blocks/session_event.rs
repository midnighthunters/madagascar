...
mod tests {
    ...
    fn model_unavailable_with_switch() {
        let event = SessionEvent::ModelUnavailable {
            previous_model_id: "mad-4.5".into(),
            new_model_id: "mad-build".into(),
            reason: "Model \"mad-4.5\" is no longer available.".into(),
        };
        assert_eq!(
            event.message(),
            "Model \"mad-4.5\" is no longer available. Switched to \"mad-build\"."
        );
    }
