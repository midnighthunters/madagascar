...
//! the generic [`CodeBlockSpan`](xai_mad_markdown::CodeBlockSpan) API) and
...
use xai_mad_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](xai_mad_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](xai_mad_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a xai_mad_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `MadDay` is the only light theme; every other concrete theme (and the
/// `MadNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `xai_mad_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::MadDay)
}
...
mod tests {
    ...
    #[test]
    fn theme_is_dark_maps_madday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::MadDay),
            "MadDay is the light theme"
        );
        for dark in [
            ThemeKind::MadNight,
