...
fn canonical_and_short_ids_resolve_to_canonical_id() {
    ...
    assert_eq!(command, "mad doctor fix ssh-wrap");
    assert_eq!(
        resolve_fix_id(command.strip_prefix("mad doctor fix ").unwrap()).unwrap(),
        SSH_WRAP_ID
    );
...
fn tmux_managed_items_coexist_and_each_apply_is_one_transaction() {
    ...
    assert_eq!(content.matches("# >>> mad doctor >>>").count(), 1);
...
fn tmux_stale_plan_and_idempotence_reuse_managed_writer_safety() {
    ...
    assert!(matches!(
        ...
        Err(FixError::TmuxManaged(
            xai_mad_config::managed_text::ManagedConfigError::StalePlan(_)
        ))
    ));
...
fn bash_zsh_and_fish_plans_use_exact_paths_and_aliases() {
    ...
    for (shell, relative, alias) in [
        ("/bin/bash", ".bashrc", "alias ssh='mad wrap ssh'"),
        ("/bin/zsh", ".zshrc", "alias ssh='mad wrap ssh'"),
        (
            "/usr/local/bin/fish",
...
fn non_utf8_source_fails_closed_before_conflict_policy() {
    ...
    assert!(matches!(
        ...
        Err(FixError::Managed(
            xai_mad_config::managed_text::ManagedConfigError::UnsafePath { .. }
        ))
    ));
}
...
fn shell_aliases_expand_to_exact_argv_and_bypass_is_explicit() {
    ...
    .unwrap();
    use std::os::unix::fs::PermissionsExt as _;
    std::fs::set_permissions(&mad, std::fs::Permissions::from_mode(0o755)).unwrap();

    if let Some(bash) = find_on_path("bash") {
        let rc = temp.path().join("bashrc");
        std::fs::write(&rc, "alias ssh='mad wrap ssh'\n").unwrap();
        let command = format!(
    ...
    let mut shell = std::process::Command::new(bash);
    shell
        .args(["-ic", "alias ssh='mad wrap ssh'; command ssh host"])
        .env("CAPTURE", &capture)
        .env(
            "PATH",
            format!(
    ...
    if let Some(fish) = find_on_path("fish") {
        ...
        let fish_mad = temp.path().join("fish-mad");
        std::fs::write(
            &fish_mad,
            format!(
                "#!/bin/sh\nprintf '%s\\n' \"$@\" > '{}'\n",
        ...
        .unwrap();
        std::fs::set_permissions(&fish_mad, std::fs::Permissions::from_mode(0o755)).unwrap();
        let rc = temp.path().join("config.fish");
        std::fs::write(&rc, "alias ssh 'fish-mad wrap ssh'\n").unwrap();
        let command = format!(
