...
use xai_mad_shell::agent::auth_method::AuthMethodKind;
use xai_mad_shell::agent::config::Config as AgentConfig;
use xai_mad_shell::sampling::types::ReasoningEffort;
...
pub struct AcpConnection {
    ...
    /// Whether the agent is a mad-shell instance.
    pub is_mad_shell: bool,
    ...
    // When available, add field: startup_announcements: Option<Vec<xai_mad_announcements::RemoteAnnouncement>>
    /// Whether interactive login is required (deferred auth for `mad.com`).
    ...
    pub auth_manager: std::sync::Arc<xai_mad_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<xai_mad_shell::util::config::RemoteSettings>,
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = spawn::spawn_mad_shell(agent_config, cancel, memory_config).await?;
...
pub async fn connect_via_leader(
    ...
    let auth_manager = std::sync::Arc::new(xai_mad_shell::auth::AuthManager::new(
    ...
    xai_mad_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = xai_mad_shell::util::mad_home::mad_home().join("config.toml");
...
async fn initialize(
    ...
    // Check if this is a mad-shell agent
    let is_mad_shell = resp
        .meta
        .as_ref()
        .and_then(|m| m.get("madShell"))
...
///
/// Matches TUI startup behavior: if the first method is `mad.com`, defer auth
...
/// for a `mad.com` or `oidc` method — these are the ones that can trigger
...
mod tests {
    ...
    fn parse_available_commands_missing_key_returns_empty() {
        let meta = serde_json::json!({ "madShell": true });
        let cmds = parse_available_commands(meta.as_object());
    ...
    fn parse_session_recap_available_defaults_off_when_missing() {
        let meta = serde_json::json!({ "madShell": true, "cancelRewind": true });
        assert!(!parse_session_recap_available(meta.as_object()));
    ...
    #[test]
    fn startup_auth_mad_com_no_provider_needs_login_pending() {
        let methods = vec![make_auth_method("mad.com", "mad.com", None)];
        let (needs, label, method_id, mode) = startup_auth_metadata(&methods);
        assert!(needs);
        assert_eq!(label.as_deref(), Some("mad.com"));
        assert_eq!(method_id.as_ref().unwrap().0.as_ref(), "mad.com");
        assert_eq!(mode, AuthStartMode::Pending);
    }

    #[test]
    fn startup_auth_mad_com_with_external_provider_command() {
        let meta = serde_json::json!({ "external_provider": true });
        let methods = vec![make_auth_method("mad.com", "Acme Corp", Some(meta))];
        let (needs, label, method_id, mode) = startup_auth_metadata(&methods);
        assert!(needs);
    ...
    #[test]
    fn startup_auth_non_mad_com_no_login() {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.mad/config.toml` with a
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use xai_mad_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};

        let built = build_auth_methods(AuthMethodsBuildInputs {
            // enterprise-style: model has `env_key` set and the env var resolves,
            // so the shell-side predicate returns true.
            has_external_api_key: true,
            // Realistic enterprise user: no cached session token, default `mad.com`
    ...
    /// `startup_auth_mad_com_no_provider_needs_login_pending` above and
    /// either passes or fails on a meaningful new code path.
    #[test]
    fn startup_auth_xai_api_key_not_first_still_requires_login() {
        use xai_mad_shell::agent::auth_method::{MAD_COM_METHOD_ID, XAI_API_KEY_METHOD_ID};

        let methods = vec![
            make_auth_method(MAD_COM_METHOD_ID, "Mad", None),
            make_auth_method(XAI_API_KEY_METHOD_ID, "xai.api_key", None),
        ];
        let (needs, _, _, _) = startup_auth_metadata(&methods);
        assert!(
            needs,
            "with mad.com first, the pager must require login -- pinning \
    ...
    fn startup_auth_method_id_is_copied_not_synthesized() {
        let methods = vec![make_auth_method("mad.com", "My Login", None)];
    ...
    fn startup_auth_external_provider_false_is_pending() {
        ...
        let methods = vec![make_auth_method("mad.com", "mad.com", Some(meta))];
