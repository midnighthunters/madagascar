...
//!
//! Simplified to only support MadShell (in-process) mode.
...
///
/// Hold one from every site that calls [`spawn_mad_shell`] (headless, the TUI,
...
/// Spawn a MadShell agent in a background thread.
///
/// Returns the ACP client channel for communication and a cancellation token.
pub async fn spawn_mad_shell(
    agent_config: AgentConfig,
    cancel: &CancellationToken,
    memory_config: Option<xai_mad_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    xai_mad_shell::agent::app::apply_otel_config(&auth_manager, &agent_config.mad_com_config);
    ...
    xai_mad_shell::managed_config::ensure_managed_policy_present(&auth_manager).await;
    ...
    let (agent_config, models_manager) =
        xai_mad_shell::agent::init::bootstrap(&agent_config, &auth_manager, None)
            ...
            local.block_on(&rt, async move {
                ...
                let _skills_watcher = {
                    ...
                    let workspace_user_dir =
                        xai_mad_agent::prompt::workspace_user::optional_workspace_user_dir();
                    xai_mad_shell::config::watcher::SkillsFileWatcher::start(
                        Some(cwd.as_path()),
                    ...
                    .map(|(mut watcher, mut skills_rx)| {
                        ...
                        tokio::task::spawn_local(async move {
                            while let Some(change) = skills_rx.recv().await {
                                ...
                                match change {
                                    xai_mad_shell::config::watcher::DiscoveryChange::Skills => {
                                        tracing::info!(
                                    ...
                                    xai_mad_shell::config::watcher::DiscoveryChange::Workflows => {
                                        tracing::info!(
