...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: xai_mad_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    /// Open mad.com in the browser for SuperMad subscription upsell.
    OpenSupermadUrl,
    ...
    /// Open mad.com managed connectors, appending session teamId when set.
    ...
    OpenTranscriptPager,
    /// Minimal mode (`mad --minimal`): re-print the most-recently committed
    ...
    OpenExtensionsModal {
        ...
        trigger: xai_mad_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    AnnouncementsOpenCta(xai_mad_telemetry::events::AnnouncementCtaSurface),
    ...
    /// Commit the active theme (canonical name, e.g. `"madnight"`, `"auto"`).
    SetTheme(String),
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `mad dashboard`).
    OpenDashboard,
...
///
/// Both variants write to `~/.mad/config.toml` and route ACP
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: xai_mad_workspace::foreign_sessions::EnabledForeignSessionSources,
        mad_home: std::path::PathBuf,
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.mad/config.toml`.
    ...
    /// Persist a typed setting to `~/.mad/config.toml`. On failure,
    ...
    /// registry (`~/.mad/active_sessions.json`).
    ...
    FetchPromptSuggestion {
        agent_id: AgentId,
        generation: u64,
        /// Suggestion model resolved by the pager (`mad-build-0.1` when the
...
pub enum TaskResult {
    ...
    WorktreeForked {
        ...
        restore_degree: Option<xai_mad_workspace::session::git::RestoreDegree>,
    },
    ...
    SessionListLoaded {
        ...
        scope: xai_mad_shell::session::unified_list::ListScope,
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<xai_mad_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<xai_mad_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<xai_mad_shell::util::changelog::ChangelogEntry>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<xai_mad_tools::implementations::skills::types::SkillInfo>, String>,
    },
    WorkflowsListLoaded {
        agent_id: AgentId,
        session_id: acp::SessionId,
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        info: Box<xai_mad_shell::session::SessionInfoResponse>,
        text: String,
    },
    ...
    SessionUsageComplete {
        agent_id: AgentId,
        session_id: acp::SessionId,
        usage: Box<xai_mad_shell::extensions::notification::PromptUsage>,
    },
    ...
    DeepSearchResults {
        results: Vec<xai_mad_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
