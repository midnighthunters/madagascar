...
/// `xai-mad-shell/src/agent/handlers/session.rs::handle_roster_list`), which
...
mod tests {
    ...
    fn agent_entry() -> xai_mad_shell::agent::roster::RosterEntry {
        use xai_mad_shell::agent::roster as agent;
        agent::RosterEntry {
    ...
    fn roster_list_response_survives_result_envelope() {
        ...
        assert_eq!(e.model_id.as_deref(), Some("mad-4"));
        assert!(e.yolo);
