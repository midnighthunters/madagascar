...
use super::*;
use xai_mad_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn format_acp_error_rate_limit_surfaces_detail_or_fallback() {
    ...
    let rpm_body = "You are sending requests too quickly. Please slow down, or upgrade to a Mad subscription for higher limits: https://mad.com/supermad";
    let rpm = acp::Error::new(RATE_LIMITED_ERROR_CODE, "Rate limited")
        .data(format!("API error (status 429 Too Many Requests): {rpm_body}"));
    assert!(format_acp_error(&rpm, false).contains("mad.com/supermad"));
...
/// Untitled mad.com chats must stay listed, rendered as "Untitled".
#[test]
...
fn parse_kill_outcome_reads_result_envelope() {
    use xai_mad_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use xai_mad_workspace::session::git::RestoreDegree;
...
/// Redirect `MAD_HOME` to a tempdir for test isolation.
fn setup_mad_home_in_tempdir() -> tempfile::TempDir {
    let tmp = tempfile::tempdir().expect("tempdir creation");
    unsafe {
        std::env::set_var("MAD_HOME", tmp.path());
    }
    tmp
}
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use xai_mad_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
            xai_mad_shell::active_sessions::list_in(dir.path())
                .expect("list")
                .is_empty(),
...
async fn persist_permission_mode_acp_notification_fires_once_on_best_effort() {
    ...
    let _guard = setup_mad_home_in_tempdir();
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: xai_mad_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
            mad_home: PathBuf::from("/path/that/must/not/be-read"),
...
fn agent_profile_names_are_valid_builtins() {
    use std::str::FromStr;
    use xai_mad_agent::config::BuiltinAgentName;
...
/// Default flags produce no agent profile (uses mad-build default).
#[test]
fn default_flags_produce_no_profile() {
    let flags = SessionFlags::default();
    assert_eq!(flags.agent_profile(), None);
}
/// --subagents alone produces no profile (mad-build already has TaskTool).
#[test]
...
/// Neutralize `MAD_AGENT` for the profile-matrix tests below: agent-driven
...
/// Callers must be `#[serial_test::serial(MAD_AGENT)]` (process-global env).
fn without_mad_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("MAD_AGENT", "")
}
...
#[serial_test::serial(MAD_AGENT)]
#[test]
fn runtime_default_flags_produce_plan_meta() {
    let _env = without_mad_agent();
    let flags = SessionFlags {
        plan_mode: true,
        subagents: true,
        ask_user: true,
    ...
    assert_eq!(meta["agentProfile"], "mad-build-plan");
...
fn plan_only_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "mad-build-plan-no-subagents");
...
/// --ask-user alone selects the mad-build-ask-user profile.
#[serial_test::serial(MAD_AGENT)]
#[test]
fn ask_user_alone_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "mad-build-ask-user");
    assert!(meta.get("askUserQuestion").is_none());
    assert_eq!(meta["yoloMode"], false);
}
...
/// does not need an `agentProfile` (default `mad-build` already has it).
#[test]
...
/// All three flags on at the runtime default produce mad-build-plan
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> xai_mad_shell::session::SessionInfoResponse {
    use xai_mad_shell::session::acp_types::{ContextInfo, SessionInfoData};
    xai_mad_shell::session::SessionInfoResponse {
...
fn format_session_info_session_auth_ignores_api_key_env() {
    ...
    assert!(
            text.contains("Manage account and credits: https://mad.com/?_s=billing"),
            "{text}"
        );
    assert!(!text.contains("Also present: XAI_API_KEY"), "{text}");
    assert!(!text.contains("console.x.ai"), "{text}");
    assert!(!text.contains("mad login"), "{text}");
}
#[test]
fn format_session_info_api_key_without_env() {
    ...
    assert!(
            text.contains("Run `mad login` to use your SuperMad subscription instead."),
            "{text}"
        );
    assert!(!text.contains("mad.com"), "{text}");
}
#[test]
...
#[test]
fn format_session_info_session_only_manage_at_mad_com() {
    let info = make_session_info("auto", None, 1000, 10000);
...
fn format_session_info_shows_resolved_when_enabled_and_different() {
    let info = make_session_info("mad-4.5", Some("mad-4.3"), 1000, 10000);
    let text = format_session_info(&info, None, true, false, false);
    assert!(text.contains("Model: mad-4.5 (mad-4.3)"));
}
#[test]
fn format_session_info_hides_resolved_when_disabled() {
    let info = make_session_info("mad-4.5", Some("mad-4.3"), 1000, 10000);
    let text = format_session_info(&info, None, false, false, false);
    assert!(text.contains("Model: mad-4.5"));
    assert!(!text.contains("mad-4.3"));
}
#[test]
fn format_session_info_no_parens_when_resolved_matches_requested() {
    let info = make_session_info("mad-4.5", Some("mad-4.5"), 1000, 10000);
    let text = format_session_info(&info, None, true, false, false);
    assert!(text.contains("Model: mad-4.5"));
    assert!(!text.contains("(mad-4.5)"));
}
#[test]
...
fn format_session_info_shows_model_hash_for_coding_slug_without_flag() {
    let mut info = make_session_info("mad-build", None, 1000, 10000);
    info.data.model_fingerprint = Some("abc123".into());
    info.data.show_model_fingerprint = false;
    let text = format_session_info(&info, None, false, false, false);
    assert!(text.contains("Model Hash: abc123"));
...
fn session_picker_entry_maps_to_dormant_roster_row() {
    ...
    assert_eq!(roster.model_id.as_deref(), Some("mad-4"));
