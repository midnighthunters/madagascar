...
use xai_mad_shell::sampling::error::http_status_from_error;
use xai_mad_shell::session::{ExtMethodResult, SessionInfoResponse};
pub(crate) fn execute(
    ...
    match effect {
        Effect::RegisterActiveSession { session_id, cwd } => {
            ...
            if let Err(e) = xai_mad_shell::active_sessions::register(xai_mad_shell::active_sessions::ActiveSession {
                session_id,
                pid: std::process::id(),
                cwd,
        ...
        Effect::CreateSession {
            ...
            let tx = acp_tx.clone();
            let compat = xai_mad_tools::types::compat::CompatConfig::default();
            ...
            tasks
                .spawn(async move {
                    if let Some(sid) = load_session_id {
                        ...
                        let wt_type = xai_mad_shell::util::config::worktree_type();
        ...
        Effect::ScanForeignSessions { cwd, compat, mad_home, coordinator, seq } => {
            ...
            let abort_handle = tasks
                .spawn(async move {
                    ...
                    let summaries = tokio::task::spawn_blocking(move || {
                            let _permit = permit;
                            xai_mad_workspace::foreign_sessions::scan_foreign_sessions(
                                &cwd,
                                enabled,
                            )
                        })
            ...
            tasks
                .spawn(async move {
                    ...
                    let recent = crate::app::foreign_sessions::with_gated_sources_async(
                            ...
                            &mad_home,
                            |enabled| async move {
                                tokio::task::spawn_blocking(move || xai_mad_workspace::foreign_sessions::most_recent_foreign_session(
                                        &cwd_for_scan,
                                        enabled,
        ...
        Effect::RestoreAndLoadSession { agent_id, session_id, session_cwd: _ } => {
            use xai_mad_shell::agent::session_registry_client::SessionRegistryClient;
            use xai_mad_shell::session::restore::restore_session_with_storage;
            let setup_started = std::time::Instant::now();
            let raw_config = xai_mad_shell::config::load_effective_config();
            let setup = raw_config
                .ok()
                .and_then(|raw| {
                    let cfg = xai_mad_shell::agent::config::Config::new_from_toml_cfg(
                            &raw,
                        )
                        .ok()?;
                    ...
                    let storage = xai_mad_shell::auth::credential_provider::build_storage_client_for_proxy(
                        &proxy_base,
                        deployment_key,
                        alpha_test_key,
            ...
            tasks
                .spawn(async move {
                    ...
                    let progress: Option<
                        xai_mad_shell::session::restore::ProgressCallback,
                    > = {
                        use xai_mad_shell::session::restore::{PhaseStep, RestorePhase};
                        Some(
                            Box::new(move |event| {
            ...
            tasks
                .spawn(async move {
                    ...
                    let detail = tokio::task::spawn_blocking(move || {
                            ...
                            let history_path = xai_mad_shell::session::persistence::session_dir(
                                    &info,
                                )
        ...
        Effect::SendBashCommand { agent_id, session_id, command, prompt_id } => {
            ...
            tasks
                .spawn(async move {
                    use xai_mad_shell::extensions::prompt_meta::PromptBlockMeta;
                    ulog::info(
                        "prompt.acp_send.start",
                        Some(&session_id.0),
                        Some(
        ...
        Effect::KillBgTask { session_id, task_id } => {
            ...
            tasks
                .spawn(async move {
                    let params = xai_mad_shell::extensions::task::KillTaskRequest {
        ...
        Effect::SwitchModel {
            ...
            tasks
                .spawn(async move {
                    let meta = effort
                        .map(|eff| {
                            use xai_mad_shell::sampling::types::{
                    ...
                    let result = acp_send(req, &tx)
                        ...
                        .map(|_| ())
                        .map_err(|e| {
                            use xai_mad_shell::agent::config::ModelSwitchIncompatibleAgentError;
            ...
            tasks
                .spawn(async move {
                    let changelog = tokio::task::spawn_blocking(|| {
                            xai_mad_shell::util::changelog::ChangelogManager::new()
                                .fetch()
                        })
                        .await
                        .unwrap_or_else(|e| {
                            tracing::warn!(error = %e, "changelog fetch task failed");
                            xai_mad_shell::util::changelog::Changelog {
                                markdown: None,
                                entries: None,
                            }
                        });
        ...
        Effect::PersistAnnouncementsHidden { hidden_ids } => {
            tasks
                .spawn(async move {
                    xai_mad_announcements::write_hidden_announcement_ids(&hidden_ids)
                        .await;
        ...
        Effect::PersistPrivacyBannerAcked { acked_at } => {
            tasks
                .spawn(async move {
                    if let Err(e) = xai_mad_shell::util::config::set_privacy_banner_acked(
                            acked_at,
                        )
                        .await
                    {
        ...
        Effect::PersistPreferredModel { model_id, reasoning_effort } => {
            ...
            tasks
                .spawn(async move {
                    let result = xai_mad_shell::util::config::persist_models_default(
                            Some(model_id_str),
                            reasoning_effort,
                        )
                        .await
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            serde_json::from_value::<
                                Vec<
                                    xai_mad_tools::implementations::skills::types::SkillInfo,
                                >,
            ...
            tasks
                .spawn(async move {
                    ...
                    struct McpUpsertRequest {
                        ...
                        server_name: String,
                        #[serde(flatten)]
                        config: xai_mad_shell::util::config::McpServerConfig,
                    }
        ...
        Effect::ShareSession { agent_id, session_id } => {
            use xai_mad_shell::session::{ShareSessionRequest, ShareSessionResponse};
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    let request = acp::ExtRequest::new(
        ...
        Effect::ShowSessionInfo { agent_id, session_id, show_resolved_model } => {
            let is_api_key_auth = session_flags.is_api_key_auth;
            let api_key_env_set = xai_mad_shell::agent::auth_method::has_xai_api_key_env();
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    match fetch_session_info(&session_id, &tx).await {
        ...
        Effect::SendFeedback { agent_id, session_id, feedback_text } => {
            use xai_mad_shell::session::ClientType;
            use xai_mad_shell::session::acp_types::ClientFeedbackInput;
        ...
        Effect::SaveMemoryNote { agent_id, text, cwd } => {
            tasks
                .spawn(async move {
                    let result = tokio::task::spawn_blocking(move || {
                            let storage = xai_mad_shell::session::memory::MemoryStorage::new(
                                &cwd,
                                None,
                            );
                            storage
                                .append_to_memory(
                                    xai_mad_shell::session::memory::MemoryScope::Global,
                                    &text,
                                )
                        })
                        .await
                            ...
                            Ok(Ok(resp)) => {
                                ...
                                if let Some(hits) = payload.get("results") {
                                    results = serde_json::from_value::<
                                        Vec<
                                            xai_mad_shell::extensions::session_search::SearchSessionHit,
                                        >,
                                    >(hits.clone())
                                        .unwrap_or_default();
                                }
        ...
        Effect::HydrateSessionTitleFromDisk { agent_id, session_id, cwd } => {
            tasks
                .spawn(async move {
                    ...
                    let path = xai_mad_shell::session::persistence::session_dir(&info)
                        .join("summary.json");
                    let title = tokio::task::spawn_blocking(move || -> Option<
                            (String, bool),
                        > {
                            let raw = std::fs::read_to_string(path).ok()?;
                            let summary: xai_mad_shell::session::persistence::Summary = serde_json::from_str(
                                    &raw,
                                )
                                .ok()?;
        ...
        Effect::FetchBilling { agent_id, silent } => {
            ...
            tasks
                .spawn(async move {
                    use xai_mad_shell::extensions::billing::BillingConfigResponse;
                    let req = acp::ExtRequest::new(
                        "x.ai/billing",
                        serde_json::value::to_raw_value(&serde_json::json!({}))
            ...
            tasks
                .spawn(async move {
                    let settings = tokio::task::spawn_blocking(|| {
                            if !xai_mad_shell::util::config::resolve_remote_fetch_enabled() {
                                return None;
                            }
                            let mad_home = xai_mad_shell::util::mad_home::mad_home();
                            let store = xai_mad_shell::auth::read_auth_json(
                                    &mad_home.join("auth.json"),
                                )
                                .ok()?;
                            let scope = xai_mad_shell::auth::MadComConfig::default()
                                .auth_scope();
                            let auth = xai_mad_shell::auth::lookup_auth(
                                &store,
                                &scope,
                            )?;
                            let proxy_base = std::env::var(
                                ...
                                .unwrap_or_else(|_| {
                                    xai_mad_shell::agent::config::CLI_CHAT_PROXY_BASE_URL_DEFAULT
                                        .to_owned()
                                });
                            xai_mad_shell::remote::fetch_settings_blocking(
                                    &proxy_base,
                                    &auth,
                                    None,
                                )
...
async fn fetch_session_usage(
    session_id: &acp::SessionId,
    tx: &AcpAgentTx,
) -> Result<xai_mad_shell::extensions::notification::PromptUsage, String> {
    let request = acp::ExtRequest::new(
        "x.ai/session/usage",
        serde_json::value::to_raw_value(
                &serde_json::json!({
    ...
    let parsed: xai_mad_shell::extensions::usage::SessionUsageResponse = serde_json::from_str(
            resp.0.get(),
        )
        .map_err(|e| {
...
async fn lookup_session_title(session_id: &acp::SessionId) -> Option<String> {
    let summaries = xai_mad_shell::session::persistence::list_summaries(None)
        .await
        .ok()?;
    summaries
        .into_iter()
...
fn format_session_info(
    ...
    let model_display = xai_mad_shell::session::model_display_name(
    ...
    let model_hash_line = if xai_mad_shell::session::should_show_model_fingerprint(
    ...
    let backend_line = info
        ...
        .map(|b| format!("\n  API Backend: {b}"))
        .unwrap_or_default();
    let sandbox_line = xai_mad_sandbox::profile_name()
        .map(|profile| format!("\n  Sandbox: {profile}"))
        .unwrap_or_default();
    ...
    let conversation_line = info
        ...
        .map(|id| format!("\n  Conversation ID: {id}"))
        .unwrap_or_default();
    let version_display = xai_mad_version::display_version(
...
fn format_auth_lines(is_api_key_auth: bool, api_key_env_set: bool) -> String {
    if is_api_key_auth {
        ...
        return format!(
            "{method}  Manage account and credits: console.x.ai\n  Run `mad login` to use your SuperMad subscription instead.\n"
        );
    }
    String::from(
        "  Auth method: OAuth\n  Manage account and credits: https://mad.com/?_s=billing\n",
    )
}
