...
#[test]
#[ignore = "leader-cluster: needs single-process isolation (process-global env + mad_home OnceLock in the shared lib test binary); run: cargo test -p xai-mad-pager --lib -- app::leader_cluster --ignored --test-threads=1"]
...
/// Locate `<mad_home>/sessions/<enc-cwd>/<sid>/updates.jsonl` without the
...
fn find_session_updates_file(sid: &str) -> Option<PathBuf> {
    let sessions = effective_mad_home().join("sessions");
