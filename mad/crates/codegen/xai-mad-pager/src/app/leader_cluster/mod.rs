...
//!
//! Env sandboxing follows this crate's `serial(MAD_HOME)` idiom; note
//! `mad_home()` is process-cached (OnceLock), so disk assertions always go
//! through [`effective_mad_home`] rather than assuming the temp dir won.
//!
//! The scenarios are `#[ignore]`d in the shared lib test binary: the harness
//! mutates process-global env (proxy URLs, `XAI_API_KEY`,
//! `MAD_LEADER_SOCKET`, `MAD_HOME`) for a real agent's whole lifetime, and
//! in a several-thousand-test process that poisons concurrently-running tests
//! (and `mad_home()`'s OnceLock is usually already pinned). Run on demand:
//!
//! ```bash
//! cargo test -p xai-mad-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
/// The mad home the agent actually persisted under: `mad_home()` is
/// process-cached, so an earlier test in this binary may have pinned it.
fn effective_mad_home() -> PathBuf {
    xai_mad_config::mad_home()
}
...
struct PagerLeaderCluster {
    ...
    /// agent on the same MAD_HOME (two agents on one updates.jsonl is the
...
impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(MAD_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let sock_path = mad_home.path().join("leader-cluster.sock");

        let env = vec![
            crate::test_util::EnvVarGuard::set("MAD_HOME", mad_home.path()),
            crate::test_util::EnvVarGuard::set("MAD_CLI_CHAT_PROXY_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("MAD_XAI_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("XAI_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("MAD_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("MAD_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("MAD_TRACE_UPLOAD", "false"),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (xai_mad_version), not
        ...
        generation_tasks.push(tokio::task::spawn_local(async move {
            let _ = run_leader_server(
                ...
                xai_mad_shell::agent::activity::AgentActivity::default(),
                tokio::sync::watch::channel(true).1,
                tokio::sync::watch::channel(false).0,
                tokio::sync::watch::channel(xai_mad_shell::leader::ShutdownReason::Manual).0,
                None,
                control_state,
            )
            .await;
        ...
        generation_tasks.extend(xai_mad_shell::leader::in_process::spawn_agent(
            acp_rx,
            response_tx,
        ));
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same MAD_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `MAD_LEADER_SOCKET`, flock held
    ...
    async fn client(&mut self, name: &str, reconnect: bool) -> ClusterClient {
        ...
        let (reconnector, status_rx) = if reconnect {
            ...
            let reconnector = LeaderReconnector::new(
                ...
                ClientMode::Stdio,
                LeaderEnvUrls {
                    mad_ws_url: String::new(),
                    mad_ws_origin: String::new(),
                },
                LeaderClientCapabilities {
                    client_version: Some("0.0.0-test".to_string()),
                    ..Default::default()
