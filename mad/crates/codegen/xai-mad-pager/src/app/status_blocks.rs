...
fn format_cost(m: &xai_mad_shell::extensions::notification::PromptUsageModel) -> String {
    use xai_mad_shell::extensions::notification::ticks_to_usd;
...
mod tests {
    use super::*;
    use xai_mad_shell::extensions::notification::{PromptUsage, PromptUsageModel};
    ...
    fn session_usage_block_lists_models_when_multiple() {
        ...
        usage
            .model_usage
            .insert("mad-build".into(), model_row(100, 10, None));
        usage
            .model_usage
            .insert("mad-4".into(), model_row(50, 5, None));
        let text = session_usage_block_text(&usage);
        assert!(text.contains("By model:"), "{text}");
        assert!(text.contains("mad-build — 100 in / 10 out"), "{text}");
        assert!(text.contains("mad-4 — 50 in / 5 out"), "{text}");
    }
