...
use xai_mad_shell::session::storage::{ReplayEmission, stream_replay_updates_at};
...
/// Mad home for the replay path. In production this is just `mad_home()`; the
/// whole test override below is `#[cfg(test)]`, so no thread-local or dead
/// always-false branch ships in release.
#[cfg(not(test))]
fn effective_mad_home() -> std::path::PathBuf {
    xai_mad_shell::util::mad_home::mad_home()
}
#[cfg(test)]
thread_local! {
    static REPLAY_MAD_HOME: std::cell::RefCell<Option<std::path::PathBuf>> =
        const { std::cell::RefCell::new(None) };
}
/// Override mad home for disk-replay unit tests (thread-local).
#[cfg(test)]
pub(crate) fn set_replay_mad_home_for_tests(home: Option<std::path::PathBuf>) {
    REPLAY_MAD_HOME.with(|h| *h.borrow_mut() = home);
}
#[cfg(test)]
fn effective_mad_home() -> std::path::PathBuf {
    if let Some(home) = REPLAY_MAD_HOME.with(|h| h.borrow().clone()) {
        return home;
    }
    xai_mad_shell::util::mad_home::mad_home()
}
...
pub(crate) fn enrich_from_meta(
    ...
    enrich_from_meta_with_home(info, &effective_mad_home(), parent_cwd, parent_session_id);
}
...
mod tests {
    ...
    fn ensure_subagent_child_replayed_releases_retained_memory_once() {
        ...
        set_replay_mad_home_for_tests(Some(home.path().to_path_buf()));
        let mut parent = make_min_child_view();
        parent
            .subagent_views
        ...
        set_replay_mad_home_for_tests(None);
    }
    #[test]
    ...
    fn subagent_meta_all_fields() {
        assert_eq!(
            format_subagent_meta(Some("researcher"), Some("analyst"), Some("mad-3")),
            " (researcher \u{00b7} analyst \u{00b7} mad-3)"
        );
    }
    #[test]
    fn subagent_meta_partial_skips_nones() {
        assert_eq!(
            format_subagent_meta(Some("researcher"), None, Some("mad-3")),
            " (researcher \u{00b7} mad-3)"
        );
    }
    #[test]
    ...
    fn subagent_meta_collapses_duplicate_persona_role() {
        assert_eq!(
            format_subagent_meta(Some("reviewer"), Some("reviewer"), Some("mad-3")),
            " (reviewer \u{00b7} mad-3)"
        );
    }
    #[test]
    ...
    fn subagent_meta_drops_both_empty_persona_role() {
        assert_eq!(
            format_subagent_meta(Some(""), Some(" "), Some("mad-3")),
            " (mad-3)"
        );
    }
    #[test]
