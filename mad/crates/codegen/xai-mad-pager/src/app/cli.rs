...
pub enum Command {
    /// Run Mad without the interactive UI
    Agent(Box<AgentArgs>),
    /// Show the configuration Mad discovers for this directory
    Inspect {
        /// Emit machine-readable JSON output.
        #[arg(long)]
        json: bool,
    },
    /// Check terminal, clipboard, color, and input support without starting Mad
...
Examples:
  mad wrap docker exec -it my-container bash
  mad wrap kubectl exec -it my-pod -- bash

See ~/.mad/README.md for more information.
")]
    Wrap(WrapArgs),
    ...
    /// `MAD_WORKSPACE_COMMAND=1` to enable it locally for testing.
    #[command(hide = true)]
    ...
    /// `~/.mad/config.toml` or when the `MAD_AGENT_DASHBOARD=0` env
    /// var is set.
    Dashboard,
}
...
/// Targets a running leader process by PID (used by `mad leader` / `mad workspace`).
#[derive(Debug, clap::Args, Clone, Default)]
pub struct LeaderTargetArgs {
    /// Leader process ID from `mad leader list`.
    #[arg(long)]
    pub pid: Option<u32>,
}
#[derive(Debug, clap::Args, Clone)]
    ...
    pub fn canonical_plugin_dirs(&self) -> Vec<PathBuf> {
        self.plugin_dirs
            ...
            .filter_map(|p| match dunce::canonicalize(p) {
                ...
                Ok(_) => {
                    eprintln!(
                        "mad: --plugin-dir {}: not a directory; skipping",
                        p.display()
                    );
                    None
                }
                Err(e) => {
                    eprintln!("mad: --plugin-dir {}: {e}; skipping", p.display());
                    None
                }
            })
            .collect()
...
pub enum AgentCmd {
    ...
    Stdio,
    /// Run the agent headlessly over the Mad WebSocket relay
...
pub struct HeadlessArgs {
    #[arg(long = "mad-ws-origin")]
    pub mad_ws_origin: Option<String>,
    #[arg(long = "mad-ws-url")]
    pub mad_ws_url: Option<String>,
}
...
pub struct LeaderArgs {
    ...
    /// Defer the mad.com relay WebSocket until the first headless IPC client
...
pub struct PagerArgs {
    ...
    #[arg(long)]
    pub cwd: Option<PathBuf>,
    /// Use a custom leader socket path instead of the default `~/.mad/leader.sock`.
    #[arg(
        long = "leader-socket",
        value_name = "PATH",
        global = true,
    ...
    /// persists per-segment markdown to grep. Sets `MAD_COMPACTION_MODE`.
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            xai_mad_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
    ...
    /// Default for all `mad -p` runs is to wait (up to `--background-wait-timeout`)
    ...
    #[arg(long, env = "MAD_SANDBOX", value_name = "PROFILE")]
    ...
    /// Session-scoped only — does not write config. To default plain `mad` to
    /// minimal, set `[ui] screen_mode = "minimal"` in ~/.mad/config.toml.
    #[arg(long = "minimal")]
    pub minimal: bool,
    ...
    pub fullscreen: bool,
    /// Write sampling events to ~/.mad/logs/sampling.jsonl.
    #[arg(long = "log-sampling", env = "MAD_LOG_SAMPLING", hide = true)]
    ...
    pub no_leader: bool,
    /// Initial prompt for the interactive session, e.g. `mad "fix the bug"` or `mad --worktree=feat "create this feature"`.
    #[arg(
        value_name = "PROMPT",
        conflicts_with_all = &["single",
        "prompt_json",
...
impl PagerArgs {
    ...
    /// explicit `--sandbox`/`MAD_SANDBOX` that differs from the saved profile is
    ...
    pub fn saved_resume_profile_for_cwd(&self, cwd: Option<&str>) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                xai_mad_shell::session::persistence::resumed_session_sandbox_profile(
                    Some(&id),
                    cwd,
                )
            }
            ResumeTarget::MostRecentForCwd => {
                xai_mad_shell::session::persistence::resumed_session_sandbox_profile(None, cwd)
            }
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<xai_mad_sandbox::ProfileName>().ok()
                    != s.parse::<xai_mad_sandbox::ProfileName>().ok() =>
            {
    ...
    /// whitespace. This is the `mad "<prompt>"` launch form; the headless
...
mod tests {
    ...
    fn version_flags_parse_as_early_intent_without_exiting() {
        for flag in ["--version", "-v", "-V"] {
            let args = PagerArgs::try_parse_from(["mad", flag]).expect("version flag parses");
    ...
    fn ordinary_and_doctor_parsing_do_not_set_version_intent() {
        assert!(!PagerArgs::try_parse_from(["mad"]).unwrap().version);
        assert!(
            !PagerArgs::try_parse_from(["mad", "doctor"])
                .unwrap()
                .version
        );
        assert!(matches!(
            PagerArgs::try_parse_from(["mad", "version"])
                .unwrap()
                .command,
    ...
    fn doctor_accepts_report_and_explicit_fix_forms() {
        let bare = PagerArgs::try_parse_from(["mad", "doctor"]).expect("bare doctor parses");
        assert!(matches!(
            bare.command,
        ...
        let json =
            PagerArgs::try_parse_from(["mad", "doctor", "--json"]).expect("doctor --json parses");
        assert!(matches!(
            json.command,
            Some(Command::Doctor(crate::doctor_cmd::DoctorArgs {
                json: true,
        ...
        for id in [
            ...
            let fix = PagerArgs::try_parse_from(["mad", "doctor", "fix", id, "--yes"])
        ...
        let list = PagerArgs::try_parse_from(["mad", "doctor", "fix"])
        ...
        for unsupported in [
            vec!["mad", "doctor", "all"],
            vec!["mad", "doctor", "fix", "ssh-wrap", "extra"],
            vec!["mad", "doctor", "fix", "--yes"],
            vec!["mad", "doctor", "--json", "fix", "terminal.ssh-wrap"],
        ] {
    ...
    fn resume_target_classifies_flags() {
        assert_eq!(
            PagerArgs::try_parse_from(["mad"]).unwrap().resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["mad", "-c"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["mad", "--resume"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["mad", "--resume", "sess-1"])
                .unwrap()
                .resume_target(),
            ResumeTarget::SessionId("sess-1".to_string())
        );
        assert_eq!(
            PagerArgs::try_parse_from(["mad", "-s", "sess-2"])
                .unwrap()
                .resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["mad", "-r", "old", "--fork-session"])
                .unwrap()
                .resume_target(),
    ...
    fn minimal_and_fullscreen_flags_conflict() {
        let args = PagerArgs::try_parse_from(["mad", "--minimal"]).unwrap();
        assert!(args.minimal && !args.fullscreen);
        let args = PagerArgs::try_parse_from(["mad", "--fullscreen"]).unwrap();
        assert!(args.fullscreen && !args.minimal);
        let err = PagerArgs::try_parse_from(["mad", "--minimal", "--fullscreen"]).unwrap_err();
    ...
    fn startup_sandbox_profile_no_resume() {
        assert_eq!(
            PagerArgs::try_parse_from(["mad", "--sandbox", "strict"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(Some("strict".to_string()))
        );
        assert_eq!(
            PagerArgs::try_parse_from(["mad", "--sandbox", ""])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(None)
        );
        assert_eq!(
            PagerArgs::try_parse_from(["mad"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(None)
        );
    ...
    fn leader_socket_flag_parses_at_root() {
        let args = PagerArgs::try_parse_from(["mad", "--leader-socket", "/tmp/leader-x.sock"])
    ...
    fn leader_socket_flag_defaults_to_none() {
        let args = PagerArgs::try_parse_from(["mad"]).expect("bare mad parses");
        assert!(args.leader_socket.is_none());
    }
    #[test]
    fn leader_mgmt_list_info_kill_parse() {
        let list = PagerArgs::try_parse_from(["mad", "leader", "list", "--json"])
            .expect("mad leader list --json");
        assert!(matches!(
            list.command,
        ...
        let info = PagerArgs::try_parse_from(["mad", "leader", "info", "--pid", "42"])
            .expect("mad leader info --pid");
        assert!(matches!(
            info.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        let kill = PagerArgs::try_parse_from(["mad", "leader", "kill"]).expect("mad leader kill");
        assert!(matches!(
            kill.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        assert!(PagerArgs::try_parse_from(["mad", "leader", "profile"]).is_err());
    }
    #[test]
    fn debug_file_flag_parses_and_is_global() {
        let root = PagerArgs::try_parse_from(["mad", "--debug-file", "/tmp/fire.txt"])
        ...
        let sub =
            PagerArgs::try_parse_from(["mad", "agent", "stdio", "--debug-file", "/tmp/f.txt"])
    ...
    fn worktree_flag_and_initial_prompt_combine() {
        ...
        let c = PagerArgs::try_parse_from(["mad", "-w", "x"]).expect("-w x parses");
        assert_eq!(c.worktree.as_deref(), Some("x"));
        assert_eq!(c.initial_prompt(), None);
    }
    #[test]
    fn trust_flag_parses_on_pager_and_alias() {
        let bare = PagerArgs::try_parse_from(["mad"]).expect("bare mad parses");
        assert!(!bare.trust);
        let long = PagerArgs::try_parse_from(["mad", "--trust"]).expect("--trust parses");
        assert!(long.trust);
        let alias =
            PagerArgs::try_parse_from(["mad", "--trust-folder"]).expect("--trust-folder parses");
        assert!(alias.trust);
    }
    #[test]
    fn reasoning_effort_and_effort_alias_parse_same_field() {
        let long = PagerArgs::try_parse_from(["mad", "--reasoning-effort", "high"])
            .expect("--reasoning-effort parses");
        assert_eq!(long.reasoning_effort.as_deref(), Some("high"));
        let alias =
            PagerArgs::try_parse_from(["mad", "--effort", "high"]).expect("--effort alias parses");
        assert_eq!(alias.reasoning_effort.as_deref(), Some("high"));
    }
    #[test]
    fn reasoning_effort_accepts_max_and_remapped_ids() {
        let max = PagerArgs::try_parse_from(["mad", "--effort", "max"]).expect("max parses");
        assert_eq!(max.reasoning_effort.as_deref(), Some("max"));
        let deep =
            PagerArgs::try_parse_from(["mad", "--reasoning-effort", "deep"]).expect("deep parses");
        assert_eq!(deep.reasoning_effort.as_deref(), Some("deep"));
    }
    #[test]
    fn reasoning_effort_last_flag_wins_when_both_names_set() {
        let args =
            PagerArgs::try_parse_from(["mad", "--reasoning-effort", "low", "--effort", "high"])
                .expect("both effort flag names parse");
        assert_eq!(args.reasoning_effort.as_deref(), Some("high"));
        let reverse =
            PagerArgs::try_parse_from(["mad", "--effort", "high", "--reasoning-effort", "low"])
    ...
    fn agent_args_effort_alias_parses() {
        let args = PagerArgs::try_parse_from(["mad", "agent", "--effort", "max", "stdio"])
