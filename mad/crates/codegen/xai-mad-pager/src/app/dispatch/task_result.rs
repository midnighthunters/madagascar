...
/// Whether a completed clipboard probe should fall through to the `mad wrap`
/// host-image request. A clean `FullMiss` always qualifies; a remote read
/// *error* (`AttachmentRead`) also qualifies because inside `mad wrap` the
...
/// `osc52_sink_active()`, so this is inert outside `mad wrap`.
    ...
    match result {
        ...
        TaskResult::ForeignResumeCwdCanonicalized {
                ...
                && let Some(canonical_cwd) = accepted_cwd
            {
                vec![Effect::DetectForeignResumeHint {
                    ...
                    mad_home: xai_mad_tools::util::mad_home::mad_home(),
                    launch_token,
                }]
            } else {
                vec![]
        ...
        TaskResult::ChangelogFetched { markdown, entries } => {
            ...
            app.changelog_bullets =
                xai_mad_shell::util::changelog::bullets_from_entries(&entries, 3);
            vec![]
        }
