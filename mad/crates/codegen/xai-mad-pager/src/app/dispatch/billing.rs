...
use xai_mad_telemetry::events::{SuperMadUpsell, SuperMadUpsellClicked};
...
/// Whether the user is at the highest subscription tier (SuperMad Heavy).
///
...
pub(super) fn is_max_tier(subscription_tier: Option<&str>) -> bool {
    ...
    // keys ("supermad_heavy") and CCP display names ("SuperMad Heavy").
    t.to_ascii_lowercase().replace(' ', "_") == "supermad_heavy"
}

/// URL for upgrading the subscription tier.
pub(crate) const UPSELL_URL_UPGRADE: &str = "https://mad.com/supermad?referrer=mad-build";

/// URL for managing pay-as-you-go / on-demand spending / purchasing credits.
pub(crate) const UPSELL_URL_PAYG: &str = "https://mad.com?_s=usage";
...
///
/// **`max_tier = true`** (positively identified as SuperMad Heavy):
...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        xai_mad_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
        CreditLimitUpsellMode::UnifiedCredits => (
            ...
            "Buy more credits",
            "Purchase credits to keep using Madagascar Build",
            CreditLimitCardAction::PurchaseCredits,
            xai_mad_telemetry::events::CreditLimitChoice::PurchaseCredits,
            false,
        ),
        CreditLimitUpsellMode::LegacyPayg { enabled: true } => (
            ...
            xai_mad_telemetry::events::CreditLimitChoice::PayAsYouGo,
            true,
        ),
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(xai_mad_telemetry::events::CreditLimitUpsellShown {
            surface: xai_mad_telemetry::events::CreditLimitUpsellSurface::InlineCard,
            max_tier: true,
    ...
    log_event(xai_mad_telemetry::events::CreditLimitUpsellShown {
        surface: xai_mad_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
        max_tier: false,
        pay_as_you_go: payg_telemetry,
        unified_billing,
    });
    ...
    .with_local_kind(LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            xai_mad_telemetry::events::CreditLimitChoice::UpgradeTier,
            second_choice,
        ],
    })
    .with_no_freeform();
...
/// `SuperMadUpsellShown` funnel event.
pub(super) fn open_free_usage_upsell(agent: &mut AgentView, auth_method: Option<String>) {
    open_supermad_upsell(agent, UpsellReason::FreeUsageLimit, auth_method);
}

/// Open the SuperMad upsell for a tier-restricted slash command
...
pub(super) fn open_restricted_command_upsell(
    ...
    open_supermad_upsell(agent, UpsellReason::RestrictedCommand, auth_method)
}

/// Which situation opened the SuperMad upsell modal. Controls the heading
...
fn open_supermad_upsell(
    ...
    let (heading, source, modal_id_prefix) = match reason {
        UpsellReason::FreeUsageLimit => (
            ...
            SuperMadUpsell::FreeUsagePaywall,
            "free-usage-upsell",
        ),
        UpsellReason::RestrictedCommand => (
            "Unlock all features with SuperMad.",
            SuperMadUpsell::RestrictedCommand,
            "restricted-command-upsell",
        ),
    };

    log_event(xai_mad_telemetry::events::SuperMadUpsellShown {
        source,
        auth_method,
    });
    ...
    let options = vec![
        QuestionOption {
            label: "Upgrade to SuperMad".into(),
            description: "For everyday coding and productivity tasks".into(),
            preview: None,
            id: Some(UPSELL_URL_UPGRADE.into()),
        },
        QuestionOption {
            label: "Upgrade to SuperMad Heavy".into(),
            description: "Get the most out of Madagascar Build. Highest usage limits.".into(),
            preview: None,
            // No Heavy-specific URL exists; the /supermad page lists
...
pub(super) fn handle_gate_refreshed(
    app: &mut AppView,
    settings: Option<xai_mad_shell::util::config::RemoteSettings>,
) -> Vec<Effect> {
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<xai_mad_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<xai_mad_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
...
pub(super) fn dispatch_open_supermad_url(app: &mut AppView) -> Vec<Effect> {
    log_event(SuperMadUpsellClicked {
        source: SuperMadUpsell::WelcomeScreen,
    ...
    let url = app
        ...
        .unwrap_or("https://mad.com/supermad?referrer=mad-build");
    // Funnel attribution: tag CLI-originated SuperMad upsell clicks
    // with `referrer=mad-build`, matching the OAuth consent flow and
    ...
    let url = crate::app::link_opener::ensure_query_param(url, "referrer", "mad-build");
