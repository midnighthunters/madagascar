...
/// Plant a Build session under the process `mad_home()` (OnceLock-cached;
/// do not rely on setting `MAD_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    ...
    let encoded = xai_mad_shell::util::mad_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: xai_mad_tools::implementations::mad_build::ask_user_question::AskUserQuestionMode::Default,
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    xai_mad_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
const POLICY_WARNING: &str =
    xai_mad_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
