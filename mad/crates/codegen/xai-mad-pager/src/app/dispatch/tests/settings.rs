...
fn plugin_cta_catalog_reload_empty_candidates_resets_matched_phase() {
    ...
    let response = xai_hooks_plugins_types::MarketplaceListResponse {
        sources: vec![xai_hooks_plugins_types::MarketplaceScanResult {
            source_name: xai_mad_plugin_marketplace::OFFICIAL_SOURCE_NAME.into(),
            source_kind: "git".into(),
            source_url_or_path: xai_mad_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.into(),
...
fn slash_model_valid_dispatches_set_default_model_with_switch_and_persist() {
    let mut app = test_app_with_agent();
    let id = AgentId(0);
    let model_id = acp::ModelId::new(std::sync::Arc::from("mad-4.5"));
    app.agents
        .get_mut(&id)
        .unwrap()
        .session
        ...
        .insert(
            model_id.clone(),
            acp::ModelInfo::new(model_id.clone(), "Mad 4.5".to_string()),
        );
    let effects = dispatch(Action::SendPrompt("/model Mad 4.5".into()), &mut app);
    assert_eq!(
        effects.len(),
        2,
...
fn clear_default_model_persists_but_keeps_live_current() {
    ...
    use std::sync::Arc;
    let mut app = test_app_with_agent();
    let id = acp::ModelId::new(Arc::from("mad-test"));
    let info = acp::ModelInfo::new(id.clone(), "Mad Test".to_string());
...
fn set_default_model_resolves_known_name() {
    ...
    use std::sync::Arc;
    let mut app = test_app_with_agent();
    let id = acp::ModelId::new(Arc::from("mad-4.5"));
    let info = acp::ModelInfo::new(id.clone(), "Mad 4.5".to_string());
    let agent_id = AgentId(0);
    app.agents
        .get_mut(&agent_id)
        .unwrap()
...
fn set_default_model_idempotent_when_already_current() {
    ...
    use std::sync::Arc;
    let mut app = test_app_with_agent();
    let id = acp::ModelId::new(Arc::from("mad-already"));
    let info = acp::ModelInfo::new(id.clone(), "Mad Already".to_string());
    let agent_id = AgentId(0);
    app.agents
        .get_mut(&agent_id)
        .unwrap()
...
/// `clamp_to_terminal` might fold to MadNight in non-truecolor
...
fn set_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        ...
        crate::theme::cache::set(crate::theme::ThemeKind::MadNight);
        let effects = dispatch(Action::SetTheme("madday".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
                ...
                assert_eq!(*key, "theme");
                assert_eq!(*value, SettingValue::Enum("madday"));
                assert_eq!(*rollback_value, SettingValue::Enum("madnight"));
            }
            other => panic!("expected PersistSetting, got {other:?}"),
        }
        assert_eq!(app.current_ui.theme.as_deref(), Some("madday"));
        assert!(
...
/// for the auto-dark sibling. Uses `madday` to avoid the
...
fn set_auto_dark_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let effects = dispatch(Action::SetAutoDarkTheme("madday".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
        ...
        assert_eq!(app.current_ui.auto_dark_theme.as_deref(), Some("madday"));
    });
}
#[test]
fn set_auto_light_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let effects = dispatch(Action::SetAutoLightTheme("madnight".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
                ...
                assert_eq!(*value, SettingValue::Enum("madnight"));
                assert_eq!(*rollback_value, SettingValue::Enum("madday"));
            }
            other => panic!("expected PersistSetting, got {other:?}"),
        }
        assert_eq!(
...
///
/// Scenario: `theme="madnight"` (concrete) + system=Dark. User
/// commits `auto_dark_theme="madday"`. The setting is dormant
/// (parent theme is concrete, not auto), so the live display
/// must stay on MadNight even though we're committing a
/// different theme. Uses `madday` to avoid `clamp_to_terminal`
...
fn set_auto_dark_theme_does_not_apply_when_theme_is_not_auto() {
    with_theme_test_env(|| {
        ...
        let mut app = test_app_with_agent();
        let _ = dispatch(Action::SetTheme("madnight".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::MadNight,
        );
        let _ = dispatch(Action::SetAutoDarkTheme("madday".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::MadNight,
...
///
/// Uses `MadDay` (non-truecolor-requiring) for the dark-mode
...
/// MadNight. Using a non-truecolor theme avoids the clamp
...
/// Uses `madnight` for the auto_dark_theme
/// value to avoid `clamp_to_terminal` ambiguity (we want a
/// concrete kind that's clearly different from MadDay, the
...
fn set_auto_light_theme_applies_when_theme_is_auto_and_system_is_light() {
    with_theme_test_env(|| {
        ...
        let _ = dispatch(Action::SetAutoLightTheme("madnight".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::MadNight,
...
fn set_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let _ = dispatch(Action::SetTheme("madday".into()), &mut app);
        let toast = read_toast(&app);
        assert!(
        ...
        assert!(
            toast.contains("Mad Day"),
            "toast must use display name `Mad Day`, not canonical `madday`, got: {toast:?}",
        );
...
fn set_auto_light_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("Mad Night"));
    });
}
...
///
/// Uses non-truecolor themes (`madday` ↔ `madnight`) to avoid
...
fn rollback_theme_reverts_current_ui_and_cache() {
    ...
    with_theme_test_env(|| {
        ...
        let _ = dispatch(
            Action::TaskComplete(TaskResult::SettingPersistFailed {
                key: "theme",
                rollback_value: SettingValue::Enum("madnight"),
    ...
    with_theme_test_env(|| {
        ...
        let _ = dispatch(
            Action::TaskComplete(TaskResult::SettingPersistFailed {
                ...
                rollback_value: SettingValue::Enum("madday"),
                error: "disk full".into(),
            }),
            &mut app,
        );
        assert_eq!(app.current_ui.auto_light_theme.as_deref(), Some("madday"));
    });
}
