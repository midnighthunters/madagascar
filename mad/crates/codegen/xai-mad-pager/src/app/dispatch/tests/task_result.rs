...
fn doctor_planning_opens_refuses_remote_and_rejects_stale_identity() {
    ...
    dispatch_task_result(
        TaskResult::DoctorFixPlanned {
            ...
            result: Ok(crate::app::actions::DoctorPlanningOutcome::RunLocally(
                "mad doctor fix ssh-wrap".to_owned(),
            )),
        },
        &mut app,
    );
    assert!(
        last_system_text(&app, id)
            .contains("On your local computer, run: mad doctor fix ssh-wrap")
    );
...
fn stale_auth_copy_timeout_does_not_clear_newer_feedback() {
    ...
    app.auth_state = AuthState::Authenticating {
        request_seq: 1,
        handle: None,
        auth_url: Some("https://mad.com/auth".to_owned()),
...
) -> xai_mad_workspace::foreign_sessions::RecentForeignSession {
    xai_mad_workspace::foreign_sessions::RecentForeignSession {
        tool,
...
fn foreign_resume_results_require_launch_token_and_canonical_cwd() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                xai_mad_workspace::foreign_sessions::ForeignSessionTool::Cursor,
            )),
        }),
        &mut launch,
    );
    assert_eq!(
        ...
        Some(xai_mad_workspace::foreign_sessions::ForeignSessionTool::Cursor)
    );
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                xai_mad_workspace::foreign_sessions::ForeignSessionTool::Codex,
            )),
        }),
        &mut stale,
    );
...
fn wrap_host_image_request_eligible_covers_full_miss_and_attachment_error_only() {
    ...
    // host-image request — the headless-SSH `mad wrap` image-paste fix.
...
fn switch_to_non_reasoning_model_clears_persisted_effort() {
    ...
    app.agents
        ...
        .insert(
            model_id.clone(),
            acp::ModelInfo::new(model_id.clone(), "Madagascar Build".to_string()),
        );
    app.agents
        .get_mut(&id)
        .unwrap()
...
fn switch_model_incompatible_agent_shows_question_modal() {
    ...
    let err = xai_mad_shell::agent::config::ModelSwitchIncompatibleAgentError {
        code: "MODEL_SWITCH_INCOMPATIBLE_AGENT".into(),
        active_agent_type: "mad-build".into(),
...
fn same_agent_type_switch_no_modal() {
    ...
    let mut app = test_app_with_agent();
    let id = AgentId(0);
    let model_a = acp::ModelId::new(std::sync::Arc::from("mad-build-a"));
    let model_b = acp::ModelId::new(std::sync::Arc::from("mad-build-b"));

    // Add both models to the catalog (no agentType → both use mad-build).
    let agent = app.agents.get_mut(&id).unwrap();
    agent.session.models.available.insert(
        model_a.clone(),
        acp::ModelInfo::new(model_a.clone(), "Madagascar Build A".to_string()),
    );
    agent.session.models.set_current(model_a, None);
    agent.session.models.available.insert(
        model_b.clone(),
        acp::ModelInfo::new(model_b.clone(), "Madagascar Build B".to_string()),
    );
    agent.session.model_switch_pending = true;
...
fn delete_both_session_clears_modal_and_welcome_content_hits() {
    ...
    let hit = xai_mad_shell::extensions::session_search::SearchSessionHit {
...
fn gate_refreshed_emits_check_subscription_on_gate_lift() {
    ...
    app.gate = Some(xai_mad_shell::auth::GateInfo {
        message: "SuperMad subscription required".into(),
        url: Some("https://mad.com/supermad".into()),
        label: Some("Subscribe".into()),
    });
    assert!(!app.has_access());

    // Server-side settings now show no gate (user purchased subscription).
    let settings = xai_mad_shell::util::config::RemoteSettings::default();
...
fn gate_refreshed_no_effect_when_still_gated() {
    ...
    let settings = xai_mad_shell::util::config::RemoteSettings {
...
fn test_gate() -> xai_mad_shell::auth::GateInfo {
    xai_mad_shell::auth::GateInfo {
        message: "Subscribe".into(),
        url: None,
        label: None,
    }
...
fn verify_check_with_meta_resolves_pending_gate() {
    ...
    let meta = serde_json::to_value(xai_mad_shell::auth::AuthMeta::default()).unwrap();
    dispatch_task_result(
...
fn verify_check_with_gated_meta_shows_gate() {
    ...
    let meta = serde_json::to_value(xai_mad_shell::auth::AuthMeta {
        gate: Some(test_gate()),
        ..Default::default()
    })
    .unwrap();
