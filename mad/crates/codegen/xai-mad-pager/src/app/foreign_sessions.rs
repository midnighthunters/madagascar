...
use xai_mad_workspace::foreign_sessions::{
...
impl ForeignPickerSource {
    ...
    fn skill_paths(self, mad_home: &Path) -> [PathBuf; 2] {
        ...
        [
            mad_home
                ...
                .join(skill)
                .join("SKILL.md"),
            mad_home.join("skills").join(skill).join("SKILL.md"),
        ]
    }
}
    ...
    Fut: Future<Output = bool>,
{
    ...
    for source in ForeignPickerSource::ALL {
        ...
        for path in source.skill_paths(mad_home) {
...
pub(crate) async fn gated_sources_async(
    ...
    gated_sources_async_with(compat, mad_home, |path| async move {
...
{
    let enabled = gated_sources_async_with(compat, mad_home, metadata_exists).await;
...
mod tests {
    ...
    use xai_mad_workspace::foreign_sessions::ForeignSessionSource;
    ...
    async fn async_gate_supports_bundled_and_user_skill_locations() {
        let enabled = gated_sources_async_with(compat_all(), Path::new("/mad"), |path| {
