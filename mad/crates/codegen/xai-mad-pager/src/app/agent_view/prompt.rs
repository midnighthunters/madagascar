...
impl AgentView {
    ...
    pub(in crate::app) fn handle_prompt_key(
            ...
            && self.prompt.prompt_suggestion_visible()
        {
            if key!(Tab).matches(key) || key!(Right).matches(key) {
                if self.prompt.accept_prompt_suggestion() {
                    ...
                    xai_mad_telemetry::session_ctx::log_event(
                        xai_mad_telemetry::events::PromptSuggestion {
                            action: xai_mad_telemetry::events::PromptSuggestionAction::Accepted,
                            chars,
                            words,
                        },
                    );
                ...
                && self.prompt.text().is_empty()
            {
                ...
                xai_mad_telemetry::session_ctx::log_event(
                    xai_mad_telemetry::events::PromptSuggestion {
                        action: xai_mad_telemetry::events::PromptSuggestionAction::Dismissed,
                        chars,
                        words,
                    },
                );
            ...
            match self.prompt.handle_key(key) {
                PromptEvent::Edited => {
                    if undo_tip_accepted {
                        xai_mad_telemetry::session_ctx::log_event(
                            xai_mad_telemetry::events::ContextualTip {
    ...
    /// The pty-only `MAD_ESC_DOUBLE_PRESS_MS` override can exceed this; no
