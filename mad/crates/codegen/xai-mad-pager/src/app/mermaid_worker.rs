...
//! inside [`xai_mad_mermaid::render_checked`] is a no-op there: a panic in the
...
//!
//! 1. The pager re-execs itself as `xai-mad-pager __mermaid-render` (see
...
//!    via [`xai_mad_mermaid::run_with_timeout`], which **kills and reaps** the
...
//!    ([`xai_mad_mermaid::RenderLimits`]) and the raster megapixel/height caps
//!    (see `xai-mad-mermaid`). The parent also rejects obviously-oversized
...
fn render_source_to_png(
    ...
    target_width_px: u32,
    quality: MermaidRenderQuality,
) -> Result<RenderedDiagram, xai_mad_mermaid::MermaidError> {
...
thread_local! {
    ...
    /// `MAD_HOME` (whose `mad_home()` value is cached first-write-wins, an
    /// isolation hazard under the full parallel suite — PNGs could land in the
    /// real `~/.mad`). Thread-local, so each parallel test is independent; the
    /// `TempDir` guard lives here so the dir outlives the view. Mirrors the
    /// `subagent::REPLAY_MAD_HOME` test seam. Production never sets this.
...
impl AgentView {
    ...
    fn mermaid_out_path(&self, key: &MermaidCacheKey) -> Option<PathBuf> {
        // Test seam: a hermetic per-test cache dir (no `MAD_HOME` mutation).
        #[cfg(test)]
...
mod tests {
    ...
    fn is_render_subcommand_matches_only_argv1() {
        ...
        assert!(!is_render_subcommand(&argv(&["mad"])));
        assert!(!is_render_subcommand(&argv(&["mad", "chat"])));
        assert!(!is_render_subcommand(&argv(&[])));
    ...
    fn per_theme_renders_land_in_distinct_files() {
        ...
        let light_key =
            MermaidCacheKey::derive(src, ThemeKind::MadDay, 80, MermaidRenderQuality::Terminal);
        assert_ne!(
    ...
    /// hermetic, with no process-global `MAD_HOME` mutation. The `TempDir` lives
    ...
    /// so there is no cross-test contamination and no `mad_home()` cache race.
    ...
    fn agent_with_session(name: &str) -> AgentView {
        ...
        let cwd = PathBuf::from("/mad-mermaid-test").join(name);
    ...
    fn mermaid_is_rendering_matches_by_source_across_theme_width_change() {
        ...
        let click_key =
            MermaidCacheKey::derive(src, ThemeKind::MadNight, 80, MermaidRenderQuality::Open);
        let mut rt = MermaidRuntime::new();
        rt.pending.push(PendingMermaidAction {
            key: click_key.clone(),
            action: MermaidClickAction::Open,
        ...
        let live_key =
            MermaidCacheKey::derive(src, ThemeKind::MadDay, 240, MermaidRenderQuality::Open);
        assert_ne!(
            click_key, live_key,
