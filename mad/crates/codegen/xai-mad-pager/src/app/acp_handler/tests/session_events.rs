#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn retry_exhausted_rate_limited_empty_reason_uses_oauth_fallback() {
        use xai_mad_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_OAUTH;
    ...
    fn retry_exhausted_api_key_rewrites_consumer_subscription_upsell() {
        use xai_mad_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_API_KEY;
        ...
        match last_session_event(&scrollback) {
            Some(SessionEvent::RetryFailed { error, .. }) => {
                ...
                assert!(!error.contains("mad.com/supermad"));
            }
    ...
    fn apply_retry_state_auth_failure_pushes_reauth_prompt() {
        ...
        apply_retry_state(
            &RetryState::Failed {
                ...
                message: "Unauthorized (401) from https://cli-chat-proxy.mad.com/v1/messages: \
                          no auth context"
                    .into(),
            },
            &mut session,
    ...
    /// Legacy WebLogin auth keeps its verbose message (with `mad logout` /
    /// `mad login` guidance), not the generic re-auth prompt.
    #[test]
    ...
    fn child_compact_started_does_not_reset_context_used() {
        ...
        child_view.context_state = Some(xai_mad_shell::session::ContextInfo::from_notification(
            90_000, 131_072,
        ));
        agent
            .subagent_views
