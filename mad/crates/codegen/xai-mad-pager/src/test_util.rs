...
/// Shared MAD_HOME boundary fixture for the resume-by-title startup and
/// pre-sandbox tests.
///
/// `mad_home()` is OnceLock-cached process-wide, so summaries land under the
/// *resolved* home (possibly the real `~/.mad` when another test pinned the
...
impl Default for MadHomeFixture {
    fn default() -> Self {
        Self::new()
    }
}
