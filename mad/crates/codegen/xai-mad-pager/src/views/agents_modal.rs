...
use xai_mad_agent::config::{AgentDefinition, AgentScope, BuiltinAgentName};
use xai_mad_shell::agent::config::AgentSelectionConfig;
use xai_mad_tools::implementations::skills::discovery::extract_first_paragraph;
...
/// User-level vs project-level config files (`~/.mad` vs `{cwd}/.mad`).
...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup agent name (same chain as shell: `[agent]`, `MAD_AGENT`,
    /// model `agentType`, then `mad-build`).
...
/// Skips internal variants (MadagascarBuildConcise, MadagascarBuildPlan,
/// MadagascarBuildPlanNoSubagents, MadagascarBuildAskUser, Codex, Opencode,
/// CursorExtended, MadagascarBuildOrchestrator).
...
pub fn build_agent_list(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = xai_mad_agent::discovery::discover(cwd);
...
/// Union bundled personas with local `~/.mad/personas` and `{cwd}/.mad/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let bundled_dir = mad_home.join("bundled").join("personas");
    ...
    let dirs = [
        (ConfigFileScope::Project, cwd.join(".mad").join("personas")),
        (ConfigFileScope::User, mad_home.join("personas")),
    ];
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match xai_mad_shell::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => xai_mad_config::mad_home().join("personas"),
        ConfigFileScope::Project => cwd.join(".mad").join("personas"),
    }
}
...
/// True when `path` is a deletable local persona file (user or project `.mad/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.mad/{subdir}` or `{cwd}/.mad/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let mad_home = xai_mad_config::mad_home();
    let in_user = dunce::canonicalize(mad_home.join(subdir))
        .ok()
        .is_some_and(|d| canonical.starts_with(&d));
    let project_suffix = std::path::Path::new(".mad").join(subdir);
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    xai_mad_shell::config::load_effective_config()
        .ok()
        .and_then(|root| xai_mad_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// `MvpAgent::resolve_agent_definition` in xai-mad-shell.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    xai_mad_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        xai_mad_agent::config::PromptMode::Extend => "extend",
        xai_mad_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use xai_mad_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".mad").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".mad").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.mad/bundled/personas/b.toml".into()),
            scope_label: None,
        };
    ...
    fn delete_persona_file_allows_project_persona() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join(".mad").join("personas").join("gone.toml");
