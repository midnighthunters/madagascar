...
use xai_mad_workspace::permission::bash_command_splitting::{
...
pub struct PermissionViewState {
    ...
    /// permissions). Imported from `xai-mad-shell`, NOT duplicated locally.
...
/// ```text
///  3 [ ] Tell Mad what to do differently
...
mod tests {
    ...
    fn render_shows_planned_mcp_args() {
        ...
        assert!(
            text.contains("\"body\": \"hello from mad\""),
            "args JSON not rendered:\n{text}"
        );
    ...
    ///
    /// ```text
    /// PERMISSION_UI_RENDER_REVIEW=1 cargo test -p xai-mad-pager --lib \
    ...
    fn historic_bash_fixture_path() -> std::path::PathBuf {
        // CARGO_MANIFEST_DIR = crates/codegen/xai-mad-pager
