...
use xai_mad_tools::implementations::skills::types::SkillInfo;
...
pub fn plugin_group(plugin: &xai_hooks_plugins_types::PluginInfo) -> PluginGroup {
    ...
    match &plugin.origin {
        Some(PluginOrigin::ProjectMad) => PluginGroup::new(0, "origin:project", "Project"),
        Some(PluginOrigin::ProjectClaude) => {
            PluginGroup::new(1, "origin:project-claude", "Project (Claude)")
        }
        Some(PluginOrigin::UserMad) => PluginGroup::new(2, "origin:user", "User"),
...
impl ExtensionsTab {
    ...
    pub fn telemetry_tab(self) -> xai_mad_telemetry::events::ExtensionsModalTab {
        use xai_mad_telemetry::events::ExtensionsModalTab;
        match self {
...
pub enum ButtonAction {
    ...
    AddMcpServer {
        name: String,
        config: Box<xai_mad_shell::util::config::McpServerConfig>,
    },
    ...
    RefreshMcpList,
    /// Open mad.com connectors page (MCP tab: press `o`).
...
pub fn derive_source_label(source_dir: &str) -> (String, bool) {
    let mad = xai_mad_config::mad_home();
    let source_path = std::path::Path::new(source_dir);
    // Plugin / installed-plugin dirs, under the user mad home (MAD_HOME-aware)
    // or a project-scoped `{cwd}/.mad/<subdir>/`. Returns the first path
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // User mad home (MAD_HOME-aware).
        if let Ok(rest) = source_path.strip_prefix(mad.join(subdir))
            && let Some(name) = first_comp(rest)
        {
            return Some(name);
        }
        // Project-scoped `.mad/<subdir>/<name>` anywhere in the path.
    ...
    // Project hooks
    if source_dir.ends_with("/.mad/hooks") || source_dir.contains("/.mad/hooks/") {
        return ("Project hooks".into(), false);
    }
    // Custom directory — removable
    let display = {
        if let Ok(rest) = source_path.strip_prefix(&mad) {
            let prefix = crate::util::display_mad_home_prefix();
...
fn skill_source_str(skill: &SkillInfo) -> String {
    if let Some(ref cs) = skill.config_source {
        match cs {
            xai_mad_tools::types::config_source::ConfigSource::User { path } => {
                if crate::util::is_under_user_mad_home(path) {
                    crate::util::display_user_mad_path("skills")
            ...
            xai_mad_tools::types::config_source::ConfigSource::Project { path } => {
            ...
            xai_mad_tools::types::config_source::ConfigSource::Plugin { plugin_name, .. } => {
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.mad/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // mad-home branch is MAD_HOME-aware; this covers the project fallback.
        let (label, is_custom) = derive_source_label("/repo/work/.mad/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.mad/installed-plugins/vendor-abc123/skills");
    ...
    fn mcp_collapsed_managed_section_omits_server_rows() {
        ...
        let servers = vec![
            make_mcp_server_for_rows("mad_com_linear", McpWireSource::Managed, vec![]),
        ...
        assert!(
            rows.labels
                .iter()
                .any(|l| l.starts_with("Managed by mad.com")),
            "managed section header must appear"
        );
        assert!(
            !rows.labels.iter().any(|l| l == "mad_com_linear"),
    ...
    fn make_skill(
        ...
        xai_mad_tools::implementations::skills::types::SkillInfo {
            ...
            metadata: None,
            path: "test".to_string(),
            scope: xai_mad_tools::implementations::skills::types::SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
            plugin_root: None,
    ...
    fn make_plugin_skill(
        ...
        skill.scope = xai_mad_tools::implementations::skills::types::SkillScope::Plugin;
    ...
    fn mcp_add_url_derives_name() {
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                assert!(matches!(
                    config.transport,
                    xai_mad_shell::util::config::McpServerTransportConfig::StreamableHttp { .. }
                ));
            }
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                match config.transport {
                    xai_mad_shell::util::config::McpServerTransportConfig::Stdio {
                        command,
                        args,
                        ..
                    } => {
    ...
    fn local_plugins_source() -> xai_hooks_plugins_types::MarketplaceScanResult {
        xai_hooks_plugins_types::MarketplaceScanResult {
            ...
            source_url_or_path: "/home/user/.mad/marketplace/local".into(),
            plugins: vec![
                TestPlugin {
                    name: "my-linter",
                    version: Some("0.1.0"),
    ...
    fn plugin_group_maps_each_origin_variant() {
        ...
        for (origin, rank, key, label) in [
            (PluginOrigin::ProjectMad, 0, "origin:project", "Project"),
            (
            ...
            (PluginOrigin::UserMad, 2, "origin:user", "User"),
            (
    ...
    fn plugin_group_fallback_without_origin() {
        ...
        assert_eq!(group.key, "mad-mp:xAI Official");
    ...
    fn plugin_group_unknown_origin_uses_scope_fallback() {
        ...
        assert_eq!(plugin_group(&unknown).key, "mad-mp:xAI Official");
    }
    ...
    fn plugins_render_groups_with_headers_in_rank_order() {
        ...
        let mut state = plugins_modal_state(vec![
            ...
            make_plugin_with_origin("user-tool", PluginOrigin::UserMad),
    ...
    fn plugins_render_multiple_plugins_under_one_group() {
        use xai_hooks_plugins_types::PluginOrigin;
        let mut state = plugins_modal_state(vec![
            make_plugin_with_origin("solo-tool", PluginOrigin::UserMad),
            make_plugin_with_origin(
                "catalog-tool",
                PluginOrigin::ClaudeMarketplace {
                    marketplace: "claude-market".into(),
    ...
    fn plugins_collapsed_group_hides_rows_and_search_forces_open() {
        use xai_hooks_plugins_types::PluginOrigin;
        let mut plugin = make_plugin_with_origin("user-tool", PluginOrigin::UserMad);
