...
use xai_mad_shell::tools::{TodoItem, TodoStatus};

use xai_mad_shell::extensions::notification::GoalClassifierVerdict;
...
mod tests {
    ...
    fn render_per_model_breakdown_shows_each_model() {
        let mut goal = make_goal();
        goal.live_tokens_by_model = vec![("mad-4".into(), 12_300), ("mad-3-mini".into(), 8_000)];
        let text = render_to_text(&goal);
        assert!(text.contains("mad-4"), "first model must render:\n{text}");
        assert!(
            text.contains("mad-3-mini"),
