...
//! loop. Recipe: `MAD_FPS=1 MAD_SCROLL_DEBUG=1 mad --resume <session>`,
//! then flip `scroll_mode` / `scroll_lines` / `invert_scroll` /
//! `scroll_speed` in `/settings` to compare variants live. For event-exact
//! capture beyond this per-frame sampling, add `MAD_SCROLL_LOG=1` — the
...
/// `MAD_SCROLL_DEBUG` (nonempty and not `"0"`) enables at startup, and the
...
impl ScrollDebugHud {
    pub fn new() -> Self {
        let env_on = std::env::var("MAD_SCROLL_DEBUG").is_ok_and(|v| !v.is_empty() && v != "0");
        Self { enabled: env_on }
    }
...
pub struct ScrollDebugPanel {
    ...
    /// Rows left free for FPS overlays stacked above (the dev `MAD_FPS`
