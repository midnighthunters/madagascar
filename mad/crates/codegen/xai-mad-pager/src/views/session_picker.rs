...
/// matters because the fast foreign scan can land rows the default Mad view
...
///
/// Default is [`Self::Mad`]: native Mad sessions only (local / remote /
/// conversation), so `/resume` does not mix Claude/Codex/Cursor foreign
/// sessions into the list. `f` cycles Mad → External → All → Local →
/// Remote — External first so one press from the default reveals foreign
/// sessions.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum SourceFilter {
    /// Native Mad sessions only — excludes Claude/Codex/Cursor foreign rows.
    #[default]
    Mad,
    Local,
    Remote,
...
impl SourceFilter {
    ...
    pub fn next(self) -> Self {
        match self {
            Self::Mad => Self::External,
            Self::External => Self::All,
            Self::All => Self::Local,
            Self::Local => Self::Remote,
            Self::Remote => Self::Mad,
        }
    }
    ...
    ///
    /// mad.com conversations carry `source == "conversation"` and live remotely,
    /// so they pass the `Remote` filter (and `Mad` / `All`) but not `Local`.
    /// Foreign sources (`claude` / `codex` / `cursor`) only pass `External` and
    /// `All`.
    pub fn matches(self, source: &str) -> bool {
        match self {
            Self::Mad => !crate::app::is_foreign_picker_source(source),
...
pub(crate) fn capture_picker_selection(
    entries: Option<&[SessionPickerEntry]>,
    content_results: Option<&[xai_mad_shell::extensions::session_search::SearchSessionHit]>,
    state: &PickerState,
    query: &str,
    grouped: bool,
...
pub(crate) fn build_content_entry_data(
    hits: &[xai_mad_shell::extensions::session_search::SearchSessionHit],
    entries_data: &[SessionPickerEntry],
    filtered_indices: &[usize],
    state: &PickerState,
    content_start: usize,
...
/// Hint shown on the default `Mad` view when the foreign-session scan loaded
/// Claude/Codex/Cursor entries it hides. Mad-only: `next(Mad) == External`
...
pub(crate) fn hidden_external_hint(
    ...
    if source_filter != SourceFilter::Mad {
        return None;
    }
    let hidden = entries?
        .iter()
...
mod tests {
    ...
    fn source_filter_matches() {
        // Default Mad filter: native only (not Claude/Codex/Cursor).
        assert!(SourceFilter::Mad.matches("local"));
        assert!(SourceFilter::Mad.matches("remote"));
        assert!(SourceFilter::Mad.matches("both"));
        assert!(SourceFilter::Mad.matches("conversation"));
        assert!(!SourceFilter::Mad.matches("claude"));
        assert!(!SourceFilter::Mad.matches("codex"));
        assert!(!SourceFilter::Mad.matches("cursor"));
        ...
        // mad.com conversations are remote: visible under Mad + All + Remote, not Local.
    ...
    fn source_filter_cycles() {
        ...
        assert_eq!(SourceFilter::Mad.next(), SourceFilter::External);
        ...
        assert_eq!(SourceFilter::Remote.next(), SourceFilter::Mad);
        assert_eq!(SourceFilter::Mad.label(), "Mad");
        assert_eq!(SourceFilter::External.label(), "External");
        assert_eq!(SourceFilter::default(), SourceFilter::Mad);
    }
    ...
    fn source_filter_filters_entries() {
        ...
        let mad = filter_session_entries(Some(&entries), "", SourceFilter::Mad);
        assert_eq!(mad, vec![0, 1, 2]); // local + remote + both, no foreign
    ...
    fn source_filter_empty_and_unknown_source() {
        ...
        // foreign, so it passes Mad + All but never Local, Remote, or External.
        assert!(SourceFilter::Mad.matches(""));
        ...
        assert!(SourceFilter::Mad.matches("unknown"));
    ...
    fn source_filter_is_active() {
        assert!(!SourceFilter::Mad.is_active());
    ...
    fn hidden_external_hint_visibility() {
        ...
        // Only the default Mad view surfaces the hint (with the count).
        assert_eq!(
            hidden_external_hint(Some(&entries), SourceFilter::Mad).as_deref(),
        ...
        assert_eq!(
            hidden_external_hint(Some(&one), SourceFilter::Mad).as_deref(),
        ...
        assert!(hidden_external_hint(Some(&native), SourceFilter::Mad).is_none());
        assert!(hidden_external_hint(None, SourceFilter::Mad).is_none());
    }
