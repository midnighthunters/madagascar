#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn cmd_a_image_chip_selection_is_path_free() {
        ...
        img.source_path = Some(PathBuf::from("/tmp/mad-test-image.png"));
        ...
        assert!(selected.contains("[Image #1]"));
        assert!(
            !selected.contains("/tmp/mad-test-image.png"),
    ...
    fn accept_inside_command_with_args_absorbs_existing_separator() {
        ...
        pw.textarea.insert_str("/mod mad-4");
        pw.textarea.set_cursor(3);
        pw.refresh_slash(&models);
    ...
    fn accept_arg_completion_replaces_arg_range() {
        ...
        let model_id = agent_client_protocol::ModelId::new(Arc::from("mad-4.5"));
        models.available.insert(
            model_id.clone(),
            agent_client_protocol::ModelInfo::new(model_id, "Mad 4.5".to_string()),
        );
        ...
        // Accept arg completion → should replace "gr" with "Mad 4.5".
        pw.accept_slash_completion(&models);
        let text = pw.textarea.text().to_string();
        assert!(
            text.contains("Mad 4.5"),
    ...
    fn fuzzy_result(path: &str, is_dir: bool) -> xai_mad_workspace::file_system::FuzzyMatchResult {
        xai_mad_workspace::file_system::FuzzyMatchResult {
