...
//! are written to `.mad/config.toml`.
...
use xai_mad_shell::claude_import::{ImportPlan, ImportableItem, PathKind, find_project_root};
...
fn build_rows(
    ...
    if !plan.global_items.is_empty() {
        ...
        let label = "Global  ~/.mad/config.toml".to_string();
    ...
    if !plan.project_items.is_empty() {
        ...
        let project_config = find_project_root(cwd).join(".mad").join("config.toml");
...
mod tests {
    use super::*;
    use xai_mad_shell::claude_import::PathKind;
    use xai_mad_workspace::permission::types::{PatternMode, PermissionRule, ToolFilter};
    ...
    fn filtered_plan_respects_per_item_selection_after_grouping() {
        use xai_mad_shell::claude_import::ImportableItem;
