#
# Madagascar CLI installer for PowerShell — https://x.ai/cli/install.ps1
#
# Auth: MAD_DEPLOYMENT_KEY env var (takes precedence) or ~/.mad/auth.json from `mad login`.
...
#   $env:MAD_VERSION="0.1.42"; irm https://x.ai/cli/install.ps1 | iex           # specific version (alt)
#   $env:MAD_DEPLOYMENT_KEY="<key>"; irm https://x.ai/cli/install.ps1 | iex
#
...
if (-not $Version -and $env:MAD_VERSION) {
    $Version = $env:MAD_VERSION
}
...
$MadDir = Join-Path $env:USERPROFILE '.mad'
...
function Read-MadToken([string]$Scope) {
    $authFile = Join-Path $MadDir 'auth.json'
...
if ($env:MAD_DEPLOYMENT_KEY) {
    $AuthSource = 'deployment key'
    Write-Host 'Auth: using deployment key.' -ForegroundColor DarkGray
} else {
    $oidcToken = Read-MadToken $OidcScope
    $legacyToken = Read-MadToken $LegacyScope
    if ($oidcToken) {
        $AuthSource = 'auth.json (oidc)'
        Write-Host 'Auth: using OIDC token from ~/.mad/auth.json.' -ForegroundColor DarkGray
    } elseif ($legacyToken) {
        $AuthSource = 'auth.json (legacy)'
        Write-Host 'Auth: using legacy token from ~/.mad/auth.json.' -ForegroundColor DarkGray
    }
}
...
$BaseUrlFallback = 'https://storage.googleapis.com/mad-build-public-artifacts/cli'
$DownloadDir = Join-Path $MadDir 'downloads'
$BinDir = if ($env:MAD_BIN_DIR) { $env:MAD_BIN_DIR } else { Join-Path $MadDir 'bin' }

New-Item -ItemType Directory -Path $DownloadDir -Force | Out-Null
New-Item -ItemType Directory -Path $BinDir -Force | Out-Null

$Channel = if ($env:MAD_CHANNEL) { $env:MAD_CHANNEL } else { 'stable' }
...
if ($AuthSource) {
    Write-Host "Installing Mad $resolvedVersion ($platform, $AuthSource)..." -ForegroundColor Cyan
} else {
    Write-Host "Installing Mad $resolvedVersion ($platform)..." -ForegroundColor Cyan
}

# --- Download binary ---

$binaryPath = Join-Path $DownloadDir "mad-$platform.exe"
$artifactBase = "$BaseUrl/mad-$resolvedVersion-$platform"
...
foreach ($binName in @('mad.exe', 'agent.exe')) {
...
Write-Host "  Installed to $BinDir\mad.exe and $BinDir\agent.exe." -ForegroundColor DarkGray

# --- Generate completions (best-effort) ---

$completionsDir = Join-Path (Join-Path $MadDir 'completions') 'powershell'
try {
    New-Item -ItemType Directory -Path $completionsDir -Force | Out-Null
    & (Join-Path $BinDir 'mad.exe') completions powershell 2>$null |
        Set-Content (Join-Path $completionsDir 'mad.ps1') -ErrorAction SilentlyContinue
} catch {}

# --- Persist installer config ---

$ConfigFile = Join-Path $MadDir 'config.toml'
...
if ($env:MAD_DEPLOYMENT_KEY) {
    $ProxyUrl = if ($env:MAD_PROXY_URL) { $env:MAD_PROXY_URL } else { 'https://cli-chat-proxy.mad.com/v1' }
    Write-Host '  Fetching deployment config...' -ForegroundColor DarkGray
    try {
        $headers = @{ 'Authorization' = "Bearer $($env:MAD_DEPLOYMENT_KEY)" }
    ...
    if ($deployResponse) {
        ...
        $managedConfigPath = Join-Path $MadDir 'managed_config.toml'
        $requirementsPath = Join-Path $MadDir 'requirements.toml'
...
Write-Host "Mad $resolvedVersion installed to $BinDir\mad.exe" -ForegroundColor Green
...
if ($pathEntries -notcontains $BinDir) {
    ...
    # Update current session so mad works immediately.
...
Write-Host ''
Write-Host "Run 'mad' or 'agent' to get started!" -ForegroundColor Cyan
