...
// postinstall.js and the bin/mad trampoline.
//
...
function makeTmpDir() {
    return fs.mkdtempSync(path.join(os.tmpdir(), 'mad-test-'));
}
...
// ─── Extracted logic (mirrors postinstall.js and bin/mad exactly) ─────
...
function installVersionedBinary(vendoredBinPath, version, canonicalDir) {
    const canonicalPath = path.join(canonicalDir, 'mad');
    fs.mkdirSync(canonicalDir, { recursive: true });

    const versionedName = `mad-${version}`;
...
function cleanupOldVersions(canonicalDir, currentVersionedName) {
    ...
    const versionedBinaries = entries
        .filter(e => e.startsWith('mad-') && !e.includes('.tmp.') && !e.includes('.link.') && e !== currentVersionedName)
        .sort(byVersionDescending('mad-'));
...
/** Mad bin dir resolution (mirrors postinstall.js and bin/mad). */
function resolveMadBinDir(env, homedir) {
    const madHome = env.MAD_HOME ?? path.join(homedir, '.mad');
    return path.join(madHome, 'bin');
}
...
/** Bootstrap canonical from vendored (same as bin/mad trampoline). */
...
test('creates versioned binary and symlink on fresh install', () => {
    ...
    try {
        const vendored = path.join(dir, 'vendored-mad');
        ...
        assert.strictEqual(target, 'mad-0.1.140');
...
test('upgrade swaps symlink and preserves old binary', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(result.canonicalPath), 'mad-0.1.141');
        ...
        const oldBinary = path.join(binDir, 'mad-0.1.140');
...
test('idempotent: reinstalling same version does not re-copy', () => {
    ...
    try {
        ...
        const versionedPath = path.join(binDir, 'mad-0.1.140');
...
test('symlink swap is atomic (no intermediate missing state)', () => {
    ...
    try {
        const binDir = path.join(dir, 'bin');
        const canonicalPath = path.join(binDir, 'mad');
...
test('handles upgrade from old-style regular file to versioned symlink', () => {
    ...
    try {
        ...
        // Simulate old installation: mad is a regular file
...
test('handles broken symlink (target deleted externally)', () => {
    ...
    try {
        ...
        const canonicalPath = path.join(binDir, 'mad');
        fs.symlinkSync('mad-0.1.99', canonicalPath);
...
test('three sequential upgrades: v1 -> v2 -> v3 all coexist', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'mad')), 'mad-0.1.3');

        // All three versioned binaries still exist (no cleanup yet)
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.1')));
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.2')));
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.3')));
    } finally {
        cleanup(dir);
    }
});
...
test('cleanup keeps N-1 version and removes older ones', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(binDir, 'mad-0.1.138'), 'v138');
        fs.writeFileSync(path.join(binDir, 'mad-0.1.139'), 'v139');
        fs.writeFileSync(path.join(binDir, 'mad-0.1.140'), 'v140');
        // mad-0.1.141 is the current version (excluded from cleanup)
        fs.writeFileSync(path.join(binDir, 'mad-0.1.141'), 'v141');

        cleanupOldVersions(binDir, 'mad-0.1.141');

        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.141')), 'current should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.140')), 'N-1 should be kept');
        assert.ok(!fs.existsSync(path.join(binDir, 'mad-0.1.139')), 'N-2 should be removed');
        assert.ok(!fs.existsSync(path.join(binDir, 'mad-0.1.138')), 'N-3 should be removed');
    } finally {
        cleanup(dir);
    }
});
...
test('cleanup with only one old version keeps it', () => {
    ...
    try {
        ...
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.140')), 'single old version should be kept');
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.141')), 'current should exist');
    } finally {
        cleanup(dir);
    }
...
test('cleanup with no old versions is a no-op', () => {
    ...
    try {
        ...
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.141')), 'current should still exist');
        const entries = fs.readdirSync(binDir).filter(e => e.startsWith('mad-'));
...
test('cleanup ignores .tmp. and .link. files', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(binDir, 'mad-0.1.141'), 'current');
        // Leftover temp files from a crashed install
        fs.writeFileSync(path.join(binDir, 'mad-0.1.140.tmp.12345'), 'crashed-tmp');
        fs.writeFileSync(path.join(binDir, 'mad.link.12345'), 'crashed-link');

        cleanupOldVersions(binDir, 'mad-0.1.141');

        // Temp files should not be touched by cleanup (they're filtered out)
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.140.tmp.12345')), 'tmp file should not be touched');
        assert.ok(fs.existsSync(path.join(binDir, 'mad.link.12345')), 'link file should not be touched');
    } finally {
        cleanup(dir);
    }
});
...
test('semver sort: 0.1.9 vs 0.1.10 (digit boundary)', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(binDir, 'mad-0.1.8'), 'v8');
        fs.writeFileSync(path.join(binDir, 'mad-0.1.9'), 'v9');
        fs.writeFileSync(path.join(binDir, 'mad-0.1.10'), 'v10');
        fs.writeFileSync(path.join(binDir, 'mad-0.1.11'), 'v11');

        cleanupOldVersions(binDir, 'mad-0.1.11');

        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.11')), 'current should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.10')), '0.1.10 should be kept (N-1)');
        assert.ok(!fs.existsSync(path.join(binDir, 'mad-0.1.9')), '0.1.9 should be removed');
        assert.ok(!fs.existsSync(path.join(binDir, 'mad-0.1.8')), '0.1.8 should be removed');
    } finally {
        cleanup(dir);
    }
});
...
test('semver sort: major version boundary (0.x vs 1.x)', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(binDir, 'mad-0.9.99'), 'old');
        fs.writeFileSync(path.join(binDir, 'mad-1.0.0'), 'v1');
        fs.writeFileSync(path.join(binDir, 'mad-1.0.1'), 'current');

        cleanupOldVersions(binDir, 'mad-1.0.1');

        assert.ok(fs.existsSync(path.join(binDir, 'mad-1.0.0')), '1.0.0 should be kept (N-1)');
        assert.ok(!fs.existsSync(path.join(binDir, 'mad-0.9.99')), '0.9.99 should be removed');
    } finally {
        cleanup(dir);
    }
});
...
test('semver sort: minor version boundary (0.1.x vs 0.2.x)', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(binDir, 'mad-0.1.999'), 'old');
        fs.writeFileSync(path.join(binDir, 'mad-0.2.0'), 'v2');
        fs.writeFileSync(path.join(binDir, 'mad-0.2.1'), 'current');

        cleanupOldVersions(binDir, 'mad-0.2.1');

        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.2.0')), '0.2.0 should be kept (N-1)');
        assert.ok(!fs.existsSync(path.join(binDir, 'mad-0.1.999')), '0.1.999 should be removed');
    } finally {
        cleanup(dir);
    }
});
...
test('byVersionDescending: unit test comparator directly', () => {
    const input = ['mad-0.1.9', 'mad-0.1.10', 'mad-0.1.2', 'mad-1.0.0', 'mad-0.2.0'];
    const sorted = [...input].sort(byVersionDescending('mad-'));
...
test('bootstrapCanonical creates versioned binary from vendored', () => {
    ...
    try {
        ...
        assert.strictEqual(result, path.join(binDir, 'mad'));
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.140')), 'versioned binary should exist');
...
test('bootstrapCanonical is idempotent', () => {
    ...
    try {
        ...
        assert.strictEqual(
            fs.readFileSync(path.join(binDir, 'mad-0.1.140'), 'utf8'),
            'original-content',
...
test('bootstrapCanonical works when canonical already exists (different version)', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(result), 'mad-0.1.141');
        // v1 should still exist
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.140')), 'old version should still exist');
    } finally {
        cleanup(dir);
    }
});
...
test('full lifecycle: install, upgrade, cleanup', () => {
    ...
    try {
        ...
        cleanupOldVersions(binDir, 'mad-0.1.142');

        // Current (v3) + N-1 (v2) should exist; v1 removed
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.142')), 'v3 should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.141')), 'v2 should be kept (N-1)');
        assert.ok(!fs.existsSync(path.join(binDir, 'mad-0.1.140')), 'v1 should be removed');

        // Canonical symlink points to v3
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'mad')), 'mad-0.1.142');
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'mad'), 'utf8'), 'v3');
    } finally {
        cleanup(dir);
    }
});
...
test('downgrade: installing older version than current', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'mad')), 'mad-0.1.140');
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'mad'), 'utf8'), 'v1');

        // v2 should still exist (never delete old binaries during install)
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.141')), 'v2 should still exist');
    } finally {
        cleanup(dir);
    }
});

test('non-mad files in bin dir are not touched by cleanup', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        fs.mkdirSync(binDir, { recursive: true });
        ...
        // Mad versions
        fs.writeFileSync(path.join(binDir, 'mad-0.1.138'), 'old1');
        fs.writeFileSync(path.join(binDir, 'mad-0.1.139'), 'old2');
        fs.writeFileSync(path.join(binDir, 'mad-0.1.140'), 'current');

        cleanupOldVersions(binDir, 'mad-0.1.140');

        assert.ok(fs.existsSync(path.join(binDir, 'other-tool')), 'non-mad file should not be touched');
        assert.ok(fs.existsSync(path.join(binDir, 'README.md')), 'non-mad file should not be touched');
    } finally {
        cleanup(dir);
    }
});
...
// ═══════════════════════════════════════════════════════════════════════
// mad vs mad-pager Isolation Tests
// ═══════════════════════════════════════════════════════════════════════
 ...
 * Uses prefix + leading digit to avoid mad-* matching mad-pager-*.
 */
...
test('installing both mad and mad-pager creates independent symlinks', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendored = path.join(dir, 'vendored');
        fs.writeFileSync(vendored, 'mad-binary');
        const vendoredPager = path.join(dir, 'vendored-pager');
        fs.writeFileSync(vendoredPager, 'pager-binary');

        installNamedBinary(vendored, 'mad', '0.1.141', binDir);
        installNamedBinary(vendoredPager, 'mad-pager', '0.1.141', binDir);

        // Both symlinks exist and point to correct targets
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'mad')), 'mad-0.1.141');
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'mad-pager')), 'mad-pager-0.1.141');

        // Both versioned files exist with correct content
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'mad-0.1.141'), 'utf8'), 'mad-binary');
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'mad-pager-0.1.141'), 'utf8'), 'pager-binary');
    } finally {
        cleanup(dir);
    }
});

test('cleanup of mad-* does not remove mad-pager-*', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        fs.mkdirSync(binDir, { recursive: true });
        ...
        // Old mad versions
        fs.writeFileSync(path.join(binDir, 'mad-0.1.138'), 'old-mad-1');
        fs.writeFileSync(path.join(binDir, 'mad-0.1.139'), 'old-mad-2');
        fs.writeFileSync(path.join(binDir, 'mad-0.1.140'), 'old-mad-3');
        // Current mad
        fs.writeFileSync(path.join(binDir, 'mad-0.1.141'), 'current-mad');

        // mad-pager versions (should not be touched)
        fs.writeFileSync(path.join(binDir, 'mad-pager-0.1.138'), 'old-pager-1');
        fs.writeFileSync(path.join(binDir, 'mad-pager-0.1.139'), 'old-pager-2');
        fs.writeFileSync(path.join(binDir, 'mad-pager-0.1.141'), 'current-pager');

        cleanupOldVersionsNamed(binDir, 'mad', '0.1.141');

        // mad cleanup: current + N-1 kept, older removed
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.141')), 'current mad should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.140')), 'N-1 mad should be kept');
        assert.ok(!fs.existsSync(path.join(binDir, 'mad-0.1.139')), 'N-2 mad should be removed');
        assert.ok(!fs.existsSync(path.join(binDir, 'mad-0.1.138')), 'N-3 mad should be removed');

        // ALL mad-pager versions must be untouched
        assert.ok(fs.existsSync(path.join(binDir, 'mad-pager-0.1.138')), 'mad-pager-0.1.138 must survive mad cleanup');
        assert.ok(fs.existsSync(path.join(binDir, 'mad-pager-0.1.139')), 'mad-pager-0.1.139 must survive mad cleanup');
        assert.ok(fs.existsSync(path.join(binDir, 'mad-pager-0.1.141')), 'mad-pager-0.1.141 must survive mad cleanup');
    } finally {
        cleanup(dir);
    }
});

test('cleanup of mad-pager-* does not remove mad-*', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        fs.mkdirSync(binDir, { recursive: true });

        // mad versions (should not be touched)
        fs.writeFileSync(path.join(binDir, 'mad-0.1.138'), 'old-mad-1');
        fs.writeFileSync(path.join(binDir, 'mad-0.1.139'), 'old-mad-2');
        fs.writeFileSync(path.join(binDir, 'mad-0.1.141'), 'current-mad');

        // Old mad-pager versions
        fs.writeFileSync(path.join(binDir, 'mad-pager-0.1.138'), 'old-pager-1');
        fs.writeFileSync(path.join(binDir, 'mad-pager-0.1.139'), 'old-pager-2');
        fs.writeFileSync(path.join(binDir, 'mad-pager-0.1.140'), 'old-pager-3');
        // Current pager
        fs.writeFileSync(path.join(binDir, 'mad-pager-0.1.141'), 'current-pager');

        cleanupOldVersionsNamed(binDir, 'mad-pager', '0.1.141');

        // mad-pager cleanup: current + N-1 kept, older removed
        assert.ok(fs.existsSync(path.join(binDir, 'mad-pager-0.1.141')), 'current pager should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'mad-pager-0.1.140')), 'N-1 pager should be kept');
        assert.ok(!fs.existsSync(path.join(binDir, 'mad-pager-0.1.139')), 'N-2 pager should be removed');
        assert.ok(!fs.existsSync(path.join(binDir, 'mad-pager-0.1.138')), 'N-3 pager should be removed');

        // ALL mad versions must be untouched
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.138')), 'mad-0.1.138 must survive pager cleanup');
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.139')), 'mad-0.1.139 must survive pager cleanup');
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.141')), 'mad-0.1.141 must survive pager cleanup');
    } finally {
        cleanup(dir);
    }
});
...
test('full dual-binary lifecycle: install, upgrade, cleanup both', () => {
    ...
    try {
        ...
        // v1
        fs.writeFileSync(vendored, 'mad-v1');
        fs.writeFileSync(vendoredPager, 'pager-v1');
        installNamedBinary(vendored, 'mad', '0.1.140', binDir);
        installNamedBinary(vendoredPager, 'mad-pager', '0.1.140', binDir);

        // v2
        fs.writeFileSync(vendored, 'mad-v2');
        fs.writeFileSync(vendoredPager, 'pager-v2');
        installNamedBinary(vendored, 'mad', '0.1.141', binDir);
        installNamedBinary(vendoredPager, 'mad-pager', '0.1.141', binDir);

        // v3
        fs.writeFileSync(vendored, 'mad-v3');
        fs.writeFileSync(vendoredPager, 'pager-v3');
        installNamedBinary(vendored, 'mad', '0.1.142', binDir);
        installNamedBinary(vendoredPager, 'mad-pager', '0.1.142', binDir);

        // Cleanup both independently
        cleanupOldVersionsNamed(binDir, 'mad', '0.1.142');
        cleanupOldVersionsNamed(binDir, 'mad-pager', '0.1.142');

        // Current + N-1 for each
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.142')));
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.141')));
        assert.ok(!fs.existsSync(path.join(binDir, 'mad-0.1.140')));

        assert.ok(fs.existsSync(path.join(binDir, 'mad-pager-0.1.142')));
        assert.ok(fs.existsSync(path.join(binDir, 'mad-pager-0.1.141')));
        assert.ok(!fs.existsSync(path.join(binDir, 'mad-pager-0.1.140')));

        // Symlinks correct
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'mad')), 'mad-0.1.142');
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'mad-pager')), 'mad-pager-0.1.142');
    } finally {
        cleanup(dir);
    }
});
...
test('mad installs normally regardless of platform key', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendored = path.join(dir, 'vendored-mad');
        ...
        for (const platform of ['darwin-arm64', 'linux-x64', 'linux-arm64']) {
            const result = installNamedBinary(vendored, 'mad', '0.1.150', binDir);
            assert.ok(fs.existsSync(result.versionedPath), `mad should install for ${platform}`);
            assert.strictEqual(fs.readlinkSync(result.canonicalPath), 'mad-0.1.150');
        }
    } finally {
        cleanup(dir);
    }
...
test('mad-pager installs when vendored binary exists (darwin-arm64 path)', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendoredPager = path.join(dir, 'vendored-pager');
        ...
        const result = installNamedBinary(vendoredPager, 'mad-pager', '0.1.150', binDir);
        assert.ok(fs.existsSync(result.versionedPath), 'pager versioned binary should exist');
        assert.strictEqual(fs.readlinkSync(result.canonicalPath), 'mad-pager-0.1.150');
...
test('Linux pager vendor files are not required for mad install', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        ...
        fs.writeFileSync(path.join(vendorBase, 'darwin-arm64', 'mad-pager'), 'mac-pager');
        ...
        assert.ok(!fs.existsSync(path.join(vendorBase, 'linux-x64', 'mad-pager')));
        assert.ok(!fs.existsSync(path.join(vendorBase, 'linux-arm64', 'mad-pager')));

        // mad install should succeed independently
        const madVendored = path.join(dir, 'vendored-mad');
        fs.writeFileSync(madVendored, 'mad-linux');
        const result = installNamedBinary(madVendored, 'mad', '0.1.150', binDir);
        assert.ok(fs.existsSync(result.versionedPath), 'mad should install without Linux pager');
    } finally {
        cleanup(dir);
    }
});

test('skipping pager install on Linux does not affect mad cleanup', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendored = path.join(dir, 'vendored');

        // Install mad across two versions
        fs.writeFileSync(vendored, 'mad-v1');
        installNamedBinary(vendored, 'mad', '0.1.149', binDir);
        fs.writeFileSync(vendored, 'mad-v2');
        installNamedBinary(vendored, 'mad', '0.1.150', binDir);

        // Simulate Linux: only run mad cleanup, skip pager entirely
        cleanupOldVersionsNamed(binDir, 'mad', '0.1.150');

        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.150')), 'current mad should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'mad-0.1.149')), 'N-1 mad should be kept');
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'mad')), 'mad-0.1.150');
...
test('canonical pager from non-npm install is preserved on Linux', () => {
    ...
    try {
        ...
        // Simulate pager installed by install-mad.sh (not npm)
        const pagerVersioned = path.join(binDir, 'mad-pager-0.1.150');
        fs.writeFileSync(pagerVersioned, 'installer-pager-binary');
        fs.chmodSync(pagerVersioned, 0o755);
        const pagerCanonical = path.join(binDir, 'mad-pager');
        fs.symlinkSync('mad-pager-0.1.150', pagerCanonical);

        // Run mad-only install + cleanup (simulating Linux postinstall)
        const vendored = path.join(dir, 'vendored');
        fs.writeFileSync(vendored, 'mad-binary');
        installNamedBinary(vendored, 'mad', '0.1.150', binDir);
        cleanupOldVersionsNamed(binDir, 'mad', '0.1.150');
        ...
        assert.strictEqual(fs.readlinkSync(pagerCanonical), 'mad-pager-0.1.150');
    } finally {
        cleanup(dir);
    }
});

console.log('\nmad home + brotli install tests\n');

test('resolveMadBinDir honors $MAD_HOME, else falls back to <home>/.mad/bin', () => {
    assert.strictEqual(
        resolveMadBinDir({ MAD_HOME: '/fast/local/.mad' }, '/home/alice'),
        path.join('/fast/local/.mad', 'bin'),
    );
    assert.strictEqual(
        resolveMadBinDir({}, '/home/alice'),
        path.join('/home/alice', '.mad', 'bin'),
    );
    assert.strictEqual(resolveMadBinDir({ MAD_HOME: '' }, '/home/alice'), path.join('', 'bin'));
});

test('writeVendorBinary returns false (not true) when the destination cannot be written', () => {
    const dir = makeTmpDir();
    try {
        const brPath = path.join(dir, 'mad.br');
...
test('decompresses brotli into the canonical dir without duplicating into node_modules', () => {
    ...
    try {
        ...
        assert.ok(!fs.existsSync(path.join(vendorBin, 'mad')), 'no uncompressed binary in node_modules');
