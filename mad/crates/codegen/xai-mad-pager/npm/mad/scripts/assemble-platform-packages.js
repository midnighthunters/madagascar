...
//   1. Brotli-compresses the built binary into `../mad-<platform>/bin/<bin>.br`
...
// (`@xai-official/mad`) lists all six as `optionalDependencies` pinned to
// the same version; npm installs only the one matching the host's
// `os` + `cpu` filters.
//
// Why brotli? npm's tarball ceiling is ~200 MB and the raw mad binary is
...
const NOTICES_SOURCE = path.resolve(
    npmRoot, '..', '..', 'xai-mad-tools', 'THIRD_PARTY_NOTICES.md');
...
async function packPlatform({ platform, arch, envVar, defaultSource, binName }) {
    const pkgDir = path.join(npmRoot, `mad-${platform}-${arch}`);
    ...
    console.log(
        `[assemble] mad-${platform}-${arch}@${VERSION}: ` +
...
async function main() {
    const targets = [
        {
            platform: 'darwin', arch: 'arm64', binName: 'mad',
            envVar: 'MAD_DARWIN_ARM64',
            defaultSource: path.join(xaiRoot, 'target', 'release', 'xai-mad-pager'),
        },
        {
            platform: 'darwin', arch: 'x64', binName: 'mad',
            envVar: 'MAD_DARWIN_X64',
            defaultSource: path.join(xaiRoot, 'target', 'x86_64-apple-darwin', 'release', 'xai-mad-pager'),
        },
        {
            platform: 'linux', arch: 'x64', binName: 'mad',
            envVar: 'MAD_LINUX_X64',
            defaultSource: path.join(xaiRoot, 'target',
                'explorer_cross_x86_64-unknown-linux-gnu',
                'x86_64-unknown-linux-gnu', 'release', 'xai-mad-pager'),
        },
        {
            platform: 'linux', arch: 'arm64', binName: 'mad',
            envVar: 'MAD_LINUX_ARM64',
            defaultSource: path.join(xaiRoot, 'target',
                'explorer_cross_aarch64-unknown-linux-gnu',
                'aarch64-unknown-linux-gnu', 'release', 'xai-mad-pager'),
        },
        {
            platform: 'win32', arch: 'x64', binName: 'mad.exe',
            envVar: 'MAD_WIN32_X64',
            defaultSource: path.join(xaiRoot, 'target', 'x86_64-pc-windows-msvc', 'release', 'xai-mad-pager.exe'),
        },
        {
            platform: 'win32', arch: 'arm64', binName: 'mad.exe',
            envVar: 'MAD_WIN32_ARM64',
            defaultSource: path.join(xaiRoot, 'target', 'aarch64-pc-windows-msvc', 'release', 'xai-mad-pager.exe'),
        },
    ];
