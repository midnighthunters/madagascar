#!/usr/bin/env node
// Runs once after npm install/update. Reads the mad binary from the
// matching per-platform optional dependency (@xai-official/mad-<platform>)
// and installs it to ~/.mad/bin/ using versioned filenames:
//
//   Unix:    mad-<version>  +  mad  (symlink)
//   Windows: mad-<version>.exe  +  mad.exe  (copy)
//
...
// $MAD_HOME (else ~/.mad), matching the Rust mad_home() including its
// canonicalized-home default. Lets fleets relocate the binary off a slow $HOME
// (NFS); old code hardcoded os.homedir().
function defaultMadHome() {
    const home = os.homedir();
    try { return path.join(fs.realpathSync(home), '.mad'); } catch { return path.join(home, '.mad'); }
}
const MAD_HOME = process.env.MAD_HOME ?? defaultMadHome();
const CANONICAL_DIR = path.join(MAD_HOME, 'bin');
...
if (!SUPPORTED.has(key)) {
    console.error(`@xai-official/mad: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@xai-official/mad-${key}`;
    try {
...
if (!version) {
    console.error('@xai-official/mad: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (!fs.existsSync(versionedPath) && !writeVendorBinary(brPath, rawPath, versionedPath)) {
        console.error(`@xai-official/mad: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@xai-official/mad: failed to update ${canonicalPath}: ${e2.message}`);
                console.error('Close all running mad processes and try again.');
                return false;
            }
        }
    } else {
    ...
    if (!fs.existsSync(canonicalPath)) {
        console.error(`@xai-official/mad: ${canonicalName} did not resolve after install`);
        return false;
    }
...
// Uses an exact prefix match + hyphen + digit to avoid mad-* matching mad-pager-*.
...
if (!platformDir) {
    console.error(`@xai-official/mad: platform package @xai-official/mad-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @xai-official/mad');
    process.exit(0);
}

installBinary('mad', platformDir, `mad${EXE}`);
cleanupOldVersions('mad');
cleanupOldVersions('mad-pager');

// Write installer config
const configDir = MAD_HOME;
...
// Persist the npm registry so `mad update` and the trampoline use the same one.
const npmRegistry = process.env.MAD_NPM_REGISTRY
    || (() => {
        try {
...
// Set MAD_INSTALL_COMPLETIONS=1 to auto-generate to ~/.mad/completions.
const MAD_PATH = path.join(CANONICAL_DIR, `mad${EXE}`);
if (process.env.MAD_INSTALL_COMPLETIONS === '1' && !IS_WINDOWS) {
    try {
        const { spawnSync } = require('child_process');
        const completionsDir = path.join(MAD_HOME, 'completions');
        const bashPath = path.join(completionsDir, 'bash', 'mad.bash');
        const zshPath = path.join(completionsDir, 'zsh', '_mad');
        fs.mkdirSync(path.dirname(bashPath), { recursive: true });
        fs.mkdirSync(path.dirname(zshPath), { recursive: true });
        const bashRes = spawnSync(MAD_PATH, ['completions', 'bash'], { encoding: 'utf8' });
        if (bashRes.status === 0) fs.writeFileSync(bashPath, bashRes.stdout);
        const zshRes = spawnSync(MAD_PATH, ['completions', 'zsh'], { encoding: 'utf8' });
        if (zshRes.status === 0) fs.writeFileSync(zshPath, zshRes.stdout);
        console.log('Completions generated to ~/.mad/completions (bash/zsh)');
    } catch {}
} else if (!IS_WINDOWS) {
    console.log('Tip: mad completions bash > ~/.local/share/bash-completion/completions/mad');
    console.log('     mad completions zsh  > ~/.zsh/completions/_mad');
}
