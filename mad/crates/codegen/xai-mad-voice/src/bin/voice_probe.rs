...
//! ```bash
//! export XAI_API_KEY=...
//! cargo run -p xai-mad-voice --bin voice-probe -- --seconds 5
//! ```
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(code) = xai_mad_voice::maybe_run_capture_subprocess() {
...
async fn run() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,xai_mad_voice=debug".into()),
        )
        .init();
    ...
    if args.mic_only {
        ...
        {
            let (bytes, chunks) =
                xai_mad_voice::run_mic_only_probe(config.sample_rate, args.seconds)?;
...
fn load_config(path: Option<&std::path::Path>) -> VoiceConfig {
    ...
    let env_base = std::env::var("MAD_XAI_API_BASE_URL").ok();
    ...
    if let Ok(home) = std::env::var("MAD_HOME")
...
Environment:
  ...
  RUST_LOG        optional (default info,xai_mad_voice=debug)

Reads [voice] from ~/.mad/config.toml unless --config PATH is set.
"#
    );
}
