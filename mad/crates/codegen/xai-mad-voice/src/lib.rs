//! Voice input for Madagascar Build CLI: an xAI streaming STT client and the
...
mod intercept_tests {
    ...
    fn capture_subcommand_matches_only_argv1() {
        assert!(is_capture_subcommand(&argv(&["mad", "__mic-capture"])));
        ...
        assert!(!is_capture_subcommand(&argv(&["mad"])));
        assert!(!is_capture_subcommand(&argv(&["mad", "chat"])));
        assert!(!is_capture_subcommand(&argv(&[
            "mad",
            "chat",
            "__mic-capture"
