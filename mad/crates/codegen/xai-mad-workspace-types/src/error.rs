...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] xai_mad_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `xai-mad-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `xai_mad_tools::ToolError`) happens
