...
//! agent-session config types in `xai-mad-config` and friends.
...
/// `xai-mad-tools` once the wire surface is firm.
...
///
/// TODO(workspace): align with `xai_mad_config::ProjectConfig`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
