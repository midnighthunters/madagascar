//! Wire types for the `xai-mad-workspace` API.
//!
...
//! `xai-mad-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `xai-mad-workspace-types` (instead of `xai-mad-workspace` or
/// `xai-mad-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `xai_mad_workspace::permission`
