...
//! `xai-mad-tools/src/implementations/skills/types.rs` (the type the
...
pub enum SkillScope {
    /// cwd/.mad/skills
    Local,
    /// repo_root/.mad/skills
    Repo,
    /// ~/.mad/skills
    User,
    /// ~/.mad/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    // Fixture mirrored field-for-field from the xai-mad-tools SkillInfo
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.mad/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.mad/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.mad/plugins/infra-plugin",
            "plugin_data": "/root/.mad/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "mad-4",
            "effort": "high",
            "user_invocable": true,
