...
/// Claude.md / `.mad/rules/*.md`) discovered from the workspace root up to
/// the git root, plus `~/.mad` and compat dirs.
...
/// Mirrors the serde shape of `xai-mad-agent`'s `AgentConfigFile`.
