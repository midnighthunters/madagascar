...
This crate extracts a first-class `Agent` type from `xai-mad-shell`.
...
`xai-mad-shell`, another in-process host, or a headless batch runner.
...
in `.mad/agents/` (project-level) or `~/.mad/agents/` (user-level).

```rust
use xai_mad_agent::{AgentDefinition, AgentBuilder};
use xai_mad_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".mad/agents/code-reviewer.md")?;
...
```rust
use xai_mad_agent::discovery;

// Find all .md files in .mad/agents/ directories
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  xai-mad-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`xai-mad-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `xai-mad-agent` depends on it for tool setup.
- **`xai-mad-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `xai-mad-agent` (AGENTS.md
...
|---|---|---|
| `mad-build` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p xai-mad-agent

# Test
cargo test -p xai-mad-agent

# Clippy
cargo clippy -p xai-mad-agent --fix --allow-dirty
