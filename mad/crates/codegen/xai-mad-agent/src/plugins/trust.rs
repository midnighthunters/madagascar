...
//!
//! Plugins from project directories (`.mad/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.mad/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.mad/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.mad/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_mad_home() so a project's `.mad/trusted-plugins` is never
        // read as the user trust store when neither MAD_HOME nor a home dir resolves.
        let Some(mad) = xai_mad_config::user_mad_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = mad.join(TRUST_FILE_NAME);
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.mad/trusted-plugins`.
    ...
    /// rewrites `~/.mad/trusted-plugins` without the revoked entry.
