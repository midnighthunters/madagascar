...
use std::sync::Arc;
use xai_mad_tools::bridge::ToolBridge;
use xai_mad_tools::computer::types::{AsyncFileSystem, TerminalBackend};
...
/// The Mad [`ToolKind`] a vendor-compat `tools:` allowlist entry resolves to, so
/// a plugin's upstream allowlist still binds. Backed by the shared vendor-to-Mad
/// tool registry in `xai-mad-tools` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    xai_mad_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.mad/worktrees/project/fork-...-overlay`) that must stay
    ...
    parent_scheduler_handle:
        Option<xai_mad_tools::implementations::mad_build::scheduler::types::SchedulerHandle>,
    ...
    memory_backend: Option<Arc<dyn xai_mad_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: xai_mad_tools::implementations::web_search::WebSearchConfig,
    ...
    backend_search: bool,
    web_fetch_config: xai_mad_tools::implementations::mad_build::web_fetch::WebFetchConfig,
    lsp: Option<std::sync::Arc<dyn xai_mad_tools::implementations::lsp::LspBackend>>,
    image_gen_config: xai_mad_tools::implementations::mad_build::image_gen::ImageGenConfig,
    video_gen_config: xai_mad_tools::implementations::mad_build::video_gen::VideoGenConfig,
    app_builder_deployer_config:
        xai_mad_tools::implementations::mad_build::deploy_app::AppBuilderDeployerConfig,
    ...
    compat: xai_mad_tools::types::compat::CompatConfig,
    ...
    api_key_provider: Option<xai_mad_tools::types::SharedApiKeyProvider>,
    attribution_callback: Option<xai_mad_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in xai-mad-shell).
    ...
    preloaded_skills: Option<Vec<xai_mad_tools::implementations::skills::types::SkillInfo>>,
}
/// Ensure plan mode tools (`enter_plan_mode`, `exit_plan_mode`,
/// `ask_user_question`) are present in the tool config.
fn ensure_plan_mode_tools(tool_config: &mut xai_mad_tools::registry::types::ToolServerConfig) {
    use xai_mad_tools::implementations::mad_build;
    let existing: std::collections::HashSet<&str> =
        tool_config.tools.iter().map(|tc| tc.id.as_str()).collect();
    let missing_enter = !existing.contains("MadagascarBuild:enter_plan_mode");
    let missing_exit = !existing.contains("MadagascarBuild:exit_plan_mode");
    let missing_ask = !existing.contains("MadagascarBuild:ask_user_question");
    drop(existing);
    if missing_enter {
        tool_config
            .tools
            .push((&mad_build::EnterPlanModeTool).into());
    }
    if missing_exit {
        tool_config
            .tools
            .push((&mad_build::ExitPlanModeTool).into());
    }
    if missing_ask {
        tool_config
            .tools
            .push((&mad_build::AskUserQuestionTool).into());
    }
}
...
fn merge_tool_params(
    tool_config: &mut xai_mad_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(xai_mad_tools::computer::local::LocalFs),
            notification_handle,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<xai_mad_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn xai_mad_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: xai_mad_tools::implementations::mad_build::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: xai_mad_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// `MAD_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: xai_mad_tools::implementations::mad_build::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn xai_mad_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    pub fn with_image_gen_config(
        mut self,
        config: xai_mad_tools::implementations::mad_build::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    pub fn with_video_gen_config(
        mut self,
        config: xai_mad_tools::implementations::mad_build::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: xai_mad_tools::implementations::mad_build::deploy_app::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: xai_mad_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Set public model slugs advertised in the MadagascarBuild Task description.
    ...
    ///
    /// When disabled, `MadagascarBuild:ask_user_question` is stripped from the
    ...
    /// Without this, only auto-discovered skills (cwd/.mad/skills, ~/.mad/skills)
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.web_search_config.is_enabled() {
                use xai_mad_tools::implementations::mad_build;
                tool_config.tools.push((&mad_build::WebSearchTool).into());
            }
            if self.web_fetch_config.is_enabled() {
                use xai_mad_tools::implementations::mad_build;
                tool_config.tools.push((&mad_build::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&xai_mad_tools::implementations::mad_build::LspTool).into());
            }
            if self.image_gen_config.image_gen_enabled() {
                tool_config
                    .tools
                    .push((&xai_mad_tools::implementations::mad_build::ImageGenTool).into());
            }
            if self.image_gen_config.image_edit_enabled() {
                tool_config
                    .tools
                    .push((&xai_mad_tools::implementations::mad_build::ImageEditTool).into());
            }
            if self.video_gen_config.is_enabled() {
                tool_config
                    .tools
                    .push((&xai_mad_tools::implementations::mad_build::ImageToVideoTool).into());
                tool_config.tools.push(
                    (&xai_mad_tools::implementations::mad_build::ReferenceToVideoTool).into(),
                );
            }
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&xai_mad_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            ensure_plan_mode_tools(&mut tool_config);
        }
        if self.memory_backend.is_none() {
            let mad_build_ns = xai_mad_tools::types::tool::ToolNamespace::MadagascarBuild.to_string();
            let mem_search_id = format!(
                "{mad_build_ns}:{}",
                xai_mad_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{mad_build_ns}:{}",
                xai_mad_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if !self.ask_user_question_enabled {
            let ask_user_id = format!(
                "{}:ask_user_question",
                xai_mad_tools::types::tool::ToolNamespace::MadagascarBuild,
            );
        ...
        if task_stripped {
            use xai_mad_tools::types::tool::ToolNamespace;
            ...
            if !has_satisfier(ToolNamespace::MadagascarBuild, "run_terminal_cmd", true)
                && !has_satisfier(ToolNamespace::MadagascarBuildConcise, "run_terminal_cmd", true)
        ...
        if let xai_mad_tools::implementations::mad_build::web_fetch::WebFetchConfig::Enabled {
            ref params,
        } = self.web_fetch_config
            && let Ok(params_value) = serde_json::to_value(params)
            && let Some(obj) = params_value.as_object()
        {
            merge_tool_params(&mut tool_config, &["MadagascarBuild:web_fetch"], obj);
        }
        if let Some(ref bash_params) = self.bash_params_json {
            merge_tool_params(
                ...
                &[
                    ...
                    "MadagascarBuildConcise:run_terminal_cmd",
                ],
                bash_params,
            );
        }
        if let Some(ref ask_params) = self.ask_user_question_params_json {
            merge_tool_params(
                &mut tool_config,
                &["MadagascarBuild:ask_user_question"],
                ask_params,
            );
        }
        ...
        if !definition.tools.is_empty() {
            ...
            if unresolved.is_empty() {
                ...
                tracing::warn!(
                    ...
                    "tools allowlist had unmappable entries; keeping full mad toolset"
                );
            }
        }
        tool_config
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                xai_mad_tools::types::resources::TruncationCfg(
                    xai_mad_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(xai_mad_sampling_types::HostedTool::WebSearch { options: None });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(xai_mad_sampling_types::HostedTool::XSearch { options: None });
            }
            xai_mad_sampling_types::apply_tool_overrides(
                &mut hosted_tools,
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        xai_mad_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: xai_mad_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: xai_mad_tools::types::config_source::ConfigSource::Project {
    ...
    fn build_task_description_lists_public_model_slugs() {
        ...
        assert!(!desc.contains(concat!("mad", " models")));
    }
    #[test]
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use xai_mad_tools::types::template_renderer::TemplateRenderer;
        use xai_mad_tools::types::tool::ToolKind;
    ...
    async fn discovery_snapshot_records_gated_and_preloaded_skills() {
        use xai_mad_tools::computer::local::LocalTerminalBackend;
        use xai_mad_tools::notification::ToolNotificationHandle;
        let tmp = tempfile::tempdir().unwrap();
        let write_skill = |dir: &str, content: &str| {
            let d = tmp.path().join(".mad/skills").join(dir);
        ...
        let mut definition = crate::config::AgentDefinition::default_mad_build();
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                label: "mad-build / subagents+ask_user",
                profile: AgentDefinition::default_mad_build,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "mad-build / subagents / no-ask-user",
                profile: AgentDefinition::default_mad_build,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "mad-build / no-subagents / ask_user",
                profile: AgentDefinition::default_mad_build,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "mad-build / no-subagents / no-ask-user",
                profile: AgentDefinition::default_mad_build,
                subagents: false,
                ask_user: false,
            },
            PagerFlagCase {
                label: "mad-build-ask-user / subagents",
                profile: AgentDefinition::mad_build_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "mad-build-ask-user / no-subagents",
                profile: AgentDefinition::mad_build_ask_user,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "mad-build-plan",
                profile: AgentDefinition::mad_build_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "mad-build-plan / no-ask-user",
                profile: AgentDefinition::mad_build_plan,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "mad-build-plan-no-subagents",
                profile: AgentDefinition::mad_build_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "mad-build-plan-no-subagents / no-ask-user",
                profile: AgentDefinition::mad_build_plan_no_subagents,
                subagents: false,
                ask_user: false,
            },
    ...
    async fn curated_empty_toolset_fails_agent_build() {
        use xai_mad_tools::computer::local::LocalTerminalBackend;
        use xai_mad_tools::notification::ToolNotificationHandle;
        let mut profile = crate::config::AgentDefinition::default_mad_build();
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use xai_mad_tools::computer::local::LocalTerminalBackend;
        use xai_mad_tools::implementations::mad_build::ask_user_question::AskUserQuestionParams;
        use xai_mad_tools::notification::ToolNotificationHandle;
        use xai_mad_tools::types::resources::Params;
        let profile = crate::config::AgentDefinition::default_mad_build();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id == "MadagascarBuild:ask_user_question"),
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use xai_mad_tools::computer::local::LocalTerminalBackend;
        use xai_mad_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_mad_build();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("MadagascarBuild:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use xai_mad_tools::computer::local::LocalTerminalBackend;
        use xai_mad_tools::implementations::mad_build::bash::BashParams;
        use xai_mad_tools::notification::ToolNotificationHandle;
        use xai_mad_tools::types::resources::Params;
        let mut definition = crate::config::AgentDefinition::default_mad_build();
    ...
    /// Compat allowlist names (`Read`, `Bash`, `Grep`) map to their Mad
    /// equivalents by `ToolKind` — a real restricted toolset, not zero tools.
    #[tokio::test]
    async fn claude_tool_names_map_to_mad_equivalents() {
    ...
    /// Shell, LSP, ask, and task-lifecycle tool names resolve to their mad
    ...
    /// treat `mcp__*` as always-on, so mad keeps the MCP meta-tools
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        use xai_mad_tools::computer::local::LocalTerminalBackend;
        use xai_mad_tools::implementations::mad_build::web_fetch::WebFetchConfig;
        use xai_mad_tools::implementations::web_search::WebSearchConfig;
        use xai_mad_tools::notification::ToolNotificationHandle;
        let mut definition = crate::config::AgentDefinition::default_mad_build();
    ...
    /// mad-build toolsets have no Skill tool — skills are read from
    ...
    /// Compat `ToolSearch` meta-tool maps to mad's `search_tool` (MCP
    ...
    async fn build_with_web_search(
        ...
        tool_overrides: Option<xai_mad_sampling_types::ToolOverrides>,
    ) -> crate::agent::Agent {
        use xai_mad_tools::computer::local::LocalTerminalBackend;
        use xai_mad_tools::implementations::web_search::WebSearchConfig;
        use xai_mad_tools::notification::ToolNotificationHandle;
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, xai_mad_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, xai_mad_sampling_types::HostedTool::XSearch { .. })),
    ...
    async fn hosted_tools_bake_definition_tool_overrides_into_options() {
        let x_search = xai_mad_sampling_types::XSearchOptions {
            date_bound: Some(
                xai_mad_sampling_types::SearchDateBound::new(None, Some("2024-03-15".into()))
                    .unwrap(),
            ),
        };
        let agent = build_with_web_search(
            ...
            true,
            &[],
            Some(xai_mad_sampling_types::ToolOverrides {
        ...
        assert!(
            agent
                .hosted_tools()
                .contains(&xai_mad_sampling_types::HostedTool::XSearch {
