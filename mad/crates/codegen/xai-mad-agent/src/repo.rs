...
//! primitive consumed cross-crate by `xai-mad-workspace`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.mad/config.toml`, `.claude/settings.json`,
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // home-level `.mad`/`.mcp.json`/plugins would look repo-local. Drop
...
/// from `xai-mad-workspace`, which depends on THIS crate) to keep the dep edge
