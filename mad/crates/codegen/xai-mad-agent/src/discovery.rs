...
//!
//! Searches `.mad/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.mad/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.mad/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".mad/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.mad/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.mad/agents` / `.claude/agents`) under each
...
pub fn all_subagents(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<SubagentEntry> {
    ...
    all_subagents_with_home(cwd, toggle, dirs::home_dir().as_deref(), mad.as_deref())
}
...
fn all_subagents_with_home(
    ...
    home: Option<&Path>,
    mad_home: Option<&Path>,
) -> Vec<SubagentEntry> {
    let discovered = discover_with_home(cwd, home, mad_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.mad/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.mad/agents/` walking from `cwd` up to repo root
/// 2. `~/.mad/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.mad/bundled/agents/` (bundled, lowest priority)
///
/// Deduplicates by name — higher-priority definitions win.
/// User-level agent directories in priority order: user mad agents, `.claude`
/// compat agents, then bundled. `.mad` dirs resolve from `mad_home`
/// (MAD_HOME-aware) plus the legacy literal `~/.mad` when MAD_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    mad_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.mad, included only when it differs from mad_home
    // (i.e. MAD_HOME points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let legacy_mad = home
        .map(|h| h.join(".mad"))
        .filter(|legacy| mad_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = mad_home {
        dirs.push((g.join("agents"), AgentScope::User));
    }
    if let Some(l) = &legacy_mad {
...
pub fn discover(cwd: &Path) -> Vec<AgentDefinition> {
    let mad = xai_mad_config::user_mad_home();
    discover_with_home(cwd, dirs::home_dir().as_deref(), mad.as_deref())
}
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, mad_home) {
        if dir.is_dir() {
...
pub fn by_name(name: &str) -> Option<AgentDefinition> {
    let mad = xai_mad_config::user_mad_home();
    by_name_with_home(name, dirs::home_dir().as_deref(), mad.as_deref())
}
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, mad_home);
...
///
/// Project-level `.mad/agents/` has highest priority, then falls back
/// to built-ins, user-level, and finally bundled definitions.
pub fn by_name_in_cwd(name: &str, cwd: &Path) -> Option<AgentDefinition> {
    let mad = xai_mad_config::user_mad_home();
    by_name_in_cwd_with_home(name, cwd, dirs::home_dir().as_deref(), mad.as_deref())
}
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, mad_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, mad_home) {
        return Some(def);
    }
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the Mad
...
/// Load project agent definitions from every `.mad/agents` / `.claude/agents`
...
mod tests {
    ...
    fn test_origin_for_scope(scope: PluginScope) -> crate::plugins::PluginOrigin {
        ...
        match scope {
            ...
            PluginScope::Project => PluginOrigin::ProjectMad,
            PluginScope::User => PluginOrigin::UserMad,
    ...
    #[test]
    fn user_agent_dirs_includes_legacy_mad_when_mad_home_differs() {
        let home = Path::new("/home/u");
        let mad = Path::new("/custom/madhome");
        let paths: Vec<_> = user_agent_dirs(Some(home), Some(mad))
            .into_iter()
            .map(|(p, _)| p)
            .collect();
        assert!(paths.contains(&mad.join("agents")));
        assert!(paths.contains(&home.join(".mad").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&mad.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".mad").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_legacy_when_mad_home_is_dot_mad() {
        let home = Path::new("/home/u");
        let mad = home.join(".mad");
        let count = user_agent_dirs(Some(home), Some(&mad))
            .into_iter()
            .filter(|(p, _)| *p == mad.join("agents"))
            .count();
        assert_eq!(
            count, 1,
            "no duplicate ~/.mad/agents when mad_home == ~/.mad"
        );
    }
    ...
    #[test]
    fn test_by_name_builtin_mad_build() {
        let def = by_name("mad-build");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "mad-build");
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".mad").join("agents");
        let agents_dir_2 = inner_dir.join(".mad").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".mad").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".mad")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".mad")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".mad").join("agents");
        let bundled_dir = home.join(".mad").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".mad")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".mad")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".mad").join("agents");
        let bundled_dir = home.join(".mad").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        // Create a project-level "mad-build" that shadows the built-in
        write_agent_file(
            &agents_dir,
            "mad-build.md",
            "mad-build",
        ...
        let def = by_name_in_cwd("mad-build", tmp.path());
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "mad-build");
        assert_eq!(def.description, "Custom mad-build");
    }

    #[test]
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .mad/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        use std::str::FromStr;
        let variant = BuiltinAgentName::from_str("mad-build-orchestrator")
            .expect("from_str must resolve mad-build-orchestrator");
        assert_eq!(variant, BuiltinAgentName::MadagascarBuildOrchestrator);
        let def = variant.definition();
        assert_eq!(def.name, "mad-build-orchestrator");
        assert!(
    ...
    fn test_orchestrator_by_name_in_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let def = by_name_in_cwd("mad-build-orchestrator", tmp.path())
            .expect("by_name_in_cwd must find mad-build-orchestrator");
        assert_eq!(def.name, "mad-build-orchestrator");
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.mad/agents/) agent named "explore" should NOT shadow
