...
//! `TemplateRenderer` in `xai-mad-tools`. This struct does NOT own a
...
pub struct PromptContext {
    ...
    /// Identity in the primary mad-build system prompt (`You are <label>…`).
    /// Not the UI picker name. Defaults to [`DEFAULT_SYSTEM_PROMPT_LABEL`].
    #[serde(default = "default_system_prompt_label")]
    pub system_prompt_label: String,
}
/// Default identity on trim-tool-descriptions (`You are Mad released by xAI`).
pub const DEFAULT_SYSTEM_PROMPT_LABEL: &str = "Mad";
...
mod tests {
    ...
    fn test_placeholders_system_prompt_label_override() {
        let mut ctx = test_context();
        ctx.system_prompt_label = "Mad Internal".into();
        let p = ctx.placeholders();
        assert_eq!(p["system_prompt_label"], "Mad Internal");
    }
    #[test]
    ...
    fn test_agents_md_paths_use_display_cwd_in_rendered_section() {
        ...
        let overlay_path = "/root/.mad/worktrees/my-project/ab-123-a-overlay";
    ...
    fn trimmed_prompts_no_redundant_identity() {
        ...
        for (name, prompt) in [
            ...
            assert!(
                !prompt.contains("You are a Madagascar Build agent"),
