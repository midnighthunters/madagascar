...
//! - `Default`: the legacy Madagascar Build prefix (built by the shell layer).
...
use xai_mad_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy Madagascar Build prefix (`<user_info>` + optional `<git_status>`), built directly by the
...
/// One discovered rule file (AGENTS.md / Claude.md / .mad/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `xai_mad_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.mad/, ~/.claude/).
    ...
    /// (`~/.mad/projects/<encoded-cwd>/mcps`). Surfaced in
