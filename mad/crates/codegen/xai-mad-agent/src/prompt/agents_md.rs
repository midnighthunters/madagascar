...
//!
//! Searches from cwd to repo root, plus `~/.mad/`. Also discovers
//! `*.md` files in rules directories: vendor-prefixed `.mad/rules/`,
...
//! (`~/.mad/rules/`, `~/.claude/rules/`, `~/.cursor/rules/`).
...
/// Find `*.md` files in `.mad/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.mad/, git repo root, and session cwd.
...
async fn read_agents_config_with_roots(
    ...
    add_discovery_root(&mut home_roots, mad_home, true, HOME_RULES_DIRS);
    ...
    candidates
        ...
        .filter_map(|candidate| {
            ...
            let content = if candidate.is_rule {
                xai_mad_tools::implementations::skills::skill::extract_skill_body(&content)
            } else {
                content
            };
...
/// Used by `xai-mad-shell` to structurally detect legacy untagged AGENTS.md
...
/// Open/close `system-reminder` (Mad) or `system_reminder` (Cursor/IDE), case-insensitive.
...
mod tests {
    ...
    async fn home_and_project_rules_have_stable_order_without_doubled_paths() {
        ...
        for path in [
            mad_home.join(".mad/rules/doubled.md"),
    ...
    async fn vendor_home_agents_and_rules_cells_are_independent() {
        let tmp = tempfile::tempdir().unwrap();
        let mad_home = tmp.path().join("mad-home");
        let home = tmp.path().join("home");
        let cwd = tmp.path().join("project");
        fs::create_dir_all(&mad_home).unwrap();
    ...
    #[tokio::test]
    async fn nested_mad_home_keeps_project_role_in_repo_order() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        let nested = repo.join("nested");
        fs::create_dir_all(nested.join("rules")).unwrap();
        fs::create_dir_all(nested.join(".mad/rules")).unwrap();
        init_git_repo(&repo);
        ...
        fs::write(nested.join(".mad/rules/project.md"), "nested-project-rule").unwrap();
    ...
    #[tokio::test]
    async fn overlapping_mad_home_and_project_root_merges_roles() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        ...
        fs::write(repo.join(".mad/rules/project.md"), "project-mad-rule").unwrap();
        fs::write(repo.join(".claude/rules/project.md"), "project-claude-rule").unwrap();
        fs::create_dir_all(repo.join(".mad/.mad/rules")).unwrap();
        fs::write(repo.join(".mad/.mad/rules/doubled.md"), "doubled").unwrap();
        ...
        .await;
        for expected in ["home-rule", "project-mad-rule", "project-claude-rule"] {
            assert_eq!(
                configs
                    .iter()
    ...
    async fn rule_frontmatter_is_stripped_but_named_frontmatter_is_preserved() {
        ...
        for (path, body) in [
            (mad_home.join("rules/global.md"), "custom-home-body"),
            (home.join(".claude/rules/global.md"), "claude-body"),
            (home.join(".cursor/rules/global.md"), "cursor-body"),
            (repo.join(".mad/rules/project.md"), "mad-project-body"),
