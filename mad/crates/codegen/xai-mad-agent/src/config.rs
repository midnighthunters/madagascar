//! Agent definition types — parsed from `.mad/agents/*.md` files.
...
use xai_mad_tools::implementations::codex;
use xai_mad_tools::implementations::mad_build;
use xai_mad_tools::implementations::mad_build_concise;
use xai_mad_tools::implementations::memory;
use xai_mad_tools::implementations::opencode;
use xai_mad_tools::implementations::search_tool;
use xai_mad_tools::implementations::use_tool;
use xai_mad_tools::registry::types::{ToolConfig, ToolServerConfig};
...
/// Orchestrator-specific prompt body appended to the standard MadagascarBuild
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&mad_build::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&mad_build::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&mad_build::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` → `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&mad_build::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` → `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&mad_build::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_mad_build_toolset()` with tools that are dynamically
...
/// zero local dispatch.
pub fn workspace_mad_build_toolset() -> ToolServerConfig {
    let mut tools = default_mad_build_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&mad_build::EnterPlanModeTool).into());
    tools.push((&mad_build::ExitPlanModeTool).into());
    tools.push((&mad_build::AskUserQuestionTool).into());
    tools.push((&mad_build::WebSearchTool).into());
    tools.push((&mad_build::ImageGenTool).into());
    tools.push((&mad_build::ImageToVideoTool).into());
    tools.push((&mad_build::ReferenceToVideoTool).into());
    tools.push((&mad_build::WebFetchTool).into());
    tools.push((&memory::search_tool::MemorySearchImpl).into());
    tools.push((&memory::get_tool::MemoryGetImpl).into());
    tools.push((&mad_build::LspTool).into());
    ToolServerConfig {
        tools,
        behavior_preset: None,
    }
}
/// Toolset for the `mad-computer` (workspace/sandbox) preset.
fn mad_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&mad_build::ReadFileTool).into(),
        (&mad_build::SearchReplaceTool).into(),
        (&opencode::OpenCodeWriteTool).into(),
        (&mad_build::ListDirTool).into(),
        (&mad_build::GrepTool).into(),
        (&mad_build::KillTerminalCommandTool).into(),
        (&mad_build::GetTerminalCommandOutputTool).into(),
    ];
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("mad-build", workspace_mad_build_toolset()),
        ("mad-build-concise", mad_build_concise_toolset()),
        ("mad-build-plan", mad_build_plan_toolset()),
        ("codex", codex_toolset()),
        ("explore", explore_toolset()),
        ("plan", plan_toolset()),
        ("mad-computer", mad_computer_toolset()),
    ]
}
...
fn default_mad_build_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&mad_build::ReadFileTool).into(),
            ...
            (&mad_build::TodoWriteTool).into(),
            task_output_tool_config(),
            wait_tasks_tool_config(),
            task_tool_config(),
            (&mad_build::SchedulerCreateTool).into(),
            (&mad_build::SchedulerDeleteTool).into(),
            (&mad_build::SchedulerListTool).into(),
            (&mad_build::MonitorTool).into(),
            (&search_tool::SearchTool).into(),
            (&use_tool::UseTool).into(),
            (&mad_build::UpdateGoalTool).into(),
            (&mad_build::WorkflowTool).into(),
        ],
        behavior_preset: None,
    }
}
fn mad_build_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&mad_build_concise::BashConciseTool).into(),
            (&mad_build_concise::ReadFileConciseTool).into(),
            (&mad_build_concise::SearchReplaceConciseTool).into(),
            (&mad_build::ListDirTool).into(),
            (&mad_build::GrepTool).into(),
            kill_task_tool_config(),
            (&mad_build::TodoWriteTool).into(),
...
pub fn mad_build_hashline_toolset(
    hashline_tools: Vec<xai_mad_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<xai_mad_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
...
/// Madagascar Build + plan mode toolset.
///
/// Extends the default `mad-build` toolset with plan mode tools:
...
fn mad_build_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            // Standard mad-build tools
            bash_tool_config(),
            (&mad_build::ReadFileTool).into(),
            (&mad_build::SearchReplaceTool).into(),
            (&mad_build::ListDirTool).into(),
            ...
            (&mad_build::WorkflowTool).into(),
            // Plan mode tools
            (&mad_build::EnterPlanModeTool).into(),
            (&mad_build::ExitPlanModeTool).into(),
            (&mad_build::AskUserQuestionTool).into(),
        ],
        behavior_preset: None,
    }
}
...
fn orchestrator_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            ...
            // Web tools
            (&mad_build::WebSearchTool).into(),
            (&mad_build::WebFetchTool).into(),
            // Imagine
            (&mad_build::ImageGenTool).into(),
            (&mad_build::ImageToVideoTool).into(),
            (&mad_build::ReferenceToVideoTool).into(),
            // Memory
...
/// Madagascar Build + plan mode toolset WITHOUT subagent tools.
///
/// Same as `mad_build_plan_toolset` but excludes `TaskTool`,
/// `TaskOutputTool`, and `KillTaskTool`. Use this when the shell
/// does not have subagent infrastructure wired up.
fn mad_build_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            // Standard mad-build tools (minus TaskTool only — KillTaskTool and
...
/// Default Madagascar Build toolset + `ask_user_question`.
///
/// Same as `default_mad_build_toolset` with the `AskUserQuestionTool` added,
/// allowing the agent to ask structured questions without full plan mode.
fn mad_build_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&mad_build::ReadFileTool).into(),
...
/// - `model: mad-3-fast` → `Override("mad-3-fast")`
...
pub enum BuiltinAgentName {
    ...
    MadagascarBuildPlan,
    MadagascarBuildPlanNoSubagents,
    MadagascarBuildAskUser,
    Codex,
    Opencode,
    GeneralPurpose,
    Explore,
    Plan,
    BrowserUse,
    #[strum(serialize = "mad-build-orchestrator")]
    MadagascarBuildOrchestrator,
}
...
impl BuiltinAgentName {
    ...
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::MadagascarBuild => AgentDefinition::default_mad_build(),
            Self::MadagascarBuildConcise => AgentDefinition::mad_build_concise(),
            Self::MadagascarBuildPlan => AgentDefinition::mad_build_plan(),
            Self::MadagascarBuildPlanNoSubagents => AgentDefinition::mad_build_plan_no_subagents(),
            Self::MadagascarBuildAskUser => AgentDefinition::mad_build_ask_user(),
            ...
            Self::MadagascarBuildOrchestrator => AgentDefinition::mad_build_orchestrator(),
        }
    }
...
/// Portable agent identity — parsed from .mad/agents/*.md.
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_mad_build_toolset")]
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub tool_overrides: Option<xai_mad_sampling_types::ToolOverrides>,
...
pub enum AgentScope {
    /// .mad/agents/ (project-level, highest priority)
    Project,
    /// ~/.mad/agents/ (user-level)
    User,
    /// ~/.mad/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_mad_build(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.mad/agent-memory/<name>/`
    User,
    /// `<project>/.mad/agent-memory/<name>/`
    Project,
    /// `<project>/.mad/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: xai_mad_config::mad_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".mad/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".mad/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `xai_mad_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let mad = xai_mad_config::user_mad_home();
        let home = dirs::home_dir();
        for (dir, scope) in crate::discovery::user_agent_dirs(home.as_deref(), mad.as_deref()) {
            if path.starts_with(&dir) {
                return scope;
            }
        }
        if path_str.contains(".mad/agents/") || path_str.contains(".mad\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".mad/bundled/agents/")
            || path_str.contains(".mad\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    /// (`!inject_default_tools`). Stock `mad-build*` agents leave all
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<xai_mad_tools::registry::types::ToolConfig>,
    ) {
        const FILE_TOOL_SLOTS: &[[&str; 2]] = &[
            ["MadagascarBuild:read_file", "MadagascarBuildHashline:hashline_read"],
            [
                "MadagascarBuild:search_replace",
                "MadagascarBuildHashline:hashline_edit",
            ],
            ["MadagascarBuild:grep", "MadagascarBuildHashline:hashline_grep"],
        ];
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_mad_build_toolset(),
    ...
    pub fn default_mad_build() -> Self {
        Self::base(
            BuiltinAgentName::MadagascarBuild,
            "Madagascar Build agent for software engineering tasks.",
        )
    }
    /// Madagascar Build Concise agent definition — concise output format for SFT/RL.
    pub fn mad_build_concise() -> Self {
        Self {
            tool_config: mad_build_concise_toolset(),
            agents_md: false,
            ..Self::base(
                BuiltinAgentName::MadagascarBuildConcise,
                "Madagascar Build agent with concise output format.",
            )
        }
    }
    /// Madagascar Build agent with plan mode tools.
    pub fn mad_build_plan() -> Self {
        Self {
            tool_config: mad_build_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::MadagascarBuildPlan,
                "Madagascar Build agent with plan mode support.",
            )
        }
    }
    /// Madagascar Build + plan mode WITHOUT subagent tools.
    pub fn mad_build_plan_no_subagents() -> Self {
        Self {
            tool_config: mad_build_plan_no_subagents_toolset(),
            ..Self::base(
                BuiltinAgentName::MadagascarBuildPlanNoSubagents,
                "Madagascar Build agent with plan mode (no subagents).",
            )
        }
    }
    /// Default Madagascar Build agent with the `ask_user_question` tool.
    pub fn mad_build_ask_user() -> Self {
        Self {
            tool_config: mad_build_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::MadagascarBuildAskUser,
                "Madagascar Build agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    /// Madagascar Build Orchestrator — GBL model with full MadagascarBuild tools
    ...
    pub fn mad_build_orchestrator() -> Self {
        Self {
            tool_config: orchestrator_toolset(),
            inject_default_tools: false,
            prompt_body: Some(ORCHESTRATOR_PROMPT_BODY.to_string()),
            ..Self::base(
                BuiltinAgentName::MadagascarBuildOrchestrator,
                "MadagascarBuild orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_mad_build_toolset();
        }
...
mod tests {
    ...
    fn presets_select_distinct_toolsets_by_size() {
        let gb = toolset_for_preset("mad-build").unwrap();
    ...
    fn mad_computer_exclusive_ids() -> Vec<String> {
        #[allow(unused_mut)]
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&mad_build::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&mad_build::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn mad_computer_preset_is_curated_mad_build_subset() {
        let gc = toolset_for_preset("mad-computer").unwrap();
        let gb = toolset_for_preset("mad-build").unwrap();
        let gb_ids: std::collections::HashSet<&str> =
            gb.tools.iter().map(|t| t.id.as_str()).collect();
        let exclusive_ids = mad_computer_exclusive_ids();
        ...
        for t in &gc.tools {
            ...
            assert!(
                ...
                "mad-computer tool `{}` must also ship in the mad-build preset",
                t.id
            );
        }
        assert!(
            gc.tools.len() < gb.tools.len(),
            "mad-computer should be a curated subset of mad-build"
        );
    }
    #[test]
    fn mad_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("mad-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&mad_build::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            ids.contains(
                ToolConfig::from(&mad_build::KillTerminalCommandTool)
                    .id
                    .as_str()
            )
        );
        assert!(!ids.contains(ToolConfig::from(&mad_build::TaskOutputTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&mad_build::KillTaskTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&mad_build::TaskTool).id.as_str()));
        for t in &gc.tools {
            if t.id == ToolConfig::from(&mad_build::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&mad_build::KillTerminalCommandTool).id
            {
                assert!(t.name_override.is_none(), "tool `{}` must not rename", t.id);
            }
        }
    }
    /// The mad-computer preset must ship a full-file write tool (legacy
    /// `write_file` parity) — the same OpenCode `write` tool the mad-build
    ...
    #[test]
    fn mad_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("mad-computer").unwrap();
        let write_id = ToolConfig::from(&opencode::OpenCodeWriteTool).id;
        assert!(
            gc.tools.iter().any(|t| t.id == write_id),
            "mad-computer preset must include the `{write_id}` tool"
        );
    }
    #[test]
    fn mad_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("mad-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&mad_build::LspTool).id,
            ToolConfig::from(&mad_build::EnterPlanModeTool).id,
            ToolConfig::from(&mad_build::ExitPlanModeTool).id,
        ] {
            assert!(
                !gc_ids.contains(excluded.as_str()),
                "mad-computer preset must not advertise `{excluded}`"
            );
        }
        let full = workspace_mad_build_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_mad_build_toolset must ship `{present}`"
            );
        }
    }
    ...
    fn expected_strict_harness(name: BuiltinAgentName) -> bool {
        match name {
            BuiltinAgentName::Codex | BuiltinAgentName::MadagascarBuildOrchestrator => true,
            BuiltinAgentName::MadagascarBuild
            | BuiltinAgentName::MadagascarBuildConcise
            | BuiltinAgentName::MadagascarBuildPlan
            | BuiltinAgentName::MadagascarBuildPlanNoSubagents
    ...
    fn is_strict_harness_agent_type_classifies_by_name() {
        for strict in ["codex", "mad-build-orchestrator"] {
            assert!(
...
"#;
    ...
    fn test_model_override_display_shows_id() {
        ...
        assert_eq!(
            ModelOverride::Override("mad-3-fast".to_string()).to_string(),
            "mad-3-fast"
        );
    }
    #[test]
...
"#;
        ...
        assert_eq!(def.model, ModelOverride::Override("mad-3".to_string()));
    }
    #[test]
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.mad/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.mad/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_mad_build_tools() {
        let content = r#"---
name: default-tools
...
"#;
        ...
        assert!(
            ...
            "default tool_config should have mad_build tools"
        );
    }
    #[test]
    ...
    fn test_from_json_has_default_toolset_with_task_tool() {
        ...
        let def = AgentDefinition::from_json(&json).unwrap();
        let task_tool_id = "MadagascarBuild:task";
        assert!(
    ...
    fn test_model_override_serde_explicit_model_id() {
        let yaml = "\"mad-3-fast\"";
        let m: ModelOverride = serde_yaml::from_str(yaml).unwrap();
        assert_eq!(m, ModelOverride::Override("mad-3-fast".to_string()));
    }
    #[test]
    ...
    fn test_model_override_serialize_override() {
        let m = ModelOverride::Override("mad-3-fast".to_string());
        let s = serde_json::to_string(&m).unwrap();
        assert_eq!(s, "\"mad-3-fast\"");
    }
    #[test]
    fn test_model_override_in_frontmatter() {
        ...
        assert_eq!(
            def.model,
            ModelOverride::Override("mad-3-fast".to_string())
        );
    }
    #[test]
    ...
    fn test_model_override_in_json() {
        let json = serde_json::json!({
            "name": "test",
            "description": "Test",
            "model": "mad-code-fast-1"
        });
        let def = AgentDefinition::from_json(&json).unwrap();
        assert_eq!(
            def.model,
            ModelOverride::Override("mad-code-fast-1".to_string())
        );
    }
    #[test]
    fn test_builtin_agent_name_strum_round_trip() {
        use std::str::FromStr;
        for (s, expected) in [
            ("mad-build", BuiltinAgentName::MadagascarBuild),
            ("mad-build-concise", BuiltinAgentName::MadagascarBuildConcise),
            ("mad-build-ask-user", BuiltinAgentName::MadagascarBuildAskUser),
    ...
    fn def_with_template(tpl: crate::prompt::context::TemplateOverride) -> AgentDefinition {
        let mut def = AgentDefinition::default_mad_build();
