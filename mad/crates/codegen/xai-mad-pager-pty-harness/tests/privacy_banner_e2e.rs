...
//! `MAD_PRIVACY_NOTICE_ROLLOUT=1`.
//!
//! ```bash
//! cargo test -p xai-mad-pager-pty-harness --test privacy_banner_e2e \
//!   -- --ignored --nocapture
//! ```
...
/// Poll `<home>/.mad/config.toml` for the async `privacy_banner_acked`
...
fn wait_for_ack_on_disk(pager: &mut PtyHarness, home: &Path, timeout: Duration) -> Result<()> {
    let path = home.join(".mad").join("config.toml");
