...
//! `MadAuth` → auth meta → `AppView::coding_data_sharing_lock()` →
...
//! `MAD_PRIVACY_NOTICE_ROLLOUT=1` — asserted on the authenticated welcome
//! screen before opening settings. Row/input details are unit-tested in
//! `xai-mad-pager` (`views/settings_modal/tests.rs`, `locked_coding_*`);
//! this suite covers the auth-to-render pipeline.
//!
//! ```bash
//! cargo test -p xai-mad-pager-pty-harness --test settings_locked_row_e2e \
//!   -- --ignored --nocapture
//! ```
...
const ROWS: u16 = 50;
const COLS: u16 = 120;
const BANNER_TITLE: &str = "Help improve Mad";
...
fn locked_row_env_ops() -> [EnvOp<'static>; 3] {
    [
        EnvOp::set("MAD_PRIVACY_NOTICE_ROLLOUT", "1"),
        EnvOp::set("MAD_ZDR_ACCESS_ENABLED", "1"),
