//! `pty-bench` — PTY benchmark CLI for `xai-mad-pager`.
//!
...
//! ```bash
//! cargo bench -p xai-mad-pager-pty-harness \
...
//! ```bash
//! PAGER_BINARY=./artifacts/mad-${VERSION}-linux-x86_64 \
//!   cargo bench -p xai-mad-pager-pty-harness \
//!   --bench pty_bench -- --all \
...
#[command(
    name = "pty-bench",
    about = "PTY benchmark harness for xai-mad-pager",
    long_about = None,
)]
struct Cli {
