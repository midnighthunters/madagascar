...
//! [`MockInferenceServer`] from `xai-mad-test-support` and provides the
...
use xai_mad_test_support::{MockInferenceServer, TestSandbox};

pub use xai_mad_test_support::mock_server::LogEntry;
pub use xai_mad_test_support::mock_server::MockModelEntry as MockModel;
pub use xai_mad_test_support::mock_server::StorageUpload;
pub use xai_mad_test_support::sse;
pub use xai_mad_test_support::{
...
impl ContentController {
    ...
    pub async fn start_with_models(models: Vec<MockModel>) -> Result<Self> {
        ...
        sandbox.set_env("MAD_PROMPT_SUGGESTIONS", "false");
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.mad
...
mod tests {
    ...
    async fn read_foreground(url: String, endpoint: InferenceEndpoint) -> String {
        ...
        reqwest::Client::new()
            ...
            .header("x-mad-turn-idx", "1")
            .header("x-mad-req-id", format!("direct-{endpoint:?}"))
            .json(&body)
            .send()
            .await
    ...
    /// on the SuperMad upsell screen and breaks every PTY test.
    #[tokio::test]
