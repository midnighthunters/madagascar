//! Unified PTY harness for xai-mad-pager.
//!
...
//!    `xai-mad-pager/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `MAD_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// (XTVERSION probes and mad wrap). Content-backed launches must use
    /// [`Self::new_in_sandbox`].
    ...
    /// Spawn from a canonical [`xai_mad_test_support::TestSandbox`] baseline
    ...
    pub fn new_in_sandbox(
        ...
        cols: u16,
        args: &[&str],
        sandbox: &xai_mad_test_support::TestSandbox,
        env: &[(&str, &str)],
        cwd: Option<&Path>,
    ) -> Result<Self> {
    ...
    /// # use xai_mad_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
    ...
    /// through mad's stdout. Used to reproduce the doubled-line class of bugs
    /// where mad's diff renderer never re-asserts a region it didn't write
