...
use xai_mad_test_support::{TestProcessTree, TestSandbox, process_has_exited_without_reap};
...
impl PtyController {
    /// Inherit the parent environment for terminal-brand probes, mad-wrap
    ...
    fn spawn_inner(
        ...
        let process_tree = process_pid.map(|pid| TestProcessTree::attach(pid, "mad PTY child"));
...
const CLIPBOARD_SINK_ENV_VARS: &[&str] = &["MAD_OSC52_SINK", "LC_MAD_OSC52_SINK"];

/// Host terminal identity markers stripped from the child environment.
///
/// The pager's terminal detection
/// (`xai-mad-pager-render/src/terminal/mod.rs`:
...
/// [`xai_mad_test_support::TestSandbox`], which always clears inheritance.
/// The explicitly named inherited-env path is reserved for terminal probing and
/// mad-wrap fixtures. Caller overrides are always applied last.
fn apply_child_env(cmd: &mut CommandBuilder, sandbox: Option<&TestSandbox>, env: &[EnvOp<'_>]) {
    ...
    // A harness launched under `mad wrap` must not silently confirm clipboard
...
mod tests {
    ...
    fn apply_child_env_strips_all_host_terminal_markers() {
        ...
        cmd.env("MAD_SCROLL_LOG", "/tmp/scroll.jsonl");
        let sandbox = TestSandbox::new();
        ...
        assert_eq!(
            cmd.get_env("MAD_SCROLL_LOG").and_then(|v| v.to_str()),
            None,
            "hermetic baseline must remove unrelated inherited vars"
        );
        assert_eq!(
            cmd.get_env("MAD_HOME").and_then(|v| v.to_str()),
            sandbox.mad_home().to_str()
        );
    }
    ...
    fn apply_child_env_uses_sandbox_baseline() {
        ...
        assert_eq!(cmd.get_env("MAD_LEADER_SOCKET"), None);
    }
    ...
    fn apply_child_env_remove_deletes_sandbox_credential() {
        ...
        assert_eq!(
            cmd.get_env("MAD_XAI_API_BASE_URL")
                .and_then(|v| v.to_str()),
    ...
    fn apply_child_env_caller_env_overrides_survive_strips() {
        ...
        assert_eq!(
            cmd.get_env("MAD_OSC52_SINK").and_then(|v| v.to_str()),
            Some("1"),
