//! Scripted TUI scenario runner for xai-mad-pager.
//!
...
pub struct EnvironmentConfig {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Optional `config.toml` written into the run's isolated `$MAD_HOME`
...
impl ScriptedScenarioRunner {
    ...
    pub async fn run(&self, scenario: &ScriptedScenario) -> Result<ScriptedRunReport> {
        ...
        if let Some(config_toml) = &scenario.environment.config_toml {
            ...
            fs::create_dir_all(&mad_home)
                .with_context(|| format!("create scenario MAD_HOME {}", mad_home.display()))?;
            fs::write(mad_home.join("config.toml"), config_toml)
...
mod tests {
    ...
    fn workspace_rejects_paths_outside_the_tempdir() {
        ...
        let sandbox = xai_mad_test_support::TestSandbox::new();
