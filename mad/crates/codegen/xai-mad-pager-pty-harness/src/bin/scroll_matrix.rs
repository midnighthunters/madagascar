//! `scroll-matrix` — scroll validation matrix sweep for xai-mad-pager.
//!
...
use xai_mad_pager_pty_harness::pager_binary;
use xai_mad_pager_pty_harness::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against xai-mad-pager",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary. Defaults to PAGER_BINARY, CARGO_BIN_EXE_xai-mad-pager,
...
async fn run() -> Result<ExitCode> {
    ...
    // The pager child resolves MAD_SCROLL_LOG against ITS cwd (the
