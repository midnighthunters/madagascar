...
pub enum CompactionTarget {
    ...
    /// with a single summary — mad-build's full-replace strategy. No tail is
...
/// stream processor. Implemented by Mad chat's
/// `StreamProcessor` (`Item = Arc<MadTurn>`).
///
...
pub trait CompactionStreamProc: Send + Sync {
    ...
    /// item type (`Arc<MadTurn>`) already satisfies it.
