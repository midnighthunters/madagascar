...
///
/// - `FullReplace` (default): mad-build's full-replace strategy — summarize
...
///
/// Uses **percentage** thresholds (borrowed from mad-shell's
...
/// time (token threshold only, matching mad-build) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as mad-build full-replace auto-compact). Worthless early passes are
    ...
    /// `<mad_user_queries>` preamble prepended to the history compaction
...
pub const DEFAULT_COMPACTION_MODEL_NAME: &str = "mad-4.20";
