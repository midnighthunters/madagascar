...
// The `Shared` summarizer reuses mad-build's full-replace summarization core
...
/// `FullReplace` strategy (default): mad-build's full-replace — summarize the
...
/// `<mad_user_queries>` preamble: the shared `code_compaction` summarizer
/// (always [`IntraSummarizer::Shared`] here, regardless of `policy.summarizer`)
/// preserves user intent itself, matching mad-build. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize the whole conversation through mad-build's shared core.
    ...
    //     Shared with Mad chat inter-compaction via `append_reminder_block` so
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 3a. For `History` target, split prior `<mad_user_queries>` blocks
    //     out of any prior compaction summary items before sampling — same
    //     primitive inter-compaction uses, so the LLM never sees
    //     `<mad_user_queries>` and won't re-emit it (which would snowball
    ...
    //     and the `<mad_user_queries>` preamble below — stays intra's.
    let summary_text = match policy.summarizer {
        ...
        // New (default): mad-build's shared summarization core from
    ...
    // 3c. For `History` target, prepend a `<mad_user_queries>` preamble so
...
/// — mad-build's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and mad-build stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // mad-build appends the summarization prompt as the final user message;
    ...
    .await
    {
        // mad-build returns the raw summary and cleans it in its assembler;
...
/// metric label. Keep these in sync with the doc string on Mad chat's
...
/// the literal error messages produced by the Mad chat sampler —
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the Mad chat sampler when the
    ...
    /// — guards the forwarding that Mad chat (`Arc<MadTurn>`) relies on.
    #[test]
