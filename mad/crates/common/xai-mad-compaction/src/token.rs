...
//!
//! - Mad chat has a real tokenizer (`TextTokenizer` / `ImageTokenizer`) and
//!   counts whole turns via `MadTurn::get_num_tokens`.
//! - mad-build estimates with `bytes / 4`.
//!
...
//! (Mad chat: `ItemTokenCounter<Arc<MadTurn>>`), so exactly one mechanism
//! is in play.
