...
//! harness implements [`CompactionItem`] for its `MadTurn`;
//! mad-build implements it for `xai_mad_sampling_types::ConversationItem`.
//!
...
//! `<mad_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `MadRole` (Mad chat) and the
/// `ConversationItem` variants (mad-build).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (Mad chat) — maps to System on harnesses without a
...
/// extraction (`<mad_file id=".." name=".." />` lines in the
/// `<mad_user_queries>` preamble).
...
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (Mad chat's
    /// `MadTurn`) compute the flattened text on demand rather than storing a
    /// borrowable slice.
    ...
    /// Whether this item carries a *prior compaction summary* (Mad chat: a
    ...
    /// strips their `<mad_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<mad_file>` lines in the
    /// `<mad_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (Mad chat: a `Developer` turn with category
    ...
    /// requests/results (Mad chat: keep only `Channel` contents of an
...
/// constructs the typed harness items that make up mad-build's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by Mad chat's
/// `MadTurn`; adding these constructors to it as required methods would break
/// that impl. They are also mad-build-specific (Mad chat's tail-keep path
...
///
/// The mad-build implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (Mad chat stores turns as `Arc<MadTurn>`).
