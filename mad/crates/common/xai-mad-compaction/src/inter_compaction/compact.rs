...
//! extraction, chunk summarisation, and the final `<mad_user_queries>`
//! + `<chunk_summary>` assembly — is shared. The harness supplies the
//! candidate items, the *current* user-queries preamble (Mad chat
...
pub struct ChunkedCompactionOutput {
    /// `<mad_user_queries>` preamble + `<chunk_summary index="i">` blocks.
...
/// 2. [`separate_prior_user_queries`] — split prior `<mad_user_queries>`
...
/// round's user messages (Mad chat: verbatim from the raw request, with
/// attachment refs). `conversation_id` / `response_id` are threaded
/// through for log correlation only.
///
/// Observer events (the Mad chat observer maps them to the
...
pub async fn sample_compaction_chunked<T: CompactionItemBuilder + Send + Sync>(
    ...
    // Step 2 — split prior `<mad_user_queries>` out of every prior
