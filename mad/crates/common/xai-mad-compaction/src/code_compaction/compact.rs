//! mad-build's full-replace compaction pass.
//!
//! mad-build does not select a tail to keep; it summarizes the whole
...
//! ```
//!
//! Per-harness concerns stay in the product host (for example `xai-mad-shell`): the triggers, the
...
pub struct FullReplaceContext<T> {
    ...
    /// turn). mad-build keeps this; pass empty to drop it.
...
///
/// Returned by [`sample_full_replace_summary`] for harnesses (mad-build's
...
/// Run mad-build's full-replace compaction pass and return the rebuilt
...
///
/// This is the seam mad-build's shell uses: it drives the verbatim → fitted →
    ...
    O: FullReplaceObserver + ?Sized,
{
    ...
    let prompt = CompactionPrompt {
        // mad-build appends the summarization prompt as the final user
...
mod tests {
    ...
    /// returns a structured summary must produce mad-build's exact compacted
    /// history shape, with the LLM output cleaned and the agent-state reminder
    /// carried through as the final item.
    #[tokio::test]
    async fn full_replace_produces_mad_build_history_shape() {
        let llm_turns = vec![
