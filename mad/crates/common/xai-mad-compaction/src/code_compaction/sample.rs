...
//!
//! The canonical `sample → classify → retry` loop, used by **both** mad-build's
//! full-replace pass ([`sample_full_replace_summary`](super::sample_full_replace_summary))
//! and Mad chat's intra `Shared` summarizer
...
//! the winning summary (mad-build cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the mad-build host uses to step down its input size).
