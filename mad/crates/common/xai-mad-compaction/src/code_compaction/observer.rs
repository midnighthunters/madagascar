//! Observability seam for the full-replace (mad-build) pass.
//!
//! The shared orchestrator reports per-attempt and terminal outcomes through
//! this trait so each harness can emit its own telemetry (mad-build:
...
//!
//! Emission points are part of the behavior contract: the mad-build observer
