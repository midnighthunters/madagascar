...
//! stay in each product host (for example `xai-mad-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `xai-mad-sampling-types`. It is decoupled from both Mad chat and
//! mad-build hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — mad-build's whole-session **full-replace**
//!   subsystem (prompt/summary/failure/config, assemble, orchestration).
//! - [`intra_compaction`] — Mad chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — Mad chat's chunked, between-turn pass.
//!
...
//! (active-agent-state `<system-reminder>` formatting shared by Mad chat and
//! mad-build; hosts still own snapshotting and host-only sections).
...
// mad-build's full-replace subsystem now lives under `code_compaction`;
