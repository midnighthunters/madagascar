...
//!
//! Behavior is byte-for-byte identical for Mad chat (`T = Arc<MadTurn>`).
...
///   summaries so their `<mad_user_queries>` sections can be split out.
...
/// Extracts **all** `<mad_user_queries>...</mad_user_queries>` blocks
...
pub fn split_prior_compaction_text(text: &str) -> (Option<String>, String) {
    let start_tag = "<mad_user_queries>";
    let end_tag = "</mad_user_queries>";
...
/// Extract a `<mad_user_queries>` XML block from `User` items in `turns`.
///
/// Walks `turns`, finds `User` items, and formats each as a `<mad_query>`
/// element with text content (from [`CompactionItem::text`]) and any
/// `<mad_file id="..." name="..." />` lines for the item's attachment
...
pub fn extract_user_queries_from_turns<T: CompactionItem>(
    ...
    let mut result = String::from("<mad_user_queries>\n");
    ...
    result.push_str("</mad_user_queries>");
    Some(result)
}

/// Walk `turns`, find any prior compaction summary items, extract their
/// `<mad_user_queries>` blocks via [`split_prior_compaction_text`], and
/// concatenate them.
///
...
pub struct SeparatedHistoryTurns<T> {
    /// `turns` with the `<mad_user_queries>` block stripped from every
    ...
    /// Concatenation of every `<mad_user_queries>` block found (in
    ...
    /// regardless of whether it contained a `<mad_user_queries>` block.
...
/// `<mad_user_queries>` block (preserved verbatim for the next summary)
...
///   loop, so the LLM never sees `<mad_user_queries>` from earlier rounds.
...
///   would see the prior `<mad_user_queries>` and tend to copy it into the
...
pub fn separate_prior_user_queries<T: CompactionItemBuilder>(
    ...
    for turn in turns {
        if turn.is_compaction_summary() {
            ...
            // `<mad_user_queries>` block (and therefore stripped to an empty
...
mod tests {
    ...
    fn extract_user_queries_wraps_single_user_turn() {
        ...
        assert!(out.starts_with("<mad_user_queries>"));
        assert!(out.ends_with("</mad_user_queries>"));
        assert!(out.contains("<mad_query>hello world</mad_query>"));
    }
    ...
    fn extract_prior_user_queries_concatenates_blocks() {
        let inner = "<mad_user_queries>\n<mad_query>first</mad_query>\n</mad_user_queries>";
        let inner2 = "<mad_user_queries>\n<mad_query>second</mad_query>\n</mad_user_queries>";
    ...
    fn separate_strips_user_queries_from_summary_item() {
        let prior = "<mad_user_queries>\n<mad_query>Q1</mad_query>\n</mad_user_queries>\n\n<chunk_summary index=\"0\">S1</chunk_summary>";
        ...
        assert!(prior.contains("<mad_user_queries>"));
        ...
        match &sep.turns_for_llm[0] {
            MockItem::Developer { text, summary } => {
                ...
                assert!(!text.contains("<mad_user_queries>"));
    ...
    fn separate_drops_summary_item_with_no_rest() {
        let only_queries = "<mad_user_queries>\n<mad_query>Q</mad_query>\n</mad_user_queries>";
