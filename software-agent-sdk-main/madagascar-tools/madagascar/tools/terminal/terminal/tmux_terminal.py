...
from madagascar.tools.terminal.env import (
    build_terminal_env,
    normalize_terminal_env,
)
from madagascar.tools.terminal.metadata import CmdOutputMetadata
from madagascar.tools.terminal.terminal import TerminalInterface
from madagascar.tools.terminal.terminal.interface import parse_ctrl_key
...
class TmuxTerminal(TerminalInterface):
    ...
    def initialize(self) -> None:
        ...
        # Use a dedicated socket to isolate Madagascar sessions from the user's tmux
        self.server = libtmux.Server(socket_name=TMUX_SOCKET_NAME, environment=env)
        _shell_command = "/bin/bash"
        if self.username in ["root", "madagascar"]:
        ...
        session_name = f"madagascar-{self.username}-{uuid.uuid4()}"
    ...
    def is_running(self) -> bool:
        ...
        try:
            ...
            from madagascar.tools.terminal.constants import CMD_OUTPUT_PS1_END
