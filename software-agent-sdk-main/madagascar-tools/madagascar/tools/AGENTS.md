...
- This directory (`madagascar-tools/madagascar/tools/`) contains runtime tool implementations under the `madagascar.tools.*` namespace.
...
- Treat `madagascar-tools/madagascar/tools/__init__.py` as the published surface for `madagascar-tools`; `__all__` is considered public API.
