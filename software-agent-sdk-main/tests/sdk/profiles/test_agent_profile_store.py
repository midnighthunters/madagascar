...
@pytest.fixture
def madagascar_profile() -> MadagascarAgentProfile:
...
def test_list_with_profiles(
    agent_store: AgentProfileStore, madagascar_profile: MadagascarAgentProfile
) -> None:
    agent_store.save(madagascar_profile)
    agent_store.save(madagascar_profile.model_copy(update={"name": "oh2"}))
...
def test_save_persists_id_inside_file(
    ...
    agent_store.save(madagascar_profile)
    data = json.loads((agent_store.base_dir / "oh.json").read_text())
    assert data["id"] == str(madagascar_profile.id)
...
def test_save_with_invalid_profile_name(
    ...
    profile = MadagascarAgentProfile(name=name, llm_profile_ref="default")
...
def test_save_writes_valid_json(
    ...
    agent_store.save(madagascar_profile)
    data = json.loads((agent_store.base_dir / "oh.json").read_text())
    assert data["agent_kind"] == "madagascar"
...
def test_save_overwrites_existing(
    ...
    agent_store.save(madagascar_profile)
    agent_store.save(madagascar_profile.model_copy(update={"llm_profile_ref": "other"}))

    loaded = agent_store.load("oh")
    assert isinstance(loaded, MadagascarAgentProfile)
...
def test_roundtrip_madagascar(
    ...
    assert isinstance(loaded, MadagascarAgentProfile)
    assert loaded.id == madagascar_profile.id
...
def test_rename_syncs_internal_name_and_preserves_id(
    ...
    agent_store.save(madagascar_profile)
    original_id = madagascar_profile.id
    agent_store.rename("oh", "renamed")
...
def test_rename_target_exists_raises(
    ...
    agent_store.save(madagascar_profile)
    agent_store.save(madagascar_profile.model_copy(update={"name": "taken"}))
...
def test_list_summaries_returns_fk_fields(
    agent_store: AgentProfileStore,
    madagascar_profile: MadagascarAgentProfile,
    ...
    oh = by_name["oh"]
    assert oh["id"] == str(madagascar_profile.id)
    assert oh["agent_kind"] == "madagascar"
...
def test_save_with_max_profiles_blocks_over_limit(
    ...
    agent_store.save(madagascar_profile.model_copy(update={"name": "a"}))
    agent_store.save(madagascar_profile.model_copy(update={"name": "b"}))

    with pytest.raises(ProfileLimitExceeded, match="2"):
        agent_store.save(
            madagascar_profile.model_copy(update={"name": "c"}), max_profiles=2
        )
...
def test_save_with_max_profiles_allows_overwrite(
    ...
    agent_store.save(madagascar_profile.model_copy(update={"name": "a"}), max_profiles=2)
...
def test_save_with_max_profiles_ignores_invalid_filenames(
    ...
    agent_store.save(madagascar_profile.model_copy(update={"name": "real"}))
    (agent_store.base_dir / ".hidden.json").write_text('{"name": "x"}')
    (agent_store.base_dir / "bad@name.json").write_text('{"name": "x"}')

    agent_store.save(
        madagascar_profile.model_copy(update={"name": "another"}), max_profiles=2
    )
...
def test_concurrent_reads_and_writes(tmp_path: Path) -> None:
    ...
    for i in range(5):
        store.save(
            MadagascarAgentProfile(name=f"profile_{i}", llm_profile_ref=f"llm_{i}")
        )
    ...
    def write_profile(index: int) -> None:
        try:
            store.save(
                MadagascarAgentProfile(name=f"new_profile_{index}", llm_profile_ref="x")
            )
...
def test_full_workflow(agent_store: AgentProfileStore) -> None:
    profile = MadagascarAgentProfile(name="wf", llm_profile_ref="default", revision=1)
