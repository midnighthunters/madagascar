...
- This directory (`madagascar-workspace/madagascar/workspace/`) contains workspace implementations under the `madagascar.workspace.*` namespace (Docker, Apptainer, cloud, and API-remote).
- Each backend lives in its own subpackage (e.g. `docker/`, `cloud/`) and typically exposes a `*Workspace` class from `workspace.py`.
- The published import surface is `madagascar-workspace/madagascar/workspace/__init__.py` (`__all__` is treated as public API). Keep imports lightweight so `import madagascar.workspace` does not pull in build-time dependencies.
- These classes should remain compatible with the SDK workspace interfaces and types (for example `madagascar.sdk.workspace.RemoteWorkspace`, `TargetType`, `PlatformType`).
