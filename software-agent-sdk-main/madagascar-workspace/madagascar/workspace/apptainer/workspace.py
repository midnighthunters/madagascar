...
from madagascar.sdk.utils.command import execute_command
from madagascar.sdk.workspace import PlatformType, RemoteWorkspace
from madagascar.workspace.docker.workspace import (
...
class ApptainerWorkspace(RemoteWorkspace):
    ...
    This workspace creates an Apptainer container running a pre-built Madagascar
    ...
    Example:
        with ApptainerWorkspace(
            server_image="ghcr.io/madagascar/agent-server:latest-python"
        ) as workspace:
    ...
    extra_bind_mounts: list[str] = Field(
        ...
        description=(
            ...
            "Use src[:dest[:opts]] syntax. MADAGASCAR_APPTAINER_EXTRA_BINDS can "
    ...
    def _start_container(self) -> None:
        ...
        env_extra_binds = [
            item.strip()
            for item in os.getenv("MADAGASCAR_APPTAINER_EXTRA_BINDS", "").split(",")
            if item.strip()
        ]
