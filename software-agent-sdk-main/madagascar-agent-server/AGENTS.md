...
When adding non-Python files (JS, templates, etc.) loaded at runtime, add them to `madagascar-agent-server/madagascar/agent_server/agent-server.spec` using `collect_data_files`.
...
(under [#3117](https://github.com/Madagascar/software-agent-sdk/issues/3117)).
...
3. Do **not** use `madagascar.sdk.utils.deprecation.deprecated` for FastAPI routes.
...
schema against the previous `madagascar-agent-server` release selected from PyPI,
...
- FastAPI route handlers must not use `madagascar.sdk.utils.deprecation.deprecated`.
