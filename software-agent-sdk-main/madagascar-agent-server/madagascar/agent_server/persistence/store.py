...
Following the same pattern as Madagascar app-server's FileSettingsStore
...
from madagascar.agent_server.persistence.models import (
    CustomSecret,
    PersistedSettings,
    PersistedWorkspaces,
    Secrets,
...
from madagascar.sdk.profiles.agent_profile_store import AgentProfileStore
...
DEFAULT_PERSISTENCE_DIR = Path("workspace/.madagascar")
...
def _get_persistence_dir(config: Config | None = None) -> Path:
    ...
    if config is not None:
        return config.conversations_path.parent / ".madagascar"
...
def _get_profile_persistence_dir() -> Path:
    ...
    ``OH_PERSISTENCE_DIR`` they fall back to the user's ``~/.madagascar``
...
def get_llm_profile_store() -> LLMProfileStore:
    ...
    when set, else ``~/.madagascar``. This honors isolated agent-server
