...
class ServerInfo(BaseModel):
    uptime: float
    idle_time: float
    title: str = "Madagascar Agent Server"

    version: str = Field(
        default_factory=lambda: _package_version("madagascar-agent-server")
    )
    sdk_version: str = Field(default_factory=lambda: _package_version("madagascar-sdk"))
    tools_version: str = Field(
        default_factory=lambda: _package_version("madagascar-tools")
    )
    workspace_version: str = Field(
        default_factory=lambda: _package_version("madagascar-workspace")
    )

    build_git_sha: str = Field(
        default_factory=lambda: os.environ.get("MADAGASCAR_BUILD_GIT_SHA", "unknown")
    )
    build_git_ref: str = Field(
        default_factory=lambda: os.environ.get("MADAGASCAR_BUILD_GIT_REF", "unknown")
    )
