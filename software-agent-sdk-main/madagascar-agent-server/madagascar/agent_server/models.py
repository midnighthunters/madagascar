...
from madagascar.sdk.conversation.request import (  # re-export for backward compat
...
from madagascar.sdk.conversation.types import ConversationTags
from madagascar.sdk.event.base import Event
from madagascar.sdk.hooks import HookConfig
from madagascar.sdk.llm.message import (  # re-export
...
from madagascar.sdk.utils import MadagascarUUID, utc_now
...
class StoredConversation(StartConversationRequest):
    ...
    forked_from_conversation_id: MadagascarUUID | None = Field(
        default=None,
        description=(
...
class _ConversationInfoBase(BaseModel):
    ...
    created_at: datetime = Field(default_factory=utc_now)
    updated_at: datetime = Field(default_factory=utc_now)
    # Plain ``UUID`` (not ``MadagascarUUID``) so it JSON-serializes dashed, exactly
    ...
    current_model_id: str | None = Field(
        ...
        description=(
            ...
            "still initializing. Native Madagascar agents leave this ``None`` — "
    ...
    available_models: list[ACPModelInfo] = Field(
        ...
        description=(
            ...
            "the (UNSTABLE) capability and for native Madagascar agents. "
    ...
    supports_runtime_model_switch: bool = Field(
        ...
        description=(
            ...
            "``False`` for native "
            "Madagascar agents, for a known provider that declares no support, "
...
def trim_conversation_response_skills(info: ConversationInfo) -> ConversationInfo:
    ...
    actually reads from the HTTP response (agent-canvas, Madagascar
...
class EventPage(MadagascarModel):
    items: list[Event]
    next_page_id: str | None = None
...
class BashEventPage(MadagascarModel):
