...
The Madagascar Agent Server is a minimal REST API and WebSocket server that provides a programmatic interface for interacting with Madagascar AI agents. It uses the local filesystem to store conversations, events, and workspace files, making it ideal for development, testing, and lightweight deployments.
...
uv run python -m madagascar.agent_server

# Start with custom host and port
uv run python -m madagascar.agent_server --host localhost --port 3000

# Start with auto-reload (for dev)
uv run python -m madagascar.agent_server --reload
```
...
|----------|-------------|---------|
| `MADAGASCAR_AGENT_SERVER_CONFIG_PATH` | Path to JSON configuration file | `workspace/madagascar_agent_server_config.json` |
...
Create a JSON configuration file (default: `workspace/madagascar_agent_server_config.json`):
...
```
workspace/
├── madagascar_agent_server_config.json    # Configuration file
...
# Run with coverage
uv run pytest tests/agent_server/ --cov=madagascar.agent_server
```
...
3. **Configuration not found**: Check the `MADAGASCAR_AGENT_SERVER_CONFIG_PATH` environment variable
