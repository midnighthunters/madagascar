...
MADAGASCAR_BASE_URL = os.environ.get("MADAGASCAR_BASE_URL", "https://app.all-hands.dev")
...
MADAGASCAR_SENSITIVE_KEYS = frozenset({"session_api_key"})


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Start an Madagascar Cloud conversation that checks a GitHub issue "
            "for duplicates."
        )
    )
    parser.add_argument(
...
def github_headers() -> dict[str, str]:
    headers = {
        ...
        "User-Agent": "madagascar-issue-duplicate-check",
...
def madagascar_headers() -> dict[str, str]:
    api_key = os.environ.get("MADAGASCAR_API_KEY")
    if not api_key:
        raise RuntimeError("MADAGASCAR_API_KEY environment variable is required")
    return {
...
def summarize_madagascar_item(item: dict[str, Any]) -> str:
    summary = {}
    for key in MADAGASCAR_DEBUG_KEYS:
        if key not in item:
            continue
        value = item[key]
        if value in (None, "", [], {}):
    ...
    available_keys = sorted(
        key
        for key in item
        if key not in summary and key not in MADAGASCAR_SENSITIVE_KEYS
    )
    if available_keys:
        summary["available_keys"] = available_keys
    sensitive_keys_present = sorted(
        key for key in item if key in MADAGASCAR_SENSITIVE_KEYS
    )
...
def poll_start_task(
    ...
    while time.time() < deadline:
        payload = request_json(
            ...
            headers={"Authorization": madagascar_headers()["Authorization"]},
        )
        ...
        if status in {"ERROR", "FAILED"}:
            raise RuntimeError(
                f"Madagascar start task failed: {summarize_madagascar_item(item)}"
            )
        time.sleep(poll_interval_seconds)
    raise TimeoutError(
...
def poll_conversation(
    ...
    while time.time() < deadline:
        ...
        if execution_status in FAILED_EXECUTION_STATUSES:
            raise RuntimeError(
                "Madagascar conversation ended with "
                f"{execution_status}: {summarize_madagascar_item(item)}"
            )
...
def main() -> int:
    ...
    if not app_conversation_id:
        ...
        if not task_id:
            raise RuntimeError(
                ...
                f"{summarize_madagascar_item(start_task)}"
            )
        ...
        if not app_conversation_id:
            raise RuntimeError(
                ...
                f"{summarize_madagascar_item(ready_task)}"
            )
    ...
    conversation_url = (
        ...
        or f"{MADAGASCAR_BASE_URL}/conversations/{app_conversation_id}"
    )
    ...
    if session_api_key_value and not isinstance(session_api_key_value, str):
        raise RuntimeError(
            "session_api_key had unexpected type in the Madagascar conversation: "
        ...
        try:
            ...
            if not session_api_key:
                raise RuntimeError(
                    ...
                    "session_api_key was missing from the Madagascar conversation"
                ) from exc
