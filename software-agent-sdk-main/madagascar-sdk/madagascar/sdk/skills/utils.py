...
from madagascar.sdk.git.cached_repo import GitHelper, try_cached_clone_or_update
...
if TYPE_CHECKING:
    from madagascar.sdk.skills.skill import Skill, SkillResources
...
def load_and_categorize(
    ...
    from madagascar.sdk.skills.skill import Skill

    skill = Skill.load(path, skill_base_dir)

    # AgentSkills (SKILL.md directories) are a separate category from Madagascar skills.
...
def get_skills_cache_dir() -> Path:
    ...
    Returns:
        Path to the skills cache directory (~/.madagascar/cache/skills).
    """
    cache_dir = Path.home() / ".madagascar" / "cache" / "skills"
...
def update_skills_repository(
    ...
    Uses the shared git caching infrastructure from madagascar.sdk.git.cached_repo.
...
def discover_skill_resources(skill_dir: Path) -> SkillResources:
    ...
    """
    # Import here to avoid circular dependency
    from madagascar.sdk.skills.skill import SkillResources
