...
from madagascar.sdk.skills.exceptions import SkillError, SkillValidationError
from madagascar.sdk.skills.execute import render_content_with_commands
from madagascar.sdk.skills.trigger import (
    KeywordTrigger,
    PathTrigger,
    TaskTrigger,
)
...
from madagascar.sdk.utils import DEFAULT_TRUNCATE_NOTICE, maybe_truncate
...
class Skill(BaseModel):
    ...
    This model supports both Madagascar-specific fields and AgentSkills standard
    ...
    def load(
        ...
        Supports both Madagascar-specific frontmatter fields and AgentSkills
        ...
        if path.name.lower() == "skill.md":
            ...
            return cls._load_legacy_madagascar_skill(path, file_content, skill_base_dir)
    ...
    @classmethod
    def _load_legacy_madagascar_skill(
        cls, path: Path, file_content: str, skill_base_dir: Path | None
    ) -> "Skill":
        """Load a skill from a legacy Madagascar-format file.
...
def load_skills_from_dir(
    ...
    - Madagascar format: skills/*.md files
    - AgentSkills format: skills/skill-name/SKILL.md directories

    Note, legacy repo instructions will not be loaded here.

    Args:
        skill_dir: Path to the skills directory (e.g. .madagascar/skills)
...
USER_SKILLS_DIRS = [
    ...
    Path.home() / ".madagascar" / "skills",
    Path.home() / ".madagascar" / "microagents",  # Legacy support
]
...
def load_user_skills() -> list[Skill]:
    ...
    Searches for skills in ~/.agents/skills/, ~/.madagascar/skills/, and
    ~/.madagascar/microagents/ (legacy). Skills from all directories are merged,
    with earlier entries in USER_SKILLS_DIRS taking precedence for duplicate
    names.

    Also loads enabled installed skills from ~/.madagascar/skills/installed/
    ...
    try:
        from madagascar.sdk.skills.installed import load_installed_skills
...
def load_project_skills(work_dir: str | Path) -> list[Skill]:
    ...
    Searches for skills in {work_dir}/.agents/skills/, {work_dir}/.madagascar/skills/,
    and {work_dir}/.madagascar/microagents/ (legacy).
    ...
    Use .agents/skills for new skills. .madagascar/skills is the legacy Madagascar
    location, and .madagascar/microagents is deprecated.

    Example: If "my-skill" exists in both .agents/skills/ and .madagascar/skills/,
    ...
    # search root. This ensures they are loaded even if .madagascar/skills doesn't
    # exist.
    ...
    # Load project-specific skills from .agents/skills, .madagascar/skills,
    # and legacy microagents (priority order; first wins for duplicates)
    for root in search_roots:
        project_skills_dirs = [
            root / ".agents" / "skills",
            root / ".madagascar" / "skills",
            root / ".madagascar" / "microagents",  # Legacy support
        ]
...
PUBLIC_SKILLS_REPO = "https://github.com/Madagascar/extensions"
...
def load_marketplace_skill_names(
    ...
    Uses the existing Marketplace model from madagascar.sdk.plugin to parse
...
def load_public_skills(
    ...
    """Load skills from the public Madagascar skills repository.

    This function maintains a local git clone of the public skills registry at
    https://github.com/Madagascar/extensions. On first run, it clones the repository
    to ~/.madagascar/skills-cache/. On subsequent runs within the same process, it
    ...
    Example:
        ...
        >>> from madagascar.sdk.skills import load_public_skills
        >>>
        >>> # Load public skills
...
def load_available_skills(
    ...
    *,
    ...
    Args:
        ...
        include_public: Load public skills from the Madagascar extensions repo.
