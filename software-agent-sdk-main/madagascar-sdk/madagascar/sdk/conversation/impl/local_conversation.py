...
from madagascar.sdk.conversation.secret_registry import SecretValue
from madagascar.sdk.conversation.state import (
    ConversationExecutionStatus,
    ConversationState,
)
from madagascar.sdk.conversation.stuck_detector import StuckDetector
from madagascar.sdk.conversation.title_utils import generate_conversation_title
...
from madagascar.sdk.hooks import HookConfig, HookEventProcessor, create_hook_callback
from madagascar.sdk.io import FileStore, LocalFileStore
from madagascar.sdk.llm import LLM, Message, TextContent, content_to_str
...
from madagascar.sdk.llm.llm_registry import LLMRegistry
from madagascar.sdk.logger import get_logger
from madagascar.sdk.marketplace.registry import MarketplaceRegistry
from madagascar.sdk.mcp.config import MCPServer, coerce_mcp_config, dump_mcp_config
from madagascar.sdk.mcp.utils import DefaultMCPToolProvider, MCPToolProvider
...
from madagascar.sdk.skills import load_available_skills, merge_skills_by_name
...
from madagascar.sdk.tool.builtins import InvokeSkillTool
...
class LocalConversation(BaseConversation):
    ...
    def __init__(
        ...
        **_: object,
        ...
        if resolved_client_tools:
            from madagascar.sdk.tool.client_tool import register_client_tools
    ...
    def _recover_persisted_client_tools(
        ...
        from madagascar.sdk.tool.client_tool import extract_client_tool_specs
    ...
    def _runtime_mcp_tools_for_agent(self) -> list[ToolDefinition]:
        if not self.agent.supports_madagascar_tools or not self.agent.mcp_config:
            return []
    ...
    def _register_file_based_agents(self) -> None:
        ...
        Registration order (highest to lowest priority):
          ...
          3. Project-level file agents (`{project}/.agents/agents/*.md`,
                then `{project}/.madagascar/agents/*.md`)
          4. User-level file agents (`~/.agents/agents/*.md`,
                then `~/.madagascar/agents/*.md`)
        """
    ...
    def _ensure_agent_ready(self) -> None:
        ...
        with self._state:
            ...
            try:
                if self.agent.supports_madagascar_tools:
    ...
    def switch_acp_model(self, model: str) -> None:
        ...
        Unlike :meth:`switch_llm`, which swaps Madagascar' own LLM object, this
    ...
    def interrupt(self) -> None:
        ...
        :class:`~madagascar.sdk.event.InterruptEvent`.
