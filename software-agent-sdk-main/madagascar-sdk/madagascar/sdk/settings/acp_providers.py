...
Callers outside the SDK (e.g. ``madagascar-agent-server``, the ``Madagascar``
frontend, and the ``@madagascar/typescript-client`` mirror) can import
...
class ACPFileSecretSpec(BaseModel):
    ...
    :attr:`secret_name` — and :class:`~madagascar.sdk.agent.ACPAgent` materialises
    ...
    :attr:`~madagascar.sdk.agent.ACPAgent.acp_file_secrets` to support other ACP
...
class ACPProviderInfo:
    ...
    pointing at it) that :class:`~madagascar.sdk.agent.ACPAgent` materialises
    ...
    :meth:`~madagascar.sdk.settings.model.ACPAgentSettings.resolve_acp_command`
    ...
    :attr:`~madagascar.sdk.agent.ACPAgent.acp_isolate_data_dir`.
    """
...
# madagascar-agent-server/madagascar/agent_server/docker/Dockerfile — a bump must
...
def default_acp_file_secrets() -> tuple[ACPFileSecretSpec, ...]:
    ...
    :attr:`~madagascar.sdk.agent.ACPAgent.acp_file_secrets`, which a downstream
