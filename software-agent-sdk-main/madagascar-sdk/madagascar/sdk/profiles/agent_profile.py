...
:data:`~madagascar.sdk.settings.model.AgentSettingsConfig`: the profile carries
...
class ProfileVerificationSettings(BaseModel):
    ...
    :class:`~madagascar.sdk.settings.model.VerificationSettings`: ``critic_api_key``
...
class AgentProfileBase(BaseModel):
    ...
    (or an Madagascar payload carrying ``acp_*``) is rejected rather than
    ...
    # selects them by *exclusion* (``MadagascarAgentProfile.disabled_skills``, a
...
class MadagascarAgentProfile(AgentProfileBase):
    """``agent_kind="madagascar"`` profile — references an LLM profile by name.

    Mirrors the configurable surface of
    :class:`~madagascar.sdk.settings.model.MadagascarAgentSettings`, except the
    ...
    agent_kind: Literal["madagascar"] = Field(
        default="madagascar",
        description=(
            "Discriminator for the ``AgentProfile`` union. ``'madagascar'`` "
            "selects the standard built-in Madagascar agent."
        ),
    )
...
class ACPAgentProfile(AgentProfileBase):
    ...
    :data:`~madagascar.sdk.settings.acp_providers.ACP_PROVIDERS` registry. The
...
def _agent_profile_discriminator(value: Any) -> str:
    """Discriminator for :data:`AgentProfile` — defaults to ``'madagascar'``.

    A payload without an explicit ``agent_kind`` is treated as the standard
    Madagascar variant, mirroring
    :func:`~madagascar.sdk.settings.model._agent_settings_discriminator`.
    """
...
AgentProfile = Annotated[
    Annotated[MadagascarAgentProfile, Tag("madagascar")]
...
def _migrate_v1_to_v2(payload: dict[str, Any]) -> dict[str, Any]:
    if (
        payload.get("agent_kind", "madagascar") == "madagascar"
...
_AGENT_PROFILE_ADAPTER: TypeAdapter[MadagascarAgentProfile | ACPAgentProfile] = (
...
def validate_agent_profile(
    ...
    *,
...
) -> MadagascarAgentProfile | ACPAgentProfile:
    ...
    """
    if isinstance(data, MadagascarAgentProfile | ACPAgentProfile):
        return data
