...
An ``MadagascarAgentProfile.llm_profile_ref`` is a soft FK onto an LLM-profile
...
:class:`~madagascar.sdk.profiles.agent_profile_store.AgentProfileStoreProtocol`,
...
from madagascar.sdk.profiles.agent_profile_store import PROFILE_NAME_REGEX


if TYPE_CHECKING:
    from madagascar.sdk.llm.llm_profile_store import LLMProfileMutator
    from madagascar.sdk.profiles.agent_profile_store import AgentProfileStoreProtocol
...
def _scan_referrers(
    ...
    instantiation). Only the Madagascar variant carries ``llm_profile_ref``.
    """
    return [
        str(summary["name"])
        for summary in store.list_summaries()
        if summary.get("agent_kind", "madagascar") == "madagascar"
