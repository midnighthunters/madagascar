...
class HookConfig(BaseModel):
    ...
    Hooks can be configured either by loading from `.madagascar/hooks.json` or
    by directly instantiating with typed fields:
        ...
        # Load from JSON file:
        config = HookConfig.load(".madagascar/hooks.json")
    """
    ...
    def load(
        ...
        """Load config from path or search .madagascar/hooks.json locations.

        Args:
            path: Explicit path to hooks.json file. If provided, working_dir is ignored.
            working_dir: Project directory for discovering .madagascar/hooks.json.
        ...
        if path is None:
            ...
            search_paths = [
                base_dir / ".madagascar" / "hooks.json",
                Path.home() / ".madagascar" / "hooks.json",
            ]
