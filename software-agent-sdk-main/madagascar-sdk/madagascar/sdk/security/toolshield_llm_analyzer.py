...
Example (bare guardrail):
    ...
    >>> from madagascar.sdk.security import ToolShieldLLMSecurityAnalyzer
...
Example (with ToolShield tool-specific safety experiences):
    Install the optional extra ``pip install madagascar-sdk[toolshield]``
...
# per the actor's system prompt (see madagascar.sdk.event.llm_convertible.
...
class ToolShieldLLMSecurityAnalyzer(SecurityAnalyzerBase):
    ...
    ``pip install madagascar-sdk[toolshield]`` and pass the rendered
    ...
    safety_experiences: str = Field(
        ...
        description=(
            ...
            "(``pip install madagascar-sdk[toolshield]``). Callers with "
