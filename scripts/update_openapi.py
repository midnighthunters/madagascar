#!/usr/bin/env python3
"""
Update Madagascar OpenAPI documentation.
...
Behavior:
- Uses madagascar.app_server.app.app.openapi() to build the spec.
- Preserves existing "servers" from docs/openapi.json if present; otherwise
  writes sensible defaults.
- Sets info.version to madagascar.__version__.
...
os.environ['MADAGASCAR_LOG_LEVEL'] = 'CRITICAL'
...
try:
    from madagascar import __version__
    from madagascar.app_server.app import app
except ImportError as e:
    print(f'Error importing Madagascar modules: {e}')
    print(
