...
describe("useAppTitle", () => {
  ...
  it("should return 'Madagascar' if is OSS and NOT in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() => expect(result.current).toBe("Madagascar"));
  });

  it("should return 'Madagascar Cloud' if is SaaS and NOT in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    getConfigSpy.mockResolvedValue({
      app_mode: "saas",
    });
    ...
    await waitFor(() => expect(result.current).toBe("Madagascar Cloud"));
  });

  it("should return '{some title} | Madagascar' if is OSS and in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() =>
      expect(result.current).toBe("My Conversation | Madagascar"),
    );
  });

  it("should return '{some title} | Madagascar Cloud' if is SaaS and in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() =>
      expect(result.current).toBe(
        "Another Conversation Title | Madagascar Cloud",
      ),
    );
  });
