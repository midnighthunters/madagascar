 ...
 */
function groupEventsByPhase(events: MadagascarEvent[]): MadagascarEvent[][] {
  const phases: MadagascarEvent[][] = [];
  let currentPhase: MadagascarEvent[] = [];
 ...
 */
export function usePlanPreviewEvents(allEvents: MadagascarEvent[]): Set<string> {
  return useMemo(() => {
