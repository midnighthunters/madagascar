...
`uv` must already have the SDK workspace dependencies available. The desktop runtime deliberately uses `uv run --offline --project agentcore --package madagascar-agent-server`; it never resolves a published package or requires Docker.
