...
from madagascar.agent_server.openai.models import (
    OpenAIChatCompletionRequest,
    OpenAIChatCompletionResponse,
    OpenAIModelListResponse,
)
from madagascar.agent_server.openai.service import (
...
def check_openai_api_key(
    ...
    """Accept the same session key through Madagascar and OpenAI auth shapes.
...
def _parse_observability_overrides(
    *,
    ...
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail="X-Madagascar-Observability-Metadata must be a JSON object",
        ) from exc
...
async def create_chat_completion(
    ...
    request: Request,
    response: Response,
    x_madagascar_server_conversation_id: Annotated[
        UUID | None, Header(alias="X-Madagascar-ServerConversation-ID")
    ] = None,
    x_madagascar_observability_span_name: Annotated[
        str | None, Header(alias="X-Madagascar-Observability-Span-Name")
    ] = None,
    x_madagascar_observability_tags: Annotated[
        str | None, Header(alias="X-Madagascar-Observability-Tags")
    ] = None,
    x_madagascar_observability_metadata: Annotated[
        str | None, Header(alias="X-Madagascar-Observability-Metadata")
    ] = None,
    ...
    result = await run_chat_completion(
        ...
        reusable_conversation_id=x_madagascar_server_conversation_id,
        observability_overrides=_parse_observability_overrides(
            span_name=x_madagascar_observability_span_name,
            tags=x_madagascar_observability_tags,
            metadata=x_madagascar_observability_metadata,
        ),
    )
    ...
    response.headers["X-Madagascar-ServerConversation-ID"] = conversation_id
