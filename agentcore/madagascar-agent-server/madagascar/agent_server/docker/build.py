...
from madagascar.sdk.logger import IN_CI, get_logger, rolling_log_view
from madagascar.sdk.workspace import PlatformType, TargetType
...
def _default_sdk_project_root() -> Path:
    """
    Resolve top-level Madagascar UV workspace root:
    ...
    def _is_workspace_root(d: Path) -> bool:
        ...
        _EXPECTED = (
            "madagascar-sdk/pyproject.toml",
            "madagascar-tools/pyproject.toml",
            "madagascar-workspace/pyproject.toml",
            "madagascar-agent-server/pyproject.toml",
        )
    ...
    def validate(p: Path, src: str) -> Path:
        ...
        if not _is_workspace_root(root):
            raise RuntimeError(
                f"{src}: couldn't find the Madagascar UV workspace root "
                f"starting at '{p}'.\n\n"
    ...
    raise RuntimeError(
        "Could not resolve the Madagascar UV workspace root.\n\n"
        "Expected repo layout:\n"
        "  pyproject.toml  (with [tool.uv.workspace].members "
        "including madagascar/* subprojects)\n"
        "  madagascar-sdk/pyproject.toml\n"
        "  madagascar-tools/pyproject.toml\n"
        "  madagascar-workspace/pyproject.toml\n"
        "  madagascar-agent-server/pyproject.toml\n\n"
...
class BuildOptions(BaseModel):
    ...
    image: str = Field(default="ghcr.io/madagascar/agent-server")
    ...
    sdk_project_root: Path = Field(
        ...
        description="Path to Madagascar SDK root. Auto if None.",
    )
...
def _default_local_cache_dir() -> Path:
    ...
    return Path(xdg) / "madagascar" / "buildx-cache"
...
def build_with_telemetry(opts: BuildOptions) -> BuildResult:
    ...
    args = [
        ...
        "--build-arg",
        f"MADAGASCAR_BUILD_GIT_SHA={opts.git_sha}",
        "--build-arg",
        f"MADAGASCAR_BUILD_GIT_REF={opts.git_ref}",
    ]
    if push:
    ...
    cache_export_mode = os.environ.get("MADAGASCAR_BUILDKIT_CACHE_MODE", "max").lower()
    if cache_export_mode not in ("off", "max", "min"):
        logger.warning(
            f"[build] Invalid MADAGASCAR_BUILDKIT_CACHE_MODE='{cache_export_mode}', "
            "defaulting to 'max'"
        )
    ...
    if push:
        ...
        if driver == "docker-container":
            ...
            logger.warning(
                ...
                "  1. docker buildx create --name madagascar-builder "
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--image",
        default=_env("IMAGE", "ghcr.io/madagascar/agent-server"),
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to Madagascar SDK root (default: auto-detect).",
    )
    parser.add_argument(
        "--prebuilt-sdist",
        type=Path,
