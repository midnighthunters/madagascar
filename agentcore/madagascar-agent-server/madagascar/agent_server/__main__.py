...
from madagascar.agent_server.logging_config import LOGGING_CONFIG
...
def check_browser():
    ...
    try:
        ...
        from madagascar.tools.preset.default import register_default_tools

        register_default_tools(enable_browser=True)

        # Import browser components
        from madagascar.tools.browser_use.definition import BrowserNavigateAction
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Madagascar Agent Server App")
    parser.add_argument(
    ...
    print(f"Starting Madagascar Agent Server on {args.host}:{args.port}")
    ...
    config = Config(
        "madagascar.agent_server.api:api",
        host=args.host,
        port=args.port,
        reload=args.reload,
        reload_includes=[
