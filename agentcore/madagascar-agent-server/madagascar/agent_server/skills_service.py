"""Skills service for Madagascar Agent Server.
...
Skill Sources:
- Public skills: registered marketplace plugins or GitHub Madagascar/extensions
- User skills: ~/.madagascar/skills/ and ~/.madagascar/microagents/
- Project skills: {workspace}/.madagascar/skills/, .cursorrules, agents.md
- Organization skills: {org}/.madagascar or {org}/madagascar-config
...
from madagascar.sdk.utils import sanitized_env
...
def load_org_skills_from_url(
    ...
    # the same org_name (e.g. {login}/.madagascar and {login}/.agents), and
    ...
    if working_dir:
        ...
        temp_dir = Path(tempfile.mkdtemp(prefix=f"madagascar_org_skills_{org_name}_"))
...
def discover_profile_skills() -> list[Skill]:
    """Skill catalog an Madagascar profile launches with (#4017).
...
def sync_public_skills() -> tuple[bool, str]:
    ...
    the latest skills from the Madagascar/extensions repository.
...
def service_install_skill(
    ...
    Args:
        source: Skill source - git URL, GitHub shorthand, or local path.
            ...
            - GitHub URL: https://github.com/Madagascar/extensions/tree/main/skills/github
            - GitHub shorthand: github:Madagascar/extensions/skills/github
        ...
        installed_dir: Directory for installed skills.
            Defaults to ~/.madagascar/skills/installed/.
