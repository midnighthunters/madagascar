...
#   MADAGASCAR_PROJECT_DIR - Project directory
#   MADAGASCAR_SESSION_ID - Session ID
#   GITHUB_TOKEN - GitHub API token (if available)

set -o pipefail

PROJECT_DIR="${MADAGASCAR_PROJECT_DIR:-$(pwd)}"
...
if [ -n "$CHANGED_FILES" ]; then
    ...
    while IFS= read -r file; do
        case "$file" in
            madagascar-sdk/*) add_project "tests/sdk" ;;
            madagascar-tools/*) add_project "tests/tools" ;;
            madagascar-workspace/*) add_project "tests/workspace" ;;
            madagascar-agent-server/*) add_project "tests/agent_server" ;;
