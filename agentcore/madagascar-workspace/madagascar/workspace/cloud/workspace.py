"""Madagascar Cloud workspace implementation using Cloud API."""
...
from madagascar.sdk.mcp.config import MCPServer, MCPTransport
from madagascar.sdk.workspace.remote.base import RemoteWorkspace
from madagascar.sdk.workspace.repo import CloneResult, RepoMapping, RepoSource


if TYPE_CHECKING:
    from madagascar.sdk.context import AgentContext
    from madagascar.sdk.llm.llm import LLM
    from madagascar.sdk.secret import LookupSecret
    from madagascar.sdk.skills import Skill


logger = get_logger(__name__)

# Standard exposed URL names from Madagascar Cloud
...
class MadagascarCloudWorkspace(RemoteWorkspace):
    """Remote workspace using Madagascar Cloud API.

    This workspace connects to Madagascar Cloud (app.all-hands.dev) to provision
    ...
    running inside an Madagascar Cloud Runtime sandbox.  Instead of creating or
    ...
    Example:
        ...
        workspace = MadagascarCloudWorkspace(
            cloud_api_url="https://app.all-hands.dev",
            cloud_api_key="your-api-key",
            sandbox_spec_id="ghcr.io/madagascar/agent-server:main-python",
        )

        # Running inside an Madagascar Cloud Runtime (local agent-server mode)
        workspace = MadagascarCloudWorkspace(
            local_agent_server_mode=True,
            cloud_api_url="https://app.all-hands.dev",
            cloud_api_key=os.environ["MADAGASCAR_API_KEY"],
        )
    """
    ...
    local_agent_server_mode: bool = Field(
        default=False,
        description=(
            "When True, assume the SDK is running inside an Madagascar Cloud "
    ...
    cloud_api_url: str = Field(
        description=(
            "Base URL of Madagascar Cloud API "
    ...
    cloud_api_key: str = Field(
        description=(
            "API key for authenticating with Madagascar Cloud. "
            "Required in all modes — used for get_llms / get_secrets."
        ),
    )
    ...
    def default_conversation_tags(self) -> dict[str, str]:
        ...
        When running inside an Madagascar Cloud Runtime (local_agent_server_mode=True),
    ...
    def _api_headers(self) -> dict[str, str]:
        ...
        Uses Bearer token authentication as per Madagascar Cloud API.
        """
    ...
    def _create_new_sandbox(self) -> None:
        ...
        logger.info("Starting sandbox via Madagascar Cloud API...")
    ...
    def pause(self) -> None:
        ...
        Note: Madagascar Cloud does not currently support pausing sandboxes.
        ...
        raise NotImplementedError(
            "MadagascarCloudWorkspace.pause() is not yet supported - "
    ...
    def get_llm(self, profile_name: str | None = None, **llm_kwargs: Any) -> LLM:
        ...
        Example:
            >>> with MadagascarCloudWorkspace(...) as workspace:
    ...
    def __enter__(self) -> MadagascarCloudWorkspace:
        return self
