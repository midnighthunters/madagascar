...
Example usage:
    from madagascar.sdk import register_agent, Agent, AgentContext
...
def agent_definition_to_factory(
    ...
    if agent_def.skills:
        from madagascar.sdk.skills import load_available_skills
...
def register_file_agents(work_dir: str | Path) -> list[str]:
    ...
    `.madagascar/agents`, and user-level `~/.agents/agents` and `~/.madagascar/agents`
    directories.

    Project-level definitions take priority over user-level ones, and within
    each level `.agents/` takes priority over `.madagascar/`.
