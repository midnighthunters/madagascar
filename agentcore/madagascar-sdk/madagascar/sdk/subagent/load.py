...
`.agents/agents` or `.madagascar/agents` directories at the project or user level.
...
Directory convention (in priority order):
    ...
    ~/.madagascar/               # User-level, legacy (lowest file priority)
        agents/
            my-global-agent.md
...
Priority (highest to lowest):
  ...
  4. Project-level `.madagascar/agents/*.md`
  5. User-level `~/.agents/agents/*.md`
  6. User-level `~/.madagascar/agents/*.md`
"""
...
def load_project_agents(project_dir: str | Path) -> list[AgentDefinition]:
    ...
    Searches for
        ...
        - project_dir/.madagascar/agents (in that order).
...
def load_user_agents() -> list[AgentDefinition]:
    ...
    Searches for
        - ~/.agents/agents and
        - ~/.madagascar/agents (in that order).
    Note that `.agents/agents` definitions take precedence for duplicate names.
...
def discover_agents(
    ...
    *,
    ...
    ``madagascar-tools`` (not an SDK dependency), so callers wanting them add
