...
This package (`madagascar.sdk.subagent`) centralizes **subagent discovery** and **registration**.
...
2. `{project}/.madagascar/agents/*.md`

**User-level:**

3. `~/.agents/agents/*.md`
4. `~/.madagascar/agents/*.md`
...
3. **Project** file-based agents
   - `{project}/.agents/agents/*.md` then `{project}/.madagascar/agents/*.md`
4. **User** file-based agents
   - `~/.agents/agents/*.md` then `~/.madagascar/agents/*.md`
...
1. **Within a level** (`load_project_agents` / `load_user_agents`):
   - `.agents/agents` wins over `.madagascar/agents` for the same agent name.
...
- Docs PR tracking this feature: https://github.com/Madagascar/docs/pull/358
