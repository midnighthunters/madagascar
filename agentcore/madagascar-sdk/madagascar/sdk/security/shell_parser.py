...
- ``madagascar-tools`` for command splitting and escape doubling
  (``terminal.utils.command``).
- ``madagascar-sdk`` for AST-aware security detection
  (the planned ``security.defense_in_depth.*`` analyzers).

Hosting the parser inside ``madagascar-sdk`` lets both consumers depend
...
``madagascar-tools/madagascar/tools/terminal/utils/command.py``: sharing
