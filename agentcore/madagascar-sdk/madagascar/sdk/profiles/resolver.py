...
:data:`~madagascar.sdk.settings.model.AgentSettingsConfig` embeds the resolved
...
if TYPE_CHECKING:
    ...
    from madagascar.sdk.llm.llm_profile_store import LLMProfileLoader
...
class AgentProfileDiagnostics(BaseModel):
    ...
    # Skill selection (Madagascar only). ``disabled_skills`` is a deny-list over
...
def _build_madagascar_settings(
    profile: MadagascarAgentProfile,
    llm: LLM,
    mcp_config: dict[str, MCPServer],
    filtered_skills: list[Skill],
) -> AgentSettingsConfig:
    """Compose the resolved ``MadagascarAgentSettings`` from a profile + LLM.
...
def resolve_agent_profile(
    profile: MadagascarAgentProfile | ACPAgentProfile,
    *,
    ...
    Madagascar profile keeps all of it except the names in ``disabled_skills``.
    ...
    Raises:
        ProfileNotFound: ``llm_profile_ref`` does not exist (Madagascar path).
    ...
    if isinstance(profile, MadagascarAgentProfile):
        ...
        return _build_madagascar_settings(profile, llm, filtered_mcp, filtered_skills)
...
def resolve_agent_profile_dry_run(
    ...
    *,
    ...
    # Skill selection report (Madagascar only; ACP injects no user/public skills).
    ...
    if diagnostics.valid:
        ...
        try:
            if isinstance(profile, MadagascarAgentProfile):
                ...
                if llm is None:
                    raise RuntimeError(
                        "Madagascar profile marked valid without a resolved LLM"
                    )
                settings = _build_madagascar_settings(
