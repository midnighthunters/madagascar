"""Canonical default tool names for the standard Madagascar agent.
...
the canonical defaults here lets ``madagascar-sdk`` (which must not import
``madagascar-tools``) default a toolset from data alone — ``Tool`` is a spec
(name + params) resolved to an implementation only at runtime via the registry.

``madagascar.tools.preset.default.get_default_tools`` remains the constructor
...
"""Names of the standard exec tools every default Madagascar agent gets."""
...
def default_tool_specs(
    *,
    ...
    """Default tool specs for an Madagascar agent whose settings carry no tools.
