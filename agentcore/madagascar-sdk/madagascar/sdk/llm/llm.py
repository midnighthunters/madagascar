...
from madagascar.sdk.llm.utils.model_info import get_litellm_model_info
from madagascar.sdk.settings.metadata import SettingProminence, field_meta
from madagascar.sdk.utils.pydantic_secrets import serialize_secret, validate_secret


if TYPE_CHECKING:  # type hints only, avoid runtime import cycle
    from madagascar.sdk.llm.auth import SupportedVendor
    from madagascar.sdk.llm.auth.openai import OpenAIAuthMethod
    from madagascar.sdk.tool.tool import ToolDefinition

from madagascar.sdk.llm.auth.openai import transform_for_subscription
...
from madagascar.sdk.llm.mixins.non_native_fc import NonNativeToolCallingMixin
...
from madagascar.sdk.llm.utils.litellm_provider import infer_litellm_provider
...
from madagascar.sdk.llm.utils.retry_mixin import RetryMixin
...
from madagascar.sdk.logger import ENV_LOG_DIR, get_logger
...
# Minimum context window size required for Madagascar to function properly.
...
class LLM(BaseModel, RetryMixin, NonNativeToolCallingMixin):
    """Language model interface for Madagascar agents.
    ...
    model_canonical_name: str | None = Field(
        ...
        description=(
            ...
            "The Madagascar SDK maintains a model feature registry that "
    ...
    def _validate_context_window_size(self) -> None:
        """Validate that the context window is large enough for Madagascar."""
    ...
    def from_persisted(cls, data: Any, *, context: dict[str, Any] | None = None) -> LLM:
        ...
        payload = canonicalize_madagascar_llm_payload(payload)
    ...
    def subscription_login(
        ...
        **llm_kwargs,
        ...
        Example:
            ...
            ```
        """
        from madagascar.sdk.llm.auth.openai import subscription_login
