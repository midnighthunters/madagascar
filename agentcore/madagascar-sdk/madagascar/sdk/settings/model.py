...
from madagascar.sdk.conversation.request import SendMessageRequest
...
if TYPE_CHECKING:
    ...
    from madagascar.sdk.context.condenser import CondenserBase, LLMSummarizingCondenser
...
class SettingsFieldSchema(BaseModel):
    ...
    variant: str | None = Field(
        ...
        description=(
            ...
            "variant (``'madagascar'`` or ``'acp'``). The GUI filters fields by the "
...
class SettingsSectionSchema(BaseModel):
    ...
    variant: str | None = Field(
        ...
        description=(
            ...
            "variant (e.g. ``'madagascar'`` or ``'acp'``). The GUI filters sections by "
...
class AgentSettingsBase(BaseModel):
    ...
    either the :class:`MadagascarAgentSettings` or :class:`ACPAgentSettings`
    ...
    def create_agent(self) -> AgentBase:
        ...
        Subclasses (:class:`MadagascarAgentSettings`, :class:`ACPAgentSettings`)
        override this to return the appropriate
        :class:`~madagascar.sdk.agent.base.AgentBase` subclass.
...
def _migrate_agent_settings_v1_to_v2(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    Before the v1.19.0 ``LLMAgentSettings`` → ``MadagascarAgentSettings`` rename,
    persisted payloads carried ``agent_kind: 'llm'``. The two classes are
    field-compatible (``LLMAgentSettings`` is a subclass of
    ``MadagascarAgentSettings`` that only narrows the discriminator literal),
    ...
    ``MadagascarAgentSettings`` (the canonical class) accept legacy data
    without losing any fields.
    """
    migrated = dict(payload)
    migrated["schema_version"] = 2
    if migrated.get("agent_kind") == "llm":
        migrated["agent_kind"] = "madagascar"
    return migrated
...
def _migrate_agent_settings_v3_to_v4(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(llm, dict):
        migrated["llm"] = canonicalize_madagascar_llm_payload(llm)
...
class ConversationSettings(BaseModel):
    ...
    agent_settings: AgentSettingsConfig | None = Field(
        ...
        description=(
            ...
            "``MadagascarAgentSettings`` or ``ACPAgentSettings`` variant."
        ),
    )
...
AgentKind = Literal["madagascar", "llm", "acp"]
...
:data:`~madagascar.sdk.settings.acp_providers.ACP_PROVIDERS`.
"""


class MadagascarAgentSettings(AgentSettingsBase):
    ...
    agent_kind: Literal["madagascar"] = Field(
        default="madagascar",
        description=(
            "Discriminator for the ``AgentSettings`` union. ``'madagascar'`` selects "
            "the standard built-in Madagascar agent."
        ),
    )
    agent: str = Field(
    ...
    tools: list[Tool] | None = Field(
        ...
        description=(
            ...
            "standard exec set (see madagascar.sdk.tool.defaults), plus the "
    ...
    def create_agent(self) -> Agent:
        ...
        from madagascar.sdk.llm.auth.openai import create_subscription_llm_from_config
        from madagascar.sdk.tool.builtins import BUILT_IN_TOOLS, SwitchLLMTool
        from madagascar.sdk.tool.defaults import default_tool_specs
...
class ACPAgentSettings(AgentSettingsBase):
    ...
    :class:`MadagascarAgentSettings` do not apply here.
    ...
    mcp_config: dict[str, MCPServer] = Field(
        ...
        description=(
            ...
            "Madagascar agent — where these become in-process MCP tools — the "
    ...
    acp_isolate_data_dir: bool = Field(
        ...
        description=(
            ...
            ":attr:`~madagascar.sdk.agent.ACPAgent.acp_isolate_data_dir`; off by "
    ...
    def api_key_env_var(self) -> str | None:
        ...
        Delegates to the :data:`~madagascar.sdk.settings.acp_providers.ACP_PROVIDERS`
    ...
    def resolve_acp_command(self) -> list[str]:
        ...
        up the default from :data:`~madagascar.sdk.settings.acp_providers.ACP_PROVIDERS`.
...
class LLMAgentSettings(MadagascarAgentSettings):
    """Legacy ``agent_kind='llm'`` variant of :class:`MadagascarAgentSettings`.

    ``LLMAgentSettings`` was the public class name before the v1.19.0 rename.
    The public import aliases (``from madagascar.sdk import LLMAgentSettings`` and
    ``from madagascar.sdk.settings import LLMAgentSettings``) were removed in
    v1.24.0 — use :class:`MadagascarAgentSettings` for all new code.

    The class itself is retained (reachable at
    ``madagascar.sdk.settings.model.LLMAgentSettings``) because it remains a
    ...
    # to MadagascarAgentSettings ("madagascar").
    agent_kind: Literal["llm"] = Field(  # type: ignore[assignment]
        default="llm",
        description=(
            "Discriminator for the ``AgentSettings`` union. ``'llm'`` selects "
            "the standard LLM-backed agent. Deprecated; use ``'madagascar'``."
        ),
    )


def _agent_settings_discriminator(value: Any) -> str:
    """Discriminator for :data:`AgentSettingsConfig` — defaults to ``'madagascar'``.

    Existing persisted payloads predate ``agent_kind`` and carry only
    Madagascar-agent fields. Treating a missing discriminator as ``'madagascar'``
    ...
    """
    if isinstance(value, BaseModel):
        return getattr(value, "agent_kind", "madagascar")
    if isinstance(value, dict):
        return value.get("agent_kind", "madagascar")
    return "madagascar"


AgentSettingsConfig = Annotated[
    Annotated[MadagascarAgentSettings, Tag("madagascar")]
...
_AGENT_SETTINGS_ADAPTER: TypeAdapter[
    MadagascarAgentSettings | LLMAgentSettings | ACPAgentSettings
...
def validate_agent_settings(
    ...
    *,
    ...
    ``MadagascarAgentSettings`` rename.
    """
    if isinstance(data, MadagascarAgentSettings | ACPAgentSettings):
        return data
    payload = _apply_persisted_migrations(
        data,
    ...
    # always returns a ``{madagascar, acp}`` variant.
    if payload.get("agent_kind") == "llm":
        payload["agent_kind"] = "madagascar"
...
def apply_agent_settings_diff(
    ...
    *,
    context: Mapping[str, Any] | None = None,
) -> MadagascarAgentSettings | ACPAgentSettings:
    ...
    * When ``diff`` changes ``agent_kind``, start from a *fresh* base for the
      ...
      validation (ACP's nullable ``agent_context`` is invalid for Madagascar) or
...
def default_agent_settings() -> MadagascarAgentSettings:
    """Return a default :class:`MadagascarAgentSettings` instance.
    ...
    """
    return MadagascarAgentSettings()


def create_agent_from_settings(
    settings: MadagascarAgentSettings | ACPAgentSettings,
) -> AgentBase:
    """Dispatch to the variant's ``create_agent()`` method.

    Returns either :class:`~madagascar.sdk.agent.Agent` (LLM variant) or
    :class:`~madagascar.sdk.agent.ACPAgent` (ACP variant).
    """
...
def export_agent_settings_schema() -> SettingsSchema:
    ...
    carry a ``variant`` tag (``'madagascar'``, ``'acp'``, or ``None`` for
    shared) so the frontend can filter by the page's variant.
    """
    llm_schema = MadagascarAgentSettings.export_schema()
    ...
    _merge(llm_schema, default_variant="madagascar")
