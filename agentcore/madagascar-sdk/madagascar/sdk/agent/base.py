...
from madagascar.sdk.llm.utils.model_prompt_spec import get_model_prompt_spec
...
if TYPE_CHECKING:
    from madagascar.sdk.conversation import ConversationState, LocalConversation
...
# -- SOUL.md loader -------------------------------------------------------
# SOUL.md is the agent's identity file (~/.madagascar/SOUL.md).  When present
# it replaces the default identity in the system prompt.

_SOUL_PATH = os.path.join(os.path.expanduser("~"), ".madagascar", "SOUL.md")
_DEFAULT_SOUL = (
...
def _load_soul_md() -> str:
    """Load ``~/.madagascar/SOUL.md``, falling back to the built-in default."""
    try:
...
class AgentBase(DiscriminatedUnionMixin, ABC):
    """Abstract base class for Madagascar agents.
    ...
    system_prompt: str | None = Field(
        ...
        description=(
            ...
            "task).  Setting this will override Madagascar' built-in system "
    ...
    def supports_madagascar_tools(self) -> bool:
        """``True`` if Madagascar can inject tools into this agent.

        ``False`` for :class:`~madagascar.sdk.agent.acp_agent.ACPAgent` — the
    ...
    def supports_madagascar_mcp(self) -> bool:
        """``True`` if Madagascar can create in-process MCP tools for this agent.

        ``False`` for :class:`~madagascar.sdk.agent.acp_agent.ACPAgent` — ACP
    ...
    def supports_condenser(self) -> bool:
        """``True`` if Madagascar context condensing is supported for this agent.
    ...
    @property
    def agent_kind(self) -> Literal["madagascar", "acp"]:
