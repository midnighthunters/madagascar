...
The Agent Client Protocol (ACP) lets Madagascar power conversations using
...
the ACPAgent relays user messages and collects the response. Madagascar
...
from madagascar.sdk.llm import LLM, ImageContent, Message, MessageToolCall, TextContent
from madagascar.sdk.logger import get_logger
from madagascar.sdk.mcp.config import MCPServer
from madagascar.sdk.observability.laminar import maybe_init_laminar, observe
...
from madagascar.sdk.tool import Tool  # noqa: TC002
from madagascar.sdk.tool.builtins.finish import FinishAction, FinishObservation
...
def _mcp_config_to_acp_servers(
    ...
    """Translate Madagascar MCP servers into ACP MCP server objects.
...
class _MadagascarACPBridge:
    """Bridge between Madagascar and ACP that accumulates session updates.
...
class ACPAgent(AgentBase):
    ...
    _client: Any = PrivateAttr(default=None)  # _MadagascarACPBridge
    ...
    @property
    def supports_madagascar_tools(self) -> bool:
        """``False`` — the ACP server manages its own toolset."""
        return False

    @property
    def supports_madagascar_mcp(self) -> bool:
        """``False`` — Madagascar does not create in-process MCP tools here.
    ...
    def supports_runtime_model_switch(self) -> bool:
        ...
        See
        :meth:`~madagascar.sdk.conversation.impl.local_conversation.LocalConversation.switch_acp_model`.
        """
    ...
    def _acp_file_secret_dir(self, state: ConversationState, subdir: str) -> Path:
        ...
        if state.persistence_dir:
            ...
            root = Path(state.workspace.working_dir) / ".madagascar" / "acp" / subdir
    ...
    def _reset_client_for_turn(
        ...
        ``_MadagascarACPBridge.session_update`` as tool-call notifications
    ...
    def _finalize_successful_turn(
        ...
        # _MadagascarACPBridge.session_update: one early ``started`` event per
