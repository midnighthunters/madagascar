...
from madagascar.sdk.git.models import GitChange, GitDiff
from madagascar.sdk.logger import get_logger
from madagascar.sdk.mcp.config import MCPServer
from madagascar.sdk.settings import SecretsListResponse, SettingsResponse
...
if TYPE_CHECKING:
    ...
    from madagascar.sdk.settings.model import ACPAgentSettings, LLMAgentSettings
...
class RemoteWorkspace(RemoteWorkspaceMixin, BaseWorkspace):
    """Remote workspace implementation that connects to an Madagascar agent server.

    RemoteWorkspace provides access to a sandboxed environment running on a remote
    Madagascar agent server. This is the recommended approach for production deployments
    ...
    def default_conversation_tags(self) -> dict[str, str] | None:
        ...
        Subclasses (e.g., MadagascarCloudWorkspace) can override this to provide
    ...
    # settings endpoints. Subclasses like MadagascarCloudWorkspace may override
    ...
    def _fetch_agent_settings(
        ...
        (``MadagascarAgentSettings`` or ``ACPAgentSettings``).
        """
    ...
    def get_mcp_config(self) -> dict[str, MCPServer]:
        ...
        # Runtime MCP tools only exist on MadagascarAgentSettings, not ACPAgentSettings.
        if not isinstance(settings, MadagascarAgentSettings):
            return {}
    ...
    def clone_repos(
        ...
        'madagascar-cli' instead of 'repo_0'). Automatically fetches GitHub,
    ...
    def _convert_skill_data_to_skill(self, skill_data: dict[str, Any]) -> "Skill":
        ...
        if triggers:
            # Determine trigger type based on content (same logic as Madagascar)
    ...
    def load_skills_from_agent_server(
        ...
        Args:
            ...
            load_public: Load public skills from Madagascar/extensions repo.
            load_user: Load user skills from ~/.madagascar/skills/.
