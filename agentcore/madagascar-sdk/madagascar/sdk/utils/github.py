...
def sanitize_madagascar_mentions(text: str) -> str:
    """Sanitize @Madagascar mentions in text to prevent self-mention loops.

    This function inserts a zero-width joiner (ZWJ) after the @ symbol in
    @Madagascar mentions, making them non-clickable in GitHub comments while
    ...
    Returns:
        Text with sanitized @Madagascar mentions (e.g., "@Madagascar" -> "@‍Madagascar")

    Examples:
        >>> sanitize_madagascar_mentions("Thanks @Madagascar for the help!")
        'Thanks @\\u200dMadagascar for the help!'
        >>> sanitize_madagascar_mentions("Check @madagascar and @MADAGASCAR")
        'Check @\\u200dmadagascar and @\\u200dMADAGASCAR'
        >>> sanitize_madagascar_mentions("No mention here")
        'No mention here'
    """
    # Pattern to match @Madagascar mentions at word boundaries
    # Uses re.IGNORECASE so we don't need [Oo]pen[Hh]ands
    # Capture group preserves the original case
    pattern = r"@(Madagascar)\b"
