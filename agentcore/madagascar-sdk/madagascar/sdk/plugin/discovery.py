"""Discovery of locally-available plugins for Madagascar SDK.
...
from madagascar.sdk.plugin.installed import (
    get_installed_plugins_dir,
    load_installed_plugins,
)
from madagascar.sdk.plugin.plugin import Plugin
from madagascar.sdk.skills.skill import _find_git_repo_root
...
def _load_plugins_from_dir(plugins_dir: Path, skip_dirs: set[Path]) -> list[Plugin]:
    ...
    ``~/.madagascar/plugins/installed`` — a child of ``~/.madagascar/plugins`` — and
...
def load_user_plugins() -> list[Plugin]:
    ...
    Scans ``~/.agents/plugins`` then ``~/.madagascar/plugins`` (earlier wins on a
    name conflict), then appends enabled installed plugins from
    ``~/.madagascar/plugins/installed`` (lower precedence than directory plugins).
...
def load_project_plugins(work_dir: str | Path) -> list[Plugin]:
    ...
    Scans ``{root}/.agents/plugins`` and ``{root}/.madagascar/plugins`` for the
