# Madagascar Enterprise Usage Telemetry Service
...
5. [Implementation Plan](#5-implementation-plan)
   - 5.1 [Database Schema and Models (M1)](#51-database-schema-and-models-m1)
     - 5.1.1 [Madagascar - Database Migration](#511-madagascar---database-migration)
     - 5.1.2 [Madagascar - Model Tests](#512-madagascar---model-tests)
   - 5.2 [Metrics Collection Framework (M2)](#52-metrics-collection-framework-m2)
     - 5.2.1 [Madagascar - Core Collection Framework](#521-madagascar---core-collection-framework)
     - 5.2.2 [Madagascar - Example Collectors](#522-madagascar---example-collectors)
     - 5.2.3 [Madagascar - Framework Tests](#523-madagascar---framework-tests)
   - 5.3 [Collection and Upload Processors (M3)](#53-collection-and-upload-processors-m3)
     - 5.3.1 [Madagascar - Collection Processor](#531-madagascar---collection-processor)
     - 5.3.2 [Madagascar - Upload Processor](#532-madagascar---upload-processor)
     - 5.3.3 [Madagascar - Integration Tests](#533-madagascar---integration-tests)
   - 5.4 [License Warning API (M4)](#54-license-warning-api-m4)
     - 5.4.1 [Madagascar - License Status API](#541-madagascar---license-status-api)
     - 5.4.2 [Madagascar - API Integration](#542-madagascar---api-integration)
   - 5.5 [UI Warning Banner (M5)](#55-ui-warning-banner-m5)
     - 5.5.1 [Madagascar - UI Warning Banner](#551-madagascar---ui-warning-banner)
     - 5.5.2 [Madagascar - UI Integration](#552-madagascar---ui-integration)
   - 5.6 [Helm Chart Deployment Configuration (M6)](#56-helm-chart-deployment-configuration-m6)
     - 5.6.1 [Madagascar-Cloud - Cronjob Manifests](#561-madagascar-cloud---cronjob-manifests)
     - 5.6.2 [Madagascar-Cloud - Configuration Management](#562-madagascar-cloud---configuration-management)
   - 5.7 [Documentation and Enhanced Collectors (M7)](#57-documentation-and-enhanced-collectors-m7)
     - 5.7.1 [Madagascar - Advanced Collectors](#571-madagascar---advanced-collectors)
     - 5.7.2 [Madagascar - Monitoring and Testing](#572-madagascar---monitoring-and-testing)
     - 5.7.3 [Madagascar - Technical Documentation](#573-madagascar---technical-documentation)
...
When telemetry uploads fail for more than 4 days, users will see a prominent warning banner in the Madagascar Enterprise UI:

```
⚠️ Your Madagascar Enterprise license will expire in 30 days. Please contact support if this issue persists.
```
...
2. **Generates unique customer identifiers** using administrator contact information:
   - Customer email: Determined by the following priority order:
     1. `MADAGASCAR_ADMIN_EMAIL` environment variable (if set in helm values)
...
**Tier 1: Explicit Configuration** - The `MADAGASCAR_ADMIN_EMAIL` environment variable allows administrators to explicitly specify the contact email during deployment.
...
class TelemetryUploadProcessor(MaintenanceTaskProcessor):
    ...
    async def _get_admin_email(self) -> str | None:
        ...
        env_admin_email = os.getenv('MADAGASCAR_ADMIN_EMAIL')
        if env_admin_email:
...
async def get_license_status():
    ...
    if not _is_madagascar_enterprise():
    ...
    async with a_session_maker() as session:
        ...
        if not last_upload:
            ...
            return {
                "warn": True,
                "message": "Madagascar Enterprise license verification pending. Please ensure network connectivity."
            }
        ...
        if days_since_upload > 4:
            ...
            if oldest_unsent:
                ...
                if days_until_expiration <= 0:
                    message = "Your Madagascar Enterprise license has expired. Please contact support immediately."
                else:
                    message = f"Your Madagascar Enterprise license will expire in {days_until_expiration} days. Please contact support if this issue persists."

                return {"warn": True, "message": message}

        return {"warn": False, "message": ""}

def _is_madagascar_enterprise() -> bool:
...
The cronjob configurations will be deployed via the Madagascar-Cloud helm charts.
...
```yaml
# charts/madagascar/templates/telemetry-collection-cronjob.yaml
apiVersion: batch/v1
kind: CronJob
metadata:
  name: {{ include "madagascar.fullname" . }}-telemetry-collection
  labels:
    {{- include "madagascar.labels" . | nindent 4 }}
spec:
      ...
      template:
        spec:
          ...
          - name: telemetry-collector
            ...
            env:
            {{- include "madagascar.env" . | nindent 12 }}
            command:
            - python
            - -c
            - |
...
```yaml
# charts/madagascar/templates/telemetry-upload-cronjob.yaml
apiVersion: batch/v1
kind: CronJob
metadata:
  name: {{ include "madagascar.fullname" . }}-telemetry-upload
  labels:
    {{- include "madagascar.labels" . | nindent 4 }}
spec:
        ...
        spec:
          ...
          - name: telemetry-uploader
            ...
            - name: REPLICATED_PUBLISHABLE_KEY
              valueFrom:
                secretKeyRef:
                  name: {{ include "madagascar.fullname" . }}-replicated-config
                  key: publishable-key
            - name: REPLICATED_APP_SLUG
              value: {{ .Values.telemetry.replicatedAppSlug | default "madagascar-enterprise" | quote }}
            command:
            - python
            - -c
            - |
              ...
              async def main():
                  ...
                  processor = TelemetryUploadProcessor(
                      ...
                      replicated_app_slug=os.getenv('REPLICATED_APP_SLUG', 'madagascar-enterprise')
                  )
                  task = MaintenanceTask()
                  task.set_processor(processor)
                  task.status = MaintenanceTaskStatus.PENDING
...
#### 5.2.1 Madagascar - Core Collection Framework
...
#### 5.2.2 Madagascar - Example Collectors
...
#### 5.2.3 Madagascar - Framework Tests
...
#### 5.5.1 Madagascar - UI Warning Banner
...
#### 5.5.2 Madagascar - UI Integration
...
**Repository**: Madagascar-Cloud
Create Kubernetes cronjob configurations and deployment scripts.

#### 5.6.1 Madagascar-Cloud - Cronjob Manifests

- [ ] `charts/madagascar/templates/telemetry-collection-cronjob.yaml`
- [ ] `charts/madagascar/templates/telemetry-upload-cronjob.yaml`

#### 5.6.2 Madagascar-Cloud - Configuration Management

- [ ] `charts/madagascar/templates/replicated-secret.yaml`
- [ ] Update `charts/madagascar/values.yaml` with telemetry configuration options:
  ```yaml
  # Add to values.yaml
  telemetry:
    enabled: true
    replicatedAppSlug: "madagascar-enterprise"
    adminEmail: ""  # Optional: admin email for customer identification

  # Add to deployment environment variables
  env:
    MADAGASCAR_ADMIN_EMAIL: "{{ .Values.telemetry.adminEmail }}"
  ```
...
#### 5.7.1 Madagascar - Advanced Collectors
...
#### 5.7.2 Madagascar - Monitoring and Testing
...
#### 5.7.3 Madagascar - Technical Documentation
