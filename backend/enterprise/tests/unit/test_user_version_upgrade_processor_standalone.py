...
These tests are designed to work without the full Madagascar dependency chain.
They test the core logic and behavior of the processor using comprehensive mocking.

To run these tests in an environment with Madagascar dependencies:
1. Ensure Madagascar is available in the Python path
...
class TestUserVersionUpgradeProcessorStandalone:
    """Standalone tests for UserVersionUpgradeProcessor without Madagascar dependencies."""
...
class TestUserVersionUpgradeProcessorIntegration:
    """
    Integration test scenarios for when Madagascar dependencies are available.

    These tests would require:
    1. Madagascar to be installed and available
