...
4. Create the Madagascar user record (mirroring the ``keycloak_callback``
...
* ``madagascar_user_id`` — set once ``UserStore.create_user`` returns,
...
(user-first) would leak the *entire* Madagascar cascade instead of a
...
Madagascar ``OrgMember`` row does not propagate to LiteLLM, so on
...
async def provision_user(
    ...
    # have not mutated any Madagascar state yet, so a clean error is
    # safe to return.
    try:
    ...
    # Anything after this point may need to roll back partial Madagascar
    # state (DB rows, LiteLLM team, OrgMember rows) **and** the Keycloak
    # user, to avoid leaving an Madagascar ``User``/personal-``Org``
    ...
    madagascar_user_id: UUID | None = None
    ...
    try:
        ...
        # 3. Create the Madagascar user. Mirrors ``keycloak_callback``'s
        ...
        madagascar_user_id = new_user.id
        ...
        if target_org_id != madagascar_user_id:
            ...
            # Track separately from ``madagascar_user_id`` so the unwind
        ...
        await _rollback_partial_provision(
            ...
            madagascar_user_id=madagascar_user_id,
...
async def _rollback_partial_provision(
    *,
    ...
    3. **Keycloak user.** Last, so the local Madagascar identity is
    ...
    if madagascar_user_id is not None and target_membership_added:
        try:
            await OrgMemberStore.remove_user_from_org(target_org_id, madagascar_user_id)
        except Exception:
            logger.exception(
                ...
                extra={
                    'kc_user_id': kc_user_id,
                    'target_org_id': str(target_org_id),
                    'madagascar_user_id': str(madagascar_user_id),
                },
            )
    ...
    if madagascar_user_id is not None:
        try:
            await OrgStore.delete_org_cascade(
                madagascar_user_id, requester_user_id=kc_user_id
            )
        except Exception:
            logger.exception(
    ...
    # 3. Keycloak user. Always last; runs even when no Madagascar DB
