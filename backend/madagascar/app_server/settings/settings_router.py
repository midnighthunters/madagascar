"""Settings router for Madagascar App Server.
...
def _post_merge_llm_fixups(settings: Settings) -> None:
    ...
    rules to :func:`madagascar.app_server.utils.llm.resolve_llm_base_url` so the
    personal-save and enterprise org-defaults paths stay in lockstep.
    """
    if not isinstance(settings.agent_settings, MadagascarAgentSettings):
        return
...
async def load_settings(
    ...
    try:
        ...
        if is_madagascar_model(llm.model):
