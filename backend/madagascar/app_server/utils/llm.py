...
# ---------------------------------------------------------------------------
# The ``madagascar-sdk`` package is the **single source of truth** for which
# models are verified and how bare LiteLLM names map to providers.
#
# Self-hosted mode builds the ``madagascar/…`` model list from the SDK's
# ``VERIFIED_MADAGASCAR_MODELS``.  SaaS mode overrides it with the database
# (via ``get_madagascar_models``).
# ---------------------------------------------------------------------------
from madagascar.sdk.llm.utils.verified_models import (  # noqa: E402
    VERIFIED_ANTHROPIC_MODELS as _SDK_ANTHROPIC,
)
from madagascar.sdk.llm.utils.verified_models import (
...
from madagascar.sdk.llm.utils.verified_models import (
    VERIFIED_MADAGASCAR_MODELS as _SDK_MADAGASCAR,
)

# Build the ``madagascar/…`` model list from the SDK.
MADAGASCAR_MODELS: list[str] = [f'madagascar/{m}' for m in _SDK_MADAGASCAR]
...
DEFAULT_MADAGASCAR_MODEL = 'madagascar/minimax-m2.7'
...
class ModelsResponse(BaseModel):
    ...
    * ``verified_models`` — model names (without provider prefix) that
      Madagascar has verified to work well.
...
def is_madagascar_model(model: str | None) -> bool:
    """Return True when the model uses the public Madagascar provider prefix."""
    return bool(model and model.startswith('madagascar/'))
...
def resolve_llm_base_url(
    ...
    * ``madagascar/app_server/settings/settings_router._post_merge_llm_fixups``
    ...
    * ``base_url is None`` + known Madagascar / managed model → ``managed_proxy_url``.
...
def get_madagascar_models(
    verified_models: list[str] | None = None,
) -> list[str]:
    """Return the list of Madagascar-provider model strings.

    In self-hosted mode *verified_models* is ``None`` (or empty) and the
    hardcoded ``MADAGASCAR_MODELS`` list is used.  In SaaS mode the caller
    passes the database-backed list which takes precedence.

    Args:
        verified_models: Optional list of ``"madagascar/<name>"`` strings
            loaded from the verified-models database table.

    Returns:
        A list such as ``["madagascar/claude-opus-4-6", ...]``.
    """
    return verified_models if verified_models else MADAGASCAR_MODELS
...
def _derive_verified_models(madagascar_models: list[str]) -> list[str]:
    """Extract the bare model names from the ``madagascar/…`` model list."""
    return [
        m.removeprefix('madagascar/')
        for m in madagascar_models
        if m.startswith('madagascar/')
    ]
...
def get_supported_llm_models(
    ...
    * a list of *verified* model names (the Madagascar-curated subset),
    ...
    * the recommended default model.
    ...
    madagascar_models = get_madagascar_models(verified_models)

    # Assign canonical provider prefixes to bare LiteLLM names, then dedupe.
    all_models = (
        madagascar_models + CLARIFAI_MODELS + [_assign_provider(m) for m in model_list]
    )
    unique_models = sorted(set(all_models))

    return ModelsResponse(
        models=unique_models,
        verified_models=_derive_verified_models(madagascar_models),
        verified_providers=VERIFIED_PROVIDERS,
        default_model=DEFAULT_MADAGASCAR_MODEL,
    )
