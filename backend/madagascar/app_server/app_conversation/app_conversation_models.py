...
from madagascar.app_server.integrations.service_types import ProviderType, SuggestedTask
...
from madagascar.sdk.profiles import LaunchedAgentProfile
...
class AppConversationInfo(BaseModel):
    ...
    def acp_server(self) -> str | None:
        ...
        ``agent_kind`` so a stray tag never reports a provider for an Madagascar
        conversation.
        """
...
class AppConversationStartRequest(MadagascarModel):
...
class AppConversationStartTask(MadagascarModel):
    ...
    app_conversation_id: MadagascarUUID | None = Field(
...
class AppConversationStartTaskPage(MadagascarModel):
