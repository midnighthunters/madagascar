"""Sandboxed Conversation router for Madagascar App Server."""
...
from madagascar.agent_server.models import Success
...
from madagascar.app_server.sandbox.sandbox_spec_service import SandboxSpecService
from madagascar.app_server.services.db_session_injector import set_db_session_keep_open
from madagascar.app_server.services.httpx_client_injector import (
...
from madagascar.app_server.user_auth import get_user_settings
...
from madagascar.sdk.skills import KeywordTrigger, TaskTrigger
...
async def _get_agent_server_context(
    ...
    if not sandbox_spec:
        ...
        # sandbox spec versions when updating Madagascar. When the SandboxSpecServices
...
async def get_conversation_skills(
    ...
    - Global skills (Madagascar/skills/)
    - User skills (~/.madagascar/skills/)
    - Organization skills (org/.madagascar repository)
    - Repository skills (repo .agents/skills/, .madagascar/microagents/, and legacy .madagascar/skills/)
...
async def get_conversation_hooks(
    ...
    workspace (i.e. `{project_dir}/.madagascar/hooks.json`) at request time.

    Note:
        This is intentionally a "live" view of the workspace configuration.
        If `.madagascar/hooks.json` changes over time, this endpoint reflects the
