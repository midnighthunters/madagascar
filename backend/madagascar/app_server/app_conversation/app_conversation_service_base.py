...
from madagascar.app_server.sandbox.sandbox_models import SandboxInfo
...
from madagascar.app_server.utils.auth import looks_like_jwt
from madagascar.app_server.utils.git import ensure_valid_git_branch_name
from madagascar.sdk import Agent, LLMSummarizingCondenser
...
def get_project_dir(
    ...
    `.madagascar/` configuration (setup.sh, pre-commit.sh, skills/, etc.).
    ...
    - .madagascar/setup.sh execution
    - .madagascar/pre-commit.sh (git hooks setup)
    - .madagascar/skills/ (project skills)
    - PLAN.md path

    Args:
        working_dir: Base working directory path in the sandbox
            (e.g., '/workspace/project' from sandbox_spec)
        selected_repository: Repository name (e.g., 'Madagascar/software-agent-sdk')
...
class AppConversationServiceBase(AppConversationService, ABC):
    ...
    async def load_and_merge_all_skills(
        ...
        - Public skills (from Madagascar/skills GitHub repo)
        - User skills (from ~/.madagascar/skills/)
        - Organization skills (from {org}/.madagascar repo)
        - Project/repo skills (from repo .agents/skills/, .madagascar/microagents/, and legacy .madagascar/skills/)
    ...
    async def run_setup_scripts(
        ...
        # for all .madagascar/ features (setup.sh, pre-commit.sh, skills).
    ...
    async def clone_or_init_git_repo(
        ...
        if request.selected_branch:
            ...
            madagascar_workspace_branch = f'madagascar-workspace-{random_str}'
            checkout_command = (
                f'git checkout -b {shlex.quote(madagascar_workspace_branch)}'
            )
    ...
    async def _configure_azure_devops_git_credential_helper(
        ...
        helper_path = git_dir / '.git' / 'madagascar-azure-devops-credential-helper'
        secret_path = (
...
"""
    ...
    async def maybe_setup_git_hooks(
        ...
        """Set up git hooks if .madagascar/pre-commit.sh exists in the project root.

        Args:
            workspace: Remote workspace for command execution.
            project_dir: Project root directory (repo root when a repo is selected).
        """
        command = 'mkdir -p .git/hooks && chmod +x .madagascar/pre-commit.sh'
        ...
        with tempfile.TemporaryFile(mode='w+t') as temp_file:
            ...
            if download_result.success:
                ...
                if 'This hook was installed by Madagascar' not in temp_file.read():
