...
from madagascar.app_server.sandbox.docker_sandbox_service import DockerSandboxService
from madagascar.app_server.sandbox.sandbox_models import (
    AGENT_SERVER,
    SandboxInfo,
    SandboxStatus,
)
from madagascar.app_server.sandbox.sandbox_service import SandboxService
...
from madagascar.app_server.settings.llm_profiles import resolve_profile_llm
...
from madagascar.app_server.utils.git import ensure_valid_git_branch_name
from madagascar.app_server.utils.import_utils import get_impl
from madagascar.app_server.utils.llm_metadata import (
    get_llm_metadata,
    should_set_litellm_extra_body,
)
from madagascar.app_server.utils.redis_lock import (
    LockError,
    RedisLockUnavailable,
    refresh_lock_periodically,
    try_acquire_redis_lock,
)
from madagascar.sdk import Agent, AgentContext, LocalWorkspace
from madagascar.sdk.hooks import HookConfig
from madagascar.sdk.llm import LLM
from madagascar.sdk.llm.llm_profile_store import PROFILE_NAME_REGEX
...
from madagascar.sdk.subagent import get_registered_agent_definitions
...
from madagascar.tools.preset.planning import (
...
def _expected_sdk_version() -> str | None:
    """App's pinned madagascar-sdk version, or None if its metadata is unresolvable."""
    try:
        return importlib.metadata.version('madagascar-sdk')
...
class LiveStatusAppConversationService(AppConversationServiceBase):
    ...
    web_url: str | None
    madagascar_provider_base_url: str | None
    ...
    async def _start_app_conversation(
        ...
        try:
            ...
            # ``madagascar-sdk`` release does not yet expose ``user_id`` on
            ...
            # the local SDK model knows about it. Remove this once Madagascar
            ...
            try:
                ...
                # A custom image that 500s on create is usually an madagascar-sdk
                ...
                if is_custom_sandbox_spec(sandbox.sandbox_spec_id):
                    ...
                    raise SandboxError(
                        ...
                        f'{sandbox.sandbox_spec_id}. Verify its madagascar-sdk '
    ...
    def _configure_llm(self, user: UserInfo, llm_model: str | None) -> LLM:
        ...
        base_url = resolve_provider_llm_base_url(
            ...
            provider_base_url=self.madagascar_provider_base_url,
        )
    ...
    async def _add_system_mcp_servers(
        ...
        """Add system-generated MCP servers (default Madagascar server).
        ...
        Args:
            ...
            conversation_id: Conversation ID forwarded to the Madagascar MCP server
        """
        if not self.web_url:
            return

        headers = {'X-Madagascar-ServerConversation-ID': SecretStr(str(conversation_id))}
        ...
        # Add default Madagascar MCP server (includes Tavily proxy if configured)
    ...
    def _merge_custom_mcp_config(
        ...
        Handles both Madagascar and ACP agent settings: a resolved ACP profile's
    ...
    def _build_observability_context(
        ...
        *,
        ...
        tags = ['app:madagascar', f'agent_kind:{agent_kind}']
    ...
    def _apply_server_agent_overrides(
        ...
        * LLM tracing metadata for SaaS analytics.
        ...
        # LLM tracing metadata for madagascar/ models
    ...
    async def _load_hooks_from_workspace(
        ...
        """Load hooks from .madagascar/hooks.json in the remote workspace.

        This enables project-level hooks to be automatically loaded when starting
        a conversation, similar to how Madagascar-CLI loads hooks from the workspace.
    ...
    async def _build_observability_metadata(
        ...
        Shared by the Madagascar and ACP request builders. The commit is the
    ...
    async def _resolve_registered_marketplaces(
        ...
        if not marketplace_plugin_loading_enabled():
            return None
        try:
            from madagascar.app_server.shared import SettingsStoreImpl
    ...
    async def _build_acp_start_conversation_request(
        ...
        # subprocess (#15044 §7). Only custom servers — the system Madagascar MCP
    ...
    async def _process_pending_messages(
        ...
        # The frontend uses task-{uuid.hex} format (no hyphens), matching MadagascarUUID serialization
...
class LiveStatusAppConversationServiceInjector(AppConversationServiceInjector):
    ...
    async def inject(
        ...
        async with (
            ...
            try:
                from madagascar.app_server.shared import server_config
            ...
            yield LiveStatusAppConversationService(
                ...
                web_url=web_url,
                madagascar_provider_base_url=config.madagascar_provider_base_url,
