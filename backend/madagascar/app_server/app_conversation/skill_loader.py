...
async def _determine_org_repo_path(
    ...
    Examples:
        - GitHub/Bitbucket: ('owner/.madagascar', 'owner')
        - GitLab: ('owner/madagascar-config', 'owner')
        - Azure DevOps: ('org/madagascar-config/madagascar-config', 'org')
    """
    ...
    if is_gitlab:
        org_madagascar_repo = f'{org_name}/madagascar-config'
    elif is_azure_devops:
        org_madagascar_repo = f'{org_name}/madagascar-config/madagascar-config'
    else:
        org_madagascar_repo = f'{org_name}/.madagascar'

    return org_madagascar_repo, org_name


async def _get_org_repository_url(
    org_madagascar_repo: str, user_context: UserContext
) -> str | None:
    """Get authenticated Git URL for organization repository.

    Args:
        org_madagascar_repo: Organization repository path
    ...
    try:
        remote_url = await user_context.get_authenticated_git_url(
            org_madagascar_repo, is_optional=True
        )
        return remote_url
    except AuthenticationError as e:
        _logger.debug(
            f'org-level skill directory {org_madagascar_repo} not found: {str(e)}'
        )
        return None
    except Exception as e:
        _logger.debug(
            f'Failed to get authenticated URL for {org_madagascar_repo}: {str(e)}'
        )
        return None
...
def _candidate_repo_paths(provider: ProviderType, owner: str) -> list[str]:
    ...
    GitHub-style providers expose two independent repos (``.madagascar`` and
    ``.agents``) that are loaded concurrently. GitLab and Azure DevOps use a
    single ``madagascar-config`` repository (they have no ``.agents`` analog).
    ...
    Returns:
        List of repository paths (e.g., ['owner/.madagascar', 'owner/.agents']).
    """
    if provider == ProviderType.GITLAB:
        return [f'{owner}/madagascar-config']
    if provider == ProviderType.AZURE_DEVOPS:
        return [f'{owner}/madagascar-config/madagascar-config']
    return [f'{owner}/.madagascar', f'{owner}/.agents']
...
async def build_org_configs(
    ...
    resolving both ``.madagascar`` and ``.agents`` repos for GitHub-style
    ...
    if selected_repository and len(selected_repository.split('/')) >= 2:
        try:
            org_madagascar_repo, org_name = await _determine_org_repo_path(
                selected_repository, user_context
            )
            selected_provider = await _get_provider_type(
                selected_repository, user_context
            )
            candidates.append((org_madagascar_repo, org_name, selected_provider))
