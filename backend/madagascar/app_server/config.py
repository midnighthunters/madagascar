"""Configuration for the Madagascar App Server."""
...
import madagascar.app_server.event_callback  # noqa: F401
from madagascar.agent_server.env_parser import from_env
from madagascar.app_server.app_conversation.app_conversation_info_service import (
...
from madagascar.app_server.app_conversation.app_conversation_start_task_service import (
...
from madagascar.app_server.app_lifespan.oss_app_lifespan_service import (
...
from madagascar.app_server.event.event_service import EventService, EventServiceInjector
from madagascar.app_server.event_callback.event_callback_service import (
...
from madagascar.app_server.sandbox.sandbox_service import (
    SandboxService,
...
from madagascar.app_server.services.db_session import (  # noqa: F401  (re-exported)
    depends_db_session,
    get_db_session,
)
from madagascar.app_server.services.db_session_injector import (
    DbSessionInjector,
)
from madagascar.app_server.services.httpx_client_injector import HttpxClientInjector
from madagascar.app_server.services.injector import InjectorState
from madagascar.app_server.services.jwt_service import JwtService, JwtServiceInjector
from madagascar.app_server.user.user_context import UserContext, UserContextInjector
...
from madagascar.app_server.web_client.web_client_config_injector import (
...
def get_madagascar_provider_base_url() -> str | None:
    """Return the base URL for the Madagascar provider, if configured.

    Falls back to LLM_BASE_URL for backward compatibility.
    """
    return os.getenv('MADAGASCAR_PROVIDER_BASE_URL') or os.getenv('LLM_BASE_URL') or None
...
# Madagascar provider models use this proxy at the SDK transport boundary.
# Deployments (e.g. staging) may use a different LLM proxy, configured via
# MADAGASCAR_PROVIDER_BASE_URL.
...
def resolve_provider_llm_base_url(
    ...
    When the model uses the public ``madagascar/`` prefix and the stored
    ...
    Args:
        model: LLM model name (e.g. ``madagascar/gpt-5.5``).
        base_url: The base URL from user/org settings.
        provider_base_url: Deployment provider URL.  Falls back to
            ``get_madagascar_provider_base_url()`` when *None*.
    """
    if not model or not model.startswith('madagascar/'):
        return base_url
    ...
    if provider_base_url is None:
        provider_base_url = get_madagascar_provider_base_url()
...
def _get_default_lifespan():
    ...
    if 'saas' in (os.getenv('MADAGASCAR_CONFIG_CLS') or '').lower():
...
class AppServerConfig(MadagascarModel):
    ...
    web_url: str | None = Field(
        ...
        description='The URL where Madagascar is running (e.g., http://localhost:3000)',
    )
    ...
    madagascar_provider_base_url: str | None = Field(
        default_factory=get_madagascar_provider_base_url,
        description='Base URL for the Madagascar provider',
    )
    ...
    app_mode: AppMode = AppMode.MADAGASCAR
...
def config_from_env() -> AppServerConfig:
    # Import defaults...
    from madagascar.app_server.app_conversation.live_status_app_conversation_service import (  # noqa: E501
        LiveStatusAppConversationServiceInjector,
    )
    from madagascar.app_server.app_conversation.sql_app_conversation_info_service import (  # noqa: E501
        SQLAppConversationInfoServiceInjector,
    )
    from madagascar.app_server.app_conversation.sql_app_conversation_start_task_service import (  # noqa: E501
    ...
    from madagascar.app_server.event.filesystem_event_service import (
        FilesystemEventServiceInjector,
    )
    from madagascar.app_server.event.google_cloud_event_service import (
    ...
    from madagascar.app_server.sandbox.process_sandbox_service import (
    ...
    from madagascar.app_server.user.auth_user_context import (
