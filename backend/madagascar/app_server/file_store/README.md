# Madagascar FileStore Module

The file store module provides different storage backends for file operations in Madagascar. This module implements a common interface (`FileStore`) that allows for interchangeable storage backends.
...
```python
from madagascar.app_server.file_store import get_file_store, LocalFileStore
...
To configure the file store in Madagascar, use the following configuration options:
