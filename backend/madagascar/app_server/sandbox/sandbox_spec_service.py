...
from madagascar.agent_server import env_parser
from madagascar.app_server.errors import SandboxError
from madagascar.app_server.sandbox.sandbox_spec_models import (
    SandboxSpecInfo,
    SandboxSpecInfoPage,
)
...
_DEFAULT_REPOSITORY = 'ghcr.io/madagascar/agent-server'


@cache
def _bundled_agent_server_version() -> str:
    """Version of the installed madagascar-agent-server package.

    madagascar-agent-server is a hard runtime dependency of the V1 app server;
    ...
    """
    return importlib.metadata.version('madagascar-agent-server')
    ...
    if repository == _DEFAULT_REPOSITORY:
        ...
        if len(parts) == 2 and parts[0] != bundled_version:
            ...
            logging.getLogger(__name__).warning(
                ...
                f'madagascar-sdk (using {updated_tag} instead). Pin to a custom '
