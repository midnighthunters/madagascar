...
from madagascar.app_server.sandbox.docker_sandbox_spec_service import get_docker_client
...
class DockerSandboxService(SandboxService):
    ...
    async def start_sandbox(
        ...
        # frontend when running Madagascar on a remote machine.
    ...
    async def delete_sandbox(self, sandbox_id: str) -> bool:
        ...
        try:
            ...
            try:
                volume_name = f'madagascar-workspace-{sandbox_id}'
...
class DockerSandboxServiceInjector(SandboxServiceInjector):
    ...
    host_port: int = Field(
        default=3000,
        description=(
            'The port on which the main Madagascar app server is running. '
            'Used for webhook callbacks from agent-server containers. '
            'If running Madagascar on a non-default port, set this to match. '
