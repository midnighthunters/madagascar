...
from madagascar.app_server.integrations.service_types import GitService, ProviderType
...
from madagascar.app_server.user_auth import (
    get_access_token,
    get_provider_tokens,
    get_user_id,
)
...
async def get_conversation_link(
    ...
    """Appends a followup link, in the PR body, to the Madagascar conversation that opened the PR"""
...
async def create_pr(
    ...
    """Open a PR in GitHub"""
    logger.info('Calling Madagascar MCP create_pr')

    request = get_http_request()
    headers = request.headers
    conversation_id = headers.get('X-Madagascar-ServerConversation-ID', None)
...
async def create_mr(
    ...
    """Open a MR in GitLab"""
    logger.info('Calling Madagascar MCP create_mr')
...
async def create_bitbucket_pr(
    ...
    logger.info('Calling Madagascar MCP create_bitbucket_pr')
...
async def create_bitbucket_data_center_pr(
    ...
    logger.info('Calling Madagascar MCP create_bitbucket_data_center_pr')
...
async def create_azure_devops_pr(
    ...
    logger.info('Calling Madagascar MCP create_azure_devops_pr')
