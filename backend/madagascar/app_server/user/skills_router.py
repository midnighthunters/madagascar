...
from madagascar.app_server.config import depends_user_context
...
# skills/ is at the repo root, two levels above the madagascar package __file__
GLOBAL_SKILLS_DIR = Path(madagascar.__file__).parent.parent / 'skills'
USER_SKILLS_DIR = Path.home() / '.madagascar' / 'microagents'
...
async def _clone_marketplace_repo(
    ...
    try:
        clone_dir = Path(
            tempfile.mkdtemp(prefix=f'madagascar_marketplace_{marketplace.name}_')
        )
